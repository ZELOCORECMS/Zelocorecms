<?php
/**
 * Error Protection API: ZC_Recovery_Mode_Link_Handler class
 *
 * @package ZelocoreCMS
 * @since 5.2.0
 */
...
class ZC_Recovery_Mode_Link_Service {
	...
	/**
	 * ZC_Recovery_Mode_Link_Service constructor.
	 *
	 * @since 5.2.0
	 *
	 * @param ZC_Recovery_Mode_Cookie_Service $cookie_service Service to handle setting the recovery mode cookie.
	 * @param ZC_Recovery_Mode_Key_Service    $key_service    Service to handle generating recovery mode keys.
	 */
	public function __construct( ZC_Recovery_Mode_Cookie_Service $cookie_service, ZC_Recovery_Mode_Key_Service $key_service ) {
	...
	/**
	 * Enters recovery mode when the user hits zc-login.php with a valid recovery mode link.
	 *
	 * @since 5.2.0
	 *
	...
	public function handle_begin_link( $ttl ) {
		...
		$url = add_query_arg( 'action', self::LOGIN_ACTION_ENTERED, zc_login_url() );
		zc_redirect( $url );
		die;
	}
	 ...
	 * @since 5.2.0
	 *
	 * @param string $token Recovery Mode token created by {@see ZC_Recovery_Mode_Key_Service::generate_recovery_mode_token()}.
	 * @param string $key   Recovery Mode key created by {@see ZC_Recovery_Mode_Key_Service::generate_and_store_recovery_mode_key()}.
