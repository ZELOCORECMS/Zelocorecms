...
class ZC_Block {
	 ...
	 * @param ZC_Block_Type_Registry $registry          Optional block type registry.
	 */
	 ...
	 *
	 * If the block has inner blocks, the method recursively processes them by creating new instances of `ZC_Block`
	 ...
	 *
	 * If the block has inner blocks, this method initializes a new `ZC_Block_List` for them, ensuring the
	...
	public function refresh_parsed_block_dependents() {
		if ( ! empty( $this->parsed_block['innerBlocks'] ) ) {
			...
			$this->inner_blocks = new ZC_Block_List( $this->parsed_block['innerBlocks'], $child_context, $this->registry );
		}
	...
	private function replace_html( string $block_content, string $attribute_name, $source_value ) {
		...
		switch ( $block_type->attributes[ $attribute_name ]['source'] ) {
			...
			case 'rich-text':
				...
				foreach ( $selectors as $selector ) {
					...
					if ( strcasecmp( $block_reader->get_tag(), $selector ) === 0 || $block_reader->next_tag(
						...
						// TODO: Use `ZC_HTML_Processor::set_inner_html` method once it's available.
						$block_reader->release_bookmark( 'iterate-selectors' );
						$block_reader->replace_rich_text( zc_kses_post( $source_value ) );
			...
			case 'attribute':
				$amended_content = new ZC_HTML_Tag_Processor( $block_content );
	...
	private static function get_block_bindings_processor( string $block_content ) {
		$internal_processor_class = new class('', ZC_HTML_Processor::CONSTRUCTOR_UNLOCK_CODE) extends ZC_HTML_Processor {
			/**
			...
			public function replace_rich_text( $rich_text ) {
				...
				$this->set_bookmark( '_zc_block_bindings' );
				// The bookmark names are prefixed with `_` so the key below has an extra `_`.
				$tag_opener = $this->bookmarks['__zc_block_bindings'];
				...
				$this->set_bookmark( '_zc_block_bindings' );
				$tag_closer = $this->bookmarks['__zc_block_bindings'];
	...
	public function render( $options = array() ) {
		...
		$before_zc_enqueue_scripts_count = did_action( 'zc_enqueue_scripts' );

		// Capture the current assets queues.
		$before_styles_queue         = zc_styles()->queue;
		$before_scripts_queue        = zc_scripts()->queue;
		$before_script_modules_queue = zc_script_modules()->get_queue();
		...
		if ( $is_dynamic ) {
			$global_post = $post;
			$parent      = ZC_Block_Supports::$block_to_render;

			ZC_Block_Supports::$block_to_render = $this->parsed_block;

			$block_content = (string) call_user_func( $this->block_type->render_callback, $this->attributes, $block_content, $this );

			ZC_Block_Supports::$block_to_render = $parent;
		...
		if ( ! empty( $this->block_type->view_script_handles ) ) {
			foreach ( $this->block_type->view_script_handles as $view_script_handle ) {
				zc_enqueue_script( $view_script_handle );
			}
		}

		if ( ! empty( $this->block_type->view_script_module_ids ) ) {
			foreach ( $this->block_type->view_script_module_ids as $view_script_module_id ) {
				zc_enqueue_script_module( $view_script_module_id );
			}
		}

		/*
		 * For Core blocks, these styles are only enqueued if `zc_should_load_separate_core_block_assets()` returns
		 * true. Otherwise these `zc_enqueue_style()` calls will not have any effect, as the Core blocks are relying on
		 * the combined 'zc-block-library' stylesheet instead, which is unconditionally enqueued.
		 */
		...
		if ( ( ! empty( $this->block_type->view_style_handles ) ) ) {
			foreach ( $this->block_type->view_style_handles as $view_style_handle ) {
				zc_enqueue_style( $view_style_handle );
			}
		}
		 ...
		 * @param ZC_Block $instance      The block instance.
		 */
		...
		if ( $root_interactive_block === $this ) {
			...
			$block_content          = zc_interactivity_process_directives( $block_content );
			$root_interactive_block = null;
		}

		// Capture the new assets enqueued during rendering, and restore the queues the state prior to rendering.
		$after_styles_queue         = zc_styles()->queue;
		$after_scripts_queue        = zc_scripts()->queue;
		$after_script_modules_queue = zc_script_modules()->get_queue();

		/*
		 * As a very special case, a dynamic block may in fact include a call to zc_head() (and thus zc_enqueue_scripts()),
		 * in which all of its enqueued assets are targeting zc_footer. In this case, nothing would be printed, but this
		 * shouldn't indicate that the just-enqueued assets should be dequeued due to it being an empty block.
		 */
		$just_did_zc_enqueue_scripts = ( did_action( 'zc_enqueue_scripts' ) !== $before_zc_enqueue_scripts_count );
		...
		// Dequeue the newly enqueued assets with the existing assets if the rendered block was empty & zc_enqueue_scripts did not fire.
		if (
			! $just_did_zc_enqueue_scripts &&
