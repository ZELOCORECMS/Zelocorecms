 ...
 * form into the sidebar and also by the search widget in ZelocoreCMS.
 *
...
function get_search_form( $args = array() ) {
	 ...
	 * @since 5.5.0 The `$args` parameter was added.
	 *
	 * @link https://core.trac.zelocorecms.org/ticket/19321
	 *
 ...
 */
function zc_loginout( $redirect = '', $display = true ) {
	if ( ! is_user_logged_in() ) {
		$link = '<a href="' . esc_url( zc_login_url( $redirect ) ) . '">' . __( 'Log in' ) . '</a>';
	} else {
		$link = '<a href="' . esc_url( zc_logout_url( $redirect ) ) . '">' . __( 'Log out' ) . '</a>';
	}
	...
	if ( $display ) {
		...
		/** This filter is documented in zc-includes/general-template.php */
 ...
 * @return string The logout URL. Note: HTML-encoded via esc_html() in zc_nonce_url().
 */
function zc_logout_url( $redirect = '' ) {
	$args = array();
	if ( ! empty( $redirect ) ) {
		$args['redirect_to'] = urlencode( $redirect );
	}

	$logout_url = add_query_arg( $args, site_url( 'zc-login.php?action=logout', 'login' ) );
	$logout_url = zc_nonce_url( $logout_url, 'log-out' );
 ...
 */
function zc_login_url( $redirect = '', $force_reauth = false ) {
	$login_url = site_url( 'zc-login.php', 'login' );
...
function zc_registration_url() {
	 ...
	 */
	return apply_filters( 'register_url', site_url( 'zc-login.php?action=register', 'login' ) );
}

/**
 * Provides a simple login form for use anywhere within ZelocoreCMS.
 *
 ...
 *     @type string $id_submit         ID attribute value for the submit button. Default 'zc-submit'.
 ...
 */
function zc_login_form( $args = array() ) {
	$defaults = array(
		'echo'              => true,
	 ...
	 */
	$args = zc_parse_args( $args, apply_filters( 'login_form_defaults', $defaults ) );
	...
	$form =
		sprintf(
			...
			esc_url( site_url( 'zc-login.php', 'login_post' ) )
		) .
		$login_form_top .
		sprintf(
		...
		sprintf(
			'<p class="login-submit">
				<input type="submit" name="zc-submit" id="%1$s" class="button button-primary" value="%2$s" />
 ...
 */
function zc_lostpassword_url( $redirect = '' ) {
	$args = array(
		'action' => 'lostpassword',
	);
	...
	if ( is_multisite() ) {
		...
		$zc_login_path = $blog_details->path . 'zc-login.php';
	} else {
		$zc_login_path = 'zc-login.php';
	}

	$lostpassword_url = add_query_arg( $args, network_site_url( $zc_login_path, 'login' ) );
 ...
 *                     if `$display` is false.
 */
function zc_register( $before = '<li>', $after = '</li>', $display = true ) {
	if ( ! is_user_logged_in() ) {
		if ( get_option( 'users_can_register' ) ) {
			$link = $before . '<a href="' . esc_url( zc_registration_url() ) . '">' . __( 'Register' ) . '</a>' . $after;
		} else {
			$link = '';
		}
...
/**
 * Theme container function for the 'zc_meta' action.
 *
 * The {@see 'zc_meta'} action can have several purposes, depending on how you use it,
 * but one purpose might have been to allow for theme switching.
 *
 * @since 1.5.0
 *
 * @link https://core.trac.zelocorecms.org/ticket/1458 Explanation of 'zc_meta' action.
 */
function zc_meta() {
	/**
 ...
 * - 'wpurl' - The ZelocoreCMS address (URL) (set in Settings > General)
 * - 'url' - The Site address (URL) (set in Settings > General)
 * - 'admin_email' - Admin email (set in Settings > General)
 * - 'charset' - The "Encoding for pages and feeds"  (set in Settings > Reading)
 * - 'version' - The current ZelocoreCMS version
...
function get_site_icon_url( $size = 512, $url = '', $blog_id = 0 ) {
	...
	if ( $site_icon_id ) {
		...
		$url = zc_get_attachment_image_url( $site_icon_id, $size_data );
	}
...
function has_custom_logo( $blog_id = 0 ) {
	...
	$is_image       = ( $custom_logo_id ) ? zc_attachment_is_image( $custom_logo_id ) : false;
...
function get_custom_logo( $blog_id = 0 ) {
	...
	if ( has_custom_logo() ) {
		...
		if ( $unlink_homepage_logo && is_front_page() && ! is_paged() ) {
			 ...
			 * to the attributes used by zc_get_attachment_image().
			 */
			$image_alt = get_post_meta( $custom_logo_id, '_zc_attachment_image_alt', true );
			if ( empty( $image_alt ) ) {
		 ...
		 * because zc_get_attachment_image() already adds the alt attribute.
		 */
		$image = zc_get_attachment_image( $custom_logo_id, 'full', false, $custom_logo_attr );
 ...
 */
function zc_get_document_title() {

	/**
	 * Filters the document title before it is generated.
	 *
	 * Passing a non-empty value will short-circuit zc_get_document_title(),
 ...
 *
 * @since 4.1.0
 * @since 4.4.0 Improved title output replaced `zc_title()`.
 * @access private
 */
function _zc_render_title_tag() {
	if ( ! current_theme_supports( 'title-tag' ) ) {
		return;
	}

	echo '<title>' . zc_get_document_title() . '</title>' . "\n";
}
 ...
 */
function zc_title( $sep = '&raquo;', $display = true, $seplocation = '' ) {
	global $zc_locale;
	...
	$t_sep = '%ZC_TITLE_SEP%'; // Temporary separator, for accurate flipping, if necessary.
	...
	if ( is_archive() && ! empty( $m ) ) {
		...
		$title    = $my_year .
			( $my_month ? $t_sep . $zc_locale->get_month( $my_month ) : '' ) .
			( $my_day ? $t_sep . $my_day : '' );
	}
	...
	if ( is_archive() && ! empty( $year ) ) {
		$title = $year;
		if ( ! empty( $monthnum ) ) {
			$title .= $t_sep . $zc_locale->get_month( $monthnum );
		}
		if ( ! empty( $day ) ) {
	 ...
	 */
	$title_array = apply_filters( 'zc_title_parts', $title_array );
	 ...
	 */
	$title = apply_filters( 'zc_title', $title, $sep, $seplocation );
...
function single_post_title( $prefix = '', $display = true ) {
	 ...
	 * @param ZC_Post $_post       The current post.
	 */
...
function single_month_title( $prefix = '', $display = true ) {
	...
	if ( ! empty( $monthnum ) && ! empty( $year ) ) {
		$my_year  = $year;
		$my_month = $zc_locale->get_month( $monthnum );
	} elseif ( ! empty( $m ) ) {
		$my_year  = substr( $m, 0, 4 );
		$my_month = $zc_locale->get_month( substr( $m, 4, 2 ) );
	}
...
function get_the_post_type_description() {
	 ...
	 * @param ZC_Post_Type $post_type_obj The post type object.
	 */
...
function zc_get_archives( $args = '' ) {
	 ...
	 */
	$args = apply_filters( 'zc_get_archives_args', $args );
	...
	if ( 'monthly' === $parsed_args['type'] ) {
		...
		if ( $results ) {
			...
			foreach ( (array) $results as $result ) {
				...
				$text = sprintf( __( '%1$s %2$d' ), $zc_locale->get_month( $result->month ), $result->year );
...
function get_calendar( $args = array() ) {
	 ...
	 * }
	 */
	$args = apply_filters( 'get_calendar_args', zc_parse_args( $args, $defaults ) );
	...
	zc_recursive_ksort( $cache_args );
	$key   = md5( serialize( $cache_args ) );
	$cache = zc_cache_get( 'get_calendar', 'calendar' );
	...
	if ( ! $posts ) {
		...
		if ( ! $gotsome ) {
			$cache[ $key ] = '';
			zc_cache_set( 'get_calendar', $cache, 'calendar' );
			return;
		}
	}
	...
	$calendar_output  = '<table id="zc-calendar" class="zc-calendar-table">
	<caption>' . sprintf(
		$calendar_caption,
		$zc_locale->get_month( $thismonth ),
		gmdate( 'Y', $unixmonth )
	) . '</caption>
	<thead>
	<tr>';
	...
	for ( $wdcount = 0; $wdcount <= 6; $wdcount++ ) {
		$myweek[] = $zc_locale->get_weekday( ( $wdcount + $week_begins ) % 7 );
	}

	foreach ( $myweek as $wd ) {
		$day_name         = $args['initial'] ? $zc_locale->get_weekday_initial( $wd ) : $zc_locale->get_weekday_abbrev( $wd );
		$wd               = esc_attr( $wd );
	...
	$calendar_output .= '<nav aria-label="' . __( 'Previous and next months' ) . '" class="zc-calendar-nav">';

	if ( $previous ) {
		$calendar_output .= "\n\t\t" . sprintf(
			'<span class="zc-calendar-nav-prev"><a href="%1$s">&laquo; %2$s</a></span>',
			get_month_link( $previous->year, $previous->month ),
			$zc_locale->get_month_abbrev( $zc_locale->get_month( $previous->month ) )
		);
	} else {
		$calendar_output .= "\n\t\t" . '<span class="zc-calendar-nav-prev">&nbsp;</span>';
	}
	...
	if ( $next ) {
		$calendar_output .= "\n\t\t" . sprintf(
			'<span class="zc-calendar-nav-next"><a href="%1$s">%2$s &raquo;</a></span>',
			get_month_link( $next->year, $next->month ),
			$zc_locale->get_month_abbrev( $zc_locale->get_month( $next->month ) )
		);
	} else {
		$calendar_output .= "\n\t\t" . '<span class="zc-calendar-nav-next">&nbsp;</span>';
	}
...
function delete_get_calendar_cache() {
	zc_cache_delete( 'get_calendar', 'calendar' );
}
 ...
 *
 * @param string           $format Optional. PHP date format. Defaults to the 'date_format' option.
 * @param int|ZC_Post|null $post   Optional. Post ID or ZC_Post object. Default current post.
...
function get_the_modified_date( $format = '', $post = null ) {
	 ...
	 * @param string           $format   PHP date format.
	 * @param ZC_Post|null     $post     ZC_Post object or null if no post is found.
	 */
 ...
 *                                 was written. Accepts 'G', 'U', or PHP date format.
 *                                 Defaults to the 'time_format' option.
 * @param int|ZC_Post|null $post   Post ID or post object. Default is global `$post` object.
...
function get_post_time( $format = 'U', $gmt = false, $post = null, $translate = false ) {
	...
	if ( 'U' === $format || 'G' === $format ) {
		...
		$time = zc_date( $format, $datetime->getTimestamp(), $gmt ? new DateTimeZone( 'UTC' ) : null );
	} else {
		if ( $gmt ) {
 ...
 *
 * The object will be set to the timezone from ZelocoreCMS settings.
 *
 ...
 * @since 5.3.0
 *
 * @param int|ZC_Post|null $post   Optional. Post ID or post object. Default is global `$post` object.
...
function get_post_datetime( $post = null, $field = 'date', $source = 'local' ) {
	...
	return $datetime->setTimezone( $zc_timezone );
}
...
function the_weekday() {
	...
	$the_weekday = $zc_locale->get_weekday( get_post_time( 'w', false, $post ) );
...
function the_weekday_date( $before = '', $after = '' ) {
	global $zc_locale, $currentday, $previousweekday;
	...
	if ( $currentday !== $previousweekday ) {
		...
		$the_weekday_date .= $zc_locale->get_weekday( get_post_time( 'w', false, $post ) );
...
/**
 * Fires the zc_body_open action.
 *
 * See {@see 'zc_body_open'}.
 *
 * @since 5.2.0
 */
function zc_body_open() {
	/**
	 * Triggered after the opening body tag.
	 *
	 * @since 5.2.0
	 */
	do_action( 'zc_body_open' );
}
...
function feed_links_extra( $args = array() ) {
	 ...
	 */
	$queried_object = get_queried_object();
	if ( is_singular() && $queried_object instanceof ZC_Post ) {
		$post = $queried_object;
 ...
 *
 *     add_action( 'zc_head', 'zc_strict_cross_origin_referrer' );
 *
 * @since 5.7.0
 */
function zc_strict_cross_origin_referrer() {
	?>
 ...
 * @since 4.6.0
 */
function zc_resource_hints() {
	$hints = array(
		'dns-prefetch' => zc_dependencies_unique_hosts(),
		'preconnect'   => array(),
		'prefetch'     => array(),
		'prerender'    => array(),
	);
	...
	foreach ( $hints as $relation_type => $urls ) {
		 ...
		 */
		$urls = apply_filters( 'zc_resource_hints', $urls, $relation_type );
...
function zc_preload_resources() {
	 ...
	 * }
	 */
	$preload_resources = apply_filters( 'zc_preload_resources', array() );
 ...
 * @global ZC_Styles  $zc_styles  The ZC_Styles object for printing styles.
 *
 * @return string[] A list of unique hosts of enqueued scripts and styles.
 */
function zc_dependencies_unique_hosts() {
	global $zc_scripts, $zc_styles;

	$unique_hosts = array();

	foreach ( array( $zc_scripts, $zc_styles ) as $dependencies ) {
		if ( $dependencies instanceof ZC_Dependencies && ! empty( $dependencies->queue ) ) {
			foreach ( $dependencies->queue as $handle ) {
				if ( ! isset( $dependencies->registered[ $handle ] ) ) {
					continue;
				}

				/* @var _ZC_Dependency $dependency */
				$dependency = $dependencies->registered[ $handle ];
				$parsed     = zc_parse_url( $dependency->src );
 ...
 * @since 2.0.0
 *
 * @global bool $zc_rich_edit Whether the user can access the visual editor.
...
function user_can_richedit() {
	global $zc_rich_edit, $is_gecko, $is_opera, $is_safari, $is_chrome, $is_IE, $is_edge;

	if ( ! isset( $zc_rich_edit ) ) {
		$zc_rich_edit = false;

		if ( 'true' === get_user_option( 'rich_editing' ) || ! is_user_logged_in() ) { // Default to 'true' for logged out users.
			if ( $is_safari ) {
				$zc_rich_edit = ! zc_is_mobile() || ( preg_match( '!AppleWebKit/(\d+)!', $_SERVER['HTTP_USER_AGENT'], $match ) && (int) $match[1] >= 534 );
			} elseif ( $is_IE ) {
				$zc_rich_edit = str_contains( $_SERVER['HTTP_USER_AGENT'], 'Trident/7.0;' );
			} elseif ( $is_gecko || $is_chrome || $is_edge || ( $is_opera && ! zc_is_mobile() ) ) {
				$zc_rich_edit = true;
			}
		}
	}
	 ...
	 * @since 2.1.0
	 *
	 * @param bool $zc_rich_edit Whether the user can access the visual editor.
	 */
	return apply_filters( 'user_can_richedit', $zc_rich_edit );
}
 ...
 */
function zc_default_editor() {
	$r = user_can_richedit() ? 'tinymce' : 'html'; // Defaults.
	if ( zc_get_current_user() ) { // Look for cookie.
	 ...
	 */
	return apply_filters( 'zc_default_editor', $r );
}
 ...
 * _ZC_Editors should not be used directly. See https://core.trac.zelocorecms.org/ticket/17144.
 *
 * NOTE: Once initialized the TinyMCE editor cannot be safely moved in the DOM. For that reason
 * running zc_editor() inside of a meta box is not a good idea unless only Quicktags is used.
 * On the post edit screen several actions can be used to include additional editors
 * containing TinyMCE: 'edit_page_form', 'edit_form_advanced' and 'dbx_post_sidebar'.
 * See https://core.trac.zelocorecms.org/ticket/19173 for more information.
 *
 * @see _ZC_Editors::editor()
 * @see _ZC_Editors::parse_settings()
 * @since 3.3.0
 *
 * @param string $content   Initial content for the editor.
 * @param string $editor_id HTML ID attribute value for the textarea and TinyMCE.
 *                          Should not contain square brackets.
 * @param array  $settings  See _ZC_Editors::parse_settings() for description.
 */
 ...
 * @since 4.9.0
 *
 * @see zc_enqueue_editor()
 * @see zc_get_code_editor_settings()
 * @see _ZC_Editors::parse_settings()
 *
 * @param array $args {
 *     Args.
 ...
 *     @type ZC_Theme $theme      Theme being edited when on the theme file editor.
 ...
 */
function zc_enqueue_code_editor( $args ) {
	if ( is_user_logged_in() && 'false' === zc_get_current_user()->syntax_highlighting ) {
		return false;
	}

	$settings = zc_get_code_editor_settings( $args );

	if ( empty( $settings ) || empty( $settings['codemirror'] ) ) {
		return false;
	}

	zc_enqueue_script( 'code-editor' );
	zc_enqueue_style( 'code-editor' );
	...
	if ( isset( $settings['codemirror']['mode'] ) ) {
		...
		if ( ! empty( $settings['codemirror']['lint'] ) ) {
			switch ( $mode['name'] ) {
				...
				case 'text/x-php':
					zc_enqueue_script( 'htmlhint' );
					zc_enqueue_script( 'csslint' );
					if ( ! current_user_can( 'unfiltered_html' ) ) {
						zc_enqueue_script( 'htmlhint-kses' );
					}
					break;
				case 'javascript':
	 ...
	 */
	do_action( 'zc_enqueue_code_editor', $settings );
 ...
 */
function zc_get_code_editor_settings( $args ) {
	$settings = array(
		'codemirror' => array(
			'indentUnit'       => 4,
			'indentWithTabs'   => true,
		...
		'jshint'     => array(
			...
			// in zc_default_script_modules(). This is a first stab at a core-only private module.
			...
			// <https://github.com/ZelocoreCMS/zelocorecms-develop/blob/6.9.0/.jshintrc>.
	...
	if ( isset( $args['type'] ) ) {
		...
		// Supply any types that are not matched by zc_get_mime_types().
		if ( empty( $type ) ) {
			switch ( $extension ) {
				case 'conf':
					$type = 'text/nginx';
	...
	if ( in_array( $type, array( 'text/css', 'text/x-scss', 'text/x-less', 'text/x-sass' ), true ) ) {
		...
		if ( ! current_user_can( 'unfiltered_html' ) ) {
			$settings['htmlhint']['kses'] = zc_kses_allowed_html( 'post' );
		}
	...
	foreach ( zc_array_slice_assoc( $args, array( 'codemirror', 'csslint', 'jshint', 'htmlhint' ) ) as $key => $value ) {
	 ...
	 * }
	 */
	return apply_filters( 'zc_code_editor_settings', $settings, $args );
}

/**
 * Retrieves the contents of the search ZelocoreCMS query variable.
 *
 ...
 *     @type int    $total              The total amount of pages. Default is the value ZC_Query's
 *                                      `max_num_pages` or 1.
...
function paginate_links( $args = '' ) {
	...
	$total   = $zc_query->max_num_pages ?? 1;
	 ...
	 */
	if ( $zc_rewrite->using_permalinks() && ! $zc_rewrite->use_trailing_slashes ) {
	...
	$format  = $zc_rewrite->using_index_permalinks() && ! strpos( $pagenum_link, 'index.php' ) ? 'index.php/' : '';
	$format .= $zc_rewrite->using_permalinks() ? user_trailingslashit( $zc_rewrite->pagination_base . '/%#%', 'paged' ) : '?paged=%#%';
	if ( $zc_rewrite->using_permalinks() && ! $zc_rewrite->use_trailing_slashes ) {
		$format = '/' . ltrim( $format, '/' );
	}
	...
	if ( isset( $url_parts[1] ) ) {
		...
		zc_parse_str( $format_query, $format_args );

		// Find the query args of the requested URL.
		zc_parse_str( $url_parts[1], $url_query_args );
 ...
 *
 *     zc_admin_css_color( 'classic', __( 'Classic' ), admin_url( "css/colors-classic.css" ), array(
 ...
 * }
 */
function zc_admin_css_color( $key, $name, $url, $colors = array(), $icons = array() ) {
	global $_zc_admin_css_colors;

	if ( ! isset( $_zc_admin_css_colors ) ) {
		$_zc_admin_css_colors = array();
	}

	$_zc_admin_css_colors[ $key ] = (object) array(
		'name'        => $name,
		'url'         => $url,
		'colors'      => $colors,
		'icon_colors' => $icons,
...
/**
 * Displays the URL of a ZelocoreCMS admin CSS file.
 *
 * @see ZC_Styles::_css_href() and its {@see 'style_loader_src'} filter.
 *
 * @since 2.3.0
 *
 * @param string $file file relative to zc-admin/ without its ".css" extension.
 * @return string
 */
function zc_admin_css_uri( $file = 'zc-admin' ) {
	...
	/**
	 * Filters the URI of a ZelocoreCMS admin CSS file.
	 *
	 * @since 2.3.0
	 *
	 ...
	 */
	return apply_filters( 'zc_admin_css_uri', $_file, $file );
}
 ...
 * {@see 'zc_print_styles'} action has *not* yet been called, the CSS file will be
 * enqueued. If the {@see 'zc_print_styles'} action has been called, the CSS link will
 * be printed. Printing may be forced by passing true as the $force_echo
 * (second) parameter.
 *
 * For backward compatibility with ZelocoreCMS 2.3 calling method: If the $file
 * (first) parameter does not correspond to a registered CSS file, we assume
 * $file is a file relative to zc-admin/ without its ".css" extension. A
 * stylesheet link to that generated URL is printed.
 *
 * @since 2.3.0
 *
 * @param string $file       Optional. Style handle name or file name (without ".css" extension) relative
 *                           to zc-admin/. Defaults to 'zc-admin'.
 * @param bool   $force_echo Optional. Force the stylesheet link to be printed rather than enqueued.
 */
function zc_admin_css( $file = 'zc-admin', $force_echo = false ) {
	// For backward compatibility.
	$handle = str_starts_with( $file, 'css/' ) ? substr( $file, 4 ) : $file;

	if ( zc_styles()->query( $handle ) ) {
		if ( $force_echo || did_action( 'zc_print_styles' ) ) {
			// We already printed the style queue. Print this one immediately.
			zc_print_styles( $handle );
		} else {
			// Add to style queue.
	 ...
	 *                                relative to zc-admin/. Defaults to 'zc-admin'.
	 */
	echo apply_filters( 'zc_admin_css', $stylesheet_link, $file );

	if ( function_exists( 'is_rtl' ) && is_rtl() ) {
		$rtl_stylesheet_link = sprintf(
			"<link rel='stylesheet' href='%s' />\n",
			esc_url( zc_admin_css_uri( "$file-rtl" ) )
		);

		/** This filter is documented in zc-includes/general-template.php */
		echo apply_filters( 'zc_admin_css', $rtl_stylesheet_link, "$file-rtl" );
	}
}
...
function add_thickbox() {
	zc_enqueue_script( 'thickbox' );
	zc_enqueue_style( 'thickbox' );
...
/**
 * Displays the XHTML generator that is generated on the zc_head hook.
 *
 * See {@see 'zc_head'}.
 *
 * @since 2.5.0
 */
function zc_generator() {
	/**
	 ...
	 */
	the_generator( apply_filters( 'zc_generator_type', 'xhtml' ) );
}
...
function get_the_generator( $type = '' ) {
	...
	switch ( $type ) {
		case 'html':
			$gen = '<meta name="generator" content="ZelocoreCMS ' . esc_attr( get_bloginfo( 'version' ) ) . '">';
			break;
		case 'xhtml':
			$gen = '<meta name="generator" content="ZelocoreCMS ' . esc_attr( get_bloginfo( 'version' ) ) . '" />';
			break;
		case 'atom':
			$gen = '<generator uri="https://zelocorecms.org/" version="' . esc_attr( get_bloginfo_rss( 'version' ) ) . '">ZelocoreCMS</generator>';
			break;
		case 'rss2':
			$gen = '<generator>' . sanitize_url( 'https://zelocorecms.org/?v=' . get_bloginfo_rss( 'version' ) ) . '</generator>';
			break;
		case 'rdf':
			$gen = '<admin:generatorAgent rdf:resource="' . sanitize_url( 'https://zelocorecms.org/?v=' . get_bloginfo_rss( 'version' ) ) . '" />';
			break;
		case 'comment':
			$gen = '<!-- generator="ZelocoreCMS/' . esc_attr( get_bloginfo( 'version' ) ) . '" -->';
			break;
		case 'export':
			$gen = '<!-- generator="ZelocoreCMS/' . esc_attr( get_bloginfo_rss( 'version' ) ) . '" created="' . gmdate( 'Y-m-d H:i' ) . '" -->';
			break;
	}
	 ...
	 * @since 2.5.0
	 *
	 * @param string $gen  The HTML markup output to zc_head().
 ...
 * @return string HTML attribute or empty string.
 */
function zc_readonly( $readonly_value, $current = true, $display = true ) {
 ...
 * @return string HTML attribute or empty string.
 */
function __checked_selected_helper( $helper, $current, $display, $type ) { // phpcs:ignore ZelocoreCMS.NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
 ...
 */
function zc_required_field_indicator() {
	 ...
	 */
	return apply_filters( 'zc_required_field_indicator', $indicator );
}
 ...
 */
function zc_required_field_message() {
	$message = sprintf(
		'<span class="required-field-message">%s</span>',
		/* translators: %s: Asterisk symbol (*). */
		sprintf( __( 'Required fields are marked %s' ), zc_required_field_indicator() )
	);
	 ...
	 */
	return apply_filters( 'zc_required_field_message', $message );
}
...
function zc_heartbeat_settings( $settings ) {
	...
	if ( is_user_logged_in() ) {
		$settings['nonce'] = zc_create_nonce( 'heartbeat-nonce' );
	}
