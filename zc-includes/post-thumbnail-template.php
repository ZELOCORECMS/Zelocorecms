<?php
/**
 * ZelocoreCMS Post Thumbnail Template Functions.
 *
 ...
 *
 * @since 2.9.0
 * @since 4.4.0 `$post` can be a post ID or ZC_Post object.
 *
...
function has_post_thumbnail( $post = null ) {
	 ...
	 * @param int|ZC_Post|null $post          Post ID or ZC_Post object. Default is global `$post`.
 ...
 *
 * @param ZC_Query|null $zc_query Optional. A ZC_Query instance. Defaults to the $zc_query global.
 */
function update_post_thumbnail_cache( $zc_query = null ) {
	if ( ! $zc_query ) {
		$zc_query = $GLOBALS['zc_query'];
	}

	if ( $zc_query->thumbnails_cached ) {
		return;
	}
	...
	/*
	 * $zc_query may contain an array of post objects or post IDs.
	 *
	...
	foreach ( $zc_query->posts as $post ) {
	...
	$zc_query->thumbnails_cached = true;
}
...
function get_the_post_thumbnail( $post = null, $size = 'post-thumbnail', $attr = '' ) {
	...
	if ( $post_thumbnail_id ) {
		 ...
		 *
		 * Provides "just in time" filtering of all filters in zc_get_attachment_image().
		 *
		 * @since 2.9.0
		 *
		 * @param int          $post_id           The post ID.
		...
		$html = zc_get_attachment_image( $post_thumbnail_id, $size, false, $attr );
...
function get_the_post_thumbnail_url( $post = null, $size = 'post-thumbnail' ) {
	...
	$thumbnail_url = zc_get_attachment_image_url( $post_thumbnail_id, $size );
...
function get_the_post_thumbnail_caption( $post = null ) {
	...
	$caption = zc_get_attachment_caption( $post_thumbnail_id );
