...
foreach ( $routes_by_page as $page_slug => $page_routes ) {
	...
	$global_name = 'zc_' . $page_slug_underscore . '_routes_data';
 ...
 */
function zc_register_page_routes( $page_routes, $register_function_name ) {
	...
	foreach ( $page_routes as $route ) {
		...
		if ( $route['has_content'] ) {
			...
			if ( file_exists( $content_asset_path ) ) {
				...
				$content_handle = 'zc/routes/' . $route['name'] . '/content';
				...
				zc_deregister_script_module( $content_handle );
		...
		if ( $route['has_route'] ) {
			...
			if ( file_exists( $route_asset_path ) ) {
				...
				$route_handle = 'zc/routes/' . $route['name'] . '/route';
				$extension = defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ? '.js' : '.min.js';
				// Deregister first to override any previously registered version
				// (e.g., Core's default modules when running as a plugin).
				zc_deregister_script_module( $route_handle );
				zc_register_script_module(
					$route_handle,
 ...
 */
function zc_register_options_connectors_page_routes() {
	global $zc_options_connectors_routes_data;
	zc_register_page_routes( $zc_options_connectors_routes_data, 'zc_register_options_connectors_route' );
}
add_action( 'options-connectors_init', 'zc_register_options_connectors_page_routes' );

/**
 * Register routes for options-connectors page (zc-admin mode).
 */
function zc_register_options_connectors_zc_admin_page_routes() {
	global $zc_options_connectors_routes_data;
	zc_register_page_routes( $zc_options_connectors_routes_data, 'zc_register_options_connectors_zc_admin_route' );
}
add_action( 'options-connectors-zc-admin_init', 'zc_register_options_connectors_zc_admin_page_routes' );

// Page-specific route registration functions for font-library
/**
 * Register routes for font-library page (full-page mode).
 */
function zc_register_font_library_page_routes() {
	global $zc_font_library_routes_data;
	zc_register_page_routes( $zc_font_library_routes_data, 'zc_register_font_library_route' );
}
add_action( 'font-library_init', 'zc_register_font_library_page_routes' );

/**
 * Register routes for font-library page (zc-admin mode).
 */
function zc_register_font_library_zc_admin_page_routes() {
	global $zc_font_library_routes_data;
	zc_register_page_routes( $zc_font_library_routes_data, 'zc_register_font_library_zc_admin_route' );
}
add_action( 'font-library-zc-admin_init', 'zc_register_font_library_zc_admin_page_routes' );
