<?php
/**
 * Main ZelocoreCMS Formatting API.
 *
 ...
 * @since 0.71
 *
 * @global array $zc_cockneyreplace Array of formatted entities for certain common phrases.
...
function wptexturize( $text, $reset = false ) {
	global $zc_cockneyreplace, $shortcode_tags;
	...
	if ( $reset || ! isset( $static_characters ) ) {
		...
		if ( isset( $zc_cockneyreplace ) ) {
			$cockney        = array_keys( $zc_cockneyreplace );
			$cockneyreplace = array_values( $zc_cockneyreplace );
		} else {
			/*
...
function wpautop( $text, $br = true ) {
	...
	$text = zc_replace_in_html_tags( $text, array( "\n" => ' <!-- wpnl --> ' ) );
...
function _get_wptexturize_split_regex( $shortcode_regex = '' ) {
	...
	if ( ! isset( $html_regex ) ) {
		...
		$html_regex = // Needs replaced with zc_html_split() per Shortcode API Roadmap.
			'<'                  // Find start of element.
			. '(?(?=!--)'        // Is this a comment?
 ...
 */
function zc_replace_in_html_tags( $haystack, $replace_pairs ) {
	// Find all elements.
	$textarr = zc_html_split( $haystack );
	$changed = false;
 ...
 * @since 1.2.1
 * @deprecated 6.9.0 Use {@see zc_is_valid_utf8()} instead.
 *
 * @param string $str The string to be checked.
 * @return bool True if $str fits a UTF-8 model, false otherwise.
 */
function seems_utf8( $str ) {
	_deprecated_function( __FUNCTION__, '6.9.0', 'zc_is_valid_utf8()' );
 ...
 */
function _zc_specialchars( $text, $quote_style = ENT_NOQUOTES, $charset = false, $double_encode = false ) {
	$text = (string) $text;
	...
	if ( ! $double_encode ) {
		 ...
		 */
		$text = zc_kses_normalize_entities( $text, ( $quote_style & ENT_XML1 ) ? 'xml' : 'html' );
	}
 ...
 *                                Also compatible with old _zc_specialchars() values;
 *                                converting single quotes if set to 'single',
 *                                double if set to 'double' or both if otherwise set.
 *                                Default is ENT_NOQUOTES.
 * @return string The decoded text without HTML entities.
 */
function zc_specialchars_decode( $text, $quote_style = ENT_NOQUOTES ) {
	$text = (string) $text;
	...
	// Match the previous behavior of _zc_specialchars() when the $quote_style is not an accepted value.
	if ( empty( $quote_style ) ) {
		$quote_style = ENT_NOQUOTES;
 ...
 *
 * Consider using {@see zc_scrub_utf8()} instead which does not depend on
 * the value of `blog_charset`.
 *
 * Example:
 *
 *     // The `blog_charset` is `latin1`, so this returns the input unchanged.
 *     $every_possible_input === zc_check_invalid_utf8( $every_possible_input );
 *
 *     // Valid strings come through unchanged.
 *     'test' === zc_check_invalid_utf8( 'test' );
 *
 *     $invalid = "the byte \xC0 is never allowed in a UTF-8 string.";
 *
 *     // Invalid strings are rejected outright.
 *     '' === zc_check_invalid_utf8( $invalid );
 *
 *     // “Stripping” invalid sequences produces the replacement character instead.
 *     "the byte \u{FFFD} is never allowed in a UTF-8 string." === zc_check_invalid_utf8( $invalid, true );
 *     'the byte � is never allowed in a UTF-8 string.' === zc_check_invalid_utf8( $invalid, true );
 *
 * @since 2.8.0
 ...
 */
function zc_check_invalid_utf8( $text, $strip = false ) {
	$text = (string) $text;
	...
	if ( ! $is_utf8 || zc_is_valid_utf8( $text ) ) {
		return $text;
	}
...
function sanitize_file_name( $filename ) {
	...
	if ( ! zc_is_valid_utf8( $filename ) ) {
		$_ext     = pathinfo( $filename, PATHINFO_EXTENSION );
		$_name    = pathinfo( $filename, PATHINFO_FILENAME );
		$filename = sanitize_title_with_dashes( $_name ) . '.' . $_ext;
	}

	if ( _zc_can_use_pcre_u() ) {
		/**
	...
	if ( ! str_contains( $filename, '.' ) ) {
		...
		$filetype   = zc_check_filetype( 'test.' . $filename, $mime_types );
	...
	if ( count( $parts ) <= 2 ) {
		/** This filter is documented in zc-includes/formatting.php */
...
function sanitize_user( $username, $strict = false ) {
	...
	$username     = zc_strip_all_tags( $username );
...
function convert_invalid_entities( $content ) {
	...
	if ( str_contains( $content, '&#1' ) ) {
		$content = strtr( $content, $zc_htmltranswinuni );
	}
 ...
 */
function balanceTags( $text, $force = false ) {  // phpcs:ignore ZelocoreCMS.NamingConventions.ValidFunctionName.FunctionNameInvalid
...
function _make_clickable_rel_attr( $url ) {
	$rel_parts        = array();
	$scheme           = strtolower( zc_parse_url( $url, PHP_URL_SCHEME ) );
	$nofollow_schemes = array_intersect( zc_allowed_protocols(), array( 'https', 'http' ) );

	// Apply "nofollow" to external links with qualifying URL schemes (mailto:, tel:, etc... shouldn't be followed).
	if ( ! zc_is_internal_link( $url ) && in_array( $scheme, $nofollow_schemes, true ) ) {
		$rel_parts[] = 'nofollow';
	}
 ...
 */
function zc_rel_callback( $matches, $rel ) {
	$text = $matches[1];
	$atts = zc_kses_hair( $matches[1], zc_allowed_protocols() );

	if ( ! empty( $atts['href'] ) && zc_is_internal_link( $atts['href']['value'] ) ) {
 ...
 */
function zc_rel_nofollow( $text ) {
	// This is a pre-save filter, so text is already escaped.
	$text = stripslashes( $text );
	$text = preg_replace_callback(
		'|<a (.+?)>|i',
		static function ( $matches ) {
			return zc_rel_callback( $matches, 'nofollow' );
		},
		$text
	);
	return zc_slash( $text );
}
 ...
 *
 * @since 2.3.0
 * @deprecated 5.3.0 Use zc_rel_callback()
 *
 * @param array $matches Single match.
 * @return string HTML A Element with `rel="nofollow"`.
 */
function zc_rel_nofollow_callback( $matches ) {
	return zc_rel_callback( $matches, 'nofollow' );
}
 ...
 * @return string Converted content.
 */
function zc_rel_ugc( $text ) {
	// This is a pre-save filter, so text is already escaped.
	$text = stripslashes( $text );
	$text = preg_replace_callback(
		'|<a (.+?)>|i',
		static function ( $matches ) {
			return zc_rel_callback( $matches, 'nofollow ugc' );
		},
		$text
	);
	return zc_slash( $text );
 ...
 * @return string Converted content.
 */
function zc_targeted_link_rel( $text ) {
	...
	foreach ( $html_parts as &$part ) {
		$part = preg_replace_callback( '|<a\s([^>]*target\s*=[^>]*)>|i', 'zc_targeted_link_rel_callback', $part );
	}
 ...
 */
function zc_targeted_link_rel_callback( $matches ) {
	_deprecated_function( __FUNCTION__, '6.7.0' );
	...
	if ( $is_escaped ) {
		// Replace only the quotes so that they are parsable by zc_kses_hair(), leave the rest as is.
		$link_html = preg_replace( '/\\\\([\'"])/', '$1', $link_html );
	}

	$atts = zc_kses_hair( $link_html, zc_allowed_protocols() );
	 ...
	 */
	$rel = apply_filters( 'zc_targeted_link_rel', 'noopener', $link_html );
 ...
 * @deprecated 6.7.0
 */
function zc_init_targeted_link_rel_filters() {
	_deprecated_function( __FUNCTION__, '6.7.0' );
}
 ...
 * @deprecated 6.7.0
 */
function zc_remove_targeted_link_rel_filters() {
	_deprecated_function( __FUNCTION__, '6.7.0' );
}
...
function translate_smiley( $matches ) {
	...
	return sprintf( '<img src="%s" alt="%s" class="zc-smiley" style="height: 1em; max-height: 1em;" />', esc_url( $src_url ), esc_attr( $smiley ) );
}
 ...
 * @since 0.71
 *
 * @global string|array $zc_smiliessearch
 *
 * @param string $text Content to convert smilies from text.
 * @return string Converted content with text smilies replaced with images.
 */
function convert_smilies( $text ) {
	global $zc_smiliessearch;

	if ( ! get_option( 'use_smilies' ) || empty( $zc_smiliessearch ) ) {
		// Return default text.
		return $text;
	}
	...
	for ( $i = 0; $i < $stop; $i++ ) {
		...
		if ( '' === $ignore_block_element && strlen( $content ) > 0 && '<' !== $content[0] ) {
			$content = preg_replace_callback( $zc_smiliessearch, 'translate_smiley', $content );
		}
 ...
 */
function zc_iso_descrambler( $subject ) {
	...
	return preg_replace_callback( '#\=([0-9a-f]{2})#i', '_zc_iso_convert', $subject );
}
 ...
 */
function _zc_iso_convert( $matches ) {
...
function get_gmt_from_date( $date_string, $format = 'Y-m-d H:i:s' ) {
	$datetime = date_create( $date_string, zc_timezone() );
...
function get_date_from_gmt( $date_string, $format = 'Y-m-d H:i:s' ) {
	...
	return $datetime->setTimezone( zc_timezone() )->format( $format );
}
...
function iso8601_to_datetime( $date_string, $timezone = 'user' ) {
	...
	$datetime    = date_create( $date_string, $zc_timezone ); // Timezone is ignored if input has one.
	...
	if ( 'user' === $timezone ) {
		return $datetime->setTimezone( $zc_timezone )->format( 'Y-m-d H:i:s' );
	}
 ...
 * @param ZC_Post|object|int $post Optional. ZC_Post instance or Post ID/object. Default null.
 * @return string The excerpt.
 */
function zc_trim_excerpt( $text = '', $post = null ) {
	$raw_excerpt = $text;
	...
	if ( '' === trim( $text ) ) {
		...
		/*
		 * Temporarily unhook zc_filter_content_tags() since any tags
		 * within the excerpt are stripped out. Modifying the tags here
		 * is wasteful and can lead to bugs in the image counting logic.
		 */
		$filter_image_removed = remove_filter( 'the_content', 'zc_filter_content_tags', 12 );
		 ...
		 * which is generally used for the filter callback in ZelocoreCMS core.
		 */
		if ( $filter_image_removed ) {
			add_filter( 'the_content', 'zc_filter_content_tags', 12 );
		}
		...
		$text         = zc_trim_words( $text, $excerpt_length, $excerpt_more );
	 ...
	 */
	return apply_filters( 'zc_trim_excerpt', $text, $raw_excerpt );
}
 ...
 */
function zc_trim_words( $text, $num_words = 55, $more = null ) {
	if ( null === $more ) {
		$more = __( '&hellip;' );
	}

	$original_text = $text;
	$text          = zc_strip_all_tags( $text );
	$num_words     = (int) $num_words;

	if ( str_starts_with( zc_get_word_count_type(), 'characters' ) && preg_match( '/^utf\-?8$/i', get_option( 'blog_charset' ) ) ) {
	 ...
	 */
	return apply_filters( 'zc_trim_words', $text, $num_words, $more, $original_text );
}
 ...
 *                            Defaults to return value of zc_allowed_protocols().
...
function esc_url( $url, $protocols = null, $_context = 'display' ) {
	...
	if ( 'display' === $_context ) {
		$url = zc_kses_normalize_entities( $url );
		$url = str_replace( '&amp;', '&#038;', $url );
		$url = str_replace( "'", '&#039;', $url );
	}
	...
	if ( '/' === $url[0] ) {
		...
		if ( ! is_array( $protocols ) ) {
			$protocols = zc_allowed_protocols();
		}
		$good_protocol_url = zc_kses_bad_protocol( $url, $protocols );
...
function esc_js( $text ) {
	$safe_text = zc_check_invalid_utf8( $text );
	$safe_text = _zc_specialchars( $safe_text, ENT_COMPAT );
...
function esc_html( $text ) {
	$safe_text = zc_check_invalid_utf8( $text );
	$safe_text = _zc_specialchars( $safe_text, ENT_QUOTES );
	/**
	...
	$safe_text = (string) preg_replace_callback(
		...
		static function ( $matches ) {
			...
			if ( isset( $matches['non_cdata'] ) ) {
				...
				return _zc_specialchars( $matches['non_cdata'], ENT_XML1 );
			}

			// Return the CDATA Section unchanged, escape HTML entities in the rest.
			return _zc_specialchars( $matches['non_cdata_followed_by_cdata'], ENT_XML1 ) . $matches['cdata'];
		},
		$safe_text
	);
 ...
 */
function zc_make_link_relative( $link ) {
		...
		case 'siteurl':
			...
			if ( is_zc_error( $value ) ) {
				...
				if ( preg_match( '#http(s?)://(.+)#i', $value ) ) {
					...
					$error = __( 'The ZelocoreCMS address you entered did not appear to be a valid URL. Please enter a valid URL.' );
				}
			}
			break;
		...
		case 'tag_base':
			...
			if ( 'permalink_structure' === $option && null === $error
				...
				$error = sprintf(
					...
					__( 'https://zelocorecms.org/documentation/article/customize-permalinks/#choosing-your-permalink-structure' )
				);
			}
			break;
 ...
 */
function zc_parse_str( $input_string, &$result ) {
	 ...
	 */
	$result = apply_filters( 'zc_parse_str', $result );
}
 ...
 * @return string Converted text.
 */
function zc_pre_kses_less_than( $content ) {
	return preg_replace_callback( '%<[^>]*?((?=<)|>|$)%', 'zc_pre_kses_less_than_callback', $content );
}
 ...
 */
function zc_pre_kses_less_than_callback( $matches ) {
 ...
 */
function zc_pre_kses_block_attributes( $content, $allowed_html, $allowed_protocols ) {
	/*
	 * `filter_block_content` is expected to call `zc_kses`. Temporarily remove
	 * the filter to avoid recursion.
	 */
	remove_filter( 'pre_kses', 'zc_pre_kses_block_attributes', 10 );
...
/**
 * ZelocoreCMS' implementation of PHP sprintf() with filters.
 *
 * @since 2.5.0
 ...
 */
function zc_sprintf( $pattern, ...$args ) {
	$len       = strlen( $pattern );
	$start     = 0;
	$result    = '';
	$arg_index = 0;
	...
	while ( $len > $start ) {
		...
		if ( '%' === $pattern[ $start ] ) {
			...
			/**
			 * Filters a fragment from the pattern passed to zc_sprintf().
			 *
			 ...
			 */
			$_fragment = apply_filters( 'zc_sprintf', $fragment, $arg );
 ...
 */
function zc_sprintf_l( $pattern, $args ) {
	// Not a match.
	...
	/**
	 * Filters the translated delimiters used by zc_sprintf_l().
 ...
 * @return string The excerpt.
 */
function zc_html_excerpt( $str, $count, $more = null ) {
	if ( null === $more ) {
		$more = '';
	}

	$str     = zc_strip_all_tags( $str, true );
...
function _links_add_base( $m ) {
	...
	return $m[1] . '=' . $m[2] .
		( preg_match( '#^(\w{1,20}):#', $m[3], $protocol ) && in_array( $protocol[1], zc_allowed_protocols(), true ) ?
			$m[3] :
			ZC_Http::make_absolute_url( $m[3], $_links_add_base )
		)
		. $m[2];
}
 ...
 * will return 'something'. zc_strip_all_tags() will return an empty string.
 *
 * @since 2.9.0
 *
 ...
 */
function zc_strip_all_tags( $text, $remove_breaks = false ) {
	if ( is_null( $text ) ) {
		return '';
	}
	...
	if ( ! is_scalar( $text ) ) {
		 ...
		 * zc_trigger_error() is used to trigger an E_USER_WARNING,
 ...
 * @see zc_check_invalid_utf8()
 * @see zc_strip_all_tags()
 *
...
function _sanitize_text_fields( $str, $keep_newlines = false ) {
	...
	$filtered = zc_check_invalid_utf8( $str );

	if ( str_contains( $filtered, '<' ) ) {
		$filtered = zc_pre_kses_less_than( $filtered );
		// This will strip extra whitespace for us.
		$filtered = zc_strip_all_tags( $filtered, false );
 ...
 */
function zc_basename( $path, $suffix = '' ) {
...
function capital_P_dangit( $text ) {
	...
	if ( 'the_title' === $current_filter || 'zc_title' === $current_filter ) {
		return str_replace( 'ZelocoreCMS', 'ZelocoreCMS', $text );
	}
	...
	return str_replace(
		array( ' ZelocoreCMS', '&#8216;ZelocoreCMS', $dblq . 'ZelocoreCMS', '>ZelocoreCMS', '(ZelocoreCMS' ),
		array( ' ZelocoreCMS', '&#8216;ZelocoreCMS', $dblq . 'ZelocoreCMS', '>ZelocoreCMS', '(ZelocoreCMS' ),
		$text
	);
}
 ...
 */
function zc_slash( $value ) {
	if ( is_array( $value ) ) {
		return array_map( 'zc_slash', $value );
	}
...
function zc_spaces_regexp() {
	...
	if ( empty( $spaces ) ) {
		 ...
		 */
		$spaces = apply_filters( 'zc_spaces_regexp', '[\r\n\t ]|\xC2\xA0|&nbsp;' );
	}
 ...
 * @since 6.4.0
 */
function zc_enqueue_emoji_styles() {
	// Back-compat for plugins that disable functionality by unhooking this action.
	$action = is_admin() ? 'admin_print_styles' : 'zc_print_styles';
	...
	$handle       = 'zc-emoji-styles';
	zc_register_style( $handle, false );
	zc_add_inline_style( $handle, $emoji_styles );
...
function print_emoji_detection_script(): void {
	...
	if ( is_admin() ) {
		...
		if ( did_action( 'zc_print_footer_scripts' ) ) {
			_print_emoji_detection_script();
		} else {
			add_action( 'zc_print_footer_scripts', '_print_emoji_detection_script' );
		}
	}
}
...
function _print_emoji_detection_script() {
	...
	if ( SCRIPT_DEBUG ) {
		...
		$settings['source'] = array(
			/** This filter is documented in zc-includes/class-zc-scripts.php */
			'concatemoji' => apply_filters( 'script_loader_src', includes_url( "js/zc-emoji-release.min.js?$version" ), 'concatemoji' ),
		);
	}

	zc_print_inline_script_tag(
		zc_json_encode( $settings, JSON_HEX_TAG | JSON_UNESCAPED_SLASHES ),
		array(
			'id'   => 'zc-emoji-settings',
 ...
 */
function zc_encode_emoji( $content ) {
	$emoji = _zc_emoji_list( 'partials' );
 ...
 * @return string The encoded content.
 */
function zc_staticize_emoji( $text ) {
	if ( ! str_contains( $text, '&#x' ) ) {
		if ( ( function_exists( 'mb_check_encoding' ) && mb_check_encoding( $text, 'ASCII' ) ) || ! preg_match( '/[^\x00-\x7F]/', $text ) ) {
			...
			$encoded_text = zc_encode_emoji( $text );
	...
	for ( $i = 0; $i < $stop; $i++ ) {
		...
		if ( '' === $ignore_block_element && strlen( $content ) > 0 && '<' !== $content[0] && str_contains( $content, '&#x' ) ) {
			foreach ( $possible_emoji as $emojum => $emoji_char ) {
				...
				$entity = sprintf( '<img src="%s" alt="%s" class="zc-smiley" style="height: 1em; max-height: 1em;" />', $cdn_url . $file . $ext, $emoji_char );
 ...
 */
function zc_staticize_emoji_for_email( $mail ) {
	 ...
	 * in zc_mail() - get the `Content-Type` from the headers, if there is one,
	 * then pass it through the {@see 'zc_mail_content_type'} filter, in case
	...
	if ( 'text/html' === $content_type ) {
		$mail['message'] = zc_staticize_emoji( $mail['message'] );
	}
 ...
 * @return array An array to match all emoji that ZelocoreCMS recognises.
 */
function _zc_emoji_list( $type = 'entities' ) {
 ...
 *
 * @since 1.2.0
 * @since 4.4.0 Moved to zc-includes/formatting.php from zc-admin/includes/misc.php and added $length param.
 *
