<?php
/**
 * Canonical API to handle ZelocoreCMS Redirecting
 *
 ...
 * not exist based on exact ZelocoreCMS query. Will instead try to parse the URL
...
function redirect_canonical( $requested_url = null, $do_redirect = true ) {
	...
	/*
	 * If we're not in zc-admin and the post has been published and preview nonce
	 * is non-existent or invalid then no need for preview in query.
	 */
	if ( is_preview() && get_query_var( 'p' ) && 'publish' === get_post_status( get_query_var( 'p' ) ) ) {
		if ( ! isset( $_GET['preview_id'] )
			|| ! isset( $_GET['preview_nonce'] )
			|| ! zc_verify_nonce( $_GET['preview_nonce'], 'post_preview_' . (int) $_GET['preview_id'] )
		) {
			$zc_query->is_preview = false;
		}
	}
	...
	// These tests give us a ZC-generated permalink.
	if ( is_404() ) {
		...
		if ( $year && $month && $day ) {
			...
			if ( ! zc_checkdate( $month, $day, $year, $date ) ) {
		...
		if ( get_query_var( 'page' ) ) {
			...
			if ( $zc_query->queried_object instanceof ZC_Post ) {
				$post_id = $zc_query->queried_object->ID;
			} elseif ( $zc_query->post ) {
				$post_id = $zc_query->post->ID;
			}
	...
	} elseif ( is_object( $zc_rewrite ) && $zc_rewrite->using_permalinks() ) {

		// Rewriting of old ?p=X, ?m=2004, ?m=200401, ?m=20040101.
		if ( is_attachment()
			&& ! array_diff( array_keys( $zc->query_vars ), array( 'attachment', 'attachment_id' ) )
			&& ! $redirect_url
		) {
			...
			$redirect_url = get_permalink( $zc_query->get_queried_object_id() );
			$redirect_obj = get_post( $zc_query->get_queried_object_id() );
			...
			if ( $term_count <= 1 && ! empty( $obj->term_id ) ) {
				...
				if ( $tax_url && ! is_zc_error( $tax_url ) ) {
					if ( ! empty( $redirect['query'] ) ) {
						...
						$rewrite_vars = array_diff( array_keys( $zc_query->query ), array_keys( $_GET ) );
		...
		} elseif ( is_single() && str_contains( $zc_rewrite->permalink_structure, '%category%' ) ) {
			$category_name = get_query_var( 'category_name' );

			if ( $category_name ) {
				$category = get_category_by_path( $category_name );

				if ( ! $category || is_zc_error( $category )
					|| ! has_term( $category->term_id, 'category', $zc_query->get_queried_object_id() )
				) {
					$redirect_url = get_permalink( $zc_query->get_queried_object_id() );
					$redirect_obj = get_post( $zc_query->get_queried_object_id() );
				}
		...
		if ( is_singular() && get_query_var( 'page' ) ) {
			...
			if ( $page > 1 ) {
				...
				if ( is_front_page() ) {
					$redirect_url .= user_trailingslashit( "$zc_rewrite->pagination_base/$page", 'paged' );
				} else {
		...
		if ( get_query_var( 'sitemap' ) ) {
			...
			while ( preg_match( "#/$zc_rewrite->pagination_base/?[0-9]+?(/+)?$#", $redirect['path'] )
				|| preg_match( '#/(comments/?)?(feed|rss2?|rdf|atom)(/+)?$#', $redirect['path'] )
				|| preg_match( "#/{$zc_rewrite->comments_pagination_base}-[0-9]+(/+)?$#", $redirect['path'] )
			) {
				// Strip off any existing paging.
				$redirect['path'] = preg_replace( "#/$zc_rewrite->pagination_base/?[0-9]+?(/+)?$#", '/', $redirect['path'] );
				// Strip off feed endings.
				$redirect['path'] = preg_replace( '#/(comments/?)?(feed|rss2?|rdf|atom)(/+|$)#', '/', $redirect['path'] );
				// Strip off any existing comment paging.
				$redirect['path'] = preg_replace( "#/{$zc_rewrite->comments_pagination_base}-[0-9]+?(/+)?$#", '/', $redirect['path'] );
			}

			$addl_path    = '';
			$default_feed = get_default_feed();

			if ( is_feed() && in_array( $feed, $zc_rewrite->feeds, true ) ) {
				...
				$old_feed_files = array(
					'zc-atom.php'         => 'atom',
					'zc-commentsrss2.php' => 'comments_rss2',
					'zc-feed.php'         => $default_feed,
					'zc-rdf.php'          => 'rdf',
					'zc-rss.php'          => 'rss2',
					'zc-rss2.php'         => 'rss2',
				);
			...
			if ( $paged > 0 ) {
				...
				if ( ! is_feed() ) {
					if ( ! is_single() ) {
						...
						if ( $paged > 1 ) {
							$addl_path .= user_trailingslashit( "$zc_rewrite->pagination_base/$paged", 'paged' );
						}
					}
				} elseif ( $paged > 1 ) {
			...
			if ( get_option( 'page_comments' )
				...
				$addl_path  = ( ! empty( $addl_path ) ? trailingslashit( $addl_path ) : '' );
				$addl_path .= user_trailingslashit( $zc_rewrite->comments_pagination_base . '-' . $cpage, 'commentpaged' );

				$redirect['query'] = remove_query_arg( 'cpage', $redirect['query'] );
			}

			// Strip off trailing /index.php/.
			$redirect['path'] = preg_replace( '|/' . preg_quote( $zc_rewrite->index, '|' ) . '/?$|', '/', $redirect['path'] );
			$redirect['path'] = user_trailingslashit( $redirect['path'] );

			if ( ! empty( $addl_path )
				&& $zc_rewrite->using_index_permalinks()
				&& ! str_contains( $redirect['path'], '/' . $zc_rewrite->index . '/' )
			) {
				$redirect['path'] = trailingslashit( $redirect['path'] ) . $zc_rewrite->index . '/';
			}
		...
		if ( 'zc-register.php' === basename( $redirect['path'] ) ) {
			if ( is_multisite() ) {
				/** This filter is documented in zc-login.php */
				$redirect_url = apply_filters( 'zc_signup_location', network_site_url( 'zc-signup.php' ) );
			} else {
				$redirect_url = zc_registration_url();
			}
	...
	if ( ! $zc_rewrite->using_index_permalinks() ) {
		$redirect['path'] = str_replace( '/' . $zc_rewrite->index . '/', '/', $redirect['path'] );
	}

	// Trailing slashes.
	if ( is_object( $zc_rewrite ) && $zc_rewrite->using_permalinks()
	...
	if ( $redirect_obj instanceof ZC_Post ) {
 ...
 *
 * If a user visits example.com/admin, they'll be redirected to /zc-admin.
 * Visiting /login redirects to /zc-login.php, and so on.
 *
 * @since 3.4.0
 *
 * @global ZC_Rewrite $zc_rewrite ZelocoreCMS rewrite component.
 */
function zc_redirect_admin_locations() {
	global $zc_rewrite;

	if ( ! ( is_404() && $zc_rewrite->using_permalinks() ) ) {
		return;
	}

	$admins = array(
		home_url( 'zc-admin', 'relative' ),
	...
	$logins = array(
		home_url( 'zc-login.php', 'relative' ),
