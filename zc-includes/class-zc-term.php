...
/**
 * Core class used to implement the ZC_Term object.
 *
 * @since 4.4.0
 *
...
final class ZC_Term {
	 ...
	 * @return ZC_Term|ZC_Error|false Term object, if found. ZC_Error if `$term_id` is shared between taxonomies and
	...
	public static function get_instance( $term_id, $taxonomy = null ) {
		...
		$_term = zc_cache_get( $term_id, 'terms' );
		...
		if ( ! $_term || ( $taxonomy && $taxonomy !== $_term->taxonomy ) ) {
			...
			if ( $taxonomy ) {
				...
				foreach ( $terms as $t ) {
					...
					if ( $_term ) {
						return new ZC_Error( 'ambiguous_term_id', __( 'Term ID is shared between multiple taxonomies' ), $term_id );
					}
			...
			if ( 1 === count( $terms ) ) {
				zc_cache_add( $term_id, $_term, 'terms' );
			}
		}

		$term_obj = new ZC_Term( $_term );
