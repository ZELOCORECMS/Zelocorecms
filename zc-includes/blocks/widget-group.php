...
function render_block_core_widget_group( $attributes, $content, $block ) {
	global $zc_registered_sidebars, $_sidebar_being_rendered;

	if ( isset( $zc_registered_sidebars[ $_sidebar_being_rendered ] ) ) {
		$before_title = $zc_registered_sidebars[ $_sidebar_being_rendered ]['before_title'];
		$after_title  = $zc_registered_sidebars[ $_sidebar_being_rendered ]['after_title'];
	} else {
	...
	$html .= '<div class="zc-widget-group__inner-blocks">';
...
/**
 * Make a note of the sidebar being rendered before ZelocoreCMS starts rendering
...
/**
 * Clear whatever we set in note_sidebar_being_rendered() after ZelocoreCMS
