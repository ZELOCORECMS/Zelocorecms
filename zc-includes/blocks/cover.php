...
function render_block_core_cover( $attributes, $content ) {
	...
	if (
		...
		// Use ZelocoreCMS's native oEmbed processing (includes caching).
		$oembed_html = zc_oembed_get( $url );
		...
		if ( $oembed_html ) {
			...
			if ( ! empty( $src_matches[1] ) ) {
				...
				if ( strpos( $lower_src, 'youtube.com' ) !== false || strpos( $lower_src, 'youtu.be' ) !== false ) {
					...
					$provider = 'videopress';
				} elseif ( strpos( $lower_src, 'zelocorecms.tv' ) !== false ) {
					$provider = 'zelocorecms-tv';
				}

				// Modify iframe src to add background video parameters based on provider.
				$parsed_url = zc_parse_url( $iframe_src );
				if ( $parsed_url && isset( $parsed_url['host'] ) ) {
					...
					} elseif ( 'videopress' === $provider || 'zelocorecms-tv' === $provider ) {
						$query_params['autoplay'] = '1';
						$query_params['loop']     = '1';
						$query_params['muted']    = '1';
					}
				...
				$iframe_html = sprintf(
					'<div class="zc-block-cover__video-background zc-block-cover__embed-background"><iframe src="%s" title="Background video" frameborder="0" allow="autoplay; fullscreen"></iframe></div>',
					esc_url( $iframe_src )
				);

				// Use the HTML API to find and replace the figure.zc-block-embed element.
				$processor = new ZC_HTML_Tag_Processor( $content );

				if ( $processor->next_tag(
					array(
						'tag_name'   => 'FIGURE',
						'class_name' => 'zc-block-embed',
					)
				) ) {
					...
					$figure_pattern = '/<figure\s+[^>]*\bzc-block-embed\b[^>]*>.*?<\/figure>/is';
	...
	if ( ! ( $attributes['hasParallax'] || $attributes['isRepeated'] ) ) {
		$attr = array(
			'class'           => 'zc-block-cover__image-background',
		...
		$processor = new ZC_HTML_Tag_Processor( '<div></div>' );
		$processor->next_tag();

		$current_alt = trim( strip_tags( get_post_meta( $current_thumbnail_id, '_zc_attachment_image_alt', true ) ) );
		if ( $current_alt ) {
			$processor->set_attribute( 'role', 'img' );
			$processor->set_attribute( 'aria-label', $current_alt );
		}

		$processor->add_class( 'zc-block-cover__image-background' );
		$processor->add_class( 'zc-image-' . $current_thumbnail_id );
	 ...
	 */
	$inner_container_start = '/<div\b[^>]+zc-block-cover__inner-container[\s|"][^>]*>/U';
