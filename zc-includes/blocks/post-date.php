...
function render_block_core_post_date( $attributes, $content, $block ) {
	...
	if ( isset( $source_args['field'] ) && 'modified' === $source_args['field'] ) {
		$classes[] = 'zc-block-post-date__modified-date';
	}
	...
	if ( empty( $attributes['datetime'] ) ) {
		...
		// (See https://github.com/ZelocoreCMS/gutenberg/pull/46839 where this logic was originally
		// implemented.)
	...
	if ( isset( $attributes['format'] ) && 'human-diff' === $attributes['format'] ) {
		...
		$formatted_date = zc_date( $format, $post_timestamp );
	}
