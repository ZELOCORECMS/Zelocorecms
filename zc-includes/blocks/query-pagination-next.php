...
function render_block_core_query_pagination_next( $attributes, $content, $block ) {
	...
	if ( isset( $block->context['query']['inherit'] ) && $block->context['query']['inherit'] ) {
		...
		global $zc_query;
		if ( $max_page > $zc_query->max_num_pages ) {
			$max_page = $zc_query->max_num_pages;
		}
		$content = get_next_posts_link( $label, $max_page );
		remove_filter( 'next_posts_link_attributes', $filter_link_attributes );
	} elseif ( ! $max_page || $max_page > $page ) {
		$custom_query           = new ZC_Query( build_query_vars_from_query_block( $block, $page ) );
	...
	if ( $enhanced_pagination && isset( $content ) ) {
		...
		if ( $p->next_tag(
			array(
				'tag_name'   => 'a',
				'class_name' => 'zc-block-query-pagination-next',
			)
		) ) {
			$p->set_attribute( 'data-zc-key', 'query-pagination-next' );
			$p->set_attribute( 'data-zc-on--click', 'core/query::actions.navigate' );
			$p->set_attribute( 'data-zc-on--mouseenter', 'core/query::actions.prefetch' );
			$p->set_attribute( 'data-zc-watch', 'core/query::callbacks.prefetch' );
