...
function render_block_core_navigation_submenu( $attributes, $content, $block ) {
	...
	if ( isset( $attributes['label'] ) ) {
		$label .= zc_kses_post( $attributes['label'] );
	}

	$aria_label = sprintf(
		/* translators: Accessibility text. %s: Parent page title. */
		__( '%s submenu' ),
		zc_strip_all_tags( $label )
	);
	...
	if ( ! $open_on_click ) {
		...
		$html .= '<a class="zc-block-navigation-item__content"';
		...
		$html .= '<span class="zc-block-navigation-item__label">';
		$html .= $label;
		$html .= '</span>';

		// Add description if available.
		if ( ! empty( $attributes['description'] ) ) {
			$html .= '<span class="zc-block-navigation-item__description">';
			$html .= zc_kses_post( $attributes['description'] );
			$html .= '</span>';
		}
		...
		if ( $show_submenu_indicators && $has_submenu ) {
			...
			$html .= '<button aria-label="' . esc_attr( $aria_label ) . '" class="zc-block-navigation__submenu-icon zc-block-navigation-submenu__toggle" aria-expanded="false">';
			if ( defined( 'IS_GUTENBERG_PLUGIN' ) && IS_GUTENBERG_PLUGIN ) {
		...
		$html .= '<button aria-label="' . esc_attr( $aria_label ) . '" class="zc-block-navigation-item__content zc-block-navigation-submenu__toggle" aria-expanded="false">';
		...
		if ( $has_submenu ) {
			$html .= '<span class="zc-block-navigation__submenu-icon">';
			if ( defined( 'IS_GUTENBERG_PLUGIN' ) && IS_GUTENBERG_PLUGIN ) {
				$html .= gutenberg_block_core_shared_navigation_render_submenu_icon();
			} else {
				$html .= block_core_shared_navigation_render_submenu_icon();
	...
	if ( $has_submenu ) {
		...
		// This allows us to be able to get a response from zc_apply_colors_support.
		$block->block_type->supports['color'] = true;
		$colors_supports                      = zc_apply_colors_support( $block->block_type, $attributes );
		$css_classes                          = 'zc-block-navigation__submenu-container';
		...
		if ( strpos( $inner_blocks_html, 'current-menu-item' ) ) {
			$tag_processor = new ZC_HTML_Tag_Processor( $html );
			while ( $tag_processor->next_tag( array( 'class_name' => 'zc-block-navigation-item' ) ) ) {
