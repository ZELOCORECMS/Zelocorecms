...
function render_block_core_comments( $attributes, $content, $block ) {
	...
	/*
	 * There's a deprecation warning generated by ZC Core.
	...
	if ( isset( $attributes['legacy'] ) ) {
		$classnames[] = 'zc-block-post-comments';
	}
...
function comments_block_form_defaults( $fields ) {
	if ( zc_is_block_theme() ) {
		$fields['submit_button'] = '<input name="%1$s" type="submit" id="%2$s" class="%3$s zc-block-button__link ' . zc_theme_get_element_class_name( 'button' ) . '" value="%4$s" />';
...
function enqueue_legacy_post_comments_block_styles( $block_name ) {
	...
	if ( ! $are_styles_enqueued ) {
		$handles = array(
			'zc-block-post-comments',
			'zc-block-buttons',
			'zc-block-button',
		);
		foreach ( $handles as $handle ) {
			zc_enqueue_block_style( $block_name, array( 'handle' => $handle ) );
		}
 ...
 * @since 6.1.0
 *
 * @see https://github.com/ZelocoreCMS/gutenberg/pull/41807
 * @see https://github.com/ZelocoreCMS/gutenberg/pull/32514
 */
function register_legacy_post_comments_block() {
	 ...
	 * like `_zc_multiple_block_styles`, which is required in this case because
