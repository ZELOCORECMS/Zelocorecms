...
function block_core_gallery_render( $attributes, $content, $block ) {
	// Adds a style tag for the --zc--style--unstable-gallery-gap var.
	...
	if ( is_array( $gap ) ) {
		foreach ( $gap as $key => $value ) {
			...
			if ( is_string( $value ) && str_contains( $value, 'var:preset|spacing|' ) ) {
				...
				$value           = "var(--zc--preset--spacing--$slug)";
			}
		...
		if ( is_string( $gap ) && str_contains( $gap, 'var:preset|spacing|' ) ) {
			...
			$slug            = _zc_to_kebab_case( substr( $gap, $index_to_splice ) );
			$gap             = "var(--zc--preset--spacing--$slug)";
		}
	}

	$unique_gallery_classname = zc_unique_id( 'zc-block-gallery-' );
	$processed_content        = new ZC_HTML_Tag_Processor( $content );
	$processed_content->next_tag();
	$processed_content->add_class( $unique_gallery_classname );

	// --gallery-block--gutter-size is deprecated. --zc--style--gallery-gap-default should be used by themes that want to set a default
	// gap on the gallery.
	$fallback_gap = 'var( --zc--style--gallery-gap-default, var( --gallery-block--gutter-size, var( --zc--style--block-gap, 0.5em ) ) )';
	...
	$gallery_styles = array(
		array(
			'selector'     => ".zc-block-gallery.{$unique_gallery_classname}",
			'declarations' => array(
				'--zc--style--unstable-gallery-gap' => $gap_column,
				'gap'                               => $gap_value,
			),
		),
	);
	...
	// The ZC_HTML_Tag_Processor class calls get_updated_html() internally
	 ...
	 *
	 * @see: https://github.com/ZelocoreCMS/gutenberg/pull/58733
	 */
	if ( ! empty( $attributes['randomOrder'] ) ) {
		// This pattern matches figure elements with the `zc-block-image`
		// class to avoid the gallery's wrapping `figure` element and
		// extract images only.
		$pattern = '/<figure[^>]*\bzc-block-image\b[^>]*>.*?<\/figure>/s';
	...
	$state      = zc_interactivity_state( 'core/image' );
	...
	if ( ! empty( $image_ids ) ) {
		...
		$processor->next_tag();
		$processor->set_attribute( 'data-zc-interactive', 'core/gallery' );
		...
		while ( $processor->next_tag( 'figure' ) ) {
			$zc_key = $processor->get_attribute( 'data-zc-key' );
			if ( $zc_key && isset( $state['metadata'][ $zc_key ] ) ) {
				$alt = $state['metadata'][ $zc_key ]['alt'];
