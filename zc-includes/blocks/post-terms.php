...
function render_block_core_post_terms( $attributes, $content, $block ) {
	...
	if ( isset( $attributes['prefix'] ) && $attributes['prefix'] ) {
		$prefix .= '<span class="zc-block-post-terms__prefix">' . $attributes['prefix'] . '</span>';
	}

	$suffix = '</div>';
	if ( isset( $attributes['suffix'] ) && $attributes['suffix'] ) {
		$suffix = '<span class="zc-block-post-terms__suffix">' . $attributes['suffix'] . '</span>' . $suffix;
	}
	...
	$post_terms = get_the_term_list(
		...
		$attributes['term'],
		zc_kses_post( $prefix ),
		'<span class="zc-block-post-terms__separator">' . esc_html( $separator ) . '</span>',
		zc_kses_post( $suffix )
	);

	if ( is_zc_error( $post_terms ) || empty( $post_terms ) ) {
		return '';
	}
