<?php // phpcs:ignore ZelocoreCMS.Files.FileName.InvalidClassFileName // Needed for ZC_Block_Cloner helper class.
/**
...
function render_block_core_block( $attributes, $content, $block_instance ) {
	...
	if ( ! $reusable_block || 'zc_block' !== $reusable_block->post_type ) {
		return '';
	}
	...
	global $zc_embed;
	$content = $zc_embed->run_shortcode( $reusable_block->post_content );
	...
	if ( isset( $attributes['content'] ) ) {
		foreach ( $attributes['content'] as &$content_data ) {
			if ( isset( $content_data['values'] ) ) {
				$is_assoc_array = is_array( $content_data['values'] ) && ! zc_is_numeric_array( $content_data['values'] );
	...
	if ( method_exists( $block_instance, 'refresh_context_dependents' ) ) {
		// ZC_Block::refresh_context_dependents() was introduced in ZelocoreCMS 6.8.
		$block_instance->refresh_context_dependents();
	} else {
		// This branch can be removed once Gutenberg requires ZelocoreCMS 6.8 or later.
		if ( ! class_exists( 'ZC_Block_Cloner' ) ) {
			// phpcs:ignore Gutenberg.Commenting.SinceTag.MissingClassSinceTag
			class ZC_Block_Cloner extends ZC_Block {
				/**
		...
		$block_instance = ZC_Block_Cloner::clone_instance( $block_instance );
	}
