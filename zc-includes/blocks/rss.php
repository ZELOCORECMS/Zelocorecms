...
function render_block_core_rss( $attributes ) {
	...
	foreach ( $rss_items as $item ) {
		...
		$title = "<div class='zc-block-rss__item-title'>{$title}</div>";
		...
		if ( ! empty( $attributes['displayDate'] ) ) {
			...
			if ( $timestamp ) {
				...
				$date_markup = sprintf(
					'<time datetime="%1$s" class="zc-block-rss__item-publish-date">%2$s</time> ',
		...
		if ( $attributes['displayAuthor'] ) {
			...
			if ( is_object( $author ) ) {
				...
				if ( ! empty( $author ) ) {
					$author = '<span class="zc-block-rss__item-author">' . sprintf(
		...
		if ( $attributes['displayExcerpt'] && ! empty( $description ) ) {
			...
			$excerpt = esc_attr( zc_trim_words( $excerpt, $attributes['excerptLength'], ' [&hellip;]' ) );
			...
			$excerpt = '<div class="zc-block-rss__item-excerpt">' . esc_html( $excerpt ) . '</div>';
		}

		$list_items .= "<li class='zc-block-rss__item'>{$title}{$date_markup}{$author}{$excerpt}</li>";
	}
