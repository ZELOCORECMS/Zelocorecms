...
function block_core_accordion_item_render( array $attributes, string $content ): string {
	...
	$unique_id = zc_unique_id( 'accordion-item-' );
	...
	zc_interactivity_state(
		...
		array(
			'isOpen' => function () {
				$context = zc_interactivity_get_context();
				return $context['openByDefault'];
			},
		)
	);

	if ( $p->next_tag( array( 'class_name' => 'zc-block-accordion-item' ) ) ) {
		$open_by_default = $attributes['openByDefault'] ? 'true' : 'false';
		$p->set_attribute( 'data-zc-context', '{ "id": "' . $unique_id . '", "openByDefault": ' . $open_by_default . ' }' );
		$p->set_attribute( 'data-zc-class--is-open', 'state.isOpen' );
		$p->set_attribute( 'data-zc-init', 'callbacks.initAccordionItems' );
		$p->set_attribute( 'data-zc-on-window--hashchange', 'callbacks.hashChange' );

		if ( $p->next_tag( array( 'class_name' => 'zc-block-accordion-heading__toggle' ) ) ) {
			$p->set_attribute( 'data-zc-on--click', 'actions.toggle' );
			$p->set_attribute( 'data-zc-on--keydown', 'actions.handleKeyDown' );
			$p->set_attribute( 'id', $unique_id );
			$p->set_attribute( 'aria-controls', $unique_id . '-panel' );
			$p->set_attribute( 'data-zc-bind--aria-expanded', 'state.isOpen' );

			if ( $p->next_tag( array( 'class_name' => 'zc-block-accordion-panel' ) ) ) {
				$p->set_attribute( 'id', $unique_id . '-panel' );
				$p->set_attribute( 'aria-labelledby', $unique_id );
				$p->set_attribute( 'data-zc-bind--inert', '!state.isOpen' );
