...
function render_block_core_post_excerpt( $attributes, $content, $block ) {
	...
	$more_text           = ! empty( $attributes['moreText'] ) ? '<a class="zc-block-post-excerpt__more-link" href="' . esc_url( get_the_permalink( $block->context['postId'] ) ) . '">' . zc_kses_post( $attributes['moreText'] ) . '</a>' : '';
	 ...
	 * so that zc_trim_excerpt doesn't pre-trim the content before zc_trim_words can
	...
	if ( isset( $excerpt_length ) ) {
		$excerpt = zc_trim_words( $excerpt, $excerpt_length );
	}
	...
	$content               = '<p class="zc-block-post-excerpt__excerpt">' . $excerpt;
	$show_more_on_new_line = ! isset( $attributes['showMoreOnNewLine'] ) || $attributes['showMoreOnNewLine'];
	if ( $show_more_on_new_line && ! empty( $more_text ) ) {
		$content .= '</p><p class="zc-block-post-excerpt__more-text">' . $more_text . '</p>';
	} else {
		$content .= " $more_text</p>";
	}
 ...
 * zc_trim_words can detect when trimming is needed and add the ellipsis.
 *
 * @since 7.0.0
 *
