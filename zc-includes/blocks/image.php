 ...
 * @param ZC_Block $block      The block object.
 *
...
function render_block_core_image( $attributes, $content, $block ) {
	...
	$processor = new class( $content ) extends ZC_HTML_Tag_Processor {
		/**
		 ...
		 * @since 6.9.0
		 *
		 * @return ZC_HTML_Span|false Span of input if the element is empty; otherwise false.
		 */
		public function block_core_image_extract_empty_figcaption_element() {
			...
			return new ZC_HTML_Span( $opener->start, $closer->start + $closer->length - $opener->start );
		}
	};
	...
	// Ensure the `zc-image-id` classname on the image block supports block bindings.
	if ( $has_id_binding ) {
		// If there's a mismatch with the 'zc-image-' class and the actual id, the id was
		// probably overridden by block bindings. Update it to the correct value.
		// See https://github.com/ZelocoreCMS/gutenberg/issues/62886 for why this is needed.
		$id                       = $attributes['id'];
		$image_classnames         = $processor->get_attribute( 'class' );
		$class_with_binding_value = "zc-image-$id";
		if ( is_string( $image_classnames ) && ! str_contains( $image_classnames, $class_with_binding_value ) ) {
			$image_classnames = preg_replace( '/zc-image-(\d+)/', $class_with_binding_value, $image_classnames );
	...
	if (
		...
		zc_enqueue_script_module( '@zelocorecms/block-library/image/view' );
...
function block_core_image_get_lightbox_settings( $block ) {
	...
	if ( ! isset( $lightbox_settings ) ) {
		$lightbox_settings = zc_get_global_settings( array( 'lightbox' ), array( 'block_name' => 'core/image' ) );

		// If not present in global settings, check the top-level global settings.
		//
		// NOTE: If no block-level settings are found, the previous call to
		// `zc_get_global_settings` will return the whole `theme.json` structure in
		// which case we can check if the "lightbox" key is present at the top-level
		// of the global settings and use its value.
		if ( isset( $lightbox_settings['lightbox'] ) ) {
			$lightbox_settings = zc_get_global_settings( array( 'lightbox' ) );
		}
	}
...
function block_core_image_render_lightbox( $block_content, $block, $block_instance ) {
	 ...
	 * lightbox behavior.
	 */
	$processor = new ZC_HTML_Tag_Processor( $block_content );
	...
	if ( isset( $block['attrs']['id'] ) ) {
		$img_uploaded_src = zc_get_attachment_url( $block['attrs']['id'] );
		$img_metadata     = zc_get_attachment_metadata( $block['attrs']['id'] );
		...
		$img_srcset       = zc_get_attachment_image_srcset( $block['attrs']['id'], $srcset_size );
	...
	$processor->add_class( 'zc-lightbox-container' );
	$processor->set_attribute( 'data-zc-interactive', 'core/image' );
	...
	$processor->set_attribute( 'data-zc-key', $unique_image_id );

	// Image.
	$processor->next_tag( 'img' );
	$processor->set_attribute( 'data-zc-init', 'callbacks.setButtonStyles' );
	$processor->set_attribute( 'data-zc-on--load', 'callbacks.setButtonStyles' );
	$processor->set_attribute( 'data-zc-on-window--resize', 'callbacks.setButtonStyles' );
	...
	$processor->set_attribute( 'data-zc-on--pointerenter', 'actions.preloadImageWithDelay' );
	$processor->set_attribute( 'data-zc-on--pointerdown', 'actions.preloadImage' );
	$processor->set_attribute( 'data-zc-on--pointerleave', 'actions.cancelPreload' );

	// Sets an event callback on the `img` because the `figure` element can also
	// contain a caption, and we don't want to trigger the lightbox when the
	// caption is clicked.
	$processor->set_attribute( 'data-zc-on--click', 'actions.showLightbox' );
	$processor->set_attribute( 'data-zc-class--hide', 'state.isContentHidden' );
	$processor->set_attribute( 'data-zc-class--show', 'state.isContentVisible' );
	...
	$button =
		...
		. '<button
			...
			data-zc-bind--aria-label="state.thisImage.triggerButtonAriaLabel"
			data-zc-init="callbacks.initTriggerButton"
			data-zc-on--click="actions.showLightbox"
			data-zc-style--right="state.thisImage.buttonRight"
			data-zc-style--top="state.thisImage.buttonTop"
		>
	...
	add_action( 'zc_footer', 'block_core_image_print_lightbox_overlay' );
...
function block_core_image_print_lightbox_overlay() {
	...
	if ( zc_theme_has_theme_json() ) {
		$global_styles_color = zc_get_global_styles( array( 'color' ) );
	...
	echo <<<HTML
		<div
			class="zc-lightbox-overlay zoom"
			aria-label="{$dialog_label}"
			data-zc-interactive="core/image"
			data-zc-router-region='{ "id": "core/image-overlay", "attachTo": "body" }'
			data-zc-key="zc-lightbox-overlay"
			data-zc-context='{}'
			data-zc-bind--role="state.roleAttribute"
			data-zc-bind--aria-label="state.ariaLabel"
			data-zc-bind--aria-modal="state.ariaModal"
			data-zc-class--active="state.overlayEnabled"
			data-zc-class--show-closing-animation="state.overlayOpened"
			data-zc-watch---focus="callbacks.setOverlayFocus"
			data-zc-watch---inert="callbacks.setInertElements"
			data-zc-on--keydown="actions.handleKeydown"
			data-zc-on--touchstart="actions.handleTouchStart"
			data-zc-on--touchmove="actions.handleTouchMove"
			data-zc-on--touchend="actions.handleTouchEnd"
			data-zc-on--click="actions.hideLightbox"
			data-zc-on-window--resize="callbacks.setOverlayStyles"
			data-zc-on-window--scroll="actions.handleScroll"
			data-zc-bind--style="state.overlayStyles"
			tabindex="-1"
			>
				<button type="button" style="fill:{$close_button_color}" class="zc-lightbox-close-button" data-zc-bind--aria-label="state.closeButtonAriaLabel">
					<span class="zc-lightbox-close-icon" data-zc-bind--hidden="!state.hasNavigationIcon">{$close_button_icon}</span>
					<span class="zc-lightbox-close-text" data-zc-bind--hidden="!state.hasNavigationText">{$close_button_text}</span>
				</button>
				<button type="button" style="fill:{$close_button_color}" class="zc-lightbox-navigation-button zc-lightbox-navigation-button-prev" data-zc-bind--hidden="!state.hasNavigation" data-zc-on--click="actions.showPreviousImage" data-zc-bind--aria-label="state.prevButtonAriaLabel">
					<span class="zc-lightbox-navigation-icon" data-zc-bind--hidden="!state.hasNavigationIcon">{$prev_button_icon}</span>
					<span class="zc-lightbox-navigation-text" data-zc-bind--hidden="!state.hasNavigationText">{$prev_button_text}</span>
				</button>
				<div class="lightbox-image-container">
					<figure data-zc-bind--class="state.selectedImage.figureClassNames" data-zc-bind--style="state.figureStyles">
						<img data-zc-bind--alt="state.selectedImage.alt" data-zc-bind--class="state.selectedImage.imgClassNames" data-zc-bind--style="state.imgStyles" data-zc-bind--src="state.selectedImage.currentSrc">
					</figure>
				</div>
				<div class="lightbox-image-container">
					<figure data-zc-bind--class="state.selectedImage.figureClassNames" data-zc-bind--style="state.figureStyles">
						<img
							data-zc-bind--alt="state.selectedImage.alt"
							data-zc-bind--class="state.selectedImage.imgClassNames"
							data-zc-bind--style="state.imgStyles"
							data-zc-bind--src="state.enlargedSrc"
							data-zc-bind--srcset="state.enlargedSrcset"
							data-zc-bind--srcset="state.enlargedSrcset"
							sizes="100vw"
						>
					</figure>
				</div>
				<button type="button" style="fill:{$close_button_color}" class="zc-lightbox-navigation-button zc-lightbox-navigation-button-next" data-zc-bind--hidden="!state.hasNavigation" data-zc-on--click="actions.showNextImage" data-zc-bind--aria-label="state.nextButtonAriaLabel">
					<span class="zc-lightbox-navigation-text" data-zc-bind--hidden="!state.hasNavigationText">{$next_button_text}</span>
					<span class="zc-lightbox-navigation-icon" data-zc-bind--hidden="!state.hasNavigationIcon">{$next_button_icon}</span>
				</button>
				<div data-zc-text="state.ariaLabel" aria-live="polite" aria-atomic="true" class="screen-reader-text"></div>
