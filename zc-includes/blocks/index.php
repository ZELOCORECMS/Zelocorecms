...
function register_core_block_style_handles() {
	...
	$files          = false;
	$transient_name = 'zc_core_block_css_files';

	/*
	 * Ignore transient cache when the development mode is set to 'core'. Why? To avoid interfering with
	 * the core developer's workflow.
	 */
	$can_use_cached = ! zc_is_development_mode( 'core' );

	if ( $can_use_cached ) {
		$cached_files = get_transient( $transient_name );

		// Check the validity of cached values by checking against the current ZelocoreCMS version.
		if (
			is_array( $cached_files )
			&& isset( $cached_files['version'] )
			&& $cached_files['version'] === $zc_version
	...
	if ( ! $files ) {
		$files = glob( zc_normalize_path( BLOCKS_PATH . '**/**.css' ) );

		// Normalize BLOCKS_PATH prior to substitution for Windows environments.
		$normalized_blocks_path = zc_normalize_path( BLOCKS_PATH );
	...
	$register_style = static function ( $name, $filename, $style_handle ) use ( $blocks_url, $suffix, $zc_styles, $files ) {
		$style_path = "{$name}/{$filename}{$suffix}.css";
		$path       = zc_normalize_path( BLOCKS_PATH . $style_path );
		...
		$zc_styles->add( $style_handle, $blocks_url . $style_path );
		$zc_styles->add_data( $style_handle, 'path', $path );

		$rtl_file = "{$name}/{$filename}-rtl{$suffix}.css";
		if ( is_rtl() && in_array( $rtl_file, $files, true ) ) {
			$zc_styles->add_data( $style_handle, 'rtl', 'replace' );
			$zc_styles->add_data( $style_handle, 'suffix', $suffix );
			$zc_styles->add_data( $style_handle, 'path', str_replace( "{$suffix}.css", "-rtl{$suffix}.css", $path ) );
		}
	};
	...
	foreach ( $core_blocks_meta as $name => $schema ) {
		...
		if ( ! isset( $schema['style'] ) ) {
			$schema['style'] = "zc-block-{$name}";
		}
		if ( ! isset( $schema['editorStyle'] ) ) {
			$schema['editorStyle'] = "zc-block-{$name}-editor";
		}

		// Register block theme styles.
		$register_style( $name, 'theme', "zc-block-{$name}-theme" );
 ...
 * @since 6.7.0
 */
function zc_register_core_block_metadata_collection() {
	zc_register_block_metadata_collection(
		BLOCKS_PATH,
		BLOCKS_PATH . 'blocks-json.php'
	);
}
add_action( 'init', 'zc_register_core_block_metadata_collection', 9 );
