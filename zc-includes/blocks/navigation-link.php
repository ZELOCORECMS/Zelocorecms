...
function block_core_navigation_link_maybe_urldecode( $url ) {
	...
	$query_params   = zc_parse_args( $query );
...
function render_block_core_navigation_link( $attributes, $content, $block ) {
	...
	$wrapper_attributes = get_block_wrapper_attributes(
		array(
			'class' => $css_classes . ' zc-block-navigation-item' . ( $has_submenu ? ' has-child' : '' ) .
				( $is_active ? ' current-menu-item' : '' ),
			'style' => $style_attribute,
		)
	);
	$html               = '<li ' . $wrapper_attributes . '>' .
		'<a class="zc-block-navigation-item__content" ';
	...
	$html .= '>' .
		...
		'<span class="zc-block-navigation-item__label">';

	if ( isset( $attributes['label'] ) ) {
		$html .= zc_kses_post( $attributes['label'] );
	}
	...
	if ( $has_submenu ) {
		$html .= sprintf(
			'<ul class="zc-block-navigation__submenu-container">%s</ul>',
			$inner_blocks_html
		);
	}
 ...
 * @since 5.9.0
 *
 * @param ZC_Taxonomy|ZC_Post_Type $entity post type or taxonomy entity.
...
function build_variation_for_navigation_link( $entity, $kind ) {
	...
	$default_labels = null;
	if ( $entity instanceof ZC_Post_Type ) {
		$default_labels = ZC_Post_Type::get_default_labels();
	} elseif ( $entity instanceof ZC_Taxonomy ) {
		$default_labels = ZC_Taxonomy::get_default_labels();
	}
 ...
 * @param ZC_Block_Type $block_type The full block type object.
...
function block_core_navigation_link_filter_variations( $variations, $block_type ) {
	 ...
	 *
	 * See: https://github.com/ZelocoreCMS/gutenberg/pull/72517
	 */
