 ...
 *
 * Backward compatibility: ZelocoreCMS applies default attribute values, so submenuVisibility
...
function block_core_navigation_get_submenu_visibility( $attributes ) {
	...
	// Legacy blocks have openSubmenusOnClick in the database. Since ZelocoreCMS applies
 ...
 * @since 6.5.0
 */
class ZC_Navigation_Block_Renderer {
	 ...
	 * @since 6.5.0
	 *
	 * @param ZC_Block_List $inner_blocks The list of inner blocks.
	 ...
	 * @since 6.5.0
	 *
	 * @param ZC_Block $block The block.
	 ...
	 * @since 6.5.0
	 *
	 * @param ZC_Block $inner_block The inner block.
	...
	private static function get_markup_for_inner_block( $inner_block ) {
		...
		if ( ! empty( $inner_block_content ) ) {
			if ( static::does_block_need_a_list_item_wrapper( $inner_block ) ) {
				return '<li class="zc-block-navigation-item">' . $inner_block_content . '</li>';
			}
		}
	 ...
	 * @since 6.5.0
	 *
	 * @param ZC_Block_List $blocks The list of blocks to render.
	...
	private static function get_inner_blocks_html( $attributes, $inner_blocks ) {
		...
		$container_attributes = get_block_wrapper_attributes(
			array(
				'class' => 'zc-block-navigation__container ' . $class,
				'style' => $style,
			)
		);
		...
		foreach ( $inner_blocks as $inner_block ) {
			...
			$p                  = new ZC_HTML_Tag_Processor( $inner_block_markup );
		...
		if ( $has_submenus && $is_interactive ) {
			$tags              = new ZC_HTML_Tag_Processor( $inner_blocks_html );
	 ...
	 *
	 * @param array $attributes The block attributes.
	 * @return ZC_Block_List Returns the inner blocks for the navigation block.
	 */
	private static function get_inner_blocks_from_navigation_post( $attributes ) {
		$navigation_post = get_post( $attributes['ref'] );
		if ( ! isset( $navigation_post ) ) {
			return new ZC_Block_List( array(), $attributes );
		}
		...
		if ( 'publish' === $navigation_post->post_status ) {
			...
			return new ZC_Block_List( $blocks, $attributes );
		}
	}
	...
	private static function get_inner_blocks_from_fallback( $attributes ) {
		...
		return new ZC_Block_List( $fallback_blocks, $attributes );
	}
	 ...
	 * @param array  $attributes                The block attributes.
	 * @return ZC_Block_List Returns the inner blocks for the overlay template part.
	 */
	private static function get_overlay_blocks_from_template_part( $overlay_template_part_id, $attributes ) {
		...
		$template_part_query = new ZC_Query(
			array(
				'post_type'           => 'zc_template_part',
				'post_status'         => 'publish',
		...
		if ( ! $template_part_post ) {
			...
			$block_template        = get_block_file_template( $full_template_part_id, 'zc_template_part' );
	 ...
	 *
	 * @param array    $attributes The block attributes.
	 * @param ZC_Block $block The parsed block.
	 * @return ZC_Block_List Returns the inner blocks for the navigation block.
	 */
	private static function get_inner_blocks( $attributes, $block ) {
		...
		// - we don't have a relationship to a `zc_navigation` Post (via `ref`).
		 ...
		 * @since 6.1.0
		 *
		 * @param \ZC_Block_List $inner_blocks
		 */
	...
	private static function get_responsive_container_classes( $is_hidden_by_default, $has_custom_overlay, $colors ) {
		$responsive_container_classes = array( 'zc-block-navigation__responsive-container' );
	...
	private static function get_responsive_container_markup( $attributes, $inner_blocks, $inner_blocks_html ) {
		$is_interactive  = static::is_interactive( $attributes, $inner_blocks );
		$colors          = block_core_navigation_build_css_colors( $attributes );
		$modal_unique_id = zc_unique_id( 'modal-' );
		...
		if ( ! empty( $attributes['overlay'] ) ) {
			...
			if ( $has_custom_overlay_close_block && $is_interactive ) {
				$tags                = new ZC_HTML_Tag_Processor( $overlay_blocks_html );
			...
			// fetchpriority="low" so that when zc_filter_content_tags() processes the
			// parent template part, it sees the attribute already present and calls
			// zc_get_loading_optimization_attributes() with fetchpriority="low", which both prevents
		...
		$open_button_classes = array(
			'zc-block-navigation__responsive-container-open',
		...
		if ( $is_interactive ) {
			$open_button_directives                  = '
				data-zc-on--click="actions.openMenuOnClick"
				data-zc-on--keydown="actions.handleMenuKeydown"
			';
			$responsive_container_directives         = '
				data-zc-class--has-modal-open="state.isMenuOpen"
				data-zc-class--is-menu-open="state.isMenuOpen"
				data-zc-watch="callbacks.initMenu"
				data-zc-on--keydown="actions.handleMenuKeydown"
				data-zc-on--focusout="actions.handleMenuFocusout"
				tabindex="-1"
			';
			$responsive_dialog_directives            = '
				data-zc-bind--aria-modal="state.ariaModal"
				data-zc-bind--aria-label="state.ariaLabel"
				data-zc-bind--role="state.roleAttribute"
			';
			$close_button_directives                 = '
				data-zc-on--click="actions.closeMenuOnClick"
			';
			$responsive_container_content_directives = '
				data-zc-watch="callbacks.focusFirstElement"
			';
		}
		...
		if ( $has_custom_overlay ) {
			$custom_overlay_markup = sprintf(
				'<div class="zc-block-navigation__overlay-container">%s</div>',
				$overlay_blocks_html
			);
		}
		...
		if ( ! $has_custom_overlay_close_block ) {
			$close_button_markup = sprintf(
				'<button %1$s class="zc-block-navigation__responsive-container-close" %2$s>%3$s</button>',
		...
		return sprintf(
			'<button aria-haspopup="dialog" %3$s class="%6$s" %10$s>%8$s</button>
				<div class="%5$s" %7$s id="%1$s" %11$s>
					<div class="zc-block-navigation__responsive-close" tabindex="-1">
						<div class="zc-block-navigation__responsive-dialog" %12$s>
							%13$s
							<div class="zc-block-navigation__responsive-container-content" %14$s id="%1$s-content">
								%2$s
								%15$s
							</div>
						</div>
	 ...
	 *
	 * @param array         $attributes    The block attributes.
	 * @param ZC_Block_List $inner_blocks  A list of inner blocks.
	...
	private static function get_nav_element_directives( $is_interactive ) {
		...
		$nav_element_context    = zc_interactivity_data_zc_context(
			array(
		...
		$nav_element_directives = '
		 data-zc-interactive="core/navigation" '
		. $nav_element_context;
	...
	private static function handle_view_script_module_loading( $attributes, $block, $inner_blocks ) {
		if ( static::is_interactive( $attributes, $inner_blocks ) ) {
			zc_enqueue_script_module( '@zelocorecms/block-library/navigation/view' );
		}
	}
...
if ( defined( 'IS_GUTENBERG_PLUGIN' ) && IS_GUTENBERG_PLUGIN ) {
	/**
	 * Returns the menu items for a ZelocoreCMS menu location.
	 *
	 * @since 5.9.0
	 *
	...
	function block_core_navigation_get_menu_items_at_location( $location ) {
		...
		// `zc_nav_menu()` and `gutenberg_output_block_nav_menu`.
		...
		$menu = zc_get_nav_menu_object( $locations[ $location ] );
		if ( ! $menu || is_zc_error( $menu ) ) {
			return;
		}

		$menu_items = zc_get_nav_menu_items( $menu->term_id, array( 'update_post_term_cache' => false ) );
		_zc_menu_item_classes_by_context( $menu_items );
	...
	function block_core_navigation_get_inner_blocks_from_unstable_location( $attributes ) {
		...
		return new ZC_Block_List( $parsed_blocks, $attributes );
	}
}
 ...
 *
 * Uses ZC_HTML_Tag_Processor to detect the close button in rendered output,
...
function block_core_navigation_overlay_html_has_close_block( $html ) {
	$tags = new ZC_HTML_Tag_Processor( $html );
	return $tags->next_tag(
		array(
			'tag_name'   => 'BUTTON',
			'class_name' => 'zc-block-navigation-overlay-close',
		)
	);
}
 ...
 * @since 6.5.0
 *
 * @param ZC_HTML_Tag_Processor $tags Markup of the navigation block.
...
function block_core_navigation_add_directives_to_overlay_close( $tags ) {
	...
	while ( $tags->next_tag(
		...
		$tags->set_attribute( 'data-zc-on--click', 'actions.closeMenuOnClick' );
	}
...
function block_core_navigation_add_directives_to_submenu( $tags, $block_attributes ) {
	while ( $tags->next_tag(
		...
		$tags->set_attribute( 'data-zc-interactive', 'core/navigation' );
		$tags->set_attribute( 'data-zc-context', '{ "submenuOpenedBy": { "click": false, "hover": false, "focus": false }, "type": "submenu", "modal": null, "previousFocus": null }' );
		$tags->set_attribute( 'data-zc-watch', 'callbacks.initMenu' );
		$tags->set_attribute( 'data-zc-on--focusout', 'actions.handleMenuFocusout' );
		$tags->set_attribute( 'data-zc-on--keydown', 'actions.handleMenuKeydown' );
		...
		if ( $open_on_hover ) {
			$tags->set_attribute( 'data-zc-on--pointerenter', 'actions.openMenuOnHover' );
			$tags->set_attribute( 'data-zc-on--pointerleave', 'actions.closeMenuOnHover' );
		}
		...
		if ( $tags->next_tag(
			array(
				'tag_name'   => 'BUTTON',
				'class_name' => 'zc-block-navigation-submenu__toggle',
			)
		) ) {
			$tags->set_attribute( 'data-zc-on--click', 'actions.toggleMenuOnClick' );
			$tags->set_attribute( 'data-zc-bind--aria-expanded', 'state.isMenuOpen' );
		...
		if ( $tags->next_tag(
			array(
				'tag_name'   => 'UL',
				'class_name' => 'zc-block-navigation__submenu-container',
			)
		) ) {
			$tags->set_attribute( 'data-zc-on--focus', 'actions.openMenuOnFocus' );
		}
 ...
 * @since 7.0.0
 *
 * @param ZC_Block_List $blocks           The list of blocks to check.
 ...
 *
 * @param ZC_Block_List $inner_blocks Inner block instance to be normalized.
 ...
 *
 * This aims to mirror how the fallback mechanic for zc_nav_menu works.
 * See https://developer.zelocorecms.org/reference/functions/zc_nav_menu/#more-information.
 *
 * @since 5.9.0
 *
...
function block_core_navigation_get_fallback_blocks() {
	...
	$navigation_post = ZC_Navigation_Fallback::get_fallback();
 ...
 * @since 6.0.0
 *
 * @param ZC_Block_List $inner_blocks Block list class instance.
 *
 ...
 * @since 6.0.0
 *
 * @param ZC_Block $block Instance of a block.
 *
 * @return array Array of post IDs.
 */
...
function render_block_core_navigation( $attributes, $content, $block ) {
	return ZC_Navigation_Block_Renderer::render( $attributes, $content, $block );
}
 ...
 * @throws ZC_Error An ZC_Error exception parsing the block definition.
 */
 ...
 * @since 5.9.0
 *
 * @deprecated 6.3.0 Use ZC_Navigation_Fallback::parse_blocks_from_menu_items() instead.
 *
...
function block_core_navigation_parse_blocks_from_menu_items( $menu_items, $menu_items_by_parent_id ) {
	...
	_deprecated_function( __FUNCTION__, '6.3.0', 'ZC_Navigation_Fallback::parse_blocks_from_menu_items' );
 ...
 * @since 6.2.0
 *
 * @deprecated 6.3.0 Use ZC_Navigation_Fallback::get_classic_menu_fallback() instead.
 *
 * @return object ZC_Term The classic navigation.
 */
function block_core_navigation_get_classic_menu_fallback() {

	_deprecated_function( __FUNCTION__, '6.3.0', 'ZC_Navigation_Fallback::get_classic_menu_fallback' );

	$classic_nav_menus = zc_get_nav_menus();

	// If menus exist.
	if ( $classic_nav_menus && ! is_zc_error( $classic_nav_menus ) ) {
		// Handles simple use case where user has a classic menu and switches to a block theme.

		// Returns the menu assigned to location `primary`.
		$locations = get_nav_menu_locations();
		if ( isset( $locations['primary'] ) ) {
			$primary_menu = zc_get_nav_menu_object( $locations['primary'] );
			if ( $primary_menu ) {
				return $primary_menu;
			}
		}
 ...
 * @since 6.2.0
 *
 * @deprecated 6.3.0 Use ZC_Navigation_Fallback::get_classic_menu_fallback_blocks() instead.
 *
 * @param  object $classic_nav_menu ZC_Term The classic navigation object to convert.
 * @return array the normalized parsed blocks.
 */
function block_core_navigation_get_classic_menu_fallback_blocks( $classic_nav_menu ) {

	_deprecated_function( __FUNCTION__, '6.3.0', 'ZC_Navigation_Fallback::get_classic_menu_fallback_blocks' );

	// BEGIN: Code that already exists in zc_nav_menu().
	$menu_items = zc_get_nav_menu_items( $classic_nav_menu->term_id, array( 'update_post_term_cache' => false ) );
	...
	// END: Code that already exists in zc_nav_menu().
 ...
 * @since 6.2.0
 *
 * @deprecated 6.3.0 Use ZC_Navigation_Fallback::create_classic_menu_fallback() instead.
 *
 * @return array the normalized parsed blocks.
 */
function block_core_navigation_maybe_use_classic_menu_fallback() {

	_deprecated_function( __FUNCTION__, '6.3.0', 'ZC_Navigation_Fallback::create_classic_menu_fallback' );
	...
	$zc_insert_post_result = zc_insert_post(
		array(
			'post_content' => $classic_nav_menu_blocks,
			'post_title'   => $classic_nav_menu->name,
			'post_name'    => $classic_nav_menu->slug,
			'post_status'  => 'publish',
			'post_type'    => 'zc_navigation',
		),
		true // So that we can check whether the result is an error.
	);

	if ( is_zc_error( $zc_insert_post_result ) ) {
		return;
	}
...
/**
 * Finds the most recently published `zc_navigation` Post.
 *
 * @since 6.1.0
 *
 * @deprecated 6.3.0 Use ZC_Navigation_Fallback::get_most_recently_published_navigation() instead.
 *
 * @return ZC_Post|null the first non-empty Navigation or null.
 */
function block_core_navigation_get_most_recently_published_navigation() {

	_deprecated_function( __FUNCTION__, '6.3.0', 'ZC_Navigation_Fallback::get_most_recently_published_navigation' );
	...
	$navigation_post = new ZC_Query( $parsed_args );
