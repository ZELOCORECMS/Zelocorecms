...
function render_block_core_categories( $attributes, $content, $block ) {
	...
	if ( ! empty( $attributes['displayAsDropdown'] ) ) {
		$id                       = 'zc-block-categories-' . $block_id;
		$args['id']               = $id;
		...
		$label_text     = ! empty( $attributes['label'] ) ? zc_kses_post( $attributes['label'] ) : $default_label;
		$wrapper_markup = '<div %1$s><label class="zc-block-categories__label' . $show_label . '" for="' . esc_attr( $id ) . '">' . $label_text . '</label>%2$s</div>';
		$items_markup   = zc_dropdown_categories( $args );
		$type           = 'dropdown';
		...
		$items_markup   = zc_list_categories( $args );
		$type           = 'list';

		if ( ! empty( $block->context['enhancedPagination'] ) ) {
			$p = new ZC_HTML_Tag_Processor( $items_markup );
			while ( $p->next_tag( 'a' ) ) {
	...
	$wrapper_attributes = get_block_wrapper_attributes( array( 'class' => "zc-block-categories-{$type} zc-block-categories-taxonomy-{$attributes['taxonomy']}" ) );
