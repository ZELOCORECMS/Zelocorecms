...
function render_block_core_breadcrumbs( $attributes, $content, $block ) {
	...
	if ( $is_home ) {
		...
		$text               = sprintf( __( 'Search results for: "%s"' ), zc_trim_words( get_search_query(), 10 ) );
	 ...
	 *                              When true, the label will be sanitized with zc_kses_post(),
		...
		implode(
			...
			array_map(
				static function ( $item ) {
					$label = ! empty( $item['allow_html'] ) ? zc_kses_post( $item['label'] ) : esc_html( $item['label'] );
					if ( ! empty( $item['url'] ) ) {
 ...
 * @since 7.0.0
 *
 * @param int|ZC_Post $post_id_or_object The post ID or post object.
 *
...
function block_core_breadcrumbs_get_term_ancestors_items( $term_id, $taxonomy ) {
	...
	if ( is_taxonomy_hierarchical( $taxonomy ) ) {
		...
		foreach ( $term_ancestors as $ancestor_id ) {
			...
			if ( $ancestor_term && ! is_zc_error( $ancestor_term ) ) {
				$breadcrumb_items[] = array(
...
function block_core_breadcrumbs_get_archive_breadcrumbs() {
	...
	if ( $queried_object instanceof ZC_Term ) {
		$term     = $queried_object;
		...
		$title = apply_filters( 'post_type_archive_title', $post_type_object->labels->archives, $post_type ); // phpcs:ignore ZelocoreCMS.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound
...
function block_core_breadcrumbs_get_terms_breadcrumbs( $post_id, $post_type ) {
	...
	$taxonomies = zc_filter_object_list(
	...
	if ( ! empty( $settings['taxonomy'] ) ) {
		foreach ( $taxonomies as $taxonomy ) {
			if ( $taxonomy->name === $settings['taxonomy'] ) {
				...
				if ( ! empty( $post_terms ) && ! is_zc_error( $post_terms ) ) {
