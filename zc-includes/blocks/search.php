...
function render_block_core_search( $attributes ) {
	...
	$input_id            = zc_unique_id( 'zc-block-search__input-' );
	...
	$label_inner_html = empty( $attributes['label'] ) ? __( 'Search' ) : zc_kses_post( $attributes['label'] );
	$label            = new ZC_HTML_Tag_Processor( sprintf( '<label %1$s>%2$s</label>', $inline_styles['label'], $label_inner_html ) );
	if ( $label->next_tag() ) {
		$label->set_attribute( 'for', $input_id );
		$label->add_class( 'zc-block-search__label' );
	...
	$input         = new ZC_HTML_Tag_Processor( sprintf( '<input type="search" name="s" required %s/>', $inline_styles['input'] ) );
	$input_classes = array( 'zc-block-search__input' );
	...
	if ( $input->next_tag() ) {
		...
		if ( $is_expandable_searchfield ) {
			zc_enqueue_script_module( '@zelocorecms/block-library/search/view' );

			$input->set_attribute( 'data-zc-bind--aria-hidden', '!context.isSearchInputVisible' );
			$input->set_attribute( 'data-zc-bind--tabindex', 'state.tabindex' );
	...
	if ( $show_button ) {
		$button_classes         = array( 'zc-block-search__button' );
		...
		if ( ! $use_icon_button ) {
			if ( ! empty( $attributes['buttonText'] ) ) {
				$button_internal_markup = zc_kses_post( $attributes['buttonText'] );
			}
		} else {
		...
		$button_classes[] = zc_theme_get_element_class_name( 'button' );
		$button           = new ZC_HTML_Tag_Processor( sprintf( '<button type="submit" %s>%s</button>', $inline_styles['button'], $button_internal_markup ) );

		if ( $button->next_tag() ) {
			$button->add_class( implode( ' ', $button_classes ) );
			if ( 'button-only' === $attributes['buttonPosition'] ) {
				$button->set_attribute( 'data-zc-bind--aria-label', 'state.ariaLabel' );
				$button->set_attribute( 'data-zc-bind--aria-controls', 'state.ariaControls' );
				$button->set_attribute( 'data-zc-bind--aria-expanded', 'context.isSearchInputVisible' );
				$button->set_attribute( 'data-zc-bind--type', 'state.type' );
				$button->set_attribute( 'data-zc-on--click', 'actions.openSearchInput' );
				...
				$button->set_attribute( 'aria-controls', 'zc-block-search__input-' . $input_id );
				$button->set_attribute( 'aria-expanded', 'false' );
				$button->set_attribute( 'type', 'button' );
			} else {
				$button->set_attribute( 'aria-label', zc_strip_all_tags( $attributes['buttonText'] ) );
			}
		}
	}

	$field_markup_classes = array(
		'zc-block-search__inside-wrapper',
	);
	if ( $is_button_inside && ! empty( $border_color_classes ) ) {
	...
	if ( $is_expandable_searchfield ) {
		...
		$form_context         = zc_interactivity_data_zc_context(
			array(
		...
		$form_directives      = '
		 data-zc-interactive="core/search"
		 ' . $form_context . '
		 data-zc-class--zc-block-search__searchfield-hidden="!context.isSearchInputVisible"
		 data-zc-on--keydown="actions.handleSearchKeydown"
		 data-zc-on--focusout="actions.handleSearchFocusout"
		';
	}
...
function classnames_for_block_core_search( $attributes ) {
	...
	if ( ! empty( $attributes['buttonPosition'] ) ) {
		if ( 'button-inside' === $attributes['buttonPosition'] ) {
			$classnames[] = 'zc-block-search__button-inside';
		}

		if ( 'button-outside' === $attributes['buttonPosition'] ) {
			$classnames[] = 'zc-block-search__button-outside';
		}

		if ( 'no-button' === $attributes['buttonPosition'] ) {
			$classnames[] = 'zc-block-search__no-button';
		}

		if ( 'button-only' === $attributes['buttonPosition'] ) {
			$classnames[] = 'zc-block-search__button-only zc-block-search__searchfield-hidden';
		}
	}
	...
	if ( isset( $attributes['buttonUseIcon'] ) ) {
		if ( ! empty( $attributes['buttonPosition'] ) && 'no-button' !== $attributes['buttonPosition'] ) {
			if ( $attributes['buttonUseIcon'] ) {
				$classnames[] = 'zc-block-search__icon-button';
			} else {
				$classnames[] = 'zc-block-search__text-button';
			}
		}
	}
...
function apply_block_core_search_border_style( $attributes, $property, $side, &$wrapper_styles, &$button_styles, &$input_styles ) {
	...
	$value = _zc_array_get( $attributes, $path, false );
	...
	if ( 'color' === $property && $side ) {
		...
		if ( $has_color_preset ) {
			...
			$value             = sprintf( 'var(--zc--preset--color--%s)', $named_color_value );
		}
	}
		...
		if ( is_array( $border_radius ) ) {
			...
			foreach ( $border_radius as $key => $value ) {
				...
				if ( is_string( $value ) && str_contains( $value, 'var:preset|border-radius|' ) ) {
					...
					$slug            = _zc_to_kebab_case( substr( $value, $index_to_splice ) );
					$value           = "var(--zc--preset--border-radius--$slug)";
				}
				...
				if ( null !== $value ) {
					...
					if ( $is_button_inside && ( intval( $value ) !== 0 || str_contains( $value, 'var(--zc--preset--border-radius--' ) ) ) {
						$wrapper_styles[] = sprintf(
			...
			if ( is_string( $border_radius ) && str_contains( $border_radius, 'var:preset|border-radius|' ) ) {
				...
				$slug            = _zc_to_kebab_case( substr( $border_radius, $index_to_splice ) );
				$border_radius   = "var(--zc--preset--border-radius--$slug)";
			}
...
function get_typography_styles_for_block_core_search( $attributes ) {
	...
	if ( ! empty( $attributes['style']['typography']['fontSize'] ) ) {
		$typography_styles[] = sprintf(
			'font-size: %s;',
			zc_get_typography_font_size_value(
				array(
