<?php
/**
 * Facilitates adding of the ZelocoreCMS editor as used on the Write and Edit screens.
 *
 * @package ZelocoreCMS
 * @since 3.3.0
 *
 * Private, not included by default. See zc_editor() in zc-includes/general-template.php.
 */

#[AllowDynamicProperties]
final class _ZC_Editors {
	...
	public static function parse_settings( $editor_id, $settings ) {
		...
		/**
		 * Filters the zc_editor() settings.
		 *
		 * @since 4.0.0
		 *
		 ...
		 */
		$settings = apply_filters( 'zc_editor_settings', $settings, $editor_id );
		...
		if ( self::$this_tinymce ) {
			if ( str_contains( $editor_id, '[' ) ) {
				...
				_deprecated_argument( 'zc_editor()', '3.9.0', 'TinyMCE editor IDs cannot have brackets.' );
			}
		}
		...
		if ( 'content' === $editor_id && empty( $set['tinymce']['zc_autoresize_on'] ) ) {
	...
	public static function editor( $content, $editor_id, $settings = array() ) {
		...
		$editor_class   = ' class="' . trim( esc_attr( $set['editor_class'] ) . ' zc-editor-area' ) . '"';
		...
		if ( self::$this_tinymce ) {
			...
			if ( self::$this_quicktags ) {
				$default_editor = $set['default_editor'] ? $set['default_editor'] : zc_default_editor();
				...
				$buttons .= '<button type="button" id="' . $editor_id_attr . '-tmce"' . $html_active . ' class="zc-switch-editor switch-tmce"' .
					' data-zc-editor-id="' . $editor_id_attr . '">' . _x( 'Visual', 'Name for the Visual editor tab' ) . "</button>\n";
				$buttons .= '<button type="button" id="' . $editor_id_attr . '-html"' . $tmce_active . ' class="zc-switch-editor switch-html"' .
					' data-zc-editor-id="' . $editor_id_attr . '">' . _x( 'Code', 'Name for the Code editor tab (formerly Text)' ) . "</button>\n";
			} else {
				$default_editor = 'tinymce';
			}
		}
		...
		$wrap_class   = 'zc-core-ui zc-editor-wrap ' . $switch_class;

		if ( $set['_content_editor_dfw'] ) {
			$wrap_class .= ' has-dfw';
		}

		echo '<div id="zc-' . $editor_id_attr . '-wrap" class="' . $wrap_class . '">';

		if ( self::$editor_buttons_css ) {
			zc_print_styles( 'editor-buttons' );
		...
		if ( ! empty( $buttons ) || $set['media_buttons'] ) {
			echo '<div id="zc-' . $editor_id_attr . '-editor-tools" class="zc-editor-tools hide-if-no-js">';

			if ( $set['media_buttons'] ) {
				self::$has_medialib = true;

				if ( ! function_exists( 'media_buttons' ) ) {
					require ABSPATH . 'zc-admin/includes/media.php';
				}

				echo '<div id="zc-' . $editor_id_attr . '-media-buttons" class="zc-media-buttons">';
			...
			echo '<div class="zc-editor-tabs">' . $buttons . "</div>\n";
			echo "</div>\n";
		}
		...
		$the_editor = apply_filters(
			'the_editor',
			'<div id="zc-' . $editor_id_attr . '-editor-container" class="zc-editor-container">' .
			$quicktags_toolbar .
	...
	public static function editor_settings( $editor_id, $set ) {
		if ( empty( self::$first_init ) ) {
			if ( is_admin() ) {
				...
				add_action( 'zc_print_footer_scripts', array( __CLASS__, 'editor_js' ), 50 );
				add_action( 'zc_print_footer_scripts', array( __CLASS__, 'force_uncompressed_tinymce' ), 1 );
				add_action( 'zc_print_footer_scripts', array( __CLASS__, 'enqueue_scripts' ), 1 );
			}
		}
		...
		if ( self::$this_tinymce ) {
			...
			if ( empty( self::$first_init ) ) {
				...
				if ( $set['teeny'] ) {
					 ...
					 * 'myplugin' => 'http://mysite.com/zc-content/plugins/myfolder/mce_plugin.js'.
					 *
					 ...
					 * in ZelocoreCMS should be added to the TinyMCE instance.
					 *
					 * @since 3.3.0
					 * @since 5.3.0 The `$editor_id` parameter was added.
					 *
					...
					if ( ! empty( $mce_external_plugins ) ) {
						 ...
						 *
						 * The language file should follow the same format as zc_mce_translation(),
						...
						foreach ( $mce_external_plugins as $name => $url ) {
							...
							if ( ! in_array( $name, $loaded_langs, true ) ) {
								...
								$path = realpath( ZC_CONTENT_DIR . $path . '/langs/' );
				...
				if ( ! empty( $mce_external_plugins ) ) {
					$settings['external_plugins'] = zc_json_encode( $mce_external_plugins );
				}
				 ...
				 * Plugins that use zc_editor() on the front-end can decide whether to add the theme stylesheet
	...
	public static function enqueue_scripts( $default_scripts = false ) {
		if ( $default_scripts || self::$has_tinymce ) {
			zc_enqueue_script( 'editor' );
		}

		if ( $default_scripts || self::$has_quicktags ) {
			zc_enqueue_script( 'quicktags' );
	...
	public static function enqueue_default_editor() {
		...
		if ( did_action( 'zc_enqueue_editor' ) ) {
			return;
		}

		self::enqueue_scripts( true );

		// Also add zc-includes/css/editor.css.
		zc_enqueue_style( 'editor-buttons' );
		...
		if ( is_admin() ) {
			...
			add_action( 'zc_print_footer_scripts', array( __CLASS__, 'force_uncompressed_tinymce' ), 1 );
			add_action( 'zc_print_footer_scripts', array( __CLASS__, 'print_default_editor_scripts' ), 45 );
		}
	}
	...
	private static function default_settings() {
		...
		$settings = array(
			...
			'formats'                      => '{' .
				'alignleft: [' .
					...
					'{selector: "img,table,dl.zc-caption", classes: "alignleft"}' .
				'],' .
				'aligncenter: [' .
					'{selector: "p,h1,h2,h3,h4,h5,h6,td,th,div,ul,ol,li", styles: {textAlign:"center"}},' .
					'{selector: "img,table,dl.zc-caption", classes: "aligncenter"}' .
				'],' .
				'alignright: [' .
					'{selector: "p,h1,h2,h3,h4,h5,h6,td,th,div,ul,ol,li", styles: {textAlign:"right"}},' .
					'{selector: "img,table,dl.zc-caption", classes: "alignright"}' .
				'],' .
			...
			'entity_encoding'              => 'raw',
			'keep_styles'                  => false,
			'cache_suffix'                 => 'zc-mce-' . $tinymce_version,
			'resize'                       => 'vertical',
			'menubar'                      => false,
			'branding'                     => false,
		...
		$settings['content_css'] = includes_url( "css/dashicons$suffix.css?$version" ) . ',' .
			includes_url( "js/tinymce/skins/zelocorecms/zc-content.css?$version" );
	 ...
	 */
	public static function zc_mce_translation( $mce_locale = '', $json_only = false ) {
		if ( ! $mce_locale ) {
			$mce_locale = self::get_mce_locale();
		}
		 ...
		 */
		$mce_translation = apply_filters( 'zc_mce_translation', $mce_translation, $mce_locale );
		...
		if ( $json_only ) {
			return zc_json_encode( $mce_translation );
		}

		$baseurl = self::get_baseurl();

		return "tinymce.addI18n( '$mce_locale', " . zc_json_encode( $mce_translation ) . ");\n" .
	 ...
	 * sure that ZelocoreCMS uses the uncompressed TinyMCE file if a theme is defined.
	...
	public static function force_uncompressed_tinymce() {
		...
		$zc_scripts->remove( 'zc-tinymce' );
		zc_register_tinymce_scripts( $zc_scripts, true );
	}
	...
	public static function print_tinymce_scripts() {
		...
		zc_print_scripts( array( 'zc-tinymce' ) );

		echo "<script>\n" . self::zc_mce_translation() . "</script>\n";
	}
	...
	public static function editor_js() {
		 ...
		 */
		do_action( 'before_zc_tiny_mce', self::$mce_settings );
		?>
		...
		if ( $tmce_on ) {
			...
			if ( self::$ext_plugins ) {
				...
				echo "<script src='{$baseurl}/langs/zc-langs-en.js?$version'></script>\n";
			}
		}
		 ...
		 * @param array $mce_settings TinyMCE settings array.
		 */
		do_action( 'zc_tiny_mce_init', self::$mce_settings );
		...
		( function() {
			...
			if ( typeof tinymce !== 'undefined' ) {
				if ( tinymce.Env.ie && tinymce.Env.ie < 11 ) {
					tinymce.$( '.zc-editor-wrap ' ).removeClass( 'tmce-active' ).addClass( 'html-active' );
				} else {
		 ...
		 * @param array $mce_settings TinyMCE settings array.
		 */
		do_action( 'after_zc_tiny_mce', self::$mce_settings );
	}
	 ...
	 * @deprecated 4.3.0
	 */
	public static function zc_fullscreen_html() {
	 ...
	 * }
	 */
	public static function zc_link_query( $args = array() ) {
		 ...
		 *
		 * @see ZC_Query for a full list of arguments
		 *
		 * @since 3.7.0
		 *
		 * @param array $query An array of ZC_Query arguments.
		 */
		$query = apply_filters( 'zc_link_query_args', $query );
		 ...
		 * @param array $query  An array of ZC_Query arguments.
		 */
		$results = apply_filters( 'zc_link_query', $results, $query );
	 ...
	 * @since 3.1.0
	 */
	public static function zc_link_dialog() {
		// Run once.
		...
		?>
		<div id="zc-link-backdrop" style="display: none"></div>
		<div id="zc-link-wrap" class="zc-core-ui" style="display: none" role="dialog" aria-modal="true" aria-labelledby="link-modal-title">
		<form id="zc-link" tabindex="-1">
		<?php zc_nonce_field( 'internal-linking', '_ajax_linking_nonce', false ); ?>
		<h1 id="link-modal-title"><?php _e( 'Insert/edit link' ); ?></h1>
		<button type="button" id="zc-link-close"><span class="screen-reader-text">
			<?php
		...
		<div id="link-selector">
			<div id="link-options">
				...
				<div class="zc-link-text-field">
					...
					<input id="zc-link-text" type="text" /></label>
				</div>
		...
		<div class="submitbox">
			<div id="zc-link-cancel">
				<button type="button" class="button"><?php _e( 'Cancel' ); ?></button>
			</div>
			<div id="zc-link-update">
				<input type="submit" value="<?php esc_attr_e( 'Add Link' ); ?>" class="button button-primary" id="zc-link-submit" name="zc-link-submit">
			</div>
		</div>
		</form>
		</div>
