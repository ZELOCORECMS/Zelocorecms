 ...
 *
 * The ZelocoreCMS Rewrite class writes the rewrite module rules to the .htaccess
...
class ZC_Rewrite {
	 ...
	 * `ZC_Rewrite::$index` with a trailing slash appended. Otherwise the root
	 * will be empty.
	 *
	 * @since 1.5.0
	 * @var string
	 *
	 * @see ZC_Rewrite::init()
	 * @see ZC_Rewrite::using_index_permalinks()
	 */
	public $root = '';
	...
	/**
	 * Rules that don't redirect to ZelocoreCMS' index.php.
	 *
	...
	/**
	 * Whether to write every mod_rewrite rule for ZelocoreCMS into the .htaccess file.
	 *
	 ...
	 * @var bool
	 *
	 * @see ZC_Rewrite::mod_rewrite_rules()
	 */
	 ...
	 * `ZC_Rewrite::$rewritereplace` and are rewritten to the query
	 * variables listed in ZC_Rewrite::$queryreplace.
	 *
	 ...
	 * of rewrite tags, see ZC_Rewrite::$rewritecode.
	 *
	 * @since 1.5.0
	 * @var string[]
	 */
	...
	/**
	 * Query variables that rewrite tags map to, see ZC_Rewrite::$rewritecode.
	 *
	 * @since 1.5.0
	 * @var string[]
	 */
	 ...
	 * @since 1.5.0
	 *
	 * @see ZC_Rewrite::$rewritecode
	 * @see ZC_Rewrite::$rewritereplace
	 * @see ZC_Rewrite::$queryreplace
	 *
	 ...
	 *
	 * The main ZC_Rewrite function for building the rewrite rule list. The
	 ...
	 *
	 * Shorten version of ZC_Rewrite::generate_rewrite_rules() that allows for shorter
	 * list of parameters. See the method for longer description of what generating
	 * rewrite rules does.
	 *
	 * @since 1.5.0
	 *
	 * @see ZC_Rewrite::generate_rewrite_rules() See for long description and rest of parameters.
	 *
	 ...
	 * reference to the current ZC_Rewrite instance to further manipulate the
	...
	public function rewrite_rules() {
		...
		$deprecated_files = array(
			'.*zc-(atom|rdf|rss|rss2|feed|commentsrss2)\.php$' => $this->index . '?feed=old',
			'.*zc-app\.php(/.*)?$' => $this->index . '?error=403',
		);

		// Registration rules.
		$registration_pages = array();
		if ( is_multisite() && is_main_site() ) {
			$registration_pages['.*zc-signup.php$']   = $this->index . '?signup=true';
			$registration_pages['.*zc-activate.php$'] = $this->index . '?activate=true';
		}

		// Deprecated.
		$registration_pages['.*zc-register.php$'] = $this->index . '?register=true';
		 ...
		 * @since 1.5.0
		 *
		 * @param ZC_Rewrite $zc_rewrite Current ZC_Rewrite instance (passed by reference).
		 */
	 ...
	 *
	 * The difference between this method and ZC_Rewrite::rewrite_rules() is that
	 ...
	 * @return string[] Array of rewrite rules keyed by their regex pattern.
	 */
	public function zc_rewrite_rules() {
	 ...
	 *
	 * If the {@see 'zc_loaded'} action has not occurred yet, will postpone saving to the database.
	 *
	 * @since 6.4.0
	 */
	private function refresh_rewrite_rules() {
		...
		if ( ! did_action( 'zc_loaded' ) ) {
			 ...
			 */
			add_action( 'zc_loaded', array( $this, 'flush_rules' ) );
		} else {
	 ...
	 *
	 * Will add the non_zc_rules property rules to the .htaccess file before
	 * the ZelocoreCMS rewrite rules one.
	 *
	 * @since 1.5.0
	 *
	 * @return string
	...
	<system.webServer>
		...
		$rules .= '
			<rule name="ZelocoreCMS: ' . esc_attr( home_url() ) . '" patternSyntax="Wildcard">
				<match url="*" />
					<conditions>
	 ...
	 * query strings. The new permastruct is added to the ZC_Rewrite::$extra_permastructs array.
	 *
	 * When the rewrite rules are built by ZC_Rewrite::rewrite_rules(), all of these extra
	 * permastructs are passed to ZC_Rewrite::generate_rewrite_rules() which transforms them
	 * into the regular expressions that many love to hate.
	 *
	 * The `$args` parameter gives you control over how ZC_Rewrite::generate_rewrite_rules()
	 ...
	 *     Default empty array.
	 *
	 *     @type bool $with_front  Whether the structure should be prepended with `ZC_Rewrite::$front`.
	 *                             Default true.
	 ...
	 *
	 * Calls ZC_Rewrite::zc_rewrite_rules() after removing the 'rewrite_rules' option.
	 ...
	 * ZC_Rewrite::init() after the option is updated.
	 *
	 ...
	 * the current category base and the parameter value. Calls ZC_Rewrite::init()
	 ...
	 * current tag base and the parameter value. Calls ZC_Rewrite::init() after
