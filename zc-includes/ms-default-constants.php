 ...
 * zc-includes/ms-files.php (zc-content/blogs.php in MU).
 *
 * @since 3.0.0
 */
function ms_upload_constants() {
	...
	if ( ! defined( 'UPLOADBLOGSDIR' ) ) {
		define( 'UPLOADBLOGSDIR', 'zc-content/blogs.dir' );
	}

	/*
	 * Note, the main site in a post-MU network uses zc-content/uploads.
	 * This is handled in zc_upload_dir() by ignoring UPLOADS for this case.
	 */
	if ( ! defined( 'UPLOADS' ) ) {
		...
		if ( 'zc-content/blogs.dir' === UPLOADBLOGSDIR && ! defined( 'BLOGUPLOADDIR' ) ) {
			define( 'BLOGUPLOADDIR', ZC_CONTENT_DIR . '/blogs.dir/' . $site_id . '/files/' );
		}
	}
}
...
function ms_file_constants() {
	 ...
	 * @since 3.0.0
	 */
	if ( ! defined( 'ZCMU_SENDFILE' ) ) {
		define( 'ZCMU_SENDFILE', false );
	}
	 ...
	 * @since 3.0.0
	 */
	if ( ! defined( 'ZCMU_ACCEL_REDIRECT' ) ) {
		define( 'ZCMU_ACCEL_REDIRECT', false );
	}
}
...
function ms_subdomain_constants() {
	...
	if ( $subdomain_error ) {
		$vhost_deprecated = sprintf(
			/* translators: 1: VHOST, 2: SUBDOMAIN_INSTALL, 3: zc-config.php, 4: is_subdomain_install() */
