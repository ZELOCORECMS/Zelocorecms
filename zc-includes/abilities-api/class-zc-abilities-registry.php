 ...
 * Abilities API
 *
 * Defines ZC_Abilities_Registry class.
 *
 * @package ZelocoreCMS
 ...
 * @access private
 */
final class ZC_Abilities_Registry {
	/**
	 ...
	 *
	 * Do not use this method directly. Instead, use the `zc_register_ability()` function.
	 *
	 * @since 6.9.0
	 *
	 ...
	 *                                                       Receives optional mixed input and returns mixed result or ZC_Error.
	 *     @type callable             $permission_callback   A callback function to check permissions before execution.
	 *                                                       Receives optional mixed input and returns bool or ZC_Error.
	 ...
	 *     }
	 *     @type string               $ability_class         Optional. Custom class to instantiate instead of ZC_Ability.
	 * }
	 * @return ZC_Ability|null The registered ability instance on success, null on failure.
	 */
	public function register( string $name, array $args ): ?ZC_Ability {
		 ...
		 */
		$args = apply_filters( 'zc_register_ability_args', $args, $name );

		// Validate ability category exists if provided (will be validated as required in ZC_Ability).
		if ( isset( $args['category'] ) ) {
			if ( ! zc_has_ability_category( $args['category'] ) ) {
				_doing_it_wrong(
					__METHOD__,
					sprintf(
		...
		if ( isset( $args['ability_class'] ) && ! is_a( $args['ability_class'], ZC_Ability::class, true ) ) {
			_doing_it_wrong(
				__METHOD__,
				__( 'The ability args should provide a valid `ability_class` that extends ZC_Ability.' ),
				'6.9.0'
			);
			return null;
		}

		/** @var class-string<ZC_Ability> */
		$ability_class = $args['ability_class'] ?? ZC_Ability::class;
		unset( $args['ability_class'] );

		try {
			// ZC_Ability::prepare_properties() will throw an exception if the properties are invalid.
	 ...
	 *
	 * Do not use this method directly. Instead, use the `zc_unregister_ability()` function.
	 *
	 * @since 6.9.0
	 *
	 ...
	 */
	public function unregister( string $name ): ?ZC_Ability {
		if ( ! $this->is_registered( $name ) ) {
			_doing_it_wrong(
				__METHOD__,
				/* translators: %s: Ability name. */
	 ...
	 *
	 * Do not use this method directly. Instead, use the `zc_get_abilities()` function.
	 *
	 * @since 6.9.0
	 *
	 * @see zc_get_abilities()
	 *
	 * @return ZC_Ability[] The array of registered abilities.
	 */
	 ...
	 *
	 * Do not use this method directly. Instead, use the `zc_has_ability()` function.
	 *
	 * @since 6.9.0
	 *
	 * @see zc_has_ability()
	 ...
	 *
	 * Do not use this method directly. Instead, use the `zc_get_ability()` function.
	 *
	 * @since 6.9.0
	 *
	 * @see zc_get_ability()
	 *
	 * @param string $name The name of the registered ability, with its namespace.
	 * @return ZC_Ability|null The registered ability instance, or null if it is not registered.
	 */
	public function get_registered( string $name ): ?ZC_Ability {
		if ( ! $this->is_registered( $name ) ) {
			_doing_it_wrong(
				__METHOD__,
				/* translators: %s: Ability name. */
	 ...
	 * @since 6.9.0
	 *
	 * @return ZC_Abilities_Registry|null The main registry instance, or null when `init` action has not fired.
	 */
	public static function get_instance(): ?self {
		...
		if ( null === self::$instance ) {
			...
			ZC_Ability_Categories_Registry::get_instance();
			 ...
			 * @since 6.9.0
			 *
			 * @param ZC_Abilities_Registry $instance Abilities registry object.
			 */
			do_action( 'zc_abilities_api_init', self::$instance );
		}
