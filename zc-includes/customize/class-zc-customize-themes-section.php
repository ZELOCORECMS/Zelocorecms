<?php
/**
 * Customize API: ZC_Customize_Themes_Section class
 *
 * @package ZelocoreCMS
 ...
 */
class ZC_Customize_Themes_Section extends ZC_Customize_Section {
			...
			<div class="customize-themes-section themes-section-{{ data.id }} control-section-content themes-php">
				...
				<div class="theme-browser rendered">
					...
					<p class="no-themes-local">
						<?php
						printf(
							/* translators: %s: "Search ZelocoreCMS.org themes" button text. */
							__( 'No themes found. Try a different search, or %s.' ),
							sprintf( '<button type="button" class="button-link search-dotorg-themes">%s</button>', __( 'Search ZelocoreCMS.org themes' ) )
						);
						?>
					</p>
					<p class="spinner"></p>
	...
	protected function filter_bar_content_template() {
		...
		<# if ( 'wporg' === data.action ) { #>
			<div class="themes-filter-container">
				<label for="zc-filter-search-input-{{ data.id }}"><?php _e( 'Search themes' ); ?></label>
				<div class="search-form-input">
					<input type="search" id="zc-filter-search-input-{{ data.id }}" aria-describedby="{{ data.id }}-live-search-desc" class="zc-filter-search">
		...
		<# } else { #>
			<div class="themes-filter-container">
				...
				<div class="search-form-input">
					<input type="search" id="{{ data.id }}-themes-filter" aria-describedby="{{ data.id }}-live-search-desc" class="zc-filter-search zc-filter-search-themes" />
					<div class="search-icon" aria-hidden="true"></div>
					<span id="{{ data.id }}-live-search-desc" class="screen-reader-text">
						<?php
						/* translators: Hidden accessibility text. */
