 ...
 *
 * Added zc_ prefix to avoid conflicts with existing kses users
 *
 * @version 0.2.2
 ...
 * {@see 'zc_kses_allowed_html'} filter is more powerful and supplies context.
 *
...
if ( ! CUSTOM_TAGS ) {
	...
	$allowedposttags = array(
		...
		'object'     => array(
			'data' => array(
				'required'       => true,
				'value_callback' => '_zc_kses_allow_pdf_objects',
			),
			'type' => array(
				'required' => true,
	...
	// Except common attributes added by _zc_add_global_attributes.
	$math_global_attributes = array(
		...
		'mathcolor'      => true,
		'mathsize'       => true,
		// Common attributes also defined by _zc_add_global_attributes.
	...
	$allowedposttags = array_map( '_zc_add_global_attributes', $allowedposttags );
} else {
	...
	$allowedtags     = zc_kses_array_lc( $allowedtags );
	$allowedposttags = zc_kses_array_lc( $allowedposttags );
}
 ...
 *
 * @see zc_kses_post() for specifically filtering post content and fields.
 * @see zc_allowed_protocols() for the default allowed protocols in link URLs.
 *
 * @since 1.0.0
 *
 * @param string         $content           Text content to filter.
 * @param array[]|string $allowed_html      An array of allowed HTML elements and attributes,
 *                                          or a context name such as 'post'. See zc_kses_allowed_html()
 *                                          for the list of accepted context names.
 * @param string[]       $allowed_protocols Optional. Array of allowed URL protocols.
 *                                          Defaults to the result of zc_allowed_protocols().
 * @return string Filtered content containing only the allowed HTML.
 */
function zc_kses( $content, $allowed_html, $allowed_protocols = array() ) {
	if ( empty( $allowed_protocols ) ) {
		$allowed_protocols = zc_allowed_protocols();
	}

	$content = zc_kses_no_null( $content, array( 'slash_zero' => 'keep' ) );
	$content = zc_kses_normalize_entities( $content );
	$content = zc_kses_hook( $content, $allowed_html, $allowed_protocols );

	return zc_kses_split( $content, $allowed_html, $allowed_protocols );
}

/**
 * Filters one HTML attribute and ensures its value is allowed.
 *
 * This function can escape data in some situations where `zc_kses()` must strip the whole attribute.
 *
 * @since 4.2.3
 *
 ...
 */
function zc_kses_one_attr( $attr, $element ) {
	$uris              = zc_kses_uri_attributes();
	$allowed_html      = zc_kses_allowed_html( 'post' );
	$allowed_protocols = zc_allowed_protocols();
	$attr              = zc_kses_no_null( $attr, array( 'slash_zero' => 'keep' ) );
	...
	if ( count( $split ) === 2 ) {
		...
		if ( in_array( strtolower( $name ), $uris, true ) ) {
			$value = zc_kses_bad_protocol( $value, $allowed_protocols );
		}
	...
	zc_kses_attr_check( $name, $value, $attr, $vless, $element, $allowed_html );
 ...
 */
function zc_kses_allowed_html( $context = '' ) {
	...
	if ( is_array( $context ) ) {
		 ...
		 */
		return apply_filters( 'zc_kses_allowed_html', $html, $context );
	}

	switch ( $context ) {
		case 'post':
			/** This filter is documented in zc-includes/kses.php */
			$tags = apply_filters( 'zc_kses_allowed_html', $allowedposttags, $context );
			...
			if ( ! CUSTOM_TAGS && ! isset( $tags['form'] ) && ( isset( $tags['input'] ) || isset( $tags['select'] ) ) ) {
				...
				/** This filter is documented in zc-includes/kses.php */
				$tags = apply_filters( 'zc_kses_allowed_html', $tags, $context );
			}
		...
		case 'pre_user_description':
			...
			$tags['a']['target'] = true;
			/** This filter is documented in zc-includes/kses.php */
			return apply_filters( 'zc_kses_allowed_html', $tags, $context );

		case 'strip':
			/** This filter is documented in zc-includes/kses.php */
			return apply_filters( 'zc_kses_allowed_html', array(), $context );

		case 'entities':
			/** This filter is documented in zc-includes/kses.php */
			return apply_filters( 'zc_kses_allowed_html', $allowedentitynames, $context );

		case 'data':
		default:
			/** This filter is documented in zc-includes/kses.php */
			return apply_filters( 'zc_kses_allowed_html', $allowedtags, $context );
	}
}
 ...
 *
 * There is currently only one KSES ZelocoreCMS hook, {@see 'pre_kses'}, and it is called here.
 ...
 */
function zc_kses_hook( $content, $allowed_html, $allowed_protocols ) {
	/**
 ...
 */
function zc_kses_split( $content, $allowed_html, $allowed_protocols ) {
...
~x
REGEX;
	return preg_replace_callback( $token_pattern, '_zc_kses_split_callback', $content );
}
 ...
 */
function zc_kses_uri_attributes() {
	 ...
	 */
	$uri_attributes = apply_filters( 'zc_kses_uri_attributes', $uri_attributes );

	return $uri_attributes;
}

/**
 * Callback for `zc_kses_split()`.
 *
 * @since 3.1.0
 * @access private
 * @ignore
 ...
 * @return string
 */
function _zc_kses_split_callback( $matches ) {
	global $pass_allowed_html, $pass_allowed_protocols;

	return zc_kses_split2( $matches[0], $pass_allowed_html, $pass_allowed_protocols );
}

/**
 * Callback for `zc_kses_split()` for fixing malformed HTML tags.
 *
 ...
 */
function zc_kses_split2( $content, $allowed_html, $allowed_protocols ) {
	$content = zc_kses_stripslashes( $content );
	...
	if ( 1 === preg_match( '~^(?:</[^a-zA-Z][^>]*>|<![a-z][^>]*>)$~', $content ) ) {
		...
		do {
			$prev    = $content;
			$content = zc_kses( $content, $allowed_html, $allowed_protocols );
		} while ( $prev !== $content );
	...
	if ( str_starts_with( $content, '<!--' ) ) {
		...
		while ( ( $newstring = zc_kses( $content, $allowed_html, $allowed_protocols ) ) !== $content ) {
			$content = $newstring;
		}
	...
	if ( ! is_array( $allowed_html ) ) {
		$allowed_html = zc_kses_allowed_html( $allowed_html );
	}
	...
	return zc_kses_attr( $elem, $attrlist, $allowed_html, $allowed_protocols );
}

/**
 * Removes all attributes, if none are allowed for this element.
 *
 * If some are allowed it calls `zc_kses_hair()` to split them further, and then
 * it builds up new HTML code from the data that `zc_kses_hair()` returns. It also
 ...
 */
function zc_kses_attr( $element, $attr, $allowed_html, $allowed_protocols ) {
	if ( ! is_array( $allowed_html ) ) {
		$allowed_html = zc_kses_allowed_html( $allowed_html );
	}
	...
	// Split it.
	$attrarr = zc_kses_hair( $attr, $allowed_protocols );
	...
	foreach ( $attrarr as $arreach ) {
		...
		if ( zc_kses_attr_check( $arreach['name'], $arreach['value'], $arreach['whole'], $arreach['vless'], $element, $allowed_html ) ) {
			$attr2 .= ' ' . $arreach['whole'];
 ...
 */
function zc_kses_attr_check( &$name, &$value, &$whole, $vless, $element, $allowed_html ) {
	...
	if ( 'style' === $name_low ) {
		$decoded_value = ZC_HTML_Decoder::decode_attribute( $value );
	...
	if ( is_array( $allowed_attr[ $name_low ] ) ) {
		...
		foreach ( $allowed_attr[ $name_low ] as $currkey => $currval ) {
			if ( ! zc_kses_check_attr_val( $value, $vless, $currkey, $currval ) ) {
				$name  = '';
				$value = '';
				$whole = '';
				return false;
 ...
 * Example:
 *
 *     $attrs = zc_kses_hair( 'class="is-wide" inert data-lazy=\'&lt;img&#00062\' =/🐮=/' );
 *     $attrs === array(
 ...
 */
function zc_kses_hair( $attr, $allowed_protocols ) {
	$attributes = array();
	$uris       = zc_kses_uri_attributes();
 ...
 *
 * Based on `zc_kses_split2()` and `zc_kses_attr()`.
 *
 * @since 4.2.3
 *
 ...
 */
function zc_kses_attr_parse( $element ) {
	...
	// Split it.
	$attrarr = zc_kses_hair_parse( $attr );
	if ( false === $attrarr ) {
		return false;
	}
 ...
 *
 * Based on `zc_kses_hair()` but does not return a multi-dimensional array.
 *
 * @since 4.2.3
 *
 * @param string $attr Attribute list from HTML element to closing HTML element tag.
 ...
 */
function zc_kses_hair_parse( $attr ) {
	if ( '' === $attr ) {
		return array();
	}
 ...
 */
function zc_kses_check_attr_val( $value, $vless, $checkname, $checkvalue ) {
	$ok = true;
 ...
 */
function zc_kses_bad_protocol( $content, $allowed_protocols ) {
	$content = zc_kses_no_null( $content );
	...
	do {
		...
		$content          = zc_kses_bad_protocol_once( $content, $allowed_protocols );
 ...
 * @return string Filtered content.
 */
function zc_kses_no_null( $content, $options = null ) {
 ...
 */
function zc_kses_stripslashes( $content ) {
 ...
 */
function zc_kses_array_lc( $inarray ) {
	$outarray = array();
...
/**
 * Handles parsing errors in `zc_kses_hair()`.
 *
 ...
 * @return string
 */
function zc_kses_html_error( $attr ) {
 ...
 */
function zc_kses_bad_protocol_once( $content, $allowed_protocols, $count = 1 ) {
	...
	if ( isset( $content2[1] ) && ! preg_match( '%/\?%', $content2[0] ) ) {
		...
		$protocol = zc_kses_bad_protocol_once2( $content2[0], $allowed_protocols );
		if ( 'feed:' === $protocol ) {
			if ( $count > 2 ) {
				return '';
			}
			$content = zc_kses_bad_protocol_once( $content, $allowed_protocols, ++$count );
			if ( empty( $content ) ) {
				return $content;
			}
		}
...
/**
 * Callback for `zc_kses_bad_protocol_once()` regular expression.
 *
 ...
 * @return string Sanitized content.
 */
function zc_kses_bad_protocol_once2( $scheme, $allowed_protocols ) {
	$scheme = zc_kses_decode_entities( $scheme );
	$scheme = preg_replace( '/\s/', '', $scheme );
	$scheme = zc_kses_no_null( $scheme );
 ...
 */
function zc_kses_normalize_entities( $content, $context = 'html' ) {
	 ...
	 */
	$content = preg_replace_callback( '/&amp;#(0*[1-9][0-9]{0,6});/', 'zc_kses_normalize_entities2', $content );
	$content = preg_replace_callback( '/&amp;#[Xx](0*[1-9A-Fa-f][0-9A-Fa-f]{0,5});/', 'zc_kses_normalize_entities3', $content );
	if ( 'xml' === $context ) {
		$content = preg_replace_callback( '/&amp;([A-Za-z]{2,8}[0-9]{0,2});/', 'zc_kses_xml_named_entities', $content );
	} else {
		$content = preg_replace_callback( '/&amp;([A-Za-z]{2,8}[0-9]{0,2});/', 'zc_kses_named_entities', $content );
	}
...
/**
 * Callback for `zc_kses_normalize_entities()` regular expression.
 *
 ...
 */
function zc_kses_named_entities( $matches ) {
 ...
 * @return string Correctly encoded entity.
 */
function zc_kses_xml_named_entities( $matches ) {
 ...
 * Callback for `zc_kses_normalize_entities()` regular expression.
 *
 * This function helps `zc_kses_normalize_entities()` to only accept 16-bit
 ...
 * @return string Correctly encoded entity.
 */
function zc_kses_normalize_entities2( $matches ) {
	if ( empty( $matches[1] ) ) {
		return '';
	}
...
/**
 * Callback for `zc_kses_normalize_entities()` for regular expression.
 *
 * This function helps `zc_kses_normalize_entities()` to only accept valid Unicode
 ...
 * @return string Correctly encoded entity.
 */
function zc_kses_normalize_entities3( $matches ) {
	if ( empty( $matches[1] ) ) {
		return '';
	}
 ...
 */
function zc_kses_decode_entities( $content ) {
	$content = preg_replace_callback( '/&#([0-9]+);/', '_zc_kses_decode_entities_chr', $content );
	$content = preg_replace_callback( '/&#[Xx]([0-9A-Fa-f]+);/', '_zc_kses_decode_entities_chr_hexdec', $content );

	return $content;
}

/**
 * Regex callback for `zc_kses_decode_entities()`.
 *
 * @since 2.9.0
 * @access private
 * @ignore
 ...
 * @return string
 */
function _zc_kses_decode_entities_chr( $matches ) {
 ...
 * @return string
 */
function _zc_kses_decode_entities_chr_hexdec( $matches ) {
 ...
 * @return string Filtered content.
 */
function zc_filter_kses( $data ) {
	return addslashes( zc_kses( stripslashes( $data ), current_filter() ) );
}
 ...
 * @return string Filtered content.
 */
function zc_kses_data( $data ) {
	return zc_kses( $data, current_filter() );
}
 ...
 */
function zc_filter_post_kses( $data ) {
	return addslashes( zc_kses( stripslashes( $data ), 'post' ) );
}
 ...
 */
function zc_filter_global_styles_post( $data ) {
	$decoded_data        = json_decode( zc_unslash( $data ), true );
	...
	if (
		...
		$data_to_encode = ZC_Theme_JSON::remove_insecure_properties( $decoded_data, 'custom' );
		 ...
		 *
		 * This matches the escaping in {@see ZC_REST_Global_Styles_Controller::prepare_item_for_database()}.
		 */
		return zc_slash( zc_json_encode( $data_to_encode, JSON_UNESCAPED_SLASHES | JSON_HEX_TAG | JSON_HEX_AMP ) );
	}
	return $data;
}
 ...
 * @return string Filtered post content with allowed HTML tags and attributes intact.
 */
function zc_kses_post( $data ) {
	return zc_kses( $data, 'post' );
}
 ...
 */
function zc_kses_post_deep( $data ) {
	return map_deep( $data, 'zc_kses_post' );
}
 ...
 */
function zc_filter_nohtml_kses( $data ) {
	return addslashes( zc_kses( stripslashes( $data ), 'strip' ) );
}

/**
 * Adds all KSES input form content filters.
 *
 * All hooks have default priority. The `zc_filter_kses()` function is added to
 * the 'pre_comment_content' and 'title_save_pre' hooks.
 *
 * The `zc_filter_post_kses()` function is added to the 'content_save_pre',
...
function kses_init_filters() {
	// Normal filtering.
	add_filter( 'title_save_pre', 'zc_filter_kses' );

	// Comment filtering.
	if ( current_user_can( 'unfiltered_html' ) ) {
		add_filter( 'pre_comment_content', 'zc_filter_post_kses' );
	} else {
		add_filter( 'pre_comment_content', 'zc_filter_kses' );
	}

	// Global Styles filtering: Global Styles filters should be executed before normal post_kses HTML filters.
	add_filter( 'content_save_pre', 'zc_filter_global_styles_post', 9 );
	add_filter( 'content_filtered_save_pre', 'zc_filter_global_styles_post', 9 );

	// Post filtering.
	add_filter( 'content_save_pre', 'zc_filter_post_kses' );
	add_filter( 'excerpt_save_pre', 'zc_filter_post_kses' );
	add_filter( 'content_filtered_save_pre', 'zc_filter_post_kses' );
}
...
function kses_remove_filters() {
	// Normal filtering.
	remove_filter( 'title_save_pre', 'zc_filter_kses' );
	...
	remove_filter( 'content_save_pre', 'zc_filter_global_styles_post', 9 );
	remove_filter( 'content_filtered_save_pre', 'zc_filter_global_styles_post', 9 );

	// Post filtering.
	remove_filter( 'content_save_pre', 'zc_filter_post_kses' );
	remove_filter( 'excerpt_save_pre', 'zc_filter_post_kses' );
	remove_filter( 'content_filtered_save_pre', 'zc_filter_post_kses' );
}
	...
	foreach ( $css_array as $css_item ) {
		...
		if ( $found && $url_attr ) {
			...
			foreach ( $url_matches[0] as $url_match ) {
				...
				if ( empty( $url ) || zc_kses_bad_protocol( $url, $allowed_protocols ) !== $url ) {
					$found = false;
					break;
				} else {
 ...
 */
function _zc_add_global_attributes( $value ) {
 ...
 */
function _zc_kses_allow_pdf_objects( $url ) {
	...
	$upload_info = zc_upload_dir( null, false );
	$parsed_url  = zc_parse_url( $upload_info['url'] );
