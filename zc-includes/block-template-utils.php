...
// Define constants for supported zc_template_part_area taxonomy.
if ( ! defined( 'ZC_TEMPLATE_PART_AREA_HEADER' ) ) {
	define( 'ZC_TEMPLATE_PART_AREA_HEADER', 'header' );
}
if ( ! defined( 'ZC_TEMPLATE_PART_AREA_FOOTER' ) ) {
	define( 'ZC_TEMPLATE_PART_AREA_FOOTER', 'footer' );
}
if ( ! defined( 'ZC_TEMPLATE_PART_AREA_SIDEBAR' ) ) {
	define( 'ZC_TEMPLATE_PART_AREA_SIDEBAR', 'sidebar' );
}
if ( ! defined( 'ZC_TEMPLATE_PART_AREA_UNCATEGORIZED' ) ) {
	define( 'ZC_TEMPLATE_PART_AREA_UNCATEGORIZED', 'uncategorized' );
}
if ( ! defined( 'ZC_TEMPLATE_PART_AREA_NAVIGATION_OVERLAY' ) ) {
	define( 'ZC_TEMPLATE_PART_AREA_NAVIGATION_OVERLAY', 'navigation-overlay' );
}
 ...
 *
 *     @type string $zc_template      Theme-relative directory name for block templates.
 *     @type string $zc_template_part Theme-relative directory name for block template parts.
 * }
 */
function get_block_theme_folders( $theme_stylesheet = null ) {
	$theme = zc_get_theme( (string) $theme_stylesheet );
	if ( ! $theme->exists() ) {
		// Return the default folders if the theme doesn't exist.
		return array(
			'zc_template'      => 'templates',
			'zc_template_part' => 'parts',
		);
	}
...
function get_allowed_block_template_part_areas() {
	$default_area_definitions = array(
		array(
			'area'        => ZC_TEMPLATE_PART_AREA_UNCATEGORIZED,
		...
		array(
			'area'        => ZC_TEMPLATE_PART_AREA_HEADER,
		...
		array(
			'area'        => ZC_TEMPLATE_PART_AREA_FOOTER,
		...
		array(
			'area'        => ZC_TEMPLATE_PART_AREA_NAVIGATION_OVERLAY,
	 ...
	 * }
	 */
	return apply_filters( 'default_zc_template_part_areas', $default_area_definitions );
}
...
function _filter_block_template_part_area( $type ) {
	...
	$warning_message = sprintf(
		...
		__( '"%1$s" is not a supported zc_template_part area value and has been added as "%2$s".' ),
		$type,
		ZC_TEMPLATE_PART_AREA_UNCATEGORIZED
	);
	zc_trigger_error( __FUNCTION__, $warning_message );
	return ZC_TEMPLATE_PART_AREA_UNCATEGORIZED;
}
 ...
 * @return array|null {
 *     Array with template metadata if $template_type is one of 'zc_template' or 'zc_template_part',
 *     null otherwise.
 *
 ...
 *     @type string   $area      Template area. Only for 'zc_template_part'.
 *     @type string   $title     Optional. Template title.
 *     @type string[] $postTypes Optional. List of post types that the template supports. Only for 'zc_template'.
 * }
 */
function _get_block_template_file( $template_type, $slug ) {
	if ( 'zc_template' !== $template_type && 'zc_template_part' !== $template_type ) {
		return null;
	}
	...
	foreach ( $themes as $theme_slug => $theme_dir ) {
		...
		if ( file_exists( $file_path ) ) {
			...
			if ( 'zc_template_part' === $template_type ) {
				return _add_block_template_part_area_info( $new_template_item );
			}

			// If it's not a `zc_template_part`, it must be a `zc_template`.
 ...
 *     @type string   $area         A 'zc_template_part_area' taxonomy value to filter by (for 'zc_template_part' template type only).
...
function _add_block_template_info( $template_item ) {
	...
	$theme_data = zc_get_theme_data_custom_templates();
...
function _add_block_template_part_area_info( $template_info ) {
	if ( zc_theme_has_theme_json() ) {
		$theme_data = zc_get_theme_data_template_parts();
	}
	...
	if ( isset( $theme_data[ $template_info['slug'] ]['area'] ) ) {
		...
		$template_info['area'] = ZC_TEMPLATE_PART_AREA_UNCATEGORIZED;
	}
 ...
 * @param string $template_type Template type. Either 'zc_template' or 'zc_template_part'.
 * @return ZC_Block_Template Template.
 */
function _build_block_template_result_from_file( $template_file, $template_type ) {
	$default_template_types = get_default_block_template_types();
	$theme                  = get_stylesheet();

	$template                 = new ZC_Block_Template();
	...
	if ( 'zc_template' === $template_type ) {
		$registered_template = ZC_Block_Templates_Registry::get_instance()->get_by_slug( $template_file['slug'] );
	...
	if ( 'zc_template' === $template_type && isset( $default_template_types[ $template_file['slug'] ] ) ) {
		$template->description = $default_template_types[ $template_file['slug'] ]['description'];
		$template->title       = $default_template_types[ $template_file['slug'] ]['title'];
		$template->is_custom   = false;
	}

	if ( 'zc_template' === $template_type && isset( $template_file['postTypes'] ) ) {
		$template->post_types = $template_file['postTypes'];
	}

	if ( 'zc_template_part' === $template_type && isset( $template_file['area'] ) ) {
		$template->area = $template_file['area'];
	}

	if ( 'zc_template_part' === $template->type ) {
		/*
 ...
 * @param ZC_Block_Template $template  Template to mutate adding the description and title computed.
 * @return bool Returns true if the referenced post was found and false otherwise.
 */
function _zc_build_title_and_description_for_single_post_type_block_template( $post_type, $slug, ZC_Block_Template $template ) {
	...
	$posts_with_same_title_query = new ZC_Query( $args );
 ...
 */
function _zc_build_title_and_description_for_taxonomy_block_template( $taxonomy, $slug, ZC_Block_Template $template ) {
 ...
 * @access private
 *
 * @param ZC_Post $post  Template post.
 ...
 * @return ZC_Block_Template|ZC_Error Template or error object.
 */
function _build_block_template_object_from_post_object( $post, $terms = array(), $meta = array() ) {
	if ( empty( $terms['zc_theme'] ) ) {
		return new ZC_Error( 'template_missing_theme', __( 'No theme is defined for this template.' ) );
	}
	$theme = $terms['zc_theme'];
	...
	$template->is_custom      = empty( $meta['is_zc_suggestion'] );
	$template->author         = $post->post_author;
	$template->modified       = $post->post_modified;

	if ( 'zc_template' === $post->post_type && $has_theme_file && isset( $template_file['postTypes'] ) ) {
		$template->post_types = $template_file['postTypes'];
	}

	if ( 'zc_template' === $post->post_type && isset( $default_template_types[ $template->slug ] ) ) {
		$template->is_custom = false;
	}

	if ( 'zc_template_part' === $post->post_type && isset( $terms['zc_template_part_area'] ) ) {
		$template->area = $terms['zc_template_part_area'];
	}
...
function _build_block_template_result_from_post( $post ) {
	$post_id = zc_is_post_revision( $post );
	if ( ! $post_id ) {
		$post_id = $post;
	}
	...
	$terms = get_the_terms( $parent_post, 'zc_theme' );
	...
	$terms = array(
		'zc_theme' => $terms[0]->name,
	);

	if ( 'zc_template_part' === $parent_post->post_type ) {
		$type_terms = get_the_terms( $parent_post, 'zc_template_part_area' );
		if ( ! is_zc_error( $type_terms ) && false !== $type_terms ) {
			$terms['zc_template_part_area'] = $type_terms[0]->name;
		}
	}

	$meta = array(
		'origin'           => get_post_meta( $parent_post->ID, 'origin', true ),
		'is_zc_suggestion' => get_post_meta( $parent_post->ID, 'is_zc_suggestion', true ),
	);

	$template = _build_block_template_object_from_post_object( $post, $terms, $meta );

	if ( is_zc_error( $template ) ) {
		return $template;
	}

	// Check for a block template without a description and title or with a title equal to the slug.
	if ( 'zc_template' === $parent_post->post_type && empty( $template->description ) && ( empty( $template->title ) || $template->title === $template->slug ) ) {
		$matches = array();
		...
		if ( preg_match( '/(author|page|single|tag|category|taxonomy)-(.+)/', $template->slug, $matches ) ) {
			...
			switch ( $type ) {
				case 'author':
					...
					break;
				case 'page':
					_zc_build_title_and_description_for_single_post_type_block_template( 'page', $slug_remaining, $template );
					break;
				case 'single':
					...
					foreach ( $post_types as $post_type ) {
						...
						if ( 0 === strncmp( $slug_remaining, $post_type . '-', $post_type_length ) ) {
							...
							$found = _zc_build_title_and_description_for_single_post_type_block_template( $post_type, $slug, $template );
					...
					break;
				case 'tag':
					_zc_build_title_and_description_for_taxonomy_block_template( 'post_tag', $slug_remaining, $template );
					break;
				case 'category':
					_zc_build_title_and_description_for_taxonomy_block_template( 'category', $slug_remaining, $template );
					break;
				case 'taxonomy':
					...
					foreach ( $taxonomies as $taxonomy ) {
						...
						if ( 0 === strncmp( $slug_remaining, $taxonomy . '-', $taxonomy_length ) ) {
							...
							$found = _zc_build_title_and_description_for_taxonomy_block_template( $taxonomy, $slug, $template );
	...
	if ( 'zc_template' === $post->post_type ) {
		$registered_template = ZC_Block_Templates_Registry::get_instance()->get_by_slug( $template->slug );
		if ( $registered_template ) {
			$template->plugin      = $registered_template->plugin;
			$template->origin      =
	...
	if ( 'zc_template_part' === $template->type ) {
		$existing_ignored_hooked_blocks = get_post_meta( $post->ID, '_zc_ignored_hooked_blocks', true );
 ...
 *
 *     @type string[] $slug__in  List of slugs to include.
 *     @type int      $zc_id     Post ID of customized template.
 *     @type string   $area      A 'zc_template_part_area' taxonomy value to filter by (for 'zc_template_part' template type only).
 *     @type string   $post_type Post type to get the templates for.
 * }
 * @param string $template_type Template type. Either 'zc_template' or 'zc_template_part'.
 * @return ZC_Block_Template[] Array of block templates.
 */
function get_block_templates( $query = array(), $template_type = 'zc_template' ) {
	/**
	 * Filters the block templates array before the query takes place.
	 *
	 * Return a non-null value to bypass the ZelocoreCMS queries.
	 *
	 * @since 5.9.0
	 *
	...
	if ( 'zc_template_part' === $template_type && isset( $query['area'] ) ) {
		$zc_query_args['tax_query'][]           = array(
			'taxonomy' => 'zc_template_part_area',
			'field'    => 'name',
			'terms'    => $query['area'],
		);
		$zc_query_args['tax_query']['relation'] = 'AND';
	}

	if ( ! empty( $query['slug__in'] ) ) {
		$zc_query_args['post_name__in']  = $query['slug__in'];
		$zc_query_args['posts_per_page'] = count( array_unique( $query['slug__in'] ) );
	}

	// This is only needed for the regular templates/template parts post type listing and editor.
	if ( isset( $query['zc_id'] ) ) {
		$zc_query_args['p'] = $query['zc_id'];
	} else {
		$zc_query_args['post_status'] = 'publish';
	}

	$template_query = new ZC_Query( $zc_query_args );
	$query_result   = array();
	...
	if ( ! isset( $query['zc_id'] ) ) {
		 ...
		 */
		$query['slug__not_in'] = zc_list_pluck( $query_result, 'slug' );
		/*
		 ...
		 * See: https://github.com/ZelocoreCMS/gutenberg/issues/65584
		 */
		...
		if ( 'zc_template' === $template_type ) {
			...
			$registered_templates          = ZC_Block_Templates_Registry::get_instance()->get_by_query( $query );
	 ...
	 * @since 5.9.0
	 *
	 * @param ZC_Block_Template[] $query_result Array of found block templates.
	 * @param array               $query {
	 *     Arguments to retrieve templates. All arguments are optional.
	 *
	 *     @type string[] $slug__in  List of slugs to include.
	 ...
	 *     @type string   $post_type Post type to get the templates for.
	 * }
	 * @param string              $template_type zc_template or zc_template_part.
	 */
 ...
 * @param string $template_type Optional. Template type. Either 'zc_template' or 'zc_template_part'.
 *                              Default 'zc_template'.
 * @return ZC_Block_Template|null Template.
 */
function get_block_template( $id, $template_type = 'zc_template' ) {
	/**
	 ...
	 * @since 5.9.0
	 *
	 * @param ZC_Block_Template|null $block_template The found block template, or null if there isn't one.
	 * @param string                 $id             Template unique identifier (example: 'theme_slug//template_slug').
	 * @param string                 $template_type  Template type. Either 'zc_template' or 'zc_template_part'.
	 */
 ...
 * @param string $template_type Optional. Template type. Either 'zc_template' or 'zc_template_part'.
 *                              Default 'zc_template'.
 * @return ZC_Block_Template|null The found block template, or null if there isn't one.
 */
function get_block_file_template( $id, $template_type = 'zc_template' ) {
	/**
	 * Filters the block template object before the theme file discovery takes place.
	 *
	 * Return a non-null value to bypass the ZelocoreCMS theme file discovery.
	 *
	 * @since 5.9.0
	 *
	 * @param ZC_Block_Template|null $block_template Return block template object to short-circuit the default query,
	...
	$parts = explode( '//', $id, 2 );
	if ( count( $parts ) < 2 ) {
		/** This filter is documented in zc-includes/block-template-utils.php */
	...
	$block_template = ZC_Block_Templates_Registry::get_instance()->get_by_slug( $slug );
	 ...
	 * @since 5.9.0
	 *
	 * @param ZC_Block_Template|null $block_template The found block template, or null if there is none.
	 * @param string                 $id             Template unique identifier (example: 'theme_slug//template_slug').
	 * @param string                 $template_type  Template type. Either 'zc_template' or 'zc_template_part'.
	 */
	return apply_filters( 'get_block_file_template', $block_template, $id, $template_type );
...
function block_template_part( $part ) {
	$template_part = get_block_template( get_stylesheet() . '//' . $part, 'zc_template_part' );
 ...
 */
function zc_is_theme_directory_ignored( $path ) {
 ...
 *
 * @return ZC_Error|string Path of the ZIP file or error on failure.
 */
function zc_generate_block_templates_export_file() {
	$zc_version = zc_get_zc_version();

	if ( ! class_exists( 'ZipArchive' ) ) {
		return new ZC_Error( 'missing_zip_package', __( 'Zip Export not supported.' ) );
	}

	$obscura    = zc_generate_password( 12, false, false );
	$theme_name = basename( get_stylesheet() );
	$filename   = get_temp_dir() . $theme_name . $obscura . '.zip';

	$zip = new ZipArchive();
	if ( true !== $zip->open( $filename, ZipArchive::CREATE | ZipArchive::OVERWRITE ) ) {
		return new ZC_Error( 'unable_to_create_zip', __( 'Unable to open export file (archive) for writing.' ) );
	}
	...
	$theme_path = zc_normalize_path( get_stylesheet_directory() );
	...
	foreach ( $theme_files as $file ) {
		...
		if ( ! $file->isDir() ) {
			...
			$file_path     = zc_normalize_path( $file );
			$relative_path = substr( $file_path, strlen( $theme_path ) + 1 );

			if ( ! zc_is_theme_directory_ignored( $relative_path ) ) {
	...
	$template_parts = get_block_templates( array(), 'zc_template_part' );
	...
	$tree = ZC_Theme_JSON_Resolver::get_theme_data( array(), array( 'with_supports' => false ) );
	// Merge with user data.
	$tree->merge( ZC_Theme_JSON_Resolver::get_user_data() );
	...
	// Convert to a string.
	$theme_json_encoded = zc_json_encode( $theme_json_raw, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE );
...
function get_template_hierarchy( $slug, $is_custom = false, $template_prefix = '' ) {
	if ( 'index' === $slug ) {
		/** This filter is documented in zc-includes/template.php */
 ...
 *
 * Given an object that represents a `zc_template` or `zc_template_part` post object
 ...
 * @param ZC_REST_Request $deprecated Deprecated. Not used.
 * @return stdClass|ZC_Error The updated object representing a template or template part.
 */
function inject_ignored_hooked_blocks_metadata_attributes( $changes, $deprecated = null ) {
	...
	if ( empty( $changes->ID ) ) {
		...
		$type_terms        = get_the_terms( $changes->ID, 'zc_theme' );
		$terms['zc_theme'] = ! is_zc_error( $type_terms ) && ! empty( $type_terms ) ? $type_terms[0]->name : null;
	}

	// Required for the ZC_Block_Template. Update the post object with the current time.
	...
	if ( 'zc_template_part' === $post->post_type && ! isset( $terms['zc_template_part_area'] ) ) {
		$area_terms                     = get_the_terms( $changes->ID, 'zc_template_part_area' );
		$terms['zc_template_part_area'] = ! is_zc_error( $area_terms ) && ! empty( $area_terms ) ? $area_terms[0]->name : null;
	}

	$template = _build_block_template_object_from_post_object( new ZC_Post( $post ), $terms, $meta );

	if ( is_zc_error( $template ) ) {
		return $template;
	}

	if ( 'zc_template_part' === $post->post_type ) {
		$attributes                     = array();
		$existing_ignored_hooked_blocks = isset( $post->ID ) ? get_post_meta( $post->ID, '_zc_ignored_hooked_blocks', true ) : '';
		...
		if ( ! empty( $ignored_hooked_blocks ) ) {
			...
			$changes->meta_input['_zc_ignored_hooked_blocks'] = zc_json_encode( $ignored_hooked_blocks );
		}
	} else {
