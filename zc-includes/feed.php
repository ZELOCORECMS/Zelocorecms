 ...
 */
function get_zc_title_rss( $deprecated = '&#8211;' ) {
	if ( '&#8211;' !== $deprecated ) {
	 ...
	 * @param string $deprecated Unused.
	 */
	return apply_filters( 'get_zc_title_rss', zc_get_document_title(), $deprecated );
}
 ...
 * @param string $deprecated Unused.
 */
function zc_title_rss( $deprecated = '&#8211;' ) {
	if ( '&#8211;' !== $deprecated ) {
		/* translators: %s: 'document_title_separator' filter name. */
		_deprecated_argument( __FUNCTION__, '4.4.0', sprintf( __( 'Use the %s filter instead.' ), '<code>document_title_separator</code>' ) );
	}
	 ...
	 * @since 4.4.0 The `$sep` parameter was deprecated and renamed to `$deprecated`.
	 *
	 * @see get_zc_title_rss()
	 *
	 * @param string $zc_title_rss The current blog title.
	 * @param string $deprecated   Unused.
	 */
	echo apply_filters( 'zc_title_rss', get_zc_title_rss(), $deprecated );
}
 ...
 * @since 2.5.0
 *
 * @param int|ZC_Comment $comment_id Optional comment object or ID. Defaults to global comment object.
 */
 ...
 *
 * @param int|ZC_Comment $comment Optional. Comment object or ID. Defaults to global comment object.
 */
 ...
 *
 * In the case of ZelocoreCMS, text is defined as containing no markup,
...
function prep_atom_text_construct( $data ) {
	...
	if ( ! function_exists( 'xml_parser_create' ) ) {
		zc_trigger_error( '', __( "PHP's XML extension is not available. Please contact your hosting provider to enable PHP's XML extension." ) );
...
function get_self_link() {
	...
	return set_url_scheme( 'http://' . $domain . zc_unslash( $_SERVER['REQUEST_URI'] ) );
}
...
/**
 * Gets the UTC time of the most recently modified post from ZC_Query.
 *
...
function get_feed_build_date( $format ) {
	...
	if ( ! empty( $zc_query ) && $zc_query->have_posts() ) {
		// Extract the post modified times from the posts.
		$modified_times = zc_list_pluck( $zc_query->posts, 'post_modified_gmt' );

		// If this is a comment feed, check those objects too.
		if ( $zc_query->is_comment_feed() && $zc_query->comment_count ) {
			// Extract the comment modified times from the comments.
			$comment_times = zc_list_pluck( $zc_query->comments, 'comment_date_gmt' );
 ...
 * @return SimplePie\SimplePie|ZC_Error SimplePie object on success or ZC_Error object on failure.
 */
function fetch_feed( $url ) {
	...
	$feed->get_registry()->register( SimplePie\Sanitize::class, 'ZC_SimplePie_Sanitize_KSES', true );

	/*
	 * We must manually overwrite $feed->sanitize because SimplePie's constructor
	 * sets it before we have a chance to set the sanitization class.
	 */
	$feed->sanitize = new ZC_SimplePie_Sanitize_KSES();

	// Register the cache handler using the recommended method for SimplePie 1.3 or later.
	if ( method_exists( 'SimplePie_Cache', 'register' ) ) {
		SimplePie_Cache::register( 'zc_transient', 'ZC_Feed_Cache_Transient' );
	...
	$feed->get_registry()->register( SimplePie\File::class, 'ZC_SimplePie_File', true );

	/** This filter is documented in zc-includes/class-zc-feed-cache-transient.php */
	$feed->set_cache_duration( apply_filters( 'zc_feed_cache_transient_lifetime', 12 * HOUR_IN_SECONDS, $url ) );
	 ...
	 */
	do_action_ref_array( 'zc_feed_options', array( &$feed, $url ) );
	...
	if ( empty( $url ) ) {
		...
		if ( $feed->error() ) {
			return new ZC_Error( 'simplepie-error', $feed->error() );
		}
		...
		if ( ! empty( $simplepie_errors ) ) {
			return new ZC_Error( 'simplepie-error', $simplepie_errors );
		}
