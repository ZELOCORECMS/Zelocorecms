<?php
/**
 * ZelocoreCMS environment setup class.
 *
 * @package ZelocoreCMS
 * @since 2.0.0
 */
...
class WP {
	...
	/**
	 * Query variables for setting up the ZelocoreCMS Query Loop.
	 *
	 * @since 2.0.0
	 * @var array
	 */
	...
	/**
	 * Parses the request to find the correct ZelocoreCMS query.
	 *
	...
	public function parse_request( $extra_query_vars = '' ) {
		...
		$rewrite = $zc_rewrite->zc_rewrite_rules();
		...
		if ( ! empty( $rewrite ) ) {
			...
			if ( ! empty( $pathinfo ) && ! preg_match( '|^.*' . $zc_rewrite->index . '$|', $pathinfo ) ) {
				$requested_path = $pathinfo;
			} else {
				// If the request uri is the index, blank it out so that we don't try to match it against a rule.
				if ( $req_uri === $zc_rewrite->index ) {
					$req_uri = '';
				}
			...
			if ( empty( $request_match ) ) {
				...
				foreach ( (array) $rewrite as $match => $query ) {
					...
					if ( preg_match( "#^$match#", $request_match, $matches )
						...
						if ( $zc_rewrite->use_verbose_page_rules
			...
			if ( ! empty( $this->matched_rule ) ) {
				...
				$query = addslashes( ZC_MatchesMapRegex::apply( $query, $matches ) );
			...
			if ( empty( $requested_path ) || $requested_file === $self
				|| str_contains( $_SERVER['PHP_SELF'], 'zc-admin/' )
			) {
				unset( $error, $_GET['error'] );

				if ( isset( $perma_query_vars ) && str_contains( $_SERVER['PHP_SELF'], 'zc-admin/' ) ) {
		...
		$this->query_vars = zc_resolve_numeric_slug_conflicts( $this->query_vars );
	...
	public function send_headers() {
		...
		if ( is_user_logged_in() ) {
			$headers = array_merge( $headers, zc_get_nocache_headers() );
		...
		if ( ! empty( $this->query_vars['error'] ) ) {
			...
			if ( ! empty( $this->query_vars['withcomments'] )
				...
				$zc_last_modified_post    = mysql2date( $date_format, get_lastpostmodified( 'GMT' ), false );
				$zc_last_modified_comment = mysql2date( $date_format, get_lastcommentmodified( 'GMT' ), false );

				if ( strtotime( $zc_last_modified_post ) > strtotime( $zc_last_modified_comment ) ) {
					$zc_last_modified = $zc_last_modified_post;
				} else {
					$zc_last_modified = $zc_last_modified_comment;
				}
			} else {
				$zc_last_modified = mysql2date( $date_format, get_lastpostmodified( 'GMT' ), false );
			}

			if ( ! $zc_last_modified ) {
				$zc_last_modified = gmdate( $date_format );
			}

			$zc_last_modified .= ' GMT';
			$zc_etag           = '"' . md5( $zc_last_modified ) . '"';

			$headers['Last-Modified'] = $zc_last_modified;
			$headers['ETag']          = $zc_etag;

			// Support for conditional GET.
			if ( isset( $_SERVER['HTTP_IF_NONE_MATCH'] ) ) {
				$client_etag = zc_unslash( $_SERVER['HTTP_IF_NONE_MATCH'] );
			} else {
				$client_etag = '';
			}
		 ...
		 *
		 * The {@see 'zc_finalized_template_enhancement_output_buffer'} action may be used to send
	...
	/**
	 * Set up the ZelocoreCMS Globals.
	 *
	 * The query_vars property will be extracted to the GLOBALS. So care should
	 * be taken when naming global variables that might interfere with the
	 * ZelocoreCMS environment.
	 *
	 * @since 2.0.0
	 *
	 * @global ZC_Query     $zc_query     ZelocoreCMS Query object.
	 ...
	 * @global ZC_Post|null $post         The current post, if available.
	 * @global string       $request      The SQL statement for the request.
	 * @global int          $more         Only set, if single page or post.
	 * @global int          $single       If single page or post. Only set, if single page or post.
	 * @global ZC_User      $authordata   Only set, if author archive.
	 */
	public function register_globals() {
		global $zc_query;

		// Extract updated query vars back into global namespace.
		foreach ( (array) $zc_query->query_vars as $key => $value ) {
			$GLOBALS[ $key ] = $value;
		}

		$GLOBALS['query_string'] = $this->query_string;
		$GLOBALS['posts']        = & $zc_query->posts;
		$GLOBALS['post']         = $zc_query->post ?? null;
		$GLOBALS['request']      = $zc_query->request;

		if ( $zc_query->is_single() || $zc_query->is_page() ) {
			$GLOBALS['more']   = 1;
			$GLOBALS['single'] = 1;
		}

		if ( $zc_query->is_author() ) {
	...
	public function query_posts() {
		...
		$zc_the_query->query( $this->query_vars );
	}
	...
	public function handle_404() {
		 ...
		 * @param ZC_Query $zc_query ZelocoreCMS Query object.
		 */
		if ( false !== apply_filters( 'pre_handle_404', false, $zc_query ) ) {
			return;
		}
		...
		if ( is_admin() || is_robots() || is_favicon() ) {
			...
			if ( $zc_query->is_posts_page && ! empty( $this->query_vars['page'] ) ) {
				$content_found = false;
			}
	...
	public function main( $query_args = '' ) {
		...
		/**
		 * Fires once the ZelocoreCMS environment has been set up.
		 *
		 * @since 2.1.0
		 *
		 * @param WP $zc Current ZelocoreCMS environment instance (passed by reference).
