...
function get_option( $option, $default_value = false ) {
	...
	if ( isset( $deprecated_keys[ $option ] ) && ! zc_installing() ) {
	...
	if ( ! zc_installing() ) {
		...
		if ( isset( $alloptions[ $option ] ) ) {
			...
			$notoptions = zc_cache_get( 'notoptions', 'options' );

			if ( ! is_array( $notoptions ) ) {
				$notoptions = array();
				zc_cache_set( 'notoptions', $notoptions, 'options' );
			}
			...
			$value = zc_cache_get( $option, 'options' );
			...
			if ( false === $value ) {
				...
				if ( is_object( $row ) ) {
					...
					zc_cache_add( $option, $value, 'options' );
...
function zc_prime_option_caches( $options ) {
	...
	zc_cache_set_multiple( $options_found, 'options' );
 ...
 */
function zc_prime_option_caches_by_group( $option_group ) {
	global $new_allowed_options;

	if ( isset( $new_allowed_options[ $option_group ] ) ) {
		zc_prime_option_caches( $new_allowed_options[ $option_group ] );
	}
}
...
function get_options( $options ) {
	zc_prime_option_caches( $options );
...
function zc_set_option_autoload_values( array $options ) {
	...
	foreach ( $options as $option => $autoload ) {
		zc_protect_special_option( $option ); // Ensure only valid options can be passed.
	...
	if ( $grouped_options['on'] ) {
		zc_cache_delete_multiple( $grouped_options['on'], 'options' );
		zc_cache_delete( 'alloptions', 'options' );
	} elseif ( $grouped_options['off'] ) {
		$alloptions = zc_load_alloptions( true );
		...
		zc_cache_set( 'alloptions', $alloptions, 'options' );
	}
 ...
 *
 * This is a wrapper for {@see zc_set_option_autoload_values()}, which can be used to set different autoload values for
 * each option at once.
 *
 * @since 6.4.0
 * @since 6.7.0 The autoload values 'yes' and 'no' are deprecated.
 *
 * @see zc_set_option_autoload_values()
 *
 * @param string[] $options  List of option names. Expected to not be SQL-escaped.
 * @param bool     $autoload Autoload value to control whether to load the options when ZelocoreCMS starts up.
 *                           For backward compatibility 'yes' and 'no' are also accepted, though using these values is
 *                           deprecated.
 * @return array Associative array of all provided $options as keys and boolean values for whether their autoload value
 *               was updated.
 */
function zc_set_options_autoload( array $options, $autoload ) {
	return zc_set_option_autoload_values(
 ...
 *
 * This is a wrapper for {@see zc_set_option_autoload_values()}, which can be used to set the autoload value for
 ...
 * @param bool   $autoload Autoload value to control whether to load the option when ZelocoreCMS starts up.
 *                         For backward compatibility 'yes' and 'no' are also accepted, though using these values is
 *                         deprecated.
 * @return bool True if the autoload value was modified, false otherwise.
 */
function zc_set_option_autoload( $option, $autoload ) {
	$result = zc_set_option_autoload_values( array( $option => $autoload ) );
	return $result[ $option ] ?? false;
}

/**
 * Protects ZelocoreCMS special option from being modified.
 *
 ...
 * @param string $option Option name.
 */
function zc_protect_special_option( $option ) {
...
function zc_load_alloptions( $force_cache = false ) {
	 ...
	 * zc_load_alloptions(), returning that value instead.
	 *
	 * @since 6.2.0
	 *
	 ...
	 */
	$alloptions = apply_filters( 'pre_zc_load_alloptions', null, $force_cache );
	if ( is_array( $alloptions ) ) {
		return $alloptions;
	}

	if ( ! zc_installing() || ! is_multisite() ) {
		$alloptions = zc_cache_get( 'alloptions', 'options', $force_cache );
	} else {
		$alloptions = false;
	}
	...
	if ( ! $alloptions ) {
		...
		if ( ! zc_installing() || ! is_multisite() ) {
			...
			zc_cache_add( 'alloptions', $alloptions, 'options' );
		}
	}
 ...
 *
 * If site is not multisite, then call zc_prime_option_caches().
 *
 * @since 6.6.0
 *
 * @see zc_prime_network_option_caches()
 *
 * @param string[] $options An array of option names to be loaded.
 */
function zc_prime_site_option_caches( array $options ) {
	zc_prime_network_option_caches( null, $options );
}
...
function zc_prime_network_option_caches( $network_id, array $options ) {
	...
	$cached_options = zc_cache_get_multiple( array_values( $cache_keys ), $cache_group );

	$notoptions_key = "$network_id:notoptions";
	$notoptions     = zc_cache_get( $notoptions_key, $cache_group );
	...
	zc_cache_set_multiple( $data, $cache_group );
	// If all options were found, no need to update `notoptions` cache.
	if ( count( $options_found ) === count( $options_to_prime ) ) {
		return;
	}
	...
	// Only update the cache if it was modified.
	if ( $update_notoptions ) {
		zc_cache_set( $notoptions_key, $notoptions, $cache_group );
	}
}
 ...
 * @since 6.6.0 Uses zc_prime_network_option_caches().
 *
 * @param int $network_id Optional. Network ID of network for which to prime network options cache. Defaults to current network.
 */
function zc_load_core_site_options( $network_id = null ) {
	if ( ! is_multisite() || zc_installing() ) {
		return;
	}
	...
	zc_prime_network_option_caches( $network_id, $core_options );
}
 ...
 * @param bool|null $autoload Optional. Whether to load the option when ZelocoreCMS starts up.
 *                            Accepts a boolean, or `null` to stick with the initial value or, if no initial value is
 *                            set, to leave the decision up to default heuristics in ZelocoreCMS.
 ...
 *                            For non-existent options, the default is null, which means ZelocoreCMS will determine
 *                            the autoload value.
...
function update_option( $option, $value, $autoload = null ) {
	 ...
	 *
	 * See https://core.trac.zelocorecms.org/ticket/38903
	 */
	...
	if ( ! zc_installing() ) {
		if ( ! isset( $update_args['autoload'] ) ) {
			...
			if ( isset( $alloptions[ $option ] ) ) {
				...
				zc_cache_set( $option, $serialized_value, 'options' );
			}
		} elseif ( in_array( $update_args['autoload'], zc_autoload_values_to_autoload(), true ) ) {
			// Delete the individual cache, then set in alloptions cache.
			zc_cache_delete( $option, 'options' );
 ...
 * aren't adding a protected ZelocoreCMS option. Care should be taken to not name
 ...
 *                              ZelocoreCMS. For backward compatibility 'yes' and 'no' are also accepted, though using
 ...
 *                              For options which are accessed only on few specific URLs, it is recommended
 *                              to not autoload them, by using false.
 *                              Default is null, which means ZelocoreCMS will determine the autoload value.
...
function add_option( $option, $value = '', $deprecated = '', $autoload = null ) {
	...
	if ( ! zc_installing() ) {
		if ( in_array( $autoload, zc_autoload_values_to_autoload(), true ) ) {
			$alloptions            = zc_load_alloptions( true );
			$alloptions[ $option ] = $serialized_value;
			zc_cache_set( 'alloptions', $alloptions, 'options' );
		} else {
	...
	$notoptions = zc_cache_get( 'notoptions', 'options' ); // Yes, again... we need it to be fresh.
...
/**
 * Removes an option by name. Prevents removal of protected ZelocoreCMS options.
 *
 * @since 1.2.0
 *
 * @global zcdb $zcdb ZelocoreCMS database abstraction object.
 ...
 */
function zc_determine_option_autoload_value( $option, $value, $serialized_value, $autoload ) {
	 ...
	 */
	$autoload = apply_filters( 'zc_default_autoload_value', null, $option, $value, $serialized_value );
	if ( is_bool( $autoload ) ) {
 ...
 */
function zc_filter_default_autoload_value_via_option_size( $autoload, $option, $value, $serialized_value ) {
	/**
	 ...
	 * @param string $option          The name of the option.
	 */
	$max_option_size = (int) apply_filters( 'zc_max_autoloaded_option_size', 150000, $option );
...
function delete_transient( $transient ) {
	...
	if ( zc_using_ext_object_cache() || zc_installing() ) {
		$result = zc_cache_delete( $transient, 'transient' );
	} else {
...
function get_transient( $transient ) {
	...
	if ( zc_using_ext_object_cache() || zc_installing() ) {
		$value = zc_cache_get( $transient, 'transient' );
	} else {
		...
		if ( ! zc_installing() ) {
			...
			if ( ! isset( $alloptions[ $transient_option ] ) ) {
				...
				zc_prime_option_caches( array( $transient_option, $transient_timeout ) );
...
function set_transient( $transient, $value, $expiration = 0 ) {
	...
	if ( zc_using_ext_object_cache() || zc_installing() ) {
		$result = zc_cache_set( $transient, $value, 'transient', $expiration );
	} else {
		$transient_timeout = '_transient_timeout_' . $transient;
		$transient_option  = '_transient_' . $transient;
		zc_prime_option_caches( array( $transient_option, $transient_timeout ) );
...
function zc_user_settings() {
	...
	if ( isset( $_COOKIE[ 'zc-settings-' . $user_id ] ) ) {
		$cookie = preg_replace( '/[^A-Za-z0-9=&_]/', '', $_COOKIE[ 'zc-settings-' . $user_id ] );
		...
		if ( isset( $_COOKIE[ 'zc-settings-time-' . $user_id ] ) ) {
			$current = (int) preg_replace( '/[^0-9]/', '', $_COOKIE[ 'zc-settings-time-' . $user_id ] );
		}
	...
	setcookie( 'zc-settings-' . $user_id, $settings, time() + YEAR_IN_SECONDS, SITECOOKIEPATH, '', $secure );
	setcookie( 'zc-settings-time-' . $user_id, time(), time() + YEAR_IN_SECONDS, SITECOOKIEPATH, '', $secure );
	$_COOKIE[ 'zc-settings-' . $user_id ] = $settings;
}
...
function set_user_setting( $name, $value ) {
	...
	return zc_set_all_user_settings( $all_user_settings );
}
 ...
 *                   Null if the current user is not a member of the site.
 */
function zc_set_all_user_settings( $user_settings ) {
	global $_updated_user_settings;
...
function delete_all_user_settings() {
	...
	setcookie( 'zc-settings-' . $user_id, ' ', time() - YEAR_IN_SECONDS, SITECOOKIEPATH );
}
...
function get_network_option( $network_id, $option, $default_value = false ) {
	...
	$notoptions_key = "$network_id:notoptions";
	$notoptions     = zc_cache_get( $notoptions_key, 'site-options' );
	...
	if ( ! is_multisite() ) {
		...
		if ( ! isset( $value ) || false === $value ) {
			...
			if ( is_object( $row ) ) {
				...
				zc_cache_set( $cache_key, $value, 'site-options' );
			} else {
				if ( ! is_array( $notoptions ) ) {
					$notoptions = array();
				}
				...
				$notoptions[ $option ] = true;
				zc_cache_set( $notoptions_key, $notoptions, 'site-options' );
...
function add_network_option( $network_id, $option, $value ) {
	...
	if ( ! is_multisite() ) {
		...
		// This option exists now.
		$notoptions = zc_cache_get( $notoptions_key, 'site-options' ); // Yes, again... we need it to be fresh.
...
function update_network_option( $network_id, $option, $value ) {
	 ...
	 * unnecessary database calls for otherwise identical object instances.
	 *
	 * See https://core.trac.zelocorecms.org/ticket/44956
	 */
	if ( $value === $old_value || maybe_serialize( $value ) === maybe_serialize( $old_value ) ) {
		return false;
	}
...
function delete_site_transient( $transient ) {
	...
	if ( zc_using_ext_object_cache() || zc_installing() ) {
		$result = zc_cache_delete( $transient, 'site-transient' );
	} else {
...
function get_site_transient( $transient ) {
	...
	if ( zc_using_ext_object_cache() || zc_installing() ) {
		$value = zc_cache_get( $transient, 'site-transient' );
	} else {
		...
		if ( ! in_array( $transient, $no_timeout, true ) ) {
			...
			zc_prime_site_option_caches( array( $transient_option, $transient_timeout ) );
...
function set_site_transient( $transient, $value, $expiration = 0 ) {
	...
	if ( zc_using_ext_object_cache() || zc_installing() ) {
		$result = zc_cache_set( $transient, $value, 'site-transient', $expiration );
	} else {
		$transient_timeout = '_site_transient_timeout_' . $transient;
		$option            = '_site_transient_' . $transient;
		zc_prime_site_option_caches( array( $option, $transient_timeout ) );
...
/**
 * Registers default settings available in ZelocoreCMS.
 *
 * The settings registered here are primarily useful for the REST API, so this
 * does not encompass all settings available in ZelocoreCMS.
 *
 * @since 4.7.0
...
function register_initial_settings() {
	...
	register_setting(
		...
		array(
			...
			'type'         => 'string',
			'description'  => __( 'ZelocoreCMS locale code.' ),
			'default'      => 'en_US',
		)
	);
 ...
 *
 * @global array $new_allowed_options
 * @global array $zc_registered_settings
 *
...
function register_setting( $option_group, $option_name, $args = array() ) {
	global $new_allowed_options, $zc_registered_settings;
	...
	if ( ! is_array( $zc_registered_settings ) ) {
		$zc_registered_settings = array();
	}
	...
	$zc_registered_settings[ $option_name ] = $args;
}
...
function unregister_setting( $option_group, $option_name, $deprecated = '' ) {
	...
	if ( isset( $zc_registered_settings[ $option_name ] ) ) {
		// Remove the sanitize callback if one was set during registration.
		if ( ! empty( $zc_registered_settings[ $option_name ]['sanitize_callback'] ) ) {
			remove_filter( "sanitize_option_{$option_name}", $zc_registered_settings[ $option_name ]['sanitize_callback'] );
		}

		// Remove the default filter if a default was provided during registration.
		if ( array_key_exists( 'default', $zc_registered_settings[ $option_name ] ) ) {
		...
		unset( $zc_registered_settings[ $option_name ] );
	}
}
...
function get_registered_settings() {
	global $zc_registered_settings;

	if ( ! is_array( $zc_registered_settings ) ) {
		return array();
	}

	return $zc_registered_settings;
}
 ...
 */
function zc_autoload_values_to_autoload() {
	 ...
	 */
	$filtered_values = apply_filters( 'zc_autoload_values_to_autoload', $autoload_values );
