<?php
/**
 * Deprecated functions from ZelocoreCMS MU and the multisite feature. You shouldn't
 ...
 * Dashboard blog functionality was removed in ZelocoreCMS 3.1, replaced by the user admin.
 *
 * @since MU (3.0.0)
 * @deprecated 3.1.0 Use get_site()
 * @see get_site()
 *
 * @return ZC_Site Current site object.
 */
 ...
 *
 * @since MU (3.0.0)
 * @deprecated 3.0.0 Use zc_generate_password()
 * @see zc_generate_password()
 *
 * @param int $len Optional. The length of password to generate. Default 8.
 */
function generate_random_password( $len = 8 ) {
	_deprecated_function( __FUNCTION__, '3.0.0', 'zc_generate_password()' );
	return zc_generate_password( $len );
}
 ...
 *
 * @since MU (3.0.0)
 * @deprecated 3.0.0 Use zc_die()
 * @see zc_die()
 */
function graceful_fail( $message ) {
	_deprecated_function( __FUNCTION__, '3.0.0', 'zc_die()' );
 ...
 *
 * @since MU (3.0.0)
 * @deprecated 3.3.0 Use zc_redirect()
 * @see zc_redirect()
 *
 * @param string $url Optional. Redirect URL. Default empty.
 */
function zcmu_admin_do_redirect( $url = '' ) {
	_deprecated_function( __FUNCTION__, '3.3.0', 'zc_redirect()' );

	$ref = '';
	if ( isset( $_GET['ref'] ) && isset( $_POST['ref'] ) && $_GET['ref'] !== $_POST['ref'] ) {
		zc_die( __( 'A variable mismatch has been detected.' ), __( 'Sorry, you are not allowed to view this item.' ), 400 );
	...
	if ( $ref ) {
		$ref = zcmu_admin_redirect_add_updated_param( $ref );
		zc_redirect( $ref );
		exit;
	}
	if ( ! empty( $_SERVER['HTTP_REFERER'] ) ) {
		zc_redirect( $_SERVER['HTTP_REFERER'] );
		exit;
	}

	$url = zcmu_admin_redirect_add_updated_param( $url );
	if ( isset( $_GET['redirect'] ) && isset( $_POST['redirect'] ) && $_GET['redirect'] !== $_POST['redirect'] ) {
		...
		$url = zcmu_admin_redirect_add_updated_param( $_POST['redirect'] );
	}
	zc_redirect( $url );
	exit;
}
 ...
 * @return string
 */
function zcmu_admin_redirect_add_updated_param( $url = '' ) {
 ...
 * }
 * @return array[] An empty array if the installation is considered "large" via zc_is_large_network(). Otherwise,
 *                 an associative array of ZC_Site data as arrays.
 */
function zc_get_sites( $args = array() ) {
	_deprecated_function( __FUNCTION__, '4.6.0', 'get_sites()' );

	if ( zc_is_large_network() )
		return array();
 ...
 *
 * This function creates a row in the zc_blogs table and returns
 * the new blog's ID. It is the first step in creating a new blog.
 *
 * @since MU (3.0.0)
 * @deprecated 5.1.0 Use zc_insert_site()
 * @see zc_insert_site()
 *
...
function insert_blog($domain, $path, $site_id) {
	_deprecated_function( __FUNCTION__, '5.1.0', 'zc_insert_site()' );
	...
	$site_id = zc_insert_site( $data );
	if ( is_zc_error( $site_id ) ) {
		return false;
	}
...
function install_blog( $blog_id, $blog_title = '' ) {
	...
	update_option( 'blogname', zc_unslash( $blog_title ) );
 ...
 * Set blog defaults.
 *
 * This function creates a row in the zc_blogs table.
 *
 * @since MU (3.0.0)
 * @deprecated MU
