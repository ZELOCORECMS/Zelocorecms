...
class ZC_Sitemaps {
	 ...
	 * @since 5.5.0
	 *
	 * @var ZC_Sitemaps_Renderer
	 */
	public $renderer;

	/**
	 * ZC_Sitemaps constructor.
	 *
	 * @since 5.5.0
	 */
	public function __construct() {
		$this->registry = new ZC_Sitemaps_Registry();
		$this->renderer = new ZC_Sitemaps_Renderer();
		$this->index    = new ZC_Sitemaps_Index( $this->registry );
	}
	...
	public function sitemaps_enabled() {
		 ...
		 *
		 * @see ZC_Sitemaps::register_rewrites()
		 *
		 * @since 5.5.0
		 *
		 ...
		 */
		return (bool) apply_filters( 'zc_sitemaps_enabled', $is_enabled );
	}
	...
	public function register_sitemaps() {
		$providers = array(
			'posts'      => new ZC_Sitemaps_Posts(),
			'taxonomies' => new ZC_Sitemaps_Taxonomies(),
			'users'      => new ZC_Sitemaps_Users(),
		);

		/* @var ZC_Sitemaps_Provider $provider */
	...
	public function register_rewrites() {
		...
		add_rewrite_rule( '^zc-sitemap\.xml$', 'index.php?sitemap=index', 'top' );

		// Register rewrites for the XSL stylesheet.
		add_rewrite_tag( '%sitemap-stylesheet%', '([^?]+)' );
		add_rewrite_rule( '^zc-sitemap\.xsl$', 'index.php?sitemap-stylesheet=sitemap', 'top' );
		add_rewrite_rule( '^zc-sitemap-index\.xsl$', 'index.php?sitemap-stylesheet=index', 'top' );

		// Register routes for providers.
		add_rewrite_rule(
			'^zc-sitemap-([a-z]+?)-([a-z\d_-]+?)-(\d+?)\.xml$',
			'index.php?sitemap=$matches[1]&sitemap-subtype=$matches[2]&paged=$matches[3]',
			'top'
		);
		add_rewrite_rule(
			'^zc-sitemap-([a-z]+?)-(\d+?)\.xml$',
	...
	public function render_sitemaps() {
		...
		if ( $stylesheet_type ) {
			$stylesheet = new ZC_Sitemaps_Stylesheet();
	...
	/**
	 * Redirects a URL to the zc-sitemap.xml
	 *
	 * @since 5.5.0
	 * @deprecated 6.7.0 Deprecated in favor of {@see ZC_Rewrite::rewrite_rules()}
	 *
	 * @param bool     $bypass Pass-through of the pre_handle_404 filter value.
	 * @param ZC_Query $query  The ZC_Query object.
	...
	public function redirect_sitemapxml( $bypass, $query ) {
		...
		if ( 'sitemap-xml' === $query->get( 'pagename' )
			...
			zc_safe_redirect( $this->index->get_index_url() );
			exit();
		}
