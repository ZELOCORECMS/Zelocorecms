...
/**
 * Core class used to implement the ZC_User object.
 *
 * @since 2.0.0
...
class ZC_User {
	 ...
	 * Constructor.
	 *
	 * Retrieves the userdata and passes it to ZC_User::init().
	 *
	 * @since 2.0.0
	 *
	...
	public static function get_data_by( $field, $value ) {
		...
		switch ( $field ) {
			case 'id':
				...
				break;
			case 'slug':
				$user_id  = zc_cache_get( $value, 'userslugs' );
				$db_field = 'user_nicename';
				break;
			case 'email':
				$user_id  = zc_cache_get( $value, 'useremail' );
				$db_field = 'user_email';
				break;
			case 'login':
				$value    = sanitize_user( $value );
				$user_id  = zc_cache_get( $value, 'userlogins' );
				$db_field = 'user_login';
				break;
			default:
				return false;
	...
	public function __isset( $key ) {
		if ( 'id' === $key ) {
			_deprecated_argument(
				...
				'2.1.0',
				sprintf(
					/* translators: %s: ZC_User->ID */
					__( 'Use %s instead.' ),
					'<code>ZC_User->ID</code>'
				)
			);
			$key = 'ID';
		}
	 ...
	 * the value on the ZC_User instance.
	 *
	 * @since 3.3.0
	 *
	...
	public function get_role_caps() {
		...
		foreach ( (array) $this->roles as $role ) {
			$the_role      = $zc_roles->get_role( $role );
	...
	public function set_role( $role ) {
		...
		foreach ( $old_roles as $old_role ) {
			...
			/** This action is documented in zc-includes/class-zc-user.php */
