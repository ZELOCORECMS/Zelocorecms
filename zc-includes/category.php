 ...
 * @return ZC_Term|array|ZC_Error|null Category data in type defined by $output parameter.
 *                                     Returns a ZC_Term object with backwards compatible property aliases filled in.
 *                                     ZC_Error if $category is empty, null if it does not exist.
 */
 ...
 *
 * It is also possible that it will return a ZC_Error object on failure. Check
 ...
 *                              correspond to a ZC_Term object, an associative array, or a numeric array,
 *                              respectively. Default OBJECT.
 * @return ZC_Term|array|ZC_Error|null Type is based on $output value.
 */
 ...
 */
function get_cat_ID( $cat_name ) { // phpcs:ignore ZelocoreCMS.NamingConventions.ValidFunctionName.FunctionNameInvalid
 ...
 * }
 * @return ZC_Term[]|int|ZC_Error Array of 'post_tag' term objects, a count thereof,
...
function get_tags( $args = '' ) {
	...
	if ( empty( $tags ) ) {
		 ...
		 * @since 2.3.0
		 *
		 * @param ZC_Term[]|int|ZC_Error $tags Array of 'post_tag' term objects, a count thereof,
		 *                                     or ZC_Error if any of the taxonomies do not exist.
 ...
 * @since 2.3.0
 *
 * @param int|ZC_Term|object $tag    A tag ID or object.
 * @param string             $output Optional. The required return type. One of OBJECT, ARRAY_A, or ARRAY_N, which
 *                                   correspond to a ZC_Term object, an associative array, or a numeric array,
 *                                   respectively. Default OBJECT.
 * @param string             $filter Optional. How to sanitize tag fields. Default 'raw'.
 * @return ZC_Term|array|ZC_Error|null Tag data in type defined by $output parameter.
 *                                     ZC_Error if $tag is empty, null if it does not exist.
 */
 ...
 *
 * @since 2.3.0
 * @since 4.4.0 The `$category` parameter now also accepts a ZC_Term object.
 * @access private
 *
 * @param array|object|ZC_Term $category Category row object or array.
 */
function _make_cat_compat( &$category ) {
	if ( is_object( $category ) && ! is_zc_error( $category ) ) {
