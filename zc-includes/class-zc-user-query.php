 ...
 * @since 3.1.0
 *
 * @see ZC_User_Query::prepare_query() for information on accepted arguments.
 */
...
class ZC_User_Query {
	 ...
	 * @since 4.4.0
	 *
	 * @param string|array $args Query vars, as passed to `ZC_User_Query`.
	...
	public static function fill_query_vars( $args ) {
		...
		return zc_parse_args( $args, $defaults );
	}
	...
	public function prepare_query( $query = array() ) {
		...
		/**
		 * Fires before the ZC_User_Query has been parsed.
		 *
		 * The passed ZC_User_Query object contains the query variables,
		 * not yet passed into SQL.
		 *
		 * @since 4.0.0
		 *
		 * @param ZC_User_Query $query Current instance of ZC_User_Query (passed by reference).
		 */
		...
		if ( ! empty( $qv['include'] ) ) {
			$include = zc_parse_id_list( $qv['include'] );
		} else {
			$include = false;
		}
		...
		if ( ! empty( $qv['capability'] ) || ! empty( $qv['capability__in'] ) || ! empty( $qv['capability__not_in'] ) ) {
			$zc_roles->for_site( $blog_id );
			$available_roles = $zc_roles->roles;
		}
		...
		if ( $search ) {
			...
			if ( ! $search_columns ) {
				...
				} elseif ( preg_match( '|^https?://|', $search ) && ! ( is_multisite() && zc_is_large_network( 'users' ) ) ) {
			...
			/**
			 * Filters the columns to search in a ZC_User_Query search.
			 *
		...
		if ( ! empty( $qv['date_query'] ) && is_array( $qv['date_query'] ) ) {
			$date_query         = new ZC_Date_Query( $qv['date_query'], 'user_registered' );
			$this->query_where .= $date_query->get_sql();
		}

		/**
		 * Fires after the ZC_User_Query has been parsed, and before
		 * the query is executed.
		 *
		 * The passed ZC_User_Query object contains SQL parts formed
	...
	public function query() {
		 ...
		 *
		 * Return a non-null value to bypass ZelocoreCMS' default user queries.
		 *
		 * Filtering functions that require pagination information are encouraged to set
		 * the `total_users` property of the ZC_User_Query object, passed to the filter
		 * by reference. If ZC_User_Query does not perform a database query, it will not
		...
		if ( null === $this->results ) {
			...
			if ( $qv['cache_results'] ) {
				$cache_value = zc_cache_get_salted( $cache_key, $cache_group, $last_changed );
			}
			if ( false !== $cache_value ) {
				...
				if ( isset( $qv['count_total'] ) && $qv['count_total'] ) {
					/**
					 * Filters SELECT FOUND_ROWS() query for the current ZC_User_Query instance.
					 *
					 * @since 3.2.0
				...
				if ( $qv['cache_results'] ) {
					...
					zc_cache_set_salted( $cache_key, $cache_value, $cache_group, $last_changed );
				}
			}
		}
		...
		if (
			...
			foreach ( $this->results as $userid ) {
				if ( 'all_with_meta' === $qv['fields'] ) {
					$r[ $userid ] = new ZC_User( $userid, '', $qv['blog_id'] );
				} else {
					$r[] = new ZC_User( $userid, '', $qv['blog_id'] );
				}
			}
	...
	protected function get_cache_last_changed( array $args ) {
		$last_changed = (array) zc_cache_get_last_changed( 'users' );
