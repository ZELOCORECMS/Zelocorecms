<?php
/**
 * Error Protection API: ZC_Recovery_Mode_Key_Service class
 *
 * @package ZelocoreCMS
 * @since 5.2.0
 */
...
final class ZC_Recovery_Mode_Key_Service {
	...
	public function generate_recovery_mode_token() {
		return zc_generate_password( 22, false );
	}
	 ...
	 *
	 * @since 5.2.0
	 * @since 6.8.0 The stored key is now hashed using zc_fast_hash() instead of phpass.
	 *
	 * @param string $token A token generated by {@see ZC_Recovery_Mode_Key_Service::generate_recovery_mode_token()}.
	 * @return string Recovery mode key.
	 */
	public function generate_and_store_recovery_mode_key( $token ) {
		$key = zc_generate_password( 22, false );

		$records = $this->get_keys();

		$records[ $token ] = array(
			'hashed_key' => zc_fast_hash( $key ),
			'created_at' => time(),
		);
	...
	public function validate_recovery_mode_key( $token, $key, $ttl ) {
		...
		if ( ! isset( $records[ $token ] ) ) {
			return new ZC_Error( 'token_not_found', __( 'Recovery Mode not initialized.' ) );
		}
		...
		if ( ! is_array( $record ) || ! isset( $record['hashed_key'], $record['created_at'] ) ) {
			return new ZC_Error( 'invalid_recovery_key_format', __( 'Invalid recovery key format.' ) );
		}

		if ( ! zc_verify_fast_hash( $key, $record['hashed_key'] ) ) {
			return new ZC_Error( 'hash_mismatch', __( 'Invalid recovery key.' ) );
		}

		if ( time() > $record['created_at'] + $ttl ) {
			return new ZC_Error( 'key_expired', __( 'Recovery key expired.' ) );
		}
	 ...
	 *
	 * @since 5.2.0
	 * @since 6.8.0 Each key is now hashed using zc_fast_hash() instead of phpass.
	 ...
	 *
	 * @since 5.2.0
	 * @since 6.8.0 Each key should now be hashed using zc_fast_hash() instead of phpass.
	 *
	 * @param array $keys {
	 *     Associative array of token => data pairs, where the data is an associative
	 *     array of information about the key.
