...
namespace ZelocoreCMS\AiClient\Results\DTO;
...
use ZelocoreCMS\AiClient\Common\Exception\RuntimeException;
use ZelocoreCMS\AiClient\Files\DTO\File;
use ZelocoreCMS\AiClient\Messages\DTO\Message;
use ZelocoreCMS\AiClient\Providers\DTO\ProviderMetadata;
use ZelocoreCMS\AiClient\Providers\Models\DTO\ModelMetadata;
use ZelocoreCMS\AiClient\Results\Contracts\ResultInterface;
/**
...
class GenerativeAiResult extends AbstractDataTransferObject implements ResultInterface
{
     ...
     */
    private \ZelocoreCMS\AiClient\Results\DTO\TokenUsage $tokenUsage;
    /**
     ...
     */
    public function __construct(string $id, array $candidates, \ZelocoreCMS\AiClient\Results\DTO\TokenUsage $tokenUsage, ProviderMetadata $providerMetadata, ModelMetadata $modelMetadata, array $additionalData = [])
    {
        if (empty($candidates)) {
     ...
     * @since 0.1.0
     */
    public function getTokenUsage(): \ZelocoreCMS\AiClient\Results\DTO\TokenUsage
    {
    ...
    {
        return array_values(array_map(fn(\ZelocoreCMS\AiClient\Results\DTO\Candidate $candidate) => $candidate->getMessage(), $this->candidates));
    }
    /**
     * {@inheritDoc}
     *
    ...
    {
        return ['type' => 'object', 'properties' => [self::KEY_ID => ['type' => 'string', 'description' => 'Unique identifier for this result.'], self::KEY_CANDIDATES => ['type' => 'array', 'items' => \ZelocoreCMS\AiClient\Results\DTO\Candidate::getJsonSchema(), 'minItems' => 1, 'description' => 'The generated candidates.'], self::KEY_TOKEN_USAGE => \ZelocoreCMS\AiClient\Results\DTO\TokenUsage::getJsonSchema(), self::KEY_PROVIDER_METADATA => ProviderMetadata::getJsonSchema(), self::KEY_MODEL_METADATA => ModelMetadata::getJsonSchema(), self::KEY_ADDITIONAL_DATA => ['type' => 'object', 'additionalProperties' => \true, 'description' => 'Additional data included in the API response.']], 'required' => [self::KEY_ID, self::KEY_CANDIDATES, self::KEY_TOKEN_USAGE, self::KEY_PROVIDER_METADATA, self::KEY_MODEL_METADATA]];
    }
    /**
     * {@inheritDoc}
     *
    ...
    public static function fromArray(array $array): self
    {
        ...
        $candidates = array_map(fn(array $candidateData) => \ZelocoreCMS\AiClient\Results\DTO\Candidate::fromArray($candidateData), $array[self::KEY_CANDIDATES]);
        return new self($array[self::KEY_ID], $candidates, \ZelocoreCMS\AiClient\Results\DTO\TokenUsage::fromArray($array[self::KEY_TOKEN_USAGE]), ProviderMetadata::fromArray($array[self::KEY_PROVIDER_METADATA]), ModelMetadata::fromArray($array[self::KEY_MODEL_METADATA]), $array[self::KEY_ADDITIONAL_DATA] ?? []);
    }
    /**
