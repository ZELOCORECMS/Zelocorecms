...
namespace ZelocoreCMS\AiClient\Messages\DTO;
...
class Message extends AbstractDataTransferObject
{
     ...
     */
    public function withPart(\ZelocoreCMS\AiClient\Messages\DTO\MessagePart $part): \ZelocoreCMS\AiClient\Messages\DTO\Message
    {
        $newParts = $this->parts;
        $newParts[] = $part;
        return new \ZelocoreCMS\AiClient\Messages\DTO\Message($this->role, $newParts);
    }
    /**
    ...
    {
        return ['type' => 'object', 'properties' => [self::KEY_ROLE => ['type' => 'string', 'enum' => MessageRoleEnum::getValues(), 'description' => 'The role of the message sender.'], self::KEY_PARTS => ['type' => 'array', 'items' => \ZelocoreCMS\AiClient\Messages\DTO\MessagePart::getJsonSchema(), 'minItems' => 1, 'description' => 'The parts that make up this message.']], 'required' => [self::KEY_ROLE, self::KEY_PARTS]];
    }
    /**
     * {@inheritDoc}
     *
    ...
    {
        return [self::KEY_ROLE => $this->role->value, self::KEY_PARTS => array_map(function (\ZelocoreCMS\AiClient\Messages\DTO\MessagePart $part) {
            return $part->toArray();
        }, $this->parts)];
    }
    /**
    ...
    final public static function fromArray(array $array): self
    {
        ...
        $parts = array_map(function (array $partData) {
            return \ZelocoreCMS\AiClient\Messages\DTO\MessagePart::fromArray($partData);
        }, $partsData);
        // Determine which concrete class to instantiate based on role
        if ($role->isUser()) {
            return new \ZelocoreCMS\AiClient\Messages\DTO\UserMessage($parts);
        } elseif ($role->isModel()) {
            return new \ZelocoreCMS\AiClient\Messages\DTO\ModelMessage($parts);
        } else {
