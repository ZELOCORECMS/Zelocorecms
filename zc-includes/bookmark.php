...
function get_bookmark( $bookmark, $output = OBJECT, $filter = 'raw' ) {
	...
	if ( empty( $bookmark ) ) {
		...
		zc_cache_add( $bookmark->link_id, $bookmark, 'bookmark' );
		$_bookmark = $bookmark;
	} else {
 ...
 * @return string|ZC_Error
 */
function get_bookmark_field( $field, $bookmark, $context = 'display' ) {
	$bookmark = (int) $bookmark;
	$bookmark = get_bookmark( $bookmark );

	if ( is_zc_error( $bookmark ) ) {
		return $bookmark;
	}
...
function get_bookmarks( $args = '' ) {
	...
	$cache = zc_cache_get( 'get_bookmarks', 'bookmark' );
	...
	if ( ! empty( $parsed_args['include'] ) ) {
		...
		$inclinks = zc_parse_id_list( $parsed_args['include'] );
		if ( count( $inclinks ) ) {
	...
	if ( ! empty( $parsed_args['exclude'] ) ) {
		$exlinks = zc_parse_id_list( $parsed_args['exclude'] );
		if ( count( $exlinks ) ) {
	...
	if ( ! empty( $parsed_args['category_name'] ) ) {
		...
		if ( $parsed_args['category'] ) {
			...
			$cache[ $key ] = array();
			zc_cache_set( 'get_bookmarks', $cache, 'bookmark' );
			/** This filter is documented in zc-includes/bookmark.php */
	...
	if ( ! empty( $parsed_args['category'] ) ) {
		$incategories = zc_parse_id_list( $parsed_args['category'] );
...
function clean_bookmark_cache( $bookmark_id ) {
	zc_cache_delete( $bookmark_id, 'bookmark' );
	zc_cache_delete( 'get_bookmarks', 'bookmark' );
