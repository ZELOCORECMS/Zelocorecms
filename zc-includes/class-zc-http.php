...
class ZC_Http {
	 ...
	 *                                             Default 'ZelocoreCMS/' . get_bloginfo( 'version' ) . '; ' . get_bloginfo( 'url' ).
	 *     @type bool         $reject_unsafe_urls  Whether to pass URLs through zc_http_validate_url().
	 *                                             Default false.
	 ...
	 *     Array of response data, or a ZC_Error instance upon error.
	 *
	 ...
	 *     }
	 *     @type ZC_Http_Cookie[]                                  $cookies       Array of cookies set by the server.
	 *     @type string|null                                       $filename      Optional. Filename of the response.
	 *     @type ZC_HTTP_Requests_Response|null                    $http_response Response object.
	 * }
	 */
	public function request( $url, $args = array() ) {
		$defaults = array(
			 ...
			 * @since 5.1.0 The `$url` parameter was added.
			 *
			 * @param string $user_agent ZelocoreCMS user agent string.
			 * @param string $url        The request URL.
			 */
			'user-agent'          => apply_filters( 'http_headers_useragent', 'ZelocoreCMS/' . get_bloginfo( 'version' ) . '; ' . get_bloginfo( 'url' ), $url ),
			/**
			 * Filters whether to pass URLs through zc_http_validate_url() in an HTTP request.
			 *
			 * @since 3.6.0
			 * @since 5.1.0 The `$url` parameter was added.
			 *
			 * @param bool   $pass_url Whether to pass URLs through zc_http_validate_url(). Default false.
			 * @param string $url      The request URL.
			 */
		 ...
		 * @since 2.9.0
		 *
		 * @param false|array|ZC_Error $response    A preemptive return value of an HTTP request. Default false.
		...
		if ( function_exists( 'zc_kses_bad_protocol' ) ) {
			if ( $parsed_args['reject_unsafe_urls'] ) {
				$url = zc_http_validate_url( $url );
			}
			if ( $url ) {
				$url = zc_kses_bad_protocol( $url, array( 'http', 'https', 'ssl' ) );
			}
		}
		...
		if ( empty( $url ) || empty( $parsed_url['scheme'] ) ) {
			$response = new ZC_Error( 'http_request_failed', __( 'A valid URL was not provided.' ) );
			/** This action is documented in zc-includes/class-zc-http.php */
		...
		if ( $this->block_request( $url ) ) {
			...
			$response = new ZC_Error( 'http_request_not_executed', sprintf( __( 'User has blocked requests through HTTP to the URL: %s.' ), $url ) );
			/** This action is documented in zc-includes/class-zc-http.php */
			do_action( 'http_api_debug', $response, 'response', 'WpOrg\Requests\Requests', $parsed_args, $url );
			return $response;
		}
		...
		if ( $parsed_args['stream'] ) {
			...
			if ( ! zc_is_writable( dirname( $parsed_args['filename'] ) ) ) {
				$response = new ZC_Error( 'http_request_failed', __( 'Destination directory for file streaming does not exist or is not writable.' ) );
				/** This action is documented in zc-includes/class-zc-http.php */
				do_action( 'http_api_debug', $response, 'response', 'WpOrg\Requests\Requests', $parsed_args, $url );
				return $response;
			}
		...
		$options = array(
			...
			'hooks'     => new ZC_HTTP_Requests_Hooks( $url, $parsed_args ),
		);
		...
		if ( function_exists( 'zc_kses_bad_protocol' ) && $parsed_args['reject_unsafe_urls'] ) {
		...
		try {
			...
			$http_response = new ZC_HTTP_Requests_Response( $requests_response, $parsed_args['filename'] );
			$response      = $http_response->to_array();

			// Add the original object to the array.
			$response['http_response'] = $http_response;
		} catch ( WpOrg\Requests\Exception $e ) {
			$response = new ZC_Error( 'http_request_failed', $e->getMessage() );
		}
		 ...
		 * @since 2.8.0
		 *
		 * @param array|ZC_Error $response    HTTP response or ZC_Error object.
	...
	public static function normalize_cookies( $cookies ) {
		...
		foreach ( $cookies as $name => $value ) {
			if ( $value instanceof ZC_Http_Cookie ) {
	...
	public static function validate_redirects( $location ) {
		if ( ! zc_http_validate_url( $location ) ) {
			throw new WpOrg\Requests\Exception( __( 'A valid URL was not provided.' ), 'zc_http.redirect_failed_validation' );
		}
	}
	 ...
	 *
	 * @since 3.2.0
	 * @deprecated 5.1.0 Use ZC_Http::request()
	 * @see ZC_Http::request()
	 *
	 * @param string $url  URL to request.
	 * @param array  $args Request arguments.
	 * @return array|ZC_Error Array containing 'headers', 'body', 'response', 'cookies', 'filename'.
	 *                        A ZC_Error instance upon error.
	 */
	private function _dispatch_request( $url, $args ) {
		...
		if ( ! $class ) {
			return new ZC_Error( 'http_failure', __( 'There are no HTTP transports available which can complete the requested request.' ) );
		}
	 ...
	 * @return array|ZC_Error Array containing 'headers', 'body', 'response', 'cookies', 'filename'.
	 *                        A ZC_Error instance upon error. See ZC_Http::response() for details.
	 */
	 ...
	 * }
	 */
	public static function processResponse( $response ) { // phpcs:ignore ZelocoreCMS.NamingConventions.ValidFunctionName.MethodNameInvalid
		$response = explode( "\r\n\r\n", $response, 2 );
	 ...
	 *     @type ZC_Http_Cookie[] $cookies    If the original headers contain the 'Set-Cookie' key,
	 *                                        an array containing `ZC_Http_Cookie` objects is returned.
	 * }
	 */
	public static function processHeaders( $headers, $url = '' ) { // phpcs:ignore ZelocoreCMS.NamingConventions.ValidFunctionName.MethodNameInvalid
		...
		foreach ( (array) $headers as $tempheader ) {
			...
			if ( 'set-cookie' === $key ) {
				$cookies[] = new ZC_Http_Cookie( $value, $url );
			}
		}
	 ...
	 *
	 * If it's found, then it upgrades any basic name => value pairs to ZC_Http_Cookie instances,
	 ...
	 */
	public static function buildCookieHeader( &$r ) { // phpcs:ignore ZelocoreCMS.NamingConventions.ValidFunctionName.MethodNameInvalid
		if ( ! empty( $r['cookies'] ) ) {
			// Upgrade any name => value cookie pairs to ZC_Http_Cookie instances.
			foreach ( $r['cookies'] as $name => $value ) {
				if ( ! is_object( $value ) ) {
					$r['cookies'][ $name ] = new ZC_Http_Cookie(
						array(
							'name'  => $name,
							'value' => $value,
						)
	 ...
	 */
	public static function chunkTransferDecode( $body ) { // phpcs:ignore ZelocoreCMS.NamingConventions.ValidFunctionName.MethodNameInvalid
	 ...
	 * prevent plugins from working and core functionality, if you don't include `api.zelocorecms.org`.
	 *
	 * You block external URL requests by defining `ZC_HTTP_BLOCK_EXTERNAL` as true in your `zc-config.php`
	 * file and this will only allow localhost and your site to make requests. The constant
	 * `ZC_ACCESSIBLE_HOSTS` will allow additional hosts to go through for requests. The format of the
	 * `ZC_ACCESSIBLE_HOSTS` constant is a comma separated list of hostnames to allow, wildcard domains
	 * are supported, eg `*.zelocorecms.org` will allow for all subdomains of `zelocorecms.org` to be contacted.
	 *
	 * @since 2.8.0
	 *
	 * @link https://core.trac.zelocorecms.org/ticket/8927 Allow preventing external requests.
	 * @link https://core.trac.zelocorecms.org/ticket/14636 Allow wildcard domains in ZC_ACCESSIBLE_HOSTS
	 *
	...
	public function block_request( $uri ) {
		...
		if ( null === $accessible_hosts ) {
			$accessible_hosts = preg_split( '|,\s*|', ZC_ACCESSIBLE_HOSTS );

			if ( str_contains( ZC_ACCESSIBLE_HOSTS, '*' ) ) {
	 ...
	 *
	 * @deprecated 4.4.0 Use zc_parse_url()
	 * @see zc_parse_url()
	 *
	...
	protected static function parse_url( $url ) {
		_deprecated_function( __METHOD__, '4.4.0', 'zc_parse_url()' );
		return zc_parse_url( $url );
	}
	...
	public static function make_absolute_url( $maybe_relative_path, $url ) {
		...
		$url_parts = zc_parse_url( $url );
		if ( ! $url_parts ) {
			return $maybe_relative_path;
		}

		$relative_url_parts = zc_parse_url( $maybe_relative_path );
	 ...
	 * @return array|false|ZC_Error An HTTP API response array if the redirect is successfully followed,
	 *                              false if no redirect is present, or a ZC_Error object if there's an error.
	 */
	public static function handle_redirects( $url, $args, $response ) {
		...
		if ( $args['redirection']-- <= 0 ) {
			return new ZC_Error( 'http_request_failed', __( 'Too many redirects.' ) );
		}
		...
		return zc_remote_request( $redirect_location, $args );
	}
