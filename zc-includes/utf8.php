...
if ( extension_loaded( 'mbstring' ) ) :
	 ...
	 * Example:
	 *
	 *     true === zc_is_valid_utf8( '' );
	 *     true === zc_is_valid_utf8( 'just a test' );
	 *     true === zc_is_valid_utf8( "\xE2\x9C\x8F" );    // Pencil, U+270F.
	 *     true === zc_is_valid_utf8( "\u{270F}" );        // Pencil, U+270F.
	 *     true === zc_is_valid_utf8( '✏' );              // Pencil, U+270F.
	 *
	 *     false === zc_is_valid_utf8( "just \xC0 test" ); // Invalid bytes.
	 *     false === zc_is_valid_utf8( "\xE2\x9C" );       // Invalid/incomplete sequences.
	 *     false === zc_is_valid_utf8( "\xC1\xBF" );       // Overlong sequences.
	 *     false === zc_is_valid_utf8( "\xED\xB0\x80" );   // Surrogate halves.
	 *     false === zc_is_valid_utf8( "B\xFCch" );        // ISO-8859-1 high-bytes.
	 ...
	 */
	function zc_is_valid_utf8( string $bytes ): bool {
	 ...
	 * @since 6.9.0
	 */
	function zc_is_valid_utf8( string $string ): bool {
		return _zc_is_valid_utf8_fallback( $string );
	}
endif;
...
if (
	 ...
	 *     'test' === zc_scrub_utf8( 'test' );
	 *
	 *     // Invalid sequences of bytes are replaced.
	 *     $invalid = "the byte \xC0 is never allowed in a UTF-8 string.";
	 *     "the byte \u{FFFD} is never allowed in a UTF-8 string." === zc_scrub_utf8( $invalid, true );
	 *     'the byte � is never allowed in a UTF-8 string.' === zc_scrub_utf8( $invalid, true );
	 *
	 *     // Maximal subparts are replaced individually.
	 *     '.�.' === zc_scrub_utf8( ".\xC0." );              // C0 is never valid.
	 *     '.�.' === zc_scrub_utf8( ".\xE2\x8C." );          // Missing A3 at end.
	 *     '.��.' === zc_scrub_utf8( ".\xE2\x8C\xE2\x8C." ); // Maximal subparts replaced separately.
	 *     '.��.' === zc_scrub_utf8( ".\xC1\xBF." );         // Overlong sequence.
	 *     '.���.' === zc_scrub_utf8( ".\xED\xA0\x80." );    // Surrogate half.
	 *
	 ...
	 */
	function zc_scrub_utf8( $text ) {
		return _zc_scrub_utf8_fallback( $text );
	}
endif;
...
if ( _zc_can_use_pcre_u() ) :
	 ...
	 */
	function zc_has_noncharacters( string $text ): bool {
		return 1 === preg_match(
	 ...
	 */
	function zc_has_noncharacters( string $text ): bool {
		return _zc_has_noncharacters_fallback( $text );
	}
endif;
