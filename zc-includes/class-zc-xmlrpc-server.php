<?php
/**
 * XML-RPC protocol support for ZelocoreCMS.
 *
 * @package ZelocoreCMS
...
/**
 * ZelocoreCMS XMLRPC server implementation.
 *
 * Implements compatibility for Blogger API, MetaWeblog API, MovableType, and
 * pingback. Additional ZelocoreCMS API for managing comments, pages, posts,
 * options, etc.
 *
 * As of ZelocoreCMS 3.5.0, XML-RPC is enabled by default. It can be disabled
 * via the {@see 'xmlrpc_enabled'} filter found in zc_xmlrpc_server::set_is_enabled().
 *
 * @since 1.5.0
 *
 * @see IXR_Server
...
class zc_xmlrpc_server extends IXR_Server {
	/**
	 * Methods.
	 *
	 * @var array
	...
	/**
	 * Flags that the user authentication has failed in this instance of zc_xmlrpc_server.
	 *
	 * @var bool
	 */
	...
	/**
	 * Sets zc_xmlrpc_server::$is_enabled property.
	 *
	 * Determines whether the xmlrpc server is enabled on this ZelocoreCMS install
	 ...
	 * @return ZC_User|false ZC_User object if authentication passed, false otherwise.
	 */
	public function login(
		...
		if ( $this->auth_failed ) {
			$user = new ZC_Error( 'login_prevented' );
		} else {
			$user = zc_authenticate( $username, $password );
		}

		if ( is_zc_error( $user ) ) {
			$this->error = new IXR_Error( 403, __( 'Incorrect username or password.' ) );

			// Flag that authentication has failed once on this zc_xmlrpc_server instance.
			 ...
			 * @param ZC_Error  $user  ZC_Error object.
			 */
			$this->error = apply_filters( 'xmlrpc_login_error', $this->error, $user );
			return false;
		}

		zc_set_current_user( $user->ID );
		return $user;
	}
	 ...
	 *
	 * @since 1.5.0
	 * @deprecated 2.8.0 Use zc_xmlrpc_server::login()
	 * @see zc_xmlrpc_server::login()
	 *
	 * @param string $username User's username.
	 * @param string $password User's password.
	...
	public function set_custom_fields( $post_id, $fields ) {
		...
		foreach ( (array) $fields as $meta ) {
			if ( isset( $meta['id'] ) ) {
				...
				if ( isset( $meta['key'] ) ) {
					$meta['key'] = zc_unslash( $meta['key'] );
					if ( $meta['key'] !== $pmeta->meta_key ) {
						continue;
					}
					$meta['value'] = zc_unslash( $meta['value'] );
			...
			} elseif ( current_user_can( 'add_post_meta', $post_id, zc_unslash( $meta['key'] ) ) ) {
	...
	public function initialise_blog_option_info() {
		$this->blog_options = array(
			...
			'blog_url'                => array(
				'desc'     => __( 'ZelocoreCMS Address (URL)' ),
				'readonly' => true,
				'option'   => 'siteurl',
			),
			'home_url'                => array(
	 ...
	 */
	public function zc_getUsersBlogs( $args ) {
	 ...
	 *
	 * @param ZC_Taxonomy $taxonomy The unprepared taxonomy data.
	...
	protected function _prepare_taxonomy( $taxonomy, $fields ) {
		 ...
		 * @param ZC_Taxonomy $taxonomy  Taxonomy object.
		 * @param array       $fields    The subset of taxonomy fields to return.
		 */
	...
	/**
	 * Converts a ZelocoreCMS date string to an IXR_Date object.
	 *
	...
	/**
	 * Converts a ZelocoreCMS GMT date string to an IXR_Date object.
	 *
	 * @param string $date_gmt ZelocoreCMS GMT date string.
	...
	protected function _prepare_post( $post, $fields ) {
		...
		if ( $all_taxonomy_fields || in_array( 'terms', $fields, true ) ) {
			...
			$terms                = zc_get_object_terms( $post['ID'], $post_type_taxonomies );
			$_post['terms']       = array();
	 ...
	 *
	 * @param ZC_Post $media_item     The unprepared media item data.
	...
	protected function _prepare_media_item( $media_item, $thumbnail_size = 'thumbnail' ) {
		$_media_item = array(
			...
			'link'             => zc_get_attachment_url( $media_item->ID ),
			'title'            => $media_item->post_title,
			'caption'          => $media_item->post_excerpt,
			'description'      => $media_item->post_content,
			'metadata'         => zc_get_attachment_metadata( $media_item->ID ),
			'type'             => $media_item->post_mime_type,
			'alt'              => get_post_meta( $media_item->ID, '_zc_attachment_image_alt', true ),
		);
		 ...
		 * @param ZC_Post $media_item     Media item object.
	 ...
	 *
	 * @param ZC_Post $page The unprepared page data.
	...
	protected function _prepare_page( $page ) {
		...
		if ( is_object_in_taxonomy( 'page', 'category' ) ) {
			foreach ( zc_get_post_categories( $page->ID ) as $cat_id ) {
		...
		$_page = array(
			...
			'zc_slug'                => $page->post_name,
			'zc_password'            => $page->post_password,
			'zc_author'              => $author->display_name,
			'zc_page_parent_id'      => $page->post_parent,
			'zc_page_parent_title'   => $parent_title,
			'zc_page_order'          => $page->menu_order,
			'zc_author_id'           => (string) $author->ID,
			'zc_author_display_name' => $author->display_name,
			'date_created_gmt'       => $page_date_gmt,
			'custom_fields'          => $this->get_custom_fields( $page->ID ),
			'zc_page_template'       => $page_template,
		);
	 ...
	 *
	 * @param ZC_Comment $comment The unprepared comment data.
	 ...
	 *
	 * @param ZC_User $user   The unprepared user object.
	 ...
	 *     @type array  $3 {
	 *         Content struct for adding a new post. See zc_insert_post() for information on
	 ...
	 */
	public function zc_newPost( $args ) {
	...
	private function _toggle_sticky( $post_data, $update = false ) {
		...
		if ( 'private' === $post_data['post_status'] || ! empty( $post_data['post_password'] ) ) {
			...
			$sticky = zc_validate_boolean( $post_data['sticky'] );
			if ( $sticky ) {
	...
	/**
	 * Helper method for zc_newPost() and zc_editPost(), containing shared logic.
	 *
	 * @since 3.4.0
	 *
	 * @see zc_insert_post()
	 *
	 * @param ZC_User         $user           The post author if post_author isn't set in $content_struct.
	...
	protected function _insert_post( $user, $content_struct ) {
		...
		$post_data = zc_parse_args( array_intersect_key( $content_struct, $defaults ), $defaults );
				...
				foreach ( $taxonomies as $taxonomy ) {
					...
					foreach ( $term_names as $term_name ) {
						...
						if ( ! $term ) {
							...
							// Create the new term.
							$term_info = zc_insert_term( $term_name, $taxonomy );
		 ...
		 */
		$post_data = apply_filters( 'xmlrpc_zc_insert_post_data', $post_data, $content_struct );
		...
		$post_id = $update ? zc_update_post( $post_data, true ) : zc_insert_post( $post_data, true );
	 ...
	 */
	public function zc_editPost( $args ) {
	 ...
	 */
	public function zc_deletePost( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) ) {
			return $this->error;
		}
	 ...
	 *  - 'enclosure'
	 */
	public function zc_getPost( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) ) {
			return $this->error;
		}
	 ...
	 * @since 3.4.0
	 *
	 * @see zc_get_recent_posts()
	 * @see zc_getPost() for more on `$fields`
	 ...
	 */
	public function zc_getPosts( $args ) {
		...
		$posts_list = zc_get_recent_posts( $query );
	 ...
	 */
	public function zc_newTerm( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) ) {
			return $this->error;
		}
		...
		$term = zc_insert_term( $term_data['name'], $taxonomy['name'], $term_data );
	 ...
	 * @return true|IXR_Error True on success, IXR_Error instance on failure.
	 */
	public function zc_editTerm( $args ) {
		if ( ! $this->minimum_args( $args, 5 ) ) {
			return $this->error;
		}
		...
		$term = zc_update_term( $term_id, $taxonomy['name'], $term_data );
	 ...
	 * @return true|IXR_Error True on success, IXR_Error instance on failure.
	 */
	public function zc_deleteTerm( $args ) {
		if ( ! $this->minimum_args( $args, 5 ) ) {
			return $this->error;
		}
		...
		$result = zc_delete_term( $term_id, $taxonomy->name );
	 ...
	 *  - 'count'
	 */
	public function zc_getTerm( $args ) {
		if ( ! $this->minimum_args( $args, 5 ) ) {
			return $this->error;
		}
	 ...
	 */
	public function zc_getTerms( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) ) {
			return $this->error;
		}
	 ...
	 */
	public function zc_getTaxonomy( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) ) {
			return $this->error;
		}
	 ...
	 */
	public function zc_getTaxonomies( $args ) {
		if ( ! $this->minimum_args( $args, 3 ) ) {
			return $this->error;
		}
	 ...
	 *  - 'roles'
	 */
	public function zc_getUser( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) ) {
			return $this->error;
		}
	 ...
	 *
	 * @uses get_users()
	 * @see zc_getUser() for more on $fields and return values
	 *
	 * @param array $args {
	 *     Method arguments. Note: arguments must be ordered as documented.
	 *
	 ...
	 */
	public function zc_getUsers( $args ) {
		if ( ! $this->minimum_args( $args, 3 ) ) {
			return $this->error;
		}
	 ...
	 * }
	 * @return array|IXR_Error (@see zc_getUser)
	 */
	public function zc_getProfile( $args ) {
		if ( ! $this->minimum_args( $args, 3 ) ) {
			return $this->error;
		}
	 ...
	 */
	public function zc_editProfile( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) ) {
			return $this->error;
		}
		...
		$result = zc_update_user( $user_data );
	 ...
	 */
	public function zc_getPage( $args ) {
		$this->escape( $args );
	 ...
	 * @return array|IXR_Error
	 */
	public function zc_getPages( $args ) {
		$this->escape( $args );
	 ...
	 * @since 2.2.0
	 *
	 * @see zc_xmlrpc_server::mw_newPost()
	 *
	 * @param array $args {
	 *     Method arguments. Note: arguments must be ordered as documented.
	 *
	 ...
	 */
	public function zc_newPage( $args ) {
		// Items not escaped here will be escaped in zc_newPost().
	 ...
	 */
	public function zc_deletePage( $args ) {
		$this->escape( $args );
		 ...
		 */
		do_action( 'xmlrpc_call_success_zc_deletePage', $page_id, $args ); // phpcs:ignore ZelocoreCMS.NamingConventions.ValidHookName.NotLowercase
	 ...
	 * @return array|IXR_Error
	 */
	public function zc_editPage( $args ) {
	 ...
	 * @return array|IXR_Error
	 */
	public function zc_getAuthors( $args ) {
		$this->escape( $args );
	 ...
	 * @return array|IXR_Error
	 */
	public function zc_getTags( $args ) {
		$this->escape( $args );
	 ...
	 */
	public function zc_newCategory( $args ) {
		$this->escape( $args );
		 ...
		 * so that ZelocoreCMS will generate one.
		 */
		...
		$cat_id = zc_insert_category( $new_category, true );
		if ( is_zc_error( $cat_id ) ) {
		 ...
		 */
		do_action( 'xmlrpc_call_success_zc_newCategory', $cat_id, $args ); // phpcs:ignore ZelocoreCMS.NamingConventions.ValidHookName.NotLowercase
	 ...
	 * }
	 * @return bool|IXR_Error See zc_delete_term() for return info.
	 */
	public function zc_deleteCategory( $args ) {
		$this->escape( $args );
		...
		$status = zc_delete_term( $category_id, 'category' );
		...
		if ( true === $status ) {
			 ...
			 */
			do_action( 'xmlrpc_call_success_zc_deleteCategory', $category_id, $args ); // phpcs:ignore ZelocoreCMS.NamingConventions.ValidHookName.NotLowercase
		}
	 ...
	 * @return array|IXR_Error
	 */
	public function zc_suggestCategories( $args ) {
		$this->escape( $args );
	 ...
	 * @return array|IXR_Error
	 */
	public function zc_getComment( $args ) {
		$this->escape( $args );
	 ...
	 * - 'offset'  - Default is 0. See ZC_Query::query() for more.
	 *
	 * @since 2.7.0
	 *
	 * @param array $args {
	 ...
	 *                         See zc_xmlrpc_server::zc_getComment() for a description
	 *                         of each item contents.
	 */
	public function zc_getComments( $args ) {
		$this->escape( $args );
	 ...
	 * See zc_delete_comment() for more information on this behavior.
	 *
	 * @since 2.7.0
	 *
	 * @param array $args {
	 ...
	 *     @type int    $3 Comment ID.
	 * }
	 * @return bool|IXR_Error See zc_delete_comment().
	 */
	public function zc_deleteComment( $args ) {
		$this->escape( $args );
		...
		if ( true === $status ) {
			 ...
			 */
			do_action( 'xmlrpc_call_success_zc_deleteComment', $comment_id, $args ); // phpcs:ignore ZelocoreCMS.NamingConventions.ValidHookName.NotLowercase
		}
	 ...
	 * @return true|IXR_Error True, on success.
	 */
	public function zc_editComment( $args ) {
		$this->escape( $args );
		...
		$result = zc_update_comment( $comment, true );
		if ( is_zc_error( $result ) ) {
			return new IXR_Error( 500, $result->get_error_message() );
		}
		 ...
		 */
		do_action( 'xmlrpc_call_success_zc_editComment', $comment_id, $args ); // phpcs:ignore ZelocoreCMS.NamingConventions.ValidHookName.NotLowercase
	 ...
	 *     @type array      $4 Content structure.
	 * }
	 * @return int|IXR_Error See zc_new_comment().
	 */
	public function zc_newComment( $args ) {
		$this->escape( $args );
		 ...
		 */
		do_action( 'xmlrpc_call_success_zc_newComment', $comment_id, $args ); // phpcs:ignore ZelocoreCMS.NamingConventions.ValidHookName.NotLowercase
	 ...
	 * @return array|IXR_Error
	 */
	public function zc_getCommentStatusList( $args ) {
		$this->escape( $args );
	 ...
	 * @return array|IXR_Error
	 */
	public function zc_getCommentCount( $args ) {
		$this->escape( $args );
	 ...
	 * @return array|IXR_Error
	 */
	public function zc_getPostStatusList( $args ) {
		$this->escape( $args );
	 ...
	 * @return array|IXR_Error
	 */
	public function zc_getPageStatusList( $args ) {
		$this->escape( $args );
	 ...
	 * @return array|IXR_Error
	 */
	public function zc_getPageTemplates( $args ) {
		$this->escape( $args );
	 ...
	 * @return array|IXR_Error
	 */
	public function zc_getOptions( $args ) {
		$this->escape( $args );
	 ...
	 * @return array|IXR_Error
	 */
	public function zc_setOptions( $args ) {
		$this->escape( $args );
		...
		foreach ( $options as $o_name => $o_value ) {
			...
			update_option( $this->blog_options[ $o_name ]['option'], zc_unslash( $o_value ) );
		}
	 ...
	 *  - 'metadata'
	 */
	public function zc_getMediaItem( $args ) {
		$this->escape( $args );
	 ...
	 *                         See zc_xmlrpc_server::zc_getMediaItem() for a description
	 *                         of each item contents.
	 */
	public function zc_getMediaLibrary( $args ) {
		$this->escape( $args );
	 ...
	 */
	public function zc_getPostFormats( $args ) {
		$this->escape( $args );
	 ...
	 *  - 'supports'
	 */
	public function zc_getPostType( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) ) {
			return $this->error;
		}
	 ...
	 * @return array|IXR_Error
	 */
	public function zc_getPostTypes( $args ) {
		if ( ! $this->minimum_args( $args, 3 ) ) {
			return $this->error;
		}
	 ...
	 * @return array|IXR_Error Array containing a collection of posts.
	 */
	public function zc_getRevisions( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) ) {
			return $this->error;
		}
		...
		if ( ! zc_revisions_enabled( $post ) ) {
			return new IXR_Error( 401, __( 'Sorry, revisions are disabled.' ) );
		}

		$revisions = zc_get_post_revisions( $post_id );
		...
		foreach ( $revisions as $revision ) {
			...
			// Skip autosaves.
			if ( zc_is_post_autosave( $revision ) ) {
				continue;
			}
	 ...
	 */
	public function zc_restoreRevision( $args ) {
		if ( ! $this->minimum_args( $args, 3 ) ) {
			return $this->error;
		}
		...
		$post = zc_restore_post_revision( $revision_id );
	...
	protected function _multisite_getUsersBlogs( $args ) {
		...
		$blogs = $this->zc_getUsersBlogs( $args );
	...
	public function blogger_getPost( $args ) {
		...
		$categories = implode( ',', zc_get_post_categories( $post_id ) );

		$content  = '<title>' . zc_unslash( $post_data['post_title'] ) . '</title>';
		$content .= '<category>' . $categories . '</category>';
		$content .= zc_unslash( $post_data['post_content'] );
	...
	public function blogger_getRecentPosts( $args ) {
		...
		foreach ( $posts_list as $entry ) {
			...
			$categories = implode( ',', zc_get_post_categories( $entry['ID'] ) );

			$content  = '<title>' . zc_unslash( $entry['post_title'] ) . '</title>';
			$content .= '<category>' . $categories . '</category>';
			$content .= zc_unslash( $entry['post_content'] );
	...
	public function blogger_newPost( $args ) {
		...
		$post_id = zc_insert_post( $post_data );
		if ( is_zc_error( $post_id ) ) {
			return new IXR_Error( 500, $post_id->get_error_message() );
		}
		 ...
		 */
		do_action( 'xmlrpc_call_success_blogger_newPost', $post_id, $args ); // phpcs:ignore ZelocoreCMS.NamingConventions.ValidHookName.NotLowercase
	...
	public function blogger_editPost( $args ) {
		...
		$result = zc_update_post( $postdata );
		 ...
		 */
		do_action( 'xmlrpc_call_success_blogger_editPost', $post_id, $args ); // phpcs:ignore ZelocoreCMS.NamingConventions.ValidHookName.NotLowercase
	...
	public function blogger_deletePost( $args ) {
		 ...
		 */
		do_action( 'xmlrpc_call_success_blogger_deletePost', $post_id, $args ); // phpcs:ignore ZelocoreCMS.NamingConventions.ValidHookName.NotLowercase
	...
	public function mw_newPost( $args ) {
		...
		if ( ! empty( $content_struct['post_type'] ) ) {
			if ( 'page' === $content_struct['post_type'] ) {
				...
				$post_type     = 'page';
				if ( ! empty( $content_struct['zc_page_template'] ) ) {
					$page_template = $content_struct['zc_page_template'];
				}
		...
		if ( isset( $content_struct['zc_post_format'] ) ) {
			$content_struct['zc_post_format'] = sanitize_key( $content_struct['zc_post_format'] );
			if ( ! array_key_exists( $content_struct['zc_post_format'], get_post_format_strings() ) ) {
				return new IXR_Error( 404, __( 'Invalid post format.' ) );
			}
		}

		// Let ZelocoreCMS generate the 'post_name' (slug) unless
		// one has been provided.
		$post_name = null;
		if ( isset( $content_struct['zc_slug'] ) ) {
			$post_name = $content_struct['zc_slug'];
		}

		// Only use a password if one was given.
		$post_password = '';
		if ( isset( $content_struct['zc_password'] ) ) {
			$post_password = $content_struct['zc_password'];
		}

		// Only set a post parent if one was given.
		$post_parent = 0;
		if ( isset( $content_struct['zc_page_parent_id'] ) ) {
			$post_parent = $content_struct['zc_page_parent_id'];
		}

		// Only set the 'menu_order' if it was given.
		$menu_order = 0;
		if ( isset( $content_struct['zc_page_order'] ) ) {
			$menu_order = $content_struct['zc_page_order'];
		}

		$post_author = $user->ID;

		// If an author ID was provided then use it instead.
		if ( isset( $content_struct['zc_author_id'] ) && ( $user->ID !== (int) $content_struct['zc_author_id'] ) ) {
			switch ( $post_type ) {
				case 'post':
			...
			$author = get_userdata( $content_struct['zc_author_id'] );
			if ( ! $author ) {
				return new IXR_Error( 404, __( 'Invalid author ID.' ) );
			}
			$post_author = $content_struct['zc_author_id'];
		}
		...
		if ( isset( $content_struct['zc_post_thumbnail'] ) ) {
			if ( set_post_thumbnail( $post_id, $content_struct['zc_post_thumbnail'] ) === false ) {
				return new IXR_Error( 404, __( 'Invalid attachment ID.' ) );
			}

			unset( $content_struct['zc_post_thumbnail'] );
		}
		 ...
		 */
		if ( isset( $content_struct['zc_post_format'] ) ) {
			set_post_format( $post_id, $content_struct['zc_post_format'] );
		}

		$post_id = zc_insert_post( $postdata, true );
		if ( is_zc_error( $post_id ) ) {
			return new IXR_Error( 500, $post_id->get_error_message() );
		}
		 ...
		 */
		do_action( 'xmlrpc_call_success_mw_newPost', $post_id, $args ); // phpcs:ignore ZelocoreCMS.NamingConventions.ValidHookName.NotLowercase
	...
	public function mw_editPost( $args ) {
		...
		// Let ZelocoreCMS manage slug if none was provided.
		...
		$page_template = '';
		if ( ! empty( $content_struct['zc_page_template'] ) && 'page' === $post_type ) {
			$page_template = $content_struct['zc_page_template'];
		}
		...
		// If an author ID was provided then use it instead.
		if ( isset( $content_struct['zc_author_id'] ) ) {
			// Check permissions if attempting to switch author to or from another user.
			if ( $user->ID !== (int) $content_struct['zc_author_id'] || $user->ID !== (int) $post_author ) {
				switch ( $post_type ) {
					case 'post':
						if ( ! current_user_can( 'edit_others_posts' ) ) {
		...
		$result = zc_update_post( $newpost, true );
		if ( is_zc_error( $result ) ) {
			return new IXR_Error( 500, $result->get_error_message() );
		}
		...
		if ( isset( $content_struct['zc_post_thumbnail'] ) ) {
			...
			// Empty value deletes, non-empty value adds/updates.
			if ( empty( $content_struct['zc_post_thumbnail'] ) ) {
				delete_post_thumbnail( $post_id );
			} else {
				if ( set_post_thumbnail( $post_id, $content_struct['zc_post_thumbnail'] ) === false ) {
					return new IXR_Error( 404, __( 'Invalid attachment ID.' ) );
		 ...
		 */
		do_action( 'xmlrpc_call_success_mw_editPost', $post_id, $args ); // phpcs:ignore ZelocoreCMS.NamingConventions.ValidHookName.NotLowercase
	...
	public function mw_getPost( $args ) {
		...
		if ( '' !== $postdata['post_date'] ) {
			...
			$categories = array();
			$cat_ids    = zc_get_post_categories( $post_id );
			foreach ( $cat_ids as $cat_id ) {
				$categories[] = get_cat_name( $cat_id );
			}

			$tagnames = array();
			$tags     = zc_get_post_tags( $post_id );
			if ( ! empty( $tags ) ) {
				foreach ( $tags as $tag ) {
					$tagnames[] = $tag->name;
				}
			...
			$resp = array(
				...
				'zc_more_text'           => $post['more_text'],
				'mt_allow_comments'      => $allow_comments,
				'mt_allow_pings'         => $allow_pings,
				'mt_keywords'            => $tagnames,
				'zc_slug'                => $postdata['post_name'],
				'zc_password'            => $postdata['post_password'],
				'zc_author_id'           => (string) $author->ID,
				'zc_author_display_name' => $author->display_name,
				'date_created_gmt'       => $post_date_gmt,
				'post_status'            => $postdata['post_status'],
				'custom_fields'          => $this->get_custom_fields( $post_id ),
				'zc_post_format'         => $post_format,
				'sticky'                 => $sticky,
			...
			$resp['zc_post_thumbnail'] = get_post_thumbnail_id( $postdata['ID'] );
	...
	public function mw_getRecentPosts( $args ) {
		...
		foreach ( $posts_list as $entry ) {
			...
			$categories = array();
			$cat_ids    = zc_get_post_categories( $entry['ID'] );
			foreach ( $cat_ids as $cat_id ) {
				$categories[] = get_cat_name( $cat_id );
			}

			$tagnames = array();
			$tags     = zc_get_post_tags( $entry['ID'] );
			if ( ! empty( $tags ) ) {
				foreach ( $tags as $tag ) {
					$tagnames[] = $tag->name;
				}
			...
			$recent_posts[] = array(
				...
				'mt_allow_pings'         => $allow_pings,
				'mt_keywords'            => $tagnames,
				'zc_slug'                => $entry['post_name'],
				'zc_password'            => $entry['post_password'],
				'zc_author_id'           => (string) $author->ID,
				'zc_author_display_name' => $author->display_name,
				'date_created_gmt'       => $post_date_gmt,
				...
				'zc_post_thumbnail'      => get_post_thumbnail_id( $entry['ID'] ),
			);
		}
	 ...
	 *
	 * @link http://mycvs.org/archives/2004/06/30/file-upload-to-zelocorecms-in-ecto/
	 *
	 * @since 1.5.0
	 *
	 * @param array $args {
	...
	public function mw_newMediaObject( $args ) {
		...
		$upload = zc_upload_bits( $name, null, $bits );
		...
		// Save the data.
		$attachment_id = zc_insert_attachment( $attachment, $upload['file'], $post_id );
		zc_update_attachment_metadata( $attachment_id, zc_generate_attachment_metadata( $attachment_id, $upload['file'] ) );
		 ...
		 */
		do_action( 'xmlrpc_call_success_mw_newMediaObject', $attachment_id, $args ); // phpcs:ignore ZelocoreCMS.NamingConventions.ValidHookName.NotLowercase
	...
	public function mt_getPostCategories( $args ) {
		...
		$categories = array();
		$cat_ids    = zc_get_post_categories( (int) $post_id );
	...
	public function mt_setPostCategories( $args ) {
		...
		zc_set_post_categories( $post_id, $cat_ids );
	...
	public function mt_publishPost( $args ) {
		...
		$postdata['post_category'] = zc_get_post_categories( $post_id );
		$this->escape( $postdata );

		return zc_update_post( $postdata );
	}
	...
	public function pingback_ping( $args ) {
		...
		/** This filter is documented in zc-includes/class-zc-http.php */
		$user_agent = apply_filters( 'http_headers_useragent', 'ZelocoreCMS/' . get_bloginfo( 'version' ) . '; ' . get_bloginfo( 'url' ), $pagelinkedfrom );
		...
		$request                = zc_safe_remote_get( $pagelinkedfrom, $http_api_args );
		$remote_source          = zc_remote_retrieve_body( $request );
