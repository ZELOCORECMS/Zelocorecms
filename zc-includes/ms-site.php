 ...
 *                                Passed to the `zc_initialize_site` hook.
 * }
...
function zc_insert_site( array $data ) {
	...
	$prepared_data = zc_prepare_site_data( $data, $defaults );
	if ( is_zc_error( $prepared_data ) ) {
	...
	if ( ! $new_site ) {
		return new ZC_Error( 'get_site_error', __( 'Could not retrieve site data.' ) );
	}
	 ...
	 * @since 5.1.0
	 *
	 * @param ZC_Site $new_site New site object.
	 */
	do_action( 'zc_insert_site', $new_site );
	 ...
	 */
	do_action( 'zc_initialize_site', $new_site, $args );

	// Only compute extra hook parameters if the deprecated hook is actually in use.
	if ( has_action( 'zcmu_new_blog' ) ) {
		$user_id = ! empty( $args['user_id'] ) ? $args['user_id'] : 0;
		$meta    = ! empty( $args['options'] ) ? $args['options'] : array();

		// WPLANG was passed with `$meta` to the `zcmu_new_blog` hook prior to 5.1.0.
		if ( ! array_key_exists( 'WPLANG', $meta ) ) {
			$meta['WPLANG'] = get_network_option( $new_site->network_id, 'WPLANG' );
		}

		/*
		 * Rebuild the data expected by the `zcmu_new_blog` hook prior to 5.1.0 using allowed keys.
		 * The `$allowed_data_fields` matches the one used in `zcmu_create_blog()`.
		 */
		 ...
		 *
		 * @since MU (3.0.0)
		 * @deprecated 5.1.0 Use {@see 'zc_initialize_site'} instead.
		 *
...
function zc_update_site( $site_id, array $data ) {
	...
	if ( ! $old_site ) {
		return new ZC_Error( 'site_not_exist', __( 'Site does not exist.' ) );
	}
	...
	$data = zc_prepare_site_data( $data, $defaults, $old_site );
	if ( is_zc_error( $data ) ) {
		return $data;
	}
	 ...
	 *
	 * @param ZC_Site $new_site New site object.
	 * @param ZC_Site $old_site Old site object.
	 */
	do_action( 'zc_update_site', $new_site, $old_site );
...
function zc_delete_site( $site_id ) {
	 ...
	 *
	 * Plugins should amend the `$errors` object via its `ZC_Error::add()` method. If any errors
	 * are present, the site will not be deleted.
	 *
	 * @since 5.1.0
	 *
	 * @param ZC_Error $errors   Error object to add validation errors to.
	 * @param ZC_Site  $old_site The site object to be deleted.
	 */
	do_action( 'zc_validate_site_deletion', $errors, $old_site );
	 ...
	 * @since 5.1.0
	 *
	 * @param ZC_Site $old_site Deleted site object.
	 */
	 ...
	 * @param ZC_Site $old_site Deleted site object.
	 */
	do_action( 'zc_delete_site', $old_site );
 ...
 * @since 4.6.0
 *
 * @param ZC_Site|int|null $site Optional. Site to retrieve. Default is the current site.
 * @return ZC_Site|null The site object or null if not found.
 */
function get_site( $site = null ) {
	...
	if ( $site instanceof ZC_Site ) {
		$_site = $site;
	} elseif ( is_object( $site ) ) {
		$_site = new ZC_Site( $site );
	} else {
		$_site = ZC_Site::get_instance( $site );
	}
 ...
 */
function zc_lazyload_site_meta( array $site_ids ) {
	if ( empty( $site_ids ) ) {
		return;
	}
...
function update_site_cache( $sites, $update_meta_cache = true ) {
	...
	zc_cache_add_multiple( $site_data, 'sites' );
	zc_cache_add_multiple( $blog_details_data, 'blog-details' );
...
function update_sitemeta_cache( $site_ids ) {
	...
	if ( ! has_filter( 'update_blog_metadata_cache', 'zc_check_site_meta_support_prefilter' ) ) {
		add_filter( 'update_blog_metadata_cache', 'zc_check_site_meta_support_prefilter' );
	}
 ...
 *
 * @see ZC_Site_Query::parse_query()
 *
 * @param string|array $args Optional. Array or string of arguments. See ZC_Site_Query::__construct()
 *                           for information on accepted arguments. Default empty array.
 * @return ZC_Site[]|int[]|int List of ZC_Site objects, a list of site IDs when 'fields' is set to 'ids',
 *                             or the number of sites when 'count' is passed as a query var.
 */
function get_sites( $args = array() ) {
	$query = new ZC_Site_Query();
 ...
 *                               See {@see zc_insert_site()} for the possibly included data.
 * @param array        $defaults Site data defaults to parse $data against.
 * @param ZC_Site|null $old_site Optional. Old site object if an update, or null if an insertion.
 *                               Default null.
 * @return array|ZC_Error Site data ready for a database transaction, or ZC_Error in case a validation
 *                        error occurred.
 */
function zc_prepare_site_data( $data, $defaults, $old_site = null ) {
	 ...
	 *                    See {@see zc_insert_site()} for the possibly included data.
	 */
	$data = apply_filters( 'zc_normalize_site_data', $data );

	$allowed_data_fields = array( 'domain', 'path', 'network_id', 'registered', 'last_updated', 'public', 'archived', 'mature', 'spam', 'deleted', 'lang_id' );
	$data                = array_intersect_key( zc_parse_args( $data, $defaults ), array_flip( $allowed_data_fields ) );
	 ...
	 *
	 * Plugins should amend the `$errors` object via its `ZC_Error::add()` method.
	 *
	 * @since 5.1.0
	 *
	 * @param ZC_Error     $errors   Error object to add validation errors to.
	 * @param array        $data     Associative array of complete site data. See {@see zc_insert_site()}
	 *                               for the included data.
	 * @param ZC_Site|null $old_site The old site object if the data belongs to a site being updated,
	 *                               or null if it is a new site being inserted.
	 */
	do_action( 'zc_validate_site_data', $errors, $data, $old_site );
 ...
 */
function zc_normalize_site_data( $data ) {
 ...
 * @since 5.1.0
 *
 * @param ZC_Error     $errors   Error object, passed by reference. Will contain validation errors if
 *                               any occurred.
 * @param array        $data     Associative array of complete site data. See {@see zc_insert_site()}
 *                               for the included data.
 * @param ZC_Site|null $old_site The old site object if the data belongs to a site being updated,
 *                               or null if it is a new site being inserted.
 */
function zc_validate_site_data( $errors, $data, $old_site = null ) {
	...
	foreach ( $date_fields as $date_field ) {
		...
		if ( '0000-00-00 00:00:00' !== $data[ $date_field ] ) {
			...
			$valid_date = zc_checkdate( $month, $day, $year, $data[ $date_field ] );
			if ( ! $valid_date ) {
...
function zc_initialize_site( $site_id, array $args = array() ) {
	...
	if ( ! $site ) {
		return new ZC_Error( 'site_invalid_id', __( 'Site with the ID does not exist.' ) );
	}

	if ( zc_is_site_initialized( $site ) ) {
		return new ZC_Error( 'site_already_initialized', __( 'The site appears to be already initialized.' ) );
	}
	 ...
	 * @param ZC_Site    $site    Site that is being initialized.
	 * @param ZC_Network $network Network that the site belongs to.
	 */
	$args = apply_filters( 'zc_initialize_site_args', $args, $site, $network );

	$orig_installing = zc_installing();
	if ( ! $orig_installing ) {
		zc_installing( true );
	}
	...
	populate_options(
		array_merge(
			array(
				...
				'blogname'    => zc_unslash( $args['title'] ),
				'admin_email' => '',
	...
	zc_install_defaults( $args['user_id'] );
...
function zc_uninitialize_site( $site_id ) {
	...
	if ( ! zc_is_site_initialized( $site ) ) {
		return new ZC_Error( 'site_already_uninitialized', __( 'The site appears to be already uninitialized.' ) );
	}
	 ...
	 * @since MU (3.0.0)
	 *
	 * @param string $basedir Uploads path without subdirectory. See {@see zc_upload_dir()}.
	 * @param int    $site_id The site ID.
	 */
	$dir     = apply_filters( 'zcmu_delete_blog_upload_dir', $uploads['basedir'], $site->id );
	...
	while ( $index < count( $stack ) ) {
		...
		// phpcs:disable ZelocoreCMS.PHP.NoSilencedErrors.Discouraged
		$dh = @opendir( $dir );
		if ( $dh ) {
			$file = @readdir( $dh );
			while ( false !== $file ) {
	...
	// phpcs:enable ZelocoreCMS.PHP.NoSilencedErrors.Discouraged
	if ( $switch ) {
		restore_current_blog();
	}
...
function zc_is_site_initialized( $site_id ) {
	 ...
	 */
	$pre = apply_filters( 'pre_zc_is_site_initialized', null, $site_id );
	if ( null !== $pre ) {
		return (bool) $pre;
	}
	...
	if ( get_current_blog_id() !== $site_id ) {
		$switch = true;
		remove_action( 'switch_blog', 'zc_switch_roles_and_user', 1 );
	...
	if ( $switch ) {
		restore_current_blog();
		add_action( 'switch_blog', 'zc_switch_roles_and_user', 1, 2 );
	}
 ...
 *
 * @param ZC_Site|int $blog The site object or ID to be cleared from cache.
 */
function clean_blog_cache( $blog ) {
	...
	if ( ! $blog ) {
		...
		// Make sure a ZC_Site object exists even when the site has been deleted.
		$blog = new ZC_Site(
			(object) array(
				'blog_id' => $blog_id,
				'domain'  => null,
	...
	zc_cache_delete( $blog_id, 'sites' );
	zc_cache_delete( $blog_id, 'site-details' );
	zc_cache_delete( $blog_id, 'blog-details' );
	zc_cache_delete( $blog_id . 'short', 'blog-details' );
	zc_cache_delete( $domain_path_key, 'blog-lookup' );
	zc_cache_delete( $domain_path_key, 'blog-id-cache' );
	zc_cache_delete( $blog_id, 'blog_meta' );
	 ...
	 * @param ZC_Site $blog            Site object.
 ...
 * @since 5.1.0
 *
 * @param ZC_Site      $new_site The site object that has been inserted, updated or deleted.
 * @param ZC_Site|null $old_site Optional. If $new_site has been updated, this must be the previous
 *                               state of that site. Default null.
 */
function zc_maybe_update_network_site_counts_on_update( $new_site, $old_site = null ) {
	if ( null === $old_site ) {
		zc_maybe_update_network_site_counts( $new_site->network_id );
		return;
	}

	if ( $new_site->network_id !== $old_site->network_id ) {
		zc_maybe_update_network_site_counts( $new_site->network_id );
		zc_maybe_update_network_site_counts( $old_site->network_id );
	}
}
 ...
 * @since 5.1.0
 *
 * @param ZC_Site      $new_site The site object after the update.
 * @param ZC_Site|null $old_site Optional. If $new_site has been updated, this must be the previous
 *                               state of that site. Default null.
 */
function zc_maybe_transition_site_statuses_on_update( $new_site, $old_site = null ) {
	$site_id = $new_site->id;

	// Use the default values for a site if no previous state is given.
	if ( ! $old_site ) {
		$old_site = new ZC_Site( new stdClass() );
	}
 ...
 *
 * @param ZC_Site $new_site The site object after the update.
 * @param ZC_Site $old_site The site object prior to the update.
 */
function zc_maybe_clean_new_site_cache_on_update( $new_site, $old_site ) {
 ...
 *                          for compatibility reasons. Accepts '1' or '0'.
 */
function zc_update_blog_public_option_on_site_update( $site_id, $is_public ) {

	// Bail if the site's database tables do not exist (yet).
	if ( ! zc_is_site_initialized( $site_id ) ) {
		return;
	}
 ...
 * @since 5.1.0
 */
function zc_cache_set_sites_last_changed() {
	zc_cache_set_last_changed( 'sites' );
}
