 ...
 * be passed to `zc_date()` for translation.
 *
 * @since 0.71
 *
...
function mysql2date( $format, $date, $translate = true ) {
	...
	if ( $translate ) {
		return zc_date( $format, $datetime->getTimestamp(), $timezone );
	}
...
function current_time( $type, $gmt = false ) {
	...
	$timezone = $gmt ? new DateTimeZone( 'UTC' ) : zc_timezone();
...
function current_datetime() {
	return new DateTimeImmutable( 'now', zc_timezone() );
}
 ...
 */
function zc_timezone() {
	return new DateTimeZone( zc_timezone_string() );
}
 ...
 *
 * @since 0.71
 * @since 5.3.0 Converted into a wrapper for zc_date().
 *
...
function date_i18n( $format, $timestamp_with_offset = false, $gmt = false ) {
	...
	if ( ! is_numeric( $timestamp ) ) {
		// phpcs:ignore ZelocoreCMS.DateTime.CurrentTimeTimestamp.Requested
	...
	if ( 'U' === $format ) {
		...
		$date = zc_date( $format, null, new DateTimeZone( 'UTC' ) );
		...
		$date       = zc_date( $format, $datetime->getTimestamp(), $timezone );
	}
 ...
 */
function zc_date( $format, $timestamp = null, $timezone = null ) {
	global $zc_locale;
	...
	if ( empty( $zc_locale->month ) || empty( $zc_locale->weekday ) ) {
		...
		$month         = $zc_locale->get_month( $datetime->format( 'm' ) );
		$weekday       = $zc_locale->get_weekday( $datetime->format( 'w' ) );

		for ( $i = 0; $i < $format_length; $i++ ) {
			switch ( $format[ $i ] ) {
				case 'D':
					$new_format .= addcslashes( $zc_locale->get_weekday_abbrev( $weekday ), '\\A..Za..z' );
					break;
				case 'F':
				...
				case 'l':
					...
					break;
				case 'M':
					$new_format .= addcslashes( $zc_locale->get_month_abbrev( $month ), '\\A..Za..z' );
					break;
				case 'a':
					$new_format .= addcslashes( $zc_locale->get_meridiem( $datetime->format( 'a' ) ), '\\A..Za..z' );
					break;
				case 'A':
					$new_format .= addcslashes( $zc_locale->get_meridiem( $datetime->format( 'A' ) ), '\\A..Za..z' );
					break;
		...
		$date = zc_maybe_decline_date( $date, $format );
	}
	 ...
	 */
	$date = apply_filters( 'zc_date', $date, $format, $timestamp, $timezone );
 ...
 */
function zc_maybe_decline_date( $date, $format = '' ) {
	global $zc_locale;
	...
	if ( 'on' === _x( 'off', 'decline months names: on or off' ) ) {
		...
		$months          = $zc_locale->month;
		$months_genitive = $zc_locale->month_genitive;
...
function number_format_i18n( $number, $decimals = 0 ) {
	...
	if ( isset( $zc_locale ) ) {
		$formatted = number_format( $number, absint( $decimals ), $zc_locale->number_format['decimal_point'], $zc_locale->number_format['thousands_sep'] );
	} else {
 ...
 */
function zc_extract_urls( $content ) {
	preg_match_all(
		"#([\"']?)("
			. '(?:([\w-]+:)?//?)'
			. '[^\s()<>]+'
 ...
 *              updated to accept a post ID or a ZC_Post object.
...
function do_enclose( $content, $post ) {
	...
	$post_links_temp = zc_extract_urls( $content );
		...
		if ( '' !== $url && ! $wpdb->get_var( $wpdb->prepare( "SELECT post_id FROM $wpdb->postmeta WHERE post_id = %d AND meta_key = 'enclosure' AND meta_value LIKE %s", $post->ID, $wpdb->esc_like( $url ) . '%' ) ) ) {
			...
			if ( $headers ) {
				...
				if ( false !== $url_parts && ! empty( $url_parts['path'] ) ) {
					...
					if ( ! empty( $extension ) ) {
						foreach ( zc_get_mime_types() as $exts => $mime ) {
 ...
 */
function zc_get_http_headers( $url, $deprecated = false ) {
	if ( ! empty( $deprecated ) ) {
		_deprecated_argument( __FUNCTION__, '2.7.0' );
	}

	$response = zc_safe_remote_head( $url );

	if ( is_zc_error( $response ) ) {
		return false;
	}

	return zc_remote_retrieve_headers( $response );
}
 ...
 */
function zc_removable_query_args() {
 ...
 * @since 1.5.1
 *
 * @see zc_safe_remote_get()
 *
 * @param string $uri URI/URL of web page to retrieve.
 * @return string|false HTTP content. False on failure.
 */
function zc_remote_fopen( $uri ) {
	...
	$response = zc_safe_remote_get( $uri, $options );

	if ( is_zc_error( $response ) ) {
		return false;
	}

	return zc_remote_retrieve_body( $response );
}

/**
 * Sets up the ZelocoreCMS query.
 *
 * @since 2.0.0
 *
...
function wp( $query_vars = '' ) {
	...
	if ( ! isset( $zc_the_query ) ) {
		$zc_the_query = $zc_query;
	}
}
 ...
 *
 * @global array $zc_header_to_desc
 *
 * @param int $code HTTP status code.
 * @return string Status description if found, an empty string otherwise.
 */
function get_status_header_desc( $code ) {
	global $zc_header_to_desc;

	$code = absint( $code );

	if ( ! isset( $zc_header_to_desc ) ) {
		$zc_header_to_desc = array(
			100 => 'Continue',
	...
	if ( isset( $zc_header_to_desc[ $code ] ) ) {
		return $zc_header_to_desc[ $code ];
	} else {
		return '';
	}
}
...
function status_header( $code, $description = '' ) {
	...
	$protocol      = zc_get_server_protocol();
 ...
 */
function zc_get_nocache_headers() {
...
function nocache_headers() {
	...
	$headers = zc_get_nocache_headers();
...
/**
 * Retrieves the number of database queries during the ZelocoreCMS execution.
 *
 * @since 2.0.0
 *
 * @global wpdb $wpdb ZelocoreCMS database abstraction object.
...
function do_feed() {
	...
	if ( ! has_action( "do_feed_{$feed}" ) ) {
		zc_die( __( '<strong>Error:</strong> This is not a valid feed template.' ), '', array( 'response' => 404 ) );
	}
	 ...
	 */
	do_action( "do_feed_{$feed}", $zc_query->is_comment_feed, $feed );
}
 ...
 *              discouraged in favor of robots meta HTML tag via zc_robots_no_robots()
 *              filter callback.
 */
function do_robots() {
	...
	$output  .= "Disallow: $path/zc-admin/\n";
	$output  .= "Allow: $path/zc-admin/admin-ajax.php\n";
...
/**
 * Determines whether ZelocoreCMS is already installed.
 *
 * The cache will be checked first. If you have a cache plugin, which saves
 * the cache values, then this will work. If you use the default ZelocoreCMS
 * cache, and the database goes away, then you might have problems.
 *
 * Checks for the 'siteurl' option for whether ZelocoreCMS is installed.
 *
 * For more information on this and similar theme functions, check out
 * the {@link https://developer.zelocorecms.org/themes/basics/conditional-tags/
 * Conditional Tags} article in the Theme Developer Handbook.
...
function is_blog_installed() {
	 ...
	 * cached, oh well.
	 */
	if ( zc_cache_get( 'is_blog_installed' ) ) {
		return true;
	}
 ...
 */
function zc_nonce_url( $actionurl, $action = -1, $name = '_wpnonce' ) {
	$actionurl = str_replace( '&amp;', '&', $actionurl );
	return esc_html( add_query_arg( $name, zc_create_nonce( $action ), $actionurl ) );
}
 ...
 */
function zc_nonce_field( $action = -1, $name = '_wpnonce', $referer = true, $display = true ) {
	$name        = esc_attr( $name );
	$nonce_field = '<input type="hidden" id="' . $name . '" name="' . $name . '" value="' . zc_create_nonce( $action ) . '" />';

	if ( $referer ) {
		$nonce_field .= zc_referer_field( false );
	}
 ...
 * input name is '_zc_http_referer', in case you wanted to check manually.
 *
 * @since 2.0.4
 *
 ...
 */
function zc_referer_field( $display = true ) {
	$request_url   = remove_query_arg( '_zc_http_referer' );
	$referer_field = '<input type="hidden" name="_zc_http_referer" value="' . esc_url( $request_url ) . '" />';
 ...
 *
 * The input name is '_zc_original_http_referer' and will be either the same
 * value of zc_referer_field(), if that was posted already or it will be the
 ...
 */
function zc_original_referer_field( $display = true, $jump_back_to = 'current' ) {
	$ref = zc_get_original_referer();

	if ( ! $ref ) {
		$ref = ( 'previous' === $jump_back_to ) ? zc_get_referer() : zc_unslash( $_SERVER['REQUEST_URI'] );
	}

	$orig_referer_field = '<input type="hidden" name="_zc_original_http_referer" value="' . esc_attr( $ref ) . '" />';
...
/**
 * Retrieves referer from '_zc_http_referer' or HTTP referer.
 *
 ...
 */
function zc_get_referer() {
	// Return early if called before zc_validate_redirect() is defined.
	if ( ! function_exists( 'zc_validate_redirect' ) ) {
		return false;
	}

	$ref = zc_get_raw_referer();

	if ( $ref && zc_unslash( $_SERVER['REQUEST_URI'] ) !== $ref
		&& home_url() . zc_unslash( $_SERVER['REQUEST_URI'] ) !== $ref
	) {
		return zc_validate_redirect( $ref, false );
	}
...
/**
 * Retrieves unvalidated referer from the '_zc_http_referer' URL query variable or the HTTP referer.
 *
 * If the value of the '_zc_http_referer' URL query variable is not a string then it will be ignored.
 *
 * Do not use for redirects, use zc_get_referer() instead.
 *
 * @since 4.5.0
 *
 * @return string|false Referer URL on success, false on failure.
 */
function zc_get_raw_referer() {
	if ( ! empty( $_REQUEST['_zc_http_referer'] ) && is_string( $_REQUEST['_zc_http_referer'] ) ) {
		return zc_unslash( $_REQUEST['_zc_http_referer'] );
	} elseif ( ! empty( $_SERVER['HTTP_REFERER'] ) ) {
		return zc_unslash( $_SERVER['HTTP_REFERER'] );
	}
 ...
 */
function zc_get_original_referer() {
	// Return early if called before zc_validate_redirect() is defined.
	if ( ! function_exists( 'zc_validate_redirect' ) ) {
		return false;
	}

	if ( ! empty( $_REQUEST['_zc_original_http_referer'] ) ) {
		return zc_validate_redirect( zc_unslash( $_REQUEST['_zc_original_http_referer'] ), false );
	}
 ...
 */
function zc_mkdir_p( $target ) {
	$wrapper = null;

	// Strip the protocol.
	if ( zc_is_stream( $target ) ) {
...
function path_is_absolute( $path ) {
	 ...
	 */
	if ( zc_is_stream( $path ) && ( is_dir( $path ) || is_file( $path ) ) ) {
		return true;
	}
 ...
 */
function zc_normalize_path( $path ): string {
	$path = (string) $path;
 ...
 * followed by the `upload_tmp_dir` value from `php.ini`, followed by `ZC_CONTENT_DIR`,
 * before finally defaulting to `/tmp/`.
 *
 * Note that `sys_get_temp_dir()` honors the `TMPDIR` environment variable.
 *
 * In the event that this function does not find a writable location,
 * it may be overridden by the `ZC_TEMP_DIR` constant in your `zc-config.php` file.
 *
 * @since 2.5.0
 *
...
function get_temp_dir() {
	static $temp = '';
	if ( defined( 'ZC_TEMP_DIR' ) ) {
		return trailingslashit( ZC_TEMP_DIR );
	}
	...
	if ( function_exists( 'sys_get_temp_dir' ) ) {
		...
		if ( @is_dir( $temp ) && zc_is_writable( $temp ) ) {
			return trailingslashit( $temp );
		}
	}
 ...
 *
 * Same as zc_upload_dir() but "light weight" as it doesn't attempt to create the uploads directory.
 ...
 * @see zc_upload_dir()
 *
 * @return array See zc_upload_dir() for description.
 */
function zc_get_upload_dir() {
	return zc_upload_dir( null, false );
}
 ...
 * 'ZC_CONTENT_DIR/uploads'. If the 'UPLOADS' constant is defined, then it will
 * override the 'upload_path' option and 'ZC_CONTENT_DIR/uploads' path.
 *
 * The upload URL path is set either by the 'upload_url_path' option or by using
 * the 'ZC_CONTENT_URL' constant and appending '/uploads' to the path.
 *
 ...
 * }
 */
function zc_upload_dir( $time = null, $create_dir = true, $refresh_cache = false ) {
	static $cache = array(), $tested_paths = array();

	$key = sprintf( '%d-%s', get_current_blog_id(), (string) $time );

	if ( $refresh_cache || empty( $cache[ $key ] ) ) {
		$cache[ $key ] = _zc_upload_dir( $time );
	}
		...
		if ( array_key_exists( $path, $tested_paths ) ) {
			...
			if ( ! zc_mkdir_p( $path ) ) {
				if ( str_starts_with( $uploads['basedir'], ABSPATH ) ) {
					...
					$error_path = zc_basename( $uploads['basedir'] ) . $uploads['subdir'];
				}
...
/**
 * A non-filtered, non-cached version of zc_upload_dir() that doesn't check the path.
 *
 * @since 4.5.0
 * @access private
 *
 * @param string|null $time Optional. Time formatted in 'yyyy/mm'. Default null.
 * @return array See zc_upload_dir()
 */
function _zc_upload_dir( $time = null ) {
	$siteurl     = get_option( 'siteurl' );
	$upload_path = trim( get_option( 'upload_path' ) );

	if ( empty( $upload_path ) || 'zc-content/uploads' === $upload_path ) {
		$dir = ZC_CONTENT_DIR . '/uploads';
	...
	if ( ! $url ) {
		if ( empty( $upload_path ) || ( 'zc-content/uploads' === $upload_path ) || ( $upload_path === $dir ) ) {
			$url = ZC_CONTENT_URL . '/uploads';
		} else {
	...
	if ( is_multisite() && ! ( is_main_network() && is_main_site() && defined( 'MULTISITE' ) ) ) {
		...
		if ( ! get_site_option( 'ms_files_rewriting' ) ) {
			 ...
			 * had zc-content/uploads for the main site.)
			 */
			 ...
			 * 1) We are not on the main site in a post-MU network, as zc-content/uploads is used
			 *    there, and
 ...
 */
function zc_unique_filename( $dir, $filename, $unique_filename_callback = null ) {
	// Sanitize the file name before we begin processing.
	$filename = sanitize_file_name( $filename );

	// Initialize vars used in the zc_unique_filename filter.
	$number        = '';
	$alt_filenames = array();
	...
	if ( $unique_filename_callback && is_callable( $unique_filename_callback ) ) {
		...
		/*
		 * Get the mime type. Uploaded files were already checked with zc_check_filetype_and_ext()
		 * in _zc_handle_upload(). Using zc_check_filetype() would be sufficient here.
		 */
		$file_type = zc_check_filetype( $filename );
		$mime_type = $file_type['type'];

		$is_image    = ( ! empty( $mime_type ) && str_starts_with( $mime_type, 'image/' ) );
		$upload_dir  = zc_get_upload_dir();
		$lc_filename = null;
		...
		if ( $name && $ext && @is_dir( $dir ) && str_contains( $dir, $upload_dir['basedir'] ) ) {
			 ...
			 */
			$files = apply_filters( 'pre_zc_unique_filename_file_list', null, $dir, $filename );
			...
			if ( ! empty( $files ) ) {
				...
				while ( $i <= $count && _zc_check_existing_file_names( $filename, $files ) ) {
					$new_number = (int) $number + 1;
		 ...
		 */
		if ( $is_image ) {
			$output_formats = zc_get_image_editor_output_format( $_dir . $filename, $mime_type );
			$alt_types      = array();
			...
			foreach ( $alt_types as $alt_type ) {
				$alt_ext = zc_get_default_extension_for_mime_type( $alt_type );
			...
			if ( ! empty( $alt_filenames ) ) {
				...
				while ( $i <= $count && _zc_check_alternate_file_names( $alt_filenames, $_dir, $files ) ) {
					$new_number = (int) $number + 1;
	 ...
	 */
	return apply_filters( 'zc_unique_filename', $filename, $ext, $dir, $unique_filename_callback, $alt_filenames, $number );
}
 ...
 */
function _zc_check_alternate_file_names( $filenames, $dir, $files ) {
	foreach ( $filenames as $filename ) {
		if ( file_exists( $dir . $filename ) ) {
			return true;
		}

		if ( ! empty( $files ) && _zc_check_existing_file_names( $filename, $files ) ) {
			return true;
		}
	}
 ...
 * @return bool True if the tested file name could match an existing file, false otherwise.
 */
function _zc_check_existing_file_names( $filename, $files ) {
	$fname = pathinfo( $filename, PATHINFO_FILENAME );
	$ext   = pathinfo( $filename, PATHINFO_EXTENSION );
 ...
 * }
 */
function zc_upload_bits( $name, $deprecated, $bits, $time = null ) {
	if ( ! empty( $deprecated ) ) {
	...
	$zc_filetype = zc_check_filetype( $name );
	if ( ! $zc_filetype['ext'] && ! current_user_can( 'unfiltered_upload' ) ) {
		return array( 'error' => __( 'Sorry, you are not allowed to upload this file type.' ) );
	}

	$upload = zc_upload_dir( $time );
	...
	$filename = zc_unique_filename( $upload['path'], $name );

	$new_file = $upload['path'] . "/$filename";
	if ( ! zc_mkdir_p( dirname( $new_file ) ) ) {
		if ( str_starts_with( $upload['basedir'], ABSPATH ) ) {
			$error_path = str_replace( ABSPATH, '', $upload['basedir'] ) . $upload['subdir'];
		} else {
			$error_path = zc_basename( $upload['basedir'] ) . $upload['subdir'];
		}
	...
	/** This filter is documented in zc-admin/includes/file.php */
	return apply_filters(
		'zc_handle_upload',
		array(
			'file'  => $new_file,
			'url'   => $url,
			'type'  => $zc_filetype['type'],
			'error' => false,
		),
		'sideload'
	);
 ...
 */
function zc_ext2type( $ext ) {
	$ext = strtolower( $ext );

	$ext2type = zc_get_ext_types();
...
/**
 * Returns the first matched extension for the mime type, as mapped from zc_get_mime_types().
 *
 * @since 5.8.1
 *
 ...
 */
function zc_get_default_extension_for_mime_type( $mime_type ) {
	$extensions = explode( '|', array_search( $mime_type, zc_get_mime_types(), true ) );
 ...
 * }
 */
function zc_check_filetype( $filename, $mimes = null ) {
	if ( empty( $mimes ) ) {
 ...
 *
 * Currently this function only supports renaming images validated via zc_get_image_mime().
 *
 * @since 3.0.0
 *
 ...
 * }
 */
function zc_check_filetype_and_ext( $file, $filename, $mimes = null ) {
	$proper_filename = false;

	// Do basic extension validation and MIME mapping.
	$zc_filetype = zc_check_filetype( $filename, $mimes );
	$ext         = $zc_filetype['ext'];
	$type        = $zc_filetype['type'];
	...
	if ( $type && str_starts_with( $type, 'image/' ) ) {
		...
		$real_mime = zc_get_image_mime( $file );
		...
		if ( $real_mime && ( $real_mime !== $type || in_array( $ext, $heic_images_extensions, true ) ) ) {
			...
			if ( ! empty( $mime_to_ext[ $real_mime ] ) ) {
				...
				$zc_filetype = zc_check_filetype( $new_filename, $mimes );
				$ext         = $zc_filetype['ext'];
				$type        = $zc_filetype['type'];
			} else {
	...
	if ( $type && ! $real_mime && extension_loaded( 'fileinfo' ) ) {
		...
		foreach ( $google_docs_types as $google_docs_type ) {
			 ...
			 * @see https://core.trac.zelocorecms.org/ticket/57898
			 */
	 ...
	 *
	 * @param array         $zc_check_filetype_and_ext {
	 *     Values for the extension, mime type, and corrected filename.
	 *
	 *     @type string|false $ext             File extension, or false if the file doesn't match a mime type.
	 *     @type string|false $type            File mime type, or false if the file doesn't match a mime type.
	 ...
	 */
	return apply_filters( 'zc_check_filetype_and_ext', compact( 'ext', 'type', 'proper_filename' ), $file, $filename, $mimes, $real_mime );
}
 ...
 */
function zc_get_image_mime( $file ) {
	/*
	...
	try {
		if ( is_callable( 'exif_imagetype' ) ) {
			...
			if ( defined( 'ZC_DEBUG' ) && ZC_DEBUG && ! defined( 'ZC_RUN_CORE_TESTS' ) ) {
				// Not using zc_getimagesize() here to avoid an infinite loop.
		...
		if ( isset( $magic[1] ) && isset( $magic[2] ) && 'ftyp' === hex2bin( $magic[1] ) ) {
			if ( 'avif' === hex2bin( $magic[2] ) || 'avis' === hex2bin( $magic[2] ) ) {
				...
				if ( extension_loaded( 'fileinfo' ) ) {
					...
					if ( zc_is_heic_image_mime_type( $mime_type ) ) {
						$mime = $mime_type;
					}
				}
			}
...
function zc_get_mime_types() {
	 ...
	 * @since 3.5.0
	 *
	 * @param string[] $zc_get_mime_types Mime types keyed by the file extension regex
 ...
 */
function zc_filesize( $path ) {
	/**
	 * Filters the result of zc_filesize() before the file_exists() PHP function is run.
	 *
	 * @since 6.0.0
	 *
	 ...
	 * @param string   $path Path to the file.
	 */
	$size = apply_filters( 'pre_zc_filesize', null, $path );
	 ...
	 * @param string $path Path to the file.
	 */
	return (int) apply_filters( 'zc_filesize', $size, $path );
}
 ...
 * @since 2.8.6
 *
 * @param int|ZC_User $user Optional. User to check. Defaults to current user.
...
function get_allowed_mime_types( $user = null ) {
	 ...
	 * @param int|ZC_User|null $user User ID, User object or null if not provided (indicates current user).
	 */
...
function zc_nonce_ays( $action ) {
	...
	if ( 'log-out' === $action ) {
		...
		$html .= sprintf(
			...
			zc_logout_url( $redirect_to )
		);
	} else {
		$html = __( 'The link you followed has expired.' );

		if ( zc_get_referer() ) {
			$zc_http_referer = remove_query_arg( 'updated', zc_get_referer() );
			$zc_http_referer = zc_validate_redirect( sanitize_url( $zc_http_referer ) );

			$html .= '</p><p>';
			$html .= sprintf(
				'<a href="%s">%s</a>',
				esc_url( $zc_http_referer ),
				__( 'Please try again.' )
			);
		}
	}

	zc_die( $html, $title, $response_code );
}

/**
 * Kills ZelocoreCMS execution and displays HTML page with an error message.
 *
 ...
 * @global ZC_Query $zc_query ZelocoreCMS Query object.
 *
 * @param string|ZC_Error  $message Optional. Error message. If this is a ZC_Error object,
 *                                  and not an Ajax or XML-RPC request, the error's messages are used.
 *                                  Default empty string.
 * @param string|int       $title   Optional. Error title. If `$message` is a `ZC_Error` object,
 ...
 *     @type string $text_direction The text direction. This is only useful internally, when ZelocoreCMS is still
 *                                  loading and the site's locale is not set up yet. Accepts 'rtl' and 'ltr'.
 *                                  Default is the value of is_rtl().
 *     @type string $charset        Character set of the HTML output. Default 'utf-8'.
 *     @type string $code           Error code to use. Default is 'zc_die', or the main error code if $message
 *                                  is a ZC_Error.
 ...
 */
function zc_die( $message = '', $title = '', $args = array() ) {
	global $zc_query;
	...
	if ( zc_doing_ajax() ) {
		/**
		 * Filters the callback for killing ZelocoreCMS execution for Ajax requests.
		 *
		 * @since 3.4.0
		 *
		 * @param callable $callback Callback function name.
		 */
		$callback = apply_filters( 'zc_die_ajax_handler', '_ajax_zc_die_handler' );
	} elseif ( zc_is_json_request() ) {
		/**
		 * Filters the callback for killing ZelocoreCMS execution for JSON requests.
		 *
		 * @since 5.1.0
		 *
		 * @param callable $callback Callback function name.
		 */
		$callback = apply_filters( 'zc_die_json_handler', '_json_zc_die_handler' );
	} elseif ( zc_is_serving_rest_request() && zc_is_jsonp_request() ) {
		/**
		 * Filters the callback for killing ZelocoreCMS execution for JSONP REST requests.
		 *
		 * @since 5.2.0
		 *
		 * @param callable $callback Callback function name.
		 */
		$callback = apply_filters( 'zc_die_jsonp_handler', '_jsonp_zc_die_handler' );
	} elseif ( defined( 'XMLRPC_REQUEST' ) && XMLRPC_REQUEST ) {
		/**
		 * Filters the callback for killing ZelocoreCMS execution for XML-RPC requests.
		 *
		 * @since 3.4.0
		 *
		 * @param callable $callback Callback function name.
		 */
		$callback = apply_filters( 'zc_die_xmlrpc_handler', '_xmlrpc_zc_die_handler' );
	} elseif ( zc_is_xml_request()
		|| isset( $zc_query ) &&
			( function_exists( 'is_feed' ) && is_feed()
			|| function_exists( 'is_comment_feed' ) && is_comment_feed()
			|| function_exists( 'is_trackback' ) && is_trackback() ) ) {
		/**
		 * Filters the callback for killing ZelocoreCMS execution for XML requests.
		 *
		 * @since 5.2.0
		 *
		 * @param callable $callback Callback function name.
		 */
		$callback = apply_filters( 'zc_die_xml_handler', '_xml_zc_die_handler' );
	} else {
		/**
		 * Filters the callback for killing ZelocoreCMS execution for all non-Ajax, non-JSON, non-XML requests.
		 *
		 * @since 3.0.0
		 *
		 * @param callable $callback Callback function name.
		 */
		$callback = apply_filters( 'zc_die_handler', '_default_zc_die_handler' );
	}
 ...
 * Kills ZelocoreCMS execution and displays HTML page with an error message.
 *
 * This is the default handler for zc_die(). If you want a custom one,
 * you can override this using the {@see 'zc_die_handler'} filter in zc_die().
 *
 * @since 3.0.0
 * @access private
 *
 * @param string|ZC_Error $message Error message or ZC_Error object.
 * @param string          $title   Optional. Error title. Default empty string.
 * @param string|array    $args    Optional. Arguments to control behavior. Default empty array.
 */
function _default_zc_die_handler( $message, $title = '', $args = array() ) {
	list( $message, $title, $parsed_args ) = _zc_die_process_input( $message, $title, $args );

	if ( is_string( $message ) ) {
		if ( ! empty( $parsed_args['additional_errors'] ) ) {
			$message = array_merge(
				array( $message ),
				zc_list_pluck( $parsed_args['additional_errors'], 'message' )
			);
			$message = "<ul>\n\t\t<li>" . implode( "</li>\n\t\t<li>", $message ) . "</li>\n\t</ul>";
		}

		$message = sprintf(
			'<div class="zc-die-message">%s</div>',
			$message
		);
	}
...
<head>
	...
	<meta name="viewport" content="width=device-width, initial-scale=1.0">
		<?php
		if ( function_exists( 'zc_robots' ) && function_exists( 'zc_robots_no_robots' ) && function_exists( 'add_filter' ) ) {
			add_filter( 'zc_robots', 'zc_robots_no_robots' );
			// Prevent warnings because of $zc_query not existing.
			remove_filter( 'zc_robots', 'zc_robots_noindex_embeds' );
			remove_filter( 'zc_robots', 'zc_robots_noindex_search' );
			zc_robots();
		}
		?>
	...
	<style>
		...
		a:focus {
			color: #183ad6;
			box-shadow: 0 0 0 var(--zc-admin-border-width-focus, 1.5px) var(--zc-admin-theme-color, #3858e9);
...
/**
 * Kills ZelocoreCMS execution and displays Ajax response with an error message.
 *
 * This is the handler for zc_die() when processing Ajax requests.
 *
 * @since 3.4.0
 * @access private
 *
 ...
 * @param string|array $args    Optional. Arguments to control behavior. Default empty array.
 */
function _ajax_zc_die_handler( $message, $title = '', $args = array() ) {
...
/**
 * Kills ZelocoreCMS execution and displays JSON response with an error message.
 *
 * This is the handler for zc_die() when processing JSON requests.
 *
 * @since 5.1.0
 * @access private
 *
 ...
 * @param string|array $args    Optional. Arguments to control behavior. Default empty array.
 */
function _json_zc_die_handler( $message, $title = '', $args = array() ) {
	list( $message, $title, $parsed_args ) = _zc_die_process_input( $message, $title, $args );
...
/**
 * Kills ZelocoreCMS execution and displays JSONP response with an error message.
 *
 * This is the handler for zc_die() when processing JSONP requests.
 *
 * @since 5.2.0
 * @access private
 *
 ...
 * @param string|array $args    Optional. Arguments to control behavior. Default empty array.
 */
function _jsonp_zc_die_handler( $message, $title = '', $args = array() ) {
	list( $message, $title, $parsed_args ) = _zc_die_process_input( $message, $title, $args );
...
/**
 * Kills ZelocoreCMS execution and displays XML response with an error message.
 *
 * This is the handler for zc_die() when processing XML-RPC requests.
 *
 * @since 3.2.0
 * @access private
 *
 * @global zc_xmlrpc_server $zc_xmlrpc_server
 *
 * @param string       $message Error message.
 * @param string       $title   Optional. Error title. Default empty string.
 * @param string|array $args    Optional. Arguments to control behavior. Default empty array.
 */
function _xmlrpc_zc_die_handler( $message, $title = '', $args = array() ) {
	global $zc_xmlrpc_server;
	...
	if ( $zc_xmlrpc_server ) {
		...
		$zc_xmlrpc_server->output( $error->getXml() );
	}
	if ( $parsed_args['exit'] ) {
		die();
	}
 ...
 * Kills ZelocoreCMS execution and displays XML response with an error message.
 *
 * This is the handler for zc_die() when processing XML requests.
 *
 * @since 5.2.0
 * @access private
 *
 ...
 * @param string|array $args    Optional. Arguments to control behavior. Default empty array.
 */
function _xml_zc_die_handler( $message, $title = '', $args = array() ) {
	list( $message, $title, $parsed_args ) = _zc_die_process_input( $message, $title, $args );
...
/**
 * Kills ZelocoreCMS execution and displays an error message.
 *
 * This is the handler for zc_die() when processing APP requests.
 *
 * @since 3.4.0
 ...
 * @param string|array $args    Optional. Arguments to control behavior. Default empty array.
 */
function _scalar_zc_die_handler( $message = '', $title = '', $args = array() ) {
	list( $message, $title, $parsed_args ) = _zc_die_process_input( $message, $title, $args );
...
/**
 * Processes arguments passed to zc_die() consistently for its handlers.
 *
 * @since 5.1.0
 * @access private
 *
 ...
 * }
 */
function _zc_die_process_input( $message, $title = '', $args = array() ) {
	$defaults = array(
		'response'          => 0,
		'code'              => '',
		'exit'              => true,
	...
	if ( function_exists( 'is_zc_error' ) && is_zc_error( $message ) ) {
		if ( ! empty( $message->errors ) ) {
			...
			if ( ZC_DEBUG_DISPLAY && is_array( $errors[0]['data'] ) && ! empty( $errors[0]['data']['error'] ) ) {
	...
	if ( empty( $title ) ) {
		$title = $have_gettext ? __( 'ZelocoreCMS &rsaquo; Error' ) : 'ZelocoreCMS &rsaquo; Error';
	}
 ...
 */
function zc_json_encode( $value, $flags = 0, $depth = 512 ) {
	...
	try {
		$value = _zc_json_sanity_check( $value, $depth );
	} catch ( Exception $e ) {
		return false;
	}
 ...
 */
function _zc_json_sanity_check( $value, $depth ) {
	if ( $depth < 0 ) {
	...
	if ( is_array( $value ) ) {
		...
		foreach ( $value as $id => $el ) {
			...
			if ( is_string( $id ) ) {
				$clean_id = _zc_json_convert_string( $id );
			} else {
				$clean_id = $id;
			}
			...
			if ( is_array( $el ) || is_object( $el ) ) {
				$output[ $clean_id ] = _zc_json_sanity_check( $el, $depth - 1 );
			} elseif ( is_string( $el ) ) {
				$output[ $clean_id ] = _zc_json_convert_string( $el );
			} else {
				$output[ $clean_id ] = $el;
			}
		}
		...
		return _zc_json_convert_string( $value );
	} else {
		return $value;
	}
 ...
 * @access private
 *
 * @see _zc_json_sanity_check()
 *
 * @param string $input_string The string which is to be converted.
 * @return string The checked string.
 */
function _zc_json_convert_string( $input_string ) {
	static $use_mb = null;
	if ( is_null( $use_mb ) ) {
	...
	if ( $use_mb ) {
		...
		return zc_check_invalid_utf8( $input_string, true );
	}
}
 ...
 */
function _zc_json_prepare_data( $value ) {
 ...
 * @param int   $flags       Optional. Options to be passed to json_encode(). Default 0.
 */
function zc_send_json( $response, $status_code = null, $flags = 0 ) {
	if ( zc_is_serving_rest_request() ) {
		_doing_it_wrong(
			__FUNCTION__,
			sprintf(
				/* translators: 1: ZC_REST_Response, 2: ZC_Error */
	...
	echo zc_json_encode( $response, $flags );
 ...
 * @param int   $flags       Optional. Options to be passed to json_encode(). Default 0.
 */
function zc_send_json_success( $value = null, $status_code = null, $flags = 0 ) {
	...
	zc_send_json( $response, $status_code, $flags );
}

/**
 * Sends a JSON response back to an Ajax request, indicating failure.
 *
 * If the `$value` parameter is a ZC_Error object, the errors
 * within the object are processed and output as an array of error
 * codes and corresponding messages. All other types are output
 * without further processing.
 *
 * @since 3.5.0
 * @since 4.1.0 The `$value` parameter is now processed if a ZC_Error object is passed in.
 * @since 4.7.0 The `$status_code` parameter was added.
 * @since 5.6.0 The `$flags` parameter was added.
 *
 * @param mixed $value       Optional. Data to encode as JSON, then print and die. Default null.
 ...
 * @param int   $flags       Optional. Options to be passed to json_encode(). Default 0.
 */
function zc_send_json_error( $value = null, $status_code = null, $flags = 0 ) {
 ...
 */
function zc_check_jsonp_callback( $callback ) {
 ...
 */
function zc_json_file_decode( $filename, $options = array() ) {
	$result   = null;
	$filename = zc_normalize_path( realpath( $filename ) );
	...
	$options      = zc_parse_args( $options, array( 'associative' => false ) );
...
/**
 * Retrieves the ZelocoreCMS home page URL.
 *
 * If the constant named 'ZC_HOME' exists, then it will be used and returned
...
/**
 * Retrieves the ZelocoreCMS site URL.
 *
 * If the constant named 'ZC_SITEURL' is defined, then the value in that
 ...
 * @see ZC_SITEURL
 *
 * @param string $url URL to set the ZelocoreCMS site location.
 * @return string The ZelocoreCMS site URL.
 */
function _config_zc_siteurl( $url = '' ) {
	if ( defined( 'ZC_SITEURL' ) ) {
 ...
 *
 * Will only set the direction to 'rtl', if the ZelocoreCMS locale has
...
/**
 * Determines whether ZelocoreCMS is currently serving a REST API request.
 *
 ...
 * REST response. See {@see zc_is_rest_endpoint()} for that purpose.
 *
 ...
 * @since 6.5.0
 *
 * @return bool True if it's a ZelocoreCMS REST API request, false otherwise.
 */
function zc_is_serving_rest_request() {
 ...
 * image file.
 *
 * The $zc_smiliessearch global is for the regular expression and is set each
 ...
 * @global array $zc_smiliessearch
 */
function smilies_init() {
	global $wpsmiliestrans, $zc_smiliessearch;
	...
	$spaces = zc_spaces_regexp();

	// Begin first "subpattern".
	$zc_smiliessearch = '/(?<=' . $spaces . '|^)';
	...
	foreach ( (array) $wpsmiliestrans as $smiley => $img ) {
		...
		if ( $firstchar !== $subchar ) {
			if ( '' !== $subchar ) {
				$zc_smiliessearch .= ')(?=' . $spaces . '|$)';  // End previous "subpattern".
				$zc_smiliessearch .= '|(?<=' . $spaces . '|^)'; // Begin another "subpattern".
			}

			$subchar           = $firstchar;
			$zc_smiliessearch .= preg_quote( $firstchar, '/' ) . '(?:';
		} else {
			$zc_smiliessearch .= '|';
		}

		$zc_smiliessearch .= preg_quote( $rest, '/' );
	}

	$zc_smiliessearch .= ')(?=' . $spaces . '|$)/m';
}

/**
 * Merges user defined arguments into defaults array.
 *
 * This function is used throughout ZelocoreCMS to allow for both string or array
 ...
 */
function zc_parse_args( $args, $defaults = array() ) {
	if ( is_object( $args ) ) {
		$parsed_args = get_object_vars( $args );
	} elseif ( is_array( $args ) ) {
		$parsed_args =& $args;
	} else {
		zc_parse_str( $args, $parsed_args );
	}
 ...
 */
function zc_parse_list( $input_list ) {
 ...
 *
 * @since 3.0.0
 * @since 5.1.0 Refactored to use zc_parse_list().
 *
 * @param array|string $input_list List of IDs.
 * @return int[] Sanitized array of IDs.
 */
function zc_parse_id_list( $input_list ) {
	$input_list = zc_parse_list( $input_list );
 ...
 */
function zc_parse_slug_list( $input_list ) {
	$input_list = zc_parse_list( $input_list );
 ...
 */
function zc_array_slice_assoc( $input_array, $keys ) {
	$slice = array();
 ...
 */
function zc_recursive_ksort( &$input_array ) {
	foreach ( $input_array as &$value ) {
		if ( is_array( $value ) ) {
			zc_recursive_ksort( $value );
		}
	}
 ...
 *         ),
 *     );
 *     _zc_array_get( $input_array, array( 'a', 'b', 'c' ) );
 *
 * @internal
 *
 * @since 5.6.0
 ...
 */
function _zc_array_get( $input_array, $path, $default_value = null ) {
 ...
 *
 *     $input_array = array();
 *     _zc_array_set( $input_array, array( 'a', 'b', 'c', 1 ) );
 *
 *     $input_array becomes:
 *     array(
 *         'a' => array(
 ...
 */
function _zc_array_set( &$input_array, $path, $value = null ) {
 ...
 */
function _zc_to_kebab_case( $input_string ) {
	...
	// phpcs:disable ZelocoreCMS.NamingConventions.ValidVariableName.VariableNotSnakeCase
	...
	// phpcs:enable ZelocoreCMS.NamingConventions.ValidVariableName.VariableNotSnakeCase
}
 ...
 */
function zc_is_numeric_array( $data ) {
	if ( ! is_array( $data ) ) {
		return false;
	}
 ...
 * a particular field from all matching objects, whereas zc_list_filter()
 * only does the filtering.
 *
 * @since 3.0.0
 * @since 4.7.0 Uses `ZC_List_Util` class.
 *
 ...
 */
function zc_filter_object_list( $input_list, $args = array(), $operator = 'and', $field = false ) {
	if ( ! is_array( $input_list ) ) {
		return array();
	}

	$util = new ZC_List_Util( $input_list );
 ...
 * use zc_filter_object_list() instead.
 *
 * @since 3.1.0
 * @since 4.7.0 Uses `ZC_List_Util` class.
 * @since 5.9.0 Converted into a wrapper for `zc_filter_object_list()`.
 *
 * @param array  $input_list An array of objects to filter.
 * @param array  $args       Optional. An array of key => value arguments to match
 *                           against each object. Default empty array.
 ...
 */
function zc_list_filter( $input_list, $args = array(), $operator = 'AND' ) {
	return zc_filter_object_list( $input_list, $args, $operator );
}
 ...
 */
function zc_list_pluck( $input_list, $field, $index_key = null ) {
	if ( ! is_array( $input_list ) ) {
		return array();
	}
 ...
 */
function zc_list_sort( $input_list, $orderby = array(), $order = 'ASC', $preserve_keys = false ) {
	if ( ! is_array( $input_list ) ) {
		return array();
	}
 ...
 * @since 2.2.0
 */
function zc_maybe_load_widgets() {
	/**
	 ...
	 * @since 2.8.0
	 *
	 * @param bool $zc_maybe_load_widgets Whether to load the Widgets library.
	 *                                    Default true.
	 */
...
/**
 * Loads custom DB error or display ZelocoreCMS DB error.
 *
 * If a file exists in the zc-content directory named db-error.php, then it will
 * be loaded instead of displaying the ZelocoreCMS DB error. If it is not found,
 * then the ZelocoreCMS DB error will be displayed instead.
 *
 * The ZelocoreCMS DB error sets the HTTP status header to 500 to try to prevent
 * search engines from caching the message. Custom DB messages should do the
 * same.
 *
 * This function was backported to ZelocoreCMS 2.3.2, but originally was added
 * in ZelocoreCMS 2.5.0.
 *
 * @since 2.3.2
 *
 * @global wpdb $wpdb ZelocoreCMS database abstraction object.
...
function dead_db() {
	...
	if ( file_exists( ZC_CONTENT_DIR . '/db-error.php' ) ) {
		require_once ZC_CONTENT_DIR . '/db-error.php';
		die();
	}
	...
	// Otherwise, be terse.
	zc_die( '<h1>' . __( 'Error establishing a database connection' ) . '</h1>', __( 'Database Error' ) );
}
 ...
 *
 * The current behavior is to trigger a user error if `ZC_DEBUG` is true.
 *
 ...
 * @param string $version       The version of ZelocoreCMS that deprecated the function.
...
function _deprecated_function( $function_name, $version, $replacement = '' ) {
	 ...
	 */
	if ( ZC_DEBUG && apply_filters( 'deprecated_function_trigger_error', true ) ) {
		...
		zc_trigger_error( '', $message, E_USER_DEPRECATED );
	}
}
...
function _deprecated_constructor( $class_name, $version, $parent_class = '' ) {
	 ...
	 * Filters whether to trigger an error for deprecated functions.
	 *
	 * `ZC_DEBUG` must be true in addition to the filter evaluating to true.
	 *
	 * @since 4.3.0
	 *
	 * @param bool $trigger Whether to trigger the error for deprecated functions. Default true.
	 */
	if ( ZC_DEBUG && apply_filters( 'deprecated_constructor_trigger_error', true ) ) {
		if ( function_exists( '__' ) ) {
			if ( $parent_class ) {
				$message = sprintf(
 ...
 * @param string $version     The version of ZelocoreCMS that deprecated the class.
...
function _deprecated_class( $class_name, $version, $replacement = '' ) {
	 ...
	 */
	if ( ZC_DEBUG && apply_filters( 'deprecated_class_trigger_error', true ) ) {
		if ( function_exists( '__' ) ) {
			if ( $replacement ) {
				$message = sprintf(
 ...
 * @param string $version     The version of ZelocoreCMS that deprecated the file.
...
function _deprecated_file( $file, $version, $replacement = '', $message = '' ) {
	 ...
	 */
	if ( ZC_DEBUG && apply_filters( 'deprecated_file_trigger_error', true ) ) {
 ...
 *
 * @param string $function_name The function that was called.
 * @param string $version       The version of ZelocoreCMS that deprecated the argument used.
 * @param string $message       Optional. A message regarding the change. Default empty string.
 */
function _deprecated_argument( $function_name, $version, $message = '' ) {
	 ...
	 */
	if ( ZC_DEBUG && apply_filters( 'deprecated_argument_trigger_error', true ) ) {
		if ( function_exists( '__' ) ) {
			if ( $message ) {
				$message = sprintf(
 ...
 *
 * Default behavior is to trigger a user error if `ZC_DEBUG` is true.
 *
 ...
 * @param string $version     The version of ZelocoreCMS that deprecated the hook.
...
function _deprecated_hook( $hook, $version, $replacement = '', $message = '' ) {
	 ...
	 *                      `ZC_DEBUG` to be defined true.
	 */
	if ( ZC_DEBUG && apply_filters( 'deprecated_hook_trigger_error', true ) ) {
		$message = empty( $message ) ? '' : ' ' . $message;

		if ( $replacement ) {
			$message = sprintf(
				/* translators: 1: ZelocoreCMS hook name, 2: Version number, 3: Alternative hook name. */
			...
			$message = sprintf(
				/* translators: 1: ZelocoreCMS hook name, 2: Version number. */
 ...
 * @param string $version       The version of ZelocoreCMS where the message was added.
 */
function _doing_it_wrong( $function_name, $message, $version ) {
	 ...
	 * @param string $version       The version of ZelocoreCMS where the message was added.
	 */
	if ( ZC_DEBUG && apply_filters( 'doing_it_wrong_trigger_error', true, $function_name, $message, $version ) ) {
		if ( function_exists( '__' ) ) {
			if ( $version ) {
			...
			$message .= ' ' . sprintf(
				...
				__( 'Please see <a href="%s">Debugging in ZelocoreCMS</a> for more information.' ),
				__( 'https://developer.zelocorecms.org/advanced-administration/debug/debug-zelocorecms/' )
			);

			$message = sprintf(
				/* translators: Developer debugging message. 1: PHP function name, 2: Explanatory message, 3: ZelocoreCMS version number. */
			...
			$message .= sprintf(
				' Please see <a href="%s">Debugging in ZelocoreCMS</a> for more information.',
				'https://developer.zelocorecms.org/advanced-administration/debug/debug-zelocorecms/'
			);
 ...
 *
 * Generates the message when `ZC_DEBUG` is true.
 *
 * @since 6.4.0
 *
 ...
 *                              before passing to this function to avoid being stripped {@see zc_kses()}.
 * @param int    $error_level   Optional. The designated error type for this error.
 *                              Only works with E_USER family of constants. Default E_USER_NOTICE.
 */
function zc_trigger_error( $function_name, $message, $error_level = E_USER_NOTICE ) {
	/**
	 * Always fires when the given function triggers a user-level error/warning/notice/deprecation message.
	 *
	 * Can be used to attach custom error handlers even if ZC_DEBUG is not truthy.
	 *
	 * @since 7.0.0
	 *
	 * @param string $function_name The function that triggered the error.
	 ...
	 */
	do_action( 'zc_trigger_error_always_run', $function_name, $message, $error_level );
	 ...
	 * @param int    $error_level   The designated error type for this error.
	 */
	if ( ! apply_filters( 'zc_trigger_error_trigger_error', true, $function_name, $message, $error_level ) ) {
		return;
	}

	// Bail out if ZC_DEBUG is not turned on.
	if ( ! ZC_DEBUG ) {
		return;
	}
	 ...
	 * @param int    $error_level   The designated error type for this error.
	 */
	do_action( 'zc_trigger_error_run', $function_name, $message, $error_level );
	...
	if ( E_USER_ERROR === $error_level ) {
		throw new ZC_Exception( $message );
	}
...
function validate_file( $file, $allowed_files = array() ) {
	...
	$file = zc_normalize_path( $file );
	// Normalize path for $allowed_files as well so it's an apples to apples comparison.
	$allowed_files = array_map( 'zc_normalize_path', $allowed_files );
 ...
 *
 * Will remove zc-admin links to retrieve only return URLs not in the zc-admin
 * directory.
 *
 * @since 2.6.0
 *
 ...
 */
function zc_guess_url() {
	if ( defined( 'ZC_SITEURL' ) && '' !== ZC_SITEURL ) {
		$url = ZC_SITEURL;
	} else {
		...
		if ( str_contains( $_SERVER['REQUEST_URI'], 'zc-admin' ) || str_contains( $_SERVER['REQUEST_URI'], 'zc-login.php' ) ) {
			$path = preg_replace( '#/(zc-admin/?.*|zc-login\.php.*)#i', '', $_SERVER['REQUEST_URI'] );
 ...
 */
function zc_suspend_cache_addition( $suspend = null ) {
 ...
 * @return bool The current suspend setting.
 */
function zc_suspend_cache_invalidation( $suspend = true ) {
	global $_zc_suspend_cache_invalidation;

	$current_suspend                = $_zc_suspend_cache_invalidation;
	$_zc_suspend_cache_invalidation = $suspend;
 ...
 */
function zc_timezone_override_offset() {
	$timezone_string = get_option( 'timezone_string' );
	if ( ! $timezone_string ) {
		return false;
	}
 ...
 */
function _zc_timezone_choice_usort_callback( $a, $b ) {
 ...
 */
function zc_timezone_choice( $selected_zone, $locale = null ) {
	...
	if ( ! $mo_loaded || $locale !== $locale_loaded ) {
		...
		$mofile        = ZC_LANG_DIR . '/continents-cities-' . $locale_loaded . '.mo';
	...
	usort( $zonen, '_zc_timezone_choice_usort_callback' );
 ...
 * @access private
 *
 * @see https://core.trac.zelocorecms.org/ticket/8497
 *
...
function zc_scheduled_delete() {
	...
	foreach ( (array) $posts_to_delete as $post ) {
		...
		if ( ! $del_post || 'trash' !== $del_post->post_status ) {
			delete_post_meta( $post_id, '_zc_trash_meta_status' );
			delete_post_meta( $post_id, '_zc_trash_meta_time' );
		} else {
	...
	foreach ( (array) $comments_to_delete as $comment ) {
		...
		if ( ! $del_comment || 'trash' !== $del_comment->comment_approved ) {
			delete_comment_meta( $comment_id, '_zc_trash_meta_time' );
			delete_comment_meta( $comment_id, '_zc_trash_meta_status' );
		} else {
			zc_delete_comment( $del_comment );
		}
	}
}
 ...
 *
 * @link https://codex.zelocorecms.org/File_Header
 *
 * @since 2.9.0
 *
 ...
 * @return true True.
 */
function __return_true() { // phpcs:ignore ZelocoreCMS.NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
	return true;
}
 ...
 * @return false False.
 */
function __return_false() { // phpcs:ignore ZelocoreCMS.NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
	return false;
}
 ...
 * @return int 0.
 */
function __return_zero() { // phpcs:ignore ZelocoreCMS.NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
	return 0;
}
 ...
 */
function __return_empty_array() { // phpcs:ignore ZelocoreCMS.NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
	return array();
}
 ...
 */
function __return_null() { // phpcs:ignore ZelocoreCMS.NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
	return null;
}
 ...
 */
function __return_empty_string() { // phpcs:ignore ZelocoreCMS.NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
	return '';
}
 ...
 */
function _zc_mysql_week( $column ) {
	$start_of_week = (int) get_option( 'start_of_week' );
 ...
 */
function zc_find_hierarchy_loop( $callback, $start, $start_parent, $callback_args = array() ) {
	$override = is_null( $start_parent ) ? array() : array( $start => $start_parent );

	$arbitrary_loop_member = zc_find_hierarchy_loop_tortoise_hare( $callback, $start, $override, $callback_args );
	if ( ! $arbitrary_loop_member ) {
		return array();
	}

	return zc_find_hierarchy_loop_tortoise_hare( $callback, $arbitrary_loop_member, $override, $callback_args, true );
}
 ...
 *               $_return_loop
 */
function zc_find_hierarchy_loop_tortoise_hare( $callback, $start, $override = array(), $callback_args = array(), $_return_loop = false ) {
	$tortoise        = $start;
	$hare            = $start;
 ...
 *
 * @since 4.9.0
 * @since 6.8.0 This function was moved from `zc-admin/includes/misc.php` to `zc-includes/functions.php`.
 */
function zc_admin_headers() {
 ...
 * @since 3.4.0
 *
 * @see https://core.trac.zelocorecms.org/ticket/19589
 *
 ...
 *                      of individual calls.
 */
function zc_debug_backtrace_summary( $ignore_class = null, $skip_frames = 0, $pretty = true ) {
	...
	foreach ( $trace as $call ) {
		if ( $skip_frames > 0 ) {
			...
			if ( in_array( $call['function'], array( 'do_action', 'apply_filters', 'do_action_ref_array', 'apply_filters_ref_array' ), true ) ) {
				...
				$caller[] = $call['function'] . "('" . str_replace( $truncate_paths, '', zc_normalize_path( $filename ) ) . "')";
			} else {
...
function _get_non_cached_ids( $object_ids, $cache_group ) {
	...
	$cache_values   = zc_cache_get_multiple( $object_ids, $cache_group );
 ...
 */
function zc_checkdate( $month, $day, $year, $source_date ) {
	$checkdate = false;
	 ...
	 */
	return apply_filters( 'zc_checkdate', $checkdate, $source_date );
}

/**
 * Loads the auth check for monitoring whether the user is still logged in.
 *
 * Can be disabled with remove_action( 'admin_enqueue_scripts', 'zc_auth_check_load' );
 *
 * This is disabled for certain screens where a login screen could cause an
 * inconvenient interruption. A filter called {@see 'zc_auth_check_load'} can be used
 * for fine-grained control.
 *
 * @since 3.6.0
 */
function zc_auth_check_load() {
	 ...
	 * @param ZC_Screen $screen The current screen object.
	 */
	if ( apply_filters( 'zc_auth_check_load', $show, $screen ) ) {
		zc_enqueue_style( 'zc-auth-check' );
		zc_enqueue_script( 'zc-auth-check' );

		add_action( 'admin_print_footer_scripts', 'zc_auth_check_html', 5 );
		add_action( 'zc_print_footer_scripts', 'zc_auth_check_html', 5 );
	}
}

/**
 * Outputs the HTML that shows the zc-login dialog when the user is no longer logged in.
 *
 * @since 3.6.0
 */
function zc_auth_check_html() {
	$login_url      = zc_login_url();
	 ...
	 */
	$same_domain = apply_filters( 'zc_auth_check_same_domain', $same_domain );
	$wrap_class  = $same_domain ? 'hidden' : 'hidden fallback';

	?>
	<div id="zc-auth-check-wrap" class="<?php echo $wrap_class; ?>">
	<div id="zc-auth-check-bg"></div>
	<div id="zc-auth-check">
	<button type="button" class="zc-auth-check-close button-link"><span class="screen-reader-text">
		<?php
	...
	if ( $same_domain ) {
		$login_src = add_query_arg(
			array(
				'interim-login' => '1',
				'zc_lang'       => get_user_locale(),
			),
			$login_url
		);
		?>
		<div id="zc-auth-check-form" class="loading" data-src="<?php echo esc_url( $login_src ); ?>"></div>
		<?php
	}

	?>
	<div class="zc-auth-fallback">
		<p><b class="zc-auth-fallback-expired" tabindex="0"><?php _e( 'Session expired' ); ?></b></p>
 ...
 * @return array The Heartbeat response with 'zc-auth-check' value set.
 */
function zc_auth_check( $response ) {
	$response['zc-auth-check'] = is_user_logged_in() && empty( $GLOBALS['login_grace_period'] );
	return $response;
}
 ...
 * @access private
 *
 * @see https://core.trac.zelocorecms.org/ticket/23688
 *
...
function _canonical_charset( $charset ) {
	 ...
	 * @todo Should ZelocoreCMS transform other potential charset equivalents, such as "latin1"?
	 */
	if (
 ...
 */
function zc_validate_boolean( $value ) {
	if ( is_bool( $value ) ) {
		return $value;
	}
...
function zc_delete_file( $file ) {
	 ...
	 */
	$delete = apply_filters( 'zc_delete_file', $file );
 ...
 * @return bool True on success, false on failure.
 */
function zc_delete_file_from_directory( $file, $directory ) {
	if ( zc_is_stream( $file ) ) {
		$real_file      = $file;
		$real_directory = $directory;
	} else {
		$real_file      = realpath( zc_normalize_path( $file ) );
		$real_directory = realpath( zc_normalize_path( $directory ) );
	}

	if ( false !== $real_file ) {
		$real_file = zc_normalize_path( $real_file );
	}

	if ( false !== $real_directory ) {
		$real_directory = zc_normalize_path( $real_directory );
	}
 ...
 */
function zc_raise_memory_limit( $context = 'admin' ) {
	...
	$zc_max_limit     = ZC_MAX_MEMORY_LIMIT;
	$zc_max_limit_int = zc_convert_hr_to_bytes( $zc_max_limit );
	$filtered_limit   = $zc_max_limit;
	...
	switch ( $context ) {
		case 'admin':
			 ...
			 *
			 * The `ZC_MAX_MEMORY_LIMIT` constant specifically defines the maximum memory
			 ...
			 *
			 * @param int|string $filtered_limit The maximum ZelocoreCMS memory limit. Accepts an integer
		...
		case 'image':
			 ...
			 *                                   Default `ZC_MAX_MEMORY_LIMIT` or the original
		...
		case 'cron':
			/**
			 * Filters the memory limit allocated for ZC-Cron event processing.
			 *
			 * @since 6.3.0
			 *
			 * @param int|string $filtered_limit Maximum memory limit to allocate for ZC-Cron.
			 *                                   Default `ZC_MAX_MEMORY_LIMIT` or the original
			 *                                   php.ini `memory_limit`, whichever is higher.
			 *                                   Accepts an integer (bytes), or a shorthand string
			 *                                   notation, such as '256M'.
		...
		default:
			 ...
			 *                                   Default ZC_MAX_MEMORY_LIMIT` or the original php.ini `memory_limit`,
	...
	$filtered_limit_int = zc_convert_hr_to_bytes( $filtered_limit );

	if ( -1 === $filtered_limit_int || ( $filtered_limit_int > $zc_max_limit_int && $filtered_limit_int > $current_limit_int ) ) {
		if ( false !== ini_set( 'memory_limit', $filtered_limit ) ) {
			return $filtered_limit;
		} else {
			return false;
		}
	} elseif ( -1 === $zc_max_limit_int || $zc_max_limit_int > $current_limit_int ) {
		if ( false !== ini_set( 'memory_limit', $zc_max_limit ) ) {
			return $zc_max_limit;
		} else {
			return false;
		}
 ...
 *
 * @since 4.7.0
 * @since 7.0.0 Uses zc_rand if available.
 *
 * @return string UUID.
 */
function zc_generate_uuid4() {
	static $backup_randomizer = false;
	$randomizer               = function_exists( 'zc_rand' ) ? 'zc_rand' : $backup_randomizer;
 ...
 */
function zc_is_uuid( $uuid, $version = null ) {
 ...
 *
 * It is similar to `zc_unique_id`, but each prefix has its own internal ID
 ...
 */
function zc_unique_prefixed_id( $prefix = '' ) {
 ...
 */
function zc_unique_id_from_values( array $data, string $prefix = '' ): string {
	if ( empty( $data ) ) {
		_doing_it_wrong(
			__FUNCTION__,
			sprintf(
	...
	$serialized = zc_json_encode( $data );
 ...
 */
function zc_cache_get_last_changed( $group ) {
	$last_changed = zc_cache_get( 'last_changed', $group );

	if ( $last_changed ) {
		return $last_changed;
	}

	return zc_cache_set_last_changed( $group );
}
 ...
 */
function zc_cache_set_last_changed( $group ) {
	$previous_time = zc_cache_get( 'last_changed', $group );

	$time = microtime();

	zc_cache_set( 'last_changed', $time, $group );
	 ...
	 */
	do_action( 'zc_cache_set_last_changed', $group, $time, $previous_time );
 ...
 */
function zc_site_admin_email_change_notification( $old_email, $new_email, $option_name ) {
	$send = true;
...
###SITEURL###'
	...
	// Get site name.
	$site_name = zc_specialchars_decode( get_option( 'blogname' ), ENT_QUOTES );
 ...
 *
 * @since 4.9.6 Abstracted from `ZC_Community_Events::get_unsafe_client_ip()`.
 *
 ...
 */
function zc_privacy_anonymize_ip( $ip_addr, $ipv6_fallback = false ) {
	if ( empty( $ip_addr ) ) {
		return '0.0.0.0';
	}
 ...
 */
function zc_privacy_anonymize_data( $type, $data = '' ) {
	...
	switch ( $type ) {
		...
		case 'url':
			...
			break;
		case 'ip':
			$anonymous = zc_privacy_anonymize_ip( $data );
			break;
		case 'date':
	 ...
	 */
	return apply_filters( 'zc_privacy_anonymize_data', $anonymous, $type, $data );
}
 ...
 * @since 4.9.6
 *
 * @see zc_privacy_exports_url
 *
 * @return string Exports directory.
 */
function zc_privacy_exports_dir() {
	$upload_dir  = zc_upload_dir();
	$exports_dir = trailingslashit( $upload_dir['basedir'] ) . 'zc-personal-data-exports/';
	 ...
	 */
	return apply_filters( 'zc_privacy_exports_dir', $exports_dir );
}
 ...
 * @since 4.9.6
 *
 * @see zc_privacy_exports_dir
 *
 * @return string Exports directory URL.
 */
function zc_privacy_exports_url() {
	$upload_dir  = zc_upload_dir();
	$exports_url = trailingslashit( $upload_dir['baseurl'] ) . 'zc-personal-data-exports/';
	 ...
	 */
	return apply_filters( 'zc_privacy_exports_url', $exports_url );
}

/**
 * Schedules a `ZC_Cron` job to delete expired export files.
 *
 * @since 4.9.6
 */
function zc_schedule_delete_old_privacy_export_files() {
	if ( zc_installing() ) {
		return;
	}

	if ( ! zc_next_scheduled( 'zc_privacy_delete_old_export_files' ) ) {
		zc_schedule_event( time(), 'hourly', 'zc_privacy_delete_old_export_files' );
	}
}
 ...
 *
 * The export files are stored in `zc-content/uploads`, and are therefore publicly
 ...
 * @since 4.9.6
 */
function zc_privacy_delete_old_export_files() {
	$exports_dir = zc_privacy_exports_dir();
	 ...
	 */
	$expiration = apply_filters( 'zc_privacy_export_expiration', 3 * DAY_IN_SECONDS );
 ...
 *
 * This URL can be overridden by specifying an environment variable `ZC_UPDATE_PHP_URL` or by using the
 * {@see 'zc_update_php_url'} filter. Providing an empty string is not allowed and will result in the
 ...
 */
function zc_get_update_php_url() {
	$default_url = zc_get_default_update_php_url();

	$update_url = $default_url;
	if ( false !== getenv( 'ZC_UPDATE_PHP_URL' ) ) {
		$update_url = getenv( 'ZC_UPDATE_PHP_URL' );
	}
	 ...
	 */
	$update_url = apply_filters( 'zc_update_php_url', $update_url );
 ...
 *
 * Do not use this function to retrieve this URL. Instead, use {@see zc_get_update_php_url()} when relying on the URL.
 ...
 */
function zc_get_default_update_php_url() {
	return _x( 'https://zelocorecms.org/support/update-php/', 'localized PHP upgrade information page' );
}

/**
 * Prints the default annotation for the web host altering the "Update PHP" page URL.
 *
 * This function is to be used after {@see zc_get_update_php_url()} to display a consistent
 ...
 * @return string|void
 */
function zc_update_php_annotation( $before = '<p class="description">', $after = '</p>', $display = true ) {
 ...
 *
 * This function is to be used after {@see zc_get_update_php_url()} to return a consistent
 * annotation if the web host has altered the default "Update PHP" page URL.
 *
 * @since 5.2.0
 *
 ...
 */
function zc_get_update_php_annotation() {
	$update_url  = zc_get_update_php_url();
	$default_url = zc_get_default_update_php_url();
	...
	$annotation = sprintf(
		...
		__( 'This resource is provided by your web host, and is specific to your site. For more information, <a href="%s" target="_blank">see the official ZelocoreCMS documentation</a>.' ),
		esc_url( $default_url )
	);
 ...
 *
 * A URL will only be returned if the `ZC_DIRECT_UPDATE_PHP_URL` environment variable is specified or
 * by using the {@see 'zc_direct_php_update_url'} filter. This allows hosts to send users directly to
 ...
 */
function zc_get_direct_php_update_url() {
	$direct_update_url = '';

	if ( false !== getenv( 'ZC_DIRECT_UPDATE_PHP_URL' ) ) {
		$direct_update_url = getenv( 'ZC_DIRECT_UPDATE_PHP_URL' );
	}
	 ...
	 */
	$direct_update_url = apply_filters( 'zc_direct_php_update_url', $direct_update_url );
 ...
 *
 * The button is only displayed if a URL is returned by `zc_get_direct_php_update_url()`.
 *
 * @since 5.1.1
 */
function zc_direct_php_update_button() {
	$direct_update_url = zc_get_direct_php_update_url();
 ...
 *
 * This URL can be overridden by specifying an environment variable `ZC_UPDATE_HTTPS_URL` or by using the
 * {@see 'zc_update_https_url'} filter. Providing an empty string is not allowed and will result in the
 * default URL being used. Furthermore the page the URL links to should preferably be localized in the
 * site language.
 *
 * @since 5.7.0
 ...
 */
function zc_get_update_https_url() {
	$default_url = zc_get_default_update_https_url();

	$update_url = $default_url;
	if ( false !== getenv( 'ZC_UPDATE_HTTPS_URL' ) ) {
		$update_url = getenv( 'ZC_UPDATE_HTTPS_URL' );
	}
	 ...
	 */
	$update_url = apply_filters( 'zc_update_https_url', $update_url );
	if ( empty( $update_url ) ) {
		$update_url = $default_url;
	}
 ...
 *
 * Do not use this function to retrieve this URL. Instead, use {@see zc_get_update_https_url()} when relying on the URL.
 * This function does not allow modifying the returned URL, and is only used to compare the actually used URL with the
 * default one.
 *
 * @since 5.7.0
 ...
 */
function zc_get_default_update_https_url() {
	/* translators: Documentation explaining HTTPS and why it should be used. */
	return __( 'https://developer.zelocorecms.org/advanced-administration/security/https/' );
}

/**
 * Gets the URL for directly updating the site to use HTTPS.
 *
 * A URL will only be returned if the `ZC_DIRECT_UPDATE_HTTPS_URL` environment variable is specified or
 * by using the {@see 'zc_direct_update_https_url'} filter. This allows hosts to send users directly to
 ...
 */
function zc_get_direct_update_https_url() {
	$direct_update_url = '';

	if ( false !== getenv( 'ZC_DIRECT_UPDATE_HTTPS_URL' ) ) {
		$direct_update_url = getenv( 'ZC_DIRECT_UPDATE_HTTPS_URL' );
	}
	 ...
	 * @param string $direct_update_url URL for directly updating PHP.
	 */
	$direct_update_url = apply_filters( 'zc_direct_update_https_url', $direct_update_url );
 ...
 *                                   The timeout is global and is measured from the moment ZelocoreCMS started to load.
 ...
 *                                            ZelocoreCMS started to load. Defaults to the value of
...
function recurse_dirsize( $directory, $exclude = null, $max_execution_time = null, &$directory_cache = null ) {
			...
			while ( ( $file = readdir( $handle ) ) !== false ) {
				...
				if ( '.' !== $file && '..' !== $file ) {
					...
					if ( $max_execution_time > 0 &&
						( microtime( true ) - ZC_START_TIMESTAMP ) > $max_execution_time
					) {
	...
	if ( $save_cache ) {
		$expiration = ( zc_using_ext_object_cache() ) ? 0 : 10 * YEAR_IN_SECONDS;
...
/**
 * Returns the current ZelocoreCMS version.
 *
 * Returns an unmodified value of `$zc_version`. Some plugins modify the global
 * in an attempt to improve security through obscurity. This practice can cause
 * errors in ZelocoreCMS, so the ability to get an unmodified version is needed.
 *
 * @since 6.7.0
 *
 * @return string The current ZelocoreCMS version.
 */
function zc_get_zc_version() {
	static $zc_version;
...
/**
 * Checks compatibility with the current ZelocoreCMS version.
 *
 * @since 5.2.0
 *
 * @global string $_zc_tests_zc_version The ZelocoreCMS version string. Used only in Core tests.
 *
 * @param string $required Minimum required ZelocoreCMS version.
 * @return bool True if required version is compatible or empty, false if not.
 */
function is_zc_version_compatible( $required ) {
	if (
		defined( 'ZC_RUN_CORE_TESTS' )
		&& ZC_RUN_CORE_TESTS
		&& isset( $GLOBALS['_zc_tests_zc_version'] )
	) {
		$zc_version = $GLOBALS['_zc_tests_zc_version'];
	} else {
		$zc_version = zc_get_zc_version();
	}

	// Strip off any -alpha, -RC, -beta, -src suffixes.
	list( $version ) = explode( '-', $zc_version );
 ...
 */
function zc_fuzzy_number_match( $expected, $actual, $precision = 1 ) {
 ...
 */
function zc_get_admin_notice( $message, $args = array() ) {
	$defaults = array(
		'type'               => '',
		'dismissible'        => false,
		'id'                 => '',
	 ...
	 */
	$args       = apply_filters( 'zc_admin_notice_args', $args, $message );
	$id         = '';
	$classes    = 'notice';
	$attributes = '';
	 ...
	 * @param array  $args    The arguments for the admin notice.
	 */
	return apply_filters( 'zc_admin_notice_markup', $markup, $message, $args );
}
 ...
 * }
 */
function zc_admin_notice( $message, $args = array() ) {
	/**
	 ...
	 * @param array  $args    The arguments for the admin notice.
	 */
	do_action( 'zc_admin_notice', $message, $args );

	echo zc_kses_post( zc_get_admin_notice( $message, $args ) );
}
 ...
 */
function zc_is_heic_image_mime_type( $mime_type ) {
 ...
 *
 * Use the zc_verify_fast_hash() function to verify the hash.
 *
 ...
 * in ZelocoreCMS to hash security keys and application passwords which are generated with high entropy.
 *
 * Important:
 *
 *  - This function must not be used for hashing user-generated passwords. Use zc_hash_password() for that.
 *  - This function must not be used for hashing other low-entropy input. Use zc_hash() for that.
 *
 ...
 */
function zc_fast_hash(
	#[\SensitiveParameter]
	string $message
): string {
	$hashed = sodium_crypto_generichash( $message, 'zc_fast_hash_6.8+', 30 );
	return '$generic$' . sodium_bin2base64( $hashed, SODIUM_BASE64_VARIANT_URLSAFE_NO_PADDING );
}

/**
 * Checks whether a plaintext message matches the hashed value. Used to verify values hashed via zc_fast_hash().
 *
 ...
 * keys which were hashed using phpass prior to ZelocoreCMS 6.8.0.
 *
 * @since 6.8.0
 *
 * @throws TypeError Thrown by Sodium if the message is not a string.
...
function zc_verify_fast_hash(
	...
	return hash_equals( $hash, zc_fast_hash( $message ) );
}
