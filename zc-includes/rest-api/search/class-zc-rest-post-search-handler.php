<?php
/**
 * REST API: ZC_REST_Post_Search_Handler class
 *
 * @package ZelocoreCMS
 * @subpackage REST_API
 * @since 5.0.0
 ...
 */
class ZC_REST_Post_Search_Handler extends ZC_REST_Search_Handler {
	...
	public function search_items( ZC_REST_Request $request ) {
		...
		$post_types = $request[ ZC_REST_Search_Controller::PROP_SUBTYPE ];
		if ( in_array( ZC_REST_Search_Controller::TYPE_ANY, $post_types, true ) ) {
		...
		$found_ids = zc_list_pluck( $posts, 'ID' );
	...
	public function prepare_item( $id, array $fields ) {
		...
		if ( in_array( ZC_REST_Search_Controller::PROP_ID, $fields, true ) ) {
			$data[ ZC_REST_Search_Controller::PROP_ID ] = (int) $post->ID;
		}
		...
		if ( in_array( ZC_REST_Search_Controller::PROP_TITLE, $fields, true ) ) {
			if ( post_type_supports( $post->post_type, 'title' ) ) {
				...
				$data[ ZC_REST_Search_Controller::PROP_TITLE ] = get_the_title( $post->ID );
				remove_filter( 'protected_title_format', array( $this, 'protected_title_format' ) );
				remove_filter( 'private_title_format', array( $this, 'protected_title_format' ) );
			} else {
				$data[ ZC_REST_Search_Controller::PROP_TITLE ] = '';
			}
		}

		if ( in_array( ZC_REST_Search_Controller::PROP_URL, $fields, true ) ) {
			$data[ ZC_REST_Search_Controller::PROP_URL ] = get_permalink( $post->ID );
		}

		if ( in_array( ZC_REST_Search_Controller::PROP_TYPE, $fields, true ) ) {
			$data[ ZC_REST_Search_Controller::PROP_TYPE ] = $this->type;
		}

		if ( in_array( ZC_REST_Search_Controller::PROP_SUBTYPE, $fields, true ) ) {
			$data[ ZC_REST_Search_Controller::PROP_SUBTYPE ] = $post->post_type;
		}
