<?php
/**
 * REST API: ZC_REST_Search_Controller class
 *
 * @package ZelocoreCMS
 * @subpackage REST_API
 * @since 5.0.0
...
/**
 * Core class to search through all ZelocoreCMS content via the REST API.
 *
 * @since 5.0.0
 *
 * @see ZC_REST_Controller
 */
class ZC_REST_Search_Controller extends ZC_REST_Controller {
	...
	public function __construct( array $search_handlers ) {
		...
		foreach ( $search_handlers as $search_handler ) {
			if ( ! $search_handler instanceof ZC_REST_Search_Handler ) {
				_doing_it_wrong(
					__METHOD__,
					/* translators: %s: PHP class name. */
					sprintf( __( 'REST search handlers must extend the %s class.' ), 'ZC_REST_Search_Handler' ),
					'5.0.0'
				);
				continue;
			}
	 ...
	 * @return true|ZC_Error True if the request has search access, ZC_Error object otherwise.
	 */
	...
	public function get_items( $request ) {
		...
		if ( is_zc_error( $handler ) ) {
			return $handler;
		}

		$result = $handler->search_items( $request );

		if ( ! isset( $result[ ZC_REST_Search_Handler::RESULT_IDS ] ) || ! is_array( $result[ ZC_REST_Search_Handler::RESULT_IDS ] ) || ! isset( $result[ ZC_REST_Search_Handler::RESULT_TOTAL ] ) ) {
			return new ZC_Error(
		...
		$ids = $result[ ZC_REST_Search_Handler::RESULT_IDS ];
		...
		$total     = (int) $result[ ZC_REST_Search_Handler::RESULT_TOTAL ];
		...
		$response = $is_head_request ? new ZC_REST_Response( array() ) : rest_ensure_response( $results );
		$response->header( 'X-ZC-Total', $total );
	 ...
	 * @return string[]|ZC_Error List of valid subtypes, or ZC_Error object on failure.
	 */
	public function sanitize_subtypes( $subtypes, $request, $parameter ) {
		$subtypes = zc_parse_slug_list( $subtypes );

		$subtypes = rest_parse_request_arg( $subtypes, $request, $parameter );
		if ( is_zc_error( $subtypes ) ) {
			return $subtypes;
		}
	 ...
	 *
	 * @param ZC_REST_Request $request Full details about the request.
	 * @return ZC_REST_Search_Handler|ZC_Error Search handler for the request type, or ZC_Error object on failure.
	 */
