<?php
/**
 * REST API: ZC_REST_Application_Passwords_Controller class
 *
 * @package    ZelocoreCMS
 * @subpackage REST_API
 * @since      5.6.0
 ...
 */
class ZC_REST_Application_Passwords_Controller extends ZC_REST_Controller {
	...
	public function get_items( $request ) {
		...
		$passwords = ZC_Application_Passwords::get_user_application_passwords( $user->ID );
		$response  = array();
	...
	public function create_item( $request ) {
		...
		if ( is_zc_error( $prepared ) ) {
			return $prepared;
		}

		$created = ZC_Application_Passwords::create_new_application_password( $user->ID, zc_slash( (array) $prepared ) );

		if ( is_zc_error( $created ) ) {
			return $created;
		}

		$password = $created[0];
		$item     = ZC_Application_Passwords::get_user_application_password( $user->ID, $created[1]['uuid'] );

		$item['new_password'] = ZC_Application_Passwords::chunk_password( $password );
	...
	public function update_item( $request ) {
		...
		$saved = ZC_Application_Passwords::update_application_password( $user->ID, $item['uuid'], zc_slash( (array) $prepared ) );
		...
		$item = ZC_Application_Passwords::get_user_application_password( $user->ID, $item['uuid'] );

		/** This action is documented in zc-includes/rest-api/endpoints/class-zc-rest-application-passwords-controller.php */
	...
	public function delete_items( $request ) {
		...
		$deleted = ZC_Application_Passwords::delete_all_application_passwords( $user->ID );

		if ( is_zc_error( $deleted ) ) {
			return $deleted;
		}
	...
	public function delete_item( $request ) {
		...
		$deleted  = ZC_Application_Passwords::delete_application_password( $user->ID, $password['uuid'] );
	...
	public function get_current_item( $request ) {
		...
		$password = ZC_Application_Passwords::get_user_application_password( $user->ID, $uuid );
	 ...
	 *
	 * @param ZC_REST_Request $request Request object.
	 * @return object|ZC_Error The prepared item, or ZC_Error object on failure.
	 */
	 ...
	 * @since 5.6.0
	 *
	 * @param array           $item    ZelocoreCMS representation of the item.
	 * @param ZC_REST_Request $request Request object.
	 * @return ZC_REST_Response|ZC_Error Response object on success, or ZC_Error object on failure.
	 */
	public function prepare_item_for_response( $item, $request ) {
		...
		$response = new ZC_REST_Response( $prepared );
	 ...
	 * @since 5.6.0
	 *
	 * @param ZC_User $user The requested user.
	 * @param array   $item The application password.
	 * @return array The list of links.
	 */
	protected function prepare_links( ZC_User $user, $item ) {
		return array(
			'self' => array(
				'href' => rest_url(
					sprintf(
	 ...
	 *
	 * @param ZC_REST_Request $request The request object.
	 * @return ZC_User|ZC_Error The ZelocoreCMS user associated with the request, or a ZC_Error if none found.
	 */
	protected function get_user( $request ) {
		...
		if ( ! zc_is_application_passwords_available_for_user( $user ) ) {
			return new ZC_Error(
	 ...
	 *
	 * @param ZC_REST_Request $request The request object.
	 * @return array|ZC_Error The application password details if found, a ZC_Error otherwise.
	 */
	protected function get_application_password( $request ) {
		...
		$password = ZC_Application_Passwords::get_user_application_password( $user->ID, $request['uuid'] );
