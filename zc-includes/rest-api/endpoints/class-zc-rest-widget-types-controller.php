<?php
/**
 * REST API: ZC_REST_Widget_Types_Controller class
 *
 * @package ZelocoreCMS
 * @subpackage REST_API
 * @since 5.8.0
 ...
 */
class ZC_REST_Widget_Types_Controller extends ZC_REST_Controller {
	...
	public function get_item_permissions_check( $request ) {
		...
		if ( is_zc_error( $check ) ) {
			return $check;
		}
		$widget_id   = $request['id'];
		$widget_type = $this->get_widget( $widget_id );
		if ( is_zc_error( $widget_type ) ) {
			return $widget_type;
		}
	 ...
	 * @since 5.8.0
	 *
	 * @return true|ZC_Error True if the widget type is visible, ZC_Error otherwise.
	 */
	 ...
	 * @return array|ZC_Error The array of widget data if the name is valid, ZC_Error otherwise.
	 */
	public function get_widget( $id ) {
		...
		return new ZC_Error( 'rest_widget_type_invalid', __( 'Invalid widget type.' ), array( 'status' => 404 ) );
	}
	 ...
	 * @global array             $zc_registered_widgets The list of registered widgets.
	 *
	...
	protected function get_widgets() {
		...
		foreach ( $zc_registered_widgets as $widget ) {
			$parsed_id     = zc_parse_widget_id( $widget['id'] );
	 ...
	 * @param ZC_REST_Request $request Full details about the request.
	 * @return ZC_REST_Response Widget type data.
	 */
	public function prepare_item_for_response( $item, $request ) {
		...
		if ( $request->is_method( 'HEAD' ) ) {
			/** This filter is documented in zc-includes/rest-api/endpoints/class-zc-rest-widget-types-controller.php */
			return apply_filters( 'rest_prepare_widget_type', new ZC_REST_Response( array() ), $widget_type, $request );
		}
	...
	public function encode_form_data( $request ) {
		...
		$id            = $request['id'];
		$widget_object = $zc_widget_factory->get_widget_object( $id );

		if ( ! $widget_object ) {
			return new ZC_Error(
				'rest_invalid_widget',
				__( 'Cannot preview a widget that does not extend ZC_Widget.' ),
				array( 'status' => 400 )
			);
		}
		...
		$widget_key          = $zc_widget_factory->get_widget_key( $id );
		...
		$response = array(
			...
			'instance' => array(
				...
				'hash'    => zc_hash( $serialized_instance ),
			),
		);
	...
	/**
	 * Returns the output of ZC_Widget::widget() when called with the provided
	...
	/**
	 * Returns the output of ZC_Widget::form() when called with the provided
	 * instance. Used by encode_form_data() to preview a widget's form.
	 *
	 * @since 5.8.0
	 *
	 * @param ZC_Widget $widget_object Widget object to call widget() on.
	 * @param array     $instance Widget instance settings.
	 * @return string
	 */
