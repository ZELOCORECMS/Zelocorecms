<?php
/**
 * REST API: ZC_REST_Comments_Controller class
 *
 * @package ZelocoreCMS
 * @subpackage REST_API
 * @since 4.7.0
 ...
 */
class ZC_REST_Comments_Controller extends ZC_REST_Controller {

	/**
	 * Instance of a comment meta fields object.
	 *
	 * @since 4.7.0
	 * @var ZC_REST_Comment_Meta_Fields
	 */
	protected $meta;
	 ...
	 * @return true|ZC_Error True if the request has read access, error object otherwise.
	 */
	 ...
	 *
	 * @param ZC_REST_Request $request Full details about the request.
	 * @return ZC_REST_Response|ZC_Error Response object on success, or error object on failure.
	 */
	public function get_items( $request ) {
		...
		foreach ( $parameter_mappings as $api_param => $zc_param ) {
			if ( isset( $registered[ $api_param ], $request[ $api_param ] ) ) {
				$prepared_args[ $zc_param ] = $request[ $api_param ];
			}
		}
		...
		/**
		 * Filters ZC_Comment_Query arguments when querying comments via the REST API.
		 *
		 * @since 4.7.0
		 *
		 * @link https://developer.zelocorecms.org/reference/classes/zc_comment_query/
		 *
		 * @param array           $prepared_args Array of arguments for ZC_Comment_Query.
		...
		$response = $is_head_request ? new ZC_REST_Response( array() ) : rest_ensure_response( $comments );
		$response->header( 'X-ZC-Total', (string) $total_comments );
		$response->header( 'X-ZC-TotalPages', (string) $max_pages );
	 ...
	 * @return ZC_Comment|ZC_Error Comment object if ID is valid, ZC_Error otherwise.
	 */
	 ...
	 *
	 * @param ZC_REST_Request $request Full details about the request.
	 * @return true|ZC_Error True if the request has read access for the item, error object otherwise.
	 */
	public function get_item_permissions_check( $request ) {
		$comment = $this->get_comment( $request['id'] );
		if ( is_zc_error( $comment ) ) {
			return $comment;
		}
	 ...
	 *
	 * @param ZC_REST_Request $request Full details about the request.
	 * @return true|ZC_Error True if the request has access to create items, error object otherwise.
	 */
	public function create_item_permissions_check( $request ) {
		...
		if ( ! is_user_logged_in() ) {
			 ...
			 * @param ZC_REST_Request $request Request used to generate the
			 *                                 response.
			 */
	...
	public function create_item( $request ) {
		...
		if ( is_zc_error( $prepared_comment ) ) {
		...
		if ( isset( $request['meta']['_zc_note_status'] ) ) {
			$prepared_comment['meta']['_zc_note_status'] = $request['meta']['_zc_note_status'];
		}
		...
		// Setting remaining values before zc_insert_comment so we can use zc_allow_comment().
		...
		$check_comment_lengths = zc_check_comment_data_max_lengths( $prepared_comment );

		if ( is_zc_error( $check_comment_lengths ) ) {
		...
		$prepared_comment['comment_approved'] =
			...
			'1' :
			zc_allow_comment( $prepared_comment, true );

		if ( is_zc_error( $prepared_comment['comment_approved'] ) ) {
		 ...
		 *
		 * Allows modification of the comment right before it is inserted via zc_insert_comment().
		 * Returning a ZC_Error value from the filter will short-circuit insertion and allow
		 * skipping further processing.
		 *
		 * @since 4.7.0
		 * @since 4.8.0 `$prepared_comment` can now be a ZC_Error to short-circuit insertion.
		 *
		 * @param array|ZC_Error  $prepared_comment The prepared comment data for zc_insert_comment().
		 * @param ZC_REST_Request $request          Request used to insert the comment.
		 */
		...
		$comment_id = zc_insert_comment( zc_filter_comment( zc_slash( (array) $prepared_comment ) ) );
		 ...
		 * @since 4.7.0
		 *
		 * @param ZC_Comment      $comment  Inserted or updated comment object.
	 ...
	 *
	 * @param ZC_REST_Request $request Full details about the request.
	 * @return true|ZC_Error True if the request has access to update the item, error object otherwise.
	 */
	...
	public function update_item( $request ) {
		...
		if ( empty( $prepared_args ) && isset( $request['status'] ) ) {
			...
			$check_comment_lengths = zc_check_comment_data_max_lengths( $prepared_args );
			...
			$updated = zc_update_comment( zc_slash( (array) $prepared_args ), true );
		...
		/** This action is documented in zc-includes/rest-api/endpoints/class-zc-rest-comments-controller.php */
	 ...
	 *
	 * @param ZC_REST_Request $request Full details about the request.
	 * @return true|ZC_Error True if the request has access to delete the item, error object otherwise.
	 */
	...
	public function delete_item( $request ) {
		 ...
		 * @param ZC_Comment $comment        The comment object being considered for trashing support.
		 */
		...
		if ( $force ) {
			...
			$result   = zc_delete_comment( $comment->comment_ID, true );
			...
			$result   = zc_trash_comment( $comment->comment_ID );
		 ...
		 * @since 4.7.0
		 *
		 * @param ZC_Comment       $comment  The deleted comment data.
		 * @param ZC_REST_Response $response The response returned from the API.
	 ...
	 *
	 * @param ZC_Comment      $item    Comment object.
	 * @param ZC_REST_Request $request Request object.
	...
	public function prepare_item_for_response( $item, $request ) {
		...
		if ( $request->is_method( 'HEAD' ) ) {
			/** This filter is documented in zc-includes/rest-api/endpoints/class-zc-rest-comments-controller.php */
			return apply_filters( 'rest_prepare_comment', new ZC_REST_Response( array() ), $comment, $request );
		}
		 ...
		 * @param ZC_Comment        $comment  The original comment object.
	 ...
	 *
	 * @param ZC_REST_Request $request Request object.
	 * @return array|ZC_Error Prepared comment, otherwise ZC_Error object.
	 */
	protected function prepare_item_for_database( $request ) {
		...
		if ( isset( $request['author'] ) ) {
			$user = new ZC_User( $request['author'] );
		 ...
		 * @since 4.7.0
		 *
		 * @param array           $prepared_comment The prepared comment data for `zc_insert_comment`.
		 * @param ZC_REST_Request $request          The current request.
		 */
	...
	public function get_collection_params() {
		 ...
		 * collection parameter to an internal ZC_Comment_Query parameter. Use the
		 * `rest_comment_query` filter to set ZC_Comment_Query parameters.
		 *
		 * @since 4.7.0
		 *
	...
	protected function handle_status_param( $new_status, $comment_id ) {
		$old_status = zc_get_comment_status( $comment_id );
		...
		switch ( $new_status ) {
			...
			case 'approve':
			case '1':
				$changed = zc_set_comment_status( $comment_id, 'approve' );
				break;
			case 'hold':
			case '0':
				$changed = zc_set_comment_status( $comment_id, 'hold' );
				break;
			case 'spam':
				$changed = zc_spam_comment( $comment_id );
				break;
			case 'unspam':
				$changed = zc_unspam_comment( $comment_id );
				break;
			case 'trash':
				$changed = zc_trash_comment( $comment_id );
				break;
			case 'untrash':
				$changed = zc_untrash_comment( $comment_id );
				break;
			default:
				$changed = false;
				break;
	...
	protected function check_read_post_permission( $post, $request ) {
		...
		/*
		 * Ensure the posts controller is specifically a ZC_REST_Posts_Controller instance
		 * before using methods specific to that controller.
		 */
		if ( ! $posts_controller instanceof ZC_REST_Posts_Controller ) {
			$posts_controller = new ZC_REST_Posts_Controller( $post->post_type );
		}
	 ...
	 * @return string|ZC_Error The sanitized email address, if valid,
	 *                         otherwise an error.
	 */
	...
	protected function check_is_comment_content_allowed( $prepared_comment ) {
		...
		if (
			...
			isset( $check['meta']['_zc_note_status'] ) &&
			in_array( $check['meta']['_zc_note_status'], array( 'resolved', 'reopen' ), true )
		) {
			return true;
		}
		 ...
		 * comment_content. See zc_handle_comment_submission().
		 */
