<?php
/**
 * REST API: ZC_REST_Templates_Controller class
 *
 * @package    ZelocoreCMS
 * @subpackage REST_API
 * @since 5.8.0
 ...
 */
class ZC_REST_Templates_Controller extends ZC_REST_Controller {
	 ...
	 *
	 * @param ZC_REST_Request $request The request instance.
	 * @return ZC_REST_Response|ZC_Error
	 */
	 ...
	 *
	 * @since 5.9.0
	 * @link https://core.trac.zelocorecms.org/ticket/54507
	 *
	...
	public function get_items( $request ) {
		if ( $request->is_method( 'HEAD' ) ) {
			...
			return new ZC_REST_Response( array() );
		}

		$query = array();
		if ( isset( $request['zc_id'] ) ) {
			$query['zc_id'] = $request['zc_id'];
		}
	 ...
	 *
	 * @param ZC_REST_Request $request Full details about the request.
	 * @return true|ZC_Error True if the request has read access for the item, ZC_Error object otherwise.
	 */
	...
	public function get_item( $request ) {
		...
		if ( ! $template ) {
			return new ZC_Error( 'rest_template_not_found', __( 'No templates exist with that id.' ), array( 'status' => 404 ) );
		}
	 ...
	 *
	 * @param ZC_REST_Request $request Full details about the request.
	 * @return true|ZC_Error True if the request has write access for the item, ZC_Error object otherwise.
	 */
	...
	public function update_item( $request ) {
		...
		$post_before = get_post( $template->zc_id );

		if ( isset( $request['source'] ) && 'theme' === $request['source'] ) {
			zc_delete_post( $template->zc_id, true );
		...
		if ( is_zc_error( $changes ) ) {
			return $changes;
		}

		if ( 'custom' === $template->source ) {
			$update = true;
			$result = zc_update_post( zc_slash( (array) $changes ), false );
		} else {
			$update      = false;
			$post_before = null;
			$result      = zc_insert_post( zc_slash( (array) $changes ), false );
		}
	...
	public function create_item( $request ) {
		...
		$post_id                  = zc_insert_post( zc_slash( (array) $prepared_post ), true );
		...
		$posts = get_block_templates( array( 'zc_id' => $post_id ), $this->post_type );
		if ( ! count( $posts ) ) {
			return new ZC_Error( 'rest_template_insert_error', __( 'No templates exist with that id.' ), array( 'status' => 400 ) );
		}
		$id            = $posts[0]->id;
	 ...
	 *
	 * @param ZC_REST_Request $request Full details about the request.
	 * @return true|ZC_Error True if the request has delete access for the item, ZC_Error object otherwise.
	 */
	...
	public function delete_item( $request ) {
		...
		if ( 'custom' !== $template->source ) {
			return new ZC_Error( 'rest_invalid_template', __( 'Templates based on theme files can\'t be removed.' ), array( 'status' => 400 ) );
		}
	 ...
	 * @return stdClass|ZC_Error Changes to pass to zc_update_post.
	 */
	protected function prepare_item_for_database( $request ) {
		...
		if ( null === $template ) {
			...
			$changes->tax_input   = array(
				'zc_theme' => $request['theme'] ?? get_stylesheet(),
			);
		...
		if ( 'zc_template' === $this->post_type && isset( $request['is_zc_suggestion'] ) ) {
			$changes->meta_input     = zc_parse_args(
				array(
					'is_zc_suggestion' => $request['is_zc_suggestion'],
				),
				$changes->meta_input = array()
			);
		}

		if ( 'zc_template_part' === $this->post_type ) {
			if ( isset( $request['area'] ) ) {
				$changes->tax_input['zc_template_part_area'] = _filter_block_template_part_area( $request['area'] );
			} elseif ( null !== $template && 'custom' !== $template->source && $template->area ) {
				$changes->tax_input['zc_template_part_area'] = _filter_block_template_part_area( $template->area );
			} elseif ( empty( $template->area ) ) {
				$changes->tax_input['zc_template_part_area'] = ZC_TEMPLATE_PART_AREA_UNCATEGORIZED;
			}
		}
	 ...
	 *
	 * @param ZC_Block_Template $item    Template instance.
	 * @param ZC_REST_Request   $request Request object.
	...
	public function prepare_item_for_response( $item, $request ) {
		...
		if ( rest_is_field_included( 'title.rendered', $fields ) ) {
			if ( $template->zc_id ) {
				...
				$data['title']['rendered'] = apply_filters( 'the_title', $template->title, $template->zc_id );
			} else {
		...
		if ( rest_is_field_included( 'zc_id', $fields ) ) {
			$data['zc_id'] = (int) $template->zc_id;
		}
		...
		if ( rest_is_field_included( 'is_custom', $fields ) && 'zc_template' === $template->type ) {
		...
		if ( rest_is_field_included( 'area', $fields ) && 'zc_template_part' === $template->type ) {
		...
		if ( rest_is_field_included( 'author_text', $fields ) ) {
			$data['author_text'] = self::get_zc_templates_author_text_field( $template );
		}

		if ( rest_is_field_included( 'original_source', $fields ) ) {
			$data['original_source'] = self::get_zc_templates_original_source_field( $template );
		}
	 ...
	 * @since 6.5.0
	 *
	 * @param ZC_Block_Template $template_object Template instance.
	 * @return string                            Original source of the template one of theme, plugin, site, or user.
	 */
	private static function get_zc_templates_original_source_field( $template_object ) {
		if ( 'zc_template' === $template_object->type || 'zc_template_part' === $template_object->type ) {
			/*
			 * Added by theme.
			 ...
			 * was only added to templates in ZelocoreCMS 5.9. Fallback to showing the
			 * site logo and title.
			 */
	 ...
	 */
	private static function get_zc_templates_author_text_field( $template_object ) {
		$original_source = self::get_zc_templates_original_source_field( $template_object );
		switch ( $original_source ) {
			case 'theme':
				$theme_name = zc_get_theme( $template_object->theme )->get( 'Name' );
			...
			case 'plugin':
				...
				if ( isset( $template_object->plugin ) ) {
					$plugins = zc_get_active_and_valid_plugins();
	...
	protected function prepare_links( $id ) {
		...
		if ( post_type_supports( $this->post_type, 'revisions' ) ) {
			$template = get_block_template( $id, $this->post_type );
			if ( $template instanceof ZC_Block_Template && ! empty( $template->zc_id ) ) {
				$revisions       = zc_get_latest_revision_id_and_total_count( $template->zc_id );
