<?php
/**
 * REST API: ZC_REST_Menu_Items_Controller class
 *
 * @package ZelocoreCMS
 * @subpackage REST_API
 * @since 5.9.0
 ...
 */
class ZC_REST_Menu_Items_Controller extends ZC_REST_Posts_Controller {
	 ...
	 * @return object|ZC_Error Post object if ID is valid, ZC_Error otherwise.
	 */
	protected function get_nav_menu_item( $id ) {
		...
		return zc_setup_nav_menu_item( $post );
	}
	...
	protected function check_has_read_only_access( $request ) {
		 ...
		 *                                             via the REST API.
		 * @param ZC_REST_Request    $request          Full details about the request.
		 * @param ZC_REST_Controller $controller       The current instance of the controller.
		 */
	...
	public function create_item( $request ) {
		if ( ! empty( $request['id'] ) ) {
			return new ZC_Error( 'rest_post_exists', __( 'Cannot create existing post.' ), array( 'status' => 400 ) );
		}

		$prepared_nav_item = $this->prepare_item_for_database( $request );

		if ( is_zc_error( $prepared_nav_item ) ) {
			return $prepared_nav_item;
		}
		$prepared_nav_item = (array) $prepared_nav_item;

		$nav_menu_item_id = zc_update_nav_menu_item( $prepared_nav_item['menu-id'], $prepared_nav_item['menu-item-db-id'], zc_slash( $prepared_nav_item ), false );
		if ( is_zc_error( $nav_menu_item_id ) ) {
	...
	public function update_item( $request ) {
		...
		/** This action is documented in zc-includes/rest-api/endpoints/class-zc-rest-menu-items-controller.php */
	 ...
	 *
	 * @param ZC_REST_Request $request Full details about the request.
	 * @return ZC_REST_Response|ZC_Error True on success, or ZC_Error object on failure.
	 */
	public function delete_item( $request ) {
		$menu_item = $this->get_nav_menu_item( $request['id'] );
		if ( is_zc_error( $menu_item ) ) {
			return $menu_item;
		}
		...
		if ( ! $request['force'] ) {
			...
			return new ZC_Error( 'rest_trash_not_supported', sprintf( __( "Menu items do not support trashing. Set '%s' to delete." ), 'force=true' ), array( 'status' => 501 ) );
		}

		$previous = $this->prepare_item_for_response( get_post( $request['id'] ), $request );

		$result = zc_delete_post( $request['id'], true );

		if ( ! $result ) {
			return new ZC_Error( 'rest_cannot_delete', __( 'The post cannot be deleted.' ), array( 'status' => 500 ) );
		}
	 ...
	 * @param ZC_REST_Request $request Request object.
	 *
	 * @return object|ZC_Error
	 */
	protected function prepare_item_for_database( $request ) {
		$menu_item_db_id = $request['id'];
		$menu_item_obj   = $this->get_nav_menu_item( $menu_item_db_id );
		// Need to persist the menu item data. See https://core.trac.zelocorecms.org/ticket/28138
		if ( ! is_zc_error( $menu_item_obj ) ) {
			// Correct the menu position if this was the first item. See https://core.trac.zelocorecms.org/ticket/28140
		...
		if ( ! $prepared_nav_item['menu-item-object'] ) {
			...
			if ( 'taxonomy' === $prepared_nav_item['menu-item-type'] ) {
				...
				if ( empty( $original ) || is_zc_error( $original ) ) {
		...
		// The xfn and classes properties are arrays, but passed to zc_update_nav_menu_item as a string.
	...
	public function prepare_item_for_response( $item, $request ) {
		...
		foreach ( $taxonomies as $taxonomy ) {
			...
			if ( rest_is_field_included( $base, $fields ) ) {
				...
				$term_ids = $terms ? array_values( zc_list_pluck( $terms, 'term_id' ) ) : array();
		 ...
		 * @param object           $menu_item Menu item setup by {@see zc_setup_nav_menu_item()}.
		 * @param ZC_REST_Request  $request   Request object.
		 */
		...
		$schema['properties']['classes'] = array(
			...
			'arg_options' => array(
				'sanitize_callback' => static function ( $value ) {
					return array_map( 'sanitize_html_class', zc_parse_list( $value ) );
				},
			),
		);
	...
	protected function get_menu_id( $menu_item_id ) {
		$menu_ids = zc_get_post_terms( $menu_item_id, 'nav_menu', array( 'fields' => 'ids' ) );
		$menu_id  = 0;
		if ( $menu_ids && ! is_zc_error( $menu_ids ) ) {
