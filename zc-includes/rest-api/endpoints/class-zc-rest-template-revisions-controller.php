<?php
/**
 * REST API: ZC_REST_Template_Revisions_Controller class
 *
 * @package ZelocoreCMS
 * @subpackage REST_API
 * @since 6.4.0
 ...
 */
class ZC_REST_Template_Revisions_Controller extends ZC_REST_Revisions_Controller {
	/**
	 * Parent post type.
	 *
	 * @since 6.4.0
	...
	protected function get_parent( $parent_template_id ) {
		...
		$parent_post_id = isset( $template->zc_id ) ? (int) $template->zc_id : 0;

		if ( $parent_post_id <= 0 ) {
			return new ZC_Error(
				'rest_invalid_template',
				__( 'Templates based on theme files can\'t have revisions.' ),
				array( 'status' => ZC_Http::BAD_REQUEST )
			);
		}

		return get_post( $template->zc_id );
	}
