 ...
 * @since 6.5.0
 */
class ZC_REST_Font_Collections_Controller extends ZC_REST_Controller {
	...
	public function get_items( $request ) {
		$collections_all = ZC_Font_Library::get_instance()->get_font_collections();
		...
		$response = $is_head_request ? new ZC_REST_Response( array() ) : rest_ensure_response( $items );

		$response->header( 'X-ZC-Total', (int) $total_items );
	...
	public function get_item( $request ) {
		...
		$collection = ZC_Font_Library::get_instance()->get_font_collection( $slug );

		if ( ! $collection ) {
			return new ZC_Error( 'rest_font_collection_not_found', __( 'Font collection not found.' ), array( 'status' => 404 ) );
		}
	...
	* @since 6.5.0
	*
	* @param ZC_Font_Collection $item    Font collection object.
	...
	*/
	public function prepare_item_for_response( $item, $request ) {
		...
		if ( ! empty( array_intersect( $fields, $data_fields ) ) ) {
			...
			if ( is_zc_error( $collection_data ) ) {
			 ...
			 * Can't exit at the beginning of the method due to the potential need to return a ZC_Error object.
			 */
			if ( $request->is_method( 'HEAD' ) ) {
				/** This filter is documented in zc-includes/rest-api/endpoints/class-zc-rest-font-collections-controller.php */
				return apply_filters( 'rest_prepare_font_collection', new ZC_REST_Response( array() ), $item, $request );
			}
		 ...
		 * @param ZC_Font_Collection $item     The font collection object.
	 ...
	 * @since 6.5.0
	 *
	 * @param ZC_Font_Collection $collection Font collection data
