<?php
/**
 * REST API: ZC_REST_Sidebars_Controller class
 *
 * Original code from {@link https://github.com/martin-pettersson/zc-rest-api-sidebars Martin Pettersson (martin_pettersson@outlook.com)}.
 *
 * @package ZelocoreCMS
 * @subpackage REST_API
 * @since 5.8.0
 ...
 */
class ZC_REST_Sidebars_Controller extends ZC_REST_Controller {
	...
	public function get_items_permissions_check( $request ) {
		...
		foreach ( zc_get_sidebars_widgets() as $id => $widgets ) {
	 ...
	 *
	 * @param ZC_REST_Request $request Full details about the request.
	 * @return ZC_REST_Response Response object on success.
	 */
	public function get_items( $request ) {
		...
		foreach ( zc_get_sidebars_widgets() as $id => $widgets ) {
			...
			if ( is_zc_error( $permissions_check ) && ! $this->check_read_permission( $sidebar ) ) {
				continue;
			}
	...
	public function get_item( $request ) {
		...
		$sidebar = $this->get_sidebar( $request['id'] );
		if ( ! $sidebar ) {
			return new ZC_Error( 'rest_sidebar_not_found', __( 'No sidebar exists with that id.' ), array( 'status' => 404 ) );
		}
	 ...
	 *
	 * @param ZC_REST_Request $request Full details about the request.
	 * @return ZC_REST_Response Response object on success, or ZC_Error object on failure.
	 */
		...
		if ( isset( $request['widgets'] ) ) {
			...
			foreach ( $sidebars as $sidebar_id => $widgets ) {
				foreach ( $widgets as $i => $widget_id ) {
					...
					if ( $sidebar_id === $request['id'] && ! in_array( $widget_id, $request['widgets'], true ) ) {
						$sidebars['zc_inactive_widgets'][] = $widget_id;
					}
				}
			}
	 ...
	 * @param ZC_REST_Request $request Full details about the request.
	 * @return ZC_REST_Response Prepared response object.
	 */
	public function prepare_item_for_response( $item, $request ) {
		...
		if ( $request->is_method( 'HEAD' ) ) {
			/** This filter is documented in zc-includes/rest-api/endpoints/class-zc-rest-sidebars-controller.php */
			return apply_filters( 'rest_prepare_sidebar', new ZC_REST_Response( array() ), $raw_sidebar, $request );
		}

		$id      = $raw_sidebar['id'];
		$sidebar = array( 'id' => $id );

		if ( isset( $zc_registered_sidebars[ $id ] ) ) {
			$registered_sidebar = $zc_registered_sidebars[ $id ];

			$sidebar['status']        = 'active';
			$sidebar['name']          = $registered_sidebar['name'] ?? '';
			$sidebar['description']   = isset( $registered_sidebar['description'] ) ? zc_sidebar_description( $id ) : '';
		...
		if ( rest_is_field_included( 'widgets', $fields ) ) {
			...
			$widgets  = array_filter(
				...
				static function ( $widget_id ) use ( $zc_registered_widgets ) {
					return isset( $zc_registered_widgets[ $widget_id ] );
				}
			);
