<?php
/**
 * REST API: ZC_REST_Revisions_Controller class
 *
 * @package ZelocoreCMS
 * @subpackage REST_API
 * @since 4.7.0
 ...
 */
class ZC_REST_Revisions_Controller extends ZC_REST_Controller {
	 ...
	 * @since 4.7.0
	 *
	 * @see ZC_REST_Posts_Controller::get_items()
	 *
	 * @param ZC_REST_Request $request Full details about the request.
	...
	public function get_items( $request ) {
		...
		if ( zc_revisions_enabled( $parent ) ) {
			...
			/**
			 * Filters ZC_Query arguments when querying revisions via the REST API.
			 *
			 * Serves the same purpose as the {@see 'rest_{$this->post_type}_query'} filter in
			 * ZC_REST_Posts_Controller, but for the standalone ZC_REST_Revisions_Controller.
			 *
			 * @since 5.0.0
			 *
		...
		if ( ! $is_head_request ) {
			...
			$response = new ZC_REST_Response( array() );
		}

		$response->header( 'X-ZC-Total', (int) $total_revisions );
	...
	public function delete_item_permissions_check( $request ) {
		...
		if ( ! $response || is_zc_error( $response ) ) {
			return $response;
		}
	...
	public function delete_item( $request ) {
		 ...
		 * @since 4.7.0
		 *
		 * @param ZC_Post|false|null $result The revision object (if it was deleted or moved to the Trash successfully)
	...
	public function prepare_item_for_response( $item, $request ) {
		...
		if ( $request->is_method( 'HEAD' ) ) {
			/** This filter is documented in zc-includes/rest-api/endpoints/class-zc-rest-revisions-controller.php */
			return apply_filters( 'rest_prepare_revision', new ZC_REST_Response( array() ), $post, $request );
		}
	 ...
	 * @param ZC_Post $post    Post revision object.
