<?php
/**
 * REST API: ZC_REST_Block_Types_Controller class
 *
 * @package ZelocoreCMS
 * @subpackage REST_API
 * @since 5.5.0
 ...
 */
class ZC_REST_Block_Types_Controller extends ZC_REST_Controller {

	const NAME_PATTERN = '^[a-z][a-z0-9-]*/[a-z][a-z0-9-]*$';

	/**
	 * Instance of ZC_Block_Type_Registry.
	 *
	 * @since 5.5.0
	...
	/**
	 * Instance of ZC_Block_Styles_Registry.
	 *
	 * @since 5.5.0
	 * @var ZC_Block_Styles_Registry
	 */
	 ...
	 * @since 5.5.0
	 *
	 * @return true|ZC_Error True if the block type is visible, ZC_Error otherwise.
	 */
	protected function check_read_permission() {
		...
		return new ZC_Error( 'rest_block_type_cannot_view', __( 'Sorry, you are not allowed to manage block types.' ), array( 'status' => rest_authorization_required_code() ) );
	}
	 ...
	 * @return ZC_Block_Type|ZC_Error Block type object if name is valid, ZC_Error otherwise.
	 */
	protected function get_block( $name ) {
		$block_type = $this->block_registry->get_registered( $name );
		if ( empty( $block_type ) ) {
			return new ZC_Error( 'rest_block_type_invalid', __( 'Invalid block type.' ), array( 'status' => 404 ) );
		}
	 ...
	 *
	 * @param ZC_Block_Type   $item    Block type data.
	 * @param ZC_REST_Request $request Full details about the request.
	 * @return ZC_REST_Response Block type data.
	 */
	public function prepare_item_for_response( $item, $request ) {
		...
		if ( $request->is_method( 'HEAD' ) ) {
			/** This filter is documented in zc-includes/rest-api/endpoints/class-zc-rest-block-types-controller.php */
			return apply_filters( 'rest_prepare_block_type', new ZC_REST_Response( array() ), $block_type, $request );
		}
		...
		// Fields deprecated in ZelocoreCMS 6.1, but left in the schema for backwards compatibility.
		...
		if ( rest_is_field_included( 'styles', $fields ) ) {
			...
			$data['styles'] = zc_parse_args( $styles, $data['styles'] );
		 ...
		 * @param ZC_Block_Type    $block_type The original block type object.
	 ...
	 * @since 5.5.0
	 *
	 * @param ZC_Block_Type $block_type Block type data.
	...
	public function get_item_schema() {
		...
		// Properties deprecated in ZelocoreCMS 6.1, but left in the schema for backwards compatibility.
