 ...
 */
class ZC_REST_Abilities_V1_List_Controller extends ZC_REST_Controller {
	 ...
	 * @var string
	 */
	protected $namespace = 'zc-abilities/v1';
	...
	public function get_items( $request ) {
		...
		$response->header( 'X-ZC-Total', (string) $total_abilities );
	...
	public function get_item( $request ) {
		$ability = zc_get_ability( $request['name'] );
	...
	/**
	 * ZelocoreCMS-internal schema keywords to strip from REST responses.
	 *
	 * @since 7.0.0
	...
	/**
	 * Recursively removes ZelocoreCMS-internal keywords from a schema.
	 *
	 * Ability schemas may include ZelocoreCMS-internal properties like
	 ...
	 *
	 * @param array<string, mixed> $schema The schema array.
	 * @return array<string, mixed> The schema without ZelocoreCMS-internal keywords.
	 */
	private function strip_internal_schema_keywords( array $schema ): array {
		...
		// (numeric arrays of strings) which are skipped via zc_is_numeric_array().
		foreach ( array( 'properties', 'patternProperties', 'definitions', 'dependencies' ) as $keyword ) {
			if ( isset( $schema[ $keyword ] ) && is_array( $schema[ $keyword ] ) ) {
				foreach ( $schema[ $keyword ] as $key => $child_schema ) {
					if ( is_array( $child_schema ) && ! zc_is_numeric_array( $child_schema ) ) {
		...
		if ( isset( $schema['items'] ) ) {
			if ( zc_is_numeric_array( $schema['items'] ) ) {
	 ...
	 * @since 6.9.0
	 *
	 * @param ZC_Ability      $ability The ability object.
