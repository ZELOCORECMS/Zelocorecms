<?php
/**
 * REST API: ZC_REST_Block_Directory_Controller class
 *
 * @package ZelocoreCMS
 * @subpackage REST_API
 * @since 5.5.0
 ...
 */
class ZC_REST_Block_Directory_Controller extends ZC_REST_Controller {
	...
	public function prepare_item_for_response( $item, $request ) {
		...
		$block = array(
			...
			'description'         => zc_trim_words( $plugin['short_description'], 30, '...' ),
			'id'                  => $plugin['slug'],
			...
			'author'              => zc_strip_all_tags( $plugin['author'] ),
		...
		$response = new ZC_REST_Response( $block );
	 ...
	 * @since 5.5.0
	 *
	 * @param array $plugin The plugin data from ZelocoreCMS.org.
	 * @return array
	 */
	 ...
	 * @since 5.5.0
	 *
	 * @param string $slug The ZelocoreCMS.org directory slug for a plugin.
	...
	public function get_item_schema() {
		...
		$this->schema = array(
			...
			'properties' => array(
				...
				'author'              => array(
					'description' => __( 'The ZelocoreCMS.org username of the block author.' ),
					'type'        => 'string',
					'context'     => array( 'view' ),
				),
				'icon'                => array(
