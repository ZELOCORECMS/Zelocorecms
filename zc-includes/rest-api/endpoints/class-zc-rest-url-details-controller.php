<?php
/**
 * REST API: ZC_REST_URL_Details_Controller class
 *
 * @package ZelocoreCMS
 * @subpackage REST_API
 * @since 5.9.0
 ...
 */
class ZC_REST_URL_Details_Controller extends ZC_REST_Controller {
	...
	public function __construct() {
		$this->namespace = 'zc-block-editor/v1';
				...
				array(
					...
					'args'                => array(
						'url' => array(
							...
							'validate_callback' => 'zc_http_validate_url',
	 ...
	 * @return ZC_REST_Response|ZC_Error The parsed details as a response object. ZC_Error if there are errors.
	 */
	public function parse_url_details( $request ) {
		$url = untrailingslashit( $request['url'] );

		if ( empty( $url ) ) {
			return new ZC_Error( 'rest_invalid_url', __( 'Invalid URL' ), array( 'status' => 404 ) );
		}
		...
		if ( ! empty( $cached_response ) ) {
			...
			if ( is_zc_error( $remote_url_response ) || empty( $remote_url_response ) ) {
	 ...
	 * @since 5.9.0
	 *
	 * @return true|ZC_Error True if the request has permission, else ZC_Error.
	 */
	 ...
	 * @return string|ZC_Error The HTTP response from the remote URL on success.
	 *                         ZC_Error if no response or no content.
	 */
	private function get_remote_url( $url ) {
		...
		*/
		$modified_user_agent = 'ZC-URLDetails/' . get_bloginfo( 'version' ) . ' (+' . get_bloginfo( 'url' ) . ')';
		 ...
		 *
		 * Can be used to adjust response size limit and other ZC_Http::request() args.
		 *
		 * @since 5.9.0
		 *
		...
		$response = zc_safe_remote_get( $url, $args );

		if ( ZC_Http::OK !== zc_remote_retrieve_response_code( $response ) ) {
			// Not saving the error response to cache since the error might be temporary.
			return new ZC_Error(
				'no_response',
				__( 'URL not found. Response returned a non-200 status code for this URL.' ),
				array( 'status' => ZC_Http::NOT_FOUND )
			);
		}

		$remote_body = zc_remote_retrieve_body( $response );
	...
	private function get_icon( $html, $url ) {
		...
		if ( isset( $parsed_url['scheme'] ) && isset( $parsed_url['host'] ) ) {
			...
			$icon     = ZC_Http::make_absolute_url( $icon, $root_url );
		}
	...
	private function get_image( $meta_elements, $url ) {
		...
		if ( isset( $parsed_url['scheme'] ) && isset( $parsed_url['host'] ) ) {
			$root_url = $parsed_url['scheme'] . '://' . $parsed_url['host'] . '/';
			$image    = ZC_Http::make_absolute_url( $image, $root_url );
		}
	...
	private function prepare_metadata_for_output( $metadata ) {
		...
		$metadata = zc_strip_all_tags( $metadata );
		return $metadata;
	}
