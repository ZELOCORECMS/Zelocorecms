<?php
/**
 * REST API: ZC_REST_Widgets_Controller class
 *
 * @package ZelocoreCMS
 * @subpackage REST_API
 * @since 5.8.0
 ...
 */
class ZC_REST_Widgets_Controller extends ZC_REST_Controller {
	...
	public function get_items( $request ) {
		...
		foreach ( zc_get_sidebars_widgets() as $sidebar_id => $widget_ids ) {
			...
			if ( is_zc_error( $permissions_check ) && ! $this->check_read_sidebar_permission( $sidebar_id ) ) {
				continue;
			}
		...
		return new ZC_REST_Response( $prepared );
	}
	...
	public function get_item_permissions_check( $request ) {
		...
		$sidebar_id = zc_find_widgets_sidebar( $widget_id );
	...
	protected function check_read_sidebar_permission( $sidebar_id ) {
		$sidebar = zc_get_sidebar( $sidebar_id );
	...
	public function create_item( $request ) {
		...
		if ( is_zc_error( $widget_id ) ) {
			return $widget_id;
		}

		zc_assign_widget_to_sidebar( $widget_id, $sidebar_id );
	...
	public function update_item( $request ) {
		 ...
		 * zc_inactive_widgets sidebar based on the contents of the $sidebars_widgets global.
		 *
		 * When batch requests are processed, this global is not properly updated by previous
		 * calls, resulting in widgets incorrectly being moved to the zc_inactive_widgets
		 * sidebar.
		 *
		 * See https://core.trac.zelocorecms.org/ticket/53657.
		 */
		...
		// Allow sidebar to be unset or missing when widget is not a ZC_Widget.
		$parsed_id     = zc_parse_widget_id( $widget_id );
		$widget_object = $zc_widget_factory->get_widget_object( $parsed_id['id_base'] );
	 ...
	 *
	 * @global ZC_Widget_Factory $zc_widget_factory
	 * @global array             $zc_registered_widget_updates The registered widget update functions.
	 *
	 * @param ZC_REST_Request $request Full details about the request.
	 * @return ZC_REST_Response|ZC_Error Response object on success, or ZC_Error object on failure.
	 */
	public function delete_item( $request ) {
		global $zc_widget_factory, $zc_registered_widget_updates;
		...
		if ( $request['force'] ) {
			...
			$callback = $zc_registered_widget_updates[ $id_base ]['callback'];
			$params   = $zc_registered_widget_updates[ $id_base ]['params'];
			...
			if ( $widget_object ) {
				/*
				 * ZC_Widget sets `updated = true` after an update to prevent more than one widget
			...
			zc_assign_widget_to_sidebar( $widget_id, '' );
			...
			zc_assign_widget_to_sidebar( $widget_id, 'zc_inactive_widgets' );

			$response = $this->prepare_item_for_response(
				array(
					'sidebar_id' => 'zc_inactive_widgets',
					'widget_id'  => $widget_id,
				),
				$request
			);
		 ...
		 * @param ZC_REST_Response|ZC_Error $response   The response data, or ZC_Error object on failure.
	 ...
	 * @return string|ZC_Error The saved widget ID.
	 */
	protected function save_widget( $request, $sidebar_id ) {
		...
		if ( isset( $request['id'] ) ) {
			...
			$id            = $request['id'];
			$parsed_id     = zc_parse_widget_id( $id );
			$id_base       = $parsed_id['id_base'];
		...
		if ( isset( $request['instance'] ) ) {
			if ( ! $widget_object ) {
				return new ZC_Error(
					'rest_invalid_widget',
					__( 'Cannot set instance on a widget that does not extend ZC_Widget.' ),
					array( 'status' => 400 )
				);
			}
			...
			if ( isset( $request['instance']['raw'] ) ) {
				...
				if ( ! hash_equals( zc_hash( $serialized_instance ), $request['instance']['hash'] ) ) {
					return new ZC_Error(
						'rest_invalid_widget',
	...
	public function prepare_item_for_response( $item, $request ) {
		global $zc_widget_factory, $zc_registered_widgets;
		...
		$widget = $zc_registered_widgets[ $widget_id ];
		// Don't prepare the response body for HEAD requests.
		if ( $request->is_method( 'HEAD' ) ) {
			/** This filter is documented in zc-includes/rest-api/endpoints/class-zc-rest-widgets-controller.php */
			return apply_filters( 'rest_prepare_widget', new ZC_REST_Response( array() ), $widget, $request );
		}
		...
		if (
			...
			'zc_inactive_widgets' !== $sidebar_id
		) {
			$prepared['rendered'] = trim( zc_render_widget( $widget_id, $sidebar_id ) );
		}

		if ( rest_is_field_included( 'rendered_form', $fields ) ) {
			$rendered_form = zc_render_widget_control( $widget_id );
		...
		if ( rest_is_field_included( 'instance', $fields ) ) {
			...
			if ( $widget_object && isset( $parsed_id['number'] ) ) {
				...
				$prepared['instance']['hash']    = zc_hash( $serialized_instance );
		 ...
		 * @since 5.8.0
		 *
		 * @param ZC_REST_Response|ZC_Error $response The response object, or ZC_Error object on failure.
	...
	protected function prepare_links( $prepared ) {
		...
		return array(
			...
			'about'                     => array(
				'href'       => rest_url( sprintf( 'zc/v2/widget-types/%s', $id_base ) ),
				'embeddable' => true,
			),
			'https://api.w.org/sidebar' => array(
				'href' => rest_url( sprintf( 'zc/v2/sidebars/%s/', $prepared['sidebar'] ) ),
			),
		);
	}
