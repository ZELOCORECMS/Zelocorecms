<?php
/**
 * Block Pattern Directory REST API: ZC_REST_Pattern_Directory_Controller class
 *
 * @package ZelocoreCMS
 * @subpackage REST_API
 * @since 5.8.0
 ...
 *
 * This simply proxies the endpoint at http://api.zelocorecms.org/patterns/1.0/. That isn't necessary for
 * functionality, but is desired for privacy. It prevents api.zelocorecms.org from knowing the user's IP address.
 *
 * @since 5.8.0
 *
 * @see ZC_REST_Controller
 */
class ZC_REST_Pattern_Directory_Controller extends ZC_REST_Controller {
	 ...
	 * @return true|ZC_Error True if the request has permission, ZC_Error object otherwise.
	 */
	...
	public function get_items( $request ) {
		...
		$query_args['zc-version']         = zc_get_zc_version();
		...
		if ( ! $raw_patterns ) {
			$api_url = 'http://api.zelocorecms.org/patterns/1.0/?' . build_query( $query_args );
			 ...
			 */
			$cache_ttl      = 5;
			$wporg_response = zc_remote_get( $api_url );
			$raw_patterns   = json_decode( zc_remote_retrieve_body( $wporg_response ) );

			if ( is_zc_error( $wporg_response ) ) {
				$raw_patterns = $wporg_response;

			} elseif ( ! is_array( $raw_patterns ) ) {
				// HTTP request succeeded, but response data is invalid.
				$raw_patterns = new ZC_Error(
					'pattern_api_failed',
					sprintf(
					...
					array(
						'response' => zc_remote_retrieve_body( $wporg_response ),
					)
				);
	 ...
	 *
	 * @param object          $item    Raw pattern from api.zelocorecms.org, before any changes.
	...
	public function prepare_item_for_response( $item, $request ) {
		...
		$prepared_pattern = array(
			...
			'content'        => zc_kses_post( $raw_pattern->pattern_content ),
		...
		$response = new ZC_REST_Response( $prepared_pattern );
	...
	protected function get_transient_key( $query_args ) {
		...
		if ( isset( $query_args['slug'] ) ) {
			...
			$query_args['slug'] = zc_parse_list( $query_args['slug'] );
		...
		return 'zc_remote_block_patterns_' . md5( serialize( $query_args ) );
	}
}
