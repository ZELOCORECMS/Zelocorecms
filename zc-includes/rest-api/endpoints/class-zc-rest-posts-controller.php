<?php
/**
 * REST API: ZC_REST_Posts_Controller class
 *
 * @package ZelocoreCMS
 * @subpackage REST_API
 * @since 4.7.0
 ...
 * @since 4.7.0
 *
 * @see ZC_REST_Controller
 */
class ZC_REST_Posts_Controller extends ZC_REST_Controller {
	/**
	 * Post type.
	 *
	 * @since 4.7.0
	...
	public function register_routes() {
		...
		register_rest_route(
			...
			array(
				...
				array(
					'methods'             => ZC_REST_Server::CREATABLE,
					'callback'            => array( $this, 'create_item' ),
					'permission_callback' => array( $this, 'create_item_permissions_check' ),
					'args'                => $this->get_endpoint_args_for_item_schema( ZC_REST_Server::CREATABLE ),
				),
		...
		register_rest_route(
			...
			array(
				...
				array(
					...
					'args'                => $this->get_endpoint_args_for_item_schema( ZC_REST_Server::EDITABLE ),
				),
				array(
					'methods'             => ZC_REST_Server::DELETABLE,
	 ...
	 * @since 4.7.0
	 *
	 * @param ZC_REST_Request $request Full details about the request.
	 * @return true|ZC_Error True if the request has read access, ZC_Error object otherwise.
	 */
	 ...
	 * @param ZC_Post $post     The post been password checked.
	 ...
	 *
	 * @param ZC_REST_Request $request Full details about the request.
	 * @return ZC_REST_Response|ZC_Error Response object on success, or ZC_Error object on failure.
	 */
	public function get_items( $request ) {
		 ...
		 * values are accepted as-passed, and their internal ZC_Query parameter
		 ...
		 */
		foreach ( $parameter_mappings as $api_param => $zc_param ) {
			if ( isset( $registered[ $api_param ], $request[ $api_param ] ) ) {
				$args[ $zc_param ] = $request[ $api_param ];
			}
		}
		...
		if ( isset( $registered['sticky'], $request['sticky'] ) ) {
			...
			if ( $request['sticky'] ) {
				 ...
				 * ZC_Query won't return "no posts" for post__in = array()
		...
		/**
		 * Filters ZC_Query arguments when querying posts via the REST API.
		 *
		 ...
		 *
		 * @param array           $args    Array of arguments for ZC_Query.
		 * @param ZC_REST_Request $request The REST API request.
		 */
		...
		$response = $is_head_request ? new ZC_REST_Response( array() ) : rest_ensure_response( $posts );

		$response->header( 'X-ZC-Total', (int) $total_posts );
		$response->header( 'X-ZC-TotalPages', (int) $max_pages );
	 ...
	 * @return ZC_Post|ZC_Error Post object if ID is valid, ZC_Error otherwise.
	 */
	 ...
	 *
	 * @param ZC_REST_Request $request Full details about the request.
	 * @return bool|ZC_Error True if the request has read access for the item, ZC_Error object or false otherwise.
	 */
	 ...
	 * @since 4.7.0
	 *
	 * @param ZC_Post         $post    Post to check against.
	 * @param ZC_REST_Request $request Request data to check.
	 ...
	 *
	 * @param ZC_REST_Request $request Full details about the request.
	 * @return true|ZC_Error True if the request has access to create items, ZC_Error object otherwise.
	 */
	...
	public function create_item( $request ) {
		...
		if ( ! empty( $prepared_post->post_name )
			...
			/*
			 * `zc_unique_post_slug()` returns the same slug for 'draft' or 'pending' posts.
			 *
			 * To ensure that a unique slug is generated, pass the post data with the 'publish' status.
			 */
			$prepared_post->post_name = zc_unique_post_slug(
		...
		$post_id = zc_insert_post( zc_slash( (array) $prepared_post ), true, false );
		 ...
		 * @since 4.7.0
		 *
		 * @param ZC_Post         $post     Inserted or updated post object.
		 * @param ZC_REST_Request $request  Request object.
	 ...
	 *
	 * @param ZC_REST_Request $request Full details about the request.
	 * @return true|ZC_Error True if the request has access to update the item, ZC_Error object otherwise.
	 */
	...
	public function update_item( $request ) {
		...
		if ( ! empty( $post->post_name ) && in_array( $post_status, array( 'draft', 'pending' ), true ) ) {
			...
			$post->post_name = zc_unique_post_slug(
				$post->post_name,
				$post->ID,
				'publish',
				$post->post_type,
		...
		// Convert the post object to an array, otherwise zc_update_post() will expect non-escaped input.
		$post_id = zc_update_post( zc_slash( (array) $post ), true, false );
		...
		/** This action is documented in zc-includes/rest-api/endpoints/class-zc-rest-posts-controller.php */
		...
		// Filter is fired in ZC_REST_Attachments_Controller subclass.
	 ...
	 *
	 * @param ZC_REST_Request $request Full details about the request.
	 * @return true|ZC_Error True if the request has access to delete the item, ZC_Error object otherwise.
	 */
	...
	public function delete_item( $request ) {
		 ...
		 * @param ZC_Post $post           The Post object being considered for trashing support.
		 */
		...
		if ( $force ) {
			...
			/*
			 * (Note that internally this falls through to `zc_delete_post()`
			 * if the Trash is disabled.)
			 */
			$result   = zc_trash_post( $id );
			$post     = get_post( $id );
			$response = $this->prepare_item_for_response( $post, $request );
		}
		 ...
		 * @since 4.7.0
		 *
		 * @param ZC_Post          $post     The deleted or trashed post.
		 * @param ZC_REST_Response $response The response data.
		 * @param ZC_REST_Request  $request  The request sent to the API.
		 */
	 ...
	 * @since 4.7.0
	 *
	 * @param array           $prepared_args Optional. Prepared ZC_Query arguments. Default empty array.
	 * @param ZC_REST_Request $request       Optional. Full details about the request.
	...
	protected function prepare_items_query( $prepared_args = array(), $request = null ) {
		...
		foreach ( $prepared_args as $key => $value ) {
			 ...
			 */
			$query_args[ $key ] = apply_filters( "rest_query_var-{$key}", $value ); // phpcs:ignore ZelocoreCMS.NamingConventions.ValidHookName.UseUnderscores
		}
		...
		// Map to proper ZC_Query orderby param.
	 ...
	 *
	 * @param ZC_REST_Request $request Request object.
	 * @return stdClass|ZC_Error Post object or ZC_Error.
	 */
	protected function prepare_item_for_database( $request ) {
		 ...
		 * @since 7.0.0
		 */
		$content_like_post_types = array( 'post', 'page', 'zc_block', 'zc_navigation' );
	 ...
	 * @param ZC_REST_Request $request The request object.
	 * @param string          $param   The parameter name.
	 * @return true|ZC_Error True if the status is valid, or ZC_Error if not.
	 */
	public function check_status( $status, $request, $param ) {
		if ( $request['id'] ) {
			$post = $this->get_post( $request['id'] );

			if ( ! is_zc_error( $post ) && $post->post_status === $status ) {
				return true;
			}
		}
	 ...
	 * @param ZC_Post_Type $post_type   Post type.
	 * @return string|ZC_Error Post status or ZC_Error if lacking the proper permission.
	 */
	 ...
	 * @return bool|ZC_Error Whether the post thumbnail was successfully deleted, otherwise ZC_Error.
	 */
	 ...
	 * @param ZC_REST_Request $request  Request.
	 * @return true|ZC_Error True if template is still valid or if the same as existing value, or a ZC_Error if template not supported.
	 */
	...
	public function handle_template( $template, $post_id, $validate = false ) {
		...
		if ( $validate && ! array_key_exists( $template, zc_get_theme()->get_page_templates( get_post( $post_id ) ) ) ) {
			$template = '';
		}

		update_post_meta( $post_id, '_zc_page_template', $template );
	}
	 ...
	 * @param ZC_REST_Request $request The request object with post and terms data.
	 * @return null|ZC_Error ZC_Error on an error assigning any of the terms, otherwise null.
	 */
	protected function handle_terms( $post_id, $request ) {
		$taxonomies = zc_list_filter( get_object_taxonomies( $this->post_type, 'objects' ), array( 'show_in_rest' => true ) );
		...
		foreach ( $taxonomies as $taxonomy ) {
			...
			$result = zc_set_object_terms( $post_id, $request[ $base ], $taxonomy->name );
	 ...
	 * @since 4.7.0
	 *
	 * @param ZC_Post_Type|string $post_type Post type name or object.
	 ...
	 *
	 * @param ZC_Post         $item    Post object.
	 * @param ZC_REST_Request $request Request object.
	 * @return ZC_REST_Response Response object.
	 */
	public function prepare_item_for_response( $item, $request ) {
		...
		if ( $request->is_method( 'HEAD' ) ) {
			/** This filter is documented in zc-includes/rest-api/endpoints/class-zc-rest-posts-controller.php */
			return apply_filters( "rest_prepare_{$this->post_type}", new ZC_REST_Response( array() ), $post, $request );
		}
		...
		foreach ( $taxonomies as $taxonomy ) {
			...
			if ( rest_is_field_included( $base, $fields ) ) {
				...
				$data[ $base ] = $terms ? array_values( zc_list_pluck( $terms, 'term_id' ) ) : array();
			}
		}
	 ...
	 *
	 * By default, ZelocoreCMS will show password protected or private posts with a title of
	...
	protected function prepare_links( $post ) {
		...
		if ( in_array( $post->post_type, array( 'post', 'page' ), true ) || post_type_supports( $post->post_type, 'revisions' ) ) {
			...
			$revisions_count = ! is_zc_error( $revisions ) ? $revisions['count'] : 0;
	...
	public function get_collection_params() {
		 ...
		 * collection parameter to an internal ZC_Query parameter. Use the
		 * `rest_{$this->post_type}_query` filter to set ZC_Query parameters.
		 *
		 * @since 4.7.0
		 *
	 ...
	 * @return array|ZC_Error A list of valid statuses, otherwise ZC_Error object.
	 */
	public function sanitize_post_statuses( $statuses, $request, $parameter ) {
		$statuses = zc_parse_slug_list( $statuses );

		// The default status is different in ZC_REST_Attachments_Controller.
	 ...
	 * @since 5.7.0
	 *
	 * @param array           $args    ZC_Query arguments.
	 * @param ZC_REST_Request $request Full details about the request.
	 * @return array Updated query arguments.
	 */
	private function prepare_tax_query( array $args, ZC_REST_Request $request ) {
