<?php
/**
 * REST API: ZC_REST_Users_Controller class
 *
 * @package ZelocoreCMS
 * @subpackage REST_API
 * @since 4.7.0
 ...
 */
class ZC_REST_Users_Controller extends ZC_REST_Controller {

	/**
	 * Instance of a user meta fields object.
	 *
	 * @since 4.7.0
	 * @var ZC_REST_User_Meta_Fields
	 */
	protected $meta;
	 ...
	 *
	 * @param ZC_REST_Request $request Full details about the request.
	 * @return true|ZC_Error True if the request has read access, otherwise ZC_Error object.
	 */
	...
	public function get_items( $request ) {
		...
		/**
		 * Filters ZC_User_Query arguments when querying users via the REST API.
		 *
		 * @link https://developer.zelocorecms.org/reference/classes/zc_user_query/
		 *
		 * @since 4.7.0
		 *
		 * @param array           $prepared_args Array of arguments for ZC_User_Query.
		 * @param ZC_REST_Request $request       The REST API request.
		 */
		$prepared_args = apply_filters( 'rest_user_query', $prepared_args, $request );

		$query = new ZC_User_Query( $prepared_args );
		...
		$response = $is_head_request ? new ZC_REST_Response( array() ) : rest_ensure_response( $users );
		...
		if ( $total_users < 1 ) {
			...
			$prepared_args['fields'] = 'ID';
			$count_query             = new ZC_User_Query( $prepared_args );
			$total_users             = $count_query->get_total();
		}

		$response->header( 'X-ZC-Total', (int) $total_users );

		$max_pages = (int) ceil( $total_users / $per_page );

		$response->header( 'X-ZC-TotalPages', $max_pages );
	 ...
	 * @return ZC_User|ZC_Error True if ID is valid, ZC_Error otherwise.
	 */
	 ...
	 *
	 * @param ZC_REST_Request $request Full details about the request.
	 * @return true|ZC_Error True if the request has read access for the item, otherwise ZC_Error object.
	 */
	...
	public function create_item( $request ) {
		...
		if ( ! empty( $request['roles'] ) && ! empty( $schema['properties']['roles'] ) ) {
			...
			if ( is_zc_error( $check_permission ) ) {
		...
		if ( is_multisite() ) {
			$ret = zcmu_validate_user_signup( $user->user_login, $user->user_email );

			if ( is_zc_error( $ret['errors'] ) && $ret['errors']->has_errors() ) {
				$error = new ZC_Error(
					'rest_invalid_param',
					__( 'Invalid user parameter(s).' ),
					array( 'status' => 400 )
		...
		if ( is_multisite() ) {
			$user_id = zcmu_create_user( $user->user_login, $user->user_pass, $user->user_email );
			...
			$user->ID = $user_id;
			$user_id  = zc_update_user( zc_slash( (array) $user ) );
			...
			$user_id = zc_insert_user( zc_slash( (array) $user ) );
		 ...
		 * @since 4.7.0
		 *
		 * @param ZC_User         $user     Inserted or updated user object.
	...
	public function update_item( $request ) {
		...
		/** This action is documented in zc-includes/rest-api/endpoints/class-zc-rest-users-controller.php */
	...
	public function delete_item( $request ) {
		...
		// Include user admin functions to get access to zc_delete_user().
		require_once ABSPATH . 'zc-admin/includes/user.php';

		$result = zc_delete_user( $id, $reassign );
		 ...
		 * @since 4.7.0
		 *
		 * @param ZC_User          $user     The user data.
	 ...
	 *
	 * @param ZC_User         $item    User object.
	 * @param ZC_REST_Request $request Request object.
	...
	public function prepare_item_for_response( $item, $request ) {
		...
		if ( $request->is_method( 'HEAD' ) ) {
			/** This filter is documented in zc-includes/rest-api/endpoints/class-zc-rest-users-controller.php */
			return apply_filters( 'rest_prepare_user', new ZC_REST_Response( array() ), $user, $request );
		}
		 ...
		 * @param ZC_User          $user     User object used to create response.
		 * @param ZC_REST_Request  $request  Request object.
		 */
	 ...
	 * @return true|ZC_Error True if the current user is allowed to make the role change,
	 *                       otherwise a ZC_Error object.
	 */
	protected function check_role_update( $user_id, $roles ) {
		...
		foreach ( $roles as $role ) {
			...
			if ( ! isset( $zc_roles->role_objects[ $role ] ) ) {
				return new ZC_Error(
			...
			$potential_role = $zc_roles->role_objects[ $role ];
	 ...
	 *
	 * Performs a couple of checks like edit_user() in zc-admin/includes/user.php.
	 *
	 * @since 4.7.0
	 *
	 ...
	 * @return string|ZC_Error The sanitized username, if valid, otherwise an error.
	 */
	 ...
	 * @param ZC_REST_Request $request Full details about the request.
	 * @param string          $param   The parameter name.
	 * @return string|ZC_Error The sanitized password, if valid, otherwise an error.
	 */
	...
	public function get_collection_params() {
		 ...
		 * collection parameter to an internal ZC_User_Query parameter.  Use the
		 * `rest_user_query` filter to set ZC_User_Query arguments.
		 *
		 * @since 4.7.0
		 *
