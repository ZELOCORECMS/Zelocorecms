<?php
/**
 * REST API: ZC_REST_Attachments_Controller class
 *
 * @package ZelocoreCMS
 * @subpackage REST_API
 * @since 4.7.0
 ...
 * @since 4.7.0
 *
 * @see ZC_REST_Posts_Controller
 */
class ZC_REST_Attachments_Controller extends ZC_REST_Posts_Controller {
	 ...
	 * @return true|ZC_Error Boolean true if the attachment may be created, or a ZC_Error if not.
	 */
	public function create_item_permissions_check( $request ) {
		$ret = parent::create_item_permissions_check( $request );

		if ( ! $ret || is_zc_error( $ret ) ) {
			return $ret;
		}
		 ...
		 */
		$prevent_unsupported_uploads = apply_filters( 'zc_prevent_unsupported_mime_type_uploads', true, $files['file']['type'] ?? null );
		...
		if (
			...
			if (
				...
				! zc_image_editor_supports( array( 'mime_type' => $files['file']['type'] ) )
			) {
				return new ZC_Error(
	 ...
	 *
	 * @param ZC_REST_Request $request Full details about the request.
	 * @return ZC_REST_Response|ZC_Error Response object on success, ZC_Error object on failure.
	 */
	public function create_item( $request ) {
		...
		if ( isset( $request['alt_text'] ) ) {
			update_post_meta( $attachment_id, '_zc_attachment_image_alt', sanitize_text_field( $request['alt_text'] ) );
		}

		if ( ! empty( $schema['properties']['featured_media'] ) && isset( $request['featured_media'] ) ) {
			$thumbnail_update = $this->handle_featured_media( $request['featured_media'], $attachment_id );

			if ( is_zc_error( $thumbnail_update ) ) {
		 ...
		 * @since 5.0.0
		 *
		 * @param ZC_Post         $attachment Inserted or updated attachment object.
		 * @param ZC_REST_Request $request    Request object.
		 * @param bool            $creating   True when creating an attachment, false when updating.
		 */
		do_action( 'rest_after_insert_attachment', $attachment, $request, true );

		zc_after_insert_post( $attachment, false, null );
		...
		// Include media and image functions to get access to zc_generate_attachment_metadata().
	...
	protected function insert_attachment( $request ) {
		...
		// Include image functions to get access to zc_read_image_metadata().
		require_once ABSPATH . 'zc-admin/includes/image.php';
		...
		if ( empty( $attachment->post_title ) ) {
			$attachment->post_title = preg_replace( '/\.[^.]+$/', '', zc_basename( $file ) );
		}

		// $post_parent is inherited from $attachment['post_parent'].
		$id = zc_insert_attachment( zc_slash( (array) $attachment ), $file, 0, true, false );

		if ( trim( $alt ) ) {
			update_post_meta( $id, '_zc_attachment_image_alt', sanitize_text_field( $alt ) );
		}
	...
	protected function handle_featured_media( $featured_media, $post_id ) {
		...
		// Similar check as in zc_insert_post().
	...
	public function update_item( $request ) {
		...
		if ( isset( $request['alt_text'] ) ) {
			update_post_meta( $data['id'], '_zc_attachment_image_alt', $request['alt_text'] );
		}
		...
		/** This action is documented in zc-includes/rest-api/endpoints/class-zc-rest-attachments-controller.php */
		do_action( 'rest_after_insert_attachment', $attachment, $request, false );

		zc_after_insert_post( $attachment, true, $attachment_before );
	...
	public function post_process_item( $request ) {
		switch ( $request['action'] ) {
			case 'create-image-subsizes':
				require_once ABSPATH . 'zc-admin/includes/image.php';
				zc_update_image_subsizes( $request['id'] );
				break;
		}
	...
	public function edit_media_item( $request ) {
		...
		$image_file = zc_get_original_image_path( $attachment_id );
		$image_meta = zc_get_attachment_metadata( $attachment_id );

		if (
			! $image_meta ||
			! $image_file ||
			! zc_image_file_matches_image_meta( $request['src'], $image_meta, $attachment_id )
		) {
			return new ZC_Error(
		...
		$image_editor = zc_get_image_editor( $image_file_to_edit );

		if ( is_zc_error( $image_editor ) ) {
			return new ZC_Error(
		...
		$image_name = zc_basename( $image_file, ".{$image_ext}" );
		...
		if ( preg_match( '/-edited(-\d+)?$/', $image_name ) ) {
			// Remove any `-1`, `-2`, etc. `zc_unique_filename()` will add the proper number.
		...
		$filename = zc_unique_filename( $uploads['path'], $filename );
		...
		// Unset ID so zc_insert_attachment generates a new ID.
		...
		$new_attachment_id = zc_insert_attachment( zc_slash( (array) $new_attachment_post ), $saved['path'], 0, true );

		if ( is_zc_error( $new_attachment_id ) ) {
		...
		$image_alt = isset( $request['alt_text'] ) ? sanitize_text_field( $request['alt_text'] ) : get_post_meta( $attachment_id, '_zc_attachment_image_alt', true );

		if ( ! empty( $image_alt ) ) {
			// update_post_meta() expects slashed.
			update_post_meta( $new_attachment_id, '_zc_attachment_image_alt', zc_slash( $image_alt ) );
		}
		...
		if ( zc_is_serving_rest_request() ) {
			 ...
			 * Used by the browser/client to resume creating image sub-sizes after a PHP fatal error.
			 */
			header( 'X-ZC-Upload-Attachment-ID: ' . $new_attachment_id );
		}

		// Generate image sub-sizes and meta.
		$new_image_meta = zc_generate_attachment_metadata( $new_attachment_id, $saved['path'] );
		...
		$new_image_meta['parent_image'] = array(
			...
			'file'          => _zc_relative_upload_path( $image_file ),
		);
		 ...
		 */
		$new_image_meta = apply_filters( 'zc_edited_image_metadata', $new_image_meta, $new_attachment_id, $attachment_id );

		zc_update_attachment_metadata( $new_attachment_id, $new_image_meta );
	 ...
	 *
	 * @param ZC_REST_Request $request Request object.
	 * @return stdClass|ZC_Error Post object.
	 */
	 ...
	 *
	 * @param ZC_Post         $item    Attachment object.
	 * @param ZC_REST_Request $request Request object.
	...
	public function prepare_item_for_response( $item, $request ) {
		...
		if ( in_array( 'alt_text', $fields, true ) ) {
			$data['alt_text'] = get_post_meta( $post->ID, '_zc_attachment_image_alt', true );
		}

		if ( in_array( 'media_type', $fields, true ) ) {
			$data['media_type'] = zc_attachment_is_image( $post->ID ) ? 'image' : 'file';
		}
		...
		if ( in_array( 'media_details', $fields, true ) ) {
			$data['media_details'] = zc_get_attachment_metadata( $post->ID );
			...
			if ( empty( $data['media_details'] ) ) {
				...
				foreach ( $data['media_details']['sizes'] as $size => &$size_data ) {
					...
					$image_src = zc_get_attachment_image_src( $post->ID, $size );
					if ( ! $image_src ) {
						continue;
					}
				...
				$full_src = zc_get_attachment_image_src( $post->ID, 'full' );

				if ( ! empty( $full_src ) ) {
					$data['media_details']['sizes']['full'] = array(
						'file'       => zc_basename( $full_src[0] ),
						'width'      => $full_src[1],
						'height'     => $full_src[2],
		...
		if ( in_array( 'source_url', $fields, true ) ) {
			$data['source_url'] = zc_get_attachment_url( $post->ID );
		}

		if ( in_array( 'missing_image_sizes', $fields, true ) ) {
			require_once ABSPATH . 'zc-admin/includes/image.php';
			$data['missing_image_sizes'] = array_keys( zc_get_missing_image_subsizes( $post->ID ) );

			// Handle PDFs which don't use zc_get_missing_image_subsizes().
			if ( empty( $data['missing_image_sizes'] ) && 'application/pdf' === get_post_mime_type( $post ) ) {
				$metadata = zc_get_attachment_metadata( $post->ID, true );
				...
				/** This filter is documented in zc-admin/includes/image.php */
				$fallback_sizes = apply_filters( 'fallback_intermediate_image_sizes', $fallback_sizes, $metadata );

				$registered_sizes = zc_get_registered_image_subsizes();
		...
		if ( in_array( 'exif_orientation', $fields, true ) && zc_attachment_is_image( $post ) ) {
			$metadata = zc_get_attachment_metadata( $post->ID, true );
		 ...
		 * @param ZC_Post          $post     The original attachment post.
	 ...
	 * @return array|ZC_Error Data from zc_handle_sideload().
	 */
	protected function upload_from_data( $data, $headers, $time = null ) {
		...
		// Include filesystem functions to get access to zc_tempnam() and zc_handle_sideload().
		require_once ABSPATH . 'zc-admin/includes/file.php';

		// Save the file.
		$tmpfname = zc_tempnam( $filename );
		...
		$sideloaded = zc_handle_sideload( $file_data, $overrides, $time );
	 ...
	 * @param array       $headers HTTP headers from the request.
	 * @param string|null $time    Optional. Time formatted in 'yyyy/mm'. Default null.
	 * @return array|ZC_Error Data from zc_handle_upload().
	 */
	protected function upload_from_file( $files, $headers, $time = null ) {
		...
		if ( defined( 'DIR_TESTDATA' ) && DIR_TESTDATA ) {
			$overrides['action'] = 'zc_handle_mock_upload';
		}
		...
		// Include filesystem functions to get access to zc_handle_upload().
		require_once ABSPATH . 'zc-admin/includes/file.php';

		$file = zc_handle_upload( $files['file'], $overrides, $time );
	 ...
	 * @return true|ZC_Error True if can upload, error for errors.
	 */
	...
	protected function get_edit_media_item_args() {
		 ...
		 */
		$update_item_args = $this->get_endpoint_args_for_item_schema( ZC_REST_Server::EDITABLE );
	...
	protected function get_attachment_filename( int $attachment_id ): ?string {
		$path = zc_get_original_image_path( $attachment_id );

		if ( $path ) {
			return zc_basename( $path );
		}
	...
	protected function get_attachment_filesize( int $attachment_id ): ?int {
		...
		$original_path = zc_get_original_image_path( $attachment_id );
		...
		if ( is_string( $attached_file ) && is_readable( $attached_file ) ) {
			return zc_filesize( $attached_file );
		}
