<?php
/**
 * REST API: ZC_REST_Terms_Controller class
 *
 * @package ZelocoreCMS
 * @subpackage REST_API
 * @since 4.7.0
 ...
 */
class ZC_REST_Terms_Controller extends ZC_REST_Controller {
	 ...
	 *
	 * @param ZC_REST_Request $request Full details about the request.
	 * @return bool|ZC_Error True if the request has read access, otherwise false or ZC_Error object.
	 */
	...
	public function get_items( $request ) {
		 ...
		 * Otherwise, terms will be fetched using `zc_get_object_terms()`,
		 ...
		 * @since 4.7.0
		 *
		 * @link https://developer.zelocorecms.org/reference/functions/get_terms/
		 *
		...
		if ( ! empty( $prepared_args['post'] ) ) {
			$query_result = zc_get_object_terms( $prepared_args['post'], $this->taxonomy, $prepared_args );

			// Used when calling zc_count_terms() below.
		...
		$total_terms = zc_count_terms( $count_args );

		// zc_count_terms() can return a falsey value when the term has no children.
		if ( ! $total_terms ) {
			$total_terms = 0;
		}
		...
		$response = $is_head_request ? new ZC_REST_Response( array() ) : rest_ensure_response( $response );

		// Store pagination values for headers.
		$per_page = (int) $prepared_args['number'];
		$page     = (int) ceil( ( ( (int) $prepared_args['offset'] ) / $per_page ) + 1 );

		$response->header( 'X-ZC-Total', (int) $total_terms );
	 ...
	 * @return ZC_Term|ZC_Error Term object if ID is valid, ZC_Error otherwise.
	 */
	 ...
	 *
	 * @param ZC_REST_Request $request Full details about the request.
	 * @return bool|ZC_Error True if the request has access to create items, otherwise false or ZC_Error object.
	 */
	...
	public function create_item( $request ) {
		...
		$term = zc_insert_term( zc_slash( $prepared_term->name ), $this->taxonomy, zc_slash( (array) $prepared_term ) );
		if ( is_zc_error( $term ) ) {
			/*
		 ...
		 * @since 4.7.0
		 *
		 * @param ZC_Term         $term     Inserted or updated term object.
	 ...
	 *
	 * @param ZC_REST_Request $request Full details about the request.
	 * @return true|ZC_Error True if the request has access to update the item, false or ZC_Error object otherwise.
	 */
	...
	public function update_item( $request ) {
		...
		if ( ! empty( $prepared_term ) ) {
			$update = zc_update_term( $term->term_id, $term->taxonomy, zc_slash( (array) $prepared_term ) );

			if ( is_zc_error( $update ) ) {
				return $update;
			}
		}
		...
		/** This action is documented in zc-includes/rest-api/endpoints/class-zc-rest-terms-controller.php */
	 ...
	 *
	 * @param ZC_REST_Request $request Full details about the request.
	 * @return true|ZC_Error True if the request has access to delete the item, otherwise false or ZC_Error object.
	 */
	...
	public function delete_item( $request ) {
		...
		$retval = zc_delete_term( $term->term_id, $term->taxonomy );
		 ...
		 * @since 4.7.0
		 *
		 * @param ZC_Term          $term     The deleted term.
	...
	public function prepare_item_for_database( $request ) {
		...
		if ( isset( $request['parent'] ) && ! empty( $schema['properties']['parent'] ) ) {
			...
			if ( $requested_parent ) {
				...
				if ( $parent_term instanceof ZC_Term ) {
	 ...
	 * @since 4.7.0
	 *
	 * @param ZC_Term         $item    Term object.
	 * @param ZC_REST_Request $request Request object.
	...
	public function prepare_item_for_response( $item, $request ) {
		...
		if ( $request->is_method( 'HEAD' ) ) {
			/** This filter is documented in zc-includes/rest-api/endpoints/class-zc-rest-terms-controller.php */
			return apply_filters( "rest_prepare_{$this->taxonomy}", new ZC_REST_Response( array() ), $item, $request );
		}
		 ...
		 * @param ZC_Term           $item      The original term object.
	...
	public function get_collection_params() {
		 ...
		 * collection parameter to an internal ZC_Term_Query parameter.  Use the
		 * `rest_{$this->taxonomy}_query` filter to set ZC_Term_Query parameters.
		 *
		 * @since 4.7.0
		 *
