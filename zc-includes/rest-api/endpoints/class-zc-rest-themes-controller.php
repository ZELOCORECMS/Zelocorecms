<?php
/**
 * REST API: ZC_REST_Themes_Controller class
 *
 * @package ZelocoreCMS
 * @subpackage REST_API
 * @since 5.0.0
 ...
 */
class ZC_REST_Themes_Controller extends ZC_REST_Controller {
	...
	public function get_item_permissions_check( $request ) {
		...
		$zc_theme      = zc_get_theme( $request['stylesheet'] );
		$current_theme = zc_get_theme();

		if ( $this->is_same_theme( $zc_theme, $current_theme ) ) {
			return $this->check_read_active_theme_permission();
		}
	 ...
	 * @since 5.7.0
	 *
	 * @return true|ZC_Error True if the theme can be read, ZC_Error object otherwise.
	 */
	...
	public function get_item( $request ) {
		...
		$data = $this->prepare_item_for_response( $zc_theme, $request );
	...
	public function get_items( $request ) {
		...
		if ( array( 'active' ) === $status ) {
			...
			foreach ( zc_get_themes() as $theme ) {
		...
		$response->header( 'X-ZC-Total', count( $themes ) );
		$response->header( 'X-ZC-TotalPages', 1 );
	 ...
	 *
	 * @param ZC_Theme        $item    Theme object.
	...
	public function prepare_item_for_response( $item, $request ) {
		 ...
		 * @param ZC_Theme         $theme    Theme object used to create response.
		 * @param ZC_REST_Request  $request  Request object.
		 */
	 ...
	 * @since 5.7.0
	 *
	 * @param ZC_Theme $theme Theme data.
	...
	protected function prepare_links( $theme ) {
		...
		if ( $this->is_same_theme( $theme, zc_get_theme() ) ) {
			// This creates a record for the active theme if not existent.
			$id = ZC_Theme_JSON_Resolver::get_user_global_styles_post_id();
		} else {
			$user_cpt = ZC_Theme_JSON_Resolver::get_user_data_from_zc_global_styles( $theme );
			$id       = $user_cpt['ID'] ?? null;
		}
		...
		if ( $theme->is_block_theme() && $this->is_same_theme( $theme, zc_get_theme() ) ) {
			$links['https://api.w.org/export-theme'] = array(
				'href'        => rest_url( 'zc-block-editor/v1/export' ),
				'targetHints' => array(
	 ...
	 * @since 5.7.0
	 *
	 * @param ZC_Theme $theme_a First theme to compare.
	 * @param ZC_Theme $theme_b Second theme to compare.
	 * @return bool
	 */
	...
	public function get_item_schema() {
		...
		$schema = array(
			...
			'properties' => array(
				...
				'requires_wp'                 => array(
					'description' => __( 'The minimum ZelocoreCMS version required for the theme to work.' ),
					'type'        => 'string',
					'readonly'    => true,
				),
				'screenshot'                  => array(
