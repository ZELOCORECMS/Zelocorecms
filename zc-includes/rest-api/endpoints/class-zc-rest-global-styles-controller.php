<?php
/**
 * REST API: ZC_REST_Global_Styles_Controller class
 *
 * @package    ZelocoreCMS
 * @subpackage REST_API
 * @since 5.9.0
 ...
 */
class ZC_REST_Global_Styles_Controller extends ZC_REST_Posts_Controller {
	/**
	 ...
	 */
	public function __construct( $post_type = 'zc_global_styles' ) {
	 ...
	 * @return stdClass|ZC_Error Prepared item on success. ZC_Error on when the custom CSS is not valid.
	 */
		...
		if ( isset( $request['styles'] ) || isset( $request['settings'] ) ) {
			...
			if ( isset( $request['styles'] ) ) {
				if ( isset( $request['styles']['css'] ) ) {
					...
					if ( is_zc_error( $css_validation_result ) ) {
			...
			$variations = ZC_Theme_JSON_Resolver::get_style_variations( 'block' );
			...
			$config['version']                     = ZC_Theme_JSON::LATEST_SCHEMA;
			/**
			 * JSON encode the data stored in post content.
			 * Escape characters that are likely to be mangled by HTML filters: "<>&".
			 *
			 * This data is later re-encoded by {@see zc_filter_global_styles_post()}.
			 * The escaping is also applied here as a precaution.
			 */
			$changes->post_content = zc_json_encode( $config, JSON_UNESCAPED_SLASHES | JSON_HEX_TAG | JSON_HEX_AMP );
		}
	 ...
	 *
	 * @param ZC_Post         $post    Global Styles post object.
	 * @param ZC_REST_Request $request Request object.
	...
	public function prepare_item_for_response( $post, $request ) {
		...
		if ( $is_global_styles_user_theme_json ) {
			$theme_json = new ZC_Theme_JSON( $raw_config, 'custom' );
		...
		if ( rest_is_field_included( '_links', $fields ) || rest_is_field_included( '_embedded', $fields ) ) {
			...
			if ( $theme_json ) {
				$resolved_theme_uris = ZC_Theme_JSON_Resolver::get_resolved_theme_uris( $theme_json );
	...
	protected function prepare_links( $id ) {
		...
		if ( post_type_supports( $this->post_type, 'revisions' ) ) {
			$revisions                = zc_get_latest_revision_id_and_total_count( $id );
	...
	public function get_theme_item( $request ) {
		...
		$theme  = ZC_Theme_JSON_Resolver::get_merged_data( 'theme' );
		$fields = $this->get_fields_for_response( $request );
		$data   = array();
		...
		if ( rest_is_field_included( '_links', $fields ) || rest_is_field_included( '_embedded', $fields ) ) {
			...
			$resolved_theme_uris = ZC_Theme_JSON_Resolver::get_resolved_theme_uris( $theme );
			if ( ! empty( $resolved_theme_uris ) ) {
				$links['https://api.w.org/theme-file'] = $resolved_theme_uris;
			}
			$response->add_links( $links );
	...
	public function get_theme_items( $request ) {
		...
		// Register theme-defined variations e.g. from block style variation partials under `/styles`.
		$partials = ZC_Theme_JSON_Resolver::get_style_variations( 'block' );
		zc_register_block_style_variations_from_theme_json_partials( $partials );

		$variations = ZC_Theme_JSON_Resolver::get_style_variations();
		foreach ( $variations as $variation ) {
			$variation_theme_json = new ZC_Theme_JSON( $variation );
			$resolved_theme_uris  = ZC_Theme_JSON_Resolver::get_resolved_theme_uris( $variation_theme_json );
	 ...
	 *
	 * @see ZC_Customize_Custom_CSS_Setting::validate()
	 *
	 * @param string $css CSS to validate.
	 * @return true|ZC_Error True if the input was validated, otherwise ZC_Error.
	 */
