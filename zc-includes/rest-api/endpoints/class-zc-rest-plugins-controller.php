<?php
/**
 * REST API: ZC_REST_Plugins_Controller class
 *
 * @package ZelocoreCMS
 * @subpackage REST_API
 * @since 5.5.0
 ...
 */
class ZC_REST_Plugins_Controller extends ZC_REST_Controller {
				...
				array(
					...
					'args'                => array(
						'slug'   => array(
							'type'        => 'string',
							'required'    => true,
							'description' => __( 'ZelocoreCMS.org plugin directory slug.' ),
							'pattern'     => '[\w\-]+',
						),
						'status' => array(
	...
	public function get_items( $request ) {
		...
		foreach ( get_plugins() as $file => $data ) {
			if ( is_zc_error( $this->check_read_permission( $file ) ) ) {
				continue;
			}
		...
		return new ZC_REST_Response( $plugins );
	}
	 ...
	 * @return true|ZC_Error True if can read, a ZC_Error instance otherwise.
	 */
	protected function check_read_permission( $plugin ) {
		require_once ABSPATH . 'zc-admin/includes/plugin.php';

		if ( ! $this->is_plugin_installed( $plugin ) ) {
			return new ZC_Error( 'rest_plugin_not_found', __( 'Plugin not found.' ), array( 'status' => 404 ) );
		}
	...
	public function create_item( $request ) {
		...
		if ( is_zc_error( $filesystem_available ) ) {
		...
		if ( is_null( $result ) ) {
			...
			if ( $zc_filesystem instanceof ZC_Filesystem_Base
				&& is_zc_error( $zc_filesystem->errors ) && $zc_filesystem->errors->has_errors()
			) {
				return new ZC_Error(
					'unable_to_connect_to_filesystem',
					$zc_filesystem->errors->get_error_message(),
					array( 'status' => 500 )
				);
			}
		...
		/** This filter is documented in zc-includes/update.php */
	...
	public function prepare_item_for_response( $item, $request ) {
		...
		$response = new ZC_REST_Response( $data );
	 ...
	 * @return array|ZC_Error The plugin data, or a ZC_Error if the plugin is not installed.
	 */
	 ...
	 * @since 5.5.0
	 *
	 * @param ZC_REST_Request $request The request to require the plugin matches against.
	 * @param array           $item    The plugin item.
	 * @return bool
	 */
	...
	protected function is_plugin_installed( $plugin ) {
		return file_exists( ZC_PLUGIN_DIR . '/' . $plugin );
	}
	 ...
	 * @since 5.5.0
	 *
	 * @return true|ZC_Error True if filesystem is available, ZC_Error otherwise.
	 */
	protected function is_filesystem_available() {
		...
		return new ZC_Error( 'fs_unavailable', __( 'The filesystem is currently unavailable for managing plugins.' ), array( 'status' => 500 ) );
	}
	...
	public function get_item_schema() {
		...
		$this->schema = array(
			...
			'properties' => array(
				...
				'requires_wp'  => array(
					'description' => __( 'Minimum required version of ZelocoreCMS.' ),
					'type'        => 'string',
					'readonly'    => true,
					'context'     => array( 'view', 'edit', 'embed' ),
				),
