<?php
/**
 * REST API: ZC_REST_Menus_Controller class
 *
 * @package ZelocoreCMS
 * @subpackage REST_API
 * @since 5.9.0
 ...
 */
class ZC_REST_Menus_Controller extends ZC_REST_Terms_Controller {
	...
	protected function get_term( $id ) {
		...
		$nav_term           = zc_get_nav_menu_object( $term );
	 ...
	 *
	 * @param ZC_REST_Request $request Full details about the request.
	 * @return true|ZC_Error True if the current user has permission, ZC_Error object otherwise.
	 */
	protected function check_has_read_only_access( $request ) {
		/** This filter is documented in zc-includes/rest-api/endpoints/class-zc-rest-menu-items-controller.php */
	...
	public function prepare_item_for_response( $term, $request ) {
		$nav_menu = zc_get_nav_menu_object( $term );
	...
	public function create_item( $request ) {
		if ( isset( $request['parent'] ) ) {
			if ( ! is_taxonomy_hierarchical( $this->taxonomy ) ) {
				return new ZC_Error( 'rest_taxonomy_not_hierarchical', __( 'Cannot set parent term, taxonomy is not hierarchical.' ), array( 'status' => 400 ) );
			}

			$parent = zc_get_nav_menu_object( (int) $request['parent'] );

			if ( ! $parent ) {
				return new ZC_Error( 'rest_term_invalid', __( 'Parent term does not exist.' ), array( 'status' => 400 ) );
			}
		}

		$prepared_term = $this->prepare_item_for_database( $request );

		$term = zc_update_nav_menu_object( 0, zc_slash( (array) $prepared_term ) );
		...
		if ( is_zc_error( $locations_update ) ) {
	...
	public function update_item( $request ) {
		...
		if ( ! empty( $prepared_term ) ) {
			if ( ! isset( $prepared_term->{'menu-name'} ) ) {
				// zc_update_nav_menu_object() requires that the menu-name is always passed.
				$prepared_term->{'menu-name'} = $term->name;
			}

			$update = zc_update_nav_menu_object( $term->term_id, zc_slash( (array) $prepared_term ) );
	...
	public function delete_item( $request ) {
		...
		if ( ! $request['force'] ) {
			...
			return new ZC_Error( 'rest_trash_not_supported', sprintf( __( "Menus do not support trashing. Set '%s' to delete." ), 'force=true' ), array( 'status' => 501 ) );
		}
		...
		$result = zc_delete_nav_menu( $term );

		if ( ! $result || is_zc_error( $result ) ) {
			return new ZC_Error( 'rest_cannot_delete', __( 'The menu cannot be deleted.' ), array( 'status' => 500 ) );
		}
	...
	protected function handle_auto_add( $menu_id, $request ) {
		...
		do_action( 'zc_update_nav_menu', $menu_id );
	 ...
	 * @param int             $menu_id The menu id to update.
	 * @param ZC_REST_Request $request Full details about the request.
	 * @return true|ZC_Error True on success, a ZC_Error on an error updating any of the locations.
	 */
