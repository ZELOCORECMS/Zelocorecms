<?php
/**
 * Interactivity API: ZC_Interactivity_API_Directives_Processor class.
 *
 * @package ZelocoreCMS
 ...
 * @access private
 */
final class ZC_Interactivity_API_Directives_Processor extends ZC_HTML_Tag_Processor {
	/**
	 * List of tags whose closer tag is not visited by the ZC_HTML_Tag_Processor.
	 *
	 * @since 6.5.0
	 * @var string[]
	 */
	...
	public function append_content_after_template_tag_closer( string $new_content ): bool {
		...
		$this->lexical_updates[] = new ZC_HTML_Text_Replacement( $after_closing_tag, 0, $new_content );
	...
	public function skip_to_tag_closer(): bool {
		...
		while ( $depth > 0 && $this->next_tag( array( 'tag_closers' => 'visit' ) ) ) {
			if ( ! $this->is_tag_closer() && $this->get_attribute_names_with_prefix( 'data-zc-' ) ) {
	...
	public function has_and_visits_its_closer_tag(): bool {
		...
		return null !== $tag_name && (
			! ZC_HTML_Processor::is_void( $tag_name ) &&
