<?php
/**
 * Interactivity API: ZC_Interactivity_API class.
 *
 * @package ZelocoreCMS
 ...
 * @since 6.5.0
 */
final class ZC_Interactivity_API {
	/**
	...
	private static $directive_processors = array(
		'data-zc-interactive'   => 'data_zc_interactive_processor',
		'data-zc-router-region' => 'data_zc_router_region_processor',
		'data-zc-context'       => 'data_zc_context_processor',
		'data-zc-bind'          => 'data_zc_bind_processor',
		'data-zc-class'         => 'data_zc_class_processor',
		'data-zc-style'         => 'data_zc_style_processor',
		'data-zc-text'          => 'data_zc_text_processor',
		/*
		 * `data-zc-each` needs to be processed in the last place because it moves
		 * the cursor to the end of the processed items to prevent them to be
		 * processed twice.
		 */
		'data-zc-each'          => 'data_zc_each_processor',
	);
	...
	/**
	 * Flag that indicates whether the `data-zc-router-region` directive has
	 * been found in the HTML and processed.
	 *
	 * The value is saved in a private property of the ZC_Interactivity_API
	 ...
	 * `data-zc-router-region` directive or not.
	 *
	 * @since 6.5.0
	 * @var bool
	 */
	...
	/**
	 * Stack of namespaces defined by `data-zc-interactive` directives, in
	...
	/**
	 * Stack of contexts defined by `data-zc-context` directives, in
	 * the order they are processed.
	 *
	 * This is only available during directive processing, otherwise it is `null`.
	 *
	...
	/**
	 * Registers the `@zelocorecms/interactivity` script modules.
	 *
	 * @deprecated 6.7.0 Script Modules registration is handled by {@see zc_default_script_modules()}.
	 *
	 * @since 6.5.0
	 */
	public function register_script_modules() {
		_deprecated_function( __METHOD__, '6.7.0', 'zc_default_script_modules' );
	}
	...
	public function add_hooks() {
		add_filter( 'script_module_data_@zelocorecms/interactivity', array( $this, 'filter_script_module_interactivity_data' ) );
		add_filter( 'script_module_data_@zelocorecms/interactivity-router', array( $this, 'filter_script_module_interactivity_router_data' ) );
		add_filter( 'zc_script_attributes', array( $this, 'add_load_on_client_navigation_attribute_to_script_modules' ), 10, 1 );
	}

	/**
	 * Adds the `data-zc-router-options` attribute to script modules that
	...
	public function add_load_on_client_navigation_attribute_to_script_modules( $attributes ) {
		if (
			...
			$attributes['data-zc-router-options'] = zc_json_encode( array( 'loadOnClientNavigation' => true ) );
		}
		return $attributes;
	}
	 ...
	 * `data-zc-router-options` directive.
	 *
	 * @since 6.9.0
	 *
	 ...
	 * and updates the markup accordingly.
	 *
	 * It uses the ZC_Interactivity_API instance's context and namespace stacks,
	...
	private function _process_directives( string $html ) {
		$p          = new ZC_Interactivity_API_Directives_Processor( $html );
		$tag_stack  = array();
		$unbalanced = false;
		...
		while ( $p->next_tag( array( 'tag_closers' => 'visit' ) ) ) {
			...
			if ( 'SVG' === $tag_name || 'MATH' === $tag_name ) {
				if ( $p->get_attribute_names_with_prefix( 'data-zc-' ) ) {
			...
			if ( $p->is_tag_closer() ) {
				...
				$each_child_attrs = $p->get_attribute_names_with_prefix( 'data-zc-each-child' );
				...
				if ( 0 !== count( $each_child_attrs ) ) {
					/*
					 * If the tag has a `data-zc-each-child` directive, jump to its closer
					...
					foreach ( $p->get_attribute_names_with_prefix( 'data-zc-' ) as $attribute_name ) {
						$parsed_directive = $this->parse_directive_name( $attribute_name );
						if ( empty( $parsed_directive ) ) {
							continue;
						}
						$directive_prefix = 'data-zc-' . $parsed_directive['prefix'];
	 ...
	 * - Prefix: The main directive name without "data-zc-".
	 ...
	 * See `parseDirectiveName` in https://github.com/ZelocoreCMS/gutenberg/blob/trunk/packages/interactivity/src/vdom.ts.:
	 *
	...
	private function parse_directive_name( string $directive_name ): ?array {
		// Remove the first 8 characters (assumes "data-zc-" prefix)
	 ...
	 * @since 6.9.0
	 *
	 * @param ZC_Interactivity_API_Directives_Processor $p      The directives processor instance.
	 ...
	 */
	private function get_directive_entries( ZC_Interactivity_API_Directives_Processor $p, string $prefix ) {
		$directive_attributes = $p->get_attribute_names_with_prefix( 'data-zc-' . $prefix );
		$entries              = array();
	...
	/**
	 * Processes the `data-zc-interactive` directive.
	 *
	 ...
	 */
	private function data_zc_interactive_processor( ZC_Interactivity_API_Directives_Processor $p, string $mode ) {
		...
		// Tries to decode the `data-zc-interactive` attribute value.
		$attribute_value = $p->get_attribute( 'data-zc-interactive' );

		/*
		 * Pushes the newly defined namespace or the current one if the
		 * `data-zc-interactive` definition was invalid or does not contain a
		 * namespace. It does so because the function pops out the current namespace
		 * from the stack whenever it finds a `data-zc-interactive`'s closing tag,
		 * independently of whether the previous `data-zc-interactive` definition
	...
	/**
	 * Processes the `data-zc-context` directive.
	 *
	 ...
	 * @param string                                    $mode            Whether the processing is entering or exiting the tag.
	 */
	private function data_zc_context_processor( ZC_Interactivity_API_Directives_Processor $p, string $mode ) {
	...
	/**
	 * Processes the `data-zc-bind` directive.
	 *
	 ...
	 * @param string                                    $mode            Whether the processing is entering or exiting the tag.
	 */
	private function data_zc_bind_processor( ZC_Interactivity_API_Directives_Processor $p, string $mode ) {
		if ( 'enter' === $mode ) {
			...
			foreach ( $entries as $entry ) {
				...
				if ( str_starts_with( $entry['suffix'], 'on' ) ) {
					_doing_it_wrong(
						__METHOD__,
						sprintf(
							/* translators: %s: The directive, e.g. data-zc-on--click. */
							__( 'Binding event handler attributes is not supported. Please use "%s" instead.' ),
							esc_attr( 'data-zc-on--' . substr( $entry['suffix'], 2 ) )
						),
						'6.9.2'
					);
					continue;
	...
	/**
	 * Processes the `data-zc-class` directive.
	 *
	 ...
	 * @param string                                    $mode            Whether the processing is entering or exiting the tag.
	 */
	private function data_zc_class_processor( ZC_Interactivity_API_Directives_Processor $p, string $mode ) {
		if ( 'enter' === $mode ) {
			$all_class_directives = $p->get_attribute_names_with_prefix( 'data-zc-class--' );
	...
	/**
	 * Processes the `data-zc-style` directive.
	 *
	 ...
	 * @param string                                    $mode            Whether the processing is entering or exiting the tag.
	 */
	private function data_zc_style_processor( ZC_Interactivity_API_Directives_Processor $p, string $mode ) {
		if ( 'enter' === $mode ) {
	...
	/**
	 * Processes the `data-zc-text` directive.
	 *
	 ...
	 * @param string                                    $mode            Whether the processing is entering or exiting the tag.
	 */
	private function data_zc_text_processor( ZC_Interactivity_API_Directives_Processor $p, string $mode ) {
		if ( 'enter' === $mode ) {
			$entries     = $this->get_directive_entries( $p, 'text' );
			$valid_entry = null;
			// Get the first valid `data-zc-text` entry without suffix or unique ID.
			foreach ( $entries as $entry ) {
	...
	private function get_router_animation_styles(): string {
		return <<<CSS
			.zc-interactivity-router-loading-bar {
				position: fixed;
				top: 0;
				left: 0;
				margin: 0;
			...
			.zc-interactivity-router-loading-bar.start-animation {
				animation: zc-interactivity-router-loading-bar-start-animation 30s cubic-bezier(0.03, 0.5, 0, 1) forwards
			}
			.zc-interactivity-router-loading-bar.finish-animation {
				animation: zc-interactivity-router-loading-bar-finish-animation 300ms ease-in
			}
			@keyframes zc-interactivity-router-loading-bar-start-animation {
				0% { transform: scaleX(0); transform-origin: 0 0; opacity: 1 }
				100% { transform: scaleX(1); transform-origin: 0 0; opacity: 1 }
			}
			@keyframes zc-interactivity-router-loading-bar-finish-animation {
				0% { opacity: 1 }
				50% { opacity: 1 }
				100% { opacity: 0 }
			}
CSS;
	 ...
	 *
	 * @since 6.5.0
	 * @deprecated 6.7.0 Use {@see ZC_Interactivity_API::print_router_markup} instead.
	 */
	public function print_router_loading_and_screen_reader_markup() {
		_deprecated_function( __METHOD__, '6.7.0', 'ZC_Interactivity_API::print_router_markup' );
	...
	/**
	 * Outputs markup for the @zelocorecms/interactivity-router script module.
	 *
	...
	public function print_router_markup() {
		echo <<<HTML
			<div
				class="zc-interactivity-router-loading-bar"
				data-zc-interactive="core/router/private"
				data-zc-class--start-animation="state.navigation.hasStarted"
				data-zc-class--finish-animation="state.navigation.hasFinished"
			></div>
HTML;
	}

	/**
	 * Processes the `data-zc-router-region` directive.
	 *
	 ...
	 * @param string                                    $mode            Whether the processing is entering or exiting the tag.
	 */
	private function data_zc_router_region_processor( ZC_Interactivity_API_Directives_Processor $p, string $mode ) {
		if ( 'enter' === $mode && ! $this->has_processed_router_region ) {
			...
			zc_register_style( 'zc-interactivity-router-animations', false );
			zc_add_inline_style( 'zc-interactivity-router-animations', $this->get_router_animation_styles() );
			zc_enqueue_style( 'zc-interactivity-router-animations' );

			// Adds the necessary markup to the footer.
			add_action( 'zc_footer', array( $this, 'print_router_markup' ) );
		}
	}

	/**
	 * Processes the `data-zc-each` directive.
	 *
	 ...
	 *
	 * @since 6.5.0
	 * @since 6.9.0 Include the list path in the rendered `data-zc-each-child` directives.
	 *
	 * @param ZC_Interactivity_API_Directives_Processor $p               The directives processor instance.
	 * @param string                                    $mode            Whether the processing is entering or exiting the tag.
	 * @param array                                     $tag_stack       The reference to the tag stack.
	 */
	private function data_zc_each_processor( ZC_Interactivity_API_Directives_Processor $p, string $mode, array &$tag_stack ) {
		if ( 'enter' === $mode && 'TEMPLATE' === $p->get_tag() ) {
			$entries = $this->get_directive_entries( $p, 'each' );
			if ( count( $entries ) > 1 || empty( $entries ) ) {
				// There should be only one `data-zc-each` directive per template tag.
				return;
			}
			$entry = $entries[0];
			...
			$template_end = 'data-zc-each: template end';
			$p->set_bookmark( $template_end );
			$p->next_tag();
			$manual_sdp = $p->get_attribute( 'data-zc-each-child' );
			...
			foreach ( $result as $item ) {
				...
				/*
				 * Adds the `data-zc-each-child` directive to each top-level tag
				 * rendered by this `data-zc-each` directive. The value is the
				 * `data-zc-each` directive's namespace and path.
				 *
				 * Nested `data-zc-each` directives could render
				 * `data-zc-each-child` elements at the top level as well, and
				 * they should be overwritten.
				 *
				 * @since 6.9.0
				 */
				$i = new ZC_Interactivity_API_Directives_Processor( $processed_item );
				while ( $i->next_tag() ) {
					$i->set_attribute( 'data-zc-each-child', $entry['namespace'] . '::' . $entry['value'] );
