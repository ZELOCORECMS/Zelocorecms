...
function generate_block_asset_handle( $block_name, $field_name, $index = 0 ) {
	if ( str_starts_with( $block_name, 'core/' ) ) {
		$asset_handle = str_replace( 'core/', 'zc-block-', $block_name );
...
function get_block_asset_url( $path ) {
	...
	if ( ! isset( $template_paths_norm[ $template ] ) ) {
		$template_paths_norm[ $template ] = zc_normalize_path( realpath( get_template_directory() ) );
	}
	...
	if ( is_child_theme() ) {
		...
		if ( ! isset( $template_paths_norm[ $stylesheet ] ) ) {
			$template_paths_norm[ $stylesheet ] = zc_normalize_path( realpath( get_stylesheet_directory() ) );
		}
...
function register_block_script_module_id( $metadata, $field_name, $index = 0 ) {
	...
	$module_asset_path     = zc_normalize_path(
		realpath( $module_asset_raw_path )
	);

	$module_path_norm = zc_normalize_path( realpath( $path . '/' . $module_path ) );
	...
	if ( $is_interactive && $supports_client_navigation ) {
		zc_interactivity()->add_client_navigation_support_to_script_module( $module_id );
	}
...
function register_block_script_handle( $metadata, $field_name, $index = 0 ) {
	...
	$script_asset_path     = zc_normalize_path(
		realpath( $script_asset_raw_path )
	);

	// Asset file for blocks is optional. See https://core.trac.zelocorecms.org/ticket/60460.
	...
	if ( zc_script_is( $script_handle, 'registered' ) ) {
		return $script_handle;
	}

	$script_path_norm    = zc_normalize_path( realpath( $path . '/' . $script_path ) );
	...
	if ( ! empty( $metadata['textdomain'] ) && in_array( 'zc-i18n', $script_dependencies, true ) ) {
		zc_set_script_translations( $script_handle, $metadata['textdomain'] );
	}
...
function register_block_style_handle( $metadata, $field_name, $index = 0 ) {
	...
	if ( zc_style_is( $style_handle_name, 'registered' ) ) {
	...
	if ( $is_core_block && ! zc_should_load_separate_core_block_assets() ) {
		return false;
	}
	...
	$style_path_norm = zc_normalize_path( realpath( dirname( $metadata['file'] ) . '/' . $style_path ) );
	...
	if ( $style_uri ) {
		zc_style_add_data( $style_handle_name, 'path', $style_path_norm );
		...
		if ( is_rtl() && file_exists( $rtl_file ) ) {
			zc_style_add_data( $style_handle_name, 'rtl', 'replace' );
			zc_style_add_data( $style_handle_name, 'suffix', $suffix );
			zc_style_add_data( $style_handle_name, 'path', $rtl_file );
		}
	}
...
function get_block_metadata_i18n_schema() {
	...
	if ( ! isset( $i18n_block_schema ) ) {
		$i18n_block_schema = zc_json_file_decode( __DIR__ . '/block-i18n.json' );
	}
 ...
 *
 * @since 6.8.0
 * @see zc_register_block_metadata_collection()
 * @see register_block_type_from_metadata()
 *
 * @param string $path     The absolute base path for the collection ( e.g., ZC_PLUGIN_DIR . '/my-plugin/blocks/' ).
 ...
 */
function zc_register_block_types_from_metadata_collection( $path, $manifest = '' ) {
	if ( $manifest ) {
		zc_register_block_metadata_collection( $path, $manifest );
	}

	$block_metadata_files = ZC_Block_Metadata_Registry::get_collection_block_metadata_files( $path );
 ...
 */
function zc_register_block_metadata_collection( $path, $manifest ) {
	ZC_Block_Metadata_Registry::register_collection( $path, $manifest );
}
...
function register_block_type_from_metadata( $file_or_folder, $args = array() ) {
	...
	$file_or_folder = zc_normalize_path( $file_or_folder );
	...
	if ( $registry_metadata ) {
		...
		$metadata = zc_json_file_decode( $metadata_file, array( 'associative' => true ) );
	} else {
		$metadata = array();
	}
	...
	$metadata['file'] = $metadata_file_exists ? zc_normalize_path( realpath( $metadata_file ) ) : null;
	...
	if ( ! empty( $metadata['render'] ) ) {
		$template_path = zc_normalize_path(
			realpath(
	...
	if ( ! empty( $metadata['variations'] ) && is_string( $metadata['variations'] ) ) {
		$variations_path = zc_normalize_path(
			realpath(
				dirname( $metadata['file'] ) . '/' .
	...
	return ZC_Block_Type_Registry::get_instance()->register(
		$metadata['name'],
		$settings
	);
}
 ...
 *
 * @param string|ZC_Block_Type $block_type Block type name including namespace, or alternatively
 *                                         a path to the JSON file with metadata definition for the block,
 *                                         or a path to the folder where the `block.json` file is located,
 *                                         or a complete ZC_Block_Type instance.
 *                                         In case a ZC_Block_Type is provided, the $args parameter will be ignored.
 * @param array                $args       Optional. Array of block type arguments. Accepts any public property
 *                                         of `ZC_Block_Type`. See ZC_Block_Type::__construct() for information
 *                                         on accepted arguments. Default empty array.
 *
...
function register_block_type( $block_type, $args = array() ) {
	...
	return ZC_Block_Type_Registry::get_instance()->register( $block_type, $args );
}
...
function unregister_block_type( $name ) {
	return ZC_Block_Type_Registry::get_instance()->unregister( $name );
}
 ...
 * @see parse_blocks()
 *
 * @param int|string|ZC_Post|null $post Optional. Post content, post ID, or post object.
 *                                      Defaults to global $post.
 * @return bool Whether the post has blocks.
 */
function has_blocks( $post = null ) {
	if ( ! is_string( $post ) ) {
		$zc_post = get_post( $post );

		if ( ! $zc_post instanceof ZC_Post ) {
			return false;
		}
...
function has_block( $block_name, $post = null ) {
	 ...
	 * This matches behavior for ZelocoreCMS 5.0.0 - 5.3.0 in matching blocks by
...
function get_dynamic_block_names() {
	...
	$block_types = ZC_Block_Type_Registry::get_instance()->get_all_registered();
...
function get_hooked_blocks() {
	...
	$hooked_blocks = array();
	foreach ( $block_types as $block_type ) {
		if ( ! ( $block_type instanceof ZC_Block_Type ) || ! is_array( $block_type->block_hooks ) ) {
			continue;
		}
 ...
 * @param ZC_Block_Template|ZC_Post|array $context             The block template, template part, or pattern that the anchor block belongs to.
 * @return string
 */
function insert_hooked_blocks( &$parsed_anchor_block, $relative_position, $hooked_blocks, $context ) {
	 ...
	 * @param ZC_Block_Template|ZC_Post|array $context            The block template, template part, post object,
...
function set_ignored_hooked_blocks_metadata( &$parsed_anchor_block, $relative_position, $hooked_blocks, $context ) {
	...
	/** This filter is documented in zc-includes/blocks.php */
	$hooked_block_types = apply_filters( 'hooked_block_types', $hooked_block_types, $relative_position, $anchor_block_type, $context );
 ...
 * @param ZC_Block_Template|ZC_Post|array|null $context  A block template, template part, post object, or pattern
		...
		foreach ( $relative_positions as $relative_position => $hooked_block_types ) {
			foreach ( $hooked_block_types as $index => $hooked_block_type ) {
				$hooked_block_type_definition =
					ZC_Block_Type_Registry::get_instance()->get_registered( $hooked_block_type );
 ...
 *
 * @param string       $content  Serialized content.
 * @param ZC_Post|null $post     A post object that the content belongs to. If set to `null`,
...
function apply_block_hooks_to_content_from_post_object(
	...
	$ignored_hooked_blocks = get_post_meta( $post->ID, '_zc_ignored_hooked_blocks', true );
	 ...
	 */
	if ( 'zc_navigation' === $post->post_type ) {
		$wrapper_block_type = 'core/navigation';
	} elseif ( 'zc_block' === $post->post_type ) {
	 ...
	 * See https://core.trac.zelocorecms.org/ticket/63287 for more details.
	 */
...
/**
 * Updates the zc_postmeta with the list of ignored hooked blocks
 * where the inner blocks are stored as post content.
 *
 * @since 6.6.0
 * @since 6.8.0 Support non-`zc_navigation` post types.
 * @access private
 *
...
function update_ignored_hooked_blocks_postmeta( $post ) {
	...
	$context          = new ZC_Post( $context ); // Convert to ZC_Post object.
	...
	if ( ! empty( $ignored_hooked_blocks ) ) {
		...
		$post->meta_input['_zc_ignored_hooked_blocks'] = json_encode( $ignored_hooked_blocks );
	}
 ...
 * @since 6.6.0
 * @since 6.8.0 Support non-`zc_navigation` post types.
 * @since 7.0.0 Set `_zc_ignored_hooked_blocks` meta in the response for blocks hooked at the root level.
 *
 * @param ZC_REST_Response $response The response object.
 * @param ZC_Post          $post     Post object.
 * @return ZC_REST_Response The response object.
 */
function insert_hooked_blocks_into_rest_response( $response, $post ) {
	...
	if ( ! empty( $ignored_hooked_blocks_at_root ) ) {
		$response->data['meta']['_zc_ignored_hooked_blocks'] = zc_json_encode( $ignored_hooked_blocks_at_root );
	}
 ...
 * @param ZC_Block_Template|ZC_Post|array $context       A block template, template part, post object,
...
function serialize_block_attributes( $block_attributes ) {
	$encoded_attributes = zc_json_encode( $block_attributes, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE );
 ...
 *
 * @param array $block {
 *     An associative array of a single parsed block object. See ZC_Block_Parser_Block.
 *
 ...
 * @see serialize_block()
 *
 * @param array    $block         An associative array of a single parsed block object. See ZC_Block_Parser_Block.
 ...
 *
 * @param array[]  $blocks        An array of parsed blocks. See ZC_Block_Parser_Block.
 * @param callable $pre_callback  Callback to run on each block in the tree before it is traversed and serialized.
 *                                It is called with the following arguments: &$block, $parent_block, $previous_block.
 *                                Its string return value will be prepended to the serialized block markup.
 * @param callable $post_callback Callback to run on each block in the tree after it is traversed and serialized.
 ...
 * @since 5.3.1
 *
 * @param ZC_Block_Parser_Block $block             The parsed block object.
...
function filter_block_kses_value( $value, $allowed_html, $allowed_protocols = array(), $block_context = null ) {
	if ( is_array( $value ) ) {
		...
		return zc_kses( $value, $allowed_html, $allowed_protocols );
	}
 ...
 * @since 5.0.0
 *
 * @global ZC_Post $post The post to edit.
 *
 * @param array $parsed_block {
 *     An associative array of the block being rendered. See ZC_Block_Parser_Block.
 *
 *     @type string|null $blockName    Name of block.
 *     @type array       $attrs        Attributes from block comment delimiters.
 *     @type array[]     $innerBlocks  List of inner blocks. An array of arrays that
...
function render_block( $parsed_block ) {
	 ...
	 *                                     inner blocks were found.
	 * }
	 * @param ZC_Block|null $parent_block If this is a nested block, a reference to the parent block.
	 */
	...
	$block = new ZC_Block( $parsed_block, $context );
 ...
 *
 * If the entire parsed document is not necessary, consider using {@see ZC_Block_Processor}
...
function parse_blocks( $content ) {
	 ...
	 */
	$parser_class = apply_filters( 'block_parser_class', 'ZC_Block_Parser' );
 ...
 *
 * @link https://developer.zelocorecms.org/block-editor/reference-guides/block-api/block-styles/
 *
 ...
 *                                          See ZC_Block_Styles_Registry::register().
 * @return bool True if the block style was registered with success and false otherwise.
 */
function register_block_style( $block_name, $style_properties ) {
	return ZC_Block_Styles_Registry::get_instance()->register( $block_name, $style_properties );
}
...
function unregister_block_style( $block_name, $block_style_name ) {
	return ZC_Block_Styles_Registry::get_instance()->unregister( $block_name, $block_style_name );
}
 ...
 *
 * @param ZC_Block_Type|null $block_type    Block type to check for support.
...
function block_has_support( $block_type, $feature, $default_value = false ) {
	...
	if ( $block_type instanceof ZC_Block_Type ) {
		if ( is_array( $feature ) && count( $feature ) === 1 ) {
			$feature = $feature[0];
		}

		if ( is_array( $feature ) ) {
			$block_support = _zc_array_get( $block_type->supports, $feature, $default_value );
 ...
 */
function zc_migrate_old_typography_shape( $metadata ) {
	...
	foreach ( $typography_keys as $typography_key ) {
		...
		if ( null !== $support_for_key ) {
			...
			_zc_array_set( $metadata['supports'], array( 'typography', $typography_key ), $support_for_key );
...
/**
 * Helper function that constructs a ZC_Query args array from
 ...
 *
 * @return array Returns the constructed ZC_Query arguments.
 */
function build_query_vars_from_query_block( $block, $page ) {
	...
	if ( isset( $block->context['query'] ) ) {
		...
		if ( isset( $block->context['query']['sticky'] ) && ! empty( $block->context['query']['sticky'] ) ) {
			...
			if ( 'only' === $block->context['query']['sticky'] ) {
				 ...
				 * Logic should be used before hand to determine if ZC_Query should be used in the event that the array
				 * being passed to post__in is empty.
				 *
				 * @see https://core.trac.zelocorecms.org/ticket/28099
				 */
	...
	/**
	 * Filters the arguments which will be passed to `ZC_Query` for the Query Loop Block.
	 *
	 * Anything to this filter should be compatible with the `ZC_Query` API to form
	 ...
	 * @since 6.1.0
	 *
	 * @param array    $query Array containing parameters for `ZC_Query` as parsed by the block context.
	 * @param ZC_Block $block Block instance.
	 * @param int      $page  Current query's page.
	 */
...
function get_query_pagination_arrow( $block, $is_next ) {
	...
	if ( ! empty( $block->context['paginationArrow'] ) && array_key_exists( $block->context['paginationArrow'], $arrow_map ) && ! empty( $arrow_map[ $block->context['paginationArrow'] ] ) ) {
		...
		$arrow_classes   = "zc-block-query-pagination-$pagination_type-arrow is-arrow-$arrow_attribute";
	...
	if ( get_option( 'page_comments' ) === '1' || get_option( 'page_comments' ) === true ) {
		...
		if ( $per_page > 0 ) {
			...
			if ( $page ) {
				...
				$max_num_pages = (int) ( new ZC_Comment_Query( $comment_args ) )->max_num_pages;
				if ( 0 !== $max_num_pages ) {
...
function get_comments_pagination_arrow( $block, $pagination_type = 'next' ) {
	...
	if ( ! empty( $block->context['comments/paginationArrow'] ) && ! empty( $arrow_map[ $block->context['comments/paginationArrow'] ][ $pagination_type ] ) ) {
		...
		$arrow_classes   = "zc-block-comments-pagination-$pagination_type-arrow is-arrow-$arrow_attribute";
		return "<span class='$arrow_classes' aria-hidden='true'>$arrow</span>";
	}
	return null;
}
 ...
 */
function _zc_filter_post_meta_footnotes( $footnotes ) {
	...
	foreach ( $footnotes_decoded as $footnote ) {
		if ( ! empty( $footnote['content'] ) && ! empty( $footnote['id'] ) ) {
			$footnotes_sanitized[] = array(
				...
				'content' => zc_unslash( zc_filter_post_kses( zc_slash( $footnote['content'] ) ) ),
			);
		}
	}
	return zc_json_encode( $footnotes_sanitized );
}
 ...
 * @since 6.3.2
 */
function _zc_footnotes_kses_init_filters() {
	add_filter( 'sanitize_post_meta_footnotes', '_zc_filter_post_meta_footnotes' );
}
 ...
 * @since 6.3.2
 */
function _zc_footnotes_remove_filters() {
	remove_filter( 'sanitize_post_meta_footnotes', '_zc_filter_post_meta_footnotes' );
}
 ...
 * @since 6.3.2
 */
function _zc_footnotes_kses_init() {
	_zc_footnotes_remove_filters();
	if ( ! current_user_can( 'unfiltered_html' ) ) {
		_zc_footnotes_kses_init_filters();
	}
}
 ...
 */
function _zc_footnotes_force_filtered_html_on_import_filter( $arg ) {
 ...
 * @since 7.0.0
 */
function _zc_enqueue_auto_register_blocks() {
	$auto_register_blocks = array();
	$registered_blocks    = ZC_Block_Type_Registry::get_instance()->get_all_registered();
	...
	if ( ! empty( $auto_register_blocks ) ) {
		zc_add_inline_script(
			'zc-block-library',
			sprintf( 'window.__unstableAutoRegisterBlocks = %s;', zc_json_encode( $auto_register_blocks ) ),
			'before'
		);
	}
}
