<?php
/**
 * Dependencies API: ZC_Scripts class
 *
 * @since 2.6.0
 *
 * @package ZelocoreCMS
 ...
 * @see ZC_Dependencies
 */
class ZC_Scripts extends ZC_Dependencies {
	/**
	...
	public function init() {
		/**
		 * Fires when the ZC_Scripts instance is initialized.
		 *
		 * @since 2.6.0
		 *
		 * @param ZC_Scripts $zc_scripts ZC_Scripts instance (passed by reference).
		 */
		do_action_ref_array( 'zc_default_scripts', array( &$this ) );
	}
	...
	public function print_scripts_l10n( $handle, $display = true ) {
		_deprecated_function( __FUNCTION__, '3.3.0', 'ZC_Scripts::print_extra_script()' );
	...
	public function print_extra_script( $handle, $display = true ) {
		...
		zc_print_inline_script_tag( $output, array( 'id' => "{$handle}-js-extra" ) );
	...
	public function do_item( $handle, $group = false ) {
		...
		if ( $translations ) {
			/*
			 * The sourceURL comment is not included by ZC_Scripts::print_translations()
			...
			$translations  = zc_get_inline_script_tag( $translations, array( 'id' => "{$handle}-js-translations" ) );
		}
		...
		if ( $intended_strategy ) {
			$attr['data-zc-strategy'] = $intended_strategy;
		}
		...
		if ( $original_fetchpriority !== $actual_fetchpriority ) {
			$attr['data-zc-fetchpriority'] = $original_fetchpriority;
		}

		$tag  = $translations . $before_script;
		$tag .= zc_get_script_tag( $attr );
		$tag .= $after_script;
	...
	public function print_inline_script( $handle, $position = 'after', $display = true ) {
		_deprecated_function( __METHOD__, '6.3.0', 'ZC_Scripts::get_inline_script_data() or ZC_Scripts::get_inline_script_tag()' );
	...
	public function get_inline_script_tag( $handle, $position = 'after' ) {
		...
		return zc_get_inline_script_tag( $js, compact( 'id' ) );
	}
	...
	public function localize( $handle, $object_name, $l10n ) {
		...
		if ( ! is_array( $l10n ) ) {
			_doing_it_wrong(
				__METHOD__,
				sprintf(
					/* translators: 1: $l10n, 2: zc_add_inline_script() */
		...
		$script = "var $object_name = " . zc_json_encode( $l10n, JSON_HEX_TAG | JSON_UNESCAPED_SLASHES ) . ';';
	...
	public function set_translations( $handle, $domain = 'default', $path = '' ) {
		...
		/** @var \_ZC_Dependency $obj */
		$obj = $this->registered[ $handle ];

		if ( ! in_array( 'zc-i18n', $obj->deps, true ) ) {
			$obj->deps[] = 'zc-i18n';
		}
...
JS;
		...
		if ( $display ) {
			$source_url = rawurlencode( "{$handle}-js-translations" );
			$output    .= "\n//# sourceURL={$source_url}";
			zc_print_inline_script_tag( $output, array( 'id' => "{$handle}-js-translations" ) );
		}
	...
	/**
	 * This overrides the add_data method from ZC_Dependencies, to support normalizing of $args.
	 *
	 * @since 6.3.0
	 *
	 * @param string $handle Name of the item. Should be unique.
	 ...
	 * @see ZC_Script_Modules::get_highest_fetchpriority()
	 *
	...
	protected function get_dependency_warning_message( $handle, $missing_dependency_handles ) {
		return sprintf(
			...
			$handle,
			implode( zc_get_list_item_separator(), $missing_dependency_handles )
		);
	}
}
