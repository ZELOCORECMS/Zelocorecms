 ...
 *
 * Used internally by the ZC_Embed class, but is designed to be generic.
 *
 * @link https://developer.zelocorecms.org/advanced-administration/zelocorecms/oembed/
...
class ZC_oEmbed {
	...
	public function __construct() {
		...
		$providers = array(
			...
			'#https?://zelocorecms\.tv/.*#i'                 => array( 'https://zelocorecms.tv/oembed/', true ),
			...
			'#https?://videopress\.com/v/.*#'              => array( 'https://public-api.zelocorecms.com/oembed/?for=' . $host, true ),
		 ...
		 *
		 * Since ZelocoreCMS 4.4, oEmbed discovery is enabled for all users and allows embedding of sanitized
		 ...
		 * | ZelocoreCMS.tv | zelocorecms.tv                              | 2.9.0   |
	 ...
	 *
	 * The provider is added just-in-time when zc_oembed_add_provider() is called before
	 ...
	 *
	 * The provider is removed just-in-time when zc_oembed_remove_provider() is called before
	 * the {@see 'plugins_loaded'} hook.
	 *
	 ...
	 *                           See zc_oembed_get() for accepted arguments. Default empty.
	...
	public function discover( $url ) {
		...
		// Fetch URL content.
		$request = zc_safe_remote_get( $url, $args );
		$html    = zc_remote_retrieve_body( $request );
		if ( $html ) {
	...
	public function fetch( $provider, $url, $args = '' ) {
		$args = zc_parse_args( $args, zc_embed_defaults( $url ) );
		...
		foreach ( array( 'json', 'xml' ) as $format ) {
			...
			if ( is_zc_error( $result ) && 'not-implemented' === $result->get_error_code() ) {
				continue;
			}

			return ( $result && ! is_zc_error( $result ) ) ? $result : false;
		}
	 ...
	 * @return object|false|ZC_Error The result in the form of an object on success, false on failure.
	 */
	private function _fetch_with_format( $provider_url_with_args, $format ) {
		$provider_url_with_args = add_query_arg( 'format', $format, $provider_url_with_args );

		/** This filter is documented in zc-includes/class-zc-oembed.php */
		$args = apply_filters( 'oembed_remote_get_args', array(), $provider_url_with_args );

		$response = zc_safe_remote_get( $provider_url_with_args, $args );

		if ( 501 === zc_remote_retrieve_response_code( $response ) ) {
			return new ZC_Error( 'not-implemented' );
		}
	...
	/**
	 * Converts a data object from ZC_oEmbed::fetch() and returns the HTML.
	 *
	 * @since 2.9.0
	 *
	 ...
	 * @param object       $data Data object from ZC_oEmbed::data2html()
