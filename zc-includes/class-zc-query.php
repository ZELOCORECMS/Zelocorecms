...
class ZC_Query {
	 ...
	 *
	 * @since 3.1.0
	 * @var ZC_Tax_Query|null A taxonomy query instance.
	 */
	public $tax_query;
	 ...
	 *
	 * @since 3.2.0
	 * @var ZC_Meta_Query A meta query instance.
	 */
	 ...
	 *
	 * @since 3.7.0
	 * @var ZC_Date_Query A date query instance.
	 */
	 ...
	 *
	 * @since 1.5.0
	 * @var ZC_Term|ZC_Post_Type|ZC_Post|ZC_User|null
	 */
	 ...
	 *
	 * The query flags are what page info ZelocoreCMS was able to figure out.
	 *
	 * @since 2.0.0
	 */
	 ...
	 *     @type array           $date_query             An associative array of ZC_Date_Query arguments.
	 *                                                   See ZC_Date_Query::__construct().
	 *     @type int             $day                    Day of the month. Default empty. Accepts numbers 1-31.
	 *     @type bool            $exact                  Whether to search by exact keyword. Default false.
	 *     @type string          $fields                 Post fields to query for. Accepts:
	 *                                                   - '' Returns an array of complete post objects (`ZC_Post[]`).
	 ...
	 *                                                   See ZC_Meta_Query::__construct() for accepted values and default value.
	 ...
	 *                                                   See ZC_Meta_Query::__construct() for accepted values and default value.
	 *     @type array           $meta_query             An associative array of ZC_Meta_Query arguments.
	 *                                                   See ZC_Meta_Query::__construct() for accepted values.
	 ...
	 *                                                   the 'zc_query_search_exclusion_prefix' filter.
	 ...
	 *     @type array           $tax_query              An associative array of ZC_Tax_Query arguments.
	 *                                                   See ZC_Tax_Query::__construct().
	...
	public function parse_query( $query = '' ) {
		if ( ! empty( $query ) ) {
			$this->init();
			$this->query      = zc_parse_args( $query );
		...
		if ( ( '' !== $query_vars['attachment'] ) || ! empty( $query_vars['attachment_id'] ) ) {
			...
			if ( $query_vars['day'] ) {
				if ( ! $this->is_date ) {
					...
					if ( $query_vars['monthnum'] && $query_vars['year'] && ! zc_checkdate( $query_vars['monthnum'], $query_vars['day'], $query_vars['year'], $date ) ) {
						$query_vars['error'] = '404';
					} else {
						$this->is_day  = true;
						$this->is_date = true;
		...
		if ( ! ( $this->is_singular || $this->is_archive || $this->is_search || $this->is_feed
				|| ( zc_is_serving_rest_request() && $this->is_main_query() )
		...
		if ( $this->is_home && 'page' === get_option( 'show_on_front' ) && get_option( 'page_on_front' ) ) {
			$_query = zc_parse_args( $this->query );
		...
		if ( '' !== $query_vars['pagename'] ) {
			...
			if ( isset( $this->queried_object_id ) && get_option( 'zc_page_for_privacy_policy' ) == $this->queried_object_id ) {
		...
		if ( $query_vars['page_id'] ) {
			...
			if ( get_option( 'zc_page_for_privacy_policy' ) == $query_vars['page_id'] ) {
				$this->is_privacy_policy = true;
			}
		}
	...
	public function parse_tax_query( &$query_vars ) {
		...
		foreach ( get_taxonomies( array(), 'objects' ) as $taxonomy => $t ) {
			...
			if ( $t->query_var && ! empty( $query_vars[ $t->query_var ] ) ) {
				...
				if ( is_string( $query_vars[ $t->query_var ] ) && ! empty( $t->rewrite['hierarchical'] ) ) {
					$query_vars[ $t->query_var ] = zc_basename( $query_vars[ $t->query_var ] );
				}
		...
		$this->tax_query = new ZC_Tax_Query( $tax_query );
		 ...
		 * @since 3.7.0
		 *
		 * @param ZC_Query $query The ZC_Query instance.
		 */
	...
	protected function parse_search( &$query_vars ) {
		...
		/**
		 * Filters the columns to search in a ZC_Query search.
		 *
		 ...
		 * @param ZC_Query $query          The current ZC_Query instance.
		 */
		 ...
		 */
		$exclusion_prefix = apply_filters( 'zc_query_search_exclusion_prefix', '-' );
	...
	protected function get_search_stopwords() {
		 ...
		 */
		$this->stopwords = apply_filters( 'zc_search_stopwords', $stopwords );
		return $this->stopwords;
	}
	...
	public function get_posts() {
		 ...
		 * like `is_main_query()` test against the global `$zc_query` instance, not the passed one.
		 *
		 * @since 2.0.0
		 *
		 * @param ZC_Query $query The ZC_Query instance (passed by reference).
		 ...
		 */
		$this->allow_query_attachment_by_filename = apply_filters( 'zc_allow_query_attachment_by_filename', false );
		remove_all_filters( 'zc_allow_query_attachment_by_filename' );

		// Parse meta query.
		$this->meta_query = new ZC_Meta_Query();
		...
		switch ( $query_vars['fields'] ) {
			...
			case '':
				 ...
				 *
				 * This is used in `ZC_Query::the_post` to determine if the
		...
		if ( $date_parameters ) {
			$date_query = new ZC_Date_Query( array( $date_parameters ) );
		...
		if ( ! empty( $query_vars['date_query'] ) ) {
			$this->date_query = new ZC_Date_Query( $query_vars['date_query'] );
		...
		if ( ! $query_vars['suppress_filters'] ) {
			/**
			 * Filters the search SQL that is used in the WHERE clause of ZC_Query.
			 *
			 * @since 3.0.0
			 *
			 * @param string   $search Search SQL for WHERE clause.
			 * @param ZC_Query $query  The current ZC_Query object.
			 */
		...
		if ( ! empty( $query_vars['author'] ) && '0' != $query_vars['author'] ) {
			$query_vars['author'] = zc_slash( '' . urldecode( $query_vars['author'] ) );
		...
		if ( ! empty( $query_vars['author__not_in'] ) ) {
			$author__not_in_id_list = zc_parse_id_list( $query_vars['author__not_in'] );
		...
		if ( empty( $query_vars['orderby'] ) ) {
			...
			if ( is_array( $query_vars['orderby'] ) ) {
				foreach ( $query_vars['orderby'] as $_orderby => $order ) {
					$orderby = zc_slash( urldecode( $_orderby ) );
				...
				$query_vars['orderby'] = zc_slash( $query_vars['orderby'] );
		...
		if ( $skip_post_status ) {
			...
			if ( ! empty( $queried_post_types ) ) {
				...
				foreach ( $queried_post_types as $queried_post_type ) {
					...
					if ( is_user_logged_in() && $queried_post_type_object instanceof ZC_Post_Type ) {
		...
		if ( $this->is_comment_feed && ! $this->is_singular ) {
			...
			$last_changed = array(
				zc_cache_get_last_changed( 'comment' ),
				zc_cache_get_last_changed( 'posts' ),
			);
		 ...
		 *
		 * See https://core.trac.zelocorecms.org/ticket/56841.
		 * See https://github.com/ZelocoreCMS/zelocorecms-develop/pull/6393#issuecomment-2088217429
		 */
		$old_request =
		 ...
		 *
		 * Return a non-null value to bypass ZelocoreCMS' default post queries.
		 *
		 * Filtering functions that require pagination information are encouraged to set
		 * the `found_posts` and `max_num_pages` properties of the ZC_Query object,
		 * passed to the filter by reference. If ZC_Query does not perform a database
		...
		if ( $query_vars['cache_results'] && $id_query_is_cacheable ) {
			...
			$cache_found = false;
			if ( null === $this->posts ) {
				$cached_results = zc_cache_get_salted( $cache_key, 'post-queries', $last_changed );
				...
				if ( $cached_results ) {
					...
					if ( 'ids' === $query_vars['fields'] ) {
						...
						/** @var int[] */
						$post_parents = zc_cache_get_multiple( $post_parent_cache_keys, 'posts' );
		...
		if ( 'ids' === $query_vars['fields'] ) {
			...
			if ( $query_vars['cache_results'] && $id_query_is_cacheable ) {
				...
				zc_cache_set_salted( $cache_key, $cache_value, 'post-queries', $last_changed );
			}
		...
		if ( 'id=>parent' === $query_vars['fields'] ) {
			...
			zc_cache_add_multiple( $post_parents_cache, 'posts' );
		...
		if ( $query_vars['cache_results'] && $id_query_is_cacheable && ! $cache_found ) {
			$post_ids = zc_list_pluck( $this->posts, 'ID' );
		...
		if ( ! empty( $this->posts ) && ( $this->is_single || $this->is_page ) ) {
			...
			if ( $this->is_preview && $this->posts && current_user_can( $edit_cap, $this->posts[0]->ID ) ) {
				 ...
				 * @since 2.7.0
				 *
				 * @param ZC_Post  $post_preview  The Post object.
				 * @param ZC_Query $query         The ZC_Query instance (passed by reference).
				 */
		 ...
		 * of the type ZC_Post and are filtered.
		 */
		if ( $this->posts ) {
			$this->post_count = count( $this->posts );
		...
		if ( $query_vars['lazy_load_term_meta'] ) {
			zc_queue_posts_for_term_meta_lazyload( $this->posts );
		}
	...
	/**
	 * Iterates current comment index and returns ZC_Comment object.
	 *
	 * @since 2.2.0
	 *
	 * @return ZC_Comment Comment object.
	 */
	...
	/**
	 * Sets up the ZelocoreCMS query by parsing query string.
	 *
	 * @since 1.5.0
	 *
	 ...
	 * @since 1.5.0
	 *
	 * @return ZC_Term|ZC_Post_Type|ZC_Post|ZC_User|null The queried object.
	 */
	 ...
	 * Constructor.
	 *
	 * Sets up the ZelocoreCMS query, if parameter is not empty.
	 *
	 * @since 1.5.0
	 *
	 * @see ZC_Query::parse_query() for all available arguments.
	 ...
	 * @since 3.1.0
	 *
	 * @see ZC_Query::is_category()
	 * @see ZC_Query::is_tag()
	 * @see ZC_Query::is_author()
	 * @see ZC_Query::is_date()
	 * @see ZC_Query::is_post_type_archive()
	 * @see ZC_Query::is_tax()
	 *
	...
	public function is_tax( $taxonomy = '', $term = '' ) {
		...
		$tax_array      = array_intersect( array_keys( $zc_taxonomies ), (array) $taxonomy );
	 ...
	 *
	 * Otherwise the same as {@see ZC_Query::is_home()}.
	 *
	 * @since 3.1.0
	 *
	 ...
	 *
	 * Depends on the site's "Change your Privacy Policy page" Privacy Settings 'zc_page_for_privacy_policy'.
	 *
	...
	public function is_privacy_policy() {
		if ( get_option( 'zc_page_for_privacy_policy' )
			&& $this->is_page( get_option( 'zc_page_for_privacy_policy' ) )
		) {
			return true;
		} else {
			return false;
	 ...
	 * @since 3.1.0
	 *
	 * @see ZC_Query::is_single()
	 * @see ZC_Query::is_singular()
	 *
	 ...
	 * @global int     $numpages
	 *
	 * @param ZC_Post|object|int $post ZC_Post instance or Post ID/object.
	...
	public function setup_postdata( $post ) {
		 ...
		 *
		 * @param ZC_Post  $post  The Post object (passed by reference).
		 * @param ZC_Query $query The current Query object (passed by reference).
		 */
	 ...
	 *
	 * @since 4.4.0
	 * @deprecated 4.5.0 See zc_queue_posts_for_term_meta_lazyload().
	 *
	 * @param mixed $check
	 * @param int   $term_id
	 * @return mixed
	 ...
	 *
	 * @since 4.4.0
	 * @deprecated 4.5.0 See zc_lazyload_comment_meta().
	 *
	 * @param mixed $check
