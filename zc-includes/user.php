 ...
 * true.
 *
 * Note: zc_signon() doesn't handle setting the current user. This means that if the
 * function is called before the {@see 'init'} hook is fired, is_user_logged_in() will
 * evaluate as false until that point. If is_user_logged_in() is needed in conjunction
 * with zc_signon(), zc_set_current_user() should be called explicitly.
 *
 * @since 2.5.0
 *
 ...
 * @return ZC_User|ZC_Error ZC_User on success, ZC_Error on failure.
 */
function zc_signon( $credentials = array(), $secure_cookie = '' ) {
	global $auth_secure_cookie, $zcdb;
	...
	if ( empty( $credentials ) ) {
		...
		if ( ! empty( $_POST['log'] ) && is_string( $_POST['log'] ) ) {
			$credentials['user_login'] = zc_unslash( $_POST['log'] );
		}
	 ...
	 * @since 1.5.1
	 *
	 * @todo Decide whether to deprecate the zc_authenticate action.
	 *
	 * @param string $user_login    Username (passed by reference).
	 * @param string $user_password User password (passed by reference).
	 */
	do_action_ref_array( 'zc_authenticate', array( &$credentials['user_login'], &$credentials['user_password'] ) );
	...
	// XXX ugly hack to pass this to zc_authenticate_cookie().
	$auth_secure_cookie = $secure_cookie;

	add_filter( 'authenticate', 'zc_authenticate_cookie', 30, 3 );

	$user = zc_authenticate( $credentials['user_login'], $credentials['user_password'] );

	if ( is_zc_error( $user ) ) {
		return $user;
	}

	zc_set_auth_cookie( $user->ID, $credentials['remember'], $secure_cookie );
	 ...
	 * @param ZC_User $user       ZC_User object of the logged-in user.
	 */
	do_action( 'zc_login', $user->user_login, $user );
 ...
 * @since 2.8.0
 *
 * @param ZC_User|ZC_Error|null $user     ZC_User or ZC_Error object from a previous callback. Default null.
 * @param string                $username Username for authentication.
 * @param string                $password Password for authentication.
 * @return ZC_User|ZC_Error ZC_User on success, ZC_Error on failure.
 */
function zc_authenticate_username_password(
	$user,
	$username,
	#[\SensitiveParameter]
	$password
	 ...
	 * @since 2.5.0
	 *
	 * @param ZC_User|ZC_Error $user     ZC_User or ZC_Error object if a previous
	 *                                   callback failed authentication.
	 * @param string           $password Password to check against the user.
	 */
	$user = apply_filters( 'zc_authenticate_user', $user, $password );
	if ( is_zc_error( $user ) ) {
		return $user;
	}

	$valid = zc_check_password( $password, $user->user_pass, $user->ID );
	...
	if ( ! $valid ) {
		return new ZC_Error(
			...
			' <a href="' . zc_lostpassword_url() . '">' .
			__( 'Lost your password?' ) .
			'</a>'
		);
	}

	if ( zc_password_needs_rehash( $user->user_pass, $user->ID ) ) {
		zc_set_password( $password, $user->ID );
	}
 ...
 * @since 4.5.0
 *
 * @param ZC_User|ZC_Error|null $user     ZC_User or ZC_Error object if a previous
 *                                        callback failed authentication.
 * @param string                $email    Email address for authentication.
 * @param string                $password Password for authentication.
 * @return ZC_User|ZC_Error ZC_User on success, ZC_Error on failure.
 */
function zc_authenticate_email_password(
	$user,
	$email,
	#[\SensitiveParameter]
	$password
	...
	if ( empty( $email ) || empty( $password ) ) {
		...
		if ( empty( $email ) ) {
			// Uses 'empty_username' for back-compat with zc_signon().
	...
	/** This filter is documented in zc-includes/user.php */
	$user = apply_filters( 'zc_authenticate_user', $user, $password );
...
/**
 * Authenticates the user using the ZelocoreCMS auth cookie.
 *
 * @since 2.8.0
 *
 * @global string $auth_secure_cookie
 ...
 * @return ZC_User|ZC_Error ZC_User on success, ZC_Error on failure.
 */
function zc_authenticate_cookie(
	$user,
	$username,
	#[\SensitiveParameter]
	$password
	...
	if ( empty( $username ) && empty( $password ) ) {
		$user_id = zc_validate_auth_cookie();
		if ( $user_id ) {
			return new ZC_User( $user_id );
		}
		...
		if ( ! empty( $_COOKIE[ $auth_cookie ] ) ) {
			return new ZC_Error( 'expired_session', __( 'Please log in again.' ) );
		}
 ...
 * @since 5.6.0
 *
 * @param ZC_User|ZC_Error|null $input_user ZC_User or ZC_Error object if a previous
 *                                          callback failed authentication.
 * @param string                $username   Username for authentication.
 * @param string                $password   Password for authentication.
 * @return ZC_User|ZC_Error|null ZC_User on success, ZC_Error on failure, null if
 *                               null is passed in and this isn't an API request.
 */
function zc_authenticate_application_password(
	$input_user,
	$username,
	#[\SensitiveParameter]
	$password
) {
	if ( $input_user instanceof ZC_User ) {
		return $input_user;
	}

	if ( ! ZC_Application_Passwords::is_in_use() ) {
		return $input_user;
	}
	...
	} elseif ( ! zc_is_application_passwords_available() ) {
		$error = new ZC_Error(
			'application_passwords_disabled',
			__( 'Application passwords are not available.' )
		);
	} elseif ( ! zc_is_application_passwords_available_for_user( $user ) ) {
		$error = new ZC_Error(
	...
	if ( $error ) {
		 ...
		 * @since 5.6.0
		 *
		 * @param ZC_Error $error The authentication error.
		 */
	...
	$hashed_passwords = ZC_Application_Passwords::get_user_application_passwords( $user->ID );

	foreach ( $hashed_passwords as $key => $item ) {
		if ( ! ZC_Application_Passwords::check_password( $password, $item['password'] ) ) {
			continue;
		}
		 ...
		 * @since 5.6.0
		 *
		 * @param ZC_Error $error    The error object.
		 * @param ZC_User  $user     The user authenticating.
		 * @param array    $item     The details about the application password.
		 * @param string   $password The raw supplied password.
		 */
		do_action( 'zc_authenticate_application_password_errors', $error, $user, $item, $password );

		if ( $error->has_errors() ) {
			/** This action is documented in zc-includes/user.php */
			do_action( 'application_password_failed_authentication', $error );

			return $error;
		}

		ZC_Application_Passwords::record_application_password_usage( $user->ID, $item['uuid'] );
		 ...
		 * @since 5.6.0
		 *
		 * @param ZC_User $user The user who was authenticated.
 ...
 */
function zc_validate_application_password( $input_user ) {
	// Don't authenticate twice.
	if ( ! empty( $input_user ) ) {
		return $input_user;
	}

	if ( ! zc_is_application_passwords_available() ) {
		return $input_user;
	}
	...
	$authenticated = zc_authenticate_application_password( null, $_SERVER['PHP_AUTH_USER'], $_SERVER['PHP_AUTH_PW'] );

	if ( $authenticated instanceof ZC_User ) {
 ...
 *
 * @param ZC_User|ZC_Error|null $user ZC_User or ZC_Error object from a previous callback. Default null.
 * @return ZC_User|ZC_Error ZC_User on success, ZC_Error if the user is considered a spammer.
 */
function zc_authenticate_spam_check( $user ) {
	if ( $user instanceof ZC_User && is_multisite() ) {
		/**
		 ...
		 * @param ZC_User $user    User to check against.
		 */
		$spammed = apply_filters( 'check_is_user_spammed', is_user_spammy( $user ), $user );

		if ( $spammed ) {
			return new ZC_Error( 'spammer_account', __( '<strong>Error:</strong> Your account has been marked as a spammer.' ) );
		}
	}
	return $user;
}
 ...
 */
function zc_validate_logged_in_cookie( $user_id ) {
	if ( $user_id ) {
		return $user_id;
	}
	...
	return zc_validate_auth_cookie( $_COOKIE[ LOGGED_IN_COOKIE ], 'logged_in' );
}
...
function count_user_posts( $userid, $post_type = 'post', $public_only = false ) {
	...
	$query = "SELECT COUNT(*) FROM $zcdb->posts $where";

	$last_changed = zc_cache_get_last_changed( 'posts' );
	$cache_key    = 'count_user_posts:' . md5( $query );
	$count        = zc_cache_get_salted( $cache_key, 'post-queries', $last_changed );
	if ( false === $count ) {
		$count = $zcdb->get_var( $query );
		zc_cache_set_salted( $cache_key, $count, 'post-queries', $last_changed );
	}
...
function count_many_users_posts( $users, $post_type = 'post', $public_only = false ) {
	...
	$query       = "SELECT post_author, COUNT(*) FROM $zcdb->posts $where AND post_author IN ($userlist) GROUP BY post_author";
	$cache_key   = 'count_many_users_posts:' . md5( $query );
	$cache_salts = array( zc_cache_get_last_changed( 'posts' ), zc_cache_get_last_changed( 'users' ) );
	$count       = zc_cache_get_salted( $cache_key, 'post-queries', $cache_salts );

	if ( false === $count ) {
		$result = $zcdb->get_results( $query, ARRAY_N );
		...
		zc_cache_set_salted( $cache_key, $count, 'post-queries', $cache_salts, HOUR_IN_SECONDS );
	}
...
function get_current_user_id() {
	if ( ! function_exists( 'zc_get_current_user' ) ) {
		return 0;
	}
...
function get_user_option( $option, $user = 0, $deprecated = '' ) {
	 ...
	 * @param ZC_User $user   ZC_User object of the user whose option is being retrieved.
	 */
 ...
 * it will prepend the ZelocoreCMS table prefix to the option name.
 *
...
function update_user_option( $user_id, $option_name, $newvalue, $is_global = false ) {
	...
	if ( ! $is_global ) {
		$option_name = $zcdb->get_blog_prefix() . $option_name;
	}
 ...
 * @param int $user_id User ID.
 *
 * @return ZC_User|false ZC_User object on success, false on failure.
 */
 ...
 * @see ZC_User_Query
 *
 * @param array $args Optional. Arguments to retrieve users. See ZC_User_Query::prepare_query()
 *                    for more information on accepted arguments.
 * @return array List of users.
 */
function get_users( $args = array() ) {

	$args                = zc_parse_args( $args );
	$args['count_total'] = false;

	$user_search = new ZC_User_Query( $args );
 ...
 */
function zc_list_users( $args = array() ) {
	$defaults = array(
		'orderby'       => 'name',
		'order'         => 'ASC',
		'number'        => '',
	...
	$query_args           = zc_array_slice_assoc( $parsed_args, array( 'orderby', 'order', 'number', 'exclude', 'include' ) );
	 ...
	 * @param array $parsed_args The arguments passed to zc_list_users() combined with the defaults.
	 */
	$query_args = apply_filters( 'zc_list_users_args', $query_args, $parsed_args );
...
function get_blogs_of_user( $user_id, $all = false ) {
	...
	if ( isset( $keys[ $zcdb->base_prefix . 'capabilities' ] ) && defined( 'MULTISITE' ) ) {
		$site_ids[] = 1;
		unset( $keys[ $zcdb->base_prefix . 'capabilities' ] );
	}
	...
	foreach ( $keys as $key ) {
		...
		if ( $zcdb->base_prefix && ! str_starts_with( $key, $zcdb->base_prefix ) ) {
			continue;
		}
		$site_id = str_replace( array( $zcdb->base_prefix, '_capabilities' ), '', $key );
...
function is_user_member_of_blog( $user_id = 0, $blog_id = 0 ) {
	...
	if ( 1 === $blog_id ) {
		$capabilities_key = $zcdb->base_prefix . 'capabilities';
	} else {
		$capabilities_key = $zcdb->base_prefix . $blog_id . '_capabilities';
	}
 ...
 * @since 3.0.0
 *
 * @link https://developer.zelocorecms.org/reference/functions/delete_user_meta/
 *
 * @param int    $user_id    User ID
 * @param string $meta_key   Metadata name.
 ...
 * @since 3.0.0
 *
 * @link https://developer.zelocorecms.org/reference/functions/get_user_meta/
 *
 * @param int    $user_id User ID.
 ...
 * @since 3.0.0
 *
 * @link https://developer.zelocorecms.org/reference/functions/update_user_meta/
 *
 * @param int    $user_id    User ID.
 ...
 * Assumes role names are unique phrases. Same assumption made by ZC_User_Query::prepare_query()
 * Using $strategy = 'time' this is CPU-intensive and should handle around 10^7 users.
 * Using $strategy = 'memory' this is memory-intensive and should handle around 10^5 users, but see ZC Bug #12257.
 *
 * @since 3.0.0
...
function count_users( $strategy = 'time', $site_id = null ) {
	...
	$blog_prefix = $zcdb->get_blog_prefix( $site_id );
	$result      = array();

	if ( 'time' === $strategy ) {
		if ( is_multisite() && get_current_blog_id() !== $site_id ) {
			switch_to_blog( $site_id );
			$avail_roles = zc_roles()->get_names();
		...
		foreach ( $avail_roles as $this_role => $name ) {
			$select_count[] = $zcdb->prepare( 'COUNT(NULLIF(`meta_value` LIKE %s, false))', '%' . $zcdb->esc_like( '"' . $this_role . '"' ) . '%' );
		}
		...
		$row = $zcdb->get_row(
			...
			FROM {$zcdb->usermeta}
			INNER JOIN {$zcdb->users} ON user_id = ID
 ...
 * @since 6.0.0 Moved to zc-includes/user.php.
 *
 ...
 */
function zc_maybe_update_user_counts( $network_id = null ) {
	if ( ! is_multisite() && null !== $network_id ) {
		_doing_it_wrong(
			__FUNCTION__,
			sprintf(
	...
	$is_small_network = ! zc_is_large_user_count( $network_id );
	/** This filter is documented in zc-includes/ms-functions.php */
	if ( ! apply_filters( 'enable_live_network_counts', $is_small_network, 'users' ) ) {
		return false;
	}

	return zc_update_user_counts( $network_id );
}
 ...
 * @return bool Whether the update was successful.
 */
function zc_update_user_counts( $network_id = null ) {
	global $zcdb;
	...
	$query = "SELECT COUNT(ID) as c FROM $zcdb->users";
	if ( is_multisite() ) {
 ...
 * @since 6.0.0
 */
function zc_schedule_update_user_counts() {
	if ( ! is_main_site() ) {
		return;
	}

	if ( ! zc_next_scheduled( 'zc_update_user_counts' ) && ! zc_installing() ) {
		zc_schedule_event( time(), 'twicedaily', 'zc_update_user_counts' );
	}
}
 ...
 */
function zc_is_large_user_count( $network_id = null ) {
	if ( ! is_multisite() && null !== $network_id ) {
		_doing_it_wrong(
			__FUNCTION__,
			sprintf(
	 ...
	 */
	return apply_filters( 'zc_is_large_user_count', $count > 10000, $count, $network_id );
}
 ...
 *
 * Used by zc_set_current_user() for back compat. Might be deprecated in the future.
 *
 * @since 2.0.4
 *
 * @global string  $user_login    The user username for logging in
 * @global ZC_User $userdata      User data.
 ...
 *     See ZC_User_Query::prepare_query() for additional available arguments.
 *
 ...
 */
function zc_dropdown_users( $args = '' ) {
	$defaults = array(
		'show_option_all'         => '',
		'show_option_none'        => '',
	...
	$query_args = zc_array_slice_assoc(
		$parsed_args,
		array(
			'blog_id',
			'include',
	 ...
	 *
	 * @param array $query_args  The query arguments for get_users().
	 * @param array $parsed_args The arguments passed to zc_dropdown_users() combined with the defaults.
	 */
	$query_args = apply_filters( 'zc_dropdown_users_args', $query_args, $parsed_args );
	...
	/**
	 * Filters the zc_dropdown_users() HTML output.
	 *
	 * @since 2.3.0
	 *
	 * @param string $output HTML output generated by zc_dropdown_users().
	 */
	$html = apply_filters( 'zc_dropdown_users', $output );
 ...
 * @since 3.0.0
 *
 * @param object|ZC_User $user User object or database row to be cached
...
function update_user_caches( $user ) {
	...
	zc_cache_add( $user->ID, $user, 'users' );
	zc_cache_add( $user->user_login, $user->ID, 'userlogins' );
	zc_cache_add( $user->user_nicename, $user->ID, 'userslugs' );

	if ( ! empty( $user->user_email ) ) {
		zc_cache_add( $user->user_email, $user->ID, 'useremail' );
	}
}
 ...
 *
 * @param ZC_User|int $user User object or ID to be cleaned from the cache
 */
function clean_user_cache( $user ) {
	if ( is_numeric( $user ) ) {
		$user = new ZC_User( $user );
	}
	...
	zc_cache_delete( $user->ID, 'users' );
	zc_cache_delete( $user->user_login, 'userlogins' );
	zc_cache_delete( $user->user_nicename, 'userslugs' );

	if ( ! empty( $user->user_email ) ) {
		zc_cache_delete( $user->user_email, 'useremail' );
	}

	zc_cache_delete( $user->ID, 'user_meta' );
	zc_cache_set_users_last_changed();
 ...
 *              methods for new installations. See zc_get_user_contact_methods().
 ...
 * @global zcdb $zcdb ZelocoreCMS database abstraction object.
 *
 * @param array|object|ZC_User $userdata {
 *     An array, object, or ZC_User object of user data arguments.
 *
 ...
 *                                        Default empty.
 * }
 * @return int|ZC_Error The newly created user's ID or a ZC_Error object if the user could not
 *                      be created.
 */
function zc_insert_user( $userdata ) {
	global $zcdb;

	if ( $userdata instanceof stdClass ) {
		$userdata = get_object_vars( $userdata );
	} elseif ( $userdata instanceof ZC_User ) {
	...
	if ( ! empty( $userdata['ID'] ) ) {
		...
		if ( ! $old_user_data ) {
			return new ZC_Error( 'invalid_user_id', __( 'Invalid user ID.' ) );
		}

		// Slash current user email to compare it later with slashed new user email.
		$old_user_data->user_email = zc_slash( $old_user_data->user_email );

		// Hashed in zc_update_user(), plaintext if called directly.
		...
		// Hash the password.
		$user_pass = zc_hash_password( $userdata['user_pass'] );
	}
	...
	if ( empty( $user_login ) ) {
		return new ZC_Error( 'empty_user_login', __( 'Cannot create a user with an empty login name.' ) );
	} elseif ( mb_strlen( $user_login ) > 60 ) {
		return new ZC_Error( 'user_login_too_long', __( 'Username may not be longer than 60 characters.' ) );
	}

	if ( ! $update && username_exists( $user_login ) ) {
		return new ZC_Error( 'existing_user_login', __( 'Sorry, that username already exists!' ) );
	}
	...
	if ( in_array( strtolower( $user_login ), array_map( 'strtolower', $illegal_logins ), true ) ) {
		return new ZC_Error( 'invalid_username', __( 'Sorry, that username is not allowed.' ) );
	}
	...
	if ( empty( $user_nicename ) ) {
		return new ZC_Error( 'empty_user_nicename', __( 'Cannot create a user with an empty nicename.' ) );
	} elseif ( mb_strlen( $user_nicename ) > 50 ) {
		return new ZC_Error( 'user_nicename_too_long', __( 'Nicename may not be longer than 50 characters.' ) );
	}

	$user_nicename_check = $zcdb->get_var( $zcdb->prepare( "SELECT ID FROM $zcdb->users WHERE user_nicename = %s AND user_login != %s LIMIT 1", $user_nicename, $user_login ) );
	...
	if ( $user_nicename_check ) {
		...
		while ( $user_nicename_check ) {
			...
			$user_nicename_check = $zcdb->get_var( $zcdb->prepare( "SELECT ID FROM $zcdb->users WHERE user_nicename = %s AND user_login != %s LIMIT 1", $alt_user_nicename, $user_login ) );
			++$suffix;
		}
	...
	if ( ( ! $update || ( ! empty( $old_user_data ) && 0 !== strcasecmp( $user_email, $old_user_data->user_email ) ) )
		&& ! defined( 'ZC_IMPORTING' )
		&& email_exists( $user_email )
	) {
		return new ZC_Error( 'existing_user_email', __( 'Sorry, that email address is already used!' ) );
	}
	...
	if ( mb_strlen( $user_url ) > 100 ) {
		return new ZC_Error( 'user_url_too_long', __( 'User URL may not be longer than 100 characters.' ) );
	}
	...
	if ( ! empty( $userdata['spam'] ) && ! is_multisite() ) {
		return new ZC_Error( 'no_spam', __( 'Sorry, marking a user as spam is only supported on Multisite.' ) );
	}
	 ...
	 * @param array    $userdata The raw array of data passed to zc_insert_user().
	 */
	$data = apply_filters( 'zc_pre_insert_user_data', $data, $update, ( $update ? $user_id : null ), $userdata );

	if ( empty( $data ) || ! is_array( $data ) ) {
		return new ZC_Error( 'empty_data', __( 'Not enough data to create this user.' ) );
	}
	...
	if ( $update ) {
		...
		$zcdb->update( $zcdb->users, $data, array( 'ID' => $user_id ) );
	} else {
		$zcdb->insert( $zcdb->users, $data );
		$user_id = (int) $zcdb->insert_id;
	}

	$user = new ZC_User( $user_id );

	if ( ! $update ) {
		/** This action is documented in zc-includes/pluggable.php */
		do_action( 'zc_set_password', $userdata['user_pass'], $user_id, $user );
	}
	 ...
	 *
	 * Does not include contact methods. These are added using `zc_get_user_contact_methods( $user )`.
	 *
	...
	foreach ( zc_get_user_contact_methods( $user ) as $key => $value ) {
	...
	if ( $update ) {
		 ...
		 *
		 * @param int     $user_id       User ID.
		 * @param ZC_User $old_user_data Object containing user's data prior to update.
		 * @param array   $userdata      The raw array of data passed to zc_insert_user().
		 */
 ...
 * @since 2.0.0
 *
 * @see zc_insert_user() For what fields can be set in $userdata.
 *
 * @param array|object|ZC_User $userdata An array of user data or a user object of type stdClass or ZC_User.
 * @return int|ZC_Error The updated user's ID or a ZC_Error object if the user could not be updated.
 */
function zc_update_user( $userdata ) {
	if ( $userdata instanceof stdClass ) {
		$userdata = get_object_vars( $userdata );
	} elseif ( $userdata instanceof ZC_User ) {
		$userdata = $userdata->to_array();
	...
	if ( ! empty( $userdata['user_pass'] ) && $userdata['user_pass'] !== $user_obj->user_pass ) {
		...
		$userdata['user_pass'] = zc_hash_password( $userdata['user_pass'] );

		/** This action is documented in zc-includes/pluggable.php */
		do_action( 'zc_set_password', $plaintext_pass, $user_id, $user_obj );
		 ...
		 * @since 4.3.0
		 *
		 * @see zc_insert_user() For `$user` and `$userdata` fields.
		 *
	...
	$user_id  = zc_insert_user( $userdata );

	if ( is_zc_error( $user_id ) ) {
		return $user_id;
	}

	$blog_name = zc_specialchars_decode( get_option( 'blogname' ), ENT_QUOTES );
...
###SITEURL###'
		...
		zc_mail( $pass_change_email['to'], sprintf( $pass_change_email['subject'], $blog_name ), $pass_change_email['message'], $pass_change_email['headers'] );
	}
...
###SITEURL###'
		...
		zc_mail( $email_change_email['to'], sprintf( $email_change_email['subject'], $blog_name ), $email_change_email['message'], $email_change_email['headers'] );
	}
	...
	$current_user = zc_get_current_user();
	if ( $current_user->ID === $user_id ) {
		if ( isset( $plaintext_pass ) ) {
			 ...
			 */
			$logged_in_cookie = zc_parse_auth_cookie( '', 'logged_in' );
			/** This filter is documented in zc-includes/pluggable.php */
			$default_cookie_life = apply_filters( 'auth_cookie_expiration', ( 2 * DAY_IN_SECONDS ), $user_id, false );

			zc_clear_auth_cookie();
			...
			zc_set_auth_cookie( $user_id, $remember, '', $token );
		}
	}
	 ...
	 */
	do_action( 'zc_update_user', $user_id, $userdata, $userdata_raw );
 ...
 * complex user creation use zc_insert_user() to specify more information.
 *
 * @since 2.0.0
 *
 * @see zc_insert_user() More complete way to create a new user.
 *
 ...
 *                      be created.
 */
function zc_create_user(
	$username,
	#[\SensitiveParameter]
	$password,
	$email = ''
) {
	$user_login = zc_slash( $username );
	$user_email = zc_slash( $email );
	$user_pass  = $password;

	$userdata = compact( 'user_login', 'user_email', 'user_pass' );
	return zc_insert_user( $userdata );
}

/**
 * Returns a list of meta keys to be (maybe) populated in zc_update_user().
 *
 ...
 * @access private
 *
 * @param ZC_User $user ZC_User instance.
 * @return string[] List of user keys to be populated in zc_update_user().
 */
function _get_additional_user_keys( $user ) {
	...
	return array_merge( $keys, array_keys( zc_get_user_contact_methods( $user ) ) );
}

/**
 * Sets up the user contact methods.
 *
 * Default contact methods were removed for new installations in ZelocoreCMS 3.6
 * and completely removed from the codebase in ZelocoreCMS 6.9.
 *
 ...
 *
 * @param ZC_User|null $user Optional. ZC_User object.
 * @return string[] Array of contact method labels keyed by contact method.
 */
function zc_get_user_contact_methods( $user = null ) {
	$methods = array();
	 ...
	 * @param ZC_User|null $user    ZC_User object or null if none was provided.
	 */
 ...
 *
 * Use zc_get_user_contact_methods() instead.
 *
 * @since 2.9.0
 * @access private
 *
 * @param ZC_User|null $user Optional. ZC_User object. Default null.
 * @return string[] Array of contact method labels keyed by contact method.
 */
function _zc_get_user_contactmethods( $user = null ) {
	return zc_get_user_contact_methods( $user );
}
 ...
 * @since 4.4.0
 *
 * @param ZC_User $user User to retrieve password reset key for.
 * @return string|ZC_Error Password reset key on success. ZC_Error on error.
 */
function get_password_reset_key( $user ) {
	if ( ! ( $user instanceof ZC_User ) ) {
		return new ZC_Error( 'invalidcombo', __( '<strong>Error:</strong> There is no account with that username or email address.' ) );
	}
	...
	$password_reset_allowed = zc_is_password_reset_allowed_for_user( $user );
	if ( ! $password_reset_allowed ) {
		return new ZC_Error( 'no_password_reset', __( 'Password reset is not allowed for this user' ) );
	} elseif ( is_zc_error( $password_reset_allowed ) ) {
		return $password_reset_allowed;
	}

	// Generate something random for a password reset key.
	$key = zc_generate_password( 20, false );
	...
	$hashed = time() . ':' . zc_fast_hash( $key );

	$key_saved = zc_update_user(
		array(
			'ID'                  => $user->ID,
 ...
 * but have a different ZC_Error code so good user feedback can be provided.
 *
 * @since 3.1.0
 *
 * @param string $key       The password reset key.
 * @param string $login     The user login.
 * @return ZC_User|ZC_Error ZC_User object on success, ZC_Error object for invalid or expired keys.
 */
function check_password_reset_key(
	...
	if ( empty( $key ) || ! is_string( $key ) ) {
		return new ZC_Error( 'invalid_key', __( 'Invalid key.' ) );
	}
	...
	$hash_is_correct = zc_verify_fast_hash( $key, $pass_key );

	if ( $hash_is_correct && $expiration_time && time() < $expiration_time ) {
		return $user;
	} elseif ( $hash_is_correct && $expiration_time ) {
		// Key has an expiration time that's passed.
		return new ZC_Error( 'expired_key', __( 'Invalid key.' ) );
	}
	...
	if ( hash_equals( $user->user_activation_key, $key ) || ( $hash_is_correct && ! $expiration_time ) ) {
		 ...
		 *
		 * @param ZC_Error $return  A ZC_Error object denoting an expired key.
		 *                          Return a ZC_User object to validate the key.
 ...
 * @return true|ZC_Error True when finished, ZC_Error object on error.
 */
function retrieve_password( $user_login = '' ) {
	...
	$user_login = trim( zc_unslash( $user_login ) );
	 ...
	 * @since 5.7.0
	 *
	 * @param ZC_User|false $user_data ZC_User object if found, false if the user does not exist.
	 * @param ZC_Error      $errors    A ZC_Error object containing any errors generated
	 ...
	 *
	 * The filtered ZC_Error object may, for example, contain errors for an invalid
	 * username or email address. A ZC_Error object should always be returned,
	 ...
	 * @param bool    $send       Whether to send the email.
	 * @param string  $user_login The username for the user.
	 * @param ZC_User $user_data  ZC_User object.
	 */
	 ...
	 *
	 * @see https://core.trac.zelocorecms.org/tickets/42957
	 */
	$message .= network_site_url( 'zc-login.php?login=' . rawurlencode( $user_login ) . "&key=$key&action=rp", 'login' ) . '&zc_lang=' . $locale . "\r\n\r\n";
	 ...
	 *     The default notification email arguments. Used to build zc_mail().
	 *
	...
	$subject = zc_specialchars_decode( $subject );

	if ( ! zc_mail( $to, $subject, $message, $headers ) ) {
		$errors->add(
			'retrieve_password_email_failure',
			sprintf(
				/* translators: %s: Documentation URL. */
				__( '<strong>Error:</strong> The email could not be sent. Your site may not be correctly configured to send emails. <a href="%s">Get support for resetting your password</a>.' ),
				esc_url( __( 'https://zelocorecms.org/documentation/article/reset-your-password/' ) )
			)
		);
		return $errors;
	}
...
function reset_password(
	...
	zc_set_password( $new_pass, $user->ID );
 ...
 * @return int|ZC_Error Either user's ID or error on failure.
 */
function register_new_user( $user_login, $user_email ) {
	 ...
	 * @param ZC_Error $errors               Contains any errors with submitted username and email,
	 ...
	 *
	 * The filtered ZC_Error object may, for example, contain errors for an invalid
	 * or existing username or email address. A ZC_Error object should always be returned,
	 * but may or may not contain errors.
	 *
	 ...
	 * @since 2.1.0
	 *
	 * @param ZC_Error $errors               A ZC_Error object containing any errors encountered
	 *                                       during registration.
	...
	$user_pass = zc_generate_password( 12, false );
	$user_id   = zc_create_user( $sanitized_user_login, $user_pass, $user_email );
	if ( ! $user_id || is_zc_error( $user_id ) ) {
		$errors->add(
			'registerfail',
			sprintf(
	...
	if ( ! empty( $_COOKIE['zc_lang'] ) ) {
		$zc_lang = sanitize_text_field( $_COOKIE['zc_lang'] );
		if ( in_array( $zc_lang, get_available_languages(), true ) ) {
			update_user_meta( $user_id, 'locale', $zc_lang ); // Set user locale if defined on registration.
		}
	}
 ...
 *                        Default 'both'.
 */
function zc_send_new_user_notifications( $user_id, $notify = 'both' ) {
	zc_new_user_notification( $user_id, null, $notify );
}
 ...
 * @return string Token.
 */
function zc_get_session_token() {
	$cookie = zc_parse_auth_cookie( '', 'logged_in' );
 ...
 */
function zc_get_all_sessions() {
	$manager = ZC_Session_Tokens::get_instance( get_current_user_id() );
 ...
 * @since 4.0.0
 */
function zc_destroy_current_session() {
	$token = zc_get_session_token();
	if ( $token ) {
		$manager = ZC_Session_Tokens::get_instance( get_current_user_id() );
 ...
 * @since 4.0.0
 */
function zc_destroy_other_sessions() {
	$token = zc_get_session_token();
	if ( $token ) {
		$manager = ZC_Session_Tokens::get_instance( get_current_user_id() );
 ...
 * @since 4.0.0
 */
function zc_destroy_all_sessions() {
	$manager = ZC_Session_Tokens::get_instance( get_current_user_id() );
	$manager->destroy_all();
}
 ...
 */
function zc_get_users_with_no_role( $site_id = null ) {
	global $zcdb;
	...
	$prefix = $zcdb->get_blog_prefix( $site_id );

	if ( is_multisite() && get_current_blog_id() !== $site_id ) {
		switch_to_blog( $site_id );
		$role_names = zc_roles()->get_names();
		restore_current_blog();
	} else {
		$role_names = zc_roles()->get_names();
	}
 ...
 *
 * This function is used by the pluggable functions zc_get_current_user() and
 ...
 * @access private
 *
 * @see zc_get_current_user()
 * @global ZC_User $current_user Checks if the current user is set.
 *
 * @return ZC_User Current ZC_User instance.
 */
function _zc_get_current_user() {
	global $current_user;

	if ( ! empty( $current_user ) ) {
		if ( $current_user instanceof ZC_User ) {
			return $current_user;
		}

		// Upgrade stdClass to ZC_User.
		if ( is_object( $current_user ) && isset( $current_user->ID ) ) {
			$cur_id       = $current_user->ID;
			$current_user = null;
			zc_set_current_user( $cur_id );
			return $current_user;
		}

		// $current_user has a junk value. Force to ZC_User with ID 0.
		$current_user = null;
		zc_set_current_user( 0 );
		return $current_user;
	}
 ...
 *
 * @since 3.0.0
 * @since 4.9.0 This function was moved from zc-admin/includes/ms.php so it's no longer Multisite specific.
 *
 * @global ZC_Error $errors ZC_Error object.
 */
function send_confirmation_on_profile_email() {
	...
	if ( $current_user->user_email !== $_POST['email'] ) {
		...
		$hash           = md5( $_POST['email'] . time() . zc_rand() );
		...
		$sitename = zc_specialchars_decode( get_option( 'blogname' ), ENT_QUOTES );
...
###SITEURL###'
		...
		zc_mail( $_POST['email'], sprintf( __( '[%s] Email Change Request' ), $sitename ), $content );
...
function new_user_email_admin_notice() {
	...
	if ( 'profile.php' === $pagenow && isset( $_GET['updated'] ) ) {
		...
		if ( $email ) {
			...
			zc_admin_notice( $message, array( 'type' => 'info' ) );
		}
	}
}
 ...
 */
function _zc_privacy_action_request_types() {
	return array(
 ...
 */
function zc_register_user_personal_data_exporter( $exporters ) {
	$exporters['zelocorecms-user'] = array(
		'exporter_friendly_name' => __( 'ZelocoreCMS User' ),
		'callback'               => 'zc_user_personal_data_exporter',
	);
 ...
 * }
 */
function zc_user_personal_data_exporter( $email_address ) {
	 ...
	 * }
	 * @param ZC_User  $user           The user whose data is being exported.
	 ...
	 */
	$_extra_data = apply_filters( 'zc_privacy_additional_user_profile_data', array(), $user, $reserved_names );
	...
	if ( is_array( $_extra_data ) && ! empty( $_extra_data ) ) {
		...
		if ( count( $extra_data ) !== count( $_extra_data ) ) {
			_doing_it_wrong(
				__FUNCTION__,
				sprintf(
					/* translators: %s: zc_privacy_additional_user_profile_data */
					__( 'Filter %s returned items with reserved names.' ),
					'<code>zc_privacy_additional_user_profile_data</code>'
				),
				'5.4.0'
			);
		}
	...
	if ( isset( $user_meta['community-events-location'] ) ) {
		...
		$data_to_export[] = array(
			...
			'group_description' => __( 'User&#8217;s location data used for the Community Events in the ZelocoreCMS Events and News dashboard widget.' ),
 ...
 */
function _zc_privacy_account_request_confirmed( $request_id ) {
	...
	update_post_meta( $request_id, '_zc_user_request_confirmed_timestamp', time() );
	zc_update_post(
		array(
			'ID'          => $request_id,
 ...
 */
function _zc_privacy_send_request_confirmation_notification( $request_id ) {
	$request = zc_get_user_request( $request_id );

	if ( ! ( $request instanceof ZC_User_Request ) || 'request-confirmed' !== $request->status ) {
		return;
	}

	$already_notified = (bool) get_post_meta( $request_id, '_zc_admin_notified', true );
	...
	$action_description = zc_user_request_action_description( $request->action_name );
	 ...
	 * @param ZC_User_Request $request     The request that is initiating the notification.
	 */
	 ...
	 *
	 *     @type ZC_User_Request $request     User request object.
...
###SITEURL###'
	...
	$email_sent = zc_mail( $email_data['admin_email'], $subject, $content, $headers );

	if ( $email_sent ) {
		update_post_meta( $request_id, '_zc_admin_notified', true );
	}
}
 ...
 */
function _zc_privacy_send_erasure_fulfillment_notification( $request_id ) {
	$request = zc_get_user_request( $request_id );

	if ( ! ( $request instanceof ZC_User_Request ) || 'request-completed' !== $request->status ) {
		return;
	}

	$already_notified = (bool) get_post_meta( $request_id, '_zc_user_notified', true );
...
###SITEURL###'
	...
	$email_sent = zc_mail( $user_email, $subject, $content, $headers );
	...
	if ( $email_sent ) {
		update_post_meta( $request_id, '_zc_user_notified', true );
	}
}
 ...
 */
function _zc_privacy_account_request_confirmed_message( $request_id ) {
	$request = zc_get_user_request( $request_id );
	...
	if ( $request && in_array( $request->action_name, _zc_privacy_action_request_types(), true ) ) {
		if ( 'export_personal_data' === $request->action_name ) {
 ...
 * @return int|ZC_Error                   Returns the request ID if successful, or a ZC_Error object on failure.
 */
function zc_create_user_request( $email_address = '', $action_name = '', $request_data = array(), $status = 'pending' ) {
	$email_address = sanitize_email( $email_address );
	$action_name   = sanitize_key( $action_name );

	if ( ! is_email( $email_address ) ) {
		return new ZC_Error( 'invalid_email', __( 'Invalid email address.' ) );
	}

	if ( ! in_array( $action_name, _zc_privacy_action_request_types(), true ) ) {
		return new ZC_Error( 'invalid_action', __( 'Invalid action name.' ) );
	}

	if ( ! in_array( $status, array( 'pending', 'confirmed' ), true ) ) {
		return new ZC_Error( 'invalid_status', __( 'Invalid request status.' ) );
	}

	$user    = get_user_by( 'email', $email_address );
	$user_id = $user && ! is_zc_error( $user ) ? $user->ID : 0;

	// Check for duplicates.
	$requests_query = new ZC_Query(
		array(
	...
	if ( $requests_query->found_posts ) {
		return new ZC_Error( 'duplicate_request', __( 'An incomplete personal data request for this email address already exists.' ) );
	}

	$request_id = zc_insert_post(
		array(
			'post_author'   => $user_id,
			'post_name'     => $action_name,
			'post_title'    => $email_address,
			'post_content'  => zc_json_encode( $request_data ),
 ...
 */
function zc_user_request_action_description( $action_name ) {
	switch ( $action_name ) {
 ...
 * @since 4.9.6
 *
 * @param int $request_id ID of the request created via zc_create_user_request().
 * @return true|ZC_Error True on success, `ZC_Error` on failure.
 */
function zc_send_user_request( $request_id ) {
	$request_id = absint( $request_id );
	$request    = zc_get_user_request( $request_id );

	if ( ! $request ) {
		return new ZC_Error( 'invalid_request', __( 'Invalid personal data request.' ) );
	}
	 ...
	 * See https://core.trac.zelocorecms.org/ticket/44940
	 */
	$request->confirm_key = zc_generate_user_request_key( $request_id );

	$email_data = array(
		'request'     => $request,
		'email'       => $request->email,
		'description' => zc_user_request_action_description( $request->action_name ),
...
###SITEURL###'
	...
	$email_sent = zc_mail( $email_data['email'], $subject, $content, $headers );
	...
	if ( ! $email_sent ) {
		return new ZC_Error( 'privacy_email_error', __( 'Unable to send personal data export confirmation email.' ) );
	}
 ...
 */
function zc_generate_user_request_key( $request_id ) {
	...
	zc_update_post(
		array(
			...
			'post_password' => zc_fast_hash( $key ),
		)
	);
 ...
 * @return true|ZC_Error True on success, ZC_Error on failure.
 */
function zc_validate_user_request_key(
	$request_id,
	#[\SensitiveParameter]
	$key
) {
	...
	if ( ! in_array( $request->status, array( 'request-pending', 'request-failed' ), true ) ) {
		return new ZC_Error( 'expired_request', __( 'This personal data request has expired.' ) );
	}

	if ( empty( $key ) ) {
		return new ZC_Error( 'missing_key', __( 'The confirmation key is missing from this personal data request.' ) );
	}
	...
	if ( ! zc_verify_fast_hash( $key, $saved_key ) ) {
		return new ZC_Error( 'invalid_key', __( 'The confirmation key is invalid for this personal data request.' ) );
	}

	if ( ! $expiration_time || time() > $expiration_time ) {
		return new ZC_Error( 'expired_key', __( 'The confirmation key has expired for this personal data request.' ) );
	}
 ...
 */
function zc_get_user_request( $request_id ) {
	$request_id = absint( $request_id );
	...
	return new ZC_User_Request( $post );
}
 ...
 * but may be made available using the {@see 'zc_is_application_passwords_available'} filter.
 *
 * @since 5.9.0
 *
 * @return bool
 */
function zc_is_application_passwords_supported() {
	return is_ssl() || 'local' === zc_get_environment_type();
}
 ...
 * Use the {@see 'zc_is_application_passwords_available'} filter to adjust its availability.
 *
 * @since 5.6.0
 *
 * @return bool
 */
function zc_is_application_passwords_available() {
	/**
	 ...
	 */
	return apply_filters( 'zc_is_application_passwords_available', zc_is_application_passwords_supported() );
}

/**
 * Checks if Application Passwords is available for a specific user.
 *
 * By default all users can use Application Passwords. Use {@see 'zc_is_application_passwords_available_for_user'}
 * to restrict availability to certain users.
 *
 * @since 5.6.0
 *
 * @param int|ZC_User $user The user to check.
 * @return bool
 */
function zc_is_application_passwords_available_for_user( $user ) {
	if ( ! zc_is_application_passwords_available() ) {
		return false;
	}
	 ...
	 *
	 * @param bool    $available True if available, false otherwise.
	 * @param ZC_User $user      The user to check.
	 */
	return apply_filters( 'zc_is_application_passwords_available_for_user', true, $user );
}
 ...
 * @global zcdb $zcdb ZelocoreCMS database abstraction object.
 */
function zc_register_persisted_preferences_meta() {
	/*
 ...
 * @since 6.3.0
 */
function zc_cache_set_users_last_changed() {
	zc_cache_set_last_changed( 'users' );
}
 ...
 *
 * @param int|ZC_User $user The user to check.
 * @return bool|ZC_Error True if allowed, false or ZC_Error otherwise.
 */
function zc_is_password_reset_allowed_for_user( $user ) {
	if ( ! is_object( $user ) ) {
		$user = get_userdata( $user );
	}
