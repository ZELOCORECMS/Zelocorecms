<?php
/**
 * Script Modules API: ZC_Script_Modules class.
 *
...
class ZC_Script_Modules {
	...
	/**
	 * Tracks whether the @zelocorecms/a11y script module is available.
	 *
	 ...
	 * {@see ZC_Script_Modules::print_script_module_translations()}.
	 *
	 * @since 7.0.0
	 *
	 * @param string $id     The identifier of the script module.
	...
	public function print_script_module_translations(): void {
		...
		foreach ( $module_ids as $id ) {
			...
			$output    = sprintf(
				...
				zc_json_encode( $domain, JSON_HEX_TAG | JSON_UNESCAPED_SLASHES ),
				$json_translations
			);
			...
			zc_print_inline_script_tag( $output, array( 'id' => $script_id ) );
		}
	}
	 ...
	 * when the `zc_head` actions is fired, so it needs to print everything in the
	 * footer.
	 *
	 * @since 6.5.0
	 */
	public function add_hooks() {
		...
		$position       = $is_block_theme ? 'zc_head' : 'zc_footer';
		...
		if ( $is_block_theme ) {
			 ...
			 */
			add_action( 'zc_head', array( $this, 'print_head_enqueued_script_modules' ) );
		}
		add_action( 'zc_footer', array( $this, 'print_enqueued_script_modules' ) );
		...
		/*
		 * Print translations after classic scripts like zc-i18n are loaded (at
		 * priority 10 via _zc_footer_scripts), but before the script modules
		 ...
		 */
		add_action( 'zc_footer', array( $this, 'print_script_module_translations' ), 21 );
		add_action( 'admin_print_footer_scripts', array( $this, 'print_script_module_translations' ), 11 );

		add_action( 'zc_footer', array( $this, 'print_script_module_data' ) );
		add_action( 'admin_print_footer_scripts', array( $this, 'print_script_module_data' ) );
		add_action( 'zc_footer', array( $this, 'print_a11y_script_module_html' ), 20 );
	...
	private function print_script_module( string $id ) {
		...
		if ( $fetchpriority !== $script_module['fetchpriority'] ) {
			$attributes['data-zc-fetchpriority'] = $script_module['fetchpriority'];
		}
		zc_print_script_tag( $attributes );
	}
	...
	public function print_script_module_preloads() {
		...
		foreach ( $dependency_ids as $id ) {
			...
			if ( $highest_fetchpriority !== $this->registered[ $id ]['fetchpriority'] && 'auto' !== $this->registered[ $id ]['fetchpriority'] ) {
				printf( ' data-zc-fetchpriority="%s"', esc_attr( $this->registered[ $id ]['fetchpriority'] ) );
			}
			echo ">\n";
		}
	}
	...
	public function print_import_map() {
		...
		if ( ! empty( $import_map['imports'] ) ) {
			zc_print_inline_script_tag(
				(string) zc_json_encode( $import_map, JSON_HEX_TAG | JSON_UNESCAPED_SLASHES ),
				array(
					'type' => 'importmap',
					'id'   => 'zc-importmap',
				)
	...
	private function get_import_map(): array {
		...
		if ( $zc_scripts instanceof ZC_Scripts ) {
			...
			while ( ! empty( $handles ) ) {
				...
				if ( isset( $processed[ $handle ] ) || ! isset( $zc_scripts->registered[ $handle ] ) ) {
					continue;
				}
				$processed[ $handle ] = true;

				$module_dependencies = $zc_scripts->get_data( $handle, 'module_dependencies' );
				if ( is_array( $module_dependencies ) ) {
					...
					foreach ( $module_dependencies as $module ) {
						if ( is_string( $module ) ) {
							...
							// Normally, this error scenario would be caught when ZC_Scripts::add_data() is called.
							continue;
						}
					...
					if ( count( $missing_module_dependencies ) > 0 ) {
						_doing_it_wrong(
							...
							sprintf(
								...
								$handle,
								'module_dependencies',
								implode( zc_get_list_item_separator(), $missing_module_dependencies )
							),
							'7.0.0'
						);
					}
				...
				foreach ( $zc_scripts->registered[ $handle ]->deps as $dep ) {
	...
	private function get_dependents( string $id ): array {
		...
		foreach ( $this->registered as $registered_id => $args ) {
			if ( in_array( $id, zc_list_pluck( $args['dependencies'], 'id' ), true ) ) {
			...
			if ( ! in_array( $id, $this->modules_with_missing_dependencies, true ) ) {
				_doing_it_wrong(
					...
					sprintf(
						...
						$id,
						implode( zc_get_list_item_separator(), $missing_dependencies )
					),
					'6.9.1'
				);
	 ...
	 * ZelocoreCMS version. If $version is set to null, no version is added.
	...
	public function print_script_module_data(): void {
		...
		foreach ( array_keys( $modules ) as $module_id ) {
			 ...
			 * tag with an ID of the form `zc-script-module-data-{$module_id}`.
			 *
			 * The data can be read on the client with a pattern like this:
			 *
			 * Example:
			 *
			 *     const dataContainer = document.getElementById( 'zc-script-module-data-MyScriptModuleID' );
			 *     let data = {};
			 *     if ( dataContainer ) {
			 *         try {
			...
			if ( is_array( $data ) && array() !== $data ) {
				...
				zc_print_inline_script_tag(
					(string) zc_json_encode(
						$data,
						$json_encode_flags
					),
					array(
						'type' => 'application/json',
						'id'   => "zc-script-module-data-{$module_id}",
					)
				);
			}
		}
