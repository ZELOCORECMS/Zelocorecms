...
/**
 * Taxonomy API: ZC_Term_Query class.
 *
 * @package ZelocoreCMS
 * @subpackage Taxonomy
 * @since 4.6.0
 ...
 * @since 4.6.0
 *
 * @see ZC_Term_Query::__construct() for accepted arguments.
 */
...
class ZC_Term_Query {
	 ...
	 *                                                   - 'all' Returns an array of complete term objects (`ZC_Term[]`).
	 *                                                   - 'all_with_object_id' Returns an array of term objects
	 *                                                     with the 'object_id' param (`ZC_Term[]`). Works only
	 ...
	 * @since 4.6.0
	 *
	 * @param string|array $query ZC_Term_Query arguments. See ZC_Term_Query::__construct() for accepted arguments.
	 */
	public function parse_query( $query = '' ) {
		...
		$query = zc_parse_args( $query, $this->query_var_defaults );
		 ...
		 * @since 4.6.0
		 *
		 * @param ZC_Term_Query $query Current instance of ZC_Term_Query.
		 */
	 ...
	 * ZC_Term_Query::get_terms() for details.
	 *
	 * @since 4.6.0
	 *
	 ...
	 *
	 * The following will result in an array of `ZC_Term` objects being returned:
	 *
	 *   - 'all'
	 *   - 'all_with_object_id'
	 *
	...
	public function get_terms() {
		 ...
		 * @since 4.6.0
		 *
		 * @param ZC_Term_Query $query Current instance of ZC_Term_Query (passed by reference).
		 */
		...
		if ( ! empty( $include ) ) {
			$exclude      = '';
			$exclude_tree = '';
			$inclusions   = implode( ',', zc_parse_id_list( $include ) );
		...
		if ( ! empty( $exclude_tree ) ) {
			$exclude_tree      = zc_parse_id_list( $exclude_tree );
		...
		if ( ! empty( $exclude ) ) {
			$exclusions = array_merge( zc_parse_id_list( $exclude ), $exclusions );
		}
		 ...
		 *
		 * Return a non-null value to bypass ZelocoreCMS' default term queries.
		 *
		 * @since 5.3.0
		 *
			...
			if ( false !== $cache ) {
				if ( 'ids' === $_fields ) {
					...
					|| ( 'all' === $_fields && $args['pad_counts'] || $fields_is_filtered )
						...
						$term_ids = zc_list_pluck( $cache, 'term_id' );
					} else {
		...
		if ( empty( $terms ) ) {
			...
			if ( $args['cache_results'] ) {
				zc_cache_set_salted( $cache_key, $this->terms, 'term-queries', $last_changed );
			}
				...
				if ( ! $term->count ) {
					...
					if ( is_array( $children ) ) {
						foreach ( $children as $child_id ) {
							...
							if ( $child instanceof ZC_Term && $child->count ) {
								continue 2;
							}
						}
					}
		...
		if ( $args['update_term_meta_cache'] ) {
			$term_ids = zc_list_pluck( $term_objects, 'term_id' );
		...
		if ( 'all_with_object_id' === $_fields && ! empty( $args['object_ids'] ) ) {
			...
			$term_cache = zc_list_pluck( $term_objects, 'term_id' );
		}

		if ( $args['cache_results'] ) {
			zc_cache_set_salted( $cache_key, $term_cache, 'term-queries', $last_changed );
		}
	...
	protected function parse_orderby( $orderby_raw ) {
		...
		if ( in_array( $_orderby, array( 'term_id', 'name', 'slug', 'term_group' ), true ) ) {
			...
			$include = implode( ',', zc_parse_id_list( $this->query_vars['include'] ) );
	 ...
	 * @since 6.0.0
	 *
	 * @param ZC_Term[] $term_objects Array of term objects.
	 * @param string    $_fields      Field to format.
	 *
	 * @return ZC_Term[]|int[]|string[] Array of terms / strings / ints depending on field requested.
	 */
	 ...
	 * @since 4.6.0
	 *
	 * @param string $orderby_raw Raw 'orderby' value passed to ZC_Term_Query.
	 ...
	 * @return ZC_Term[] Array of `ZC_Term` objects.
	 */
