...
use ZelocoreCMS\AiClientDependencies\Psr\SimpleCache\CacheInterface;

/**
 * ZelocoreCMS-specific PSR-16 cache adapter for the AI Client.
 *
 * Bridges PSR-16 cache operations to ZelocoreCMS object cache functions,
 * enabling the AI client to leverage ZelocoreCMS caching infrastructure.
 *
 * @since 7.0.0
 * @internal Intended only to wire up the PHP AI Client SDK to ZelocoreCMS's caching system.
 * @access private
 */
class ZC_AI_Client_Cache implements CacheInterface {
	 ...
	 * @var string
	 */
	private const CACHE_GROUP = 'zc_ai_client';
	...
	public function get( $key, $default_value = null ) {
		$found = false;
		$value = zc_cache_get( $key, self::CACHE_GROUP, false, $found );
	...
	public function set( $key, $value, $ttl = null ): bool {
		...
		return zc_cache_set( $key, $value, self::CACHE_GROUP, $expire );
	}
	...
	public function delete( $key ): bool {
		return zc_cache_delete( $key, self::CACHE_GROUP );
	}
	...
	public function clear(): bool {
		if ( ! function_exists( 'zc_cache_supports' ) || ! zc_cache_supports( 'flush_group' ) ) {
			return false;
		}

		return zc_cache_flush_group( self::CACHE_GROUP );
	}
	...
	public function getMultiple( $keys, $default_value = null ): array {
		...
		$values     = zc_cache_get_multiple( $keys_array, self::CACHE_GROUP );
		$result     = array();
	...
	public function setMultiple( $values, $ttl = null ): bool {
		...
		$expire       = $this->ttl_to_seconds( $ttl );
		$results      = zc_cache_set_multiple( $values_array, self::CACHE_GROUP, $expire );
	...
	public function deleteMultiple( $keys ): bool {
		$keys_array = $this->iterable_to_array( $keys );
		$results    = zc_cache_delete_multiple( $keys_array, self::CACHE_GROUP );
	...
	public function has( $key ): bool {
		$found = false;
		zc_cache_get( $key, self::CACHE_GROUP, false, $found );

		return (bool) $found;
	}

	/**
	 * Converts a PSR-16 TTL value to seconds for ZelocoreCMS cache functions.
	 *
	 * @since 7.0.0
	 *
