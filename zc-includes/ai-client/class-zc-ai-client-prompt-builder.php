<?php
/**
 * WP AI Client: ZC_AI_Client_Prompt_Builder class
 *
 * @package ZelocoreCMS
 * @subpackage AI
 * @since 7.0.0
...
use ZelocoreCMS\AiClient\Builders\PromptBuilder;
use ZelocoreCMS\AiClient\Common\Exception\InvalidArgumentException;
use ZelocoreCMS\AiClient\Common\Exception\TokenLimitReachedException;
use ZelocoreCMS\AiClient\Files\DTO\File;
use ZelocoreCMS\AiClient\Files\Enums\FileTypeEnum;
use ZelocoreCMS\AiClient\Files\Enums\MediaOrientationEnum;
use ZelocoreCMS\AiClient\Messages\DTO\Message;
use ZelocoreCMS\AiClient\Messages\DTO\MessagePart;
use ZelocoreCMS\AiClient\Messages\Enums\ModalityEnum;
use ZelocoreCMS\AiClient\Providers\Http\DTO\RequestOptions;
use ZelocoreCMS\AiClient\Providers\Http\Exception\ClientException;
use ZelocoreCMS\AiClient\Providers\Http\Exception\NetworkException;
use ZelocoreCMS\AiClient\Providers\Http\Exception\ServerException;
use ZelocoreCMS\AiClient\Providers\Models\Contracts\ModelInterface;
use ZelocoreCMS\AiClient\Providers\Models\DTO\ModelConfig;
use ZelocoreCMS\AiClient\Providers\Models\Enums\CapabilityEnum;
use ZelocoreCMS\AiClient\Providers\ProviderRegistry;
use ZelocoreCMS\AiClient\Results\DTO\GenerativeAiResult;
use ZelocoreCMS\AiClient\Tools\DTO\FunctionDeclaration;
use ZelocoreCMS\AiClient\Tools\DTO\FunctionResponse;
use ZelocoreCMS\AiClient\Tools\DTO\WebSearch;

/**
 * Fluent builder for constructing AI prompts, returning ZC_Error on failure.
 *
 ...
 * PromptBuilder and adds ZelocoreCMS-specific behavior including ZC_Error
 * handling instead of exceptions, snake_case method naming, and integration
 * with the Abilities API.
 *
 * Only the generating methods will return a ZC_Error, to not break the fluent
 ...
 * when a generating method is called, the ZC_Error will be returned.
 *
 * @since 7.0.0
 *
 ...
 * @method bool|ZC_Error is_supported(?CapabilityEnum $capability = null) Checks if the prompt is supported for the given capability.
 ...
 * @method GenerativeAiResult|ZC_Error generate_result(?CapabilityEnum $capability = null) Generates a result from the prompt.
 * @method GenerativeAiResult|ZC_Error generate_text_result() Generates a text result from the prompt.
 * @method GenerativeAiResult|ZC_Error generate_image_result() Generates an image result from the prompt.
 * @method GenerativeAiResult|ZC_Error generate_speech_result() Generates a speech result from the prompt.
 * @method GenerativeAiResult|ZC_Error convert_text_to_speech_result() Converts text to speech and returns the result.
 * @method GenerativeAiResult|ZC_Error generate_video_result() Generates a video result from the prompt.
 * @method string|ZC_Error generate_text() Generates text from the prompt.
 * @method list<string>|ZC_Error generate_texts(?int $candidateCount = null) Generates multiple text candidates from the prompt.
 * @method File|ZC_Error generate_image() Generates an image from the prompt.
 * @method list<File>|ZC_Error generate_images(?int $candidateCount = null) Generates multiple images from the prompt.
 * @method File|ZC_Error convert_text_to_speech() Converts text to speech.
 * @method list<File>|ZC_Error convert_text_to_speeches(?int $candidateCount = null) Converts text to multiple speech outputs.
 * @method File|ZC_Error generate_speech() Generates speech from the prompt.
 * @method list<File>|ZC_Error generate_speeches(?int $candidateCount = null) Generates multiple speech outputs from the prompt.
 * @method File|ZC_Error generate_video() Generates a video from the prompt.
 * @method list<File>|ZC_Error generate_videos(?int $candidateCount = null) Generates multiple videos from the prompt.
 */
class ZC_AI_Client_Prompt_Builder {
	...
	/**
	 * ZelocoreCMS error instance, if any error occurred during method calls.
	 *
	 * @since 7.0.0
	 * @var ZC_Error|null
	 */
	private ?ZC_Error $error = null;
	...
	public function __construct( ProviderRegistry $registry, $prompt = null ) {
		try {
			...
			$this->error   = $this->exception_to_zc_error( $e );
		}
		 ...
		 */
		$filtered_default_timeout = apply_filters( 'zc_ai_client_default_request_timeout', $default_timeout );
		if ( is_numeric( $filtered_default_timeout ) && (float) $filtered_default_timeout >= 0.0 ) {
			...
			_doing_it_wrong(
				__METHOD__,
				sprintf(
					/* translators: %s: zc_ai_client_default_request_timeout */
					__( 'The %s filter must return a non-negative number.' ),
					'<code>zc_ai_client_default_request_timeout</code>'
				),
				'7.0.0'
			);
		}
	...
	/**
	 * Registers ZelocoreCMS abilities as function declarations for the AI model.
	 *
	 * Converts each ZC_Ability to a FunctionDeclaration using the wpab__ prefix
	 * naming convention and passes them to the underlying prompt builder.
	 *
	 * @since 7.0.0
	 *
	 * @param ZC_Ability|string ...$abilities The abilities to register, either as ZC_Ability objects or ability name strings.
	...
	public function using_abilities( ...$abilities ): self {
		...
		foreach ( $abilities as $ability ) {
			if ( is_string( $ability ) ) {
				...
				$ability      = zc_get_ability( $ability );
				if ( ! $ability ) {
					_doing_it_wrong(
						__METHOD__,
						sprintf(
			...
			if ( ! $ability instanceof ZC_Ability ) {
				continue;
			}

			$function_name = ZC_AI_Client_Ability_Function_Resolver::ability_name_to_function_name( $ability->get_name() );
	 ...
	 *
	 * This allows ZelocoreCMS developers to use snake_case naming conventions. It catches
	 * any exceptions thrown, stores them, and returns a ZC_Error when a terminate method
	 * is called.
	 *
	 * @since 7.0.0
	 *
	...
	public function __call( string $name, array $arguments ) {
		...
		if ( self::is_support_check_method( $name ) || self::is_generating_method( $name ) ) {
			...
			if ( ! $prevent ) {
				 ...
				 * @param ZC_AI_Client_Prompt_Builder $builder A clone of the prompt builder instance (read-only).
				 */
				$prevent = (bool) apply_filters( 'zc_ai_client_prevent_prompt', false, clone $this );
			}
			...
			if ( $prevent ) {
				...
				// For generate_* and convert_text_to_speech* methods, create a ZC_Error.
				$this->error = new ZC_Error(
					'prompt_prevented',
					$error_message,
					array(
						'status' => 503,
	...
	/**
	 * Converts an exception into a ZC_Error with a structured error code and message.
	 *
	 * This method maps different exception types to specific ZC_Error codes and HTTP status codes.
	 * The presence of the status codes means these ZC_Error objects can be easily used in REST API responses
	 * or other contexts where HTTP semantics are relevant.
	 *
	 * @since 7.0.0
	 *
	 * @param Exception $e The exception to convert.
	 * @return ZC_Error The resulting ZC_Error object.
	 */
	private function exception_to_zc_error( Exception $e ): ZC_Error {
		...
		if ( ! is_callable( $method ) ) {
			throw new BadMethodCallException(
				sprintf(
					...
					$name, // phpcs:ignore ZelocoreCMS.Security.EscapeOutput.ExceptionNotEscaped
					get_class( $this->builder ) // phpcs:ignore ZelocoreCMS.Security.EscapeOutput.ExceptionNotEscaped
				)
			);
		}
