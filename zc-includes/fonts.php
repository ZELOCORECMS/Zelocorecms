 ...
 * }
 */
function zc_print_font_faces( $fonts = array() ) {

	if ( empty( $fonts ) ) {
		$fonts = ZC_Font_Face_Resolver::get_fonts_from_theme_json();
	}
	...
	$zc_font_face = new ZC_Font_Face();
	$zc_font_face->generate_and_print( $fonts );
}
 ...
 *
 */
function zc_print_font_faces_from_style_variations() {
	$fonts = ZC_Font_Face_Resolver::get_fonts_from_style_variations();

	if ( empty( $fonts ) ) {
		return;
	}

	zc_print_font_faces( $fonts );
}
 ...
 * }
 * @return ZC_Font_Collection|ZC_Error A font collection if it was registered
 *                                     successfully, or ZC_Error object on failure.
 */
function zc_register_font_collection( string $slug, array $args ) {
	return ZC_Font_Library::get_instance()->register_font_collection( $slug, $args );
}
 ...
 */
function zc_unregister_font_collection( string $slug ) {
	return ZC_Font_Library::get_instance()->unregister_font_collection( $slug );
}

/**
 * Retrieves font uploads directory information.
 *
 * Same as zc_font_dir() but "light weight" as it doesn't attempt to create the font uploads directory.
 ...
 * @see zc_font_dir()
 *
 * @return array See zc_font_dir() for description.
 */
function zc_get_font_dir() {
	return zc_font_dir( false );
}
 ...
 * }
 */
function zc_font_dir( $create_dir = true ) {
	/*
	...
	$font_dir = zc_upload_dir( null, $create_dir, false );
 ...
 */
function _zc_filter_font_directory( $font_dir ) {
 ...
 */
function _zc_after_delete_font_family( $post_id, $post ) {
	if ( 'zc_font_family' !== $post->post_type ) {
		return;
	}
	...
	$font_faces_ids = get_children(
		array(
			'post_parent' => $post_id,
			'post_type'   => 'zc_font_face',
			'fields'      => 'ids',
		)
	);

	foreach ( $font_faces_ids as $font_faces_id ) {
		zc_delete_post( $font_faces_id, true );
	}
}
 ...
 * @param ZC_Post $post    Post object.
 */
function _zc_before_delete_font_face( $post_id, $post ) {
	if ( 'zc_font_face' !== $post->post_type ) {
		return;
	}

	$font_files = get_post_meta( $post_id, '_zc_font_face_file', false );
	$font_dir   = untrailingslashit( zc_get_font_dir()['basedir'] );

	foreach ( $font_files as $font_file ) {
		zc_delete_file( $font_dir . '/' . $font_file );
	}
}
 ...
 * @since 6.5.0
 */
function _zc_register_default_font_collections() {
	zc_register_font_collection(
		'google-fonts',
		array(
			...
			'font_families' => 'https://s.w.org/images/fonts/zc-7.0/collections/google-fonts-with-preview.json',
			'categories'    => array(
				array(
