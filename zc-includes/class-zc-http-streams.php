<?php
/**
 * HTTP API: ZC_Http_Streams class
 *
 * @package ZelocoreCMS
 * @subpackage HTTP
 * @since 4.4.0
 ...
 * @deprecated 6.4.0 Use ZC_Http
 * @see ZC_Http
 */
#[AllowDynamicProperties]
class ZC_Http_Streams {
	/**
	 * Send a HTTP request to a URI using PHP Streams.
	 *
	 * @see ZC_Http::request() For default options descriptions.
	 *
	 * @since 2.7.0
	 * @since 3.7.0 Combined with the fsockopen transport and switched to stream_socket_client().
	 *
	 ...
	 * @return array|ZC_Error Array containing 'headers', 'body', 'response', 'cookies', 'filename'. A ZC_Error instance upon error
	 */
	public function request( $url, $args = array() ) {
		...
		ZC_Http::buildCookieHeader( $parsed_args );
		...
		if ( false === $handle ) {
			...
			if ( $secure_transport && 0 === $connection_error && '' === $connection_error_str ) {
				return new ZC_Error( 'http_request_failed', __( 'The SSL certificate for the host could not be verified.' ) );
			}

			return new ZC_Error( 'http_request_failed', $connection_error . ': ' . $connection_error_str );
		}
		...
		if ( $parsed_args['stream'] ) {
			...
			while ( ! feof( $handle ) && $keep_reading ) {
				...
				if ( ! $body_started ) {
					$response .= $block;
					if ( strpos( $response, "\r\n\r\n" ) ) {
						$processed_response = ZC_Http::processResponse( $response );
						$body_started       = true;
				...
				if ( $bytes_written_to_file !== $this_block_size ) {
					...
					return new ZC_Error( 'http_request_failed', __( 'Failed to write request to temporary file.' ) );
				}
		...
		$processed_headers = ZC_Http::processHeaders( $processed_response['headers'], $url );
		...
		// Handle redirects.
		$redirect_response = ZC_Http::handle_redirects( $url, $parsed_args, $response );
		...
		if ( ! empty( $processed_response['body'] )
			...
			$processed_response['body'] = ZC_Http::chunkTransferDecode( $processed_response['body'] );
		}

		if ( true === $parsed_args['decompress']
			&& true === ZC_Http_Encoding::should_decode( $processed_headers['headers'] )
		) {
			$processed_response['body'] = ZC_Http_Encoding::decompress( $processed_response['body'] );
		}
	...
	public static function verify_ssl_certificate( $stream, $host ) {
		 ...
		 */
		$host_type = ( ZC_Http::is_ip_address( $host ) ? 'ip' : 'dns' );
 ...
 * All code should make use of ZC_Http directly through its API.
 *
 * @see ZC_HTTP::request
 *
 * @since 2.7.0
 * @deprecated 3.7.0 Please use ZC_HTTP::request() directly
 */
class ZC_HTTP_Fsockopen extends ZC_Http_Streams {
