 ...
 * @since 3.0.0
 *
 * @param int|string|ZC_Term $menu Menu ID, slug, name, or object.
 * @return ZC_Term|false Menu object on success, false if $menu param isn't supplied or term does not exist.
 */
function zc_get_nav_menu_object( $menu ) {
	$menu_obj = false;
	...
	if ( ! $menu_obj || is_zc_error( $menu_obj ) ) {
		$menu_obj = false;
	}

	/**
	 * Filters the nav_menu term retrieved for zc_get_nav_menu_object().
	 *
	 * @since 4.3.0
	 *
	 * @param ZC_Term|false      $menu_obj Term from nav_menu taxonomy, or false if nothing had been found.
	 * @param int|string|ZC_Term $menu     The menu ID, slug, name, or object passed to zc_get_nav_menu_object().
	 */
	return apply_filters( 'zc_get_nav_menu_object', $menu_obj, $menu );
}
 ...
 * @since 3.0.0
 *
 * @param int|string|ZC_Term $menu Menu ID, slug, name, or object of menu to check.
...
function is_nav_menu( $menu ) {
	...
	$menu_obj = zc_get_nav_menu_object( $menu );

	if (
		$menu_obj &&
		! is_zc_error( $menu_obj ) &&
...
function register_nav_menus( $locations = array() ) {
	...
	$_zc_registered_nav_menus = array_merge( (array) $_zc_registered_nav_menus, $locations );
}
...
function unregister_nav_menu( $location ) {
	...
	if ( is_array( $_zc_registered_nav_menus ) && isset( $_zc_registered_nav_menus[ $location ] ) ) {
		unset( $_zc_registered_nav_menus[ $location ] );
		if ( empty( $_zc_registered_nav_menus ) ) {
...
function get_registered_nav_menus() {
	global $_zc_registered_nav_menus;
	return $_zc_registered_nav_menus ?? array();
}
 ...
 */
function zc_get_nav_menu_name( $location ) {
	$menu_name = '';
	 ...
	 * @param string $location  Menu location identifier.
	 */
	return apply_filters( 'zc_get_nav_menu_name', $menu_name, $location );
}
...
function is_nav_menu_item( $menu_item_id = 0 ) {
	return ( ! is_zc_error( $menu_item_id ) && ( 'nav_menu_item' === get_post_type( $menu_item_id ) ) );
}
 ...
 *
 * @param string $menu_name Menu name.
 * @return int|ZC_Error Menu ID on success, ZC_Error object on failure.
 */
function zc_create_nav_menu( $menu_name ) {
	// expected_slashed ($menu_name)
	return zc_update_nav_menu_object( 0, array( 'menu-name' => $menu_name ) );
}
 ...
 *
 * @param int|string|ZC_Term $menu Menu ID, slug, name, or object.
 * @return bool|ZC_Error True on success, false or ZC_Error object on failure.
 */
function zc_delete_nav_menu( $menu ) {
	$menu = zc_get_nav_menu_object( $menu );
	if ( ! $menu ) {
		return false;
	}
	...
	$result = zc_delete_term( $menu->term_id, 'nav_menu' );
	...
	if ( $result && ! is_zc_error( $result ) ) {
		 ...
		 */
		do_action( 'zc_delete_nav_menu', $menu->term_id );
	}
 ...
 * @return int|ZC_Error Menu ID on success, ZC_Error object on failure.
 */
function zc_update_nav_menu_object( $menu_id = 0, $menu_data = array() ) {
	// expected_slashed ($menu_data)
	$menu_id = (int) $menu_id;

	$_menu = zc_get_nav_menu_object( $menu_id );
	...
	if (
		$_possible_existing &&
		! is_zc_error( $_possible_existing ) &&
	...
	if ( ! $_menu || is_zc_error( $_menu ) ) {
		...
		$_menu = zc_insert_term( $menu_data['menu-name'], 'nav_menu', $args );

		if ( is_zc_error( $_menu ) ) {
			return $_menu;
		}
		 ...
		 */
		do_action( 'zc_create_nav_menu', $_menu['term_id'], $menu_data );
	...
	$update_response = zc_update_term( $menu_id, 'nav_menu', $args );

	if ( is_zc_error( $update_response ) ) {
	 ...
	 * @param array $menu_data An array of menu data.
	 */
	do_action( 'zc_update_nav_menu', $menu_id, $menu_data );
	return $menu_id;
}
 ...
 * @return int|ZC_Error The menu item's database ID or ZC_Error object on failure.
 */
function zc_update_nav_menu_item( $menu_id = 0, $menu_item_db_id = 0, $menu_item_data = array(), $fire_after_hooks = true ) {
	$menu_id         = (int) $menu_id;
	$menu_item_db_id = (int) $menu_item_db_id;

	// Make sure that we don't convert non-nav_menu_item objects into nav_menu_item objects.
	if ( ! empty( $menu_item_db_id ) && ! is_nav_menu_item( $menu_item_db_id ) ) {
		return new ZC_Error( 'update_nav_menu_item_failed', __( 'The given object ID is not that of a menu item.' ) );
	}

	$menu = zc_get_nav_menu_object( $menu_id );

	if ( ! $menu && 0 !== $menu_id ) {
		return new ZC_Error( 'invalid_menu_id', __( 'Invalid menu ID.' ) );
	}
	...
	$args = zc_parse_args( $menu_item_data, $defaults );
	...
	if ( 0 === $menu_id ) {
		...
		if ( 0 !== $menu_id ) {
			$menu_items = (array) zc_get_nav_menu_items( $menu_id, array( 'post_status' => 'publish,draft' ) );
		}
	...
	if ( 'custom' === $args['menu-item-type'] ) {
		...
		if ( 'taxonomy' === $args['menu-item-type'] ) {
			...
			if ( $original_object instanceof ZC_Term ) {
			...
			if ( $original_object instanceof ZC_Post ) {
			...
			if ( $original_object instanceof ZC_Post_Type ) {
				$original_title = $original_object->labels->archives;
			}
		}

		if ( zc_unslash( $args['menu-item-title'] ) === $original_title ) {
	...
	$post_date = zc_resolve_post_date( $args['menu-item-post-date'], $args['menu-item-post-date-gmt'] );
	if ( $post_date ) {
	...
	if ( ! $update ) {
		...
		$menu_item_db_id     = zc_insert_post( $post, true, $fire_after_hooks );
		if ( ! $menu_item_db_id || is_zc_error( $menu_item_db_id ) ) {
		 ...
		 * @since 4.4.0
		 *
		 * @see zc_update_nav_menu_item()
		 *
		 * @param int   $menu_id         ID of the updated menu.
		 * @param int   $menu_item_db_id ID of the new menu item.
		 * @param array $args            An array of arguments used to update/add the menu item.
		 */
		do_action( 'zc_add_nav_menu_item', $menu_id, $menu_item_db_id, $args );
	}

	/*
	 * Associate the menu item with the menu term.
	 * Only set the menu term if it isn't set to avoid unnecessary zc_get_object_terms().
	 */
	if ( $menu_id && ( ! $update || ! is_object_in_term( $menu_item_db_id, 'nav_menu', (int) $menu->term_id ) ) ) {
		$update_terms = zc_set_object_terms( $menu_item_db_id, array( $menu->term_id ), 'nav_menu' );
		if ( is_zc_error( $update_terms ) ) {
			return $update_terms;
		}
	}
	...
	if ( $update ) {
		...
		$update_post = zc_update_post( $post, true );
		if ( is_zc_error( $update_post ) ) {
			return $update_post;
		}
	}
	 ...
	 */
	do_action( 'zc_update_nav_menu_item', $menu_id, $menu_item_db_id, $args );
 ...
 *                    Default empty array.
 * @return ZC_Term[] An array of menu objects.
 */
function zc_get_nav_menus( $args = array() ) {
	$defaults = array(
		'taxonomy'   => 'nav_menu',
		'hide_empty' => false,
		'orderby'    => 'name',
	 ...
	 * @see get_terms()
	 *
	 * @param ZC_Term[] $menus An array of menu objects.
	 * @param array     $args  An array of arguments used to retrieve menu objects.
	 */
	return apply_filters( 'zc_get_nav_menus', get_terms( $args ), $args );
}

/**
 * Determines whether a menu item is valid.
 *
 * @link https://core.trac.zelocorecms.org/ticket/13958
 *
 * @since 3.2.0
 * @access private
 *
 ...
 */
function zc_get_nav_menu_items( $menu, $args = array() ) {
	$menu = zc_get_nav_menu_object( $menu );
	...
	$items = array_map( 'zc_setup_nav_menu_item', $items );
	 ...
	 */
	return apply_filters( 'zc_get_nav_menu_items', $items, $menu, $args );
}
 ...
 * @since 6.1.0
 *
 * @param ZC_Post[] $menu_items Array of menu item post objects.
 */
 ...
 */
function zc_setup_nav_menu_item( $menu_item ) {

	/**
	 * Filters whether to short-circuit the zc_setup_nav_menu_item() output.
	 *
	 * Returning a non-null value from the filter will short-circuit zc_setup_nav_menu_item(),
	 ...
	 * @param object      $menu_item          The menu item to modify.
	 */
	$pre_menu_item = apply_filters( 'pre_zc_setup_nav_menu_item', null, $menu_item );
	...
	if ( isset( $menu_item->post_type ) ) {
		if ( 'nav_menu_item' === $menu_item->post_type ) {
			...
			if ( 'post_type' === $menu_item->type ) {
				...
				if ( $object ) {
					...
					if ( function_exists( 'get_post_states' ) ) {
						...
						if ( $menu_post instanceof ZC_Post ) {
							$post_states = get_post_states( $menu_post );
							if ( $post_states ) {
								$menu_item->type_label = zc_strip_all_tags( implode( ', ', $post_states ) );
							}
						}
					}
				} else {
				...
				$post_content          = zc_trim_words( $menu_item->post_content, 200 );
				...
				if ( $original_object && ! is_zc_error( $original_object ) ) {
			...
			if ( ! isset( $menu_item->description ) ) {
				 ...
				 */
				$menu_item->description = apply_filters( 'nav_menu_description', zc_trim_words( $menu_item->post_content, 200 ) );
			}
			...
			/** This filter is documented in zc-includes/nav-menu.php */
	 ...
	 */
	return apply_filters( 'zc_setup_nav_menu_item', $menu_item );
}
 ...
 */
function zc_get_associated_nav_menu_items( $object_id = 0, $object_type = 'post_type', $taxonomy = '' ) {
 ...
 */
function _zc_delete_post_menu_item( $object_id ) {
	$object_id = (int) $object_id;

	$menu_item_ids = zc_get_associated_nav_menu_items( $object_id, 'post_type' );

	foreach ( (array) $menu_item_ids as $menu_item_id ) {
		zc_delete_post( $menu_item_id, true );
	}
}
 ...
 */
function _zc_delete_tax_menu_item( $object_id, $tt_id, $taxonomy ) {
	$object_id = (int) $object_id;

	$menu_item_ids = zc_get_associated_nav_menu_items( $object_id, 'taxonomy', $taxonomy );
 ...
 * @param ZC_Post $post       The post object being transitioned from one status to another.
 */
function _zc_auto_add_pages_to_menu( $new_status, $old_status, $post ) {
	...
	foreach ( $auto_add as $menu_id ) {
		$items = zc_get_nav_menu_items( $menu_id, array( 'post_status' => 'publish,draft' ) );
		if ( ! is_array( $items ) ) {
			continue;
		}
		...
		zc_update_nav_menu_item( $menu_id, 0, $args );
	}
}
 ...
 */
function _zc_delete_customize_changeset_dependent_auto_drafts( $post_id ) {
	...
	remove_action( 'delete_post', '_zc_delete_customize_changeset_dependent_auto_drafts' );
	foreach ( $data['nav_menus_created_posts']['value'] as $stub_post_id ) {
		...
		if ( 'auto-draft' === get_post_status( $stub_post_id ) ) {
			zc_delete_post( $stub_post_id, true );
		} elseif ( 'draft' === get_post_status( $stub_post_id ) ) {
			zc_trash_post( $stub_post_id );
 ...
 * @since 4.9.0
 */
function _zc_menus_changed() {
	$old_nav_menu_locations    = get_option( 'theme_switch_menu_locations', array() );
	$new_nav_menu_locations    = get_nav_menu_locations();
	$mapped_nav_menu_locations = zc_map_nav_menu_locations( $new_nav_menu_locations, $old_nav_menu_locations );
 ...
 */
function zc_map_nav_menu_locations( $new_nav_menu_locations, $old_nav_menu_locations ) {
	$registered_nav_menus   = get_registered_nav_menus();
 ...
 */
function _zc_reset_invalid_menu_item_parent( $menu_item_data ) {
