 ...
 * Core Metadata API
 *
 * Functions for retrieving and manipulating metadata of various ZelocoreCMS object types. Metadata
...
function delete_metadata( $meta_type, $object_id, $meta_key, $meta_value = '', $delete_all = false ) {
	...
	zc_cache_delete_multiple( $data, $meta_type . '_meta' );
...
function get_metadata_raw( $meta_type, $object_id, $meta_key = '', $single = false ) {
	...
	$meta_cache = zc_cache_get( $object_id, $meta_type . '_meta' );
...
function get_metadata_default( $meta_type, $object_id, $meta_key, $single = false ) {
	...
	if ( ! $single && ! zc_is_numeric_array( $value ) ) {
		$value = array( $value );
	}
...
function metadata_exists( $meta_type, $object_id, $meta_key ) {
	...
	/** This filter is documented in zc-includes/meta.php */
...
function update_metadata_by_mid( $meta_type, $meta_id, $meta_value, $meta_key = false ) {
	...
	if ( $meta ) {
		...
		/** This action is documented in zc-includes/meta.php */
		do_action( "update_{$meta_type}_meta", $meta_id, $object_id, $meta_key, $_meta_value );
...
function update_meta_cache( $meta_type, $object_ids ) {
	...
	zc_cache_add_multiple( $data, $cache_group );
 ...
 * @since 4.5.0
 *
 * @return ZC_Metadata_Lazyloader Metadata lazyloader queue.
 */
function zc_metadata_lazyloader() {
	static $zc_metadata_lazyloader;

	if ( null === $zc_metadata_lazyloader ) {
		$zc_metadata_lazyloader = new ZC_Metadata_Lazyloader();
	}

	return $zc_metadata_lazyloader;
}
...
function get_meta_sql( $meta_query, $type, $primary_table, $primary_id_column, $context = null ) {
	$meta_query_obj = new ZC_Meta_Query( $meta_query );
 ...
 *
 * @since 3.3.0
 * @since 4.6.0 {@link https://core.trac.zelocorecms.org/ticket/35658 Modified
 ...
 *
 * @global array $zc_meta_keys Global registry for meta keys.
 *
 * @param string       $object_type Type of object metadata is for. Accepts 'blog', 'post', 'comment', 'term',
 *                                  'user', or any other object type with an associated meta table.
...
function register_meta( $object_type, $meta_key, $args, $deprecated = null ) {
	...
	if ( ! $has_old_auth_cb && ! $has_old_sanitize_cb ) {
		...
		$zc_meta_keys[ $object_type ][ $object_subtype ][ $meta_key ] = $args;
...
function filter_default_metadata( $value, $object_id, $meta_key, $single, $meta_type ) {
	...
	if ( ! is_array( $zc_meta_keys ) || ! isset( $zc_meta_keys[ $meta_type ] ) ) {
		return $value;
	}

	$defaults = array();
	foreach ( $zc_meta_keys[ $meta_type ] as $sub_type => $meta_data ) {
...
function unregister_meta_key( $object_type, $meta_key, $object_subtype = '' ) {
	...
	$args = $zc_meta_keys[ $object_type ][ $object_subtype ][ $meta_key ];
	...
	unset( $zc_meta_keys[ $object_type ][ $object_subtype ][ $meta_key ] );

	// Do some clean up.
	if ( empty( $zc_meta_keys[ $object_type ][ $object_subtype ] ) ) {
		unset( $zc_meta_keys[ $object_type ][ $object_subtype ] );
	}
	if ( empty( $zc_meta_keys[ $object_type ] ) ) {
		unset( $zc_meta_keys[ $object_type ] );
	}
...
function get_registered_meta_keys( $object_type, $object_subtype = '' ) {
	...
	if ( ! is_array( $zc_meta_keys ) || ! isset( $zc_meta_keys[ $object_type ] ) || ! isset( $zc_meta_keys[ $object_type ][ $object_subtype ] ) ) {
		return array();
	}

	return $zc_meta_keys[ $object_type ][ $object_subtype ];
}
 ...
 */
function _zc_register_meta_args_allowed_list( $args, $default_args ) {
