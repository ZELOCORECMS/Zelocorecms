<?php
/**
 * ZelocoreCMS Customize Nav Menus classes
 *
 * @package ZelocoreCMS
...
final class ZC_Customize_Nav_Menus {
	...
	public function __construct( $manager ) {
		...
		// See https://github.com/xwp/zc-customize-snapshots/blob/962586659688a5b1fd9ae93618b7ce2d4e7a421c/php/class-customize-snapshot-manager.php#L469-L499
		...
		add_action( 'zc_ajax_load-available-menu-items-customizer', array( $this, 'ajax_load_available_items' ) );
		add_action( 'zc_ajax_search-available-menu-items-customizer', array( $this, 'ajax_search_available_items' ) );
		add_action( 'zc_ajax_customize-nav-menus-insert-auto-draft', array( $this, 'ajax_insert_auto_draft_post' ) );
	...
	public function filter_nonces( $nonces ) {
		$nonces['customize-menus'] = zc_create_nonce( 'customize-menus' );
		return $nonces;
	}
	...
	public function ajax_load_available_items() {
		...
		if ( isset( $_POST['item_types'] ) && is_array( $_POST['item_types'] ) ) {
			$item_types = zc_unslash( $_POST['item_types'] );
		} elseif ( isset( $_POST['type'] ) && isset( $_POST['object'] ) ) { // Back compat.
			$item_types[] = array(
				'type'   => zc_unslash( $_POST['type'] ),
				'object' => zc_unslash( $_POST['object'] ),
				'page'   => empty( $_POST['page'] ) ? 0 : absint( $_POST['page'] ),
			);
		} else {
			zc_send_json_error( 'nav_menus_missing_type_or_object_parameter' );
		}
		...
		foreach ( $item_types as $item_type ) {
			...
			if ( is_zc_error( $items ) ) {
				zc_send_json_error( $items->get_error_code() );
			}
			$all_items[ $item_type['type'] . ':' . $item_type['object'] ] = $items;
		}

		zc_send_json_success( array( 'items' => $all_items ) );
	}
	 ...
	 * @return array|ZC_Error An array of menu items on success, a ZC_Error object on failure.
	 */
	public function load_available_items_query( $object_type = 'post_type', $object_name = 'page', $page = 0 ) {
		...
		if ( 'post_type' === $object_type ) {
			...
			if ( ! $post_type ) {
				return new ZC_Error( 'nav_menus_invalid_post_type' );
			}
			...
			if ( 0 === $page && 'page' === $object_name ) {
				...
				$privacy_policy_page_id = (int) get_option( 'zc_page_for_privacy_policy' );
				if ( ! empty( $privacy_policy_page_id ) ) {
					$privacy_policy_page = get_post( $privacy_policy_page_id );
					if ( $privacy_policy_page instanceof ZC_Post && 'publish' === $privacy_policy_page->post_status ) {
	...
	public function ajax_search_available_items() {
		...
		if ( empty( $_POST['search'] ) ) {
			zc_send_json_error( 'nav_menus_missing_search_parameter' );
		}
		...
		$s     = sanitize_text_field( zc_unslash( $_POST['search'] ) );
		...
		if ( empty( $items ) ) {
			zc_send_json_error( array( 'message' => __( 'No results found.' ) ) );
		} else {
			zc_send_json_success( array( 'items' => $items ) );
		}
	}
	 ...
	 *
	 * Based on ZC_Editor::zc_link_query().
	 *
	 * @since 4.3.0
	 *
	...
	public function enqueue_scripts() {
		zc_enqueue_style( 'customize-nav-menus' );
		zc_enqueue_script( 'customize-nav-menus' );

		$temp_nav_menu_setting      = new ZC_Customize_Nav_Menu_Setting( $this->manager, 'nav_menu[-1]' );
		$temp_nav_menu_item_setting = new ZC_Customize_Nav_Menu_Item_Setting( $this->manager, 'nav_menu_item[-1]' );
		...
		$data = sprintf( 'var _wpCustomizeNavMenusSettings = %s;', zc_json_encode( $settings, JSON_HEX_TAG | JSON_UNESCAPED_SLASHES ) );
		zc_scripts()->add_data( 'customize-nav-menus', 'data', $data );
		...
		zc_localize_script( 'nav-menu', 'menus', $nav_menus_l10n );
	}
	...
	public function filter_dynamic_setting_args( $setting_args, $setting_id ) {
		if ( preg_match( ZC_Customize_Nav_Menu_Setting::ID_PATTERN, $setting_id ) ) {
			$setting_args = array(
				'type'      => ZC_Customize_Nav_Menu_Setting::TYPE,
				'transport' => 'postMessage',
			);
		} elseif ( preg_match( ZC_Customize_Nav_Menu_Item_Setting::ID_PATTERN, $setting_id ) ) {
			$setting_args = array(
				'type'      => ZC_Customize_Nav_Menu_Item_Setting::TYPE,
				'transport' => 'postMessage',
			);
		}
		return $setting_args;
	...
	/**
	 * Allows non-statically created settings to be constructed with custom ZC_Customize_Setting subclass.
	 *
	 * @since 4.3.0
	 *
	...
	public function filter_dynamic_setting_class( $setting_class, $setting_id, $setting_args ) {
		...
		if ( ! empty( $setting_args['type'] ) && ZC_Customize_Nav_Menu_Setting::TYPE === $setting_args['type'] ) {
			$setting_class = 'ZC_Customize_Nav_Menu_Setting';
		} elseif ( ! empty( $setting_args['type'] ) && ZC_Customize_Nav_Menu_Item_Setting::TYPE === $setting_args['type'] ) {
			$setting_class = 'ZC_Customize_Nav_Menu_Item_Setting';
		}
	...
	public function customize_register() {
		...
		$this->manager->register_panel_type( 'ZC_Customize_Nav_Menus_Panel' );
		$this->manager->register_control_type( 'ZC_Customize_Nav_Menu_Control' );
		$this->manager->register_control_type( 'ZC_Customize_Nav_Menu_Name_Control' );
		$this->manager->register_control_type( 'ZC_Customize_Nav_Menu_Locations_Control' );
		$this->manager->register_control_type( 'ZC_Customize_Nav_Menu_Auto_Add_Control' );
		$this->manager->register_control_type( 'ZC_Customize_Nav_Menu_Item_Control' );
		...
		/*
		 * Once multiple theme supports are allowed in ZC_Customize_Panel,
		 * this panel can be restricted to themes that support menus or widgets.
		 */
		$this->manager->add_panel(
			new ZC_Customize_Nav_Menus_Panel(
				$this->manager,
				'nav_menus',
				array(
					'title'       => __( 'Menus' ),
		...
		foreach ( $menus as $menu ) {
			$choices[ $menu->term_id ] = zc_html_excerpt( $menu->name, 40, '&hellip;' );
		}
		...
		if ( ! $this->manager->is_theme_active() ) {
			...
			$mapped_nav_menu_locations = zc_map_nav_menu_locations( $theme_mods['nav_menu_locations'], $this->original_nav_menu_locations );
		}
		...
		foreach ( $locations as $location => $description ) {
			...
			$this->manager->add_control(
				new ZC_Customize_Nav_Menu_Location_Control(
					$this->manager,
					$setting_id,
					array(
						'label'       => $description,
		...
		foreach ( $menus as $menu ) {
			...
			$this->manager->add_section(
				new ZC_Customize_Nav_Menu_Section(
					$this->manager,
					$section_id,
					array(
			...
			$this->manager->add_setting(
				new ZC_Customize_Nav_Menu_Setting(
					$this->manager,
					$nav_menu_setting_id,
					array(
						'transport' => 'postMessage',
			...
			$menu_items = (array) zc_get_nav_menu_items( $menu_id );
			...
			foreach ( array_values( $menu_items ) as $i => $item ) {
				...
				$this->manager->add_setting(
					new ZC_Customize_Nav_Menu_Item_Setting(
						$this->manager,
				...
				$this->manager->add_control(
					new ZC_Customize_Nav_Menu_Item_Control(
						$this->manager,
						$menu_item_setting_id,
						array(
							'label'    => $item->title,
	 ...
	 * }
	 * @return ZC_Post|ZC_Error Inserted auto-draft post object or error.
	 */
	public function insert_auto_draft_post( $postarr ) {
		if ( ! isset( $postarr['post_type'] ) ) {
			return new ZC_Error( 'unknown_post_type', __( 'Invalid post type.' ) );
		}
		if ( empty( $postarr['post_title'] ) ) {
			return new ZC_Error( 'empty_title', __( 'Empty title.' ) );
		}
		if ( ! empty( $postarr['post_status'] ) ) {
			return new ZC_Error( 'status_forbidden', __( 'Status is forbidden.' ) );
		}
		...
		add_filter( 'zc_insert_post_empty_content', '__return_false', 1000 );
		$r = zc_insert_post( zc_slash( $postarr ), true );
		remove_filter( 'zc_insert_post_empty_content', '__return_false', 1000 );
	...
	public function ajax_insert_auto_draft_post() {
		if ( ! check_ajax_referer( 'customize-menus', 'customize-menus-nonce', false ) ) {
			zc_send_json_error( 'bad_nonce', 400 );
		}

		if ( ! current_user_can( 'customize' ) ) {
			zc_send_json_error( 'customize_not_allowed', 403 );
		}

		if ( empty( $_POST['params'] ) || ! is_array( $_POST['params'] ) ) {
			zc_send_json_error( 'missing_params', 400 );
		}

		$params         = zc_unslash( $_POST['params'] );
		$illegal_params = array_diff( array_keys( $params ), array( 'post_type', 'post_title' ) );
		if ( ! empty( $illegal_params ) ) {
			zc_send_json_error( 'illegal_params', 400 );
		}
	...
	protected function print_custom_links_available_menu_item() {
		...
		<div id="new-custom-menu-item" class="accordion-section">
			...
			<div class="accordion-section-content customlinkdiv" id="new-custom-menu-item-content">
				...
				<p id="menu-item-url-wrap" class="zc-clearfix">
				...
				<p id="menu-item-name-wrap" class="zc-clearfix">
	...
	public function customize_preview_init() {
		add_action( 'zc_enqueue_scripts', array( $this, 'customize_preview_enqueue_deps' ) );
		add_filter( 'zc_nav_menu_args', array( $this, 'filter_zc_nav_menu_args' ), 1000 );
		add_filter( 'zc_nav_menu', array( $this, 'filter_zc_nav_menu' ), 10, 2 );
		add_action( 'zc_footer', array( $this, 'export_preview_data' ), 1 );
	...
	public function make_auto_draft_status_previewable() {
		...
		$zc_post_statuses['auto-draft']->protected = true;
	}
	...
	public function sanitize_nav_menus_created_posts( $value ) {
		$post_ids = array();
		foreach ( zc_parse_id_list( $value ) as $post_id ) {
			if ( empty( $post_id ) ) {
				continue;
			}
	 ...
	 * `ZC_Customize_Nav_Menu_Items::sanitize_nav_menus_created_posts()` to
	 * remove any post IDs for which the user cannot publish or for which the
	 * post is not an auto-draft.
	 *
	 * @since 4.7.0
	 *
	 * @param ZC_Customize_Setting $setting Customizer setting object.
	 */
	public function save_nav_menus_created_posts( $setting ) {
		...
		if ( ! empty( $post_ids ) ) {
			foreach ( $post_ids as $post_id ) {
				...
				// Note that zc_publish_post() cannot be used because unique slugs need to be assigned.
				zc_update_post( zc_slash( $args ) );
	...
	/**
	 * Keeps track of the arguments that are being passed to zc_nav_menu().
	 *
	 * @since 4.3.0
	 *
	 * @see zc_nav_menu()
	 * @see ZC_Customize_Widgets::filter_dynamic_sidebar_params()
	 *
	 * @param array $args An array containing zc_nav_menu() arguments.
	 * @return array Arguments.
	 */
	public function filter_zc_nav_menu_args( $args ) {
		/*
		 * The following conditions determine whether or not this instance of
		 * zc_nav_menu() can use selective refreshed. A zc_nav_menu() can be
		 * selective refreshed if...
		 */
		$can_partial_refresh = (
			// ...if zc_nav_menu() is directly echoing out the menu (and thus isn't manipulating the string after generated),
			! empty( $args['echo'] )
			&&
	...
	/**
	 * Prepares zc_nav_menu() calls for partial refresh.
	 *
	 ...
	 * @param object $args             An object containing zc_nav_menu() arguments.
	 * @return string Nav menu HTML with selective refresh attributes added if partial can be refreshed.
	 */
	public function filter_zc_nav_menu( $nav_menu_content, $args ) {
		if ( isset( $args->customize_preview_nav_menus_args['can_partial_refresh'] ) && $args->customize_preview_nav_menus_args['can_partial_refresh'] ) {
			...
			$attributes      .= sprintf( ' data-customize-partial-placement-context="%s"', esc_attr( zc_json_encode( $args->customize_preview_nav_menus_args ) ) );
	...
	public function customize_preview_enqueue_deps() {
		zc_enqueue_script( 'customize-preview-nav-menus' ); // Note that we have overridden this.
	}
	...
	public function export_preview_data() {
		...
		// Why not zc_localize_script? Because we're not localizing, and it forces values into strings.
		$exports = array(
			'navMenuInstanceArgs' => $this->preview_nav_menu_instance_args,
		);
		zc_print_inline_script_tag( sprintf( 'var _wpCustomizePreviewNavMenusExports = %s;', zc_json_encode( $exports, JSON_HEX_TAG | JSON_UNESCAPED_SLASHES ) ) . "\n//# sourceURL=" . rawurlencode( __METHOD__ ) );
	}

	/**
	 * Exports any zc_nav_menu() calls during the rendering of any partials.
	 *
	 * @since 4.5.0
	 *
	...
	/**
	 * Renders a specific menu via zc_nav_menu() using the supplied arguments.
	 *
	 * @since 4.3.0
	 *
	 * @see zc_nav_menu()
