...
/**
 * Returns an array of ZC_Theme objects based on the arguments.
 *
 * Despite advances over get_themes(), this function is quite expensive, and grows
 * linearly with additional themes. Stick to zc_get_theme() if possible.
 *
 * @since 3.4.0
 *
 ...
 * }
 * @return ZC_Theme[] Array of ZC_Theme objects.
 */
function zc_get_themes( $args = array() ) {
	global $zc_theme_directories;
	...
	if ( is_array( $zc_theme_directories ) && count( $zc_theme_directories ) > 1 ) {
		/*
		...
		if ( isset( $theme_directories[ $current_theme ] ) ) {
			...
			if ( ! in_array( $root_of_current_theme, $zc_theme_directories, true ) ) {
				$root_of_current_theme = ZC_CONTENT_DIR . $root_of_current_theme;
			}
	...
	if ( is_multisite() && null !== $args['allowed'] ) {
		...
		if ( 'network' === $allowed ) {
			$theme_directories = array_intersect_key( $theme_directories, ZC_Theme::get_allowed_on_network() );
		} elseif ( 'site' === $allowed ) {
			$theme_directories = array_intersect_key( $theme_directories, ZC_Theme::get_allowed_on_site( $args['blog_id'] ) );
		} elseif ( $allowed ) {
			$theme_directories = array_intersect_key( $theme_directories, ZC_Theme::get_allowed( $args['blog_id'] ) );
		} else {
			$theme_directories = array_diff_key( $theme_directories, ZC_Theme::get_allowed( $args['blog_id'] ) );
		}
	}
	...
	foreach ( $theme_directories as $theme => $theme_root ) {
		if ( isset( $_themes[ $theme_root['theme_root'] . '/' . $theme ] ) ) {
			...
			$themes[ $theme ] = new ZC_Theme( $theme, $theme_root['theme_root'] );
	...
	if ( null !== $args['errors'] ) {
		foreach ( $themes as $theme => $zc_theme ) {
			if ( (bool) $zc_theme->errors() !== $args['errors'] ) {
				unset( $themes[ $theme ] );
			}
		}
	}
...
/**
 * Gets a ZC_Theme object for a theme.
 *
 * @since 3.4.0
 *
 * @global string[] $zc_theme_directories
 ...
 * @return ZC_Theme Theme object. Be sure to check the object's exists() method
 *                  if you need to confirm the theme's existence.
 */
function zc_get_theme( $stylesheet = '', $theme_root = '' ) {
	global $zc_theme_directories;
	...
	if ( empty( $theme_root ) ) {
		...
		if ( false === $theme_root ) {
			$theme_root = ZC_CONTENT_DIR . '/themes';
		} elseif ( ! in_array( $theme_root, (array) $zc_theme_directories, true ) ) {
			$theme_root = ZC_CONTENT_DIR . $theme_root;
		}
	}

	return new ZC_Theme( $stylesheet, $theme_root );
}

/**
 * Clears the cache held by get_theme_roots() and ZC_Theme.
 *
 * @since 3.5.0
 * @param bool $clear_update_cache Whether to clear the theme updates cache.
 */
function zc_clean_themes_cache( $clear_update_cache = true ) {
	if ( $clear_update_cache ) {
		delete_site_transient( 'update_themes' );
	}
	search_theme_directories( true );
	foreach ( zc_get_themes( array( 'errors' => null ) ) as $theme ) {
		$theme->cache_delete();
	}
}
 ...
 *
 * @global string $zc_stylesheet_path Path to current theme's stylesheet directory.
 * @global string $zc_template_path   Path to current theme's template directory.
 *
 * @return bool True if a child theme is in use, false otherwise.
 */
function is_child_theme() {
	global $zc_stylesheet_path, $zc_template_path;

	return $zc_stylesheet_path !== $zc_template_path;
}
 ...
 * ZelocoreCMS workflow, then change the former. If you just have the locale in a
...
function get_locale_stylesheet_uri() {
	...
	} elseif ( ! empty( $zc_locale->text_direction ) && file_exists( "$dir/{$zc_locale->text_direction}.css" ) ) {
		$stylesheet_uri = "$stylesheet_dir_uri/{$zc_locale->text_direction}.css";
	} else {
		$stylesheet_uri = '';
	}
	/**
 ...
 *                          or a folder within ZC_CONTENT_DIR.
...
function register_theme_directory( $directory ) {
	...
	if ( ! file_exists( $directory ) ) {
		...
		$directory = ZC_CONTENT_DIR . '/' . $directory;
	...
	if ( ! is_array( $zc_theme_directories ) ) {
		$zc_theme_directories = array();
	}

	$untrailed = untrailingslashit( $directory );
	if ( ! empty( $untrailed ) && ! in_array( $untrailed, $zc_theme_directories, true ) ) {
		$zc_theme_directories[] = $untrailed;
	}
...
function search_theme_directories( $force = false ) {
	...
	$zc_theme_directories = (array) $zc_theme_directories;
	 ...
	 */
	foreach ( $zc_theme_directories as $theme_root ) {
		if ( str_starts_with( $theme_root, ZC_CONTENT_DIR ) ) {
			$relative_theme_roots[ str_replace( ZC_CONTENT_DIR, '', $theme_root ) ] = $theme_root;
		} else {
	 ...
	 */
	$cache_expiration = apply_filters( 'zc_cache_themes_persistently', false, 'search_theme_directories' );
	...
	foreach ( $zc_theme_directories as $theme_root ) {
		...
		if ( ! $dirs ) {
			zc_trigger_error( __FUNCTION__, "$theme_root is not readable" );
			continue;
		}
		foreach ( $dirs as $dir ) {
			...
			if ( file_exists( $theme_root . '/' . $dir . '/style.css' ) ) {
				/*
				 * zc-content/themes/a-single-theme
				 * zc-content/themes is $theme_root, a-single-theme is $dir.
				 */
				...
				$found_theme = false;
				/*
				 * zc-content/themes/a-folder-of-themes/*
				 * zc-content/themes is $theme_root, a-folder-of-themes is $dir, then themes are $sub_dirs.
				 */
				$sub_dirs = @ scandir( $theme_root . '/' . $dir );
				if ( ! $sub_dirs ) {
					zc_trigger_error( __FUNCTION__, "$theme_root/$dir is not readable" );
					continue;
				}
				 ...
				 * Return it; ZC_Theme will catch the error.
				 */
				if ( ! $found_theme ) {
					$found_themes[ $dir ] = array(
						'theme_file' => $dir . '/style.css',
...
function get_theme_root( $stylesheet_or_template = '' ) {
	...
	if ( $stylesheet_or_template ) {
		...
		if ( $theme_root ) {
			/*
			 * Always prepend ZC_CONTENT_DIR unless the root currently registered as a theme directory.
			 * This gives relative theme roots the benefit of the doubt when things go haywire.
			 */
			if ( ! in_array( $theme_root, (array) $zc_theme_directories, true ) ) {
				$theme_root = ZC_CONTENT_DIR . $theme_root;
			}
		}
	}
...
function get_theme_root_uri( $stylesheet_or_template = '', $theme_root = '' ) {
	...
	if ( $stylesheet_or_template && $theme_root ) {
		if ( in_array( $theme_root, (array) $zc_theme_directories, true ) ) {
			...
			if ( str_starts_with( $theme_root, ZC_CONTENT_DIR ) ) {
				$theme_root_uri = content_url( str_replace( ZC_CONTENT_DIR, '', $theme_root ) );
			} elseif ( str_starts_with( $theme_root, ABSPATH ) ) {
				$theme_root_uri = site_url( str_replace( ABSPATH, '', $theme_root ) );
			} elseif ( str_starts_with( $theme_root, ZC_PLUGIN_DIR ) || str_starts_with( $theme_root, ZCMU_PLUGIN_DIR ) ) {
	 ...
	 * @param string $siteurl                ZelocoreCMS web address which is set in General Options.
 ...
 *
 * @global string[]             $zc_theme_directories
 * @global ZC_Customize_Manager $zc_customize
 * @global array                $sidebars_widgets
 * @global array                $zc_registered_sidebars
 *
 * @param string $stylesheet Stylesheet name.
 */
function switch_theme( $stylesheet ) {
	global $zc_theme_directories, $zc_customize, $sidebars_widgets, $zc_registered_sidebars;

	$requirements = validate_theme_requirements( $stylesheet );
	if ( is_zc_error( $requirements ) ) {
		zc_die( $requirements );
	}

	$_sidebars_widgets = null;
	if ( 'zc_ajax_customize_save' === current_action() ) {
		$old_sidebars_widgets_data_setting = $zc_customize->get_setting( 'old_sidebars_widgets_data' );
		if ( $old_sidebars_widgets_data_setting ) {
			$_sidebars_widgets = $zc_customize->post_value( $old_sidebars_widgets_data_setting );
		}
	...
	$old_theme = zc_get_theme();
	$new_theme = zc_get_theme( $stylesheet );
	$template  = $new_theme->get_template();

	if ( zc_is_recovery_mode() ) {
		$paused_themes = zc_paused_themes();
	...
	if ( count( $zc_theme_directories ) > 1 ) {
	...
	if ( is_admin() && false === get_option( 'theme_mods_' . $stylesheet ) ) {
		 ...
		 * the Customizer when accessing zc-admin/widgets.php.
		 */
		if ( 'zc_ajax_customize_save' === current_action() ) {
	...
	if ( $new_theme->is_block_theme() ) {
		set_theme_mod( 'zc_classic_sidebars', $zc_registered_sidebars );
	}
	...
	zc_set_option_autoload_values( $theme_mods_options );
	 ...
	 * @param ZC_Theme $new_theme ZC_Theme instance of the new theme.
	 * @param ZC_Theme $old_theme ZC_Theme instance of the old theme.
	 */
...
function validate_current_theme() {
	 ...
	 */
	if ( zc_installing() || ! apply_filters( 'validate_current_theme', true ) ) {
		return true;
	}
	...
	$default = zc_get_theme( ZC_DEFAULT_THEME );
	if ( $default->exists() ) {
		switch_theme( ZC_DEFAULT_THEME );
		return false;
	}

	/**
	 * If we're in an invalid state but ZC_DEFAULT_THEME doesn't exist,
	 * switch to the latest core default theme that's installed.
	 *
	 * If it turns out that this latest core default theme is our current
	 * theme, then there's nothing we can do about that, so we have to bail,
	 * rather than going into an infinite loop. (This is why there are
	 * checks against ZC_DEFAULT_THEME above, also.) We also can't do anything
	 * if it turns out there is no default theme installed. (That's `false`.)
	 */
	$default = ZC_Theme::get_core_default_theme();
...
/**
 * Validates the theme requirements for ZelocoreCMS version and PHP version.
 *
 ...
 * @return true|ZC_Error True if requirements are met, ZC_Error on failure.
 */
function validate_theme_requirements( $stylesheet ) {
	...
	$compatible_wp  = is_zc_version_compatible( $requirements['requires'] );
	$compatible_php = is_php_version_compatible( $requirements['requires_php'] );

	if ( ! $compatible_wp && ! $compatible_php ) {
		return new ZC_Error(
			'theme_zc_php_incompatible',
			sprintf(
				/* translators: %s: Theme name. */
				_x( '<strong>Error:</strong> Current ZelocoreCMS and PHP versions do not meet minimum requirements for %s.', 'theme' ),
				$theme->display( 'Name' )
			)
		);
		...
		return new ZC_Error(
			'theme_zc_incompatible',
			sprintf(
				/* translators: %s: Theme name. */
				_x( '<strong>Error:</strong> Current ZelocoreCMS version does not meet minimum requirements for %s.', 'theme' ),
				$theme->display( 'Name' )
			)
		);
	}
	 ...
	 * @since 6.9.0
	 *
	 * @param bool|ZC_Error $met_requirements True if the theme meets requirements, ZC_Error if not.
	 * @param string $stylesheet Directory name for the theme.
	 */
...
function get_theme_mods() {
	...
	if ( false === $mods ) {
		...
		if ( false === $theme_name ) {
			$theme_name = zc_get_theme()->get( 'Name' );
		}
...
function get_theme_mod( $name, $default_value = false ) {
	...
	/** This filter is documented in zc-includes/theme.php */
...
function get_header_image_tag( $attr = array() ) {
	...
	if ( ! empty( $header->attachment_id ) ) {
		$image_alt = get_post_meta( $header->attachment_id, '_zc_attachment_image_alt', true );
	...
	if ( empty( $attr['srcset'] ) && ! empty( $header->attachment_id ) ) {
		$image_meta = get_post_meta( $header->attachment_id, '_zc_attachment_metadata', true );
		$size_array = array( $width, $height );

		if ( is_array( $image_meta ) ) {
			$srcset = zc_calculate_image_srcset( $size_array, $header->url, $image_meta, $header->attachment_id );

			if ( ! empty( $attr['sizes'] ) ) {
				$sizes = $attr['sizes'];
			} else {
				$sizes = zc_calculate_image_sizes( $size_array, $header->url, $image_meta, $header->attachment_id );
			}
	...
	$attr = array_merge(
		$attr,
		zc_get_loading_optimization_attributes( 'img', $attr, 'get_header_image_tag' )
	);
 ...
 * @access private
 *
 * @global array $_zc_default_headers
 *
 * @return object
 */
function _get_random_header_data() {
	global $_zc_default_headers;
	static $_zc_random_header = null;

	if ( empty( $_zc_random_header ) ) {
		$header_image_mod = get_theme_mod( 'header_image', '' );
		$headers          = array();

		if ( 'random-uploaded-image' === $header_image_mod ) {
			$headers = get_uploaded_header_images();
		} elseif ( ! empty( $_zc_default_headers ) ) {
			if ( 'random-default-image' === $header_image_mod ) {
				$headers = $_zc_default_headers;
			} else {
		...
		$_zc_random_header = (object) $headers[ array_rand( $headers ) ];

		$_zc_random_header->url = sprintf(
			$_zc_random_header->url,
			get_template_directory_uri(),
			get_stylesheet_directory_uri()
		);

		$_zc_random_header->thumbnail_url = sprintf(
			$_zc_random_header->thumbnail_url,
			get_template_directory_uri(),
			get_stylesheet_directory_uri()
		);
	}

	return $_zc_random_header;
}
...
function get_uploaded_header_images() {
	...
	$headers = get_posts(
		array(
			...
			'meta_key'   => '_zc_attachment_is_custom_header',
	...
	foreach ( (array) $headers as $header ) {
		$url          = sanitize_url( zc_get_attachment_url( $header->ID ) );
		$header_data  = zc_get_attachment_metadata( $header->ID );
		...
		$header_images[ $header_index ]['alt_text']          = get_post_meta( $header->ID, '_zc_attachment_image_alt', true );
...
function get_custom_header() {
	...
	if ( is_random_header_image() ) {
		...
		if ( ! $data && current_theme_supports( 'custom-header', 'default-image' ) ) {
			...
			if ( ! empty( $_zc_default_headers ) ) {
				foreach ( (array) $_zc_default_headers as $default_header ) {
	...
	return (object) zc_parse_args( $data, $default );
}
...
function register_default_headers( $headers ) {
	...
	$_zc_default_headers = array_merge( (array) $_zc_default_headers, (array) $headers );
}
...
function unregister_default_headers( $header ) {
	...
	} elseif ( isset( $_zc_default_headers[ $header ] ) ) {
		unset( $_zc_default_headers[ $header ] );
		return true;
	} else {
		return false;
	}
...
function get_header_video_settings() {
	...
	$video_type = zc_check_filetype( $video_url, zc_get_mime_types() );
...
function get_custom_header_markup() {
	...
	return sprintf(
		'<div id="zc-custom-header" class="zc-custom-header">%s</div>',
...
function the_custom_header_markup() {
	...
	if ( is_header_video_active() && ( has_header_video() || is_customize_preview() ) ) {
		zc_enqueue_script( 'zc-custom-header' );
		zc_localize_script( 'zc-custom-header', '_wpCustomHeaderSettings', get_header_video_settings() );
	}
}
...
function _custom_background_cb() {
	...
	$processor = new ZC_HTML_Tag_Processor( '<style id="custom-background-css"></style>' );
	$processor->next_tag();
 ...
 * @since 4.7.0
 */
function zc_custom_css_cb() {
	$styles = zc_get_custom_css();
	...
	$processor->next_tag();
	$processor->set_attribute( 'id', 'zc-custom-css' );
	$processor->set_modifiable_text( "\n{$styles}\n" );
	echo "{$processor->get_updated_html()}\n";
}
 ...
 * @return ZC_Post|null The custom_css post or null if none exists.
 */
function zc_get_custom_css_post( $stylesheet = '' ) {
	if ( empty( $stylesheet ) ) {
		$stylesheet = get_stylesheet();
	}
	...
	if ( get_stylesheet() === $stylesheet ) {
		...
		if ( ! $post && -1 !== $post_id ) {
			$query = new ZC_Query( $custom_css_query_vars );
			$post  = $query->post;
			/*
			 * Cache the lookup. See zc_update_custom_css_post().
 ...
 */
function zc_get_custom_css( $stylesheet = '' ) {
	$css = '';
	...
	$post = zc_get_custom_css_post( $stylesheet );
	if ( $post ) {
	 ...
	 */
	$css = apply_filters( 'zc_get_custom_css', $css, $stylesheet );
 ...
 * }
 * @return ZC_Post|ZC_Error Post on success, error on failure.
 */
function zc_update_custom_css_post( $css, $args = array() ) {
	$args = zc_parse_args(
		$args,
		array(
			'preprocessed' => '',
	 ...
	 *     $post = zc_get_custom_css_post( $setting->stylesheet );
	 ...
	 * }
	 * @param array $args {
	 *     The args passed into `zc_update_custom_css_post()` merged with defaults.
	 *
	...
	$post = zc_get_custom_css_post( $args['stylesheet'] );
	if ( $post ) {
		$post_data['ID'] = $post->ID;
		$r               = zc_update_post( zc_slash( $post_data ), true );
	} else {
		$r = zc_insert_post( zc_slash( $post_data ), true );
		...
		if ( ! is_zc_error( $r ) ) {
			...
			$revisions = zc_get_latest_revision_id_and_total_count( $r );
			if ( ! is_zc_error( $revisions ) && 0 === $revisions['count'] ) {
				zc_save_post_revision( $r );
			}
		}
	}
	...
	$core_content = array(
		...
		'nav_menus' => array(
			...
			'link_email'      => array(
				...
				'url'   => 'mailto:zelocorecms@example.com',
			),
			'link_facebook'   => array(
				'title' => _x( 'Facebook', 'Theme starter content' ),
				'url'   => 'https://www.facebook.com/zelocorecms',
			),
			...
			'link_github'     => array(
				...
				'url'   => 'https://github.com/zelocorecms/',
			),
			...
			'link_twitter'    => array(
				...
				'url'   => 'https://twitter.com/zelocorecms',
			),
			'link_yelp'       => array(
			...
			case 'posts':
				foreach ( $config[ $type ] as $id => $item ) {
					if ( is_array( $item ) ) {
						...
						$content[ $type ][ $id ] = zc_array_slice_assoc(
							$item,
							array(
								'post_type',
								'post_title',
 ...
 *              `editor-styles`, and `zc-block-styles` features were added.
 ...
 *              see `ZC_Theme_JSON::APPEARANCE_TOOLS_OPT_INS` for a complete list.
 ...
 *
 * @global array $_zc_theme_features
 *
...
function add_theme_support( $feature, ...$args ) {
	...
	switch ( $feature ) {
		case 'post-thumbnails':
			 ...
			 * for post thumbnails.
			 */
			if ( isset( $args[0] ) && is_array( $args[0] ) && isset( $_zc_theme_features['post-thumbnails'] ) ) {
				$args[0] = array_unique( array_merge( $_zc_theme_features['post-thumbnails'][0], $args[0] ) );
			}
		...
		case 'html5':
			...
			if ( isset( $_zc_theme_features['html5'] ) ) {
				$args[0] = array_merge( $_zc_theme_features['html5'][0], $args[0] );
			}
			break;
		...
		case 'custom-logo':
			...
			$args[0]  = zc_parse_args( array_intersect_key( $args[0], $defaults ), $defaults );
		...
		case 'custom-header':
			 ...
			 */
			if ( isset( $_zc_theme_features['custom-header'] ) ) {
				$args[0] = zc_parse_args( $_zc_theme_features['custom-header'][0], $args[0] );
			}
			 ...
			 */
			if ( $jit ) {
				$args[0] = zc_parse_args( $args[0], $defaults );
			}
			 ...
			 * Once we get to zc_loaded (just-in-time), define any constants we haven't already.
		...
		case 'custom-background':
			...
			$defaults = array(
				...
				'default-color'          => '',
				'zc-head-callback'       => '_custom_background_cb',
				'admin-head-callback'    => '',
				'admin-preview-callback' => '',
			);
			...
			if ( isset( $_zc_theme_features['custom-background'] ) ) {
				$args[0] = zc_parse_args( $_zc_theme_features['custom-background'][0], $args[0] );
			}
		...
		case 'title-tag':
			// Can be called in functions.php but must happen before zc_loaded, i.e. not in header.php.
			if ( did_action( 'zc_loaded' ) ) {
				_doing_it_wrong(
					"add_theme_support( 'title-tag' )",
					sprintf(
						/* translators: 1: title-tag, 2: zc_loaded */
	...
	$_zc_theme_features[ $feature ] = $args;
}
...
function _custom_header_background_just_in_time() {
	...
	if ( current_theme_supports( 'custom-header' ) ) {
		...
		if ( $args[0]['zc-head-callback'] ) {
			add_action( 'zc_head', $args[0]['zc-head-callback'] );
		}

		if ( is_admin() ) {
			require_once ABSPATH . 'zc-admin/includes/class-custom-image-header.php';
...
function get_theme_support( $feature, ...$args ) {
	...
	if ( ! isset( $_zc_theme_features[ $feature ] ) ) {
		return false;
	}

	if ( ! $args ) {
		return $_zc_theme_features[ $feature ];
	}
	...
	switch ( $feature ) {
		...
		case 'custom-header':
		case 'custom-background':
			return $_zc_theme_features[ $feature ][0][ $args[0] ] ?? false;
...
function _remove_theme_support( $feature ) {
	...
	switch ( $feature ) {
		case 'custom-header':
			...
			if ( isset( $support[0]['zc-head-callback'] ) ) {
				remove_action( 'zc_head', $support[0]['zc-head-callback'] );
			}
	...
	unset( $_zc_theme_features[ $feature ] );
...
function current_theme_supports( $feature, ...$args ) {
	...
	if ( ! $args ) {
		/** This filter is documented in zc-includes/theme.php */
		return apply_filters( "current_theme_supports-{$feature}", true, $args, $_zc_theme_features[ $feature ] ); // phpcs:ignore ZelocoreCMS.NamingConventions.ValidHookName.UseUnderscores
	}
	...
	switch ( $feature ) {
		case 'post-thumbnails':
			 ...
			 */
			if ( true === $_zc_theme_features[ $feature ] ) {  // Registered for all types.
				return true;
			}
			$content_type = $args[0];
			return in_array( $content_type, $_zc_theme_features[ $feature ][0], true );
		...
		case 'post-formats':
			 ...
			 */
			$type = $args[0];
			return in_array( $type, $_zc_theme_features[ $feature ][0], true );

		case 'custom-logo':
		case 'custom-header':
		case 'custom-background':
			// Specific capabilities can be registered by passing an array to add_theme_support().
			return ( isset( $_zc_theme_features[ $feature ][0][ $args[0] ] ) && $_zc_theme_features[ $feature ][0][ $args[0] ] );
	}
 ...
 * @see add_theme_support()
 *
 * @global array $_zc_registered_theme_features
 *
 ...
 *      }
 * }
 * @return true|ZC_Error True if the theme feature was successfully registered, a ZC_Error object if not.
 */
function register_theme_feature( $feature, $args = array() ) {
	global $_zc_registered_theme_features;

	if ( ! is_array( $_zc_registered_theme_features ) ) {
		$_zc_registered_theme_features = array();
	}
	...
	if ( is_array( $args['show_in_rest'] ) ) {
		$args['show_in_rest'] = zc_parse_args(
			$args['show_in_rest'],
			array(
				'schema'           => array(),
				'name'             => $feature,
	...
	if ( is_array( $args['show_in_rest'] ) ) {
		...
		$args['show_in_rest']['schema'] = zc_parse_args(
	...
	$_zc_registered_theme_features[ $feature ] = $args;
...
function get_registered_theme_features() {
	...
	return $_zc_registered_theme_features;
}
...
function get_registered_theme_feature( $feature ) {
	...
	return $_zc_registered_theme_features[ $feature ] ?? null;
}
...
function _delete_attachment_theme_mod( $id ) {
	$attachment_image = zc_get_attachment_url( $id );
...
function check_theme_switched() {
	...
	if ( $stylesheet ) {
		$old_theme = zc_get_theme( $stylesheet );

		// Prevent widget & menu mapping from running since Customizer already called it up front.
		if ( get_option( 'theme_switched_via_customizer' ) ) {
			remove_action( 'after_switch_theme', '_zc_menus_changed' );
			remove_action( 'after_switch_theme', '_zc_sidebars_changed' );
		...
		if ( $old_theme->exists() ) {
			...
			/** This action is documented in zc-includes/theme.php */
...
/**
 * Includes and instantiates the ZC_Customize_Manager class.
 *
 * Loads the Customizer at plugins_loaded when accessing the customize.php admin
 * page or when any request includes a zc_customize=on param or a customize_changeset
 * param (a UUID). This param is a signal for whether to bootstrap the Customizer when
 * ZelocoreCMS is loading, especially in the Customizer preview
 ...
 * @global ZC_Customize_Manager $zc_customize
 */
function _zc_customize_include() {

	$is_customize_admin_page = ( is_admin() && 'customize.php' === basename( $_SERVER['PHP_SELF'] ) );
	$should_include          = (
		$is_customize_admin_page
		||
		( isset( $_REQUEST['zc_customize'] ) && 'on' === $_REQUEST['zc_customize'] )
		||
	...
	/*
	 * Note that zc_unslash() is not being used on the input vars because it is
	 * called before zc_magic_quotes() gets called. Besides this fact, none of
	...
	$input_vars = array_merge(
		zc_array_slice_assoc( $_GET, $keys ),
		zc_array_slice_assoc( $_POST, $keys )
	);
	...
	// Note that theme will be sanitized via ZC_Theme.
	...
	$is_customize_save_action = (
		...
		&&
		'customize_save' === zc_unslash( $_REQUEST['action'] )
	);
 ...
 * @access private
 *
 * @global ZC_Customize_Manager $zc_customize Customizer instance.
 *
 * @param string  $new_status     New post status.
 * @param string  $old_status     Old post status.
 * @param ZC_Post $changeset_post Changeset post object.
 */
function _zc_customize_publish_changeset( $new_status, $old_status, $changeset_post ) {
	global $zc_customize;
	...
	if ( ! did_action( 'customize_register' ) ) {
		 ...
		 * will have a $zc_customize with those settings registered since they
		 ...
		 */
		remove_action( 'customize_register', array( $zc_customize, 'register_controls' ) );
		$zc_customize->register_controls();

		/** This filter is documented in zc-includes/class-zc-customize-manager.php */
		do_action( 'customize_register', $zc_customize );
	}
	$zc_customize->_publish_changeset_values( $changeset_post->ID );
	 ...
	 */
	if ( ! zc_revisions_enabled( $changeset_post ) ) {
		$zc_customize->trash_changeset_post( $changeset_post->ID );
	}
}
 ...
 */
function _zc_customize_changeset_filter_insert_post_data( $post_data, $supplied_post_data ) {
 ...
 * @since 3.4.0
 */
function _zc_customize_loader_settings() {
	...
	$browser = array(
		'mobile' => zc_is_mobile(),
		'ios'    => zc_is_mobile() && preg_match( '/iPad|iPod|iPhone/', $_SERVER['HTTP_USER_AGENT'] ),
	);
	...
	$script = 'var _wpCustomizeLoaderSettings = ' . zc_json_encode( $settings, JSON_HEX_TAG | JSON_UNESCAPED_SLASHES ) . ';';

	$zc_scripts = zc_scripts();
	$data       = $zc_scripts->get_data( 'customize-loader', 'data' );
	if ( $data ) {
		$script = "$data\n$script";
	}

	$zc_scripts->add_data( 'customize-loader', 'data', $script );
}
 ...
 * @return string
 */
function zc_customize_url( $stylesheet = '' ) {
 ...
 */
function zc_customize_support_script() {
	$admin_origin = parse_url( admin_url() );
	$home_origin  = parse_url( home_url() );
	$cross_domain = ( strtolower( $admin_origin['host'] ) !== strtolower( $home_origin['host'] ) );
	ob_start();
...
function is_customize_preview() {
	...
	return ( $zc_customize instanceof ZC_Customize_Manager ) && $zc_customize->is_preview();
}
 ...
 * garbage-collected at the same time by `zc_delete_auto_drafts()`. Otherwise,
 ...
 *
 * @link https://core.trac.zelocorecms.org/ticket/39752
 *
 * @since 4.8.0
 * @access private
...
function _zc_keep_alive_customize_changeset_dependent_auto_drafts( $new_status, $old_status, $post ) {
	...
	if ( 'auto-draft' === $new_status ) {
		 ...
		 */
		$post_args['post_date'] = $post->post_date; // Note zc_delete_auto_drafts() only looks at this date.
	} else {
		/*
...
function create_initial_theme_features() {
	...
	register_theme_feature(
		'zc-block-styles',
		array(
			'description'  => __( 'Whether theme opts in to default ZelocoreCMS block styles for viewing.' ),
			'show_in_rest' => true,
		)
	);
}
 ...
 */
function zc_is_block_theme() {
	if ( empty( $GLOBALS['zc_theme_directories'] ) ) {
		_doing_it_wrong( __FUNCTION__, __( 'This function should not be called before the theme directory is registered.' ), '6.8.0' );
		return false;
	}

	return zc_get_theme()->is_block_theme();
}

/**
 * Given an element name, returns a class name.
 *
 * Alias of ZC_Theme_JSON::get_element_class_name.
 *
 * @since 6.1.0
 *
 ...
 */
function zc_theme_get_element_class_name( $element ) {
	return ZC_Theme_JSON::get_element_class_name( $element );
}
...
function _add_default_theme_supports() {
	...
	add_filter(
		...
		static function ( $active, ZC_Customize_Panel $panel ) {
			if (
				'nav_menus' === $panel->id &&
