 ...
 */
function zc_get_elements_class_name( $block ) {
	return 'zc-elements-' . md5( serialize( $block ) );
}
 ...
 */
function zc_should_add_elements_class_name( $block, $options ) {
	...
	foreach ( $element_color_properties as $element_config ) {
		...
		foreach ( $element_config['paths'] as $path ) {
			if ( null !== _zc_array_get( $elements_style_attributes, $path, null ) ) {
				return true;
			}
		}
	}
 ...
 */
function zc_render_elements_support_styles( $parsed_block ) {
	/*
	 ...
	 *
	 * @link https://github.com/ZelocoreCMS/gutenberg/pull/59535
	 *
	...
	$skip_link_color_serialization         = zc_should_skip_block_supports_serialization( $block_type, 'color', 'link' );
	$skip_heading_color_serialization      = zc_should_skip_block_supports_serialization( $block_type, 'color', 'heading' );
	$skip_button_color_serialization       = zc_should_skip_block_supports_serialization( $block_type, 'color', 'button' );
	...
	if ( ! zc_should_add_elements_class_name( $parsed_block, $options ) ) {
		return $parsed_block;
	}

	$class_name         = zc_get_elements_class_name( $parsed_block );
	...
	$element_types = array(
		'button'  => array(
			'selector' => ".$class_name .zc-element-button, .$class_name .zc-block-button__link",
			'skip'     => $skip_button_color_serialization,
		),
		'link'    => array(
			'selector'       => ".$class_name a:where(:not(.zc-element-button))",
			'hover_selector' => ".$class_name a:where(:not(.zc-element-button)):hover",
 ...
 * block's markup.
 *
 * @see zc_render_elements_support_styles
 * @since 6.6.0
 *
 ...
 */
function zc_render_elements_class_name( $block_content, $block ) {
	$class_string = $block['attrs']['className'] ?? '';
	preg_match( '/\bzc-elements-\S+\b/', $class_string, $matches );
...
add_filter( 'render_block', 'zc_render_elements_class_name', 10, 2 );
add_filter( 'render_block_data', 'zc_render_elements_support_styles', 10, 1 );
