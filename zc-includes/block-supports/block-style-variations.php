 ...
 */
function zc_get_block_style_variation_name_from_class( $class_string ) {
 ...
 */
function zc_resolve_block_style_variation_ref_values( &$variation_data, $theme_json ) {
	foreach ( $variation_data as $key => &$value ) {
		...
		if ( is_array( $value ) ) {
			...
			if ( array_key_exists( 'ref', $value ) ) {
				...
				zc_resolve_block_style_variation_ref_values( $value, $theme_json );
			}
		}
	}
}
 ...
 */
function zc_render_block_style_variation_support_styles( $parsed_block ) {
	$classes    = $parsed_block['attrs']['className'] ?? null;
	$variations = zc_get_block_style_variation_name_from_class( $classes );
	 ...
	 * theme_json data.
	 */
	zc_resolve_block_style_variation_ref_values( $variation_data, $theme_json );

	$variation_instance = zc_unique_id( $variation . '--' );
	...
	$config = array(
		'version'  => ZC_Theme_JSON::LATEST_SCHEMA,
		'settings' => array(
			'spacing' => array(
				'blockGap' => true,
			),
	...
	if ( ! is_admin() ) {
		remove_filter( 'zc_theme_json_get_style_nodes', 'zc_filter_out_block_nodes' );
	}
	...
	$variation_theme_json = new ZC_Theme_JSON( $config, 'blocks' );
	...
	zc_register_style( 'block-style-variation-styles', false, array( 'zc-block-library', 'global-styles' ) );
	zc_add_inline_style( 'block-style-variation-styles', $variation_styles );
 ...
 * @access private
 *
 * @see zc_render_block_style_variation_support_styles
 *
 ...
 */
function zc_render_block_style_variation_class_name( $block_content, $block ) {
 ...
 * @access private
 */
function zc_enqueue_block_style_variation_styles() {
	zc_enqueue_style( 'block-style-variation-styles' );
}

// Register the block support.
ZC_Block_Supports::get_instance()->register( 'block-style-variation', array() );

add_filter( 'render_block_data', 'zc_render_block_style_variation_support_styles', 10, 2 );
add_filter( 'render_block', 'zc_render_block_style_variation_class_name', 10, 2 );
add_action( 'zc_enqueue_scripts', 'zc_enqueue_block_style_variation_styles', 1 );
 ...
 */
function zc_register_block_style_variations_from_theme_json_partials( $variations ) {
	if ( empty( $variations ) ) {
		return;
	}
