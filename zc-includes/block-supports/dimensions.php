 ...
 */
function zc_register_dimensions_support( $block_type ) {
 ...
 */
function zc_apply_dimensions_support( $block_type, $block_attributes ) {
	$attributes = array();

	if ( zc_should_skip_block_supports_serialization( $block_type, 'dimensions' ) ) {
		return $attributes;
	}
	...
	foreach ( $supported_features as $feature ) {
		...
		$skip_serialization = zc_should_skip_block_supports_serialization( $block_type, 'dimensions', $feature );
	...
	$styles = zc_style_engine_get_styles( array( 'dimensions' => $dimensions_block_styles ) );
 ...
 */
function zc_render_dimensions_support( $block_content, $block ) {
	...
	if (
		...
		zc_should_skip_block_supports_serialization( $block_type, 'dimensions', 'aspectRatio' )
	) {
...
add_filter( 'render_block', 'zc_render_dimensions_support', 10, 2 );

// Register the block support.
ZC_Block_Supports::get_instance()->register(
	'dimensions',
	array(
		'register_attribute' => 'zc_register_dimensions_support',
		'apply'              => 'zc_apply_dimensions_support',
	)
);
