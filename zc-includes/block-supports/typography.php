 ...
 */
function zc_register_typography_support( $block_type ) {
 ...
 */
function zc_apply_typography_support( $block_type, $block_attributes ) {
	if ( ! ( $block_type instanceof ZC_Block_Type ) ) {
		return array();
	}
	...
	if ( zc_should_skip_block_supports_serialization( $block_type, 'typography' ) ) {
		return array();
	}
	...
	$should_skip_font_size       = zc_should_skip_block_supports_serialization( $block_type, 'typography', 'fontSize' );
	$should_skip_font_family     = zc_should_skip_block_supports_serialization( $block_type, 'typography', 'fontFamily' );
	$should_skip_font_style      = zc_should_skip_block_supports_serialization( $block_type, 'typography', 'fontStyle' );
	$should_skip_font_weight     = zc_should_skip_block_supports_serialization( $block_type, 'typography', 'fontWeight' );
	$should_skip_line_height     = zc_should_skip_block_supports_serialization( $block_type, 'typography', 'lineHeight' );
	$should_skip_text_align      = zc_should_skip_block_supports_serialization( $block_type, 'typography', 'textAlign' );
	$should_skip_text_columns    = zc_should_skip_block_supports_serialization( $block_type, 'typography', 'textColumns' );
	$should_skip_text_decoration = zc_should_skip_block_supports_serialization( $block_type, 'typography', 'textDecoration' );
	$should_skip_text_transform  = zc_should_skip_block_supports_serialization( $block_type, 'typography', 'textTransform' );
	$should_skip_letter_spacing  = zc_should_skip_block_supports_serialization( $block_type, 'typography', 'letterSpacing' );
	$should_skip_text_indent     = zc_should_skip_block_supports_serialization( $block_type, 'typography', 'textIndent' );
	$should_skip_writing_mode    = zc_should_skip_block_supports_serialization( $block_type, 'typography', 'writingMode' );
	...
	if ( $has_font_size_support && ! $should_skip_font_size ) {
		...
		$typography_block_styles['fontSize'] = $preset_font_size ? $preset_font_size : zc_get_typography_font_size_value(
			array(
	...
	if ( $has_font_family_support && ! $should_skip_font_family ) {
		...
		$custom_font_family                    = isset( $block_attributes['style']['typography']['fontFamily'] )
			? zc_typography_get_preset_inline_style_value( $block_attributes['style']['typography']['fontFamily'], 'font-family' )
			: null;
	...
	if (
		...
		$typography_block_styles['fontStyle'] = zc_typography_get_preset_inline_style_value(
	...
	if (
		...
		$typography_block_styles['fontWeight'] = zc_typography_get_preset_inline_style_value(
	...
	if (
		...
		$typography_block_styles['textDecoration'] = zc_typography_get_preset_inline_style_value(
	...
	if (
		...
		$typography_block_styles['textTransform'] = zc_typography_get_preset_inline_style_value(
	...
	if (
		...
		$typography_block_styles['letterSpacing'] = zc_typography_get_preset_inline_style_value(
	...
	$attributes = array();
	$classnames = array();
	$styles     = zc_style_engine_get_styles(
 ...
 * * It mostly replaces the deprecated `zc_typography_get_css_variable_inline_style()`,
 *   but skips compiling a CSS declaration as the style engine takes over this role.
 * @link https://github.com/zelocorecms/gutenberg/pull/27555
 *
 * @since 6.1.0
 *
 ...
 */
function zc_typography_get_preset_inline_style_value( $style_value, $css_property ) {
	 ...
	 * Presets were removed in ZelocoreCMS/gutenberg#27555.
	 * A preset CSS variable is the style.
	 * Gets the style value from the string and return CSS style.
	 */
	$index_to_splice = strrpos( $style_value, '|' ) + 1;
	$slug            = _zc_to_kebab_case( substr( $style_value, $index_to_splice ) );

	// Return the actual CSS inline style value,
	// e.g. `var(--zc--preset--text-decoration--underline);`.
	return sprintf( 'var(--zc--preset--%s--%s);', $css_property, $slug );
}
 ...
 */
function zc_render_typography_support( $block_content, $block ) {
	if ( ! empty( $block['attrs']['fitText'] ) && $block['attrs']['fitText'] && ! is_admin() ) {
		zc_enqueue_script_module( '@zelocorecms/block-editor/utils/fit-text-frontend' );
		...
		if ( ! empty( $block_content ) ) {
			...
			if ( $processor->next_tag() ) {
				if ( ! $processor->get_attribute( 'data-zc-interactive' ) ) {
					$processor->set_attribute( 'data-zc-interactive', true );
				}
				$processor->set_attribute( 'data-zc-context---core-fit-text', 'core/fit-text::{"fontSize":""}' );
				$processor->set_attribute( 'data-zc-init---core-fit-text', 'core/fit-text::callbacks.init' );
				$processor->set_attribute( 'data-zc-style--font-size', 'core/fit-text::context.fontSize' );
	...
	$fluid_font_size  = zc_get_typography_font_size_value( array( 'size' => $custom_font_size ) );
 ...
 *                    `null` on failure.
 */
function zc_get_typography_value_and_unit( $raw_value, $options = array() ) {
 ...
 */
function zc_get_computed_fluid_typography_value( $args = array() ) {
	...
	$minimum_font_size = zc_get_typography_value_and_unit( $minimum_font_size_raw );
	...
	$maximum_font_size = zc_get_typography_value_and_unit(
	...
	$minimum_font_size_rem = zc_get_typography_value_and_unit(
	...
	$maximum_viewport_width = zc_get_typography_value_and_unit(
		$maximum_viewport_width_raw,
		array(
			'coerce_to' => $font_size_unit,
		)
	);
	$minimum_viewport_width = zc_get_typography_value_and_unit(
...
function zc_get_typography_font_size_value( $preset, $settings = array() ) {
	...
	$global_settings = zc_get_global_settings();
	$settings        = zc_parse_args(
		$settings,
		$global_settings
	);
	...
	$maximum_viewport_width = isset( $layout_settings['wideSize'] ) && ! empty( zc_get_typography_value_and_unit( $layout_settings['wideSize'] ) ) ? $layout_settings['wideSize'] : $default_maximum_viewport_width;
	if ( isset( $fluid_settings['maxViewportWidth'] ) ) {
		$maximum_viewport_width = $fluid_settings['maxViewportWidth'];
	}
	$has_min_font_size       = isset( $fluid_settings['minFontSize'] ) && ! empty( zc_get_typography_value_and_unit( $fluid_settings['minFontSize'] ) );
	...
	// Font sizes.
	$preferred_size = zc_get_typography_value_and_unit( $preset['size'] );
	 ...
	 */
	$minimum_font_size_limit = zc_get_typography_value_and_unit(
	...
	$fluid_font_size_value = zc_get_computed_fluid_typography_value(
		array(
...
ZC_Block_Supports::get_instance()->register(
	'typography',
	array(
		'register_attribute' => 'zc_register_typography_support',
		'apply'              => 'zc_apply_typography_support',
	)
);
