 ...
 * } $parsed_block
 */
function zc_render_custom_css_support_styles( $parsed_block ) {
	$custom_css = $parsed_block['attrs']['style']['css'] ?? null;
	if ( ! is_string( $custom_css ) || '' === trim( $custom_css ) ) {
		return $parsed_block;
	}

	$block_type = ZC_Block_Type_Registry::get_instance()->get_registered( $parsed_block['blockName'] );
	...
	$class_name          = zc_unique_id_from_values( $parsed_block, 'zc-custom-css-' );
	...
	_zc_array_set( $parsed_block, array( 'attrs', 'className' ), $updated_class_name );

	// Process the custom CSS using the same method as global styles.
	$selector      = '.' . $class_name;
	$processed_css = ZC_Theme_JSON::process_blocks_custom_css( $custom_css, $selector );
	...
	if ( ! empty( $processed_css ) ) {
		 ...
		 */
		zc_register_style( 'zc-block-custom-css', false, array( 'global-styles' ) );
		zc_add_inline_style( 'zc-block-custom-css', $processed_css );
	}
 ...
 * @since 7.0.0
 */
function zc_enqueue_block_custom_css() {
	zc_enqueue_style( 'zc-block-custom-css' );
}

/**
 * Applies the custom CSS class name to the block's rendered HTML.
 *
 * The class name is generated in {@see zc_render_custom_css_support_styles()}
 ...
 * } $block
 */
function zc_render_custom_css_class_name( $block_content, $block ) {
	$class_name_attr = $block['attrs']['className'] ?? null;

	if ( ! is_string( $class_name_attr ) || ! str_contains( $class_name_attr, 'zc-custom-css-' ) ) {
		return $block_content;
	}

	// Parse out the 'zc-custom-css-*' class name added by zc_render_custom_css_support_styles().
	$custom_class_name = null;
	$token_delimiter   = " \t\f\r\n";
	$class_token       = strtok( $class_name_attr, $token_delimiter );
	while ( false !== $class_token ) {
		if ( str_starts_with( $class_token, 'zc-custom-css-' ) ) {
...
add_filter( 'render_block', 'zc_render_custom_css_class_name', 10, 2 );
add_filter( 'render_block_data', 'zc_render_custom_css_support_styles', 10, 1 );
add_action( 'zc_enqueue_scripts', 'zc_enqueue_block_custom_css', 1 );
 ...
 */
function zc_register_custom_css_support( $block_type ) {
 ...
 *
 * Uses {@see ZC_Block_Parser::next_token()} to scan block tokens and surgically
 ...
 */
function zc_strip_custom_css_from_blocks( $content ) {
 ...
 * Priority of 8 to run before zc_filter_global_styles_post (priority 9) and zc_filter_post_kses (priority 10).
 *
 * @since 7.0.0
 * @access private
 */
function zc_custom_css_kses_init_filters() {
	add_filter( 'content_save_pre', 'zc_strip_custom_css_from_blocks', 8 );
	add_filter( 'content_filtered_save_pre', 'zc_strip_custom_css_from_blocks', 8 );
}
 ...
 * @access private
 */
function zc_custom_css_remove_filters() {
	remove_filter( 'content_save_pre', 'zc_strip_custom_css_from_blocks', 8 );
	remove_filter( 'content_filtered_save_pre', 'zc_strip_custom_css_from_blocks', 8 );
}
 ...
 * @access private
 */
function zc_custom_css_kses_init() {
	zc_custom_css_remove_filters();
	if ( ! current_user_can( 'edit_css' ) ) {
		zc_custom_css_kses_init_filters();
	}
}
 ...
 */
function zc_custom_css_force_filtered_html_on_import_filter( $arg ) {
	if ( $arg ) {
		zc_custom_css_kses_init_filters();
	}
	return $arg;
...
// Run before zc_filter_global_styles_post (priority 9) and zc_filter_post_kses (priority 10).
add_action( 'init', 'zc_custom_css_kses_init', 20 );
add_action( 'set_current_user', 'zc_custom_css_kses_init' );
add_filter( 'force_filtered_html_on_import', 'zc_custom_css_force_filtered_html_on_import_filter', 999 );

// Register the block support.
ZC_Block_Supports::get_instance()->register(
	'custom-css',
	array(
		'register_attribute' => 'zc_register_custom_css_support',
	)
);
