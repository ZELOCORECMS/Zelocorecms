 ...
 */
function zc_register_position_support( $block_type ) {
 ...
 */
function zc_render_position_support( $block_content, $block ) {
	...
	$class_name      = zc_unique_id( 'zc-container-' );
	$selector        = ".$class_name";
		...
		foreach ( $sides as $side ) {
			...
			if ( null !== $side_value ) {
				...
				if (
					...
					$side_value = "calc($side_value + var(--zc-admin--admin-bar--position-offset, 0px))";
				}
	...
	if ( ! empty( $position_styles ) ) {
		...
		$content = new ZC_HTML_Tag_Processor( $block_content );
		$content->next_tag();
...
ZC_Block_Supports::get_instance()->register(
	'position',
	array(
		'register_attribute' => 'zc_register_position_support',
	)
);
add_filter( 'render_block', 'zc_render_position_support', 10, 2 );
