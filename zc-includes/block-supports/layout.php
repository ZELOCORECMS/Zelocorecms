 ...
 */
function zc_get_block_style_variation_name_from_registered_style( string $class_name, array $registered_styles = array() ): ?string {
	if ( ! $class_name ) {
		return null;
	}
 ...
 */
function zc_get_layout_definitions() {
		...
		'constrained' => array(
			...
			'baseStyles'    => array(
				...
				array(
					...
					'rules'    => array(
						'max-width'    => 'var(--zc--style--global--content-size)',
						'margin-left'  => 'auto !important',
						'margin-right' => 'auto !important',
					),
				),
				array(
					'selector' => ' > .alignwide',
					'rules'    => array(
						'max-width' => 'var(--zc--style--global--wide-size)',
					),
				),
			),
			'spacingStyles' => array(
 ...
 */
function zc_register_layout_support( $block_type ) {
 ...
 */
function zc_get_layout_style( $selector, $layout, $has_block_gap_support = false, $gap_value = null, $should_skip_gap_serialization = false, $fallback_gap_value = '0.5em', $block_spacing = null ) {
	...
	if ( 'default' === $layout_type ) {
		if ( $has_block_gap_support ) {
			...
			if ( null !== $gap_value && ! $should_skip_gap_serialization ) {
				...
				if ( is_string( $gap_value ) && str_contains( $gap_value, 'var:preset|spacing|' ) ) {
					...
					$slug            = _zc_to_kebab_case( substr( $gap_value, $index_to_splice ) );
					$gap_value       = "var(--zc--preset--spacing--$slug)";
				}
		...
		if ( isset( $block_spacing ) ) {
			$block_spacing_values = zc_style_engine_get_styles(
				array(
		...
		if ( $has_block_gap_support && isset( $gap_value ) ) {
			...
			foreach ( $gap_sides as $gap_side ) {
				...
				if ( is_string( $process_value ) && str_contains( $process_value, 'var:preset|spacing|' ) ) {
					...
					$slug            = _zc_to_kebab_case( substr( $process_value, $index_to_splice ) );
					$process_value   = "var(--zc--preset--spacing--$slug)";
				}
	...
	if ( ! empty( $layout_styles ) ) {
		 ...
		 * Since https://github.com/ZelocoreCMS/gutenberg/pull/42452,
		 * zc_enqueue_block_support_styles is no longer called in this block supports file.
		 */
		return zc_style_engine_get_stylesheet_from_css_rules(
			$layout_styles,
			array(
 ...
 */
function zc_render_layout_support_flag( $block_content, $block ) {
	static $global_styles = null;

	$block_type            = ZC_Block_Type_Registry::get_instance()->get_registered( $block['blockName'] );
	...
	if ( $child_layout ) {
		 ...
		 */
		$container_content_class = zc_unique_id_from_values(
			array(
		 ...
		 */
		$child_css = zc_style_engine_get_stylesheet_from_css_rules(
			$child_layout_styles,
			array(
				'context'  => 'block-supports',
				'prettify' => false,
	 ...
	 * removed in the 5.9 release (https://github.com/ZelocoreCMS/gutenberg/issues/38719). It is
	...
	if ( ! current_theme_supports( 'disable-layout-styles' ) ) {
		 ...
		 */
		$should_skip_gap_serialization = zc_should_skip_block_supports_serialization( $block_type, 'spacing', 'blockGap' );
		...
		if ( null === $global_styles ) {
			$global_styles = zc_get_global_styles();
		}
		...
		if ( $block_class_name && str_contains( $block_class_name, 'is-style-' ) && $block_name ) {
			$styles_registry   = ZC_Block_Styles_Registry::get_instance();
			$registered_styles = $styles_registry->get_registered_styles_for_block( $block_name );
			$variation_name    = zc_get_block_style_variation_name_from_registered_style( $block_class_name, $registered_styles );
			if ( $variation_name ) {
		 ...
		 */
		$container_class = zc_unique_id_from_values(
			array(
				$used_layout,
			...
			'zc-container-' . sanitize_title( $block['blockName'] ) . '-is-layout-'
		);

		$style = zc_get_layout_style(
			".$container_class",
			$used_layout,
			$has_block_gap_support,
			$gap_value,
	...
	$class_names[]    = 'zc-block-' . $full_block_name . '-' . $layout_classname;
	...
	if ( is_string( $first_chunk ) && count( $block['innerContent'] ) > 1 ) {
		$first_chunk_processor = new ZC_HTML_Tag_Processor( $first_chunk );
 ...
 * @param ZC_Block $parent_block The parent block.
 * @return array The parsed block with parent layout attribute if it exists.
 */
function zc_add_parent_layout_to_parsed_block( $parsed_block, $source_block, $parent_block ) {
...
add_filter( 'render_block_data', 'zc_add_parent_layout_to_parsed_block', 10, 3 );

// Register the block support.
ZC_Block_Supports::get_instance()->register(
	'layout',
	array(
		'register_attribute' => 'zc_register_layout_support',
	)
);
add_filter( 'render_block', 'zc_render_layout_support_flag', 10, 2 );
 ...
 * @return string Filtered block content.
 */
function zc_restore_group_inner_container( $block_content, $block ) {
	$tag_name                         = $block['attrs']['tagName'] ?? 'div';
	$group_with_inner_container_regex = sprintf(
		'/(^\s*<%1$s\b[^>]*zc-block-group(\s|")[^>]*>)(\s*<div\b[^>]*zc-block-group__inner-container(\s|")[^>]*>)((.|\S|\s)*)/U',
		preg_quote( $tag_name, '/' )
	);

	if (
		zc_theme_has_theme_json() ||
	...
	if ( $processor->next_tag( array( 'class_name' => 'zc-block-group' ) ) ) {
	...
	$replace_regex                  = sprintf(
		'/(^\s*<%1$s\b[^>]*zc-block-group[^>]*>)(.*)(<\/%1$s>\s*$)/ms',
		preg_quote( $tag_name, '/' )
	);
	$updated_content                = preg_replace_callback(
		$replace_regex,
		static function ( $matches ) {
			return $matches[1] . '<div class="zc-block-group__inner-container">' . $matches[2] . '</div>' . $matches[3];
		},
	...
	if ( ! empty( $layout_classes ) ) {
		$processor = new ZC_HTML_Tag_Processor( $updated_content );
		if ( $processor->next_tag( array( 'class_name' => 'zc-block-group__inner-container' ) ) ) {
...
add_filter( 'render_block_core/group', 'zc_restore_group_inner_container', 10, 2 );
 ...
 * @return string Filtered block content.
 */
function zc_restore_image_outer_container( $block_content, $block ) {
	if ( zc_theme_has_theme_json() ) {
		return $block_content;
	}

	$figure_processor = new ZC_HTML_Tag_Processor( $block_content );
	if (
		! $figure_processor->next_tag( 'FIGURE' ) ||
		! $figure_processor->has_class( 'zc-block-image' ) ||
		! (
	 ...
	 *
	 *     // From this…
	 *     <!-- zc:image {"className":"hires"} -->
	 *     <figure class="zc-block-image wide hires">…
	 *
	 *     // To this…
	 *     <div class="zc-block-image hires"><figure class="wide">…
	 */
	$wrapper_processor = new ZC_HTML_Tag_Processor( '<div>' );
	$wrapper_processor->next_token();
	$wrapper_processor->set_attribute(
		'class',
		is_string( $block['attrs']['className'] ?? null )
			? "zc-block-image {$block['attrs']['className']}"
			: 'zc-block-image'
	);

	// And remove them from the existing content; it has been transferred upward.
	$figure_processor->remove_class( 'zc-block-image' );
...
add_filter( 'render_block_core/image', 'zc_restore_image_outer_container', 10, 2 );
