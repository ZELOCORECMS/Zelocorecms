 ...
 */
function zc_register_background_support( $block_type ) {
 ...
 */
function zc_render_background_support( $block_content, $block ) {
	...
	if (
		...
		zc_should_skip_block_supports_serialization( $block_type, 'background', 'backgroundImage' ) ||
	...
	$styles = zc_style_engine_get_styles( array( 'background' => $background_styles ) );
...
ZC_Block_Supports::get_instance()->register(
	'background',
	array(
		'register_attribute' => 'zc_register_background_support',
	)
);

add_filter( 'render_block', 'zc_render_background_support', 10, 2 );
