 ...
 */
function zc_render_block_visibility_support( $block_content, $block ) {
	...
	if ( is_array( $block_visibility ) && ! empty( $block_visibility ) ) {
		...
		/*
		 * Viewport size definitions are in several places in ZelocoreCMS packages.
		 * The following are taken from: https://github.com/ZelocoreCMS/gutenberg/blob/trunk/packages/base-styles/_breakpoints.scss
		...
		foreach ( $hidden_on as $hidden_viewport_size ) {
			 ...
			 */
			$visibility_class = 'zc-block-hidden-' . $hidden_viewport_size;
		...
		if ( ! empty( $block_content ) ) {
			...
			if ( $processor->next_tag() ) {
				...
				/*
				 * Set all IMG tags to be `fetchpriority=auto` so that zc_get_loading_optimization_attributes() won't add
...
add_filter( 'render_block', 'zc_render_block_visibility_support', 10, 2 );
