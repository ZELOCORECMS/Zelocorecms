 ...
 */
function zc_register_aria_label_support( $block_type ) {
 ...
 */
function zc_apply_aria_label_support( $block_type, $block_attributes ) {
	if ( ! $block_attributes ) {
		return array();
	}
	...
	if (
		...
		zc_should_skip_block_supports_serialization( $block_type, 'ariaLabel' )
	) {
		return array();
	}
...
ZC_Block_Supports::get_instance()->register(
	'aria-label',
	array(
		'register_attribute' => 'zc_register_aria_label_support',
		'apply'              => 'zc_apply_aria_label_support',
	)
);
