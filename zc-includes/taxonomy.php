 ...
 *
 * This function fires twice: in zc-settings.php before plugins are loaded (for
 * backward compatibility reasons), and again on the {@see 'init'} action. We must
 * avoid registering rewrite rules before the {@see 'init'} action.
 *
 * @since 2.8.0
 * @since 5.9.0 Added `'zc_template_part_area'` taxonomy.
 *
...
function create_initial_taxonomies() {
	...
	ZC_Taxonomy::reset_default_labels();
	...
	if ( ! did_action( 'init' ) ) {
		...
		$rewrite          = array(
			'category'    => array(
				...
				'with_front'   => ! get_option( 'category_base' ) || $zc_rewrite->using_index_permalinks(),
				'ep_mask'      => EP_CATEGORIES,
			),
			'post_tag'    => array(
				'hierarchical' => false,
				'slug'         => get_option( 'tag_base' ) ? get_option( 'tag_base' ) : 'tag',
				'with_front'   => ! get_option( 'tag_base' ) || $zc_rewrite->using_index_permalinks(),
				'ep_mask'      => EP_TAGS,
			),
	...
	register_taxonomy(
		...
		array(
			...
			'rest_controller_class' => 'ZC_REST_Terms_Controller',
		)
	);
	...
	register_taxonomy(
		...
		array(
			...
			'show_in_rest'          => true,
			'rest_base'             => 'menus',
			'rest_controller_class' => 'ZC_REST_Menus_Controller',
		)
	);
	...
	register_taxonomy(
		'zc_theme',
		array( 'zc_template', 'zc_template_part', 'zc_global_styles' ),
		array(
			'public'            => false,
			'hierarchical'      => false,
			'labels'            => array(
	...
	register_taxonomy(
		'zc_template_part_area',
		array( 'zc_template_part' ),
		array(
			'public'            => false,
			'hierarchical'      => false,
			'labels'            => array(
 ...
 * @since 3.0.0
 *
 * @global ZC_Taxonomy[] $zc_taxonomies The registered taxonomies.
 *
 ...
 *                         Default 'and'.
 * @return string[]|ZC_Taxonomy[] An array of taxonomy names or objects.
 */
function get_taxonomies( $args = array(), $output = 'names', $operator = 'and' ) {
	global $zc_taxonomies;

	$field = ( 'names' === $output ) ? 'name' : false;

	return zc_filter_object_list( $zc_taxonomies, $args, $operator, $field );
}
 ...
 * @global ZC_Taxonomy[] $zc_taxonomies The registered taxonomies.
 *
 * @param string|string[]|ZC_Post $object_type Name of the type of taxonomy object, or an object (row from posts).
 * @param string                  $output      Optional. The type of output to return in the array. Accepts either
 *                                             'names' or 'objects'. Default 'names'.
 * @return string[]|ZC_Taxonomy[] The names or objects of all taxonomies of `$object_type`.
 */
function get_object_taxonomies( $object_type, $output = 'names' ) {
	...
	$taxonomies = array();
	foreach ( (array) $zc_taxonomies as $tax_name => $tax_obj ) {
 ...
 * @return ZC_Taxonomy|false The taxonomy object or false if $taxonomy doesn't exist.
 */
...
function taxonomy_exists( $taxonomy ) {
	...
	return is_string( $taxonomy ) && isset( $zc_taxonomies[ $taxonomy ] );
}
 ...
 *
 * @global ZC_Taxonomy[] $zc_taxonomies Registered taxonomies.
 *
 ...
 *     @type string        $rest_controller_class REST API Controller class name. Default is 'ZC_REST_Terms_Controller'.
 ...
 *         @type bool   $with_front   Should the permastruct be prepended with ZC_Rewrite::$front. Default true.
 ...
 *                                                provided to `zc_set_object_terms()`. Default null which equates to false.
 *     @type array         $args                  Array of arguments to automatically use inside `zc_get_object_terms()`
 *                                                for this taxonomy.
 *     @type bool          $_builtin              This taxonomy is a "built-in" taxonomy. INTERNAL USE ONLY!
 *                                                Default false.
 * }
 * @return ZC_Taxonomy|ZC_Error The registered taxonomy object on success, ZC_Error object on failure.
 */
function register_taxonomy( $taxonomy, $object_type, $args = array() ) {
	global $zc_taxonomies;

	if ( ! is_array( $zc_taxonomies ) ) {
		$zc_taxonomies = array();
	}
	...
	if ( empty( $taxonomy ) || strlen( $taxonomy ) > 32 ) {
		...
		return new ZC_Error( 'taxonomy_length_invalid', __( 'Taxonomy names must be between 1 and 32 characters in length.' ) );
	}

	$taxonomy_object = new ZC_Taxonomy( $taxonomy, $object_type, $args );
	$taxonomy_object->add_rewrite_rules();

	$zc_taxonomies[ $taxonomy ] = $taxonomy_object;
 ...
 * @since 4.5.0
 *
 * @global ZC_Taxonomy[] $zc_taxonomies List of taxonomies.
 *
 * @param string $taxonomy Taxonomy name.
 * @return true|ZC_Error True on success, ZC_Error on failure or if the taxonomy doesn't exist.
 */
function unregister_taxonomy( $taxonomy ) {
	...
	if ( $taxonomy_object->_builtin ) {
		return new ZC_Error( 'invalid_taxonomy', __( 'Unregistering a built-in taxonomy is not allowed.' ) );
	}
 ...
 *
 * @param ZC_Taxonomy $tax Taxonomy object.
 * @return object {
...
function get_taxonomy_labels( $tax ) {
	...
	$nohier_vs_hier_defaults = ZC_Taxonomy::get_default_labels();
...
function register_taxonomy_for_object_type( $taxonomy, $object_type ) {
	...
	if ( ! isset( $zc_taxonomies[ $taxonomy ] ) ) {
		return false;
	}
	...
	if ( ! in_array( $object_type, $zc_taxonomies[ $taxonomy ]->object_type, true ) ) {
		$zc_taxonomies[ $taxonomy ]->object_type[] = $object_type;
	}

	// Filter out empties.
	$zc_taxonomies[ $taxonomy ]->object_type = array_filter( $zc_taxonomies[ $taxonomy ]->object_type );
...
function unregister_taxonomy_for_object_type( $taxonomy, $object_type ) {
	...
	$key = array_search( $object_type, $zc_taxonomies[ $taxonomy ]->object_type, true );
	if ( false === $key ) {
		return false;
	}

	unset( $zc_taxonomies[ $taxonomy ]->object_type[ $key ] );
 ...
 * On failure of finding a valid taxonomy, it will return a ZC_Error.
 *
 ...
 * }
 * @return string[]|ZC_Error An array of object IDs as numeric strings on success,
...
function get_tax_sql( $tax_query, $primary_table, $primary_id_column ) {
	$tax_query_obj = new ZC_Tax_Query( $tax_query );
 ...
 *
 * @since 2.3.0
 * @since 4.4.0 Converted to return a ZC_Term object if `$output` is `OBJECT`.
 *              The `$taxonomy` parameter was made optional.
 *
 * @see sanitize_term_field() The $context param lists the available values for get_term_by() $filter param.
 *
 * @param int|ZC_Term|object $term     If integer, term data will be fetched from the database,
 *                                     or from the cache if available.
 *                                     If stdClass object (as in the results of a database query),
 *                                     will apply filters and return a `ZC_Term` object with the `$term` data.
 *                                     If `ZC_Term`, will return `$term`.
 * @param string             $taxonomy Optional. Taxonomy name that `$term` is part of.
 * @param string             $output   Optional. The required return type. One of OBJECT, ARRAY_A, or ARRAY_N, which
 *                                     correspond to a ZC_Term object, an associative array, or a numeric array,
 *                                     respectively. Default OBJECT.
 * @param string             $filter   Optional. How to sanitize term fields. Default 'raw'.
 * @return ZC_Term|array|ZC_Error|null ZC_Term instance (or array) on success, depending on the `$output` value.
 *                                     ZC_Error if `$taxonomy` does not exist. Null for miscellaneous failure.
 */
function get_term( $term, $taxonomy = '', $output = OBJECT, $filter = 'raw' ) {
	if ( empty( $term ) ) {
		return new ZC_Error( 'invalid_term', __( 'Empty Term.' ) );
	}
	...
	if ( $term instanceof ZC_Term ) {
		$_term = $term;
	} elseif ( is_object( $term ) ) {
		if ( empty( $term->filter ) || 'raw' === $term->filter ) {
			$_term = sanitize_term( $term, $taxonomy, 'raw' );
			$_term = new ZC_Term( $_term );
		} else {
			$_term = ZC_Term::get_instance( $term->term_id );
		}
	} else {
		$_term = ZC_Term::get_instance( $term, $taxonomy );
	}

	if ( is_zc_error( $_term ) ) {
		return $_term;
	} elseif ( ! $_term ) {
		return null;
	}
	 ...
	 *
	 * @since 2.3.0
	 * @since 4.4.0 `$_term` is now a ZC_Term object.
	 *
	 * @param ZC_Term $_term    Term object.
 ...
 *              a ZC_Term object if `$output` is `OBJECT`.
 ...
 *                             respectively. Default OBJECT.
 * @param string     $filter   Optional. How to sanitize term fields. Default 'raw'.
 * @return ZC_Term|array|false ZC_Term instance (or array) on success, depending on the `$output` value.
...
function get_term_by( $field, $value, $taxonomy = '', $output = OBJECT, $filter = 'raw' ) {
	...
	if ( is_zc_error( $terms ) || empty( $terms ) ) {
		return false;
	}
 ...
 * @param string $taxonomy Taxonomy name.
 * @return array|ZC_Error List of term IDs. ZC_Error returned if `$taxonomy` does not exist.
 */
 ...
 *
 * @since 2.3.0
 * @since 4.4.0 The `$taxonomy` parameter was made optional. `$term` can also now accept a ZC_Term object.
 *
 * @see sanitize_term_field()
 *
 * @param string      $field    Term field to fetch.
 * @param int|ZC_Term $term     Term ID or object.
 * @param string      $taxonomy Optional. Taxonomy name. Default empty.
 * @param string      $context  Optional. How to sanitize term fields. Look at sanitize_term_field() for available options.
 *                              Default 'display'.
 * @return string|int|null|ZC_Error Will return an empty string if $term is not an object or if $field is not set in $term.
 */
 ...
 * @param string     $taxonomy Taxonomy name.
 * @return string|int|null|ZC_Error Will return empty string if $term is not an object.
 */
 ...
 * ZC_Term_Query::get_terms() for details. In all cases, a `ZC_Error` object will
 ...
 *              a list of ZC_Term objects.
 * @since 4.5.0 Changed the function signature so that the `$args` array can be provided as the first parameter.
 *              Introduced 'meta_key' and 'meta_value' parameters. Introduced the ability to order results by metadata.
 * @since 4.8.0 Introduced 'suppress_filter' parameter.
 *
 * @param array|string $args       Optional. Array or string of arguments. See ZC_Term_Query::__construct()
 *                                 for information on accepted arguments. Default empty array.
 * @param array|string $deprecated Optional. Argument array, when using the legacy function parameter format.
 *                                 If present, this parameter will be interpreted as `$args`, and the first
 *                                 function parameter will be parsed as a taxonomy or array of taxonomies.
 *                                 Default empty.
 * @return ZC_Term[]|int[]|string[]|string|ZC_Error Array of terms, a count thereof as a numeric string,
 *                                                  or ZC_Error if any of the taxonomies do not exist.
 *                                                  See the function description for more information.
 */
function get_terms( $args = array(), $deprecated = '' ) {
	$term_query = new ZC_Term_Query();
	 ...
	 */
	$_args          = zc_parse_args( $args );
	$key_intersect  = array_intersect_key( $term_query->query_var_defaults, (array) $_args );
	$do_legacy_args = $deprecated || empty( $key_intersect );

	if ( $do_legacy_args ) {
		$taxonomies       = (array) $args;
		$args             = zc_parse_args( $deprecated, $defaults );
	...
	// Don't pass suppress_filter to ZC_Term_Query.
	 ...
	 * @param ZC_Term_Query $term_query The ZC_Term_Query object.
	 */
 ...
 *                           Default false.
 * @return int|false|ZC_Error Meta ID on success, false on failure.
 *                            ZC_Error when term_id is ambiguous between taxonomies.
 */
function add_term_meta( $term_id, $meta_key, $meta_value, $unique = false ) {
	if ( zc_term_is_shared( $term_id ) ) {
		return new ZC_Error( 'ambiguous_term_id', __( 'Term meta cannot be added to terms that are shared between taxonomies.' ), $term_id );
	}
 ...
 * @return int|bool|ZC_Error Meta ID if the key didn't exist. true on successful update,
 ...
 * @param array $term_ids List of term IDs.
 */
function zc_lazyload_term_meta( array $term_ids ) {
	if ( empty( $term_ids ) ) {
		return;
	}
...
function has_term_meta( $term_id ) {
	$check = zc_check_term_meta_support_prefilter( null );
	if ( null !== $check ) {
		return $check;
	}
...
function term_exists( $term, $taxonomy = '', $parent_term = null ) {
	...
	if ( is_int( $term ) ) {
		...
		$args  = zc_parse_args( array( 'include' => array( $term ) ), $defaults );
		$terms = get_terms( $args );
	} else {
		$term = trim( zc_unslash( $term ) );
		if ( '' === $term ) {
			return null;
		}

		$args  = zc_parse_args( array( 'slug' => sanitize_title( $term ) ), $defaults );
		$terms = get_terms( $args );
		if ( empty( $terms ) || is_zc_error( $terms ) ) {
			$args  = zc_parse_args( array( 'name' => $term ), $defaults );
			$terms = get_terms( $args );
		}
	}
 ...
 *                                 function parameter will be parsed as a taxonomy or array of taxonomies.
 *                                 Default empty.
 * @return string|ZC_Error Numeric string containing the number of terms in that
 *                         taxonomy or ZC_Error if the taxonomy does not exist.
 */
function zc_count_terms( $args = array(), $deprecated = '' ) {
	...
	if ( $args
		&& ( is_string( $args ) && taxonomy_exists( $args )
			|| is_array( $args ) && zc_is_numeric_array( $args ) )
	) {
 ...
 */
function zc_delete_object_term_relationships( $object_id, $taxonomies ) {
	...
	foreach ( (array) $taxonomies as $taxonomy ) {
		$term_ids = zc_get_object_terms( $object_id, $taxonomy, array( 'fields' => 'ids' ) );
		if ( ! is_array( $term_ids ) ) {
			// Skip return value in the case of an error or the 'zc_get_object_terms' filter returning an invalid value.
			continue;
		}
		$term_ids = array_map( 'intval', $term_ids );
		zc_remove_object_terms( $object_id, $term_ids, $taxonomy );
	}
}
 ...
 *                               Default false.
 * }
 * @return bool|int|ZC_Error True on success, false if term does not exist. Zero on attempted
...
function zc_delete_term( $term, $taxonomy, $args = array() ) {
	...
	foreach ( $object_ids as $object_id ) {
		if ( ! isset( $default ) ) {
			zc_remove_object_terms( $object_id, $term, $taxonomy );
			continue;
		}

		$terms = zc_get_object_terms(
			$object_id,
			$taxonomy,
			array(
				'fields'  => 'ids',
		...
		zc_set_object_terms( $object_id, $terms, $taxonomy );
	}
	 ...
	 * @param int     $tt_id        Term taxonomy ID.
	 * @param string  $taxonomy     Taxonomy slug.
	 * @param ZC_Term $deleted_term Copy of the already-deleted term.
 ...
 * @return bool|int|ZC_Error Returns true if completes delete action; false if term doesn't exist;
 *                           Zero on attempted deletion of default Category; ZC_Error object is
 *                           also a possibility.
 */
function zc_delete_category( $cat_id ) {
	return zc_delete_term( $cat_id, 'category' );
}
 ...
 *              'all_with_object_id', an array of `ZC_Term` objects will be returned.
 * @since 4.7.0 Refactored to use ZC_Term_Query, and to support any ZC_Term_Query arguments.
 ...
 * @param array|string    $args       See ZC_Term_Query::__construct() for supported arguments.
 * @return ZC_Term[]|int[]|string[]|string|ZC_Error Array of terms, a count thereof as a numeric string,
 *                                                  or ZC_Error if any of the taxonomies do not exist.
 *                                                  See ZC_Term_Query::get_terms() for more information.
 */
function zc_get_object_terms( $object_ids, $taxonomies, $args = array() ) {
	 ...
	 *                             See {@see zc_get_object_terms()} for details.
	 * @param int[]    $object_ids Array of object IDs.
	 * @param string[] $taxonomies Array of taxonomy names to retrieve terms from.
	 */
	$args = apply_filters( 'zc_get_object_terms_args', $args, $object_ids, $taxonomies );
	...
	if ( count( $taxonomies ) > 1 ) {
		foreach ( $taxonomies as $index => $taxonomy ) {
			...
			if ( isset( $t->args ) && is_array( $t->args ) && array_merge( $args, $t->args ) != $args ) {
				...
				$terms = array_merge( $terms, zc_get_object_terms( $object_ids, $taxonomy, array_merge( $args, $t->args ) ) );
			}
		}
	} else {
	 ...
	 * @since 4.2.0
	 *
	 * @param ZC_Term[]|int[]|string[]|string $terms      Array of terms or a count thereof as a numeric string.
	 * @param int[]                           $object_ids Array of object IDs for which terms were retrieved.
	 * @param string[]                        $taxonomies Array of taxonomy names from which terms were retrieved.
	 * @param array                           $args       Array of arguments for retrieving terms for the given
	 *                                                    object(s). See zc_get_object_terms() for details.
	 */
	 ...
	 *                                                    object(s). See zc_get_object_terms() for details.
	 */
	return apply_filters( 'zc_get_object_terms', $terms, $object_ids, $taxonomies, $args );
}
 ...
 * a ZC_Error object will be returned.
 *
 ...
 *     An array of the new term data, ZC_Error otherwise.
 *
...
function zc_insert_term( $term, $taxonomy, $args = array() ) {
	 ...
	 *
	 * @param string|ZC_Error $term     The term name to add, or a ZC_Error object if there's an error.
	 * @param string          $taxonomy Taxonomy slug.
	 * @param array|string    $args     Array or query string of arguments passed to zc_insert_term().
	 */
	...
	if ( is_int( $term ) && 0 === $term ) {
		return new ZC_Error( 'invalid_term_id', __( 'Invalid term ID.' ) );
	}

	if ( '' === trim( $term ) ) {
		return new ZC_Error( 'empty_term_name', __( 'A name is required for this term.' ) );
	}
	...
	if ( (int) $args['parent'] > 0 && ! term_exists( (int) $args['parent'] ) ) {
		return new ZC_Error( 'missing_parent', __( 'Parent term does not exist.' ) );
	}
	...
	$name        = zc_unslash( $args['name'] );
	$description = zc_unslash( $args['description'] );
	$parent      = (int) $args['parent'];

	// Sanitization could clean the name to an empty string that must be checked again.
	if ( '' === $name ) {
		return new ZC_Error( 'invalid_term_name', __( 'Invalid term name.' ) );
	}
	...
	if ( $name_match ) {
		...
		if ( ! $slug_provided || $name_match->slug === $slug || $slug_match ) {
			if ( is_taxonomy_hierarchical( $taxonomy ) ) {
				...
				$existing_term = null;
				$sibling_names = zc_list_pluck( $siblings, 'name' );
				$sibling_slugs = zc_list_pluck( $siblings, 'slug' );
				...
				if ( $existing_term ) {
					return new ZC_Error( 'term_exists', __( 'A term with the name provided already exists with this parent.' ), $existing_term->term_id );
				}
			} else {
				return new ZC_Error( 'term_exists', __( 'A term with the name provided already exists in this taxonomy.' ), $name_match->term_id );
			}
		}
	}

	$slug = zc_unique_term_slug( $slug, (object) $args );
	 ...
	 * @param string $taxonomy Taxonomy slug.
	 * @param array  $args     Arguments passed to zc_insert_term().
	 */
	 ...
	 *
	 * Term parent + taxonomy + slug combinations are meant to be unique, and zc_insert_term()
...
function zc_set_object_terms( $object_id, $terms, $taxonomy, $append = false ) {
	...
	if ( ! $append ) {
		$old_tt_ids = zc_get_object_terms(
			$object_id,
			$taxonomy,
			array(
				'fields'                 => 'tt_ids',
	...
	foreach ( (array) $terms as $term ) {
		...
		if ( ! $term_info ) {
			...
			$term_info = zc_insert_term( $term, $taxonomy );
		}

		if ( is_zc_error( $term_info ) ) {
			return $term_info;
		}
	...
	if ( $new_tt_ids ) {
		zc_update_term_count( $new_tt_ids, $taxonomy );
	}
	...
	if ( ! $append && isset( $t->sort ) && $t->sort ) {
		...
		$final_tt_ids = zc_get_object_terms(
			$object_id,
			$taxonomy,
			array(
				'fields'                 => 'tt_ids',
 ...
 * @return array|ZC_Error Term taxonomy IDs of the affected terms.
 */
function zc_add_object_terms( $object_id, $terms, $taxonomy ) {
	return zc_set_object_terms( $object_id, $terms, $taxonomy, true );
}
...
function zc_remove_object_terms( $object_id, $terms, $taxonomy ) {
	...
	if ( $tt_ids ) {
		...
		zc_update_term_count( $tt_ids, $taxonomy );
...
function zc_unique_term_slug( $slug, $term ) {
	...
	if ( $needs_suffix && is_taxonomy_hierarchical( $term->taxonomy ) && ! empty( $term->parent ) ) {
		...
		while ( ! empty( $the_parent ) ) {
			...
			if ( is_zc_error( $parent_term ) || empty( $parent_term ) ) {
				break;
			}
	 ...
	 */
	if ( apply_filters( 'zc_unique_term_slug_is_bad_slug', $needs_suffix, $slug, $term ) ) {
		if ( $parent_suffix ) {
			$slug .= $parent_suffix;
		}
	 ...
	 */
	return apply_filters( 'zc_unique_term_slug', $slug, $term, $original_slug );
}
 ...
 * If you set 'slug' and it isn't unique, then a ZC_Error is returned.
 ...
 *     @type string $slug        The term slug to use. Default empty string.
 * }
 * @return array|ZC_Error An array containing the `term_id` and `term_taxonomy_id`,
 *                        ZC_Error otherwise.
 */
function zc_update_term( $term_id, $taxonomy, $args = array() ) {
	 ...
	 * @param array  $args        Arguments passed to zc_update_term().
	 */
	$parent = (int) apply_filters( 'zc_update_term_parent', $args['parent'], $term_id, $taxonomy, $parsed_args, $args );
	...
	if ( $duplicate && $duplicate->term_id !== $term_id ) {
		...
		if ( $empty_slug || ( $parent !== (int) $term['parent'] ) ) {
			...
			return new ZC_Error( 'duplicate_term_slug', sprintf( __( 'The slug &#8220;%s&#8221; is already in use by another term.' ), $slug ) );
		}
	}
 ...
 */
function zc_defer_term_counting( $defer = null ) {
	static $_defer = false;
	...
	if ( is_bool( $defer ) ) {
		...
		if ( ! $defer ) {
			zc_update_term_count( null, null, true );
		}
	}
 ...
 */
function zc_update_term_count( $terms, $taxonomy, $do_deferred = false ) {
	static $_deferred = array();

	if ( $do_deferred ) {
		foreach ( (array) array_keys( $_deferred ) as $tax ) {
			zc_update_term_count_now( $_deferred[ $tax ], $tax );
			unset( $_deferred[ $tax ] );
		}
	}
	...
	return zc_update_term_count_now( $terms, $taxonomy );
}
 ...
 */
function zc_update_term_count_now( $terms, $taxonomy ) {
	$terms = array_map( 'intval', $terms );
...
function clean_object_term_cache( $object_ids, $object_type ) {
	...
	foreach ( $taxonomies as $taxonomy ) {
		zc_cache_delete_multiple( $object_ids, "{$taxonomy}_relationships" );
	}
...
function clean_term_cache( $ids, $taxonomy = '', $clean_taxonomy = true ) {
	...
	if ( empty( $taxonomy ) ) {
		...
		zc_cache_delete_multiple( $ids, 'terms' );
...
function clean_taxonomy_cache( $taxonomy ) {
	zc_cache_delete( 'all_ids', $taxonomy );
	zc_cache_delete( 'get', $taxonomy );
	zc_cache_set_terms_last_changed();
 ...
 *
 * @since 2.3.0
 * @since 4.7.0 Returns a `ZC_Error` object if there's an error with
 *              any of the matched terms.
 *
 * @param int    $id       Term object ID, for example a post, comment, or user ID.
 * @param string $taxonomy Taxonomy name.
 * @return bool|ZC_Term[]|ZC_Error Array of `ZC_Term` objects, if cached.
 *                                 False if cache is empty for `$taxonomy` and `$id`.
 *                                 ZC_Error if get_term() returns an error object for any term.
 */
function get_object_term_cache( $id, $taxonomy ) {
	$_term_ids = zc_cache_get( $id, "{$taxonomy}_relationships" );
...
function update_object_term_cache( $object_ids, $object_type ) {
	...
	foreach ( $taxonomies as $taxonomy ) {
		$cache_values = zc_cache_get_multiple( (array) $object_ids, "{$taxonomy}_relationships" );
	...
	foreach ( $cache_values as $taxonomy => $data ) {
		zc_cache_add_multiple( $data, "{$taxonomy}_relationships" );
	}
}
 ...
 * @since 2.3.0
 *
 * @param ZC_Term[] $terms    Array of term objects to change.
...
function update_term_cache( $terms, $taxonomy = '' ) {
	...
	zc_cache_add_multiple( $data, 'terms' );
}
 ...
 * @return array|ZC_Error The subset of $terms that are descendants of $term_id.
 */
...
function _update_post_term_count( $terms, $taxonomy ) {
	 ...
	 * @param ZC_Taxonomy $taxonomy      Current taxonomy object.
	 */
 ...
 *                                     Default: true.
 * @return int|ZC_Error When the current term does not need to be split (or cannot be split on the current
...
function _split_shared_term( $term_id, $term_taxonomy_id, $record = true ) {
	...
	// Keep a record of term_ids that have been split, keyed by old term_id. See zc_get_split_term().
	if ( $record ) {
...
function _zc_batch_split_terms() {
	...
	if ( ! $lock_result ) {
		...
		if ( ! $lock_result || ( $lock_result > ( time() - HOUR_IN_SECONDS ) ) ) {
			zc_schedule_single_event( time() + ( 5 * MINUTE_IN_SECONDS ), 'zc_split_shared_term_batch' );
			return;
		}
	}
	...
	zc_schedule_single_event( time() + ( 2 * MINUTE_IN_SECONDS ), 'zc_split_shared_term_batch' );
...
/**
 * In order to avoid the _zc_batch_split_terms() job being accidentally removed,
 * checks that it's still scheduled while we haven't finished splitting terms.
 *
 * @ignore
 * @since 4.3.0
 */
function _zc_check_for_scheduled_split_terms() {
	if ( ! get_option( 'finished_splitting_shared_terms' ) && ! zc_next_scheduled( 'zc_split_shared_term_batch' ) ) {
		zc_schedule_single_event( time() + MINUTE_IN_SECONDS, 'zc_split_shared_term_batch' );
	}
}
 ...
 * @param string $taxonomy         Taxonomy for the split term.
 */
function _zc_check_split_default_terms( $term_id, $new_term_id, $term_taxonomy_id, $taxonomy ) {
	if ( 'category' !== $taxonomy ) {
		return;
	}
 ...
 * @param string $taxonomy         Taxonomy for the split term.
 */
function _zc_check_split_nav_menu_terms( $term_id, $new_term_id, $term_taxonomy_id, $taxonomy ) {
 ...
 */
function zc_get_split_terms( $old_term_id ) {
 ...
 */
function zc_get_split_term( $old_term_id, $taxonomy ) {
	$split_terms = zc_get_split_terms( $old_term_id );
 ...
 * @global ZC_Rewrite $zc_rewrite ZelocoreCMS rewrite component.
 *
 * @param ZC_Term|int|string $term     The term object, ID, or slug whose link will be retrieved.
 * @param string             $taxonomy Optional. Taxonomy. Default empty.
 * @return string|ZC_Error URL of the taxonomy term archive on success, ZC_Error if term does not exist.
 */
function get_term_link( $term, $taxonomy = '' ) {
	...
	if ( ! is_object( $term ) ) {
		$term = new ZC_Error( 'invalid_term', __( 'Empty Term.' ) );
	}
	...
	$termlink = $zc_rewrite->get_extra_permastruct( $taxonomy );
	 ...
	 * @param ZC_Term $term     The term object.
	 */
 ...
 *
 *     @type int|ZC_Post $post   Post ID or object to get taxonomies of. Default current post.
...
function get_the_taxonomies( $post = 0, $args = array() ) {
	...
	foreach ( get_object_taxonomies( $post ) as $taxonomy ) {
		...
		if ( false === $terms ) {
			$terms = zc_get_object_terms( $post->ID, $taxonomy, $t['args'] );
		}
		$links = array();

		foreach ( $terms as $term ) {
			$links[] = zc_sprintf( $t['term_template'], esc_attr( get_term_link( $term ) ), $term->name );
		}
		if ( $links ) {
			$taxonomies[ $taxonomy ] = zc_sprintf( $t['template'], $t['label'], $links, $terms );
		}
	}
	return $taxonomies;
}
 ...
 * @return bool|ZC_Error ZC_Error on input error.
 */
function is_object_in_term( $object_id, $taxonomy, $terms = null ) {
	$object_id = (int) $object_id;
	if ( ! $object_id ) {
		return new ZC_Error( 'invalid_object', __( 'Invalid object ID.' ) );
	}

	$object_terms = get_object_term_cache( $object_id, $taxonomy );
	if ( false === $object_terms ) {
		$object_terms = zc_get_object_terms( $object_id, $taxonomy, array( 'update_term_meta_cache' => false ) );
		if ( is_zc_error( $object_terms ) ) {
			return $object_terms;
		}

		zc_cache_set( $object_id, zc_list_pluck( $object_terms, 'term_id' ), "{$taxonomy}_relationships" );
	}
...
function get_ancestors( $object_id = 0, $object_type = '', $resource_type = '' ) {
	...
	if ( 'taxonomy' === $resource_type ) {
		...
		while ( ! is_zc_error( $term ) && ! empty( $term->parent ) && ! in_array( $term->parent, $ancestors, true ) ) {
 ...
 */
function zc_get_term_taxonomy_parent_id( $term_id, $taxonomy ) {
	$term = get_term( $term_id, $taxonomy );
	if ( ! $term || is_zc_error( $term ) ) {
		return false;
	}
 ...
 *
 * Attached to the {@see 'zc_update_term_parent'} filter.
 *
 * @since 3.1.0
 *
 ...
 */
function zc_check_term_hierarchy_for_loops( $parent_term, $term_id, $taxonomy ) {
	...
	$loop = zc_find_hierarchy_loop( 'zc_get_term_taxonomy_parent_id', $term_id, $parent_term, array( $taxonomy ) );
	if ( ! $loop ) {
	...
	foreach ( array_keys( $loop ) as $loop_member ) {
		zc_update_term( $loop_member, $taxonomy, array( 'parent' => 0 ) );
	}
 ...
 * @since 5.1.0
 *
 * @param string|ZC_Taxonomy $taxonomy Taxonomy name or object.
 ...
 * @since 6.1.0
 *
 * @param int|ZC_Term $term Term ID or term object.
 ...
 * @since 5.0.0
 */
function zc_cache_set_terms_last_changed() {
	zc_cache_set_last_changed( 'terms' );
}
 ...
 */
function zc_check_term_meta_support_prefilter( $check ) {
