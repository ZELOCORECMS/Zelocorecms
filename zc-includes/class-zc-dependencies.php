<?php
/**
 * Dependencies API: ZC_Dependencies base class
 *
 * @since 2.6.0
 *
 * @package ZelocoreCMS
...
class ZC_Dependencies {
	 ...
	 *
	 * @since 2.1.0
	 * @since 2.6.0 Moved from `ZC_Scripts`.
	 * @since 2.8.0 Added the `$group` parameter.
	 *
	 ...
	 *                                 to the ZelocoreCMS root directory. If source is set to false,
	 ...
	 *                                 equal to current installed ZelocoreCMS version.
	...
	public function add( $handle, $src, $deps = array(), $ver = false, $args = null ) {
		...
		$this->registered[ $handle ] = new _ZC_Dependency( $handle, $src, $deps, $ver, $args );
	 ...
	 * @since 4.0.0
	 *
	 * @param string[] $queue  An array of queued _ZC_Dependency handles.
	 * @param string   $handle Name of the item. Should be unique.
	 ...
	 * @return bool|_ZC_Dependency Found, or object Item data.
	 */
	...
	public function get_etag( $load ) {
		...
		foreach ( $load as $handle ) {
			...
			$ver   = $this->registered[ $handle ]->ver ?? $zc_version;
			$etag .= "{$handle}:{$ver};";
		}
		 ...
		 * load the full ZelocoreCMS environment, it is not possible to use the salted
		 * zc_hash() function.
		 */
		return 'W/"' . md5( $etag ) . '"';
	}
