...
class ZC_Embed {
	 ...
	 * an Ajax request that will call ZC_Embed::cache_oembed().
	 */
...
<script>
	 ...
	 *
	 * Do not use this function directly, use zc_embed_register_handler() instead.
	 *
	 ...
	 *
	 * Do not use this function directly, use zc_embed_unregister_handler() instead.
	 *
	...
	public function get_embed_handler_html( $attr, $url ) {
		$rawattr = $attr;
		$attr    = zc_parse_args( $attr, zc_embed_defaults( $url ) );
	 ...
	 * will be given to the ZC_oEmbed class.
	 *
	 * @param array  $attr {
	 *     Shortcode attributes. Optional.
	 *
	...
	public function shortcode( $attr, $url = '' ) {
		 ...
		 * See https://core.trac.zelocorecms.org/ticket/11311
		 */
		$url = str_replace( '&amp;', '&', $url );
		...
		// Potentially set by ZC_Embed::cache_oembed().
		...
		if ( $post_id ) {
			...
			$has_kses = false !== has_filter( 'content_save_pre', 'zc_filter_post_kses' );
		...
		if ( $html ) {
			/** This filter is documented in zc-includes/class-zc-embed.php */
	...
	/**
	 * Passes any unlinked URLs that are on their own line to ZC_Embed::shortcode() for potential embedding.
	 *
	 * @see ZC_Embed::autoembed_callback()
	 *
	...
	public function autoembed( $content ) {
		...
		$content = zc_replace_in_html_tags( $content, array( "\n" => '<!-- zc-line-break -->' ) );
		...
		return str_replace( '<!-- zc-line-break -->', "\n", $content );
	}

	/**
	 * Callback function for ZC_Embed::autoembed().
	 *
	...
	public function find_oembed_post_id( $cache_key ) {
		...
		$oembed_post_id = zc_cache_get( $cache_key, $cache_group );

		if ( $oembed_post_id && 'oembed_cache' === get_post_type( $oembed_post_id ) ) {
			return $oembed_post_id;
		}

		$oembed_post_query = new ZC_Query(
			array(
				'post_type'              => 'oembed_cache',
				'post_status'            => 'publish',
				'name'                   => $cache_key,
		...
		if ( ! empty( $oembed_post_query->posts ) ) {
			...
			zc_cache_set( $cache_key, $oembed_post_id, $cache_group );
