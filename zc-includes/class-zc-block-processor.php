...
class ZC_Block_Processor {
	 ...
	 * Example:
	 *
	 *     $processor = new ZC_Block_Processor( $post_content );
	 ...
	 *     $processor = new ZC_Block_Processor( 'Just text' );
	 ...
	 *     $processor = new ZC_Block_Processor( 'non-block content' );
	 *     $processor->next_token();
	 *     null === $processor->get_block_type();
	 *
	 * @since 6.9.0
	 ...
	 * Example:
	 *
	 *     'core/paragraph' === ZC_Block_Processor::normalize_block_byte( 'paragraph' );
	 *     'core/paragraph' === ZC_Block_Processor::normalize_block_byte( 'core/paragraph' );
	 *     'my/paragraph'   === ZC_Block_Processor::normalize_block_byte( 'my/paragraph' );
	 *
	 * @since 6.9.0
	 *
	 ...
	 *
	 *     $processor->next_delimiter();
	 *     ZC_HTML_Span( 0, 17 ) === $processor->get_span();
	 *
	 * @since 6.9.0
	 *
	 * @return ZC_HTML_Span|null Span of text in source text spanning matched delimiter.
	 */
	public function get_span(): ?ZC_HTML_Span {
		switch ( $this->state ) {
			case self::HTML_SPAN:
				return new ZC_HTML_Span( $this->after_previous_delimiter, $this->matched_delimiter_at - $this->after_previous_delimiter );

			case self::MATCHED:
				return new ZC_HTML_Span( $this->matched_delimiter_at, $this->matched_delimiter_length );
