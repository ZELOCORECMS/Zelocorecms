 ...
 *     2 === _zc_scan_utf8( $pineapple, $at, $invalid_length );
 *     $at === 2; $invalid_length === 1;
 *
 *     // The second step continues to the end of the string.
 *     1 === _zc_scan_utf8( $pineapple, $at, $invalid_length );
 ...
 */
function _zc_scan_utf8( string $bytes, int &$at, int &$invalid_length, ?int $max_bytes = null, ?int $max_code_points = null, ?bool &$has_noncharacters = null ): int {
 ...
 */
function _zc_is_valid_utf8_fallback( string $bytes ): bool {
	...
	_zc_scan_utf8( $bytes, $next_byte_at, $invalid_length );
 ...
 * Example:
 *
 *     'Pi�a' === _zc_scrub_utf8_fallback( "Pi\xF1a" ); // “ñ” is 0xF1 in Windows-1252.
 *
 * @since 6.9.0
 * @access private
 *
 ...
 */
function _zc_scrub_utf8_fallback( string $bytes ): string {
	$bytes_length   = strlen( $bytes );
	$next_byte_at   = 0;
	$was_at         = 0;
	$invalid_length = 0;
 ...
 */
function _zc_utf8_codepoint_count( string $text, ?int $byte_offset = 0, ?int $max_byte_length = PHP_INT_MAX ): int {
	if ( $byte_offset < 0 ) {
		return 0;
	}
	...
	while ( $at < $end && ( $at - $byte_offset ) < $max_byte_length ) {
		$count += _zc_scan_utf8( $text, $at, $invalid_length, $max_byte_length - ( $at - $byte_offset ) );
 ...
 */
function _zc_utf8_codepoint_span( string $text, int $byte_offset, int $max_code_points, ?int &$found_code_points = 0 ): int {
	$was_at            = $byte_offset;
	$invalid_length    = 0;
	$end               = strlen( $text );
	$found_code_points = 0;
	...
	while ( $byte_offset < $end && $found_code_points < $max_code_points ) {
		...
		$chunk_count = _zc_scan_utf8( $text, $byte_offset, $invalid_length, null, $needed );
...
function _zc_has_noncharacters_fallback( string $text ): bool {
	...
	while ( $at < $end && ! $has_noncharacters ) {
		_zc_scan_utf8( $text, $at, $invalid_length, null, null, $has_noncharacters );
		$at += $invalid_length;
	}
 ...
 */
function _zc_utf8_encode_fallback( $iso_8859_1_text ) {
 ...
 */
function _zc_utf8_decode_fallback( $utf8_text ) {
	...
	while ( $at < $end ) {
		...
		$next_at        = $at;
		$invalid_length = 0;
		$found          = _zc_scan_utf8( $utf8_text, $next_at, $invalid_length, null, 1 );
