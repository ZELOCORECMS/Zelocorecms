 ...
 *                                   See ZC_Block_Patterns_Registry::register() for accepted arguments.
 * @return bool True if the pattern was registered with success and false otherwise.
 */
function register_block_pattern( $pattern_name, $pattern_properties ) {
	return ZC_Block_Patterns_Registry::get_instance()->register( $pattern_name, $pattern_properties );
}
...
function unregister_block_pattern( $pattern_name ) {
	return ZC_Block_Patterns_Registry::get_instance()->unregister( $pattern_name );
}
 ...
 *                                    See ZC_Block_Pattern_Categories_Registry::register() for
 *                                    accepted arguments.
 * @return bool True if the pattern category was registered with success and false otherwise.
 */
function register_block_pattern_category( $category_name, $category_properties ) {
	return ZC_Block_Pattern_Categories_Registry::get_instance()->register( $category_name, $category_properties );
}
...
function unregister_block_pattern_category( $category_name ) {
	return ZC_Block_Pattern_Categories_Registry::get_instance()->unregister( $category_name );
}
 ...
 */
function zc_normalize_remote_block_pattern( $pattern ) {
...
/**
 * Register Core's official patterns from zelocorecms.org/patterns.
 *
 * @since 5.8.0
 ...
 *
 * @param ZC_Screen $deprecated Unused. Formerly the screen that the current request was triggered from.
 */
function _load_remote_block_patterns( $deprecated = null ) {
	...
	if ( $supports_core_patterns && $should_load_remote ) {
		...
		foreach ( $patterns as $pattern ) {
			...
			$normalized_pattern = zc_normalize_remote_block_pattern( $pattern );
...
/**
 * Register `Featured` (category) patterns from zelocorecms.org/patterns.
 *
 * @since 5.9.0
...
function _load_remote_featured_patterns() {
	...
	/** This filter is documented in zc-includes/block-patterns.php */
	$should_load_remote = apply_filters( 'should_load_remote_block_patterns', true );
...
function _register_remote_theme_patterns() {
	...
	$pattern_settings = zc_get_theme_directory_pattern_slugs();
	...
	$patterns          = $response->get_data();
	$patterns_registry = ZC_Block_Patterns_Registry::get_instance();
	foreach ( $patterns as $pattern ) {
 ...
 * @since 6.4.0 Uses the `ZC_Theme::get_block_patterns` method.
 * @access private
 */
function _register_theme_block_patterns() {
	 ...
	 */
	if ( empty( zc_get_active_and_valid_themes() ) ) {
		return;
	}
