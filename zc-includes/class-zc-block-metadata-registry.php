 ...
 *
 * The ZC_Block_Metadata_Registry allows plugins to register metadata for large
 ...
 * @since 6.7.0
 */
class ZC_Block_Metadata_Registry {
	...
	public static function register_collection( $path, $manifest ) {
		$path = rtrim( zc_normalize_path( $path ), '/' );
		 ...
		 *
		 * Example:
		 * * It is allowed to register a collection with path `ZC_PLUGIN_DIR . '/my-plugin'`.
		 * * It is not allowed to register a collection with path `ZC_PLUGIN_DIR`.
		 * * It is not allowed to register a collection with path `dirname( ZC_PLUGIN_DIR )`.
		 *
		 * The default list encompasses the `zc-includes` directory, as well as the root directories for plugins,
		 ...
		 */
		$collection_roots = apply_filters( 'zc_allowed_block_metadata_collection_roots', $collection_roots );

		$collection_roots = array_unique(
			array_map(
				static function ( $allowed_root ) {
					return rtrim( zc_normalize_path( $allowed_root ), '/' );
				},
				$collection_roots
			)
		);
		...
		if ( ! self::is_valid_collection_path( $path, $collection_roots ) ) {
			_doing_it_wrong(
				...
				sprintf(
					...
					esc_html( implode( zc_get_list_item_separator(), $collection_roots ) )
				),
				'6.7.2'
			);
			return false;
	 ...
	 *
	 * For instance, if a block metadata collection is registered with path `ZC_PLUGIN_DIR . '/my-plugin/blocks/'`,
	 ...
	 * * `ZC_PLUGIN_DIR . '/my-plugin/blocks/block-a/block.json'`
	 * * `ZC_PLUGIN_DIR . '/my-plugin/blocks/block-b/block.json'`
	 *
	 * @since 6.8.0
	 *
	 ...
	 *
	 * For example:
	 * - Path: '/zc-content/plugins/my-plugin/blocks/example/block.json'
	 *   Identifier: 'example'
	 * - Path: '/zc-content/plugins/my-plugin/blocks/another-block'
	 *   Identifier: 'another-block'
	 *
	 * This default behavior matches the standard ZelocoreCMS block structure.
	 *
	 * @since 6.7.0
	 *
	...
	private static function get_default_collection_roots() {
		...
		foreach ( $theme_roots as $theme_root ) {
			$collection_roots[] = trailingslashit( zc_normalize_path( ZC_CONTENT_DIR ) ) . ltrim( zc_normalize_path( $theme_root ), '/' );
		}
