<?php
/**
 * Comment API: ZC_Comment_Query class
 *
 * @package ZelocoreCMS
 * @subpackage Comments
 * @since 4.4.0
 ...
 * @since 3.1.0
 *
 * @see ZC_Comment_Query::__construct() for accepted arguments.
 */
#[AllowDynamicProperties]
class ZC_Comment_Query {
	 ...
	 *                                                        stored in a `children` property on the `ZC_Comment` object.
	...
	public function __construct( $query = '' ) {
		$this->query_var_defaults = array(
			...
			'meta_value'                => '',
			'meta_query'                => '',
			'date_query'                => null, // See ZC_Date_Query.
			'hierarchical'              => false,
			'cache_domain'              => 'core',
	 ...
	 *
	 * @since 4.2.0 Extracted from ZC_Comment_Query::query().
	 *
	 * @param string|array $query ZC_Comment_Query arguments. See ZC_Comment_Query::__construct() for accepted arguments.
	 */
	public function parse_query( $query = '' ) {
		...
		$this->query_vars = zc_parse_args( $query, $this->query_var_defaults );
	...
	/**
	 * Sets up the ZelocoreCMS query for retrieving comments.
	 *
	 * @since 3.1.0
	...
	public function get_comments() {
		 ...
		 * @since 3.1.0
		 *
		 * @param ZC_Comment_Query $query Current instance of ZC_Comment_Query (passed by reference).
		 */
		 ...
		 *
		 * Return a non-null value to bypass ZelocoreCMS' default comment queries.
		 *
		 ...
		 * - Otherwise the filter should return an array of ZC_Comment objects.
		 *
		 * Note that if the filter returns an array of comment data, it will be assigned
		 * to the `comments` property of the current ZC_Comment_Query instance.
		 *
		 * Filtering functions that require pagination information are encouraged to set
		 * the `found_comments` and `max_num_pages` properties of the ZC_Comment_Query object,
		 * passed to the filter by reference. If ZC_Comment_Query does not perform a database
		 ...
		 */
		$_args = zc_array_slice_assoc( $this->query_vars, array_keys( $this->query_var_defaults ) );
		unset( $_args['fields'], $_args['update_comment_meta_cache'], $_args['update_comment_post_cache'] );

		$key          = md5( serialize( $_args ) );
		$last_changed = zc_cache_get_last_changed( 'comment' );

		$cache_key   = "get_comments:$key";
		$cache_value = zc_cache_get_salted( $cache_key, 'comment-queries', $last_changed );
		if ( false === $cache_value ) {
			...
			zc_cache_set_salted( $cache_key, $cache_value, 'comment-queries', $last_changed );
		} else {
		 ...
		 * @since 3.1.0
		 *
		 * @param ZC_Comment[]     $_comments An array of comments.
		 * @param ZC_Comment_Query $query     Current instance of ZC_Comment_Query (passed by reference).
		 */
		$_comments = apply_filters_ref_array( 'the_comments', array( $_comments, &$this ) );

		// Convert to ZC_Comment instances.
	...
	protected function get_comment_ids() {
		...
		$statuses       = zc_parse_list( $this->query_vars['status'] );
		...
		if ( ! empty( $this->query_vars['include_unapproved'] ) ) {
			$include_unapproved = zc_parse_list( $this->query_vars['include_unapproved'] );
		...
		if ( ! empty( $this->query_vars['parent__in'] ) ) {
			$this->sql_clauses['where']['parent__in'] = 'comment_parent IN ( ' . implode( ',', zc_parse_id_list( $this->query_vars['parent__in'] ) ) . ' )';
		}

		// Parse comment parent IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['parent__not_in'] ) ) {
			$this->sql_clauses['where']['parent__not_in'] = 'comment_parent NOT IN ( ' . implode( ',', zc_parse_id_list( $this->query_vars['parent__not_in'] ) ) . ' )';
		}

		// Parse comment post IDs for an IN clause.
		if ( ! empty( $this->query_vars['post__in'] ) ) {
			$this->sql_clauses['where']['post__in'] = 'comment_post_ID IN ( ' . implode( ',', zc_parse_id_list( $this->query_vars['post__in'] ) ) . ' )';
		}

		// Parse comment post IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['post__not_in'] ) ) {
			$this->sql_clauses['where']['post__not_in'] = 'comment_post_ID NOT IN ( ' . implode( ',', zc_parse_id_list( $this->query_vars['post__not_in'] ) ) . ' )';
		}
		...
		$plucked          = zc_array_slice_assoc( $this->query_vars, array( 'post_author', 'post_name', 'post_parent' ) );
		...
		if ( ! empty( $this->query_vars['author__in'] ) ) {
			$this->sql_clauses['where']['author__in'] = 'user_id IN ( ' . implode( ',', zc_parse_id_list( $this->query_vars['author__in'] ) ) . ' )';
		}

		// Comment author IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['author__not_in'] ) ) {
			$this->sql_clauses['where']['author__not_in'] = 'user_id NOT IN ( ' . implode( ',', zc_parse_id_list( $this->query_vars['author__not_in'] ) ) . ' )';
		}

		// Post author IDs for an IN clause.
		if ( ! empty( $this->query_vars['post_author__in'] ) ) {
			$join_posts_table                              = true;
			$this->sql_clauses['where']['post_author__in'] = 'post_author IN ( ' . implode( ',', zc_parse_id_list( $this->query_vars['post_author__in'] ) ) . ' )';
		}

		// Post author IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['post_author__not_in'] ) ) {
			$join_posts_table                                  = true;
			$this->sql_clauses['where']['post_author__not_in'] = 'post_author NOT IN ( ' . implode( ',', zc_parse_id_list( $this->query_vars['post_author__not_in'] ) ) . ' )';
		}
		...
		if ( ! empty( $this->query_vars['date_query'] ) && is_array( $this->query_vars['date_query'] ) ) {
			$this->date_query = new ZC_Date_Query( $this->query_vars['date_query'], 'comment_date' );
	...
	private function set_found_comments() {
		...
		if ( $this->query_vars['number'] && ! $this->query_vars['no_found_rows'] ) {
			 ...
			 * @param ZC_Comment_Query $comment_query        The `ZC_Comment_Query` instance.
			 */
	 ...
	 * @since 4.4.0
	 *
	 * @param ZC_Comment[] $comments Array of top-level comments whose descendants should be filled in.
	 * @return array
	 */
	protected function fill_descendants( $comments ) {
		$levels = array(
			0 => zc_list_pluck( $comments, 'comment_ID' ),
		);

		$key          = md5( serialize( zc_array_slice_assoc( $this->query_vars, array_keys( $this->query_var_defaults ) ) ) );
		$last_changed = zc_cache_get_last_changed( 'comment' );
		...
		do {
			...
			if ( $_parent_ids ) {
				...
				$cache_data = zc_cache_get_multiple_salted( array_values( $cache_keys ), 'comment-queries', $last_changed );
				foreach ( $_parent_ids as $parent_id ) {
			...
			if ( $uncached_parent_ids ) {
				...
				zc_cache_set_multiple_salted( $data, 'comment-queries', $last_changed );
			}
