...
class ZC_Roles {
	 ...
	 *
	 * @global array $zc_user_roles Used to set the 'roles' property value.
	 *
	 * @param int $site_id Site ID to initialize roles for. Default is the current site.
	 */
	public function __construct( $site_id = null ) {
		global $zc_user_roles;

		$this->use_db = empty( $zc_user_roles );
	 ...
	 *
	 * @since 2.1.0
	 * @deprecated 4.9.0 Use ZC_Roles::for_site()
	 */
	protected function _init() {
		_deprecated_function( __METHOD__, '4.9.0', 'ZC_Roles::for_site()' );
	...
	public function reinit() {
		_deprecated_function( __METHOD__, '4.7.0', 'ZC_Roles::for_site()' );
	 ...
	 *     zc_roles()->add_role( 'custom_role', 'Custom Role', array(
	 *         'read',
	 *         'edit_posts',
	 *     ) );
	 *
	...
	public function add_role( $role, $display_name, $capabilities = array() ) {
		...
		if ( zc_is_numeric_array( $capabilities ) ) {
		...
		$this->role_objects[ $role ] = new ZC_Role( $role, $capabilities );
	...
	public function init_roles() {
		...
		foreach ( array_keys( $this->roles ) as $role ) {
			$this->role_objects[ $role ] = new ZC_Role( $role, $this->roles[ $role ]['capabilities'] );
		 ...
		 * @since 4.7.0
		 *
		 * @param ZC_Roles $zc_roles A reference to the ZC_Roles object.
		 */
		do_action( 'zc_roles_init', $this );
	}
	...
	protected function get_roles_data() {
		...
		if ( ! empty( $zc_user_roles ) ) {
			return $zc_user_roles;
		}
