 ...
 *                                    loading is disabled.
 */
function zc_get_speculation_rules_configuration(): ?array {
	 ...
	 *
	 * By default, the speculation rules configuration is decided by ZelocoreCMS Core ("auto"). This filter can be used
	 ...
	 */
	$config = apply_filters( 'zc_speculation_rules_configuration', $config );
	...
	if (
		! isset( $config['mode'] ) ||
		'auto' === $config['mode'] ||
		! ZC_Speculation_Rules::is_valid_mode( $config['mode'] )
	) {
	...
	if (
		...
		! ZC_Speculation_Rules::is_valid_eagerness( $config['eagerness'] ) ||
		// 'immediate' is a valid eagerness, but for safety ZelocoreCMS does not allow it for document-level rules.
 ...
 * {@see 'zc_speculation_rules_href_exclude_paths'} filter to ensure those URL patterns are excluded.
 *
 * Additional speculation rules other than the default rule from ZelocoreCMS Core can be provided by using the
 * {@see 'zc_load_speculation_rules'} action and amending the passed ZC_Speculation_Rules object.
 *
 * @since 6.8.0
 * @access private
 *
 * @return ZC_Speculation_Rules|null Object representing the speculation rules to use, or null if speculative loading
 *                                   is disabled in the current context.
 */
function zc_get_speculation_rules(): ?ZC_Speculation_Rules {
	$configuration = zc_get_speculation_rules_configuration();
	...
	$prefixer = new ZC_URL_Pattern_Prefixer();

	$base_href_exclude_paths = array(
		$prefixer->prefix_path_pattern( '/zc-*.php', 'site' ),
		$prefixer->prefix_path_pattern( '/zc-admin/*', 'site' ),
	 ...
	 * If the ZelocoreCMS site is in a subdirectory, the exclude paths will automatically be prefixed as necessary.
	 *
	 * Note that ZelocoreCMS always excludes certain path patterns such as `/zc-login.php` and `/zc-admin/*`, and those
	 ...
	 */
	$href_exclude_paths = (array) apply_filters( 'zc_speculation_rules_href_exclude_paths', array(), $mode );
	...
	$speculation_rules = new ZC_Speculation_Rules();
	 ...
	 * @since 6.8.0
	 *
	 * @param ZC_Speculation_Rules $speculation_rules Object representing the speculation rules to use.
	 */
	do_action( 'zc_load_speculation_rules', $speculation_rules );
 ...
 * @access private
 */
function zc_print_speculation_rules(): void {
	$speculation_rules = zc_get_speculation_rules();
