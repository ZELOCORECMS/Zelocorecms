<?php
/**
 * ZC_Application_Passwords class
 *
 * @package ZelocoreCMS
 * @since   5.6.0
 */
...
class ZC_Application_Passwords {
	 ...
	 *
	 * @since 5.6.0
	 * @since 5.7.0 Returns ZC_Error if application name already exists.
	 * @since 6.8.0 The hashed password value now uses zc_fast_hash() instead of phpass.
	 *
	 ...
	 *     Application password details, or a ZC_Error instance if an error occurs.
	 *
	...
	public static function create_new_application_password( $user_id, $args = array() ) {
		...
		if ( empty( $args['name'] ) ) {
			return new ZC_Error( 'application_password_empty_name', __( 'An application name is required to create an application password.' ), array( 'status' => 400 ) );
		}

		$new_password    = zc_generate_password( static::PW_LENGTH, false );
		$hashed_password = self::hash_password( $new_password );

		$new_item = array(
			'uuid'      => zc_generate_uuid4(),
		...
		if ( ! $saved ) {
			return new ZC_Error( 'db_error', __( 'Could not save application password.' ) );
		}
		 ...
		 * }
		 */
		do_action( 'zc_create_application_password', $user_id, $new_item, $new_password, $args );
	...
	public static function get_user_application_passwords( $user_id ) {
		...
		foreach ( $passwords as $i => $password ) {
			if ( ! isset( $password['uuid'] ) ) {
				$passwords[ $i ]['uuid'] = zc_generate_uuid4();
				$save                    = true;
			}
		}
	 ...
	 *
	 * @since 5.6.0
	 * @since 6.8.0 The actual password should now be hashed using zc_fast_hash().
	 *
	 * @param int    $user_id User ID.
	 * @param string $uuid    The password's UUID.
	 * @param array  $update  {
	 ...
	 *     @type string|null $last_ip   The IP address the application password was last used by.
	 * }
	 * @return true|ZC_Error True if successful, otherwise a ZC_Error instance is returned on error.
	 */
	public static function update_application_password( $user_id, $uuid, $update = array() ) {
		...
		foreach ( $passwords as &$item ) {
			 ...
			 *
			 * @since 5.6.0
			 * @since 6.8.0 The password is now hashed using zc_fast_hash() instead of phpass.
			 ...
			 */
			do_action( 'zc_update_application_password', $user_id, $item, $update );

			return true;
		}

		return new ZC_Error( 'application_password_not_found', __( 'Could not find an application password with that id.' ) );
	}
	 ...
	 * @param int    $user_id User ID.
	 * @param string $uuid    The password's UUID.
	 * @return true|ZC_Error True if the usage was recorded, a ZC_Error if an error occurs.
	 */
	 ...
	 * @param int    $user_id User ID.
	 * @param string $uuid    The password's UUID.
	 * @return true|ZC_Error Whether the password was successfully found and deleted, a ZC_Error otherwise.
	 */
	public static function delete_application_password( $user_id, $uuid ) {
		...
		foreach ( $passwords as $key => $item ) {
			if ( $item['uuid'] === $uuid ) {
				...
				if ( ! $saved ) {
					return new ZC_Error( 'db_error', __( 'Could not delete application password.' ) );
				}
				 ...
				 */
				do_action( 'zc_delete_application_password', $user_id, $item );
	 ...
	 *
	 * @param int $user_id User ID.
	 * @return int|ZC_Error The number of passwords that were deleted or a ZC_Error on failure.
	 */
	public static function delete_all_application_passwords( $user_id ) {
		...
		if ( $passwords ) {
			...
			if ( ! $saved ) {
				return new ZC_Error( 'db_error', __( 'Could not delete application passwords.' ) );
			}

			foreach ( $passwords as $item ) {
				/** This action is documented in zc-includes/class-zc-application-passwords.php */
				do_action( 'zc_delete_application_password', $user_id, $item );
			}
	...
	public static function check_password(
		...
		if ( ! str_starts_with( $hash, '$generic$' ) ) {
			/*
			 * If the hash doesn't start with `$generic$`, it is a hash created with `zc_hash_password()`.
			 * This is the case for application passwords created before 6.8.0.
			 */
			return zc_check_password( $password, $hash );
		}

		return zc_verify_fast_hash( $password, $hash );
	}
}
