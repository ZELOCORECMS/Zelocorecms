...
class ZC_Theme_JSON_Resolver {
	...
	protected static function read_json_file( $file_path ) {
		if ( $file_path ) {
			...
			$decoded_file = zc_json_file_decode( $file_path, array( 'associative' => true ) );
	...
	protected static function translate( $theme_json, $domain = 'default' ) {
		if ( null === static::$i18n_schema ) {
			$i18n_schema         = zc_json_file_decode( __DIR__ . '/theme-i18n.json' );
	 ...
	 * @since 5.8.0
	 *
	 * @return ZC_Theme_JSON Entity that holds core data.
	 */
	public static function get_core_data() {
		...
		/**
		 * Filters the default data provided by ZelocoreCMS for global styles & settings.
		 *
		 * @since 6.1.0
		 *
		 * @param ZC_Theme_JSON_Data $theme_json Class to access and update the underlying data.
		 */
		$theme_json = apply_filters( 'zc_theme_json_data_default', new ZC_Theme_JSON_Data( $config, 'default' ) );

		/*
		 * Backward compatibility for extenders returning a ZC_Theme_JSON_Data
		 * compatible class that is not a ZC_Theme_JSON_Data object.
		 */
		if ( $theme_json instanceof ZC_Theme_JSON_Data ) {
			static::$core = $theme_json->get_theme_json();
		} else {
			$config       = $theme_json->get_data();
			static::$core = new ZC_Theme_JSON( $config, 'default' );
		}
	 ...
	 * }
	 * @return ZC_Theme_JSON Entity that holds theme data.
	 */
	public static function get_theme_data( $deprecated = array(), $options = array() ) {
		...
		$options = zc_parse_args( $options, array( 'with_supports' => true ) );

		if ( null === static::$theme || ! static::has_same_registered_blocks( 'theme' ) ) {
			$zc_theme        = zc_get_theme();
			$theme_json_file = $zc_theme->get_file_path( 'theme.json' );
			if ( is_readable( $theme_json_file ) ) {
				$theme_json_data = static::read_json_file( $theme_json_file );
				$theme_json_data = static::translate( $theme_json_data, $zc_theme->get( 'TextDomain' ) );
			} else {
				$theme_json_data = array( 'version' => ZC_Theme_JSON::LATEST_SCHEMA );
			}
			...
			zc_register_block_style_variations_from_theme_json_partials( $variations );
			 ...
			 * @param ZC_Theme_JSON_Data $theme_json Class to access and update the underlying data.
			 */
			$theme_json = apply_filters( 'zc_theme_json_data_theme', new ZC_Theme_JSON_Data( $theme_json_data, 'theme' ) );
			...
			if ( $theme_json instanceof ZC_Theme_JSON_Data ) {
				...
				$config        = $theme_json->get_data();
				static::$theme = new ZC_Theme_JSON( $config );
			}

			if ( $zc_theme->parent() ) {
				// Get parent theme.json.
				$parent_theme_json_file = $zc_theme->parent()->get_file_path( 'theme.json' );
				if ( $theme_json_file !== $parent_theme_json_file && is_readable( $parent_theme_json_file ) ) {
					$parent_theme_json_data = static::read_json_file( $parent_theme_json_file );
					$parent_theme_json_data = static::translate( $parent_theme_json_data, $zc_theme->parent()->get( 'TextDomain' ) );
					$parent_theme           = new ZC_Theme_JSON( $parent_theme_json_data );
		 ...
		 */
		$theme_support_data = ZC_Theme_JSON::get_from_editor_settings( get_classic_theme_supports_block_editor_settings() );
		if ( ! zc_theme_has_theme_json() ) {
			 ...
			 * themes. See https://github.com/ZelocoreCMS/gutenberg/issues/59989.
			 */
		...
		$with_theme_supports = new ZC_Theme_JSON( $theme_support_data );
	...
	public static function get_block_data() {
		...
		$config = array( 'version' => ZC_Theme_JSON::LATEST_SCHEMA );
		foreach ( $blocks as $block_name => $block_type ) {
		 ...
		 * @param ZC_Theme_JSON_Data $theme_json Class to access and update the underlying data.
		 */
		$theme_json = apply_filters( 'zc_theme_json_data_blocks', new ZC_Theme_JSON_Data( $config, 'blocks' ) );
		...
		if ( $theme_json instanceof ZC_Theme_JSON_Data ) {
			...
			$config         = $theme_json->get_data();
			static::$blocks = new ZC_Theme_JSON( $config, 'blocks' );
		}
	 ...
	 * @since 5.9.0
	 *
	 * @param ZC_Theme $theme              The theme object. If empty, it
	 ...
	 */
	public static function get_user_data_from_zc_global_styles( $theme, $create_post = false, $post_status_filter = array( 'publish' ) ) {
		if ( ! $theme instanceof ZC_Theme ) {
			$theme = zc_get_theme();
		}

		$user_cpt         = array();
		$post_type_filter = 'zc_global_styles';
		...
		$global_style_query = new ZC_Query();
		$recent_posts       = $global_style_query->query( $args );
		if ( count( $recent_posts ) === 1 && $recent_posts[0] instanceof ZC_Post ) {
			$user_cpt = get_object_vars( $recent_posts[0] );
		} elseif ( $create_post ) {
			$cpt_post_id = zc_insert_post(
				array(
					'post_content' => '{"version": ' . ZC_Theme_JSON::LATEST_SCHEMA . ', "isGlobalStylesUserThemeJSON": true }',
					'post_status'  => 'publish',
					'post_title'   => 'Custom Styles', // Do not make string translatable, see https://core.trac.zelocorecms.org/ticket/54518.
					'post_type'    => $post_type_filter,
					'post_name'    => sprintf( 'zc-global-styles-%s', urlencode( $stylesheet ) ),
					'tax_input'    => array(
						'zc_theme' => array( $stylesheet ),
					),
				),
				true
			);
			if ( ! is_zc_error( $cpt_post_id ) ) {
	 ...
	 *
	 * @return ZC_Theme_JSON Entity that holds styles for user data.
	 */
	public static function get_user_data() {
		...
		$config   = array();
		$user_cpt = static::get_user_data_from_zc_global_styles( zc_get_theme() );
		...
		if ( array_key_exists( 'post_content', $user_cpt ) ) {
			...
			if ( JSON_ERROR_NONE !== $json_decoding_error ) {
				zc_trigger_error( __METHOD__, 'Error when decoding a theme.json schema for user data. ' . json_last_error_msg() );
				/**
				 ...
				 * @param ZC_Theme_JSON_Data $theme_json Class to access and update the underlying data.
				 */
				$theme_json = apply_filters( 'zc_theme_json_data_user', new ZC_Theme_JSON_Data( $config, 'custom' ) );
				...
				if ( $theme_json instanceof ZC_Theme_JSON_Data ) {
					...
					$config = $theme_json->get_data();
					return new ZC_Theme_JSON( $config, 'custom' );
				}
			}
		...
		/** This filter is documented in zc-includes/class-zc-theme-json-resolver.php */
		$theme_json = apply_filters( 'zc_theme_json_data_user', new ZC_Theme_JSON_Data( $config, 'custom' ) );
		...
		if ( $theme_json instanceof ZC_Theme_JSON_Data ) {
			...
			$config       = $theme_json->get_data();
			static::$user = new ZC_Theme_JSON( $config, 'custom' );
		}
	 ...
	 *
	 * Unlike the getters
	 * {@link https://developer.zelocorecms.org/reference/classes/zc_theme_json_resolver/get_core_data/ get_core_data},
	 * {@link https://developer.zelocorecms.org/reference/classes/zc_theme_json_resolver/get_theme_data/ get_theme_data},
	 * and {@link https://developer.zelocorecms.org/reference/classes/zc_theme_json_resolver/get_user_data/ get_user_data},
	 ...
	 * @since 5.9.0
	 *
	 * @return int|null ID for a post of type `zc_global_styles`, or null if not available.
	 */
	public static function get_user_global_styles_post_id() {
		...
		$user_cpt = static::get_user_data_from_zc_global_styles( zc_get_theme(), true );
	 ...
	 * @deprecated 6.2.0 Use zc_theme_has_theme_json() instead.
	 *
	 * @return bool Whether the active theme has a theme.json file.
	 */
	public static function theme_has_support() {
		_deprecated_function( __METHOD__, '6.2.0', 'zc_theme_has_theme_json()' );

		return zc_theme_has_theme_json();
	}
	...
	public static function get_style_variations( $scope = 'theme' ) {
		...
		foreach ( $variation_files as $path => $file ) {
			...
			if ( is_array( $decoded_file ) && static::style_variation_has_scope( $decoded_file, $scope ) ) {
				$translated = static::translate( $decoded_file, zc_get_theme()->get( 'TextDomain' ) );
				$variation  = ( new ZC_Theme_JSON( $translated ) )->get_raw_data();
	 ...
	 *
	 * @param ZC_Theme_JSON $theme_json A theme json instance.
	 * @return array An array of resolved paths.
	 */
	public static function get_resolved_theme_uris( $theme_json ) {
		$resolved_theme_uris = array();

		if ( ! $theme_json instanceof ZC_Theme_JSON ) {
		 ...
		 * See: ZC_Font_Face_Resolver::to_theme_file_uri.
		 */
		$placeholder = 'file:./';
		...
		if (
			...
			$file_type          = zc_check_filetype( $background_image_url );
	 ...
	 *
	 * @param ZC_Theme_JSON $theme_json A theme json instance.
	 * @return ZC_Theme_JSON Theme merged with resolved paths, if any found.
	 */
	public static function resolve_theme_file_uris( $theme_json ) {
		...
		foreach ( $resolved_urls as $resolved_url ) {
			...
			_zc_array_set( $resolved_theme_json_data, $path, $resolved_url['href'] );
		}

		return new ZC_Theme_JSON( $resolved_theme_json_data );
	}
	...
	private static function inject_variations_from_block_style_variation_files( $data, $variations ) {
		...
		foreach ( $variations as $variation ) {
			...
			$variation_name = $variation['slug'] ?? _zc_to_kebab_case( $variation['title'] );
			...
			foreach ( $variation['blockTypes'] as $block_type ) {
				...
				_zc_array_set( $data, $path, $variation['styles'] );
			}
		}
	...
	private static function inject_variations_from_block_styles_registry( $data ) {
		$registry = ZC_Block_Styles_Registry::get_instance();
		...
		foreach ( $styles as $block_type => $variations ) {
			foreach ( $variations as $variation_name => $variation ) {
				...
				$path = array( 'styles', 'blocks', $block_type, 'variations', $variation_name );
				_zc_array_set( $data, $path, $variation['style_data'] );
			}
		}
