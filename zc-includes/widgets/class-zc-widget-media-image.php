<?php
/**
 * Widget API: ZC_Widget_Media_Image class
 *
 * @package ZelocoreCMS
 * @subpackage Widgets
 * @since 4.8.0
 ...
 * @see ZC_Widget
 */
class ZC_Widget_Media_Image extends ZC_Widget_Media {
	...
	public function render_media( $instance ) {
		...
		if ( $attachment ) {
			...
			$image_attributes = array(
				'class' => sprintf( 'image zc-image-%d %s', $attachment->ID, $instance['image_classes'] ),
			...
			if ( 'custom' === $size || ! in_array( $size, array_merge( get_intermediate_image_sizes(), array( 'full' ) ), true ) ) {
				...
				$caption_size = _zc_get_image_size_from_meta( $instance['size'], zc_get_attachment_metadata( $attachment->ID ) );
			...
			$image = zc_get_attachment_image( $attachment->ID, $size, false, $image_attributes );
		...
		if ( 'file' === $instance['link_type'] ) {
			$url = $attachment ? zc_get_attachment_url( $attachment->ID ) : $instance['url'];
	...
	public function render_control_template_scripts() {
		...
		?>
		<script type="text/html" id="tmpl-zc-media-widget-image-fields">
		...
		<script type="text/html" id="tmpl-zc-media-widget-image-preview">
