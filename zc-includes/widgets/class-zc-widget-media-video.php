<?php
/**
 * Widget API: ZC_Widget_Media_Video class
 *
 * @package ZelocoreCMS
 * @subpackage Widgets
 * @since 4.8.0
 ...
 * @see ZC_Widget
 */
class ZC_Widget_Media_Video extends ZC_Widget_Media {
	...
	public function __construct() {
		...
		$this->l10n = array_merge(
			...
			array(
				...
				'unsupported_file_type'      => sprintf( __( 'Sorry, the video at the supplied URL cannot be loaded. Please check that the URL is for a supported video file (%s) or stream (e.g. YouTube and Vimeo).' ), '<code>.' . implode( '</code>, <code>.', zc_get_video_extensions() ) . '</code>' ),
			)
		);
	}
	 ...
	 * @since 4.8.0
	 *
	 * @see ZC_REST_Controller::get_item_schema()
	 * @see ZC_REST_Controller::get_additional_fields()
	 * @link https://core.trac.zelocorecms.org/ticket/35574
	 *
	...
	public function get_instance_schema() {
		...
		$schema = array(
			...
			'content' => array(
				'type'                  => 'string',
				'default'               => '',
				'sanitize_callback'     => 'zc_kses_post',
				'description'           => __( 'Tracks (subtitles, captions, descriptions, chapters, or metadata)' ),
				'should_preview_update' => false,
			),
		);

		foreach ( zc_get_video_extensions() as $video_extension ) {
	...
	public function render_media( $instance ) {
		$instance   = array_merge( zc_list_pluck( $this->get_instance_schema(), 'default' ), $instance );
		$attachment = null;
		...
		$src = $instance['url'];
		if ( $attachment ) {
			$src = zc_get_attachment_url( $attachment->ID );
		}
		...
		if ( $attachment || preg_match( $youtube_pattern, $src ) || preg_match( $vimeo_pattern, $src ) ) {
			add_filter( 'zc_video_shortcode', array( $this, 'inject_video_max_width_style' ) );

			echo zc_video_shortcode(
				array_merge(
					$instance,
					compact( 'src' )
				),
			...
			remove_filter( 'zc_video_shortcode', array( $this, 'inject_video_max_width_style' ) );
		} else {
			echo $this->inject_video_max_width_style( zc_oembed_get( $src ) );
		}
	}
	...
	public function enqueue_preview_scripts() {
		...
		if ( 'mediaelement' === apply_filters( 'zc_video_shortcode_library', 'mediaelement' ) ) {
	...
	public function enqueue_admin_scripts() {
		...
		zc_enqueue_script( $handle );

		$exported_schema = array();
		foreach ( $this->get_instance_schema() as $field => $field_schema ) {
			$exported_schema[ $field ] = zc_array_slice_assoc( $field_schema, array( 'type', 'default', 'enum', 'minimum', 'format', 'media_prop', 'should_preview_update' ) );
		}
		...
		zc_add_inline_script(
			...
			sprintf(
				'
					...
					zc.mediaWidgets.controlConstructors[ %1$s ].prototype.l10n = _.extend( {}, zc.mediaWidgets.controlConstructors[ %1$s ].prototype.l10n, %3$s );
				',
				zc_json_encode( $this->id_base, JSON_HEX_TAG | JSON_UNESCAPED_SLASHES ),
				zc_json_encode( $this->widget_options['mime_type'], JSON_HEX_TAG | JSON_UNESCAPED_SLASHES ),
				zc_json_encode( $this->l10n, JSON_HEX_TAG | JSON_UNESCAPED_SLASHES )
			)
		);
	}
	...
	public function render_control_template_scripts() {
		...
		?>
		<script type="text/html" id="tmpl-zc-media-widget-video-preview">
