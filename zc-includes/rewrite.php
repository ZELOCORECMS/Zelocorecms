...
function add_rewrite_rule( $regex, $query, $after = 'bottom' ) {
	...
	$zc_rewrite->add_rule( $regex, $query, $after );
}
...
function add_rewrite_tag( $tag, $regex, $query = '' ) {
	...
	$zc_rewrite->add_rewrite_tag( $tag, $regex, $query );
}
...
function remove_rewrite_tag( $tag ) {
	global $zc_rewrite;
	$zc_rewrite->remove_rewrite_tag( $tag );
}
...
function add_permastruct( $name, $struct, $args = array() ) {
	...
	$zc_rewrite->add_permastruct( $name, $struct, $args );
}
 ...
 * @since 4.5.0
 *
 * @see ZC_Rewrite::remove_permastruct()
 * @global ZC_Rewrite $zc_rewrite ZelocoreCMS rewrite component.
 *
 * @param string $name Name for permalink structure.
 */
function remove_permastruct( $name ) {
	...
	$zc_rewrite->remove_permastruct( $name );
}
...
function add_feed( $feedname, $callback ) {
	...
	if ( ! in_array( $feedname, $zc_rewrite->feeds, true ) ) {
		$zc_rewrite->feeds[] = $feedname;
	}
...
function flush_rewrite_rules( $hard = true ) {
	...
	if ( is_callable( array( $zc_rewrite, 'flush_rules' ) ) ) {
		$zc_rewrite->flush_rules( $hard );
	}
}
...
function add_rewrite_endpoint( $name, $places, $query_var = true ) {
	global $zc_rewrite;
	$zc_rewrite->add_endpoint( $name, $places, $query_var );
}
 ...
 * @return string
 */
function _zc_filter_taxonomy_base( $base ) {
	if ( ! empty( $base ) ) {
 ...
 *
 * Permalinks of posts with numeric slugs can sometimes look to ZC_Query::parse_query()
 ...
 * post permalink.
 *
 * Note that, since 4.3.0, zc_unique_post_slug() prevents the creation of post slugs
...
function url_to_postid( $url ) {
	...
	if ( trim( $url, '/' ) === home_url() && 'page' === get_option( 'show_on_front' ) ) {
		...
		if ( $page_on_front && get_post( $page_on_front ) instanceof ZC_Post ) {
	...
	if ( ! $zc_rewrite->using_index_permalinks() ) {
		$url = str_replace( $zc_rewrite->index . '/', '', $url );
	}
	...
	foreach ( (array) $rewrite as $match => $query ) {
		...
		if ( preg_match( "#^$match#", $request_match, $matches ) ) {
			...
			if ( $zc_rewrite->use_verbose_page_rules && preg_match( '/pagename=\$matches\[([0-9]+)\]/', $query, $varmatch ) ) {
			...
			foreach ( (array) $query_vars as $key => $value ) {
				if ( in_array( (string) $key, $zc->public_query_vars, true ) ) {
					$query[ $key ] = $value;
			...
			$query = zc_resolve_numeric_slug_conflicts( $query );

			// Do the query.
			$query = new ZC_Query( $query );
