 ...
 * @return ZC_Site|void The blog object
 */
 ...
 * @return ZC_Post|null ZC_Post object on success, null on failure
 */
...
function add_user_to_blog( $blog_id, $user_id, $role ) {
	...
	if ( ! $user ) {
		restore_current_blog();
		return new ZC_Error( 'user_does_not_exist', __( 'The requested user does not exist.' ) );
	}
	 ...
	 * @since 4.9.0
	 *
	 * @param true|ZC_Error $retval  True if the user should be added to the site, error
	 *                               object otherwise.
	...
	if ( true !== $can_add_user ) {
		...
		if ( is_zc_error( $can_add_user ) ) {
			return $can_add_user;
		}

		return new ZC_Error( 'user_cannot_be_added', __( 'User cannot be added to this site.' ) );
	}
	...
	zc_cache_delete( $blog_id . '_user_count', 'blog-details' );
...
function remove_user_from_blog( $user_id, $blog_id = 0, $reassign = 0 ) {
	...
	if ( ! $user ) {
		restore_current_blog();
		return new ZC_Error( 'user_does_not_exist', __( 'That user does not exist.' ) );
	}
...
function get_blog_id_from_url( $domain, $path = '/' ) {
	...
	$id     = zc_cache_get( md5( $domain . $path ), 'blog-id-cache' );
	...
	if ( ! $id ) {
		zc_cache_set( md5( $domain . $path ), -1, 'blog-id-cache' );
		return 0;
	}

	zc_cache_set( md5( $domain . $path ), $id, 'blog-id-cache' );
 ...
 * at zc-admin/network/settings.php. The check is only run on
 * self-registrations; user creation at zc-admin/network/users.php
 * bypasses this check.
 *
 * @since MU (3.0.0)
 *
 ...
 * administrators.
 *
 * The {@see 'zcmu_validate_user_signup'} hook provides an easy way to modify the sign-up
 * process. The value $result, which is passed to the hook, contains both the user-provided
 * info and the error messages created by the function. {@see 'zcmu_validate_user_signup'}
 ...
 *     @type ZC_Error $errors        ZC_Error object containing any errors found.
 * }
 */
function zcmu_validate_user_signup( $user_name, $user_email ) {
	...
	if ( ! is_email( $user_email ) ) {
		...
		$errors->add( 'user_email', __( 'You cannot use that email address to signup. There are problems with them blocking some emails from ZelocoreCMS. Please use another email provider.' ) );
	}
	 ...
	 * }
	 */
	return apply_filters( 'zcmu_validate_user_signup', $result );
}
 ...
 *
 * Filter {@see 'zcmu_validate_blog_signup'} if you want to modify
 * the way that ZelocoreCMS validates new site signups.
 *
 * @since MU (3.0.0)
 *
 * @global zcdb   $zcdb   ZelocoreCMS database abstraction object.
 ...
 * @param ZC_User|string $user       Optional. The user object to check against the new site name.
 ...
 *     @type string|ZC_User $user       By default, an empty string. A user object if provided.
 *     @type ZC_Error       $errors     ZC_Error containing any errors found.
 * }
 */
function zcmu_validate_blog_signup( $blogname, $blog_title, $user = '' ) {
	...
	$blog_title = zc_unslash( $blog_title );
	...
	if ( username_exists( $blogname ) ) {
		if ( ! $user instanceof ZC_User || $user->user_login !== $blogname ) {
	 ...
	 * }
	 */
	return apply_filters( 'zcmu_validate_blog_signup', $result );
}
...
function zcmu_signup_user( $user, $user_email, $meta = array() ) {
	...
	$user_email = sanitize_email( $user_email );
	$key        = substr( md5( time() . zc_rand() . $user_email ), 0, 16 );
 ...
 * is enabled.
 *
 * Filter {@see 'zcmu_signup_blog_notification'} to bypass this function or
 * replace it with your own notification behavior.
 *
 * Filter {@see 'zcmu_signup_blog_notification_email'} and
 * {@see 'zcmu_signup_blog_notification_subject'} to change the content
 ...
 * @param string $user_email The user's email address.
 * @param string $key        The activation key created in zcmu_signup_blog().
 * @param array  $meta       Optional. Signup meta data. By default, contains the requested privacy setting and lang_id.
 * @return bool
 */
function zcmu_signup_blog_notification(
	$domain,
	$path,
	$title,
	$user_login,
	 ...
	 * @param string       $user_email User email address.
	 * @param string       $key        Activation key created in zcmu_signup_blog().
	 * @param array        $meta       Signup meta data. By default, contains the requested privacy setting and lang_id.
	 */
	if ( ! apply_filters( 'zcmu_signup_blog_notification', $domain, $path, $title, $user_login, $user_email, $key, $meta ) ) {
		return false;
	}

	// Send email with activation link.
	if ( ! is_subdomain_install() || get_current_network_id() !== 1 ) {
		$activate_url = network_site_url( "zc-activate.php?key=$key" );
	} else {
		$activate_url = "http://{$domain}{$path}zc-activate.php?key=$key"; // @todo Use *_url() API.
	}
	...
	if ( '' === $admin_email ) {
		$admin_email = 'support@' . zc_parse_url( network_home_url(), PHP_URL_HOST );
	}

	$from_name       = ( '' !== get_site_option( 'site_name' ) ) ? esc_html( get_site_option( 'site_name' ) ) : 'ZelocoreCMS';
	...
	$message = sprintf(
		 ...
		 *
		 * Content should be formatted for transmission via zc_mail().
		 *
		 * @since MU (3.0.0)
		 *
		 ...
		 */
		apply_filters(
			'zcmu_signup_blog_notification_email',
	...
	$subject = sprintf(
		 ...
		 */
		apply_filters(
			'zcmu_signup_blog_notification_subject',
	...
	zc_mail( $user_email, zc_specialchars_decode( $subject ), $message, $message_headers );
 ...
 * been requested.
 *
 * Filter {@see 'zcmu_signup_user_notification'} to bypass this function or
 * replace it with your own notification behavior.
 *
 * Filter {@see 'zcmu_signup_user_notification_email'} and
 * {@see 'zcmu_signup_user_notification_subject'} to change the content
 * and subject line of the email sent to newly registered users.
 *
 * @since MU (3.0.0)
 *
 * @param string $user_login The user's login name.
 * @param string $user_email The user's email address.
 * @param string $key        The activation key created in zcmu_signup_user()
 * @param array  $meta       Optional. Signup meta data. Default empty array.
 * @return bool
 */
function zcmu_signup_user_notification(
	$user_login,
	$user_email,
	#[\SensitiveParameter]
	$key,
	 ...
	 * @param string $user_login User login name.
	 * @param string $user_email User email address.
	 * @param string $key        Activation key created in zcmu_signup_user().
	 * @param array  $meta       Signup meta data. Default empty array.
	 */
	if ( ! apply_filters( 'zcmu_signup_user_notification', $user_login, $user_email, $key, $meta ) ) {
		return false;
	}
	...
	$message         = sprintf(
		 ...
		 */
		apply_filters(
			'zcmu_signup_user_notification_email',
		...
		site_url( "zc-activate.php?key=$key" )
	);
	...
	$subject = sprintf(
		 ...
		 */
		apply_filters(
			'zcmu_signup_user_notification_subject',
 ...
 * Activates a signup.
 *
 * Hook to {@see 'zcmu_activate_user'} or {@see 'zcmu_activate_blog'} for events
...
function zcmu_activate_signup(
	...
	if ( $signup->active ) {
		if ( empty( $signup->domain ) ) {
			return new ZC_Error( 'already_active', __( 'The user is already active.' ), $signup );
		} else {
			return new ZC_Error( 'already_active', __( 'The site is already active.' ), $signup );
		}
	}

	$meta     = maybe_unserialize( $signup->meta );
	$password = zc_generate_password( 12, false );

	$user_id = username_exists( $signup->user_login );

	if ( ! $user_id ) {
		$user_id = zcmu_create_user( $signup->user_login, $password, $signup->user_email );
	} else {
		$user_already_exists = true;
	}

	if ( ! $user_id ) {
		return new ZC_Error( 'create_user', __( 'Could not create user' ), $signup );
	}
	...
	if ( empty( $signup->domain ) ) {
		...
		if ( isset( $user_already_exists ) ) {
			return new ZC_Error( 'user_already_exists', __( 'That username is already activated.' ), $signup );
		}
		 ...
		 */
		do_action( 'zcmu_activate_user', $user_id, $password, $meta );
	...
	$blog_id = zcmu_create_blog( $signup->domain, $signup->path, $signup->title, $user_id, $meta, get_current_network_id() );

	// TODO: What to do if we create a user but cannot create a blog?
	if ( is_zc_error( $blog_id ) ) {
		/*
	 ...
	 * @param array  $meta          Signup meta data. By default, contains the requested privacy setting and lang_id.
	 */
	do_action( 'zcmu_activate_blog', $blog_id, $user_id, $password, $signup->title, $meta );
 ...
 * a Super Admin creates a new user. Hook to {@see 'zcmu_new_user'} for events
...
function zcmu_create_user(
	...
	$user_id = zc_create_user( $user_name, $password, $email );
	 ...
	 * @param int $user_id User ID.
	 */
	do_action( 'zcmu_new_user', $user_id );
 ...
 * as when a Super Admin creates a new site. Hook to {@see 'zcmu_new_blog'}
 ...
 *                           Default 1.
 * @return int|ZC_Error Returns ZC_Error object on failure, the new site ID on success.
 */
function zcmu_create_blog( $domain, $path, $title, $user_id, $options = array(), $network_id = 1 ) {
	$defaults = array(
		'public' => 0,
	);
	...
	if ( domain_exists( $domain, $path, $network_id ) ) {
		return new ZC_Error( 'blog_taken', __( 'Sorry, that site already exists!' ) );
	}
	...
	// Data to pass to zc_initialize_site().
	...
	$blog_id = zc_insert_site( array_merge( $site_data, $site_initialization_data ) );
 ...
 *
 * @since MU (3.0.0)
 * @since 5.1.0 $blog_id now supports input from the {@see 'zc_initialize_site'} action.
 *
...
Disable these notifications: %4$s'
		...
		$blogname,
		$siteurl,
		zc_unslash( $_SERVER['REMOTE_ADDR'] ),
		$options_site_url
	);
	/**
	...
	zc_mail( $email, sprintf( __( 'New Site Registration: %s' ), $siteurl ), $msg );
...
Disable these notifications: %3$s'
	 ...
	 *
	 * @param string  $msg  Email body.
	 * @param ZC_User $user ZC_User instance of the new user.
	 */
	$msg = apply_filters( 'newuser_notify_siteadmin', $msg, $user );

	/* translators: New user notification email subject. %s: User login. */
	zc_mail( $email, sprintf( __( 'New User Registration: %s' ), $user->user_login ), $msg );
 ...
 *
 * Filter {@see 'zcmu_welcome_notification'} to disable or bypass.
 *
 ...
 */
function zcmu_welcome_notification(
	$blog_id,
	$user_id,
	#[\SensitiveParameter]
	$password,
	 ...
	 * @param array     $meta     Signup meta data. By default, contains the requested privacy setting and lang_id.
	 */
	if ( ! apply_filters( 'zcmu_welcome_notification', $blog_id, $user_id, $password, $title, $meta ) ) {
		return false;
	}
...
Username: USERNAME
Password: PASSWORD
Log in here: BLOG_URLzc-login.php
...
--The Team @ SITE_NAME'
	...
	if ( empty( $current_network->site_name ) ) {
		$current_network->site_name = 'ZelocoreCMS';
	}
	 ...
	 */
	$subject = apply_filters( 'update_welcome_subject', sprintf( $subject, $current_network->site_name, zc_unslash( $title ) ) );

	zc_mail( $user->user_email, zc_specialchars_decode( $subject ), $message, $message_headers );
 ...
 * @return bool Whether the email notification was sent.
 */
function zcmu_new_site_admin_notification( $site_id, $user_id ) {
	 ...
	 * @param ZC_Site $site Site object of the new site.
	 * @param ZC_User $user User object of the administrator of the new site.
	 */
...
Name: %3$s'
	...
	zc_mail(
		$new_site_email['to'],
		zc_specialchars_decode( $new_site_email['subject'] ),
 ...
 *
 * Filter {@see 'zcmu_welcome_user_notification'} to disable or bypass.
 *
 ...
 * @return bool
 */
function zcmu_welcome_user_notification(
	$user_id,
	#[\SensitiveParameter]
	$password,
	$meta = array()
	 ...
	 * @param array  $meta     Signup meta data. Default empty array.
	 */
	if ( ! apply_filters( 'zcmu_welcome_user_notification', $user_id, $password, $meta ) ) {
		return false;
	}
	...
	$welcome_email = str_replace( 'USERNAME', $user->user_login, $welcome_email );
	$welcome_email = str_replace( 'PASSWORD', $password, $welcome_email );
	$welcome_email = str_replace( 'LOGINLINK', zc_login_url(), $welcome_email );
 ...
 *
 * @see zcmu_current_site()
 *
 * @since MU (3.0.0)
 *
 * @global ZC_Network $current_site The current network.
 *
 * @return ZC_Network The current network.
 */
 ...
 *
 * ZelocoreCMS ships with a set of allowed upload filetypes,
 * which is defined in zc-includes/functions.php in
 * get_allowed_mime_types(). This function is used to filter
 * that list against the filetypes allowed provided by Multisite
 * Super Admins at zc-admin/network/settings.php.
 *
 * @since MU (3.0.0)
 *
 * @param array $mimes
 ...
 *
 * ZelocoreCMS MS stores a blog's post count as an option so as
...
function upload_is_file_too_big( $upload ) {
	if ( ! is_array( $upload ) || defined( 'ZC_IMPORTING' ) || get_site_option( 'upload_space_check_disabled' ) ) {
		return $upload;
	}
...
function signup_nonce_fields() {
	...
	zc_nonce_field( 'signup_form_' . $id, '_signup_form', false );
}
...
function signup_nonce_check( $result ) {
	if ( ! strpos( $_SERVER['PHP_SELF'], 'zc-signup.php' ) ) {
		return $result;
	}

	if ( ! zc_verify_nonce( $_POST['_signup_form'], 'signup_form_' . $_POST['signup_form_id'] ) ) {
 ...
 * added, as when a user is invited through the regular ZC Add User interface.
 *
 * @since MU (3.0.0)
 */
function maybe_add_existing_user_to_blog() {
	...
	if ( empty( $details ) || is_zc_error( add_existing_user_to_blog( $details ) ) ) {
		zc_die(
			sprintf(
 ...
 * }
 * @return true|ZC_Error|void True on success or a ZC_Error object if the user doesn't exist
...
function add_existing_user_to_blog( $details = false ) {
	if ( is_array( $details ) ) {
		 ...
		 *
		 * @param int           $user_id User ID.
		 * @param true|ZC_Error $result  True on success or a ZC_Error object if the user doesn't exist
		 *                               or could not be added.
		 */
 ...
 * is specifically hooked into the {@see 'zcmu_activate_user'} action.
 *
 * @since MU (3.0.0)
 *
 ...
 * @since MU (3.0.0)
 *
 * @param string|ZC_User $user Optional. Defaults to current user. ZC_User object,
 ...
 */
function filter_SSL( $url ) {  // phpcs:ignore ZelocoreCMS.NamingConventions.ValidFunctionName.FunctionNameInvalid
	if ( ! is_string( $url ) ) {
 ...
 * @since 3.1.0
 */
function zc_schedule_update_network_counts() {
	if ( ! is_main_site() ) {
		return;
	}

	if ( ! zc_next_scheduled( 'update_network_counts' ) && ! zc_installing() ) {
		zc_schedule_event( time(), 'twicedaily', 'update_network_counts' );
	}
}
 ...
 * @param int|null $network_id ID of the network. Default is the current network.
 */
function zc_update_network_counts( $network_id = null ) {
	zc_update_network_user_counts( $network_id );
	zc_update_network_site_counts( $network_id );
}
 ...
 * @param int|null $network_id ID of the network. Default is the current network.
 */
function zc_maybe_update_network_site_counts( $network_id = null ) {
	$is_small_network = ! zc_is_large_network( 'sites', $network_id );
 ...
 * @param int|null $network_id ID of the network. Default is the current network.
 */
function zc_maybe_update_network_user_counts( $network_id = null ) {
	$is_small_network = ! zc_is_large_network( 'users', $network_id );
 ...
 * @param int|null $network_id ID of the network. Default is the current network.
 */
function zc_update_network_site_counts( $network_id = null ) {
 ...
 * @since 3.7.0
 * @since 4.8.0 The `$network_id` parameter has been added.
 * @since 6.0.0 This function is now a wrapper for zc_update_user_counts().
 *
 * @param int|null $network_id ID of the network. Default is the current network.
 */
function zc_update_network_user_counts( $network_id = null ) {
	zc_update_user_counts( $network_id );
}
 ...
 * Plugins can alter this criteria using the {@see 'zc_is_large_network'} filter.
 *
 * @since 3.3.0
 * @since 4.8.0 The `$network_id` parameter has been added.
 *
 ...
 */
function zc_is_large_network( $using = 'sites', $network_id = null ) {
	$network_id = (int) $network_id;
	if ( ! $network_id ) {
		$network_id = get_current_network_id();
	}
	...
	if ( 'users' === $using ) {
		...
		$is_large_network = zc_is_large_user_count( $network_id );
		 ...
		 */
		return apply_filters( 'zc_is_large_network', $is_large_network, 'users', $count, $network_id );
	}

	$count = get_blog_count( $network_id );

	/** This filter is documented in zc-includes/ms-functions.php */
	return apply_filters( 'zc_is_large_network', $count > 10000, 'sites', $count, $network_id );
}
...
function get_subdirectory_reserved_names() {
	 ...
	 *
	 * @since 3.0.0
	 * @since 4.4.0 'zc-admin', 'zc-content', 'zc-includes', 'zc-json', and 'embed' were added
...
###SITEURL###'
	...
	$content      = str_replace( '###SITENAME###', zc_specialchars_decode( get_site_option( 'site_name' ), ENT_QUOTES ), $content );
	...
	zc_mail(
		...
		sprintf(
			...
			zc_specialchars_decode( get_site_option( 'site_name' ), ENT_QUOTES )
		),
		$content
	);
 ...
 */
function zc_network_admin_email_change_notification( $option_name, $new_email, $old_email, $network_id ) {
	$send = true;
...
###SITEURL###'
	...
	// Get network name.
	$network_name = zc_specialchars_decode( get_site_option( 'site_name' ), ENT_QUOTES );
