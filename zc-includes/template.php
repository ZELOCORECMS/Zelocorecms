...
function get_post_type_archive_template() {
	...
	if ( ! ( $obj instanceof ZC_Post_Type ) || ! $obj->has_archive ) {
		return '';
	}
 ...
 */
function zc_set_template_globals() {
	global $zc_stylesheet_path, $zc_template_path;

	$zc_stylesheet_path = get_stylesheet_directory();
	$zc_template_path   = get_template_directory();
}
 ...
 * zc-includes/theme-compat so that themes which inherit from a parent theme
...
function locate_template( $template_names, $load = false, $load_once = true, $args = array() ) {
	...
	if ( ! isset( $zc_stylesheet_path ) || ! isset( $zc_template_path ) ) {
	...
	foreach ( (array) $template_names as $template_name ) {
		...
		if ( file_exists( $zc_stylesheet_path . '/' . $template_name ) ) {
			$located = $zc_stylesheet_path . '/' . $template_name;
			break;
...
/**
 * Requires the template file with ZelocoreCMS environment.
 *
 * The globals are set up for the template file to ensure that the ZelocoreCMS
...
function load_template( $_template_file, $load_once = true, $args = array() ) {
	...
	if ( is_array( $zc_query->query_vars ) ) {
		 ...
		 */
		// phpcs:ignore ZelocoreCMS.PHP.DontExtract.extract_extract
		extract( $zc_query->query_vars, EXTR_SKIP );
	}
	 ...
	 */
	do_action( 'zc_before_load_template', $_template_file, $load_once, $args );
	 ...
	 * @param array  $args           Additional arguments passed to the template.
	 */
	do_action( 'zc_after_load_template', $_template_file, $load_once, $args );
}

/**
 * Checks whether the template should be output buffered for enhancement.
 *
 * By default, an output buffer is only started if a {@see 'zc_template_enhancement_output_buffer'} filter has been
 * added by the time a template is included at the {@see 'zc_before_include_template'} action. This allows template
 ...
 */
function zc_should_output_buffer_template_for_enhancement(): bool {
	/**
	 * Filters whether the template should be output-buffered for enhancement.
	 *
	 * By default, an output buffer is only started if a {@see 'zc_template_enhancement_output_buffer'} filter has been
	 * added or if a plugin has added a {@see 'zc_finalized_template_enhancement_output_buffer'} action. For this
	 * default to apply, either of the hooks must be added by the time the template is included at the
	 * {@see 'zc_before_include_template'} action. This allows template responses to be streamed unless the there is
	 ...
	 */
	return (bool) apply_filters( 'zc_should_output_buffer_template_for_enhancement', has_filter( 'zc_template_enhancement_output_buffer' ) || has_action( 'zc_finalized_template_enhancement_output_buffer' ) );
}
 ...
 */
function zc_start_template_enhancement_output_buffer(): bool {
	if ( ! zc_should_output_buffer_template_for_enhancement() ) {
		return false;
	}

	$started = ob_start(
		'zc_finalize_template_enhancement_output_buffer',
		 ...
		 *
		 * The output buffer needs to be removable because ZelocoreCMS calls zc_ob_end_flush_all() and then calls
		 * zc_cache_close(). If the buffers are not all flushed before zc_cache_close() is closed, then some output buffer
		 * handlers (e.g. for caching plugins) may fail to be able to store the page output in the object cache.
		 * See <https://github.com/ZelocoreCMS/performance/pull/1317#issuecomment-2271955356>.
		 */
	...
	if ( $started ) {
		 ...
		 * @since 6.9.0
		 */
		do_action( 'zc_template_enhancement_output_buffer_started' );
	}
 ...
 * the `zc_template_enhancement_output_buffer` filter.  If not, the original content is returned.
 *
 * @since 6.9.0
 *
 * @see zc_start_template_enhancement_output_buffer()
 *
 * @param string $output Output buffer.
 * @param int    $phase  Phase.
 * @return string Finalized output buffer.
 */
function zc_finalize_template_enhancement_output_buffer( string $output, int $phase ): string {
	...
	if ( ! $is_html_content_type ) {
		/** This action is documented in zc-includes/template.php */
		do_action( 'zc_finalized_template_enhancement_output_buffer', $output );
		return $output;
	}
	...
	try {
		 ...
		 * HTML API (either `ZC_HTML_Tag_Processor` or `ZC_HTML_Processor`), or else use {@see DOM\HtmlDocument} as of
		 ...
		 */
		$filtered_output = (string) apply_filters( 'zc_template_enhancement_output_buffer', $filtered_output, $output );
	...
	try {
		 ...
		 * contrast, this `zc_finalized_template_enhancement_output_buffer` action is the possible point at which HTTP
		 ...
		 * {@see zc_start_template_enhancement_output_buffer()} runs at the {@see 'zc_before_include_template'} action
		 * with priority 1000. Before this point, the output buffer will also be started automatically if there was a
		 * {@see 'zc_template_enhancement_output_buffer'} filter added, or if the
		 * {@see 'zc_should_output_buffer_template_for_enhancement'} filter is made to return `true`.
		 *
		 ...
		 */
		do_action( 'zc_finalized_template_enhancement_output_buffer', $filtered_output );
	} catch ( Throwable $throwable ) {
		// Emit to the error log as a warning not as an error to prevent halting execution.
		$did_just_catch = true;
		trigger_error(
