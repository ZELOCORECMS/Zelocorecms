 ...
 *
 * @param ZC_Post|ZC_Block_Editor_Context $post_or_block_editor_context The current post object or
...
function get_block_categories( $post_or_block_editor_context ) {
	...
	$block_editor_context = $post_or_block_editor_context instanceof ZC_Post ?
		new ZC_Block_Editor_Context(
			array(
	 ...
	 * @param ZC_Block_Editor_Context $block_editor_context The current block editor context.
	 */
	...
	if ( ! empty( $block_editor_context->post ) ) {
		 ...
		 *
		 * @param array[] $block_categories Array of categories for block types.
		 * @param ZC_Post $post             Post being loaded.
		 */
...
function get_allowed_block_types( $block_editor_context ) {
	...
	if ( ! empty( $block_editor_context->post ) ) {
		 ...
		 * @param bool|string[] $allowed_block_types Array of block type slugs, or boolean to enable/disable all.
		 *                                           Default true (all registered block types supported)
		 * @param ZC_Post       $post                The post resource data.
		 */
...
function get_default_block_editor_settings() {
	...
	// zc_max_upload_size() can be expensive, so only call it when relevant for the current user.
	$max_upload_size = 0;
	...
	$all_sizes        = zc_get_registered_image_subsizes();
 ...
 * @access private
 *
 * @global ZC_Styles  $zc_styles  The ZC_Styles current instance.
 * @global ZC_Scripts $zc_scripts The ZC_Scripts current instance.
 *
 * @return array {
 ...
 * }
 */
function _zc_get_iframed_editor_assets() {
	global $zc_styles, $zc_scripts;

	// Keep track of the styles and scripts instance to restore later.
	$current_zc_styles  = $zc_styles;
	$current_zc_scripts = $zc_scripts;
	 ...
	 */
	$zc_styles->registered  = $current_zc_styles->registered;
	$zc_scripts->registered = $current_zc_scripts->registered;
	 ...
	 */
	$zc_styles->done =
		zc_theme_has_theme_json() ? array( 'zc-reset-editor-styles' ) : array();

	zc_enqueue_script( 'zc-polyfill' );
	// Enqueue the `editorStyle` handles for all core block, and dependencies.
	zc_enqueue_style( 'zc-edit-blocks' );

	if ( current_theme_supports( 'zc-block-styles' ) ) {
	...
	/** This action is documented in zc-includes/script-loader.php */
	 ...
	 */
	$has_emoji_styles = has_action( 'zc_print_styles', 'print_emoji_styles' );
	if ( $has_emoji_styles ) {
		remove_action( 'zc_print_styles', 'print_emoji_styles' );
	}

	ob_start();
	zc_print_styles();
	zc_print_font_faces();
	zc_print_font_faces_from_style_variations();
	$styles = ob_get_clean();

	if ( $has_emoji_styles ) {
		add_action( 'zc_print_styles', 'print_emoji_styles' );
	}

	ob_start();
	zc_print_head_scripts();
	zc_print_footer_scripts();
	$scripts = ob_get_clean();

	// Restore the original instances.
	$zc_styles  = $current_zc_styles;
	$zc_scripts = $current_zc_scripts;
 ...
 */
function zc_get_first_block( $blocks, $block_name ) {
	foreach ( $blocks as $block ) {
		if ( $block_name === $block['blockName'] ) {
			return $block;
		}
		if ( ! empty( $block['innerBlocks'] ) ) {
			$found_block = zc_get_first_block( $block['innerBlocks'], $block_name );
 ...
 */
function zc_get_post_content_block_attributes() {
	global $post_ID;
	...
	if ( ! empty( $current_template ) ) {
		...
		$post_content_block = zc_get_first_block( $template_blocks, 'core/post-content' );
...
function get_block_editor_settings( array $custom_settings, $block_editor_context ) {
	...
	foreach ( array_keys( ZC_Block_Type_Registry::get_instance()->get_all_registered() ) as $block_type ) {
	...
	foreach ( $presets as $preset_style ) {
		$actual_css = zc_get_global_stylesheet( array( $preset_style['css'] ) );
		if ( '' !== $actual_css ) {
	...
	$actual_css    = zc_get_global_stylesheet( array( $block_classes['css'] ) );
	if ( '' !== $actual_css ) {
	 ...
	 */
	$global_styles[] = array(
		'css'            => zc_get_global_stylesheet( array( 'custom-css' ) ),
		'__unstableType' => 'user',
		'isGlobalStyles' => true,
	);
	...
	$editor_settings['__experimentalFeatures'] = zc_get_global_settings();
	...
	$editor_settings['__unstableResolvedAssets']         = _zc_get_iframed_editor_assets();
	$editor_settings['__unstableIsBlockBasedTheme']      = zc_is_block_theme();
	...
	$post_content_block_attributes = zc_get_post_content_block_attributes();
	...
	if ( ! empty( $block_editor_context->post ) ) {
		 ...
		 *
		 * @param array   $editor_settings Default editor settings.
		 * @param ZC_Post $post            Post being edited.
		 */
...
function block_editor_rest_api_preload( array $preload_paths, $block_editor_context ) {
	global $post, $zc_scripts, $zc_styles;
	...
	if ( ! empty( $block_editor_context->post ) ) {
		 ...
		 *
		 * @param (string|string[])[] $preload_paths Array of paths to preload.
		 * @param ZC_Post             $selected_post Post being edited.
		 */
	...
	/*
	 * Ensure the global $post, $zc_scripts, and $zc_styles remain the same after
	...
	$backup_zc_scripts  = ! empty( $zc_scripts ) ? clone $zc_scripts : $zc_scripts;
	$backup_zc_styles   = ! empty( $zc_styles ) ? clone $zc_styles : $zc_styles;
	...
	// Restore the global $post, $zc_scripts, and $zc_styles as they were before API preloading.
	$post       = $backup_global_post;
	$zc_scripts = $backup_zc_scripts;
	$zc_styles  = $backup_zc_styles;
	...
	if ( $editor_styles && current_theme_supports( 'editor-styles' ) ) {
		foreach ( $editor_styles as $style ) {
			if ( preg_match( '~^(https?:)?//~', $style ) ) {
				$response = zc_remote_get( $style );
				if ( ! is_zc_error( $response ) ) {
					$styles[] = array(
						'css'            => zc_remote_retrieve_body( $response ),
						'__unstableType' => 'theme',
						'isGlobalStyles' => false,
					);
				}
 ...
 * @since 6.8.0
 */
function zc_initialize_site_preview_hooks() {
	if (
		! defined( 'IFRAME_REQUEST' ) &&
		isset( $_GET['zc_site_preview'] ) &&
		1 === (int) $_GET['zc_site_preview'] &&
