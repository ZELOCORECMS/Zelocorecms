 ...
 * and executing discrete capabilities within ZelocoreCMS. An "ability" is a
 ...
 *
 * Abilities must be registered on the `zc_abilities_api_init` action hook.
 ...
 *         );
 *     }
 *     add_action( 'zc_abilities_api_init', 'my_plugin_register_abilities' );
 *
 * Once registered, abilities can be checked, retrieved, and managed:
 *
 *     // Checks if an ability is registered, and prints its label.
 *     if ( zc_has_ability( 'my-plugin/export-users' ) ) {
 *         $ability = zc_get_ability( 'my-plugin/export-users' );
 *
 *         echo $ability->get_label();
 *     }
 *
 *     // Gets all registered abilities.
 *     $all_abilities = zc_get_abilities();
 *
 *     // Unregisters when no longer needed.
 *     zc_unregister_ability( 'my-plugin/export-users' );
 *
 * ## Best Practices
 *
 *  - Always register abilities on the `zc_abilities_api_init` hook.
 ...
 *  - Return `ZC_Error` objects for failures rather than throwing exceptions.
 ...
 *
 *  1. Hook into the `zc_abilities_api_init` action.
 *  2. Call `zc_register_ability()` with a namespaced name and configuration.
 ...
 *         );
 *     }
 *     add_action( 'zc_abilities_api_categories_init', 'my_plugin_register_categories' );
 *
 ...
 *
 * ZelocoreCMS implements a validator based on a subset of the JSON Schema Version 4
 ...
 * related ZelocoreCMS REST API Schema documentation:
 * https://developer.zelocorecms.org/rest-api/extending-the-rest-api/schema/#json-schema-basics
 *
 ...
 * optional input data and returns either a result or `ZC_Error` on failure.
 *
 *     function my_plugin_analyze_text( string $input ): string|ZC_Error {
 *         $score = My_Plugin::perform_sentiment_analysis( $input );
 *         if ( is_zc_error( $score ) ) {
 *             return $score;
 *         }
 ...
 * boolean or `ZC_Error`. Common use cases include checking user capabilities,
 * validating API keys, or verifying system state:
 *
 *     function my_plugin_can_analyze_text( string $input ): bool|ZC_Error {
 ...
 *     ),
 *
 * This allows abilities to be invoked via HTTP requests to the ZelocoreCMS REST API.
 *
 * @since 6.9.0
 *
 * @see ZC_Abilities_Registry::register()
 * @see zc_register_ability_category()
 * @see zc_unregister_ability()
 *
 ...
 *                                                     The ability category must be registered via `zc_register_ability_category()`
 ...
 *                                                     value (any type) or a `ZC_Error` object on failure.
 ...
 *                                                     must return `true`/`false` for simple checks, or `ZC_Error` for
 ...
 *                                                     instead of the default `ZC_Ability` class. The custom class
 *                                                     must extend `ZC_Ability`. Useful for advanced customization
 *                                                     of ability behavior.
 * }
 * @return ZC_Ability|null The registered ability instance on success, `null` on failure.
 */
function zc_register_ability( string $name, array $args ): ?ZC_Ability {
	if ( ! doing_action( 'zc_abilities_api_init' ) ) {
		_doing_it_wrong(
			__FUNCTION__,
			sprintf(
				/* translators: 1: zc_abilities_api_init, 2: string value of the ability name. */
				__( 'Abilities must be registered on the %1$s action. The ability %2$s was not registered.' ),
				'<code>zc_abilities_api_init</code>',
	...
	$registry = ZC_Abilities_Registry::get_instance();
	if ( null === $registry ) {
		return null;
	}
 ...
 * Example:
 *
 *     if ( zc_has_ability( 'other-plugin/some-ability' ) ) {
 *         zc_unregister_ability( 'other-plugin/some-ability' );
 *     }
 *
 * @since 6.9.0
 *
 * @see ZC_Abilities_Registry::unregister()
 * @see zc_register_ability()
 *
 * @param string $name The name of the ability to unregister, including namespace prefix
 *                     (e.g., 'my-plugin/my-ability').
 * @return ZC_Ability|null The unregistered ability instance on success, `null` on failure.
 */
function zc_unregister_ability( string $name ): ?ZC_Ability {
	$registry = ZC_Abilities_Registry::get_instance();
	if ( null === $registry ) {
		return null;
	}
 ...
 *     if ( zc_has_ability( 'premium-plugin/advanced-export' ) ) {
 ...
 * @since 6.9.0
 *
 * @see ZC_Abilities_Registry::is_registered()
 * @see zc_get_ability()
 *
 ...
 */
function zc_has_ability( string $name ): bool {
	$registry = ZC_Abilities_Registry::get_instance();
	if ( null === $registry ) {
		return false;
	}
 ...
 *     $ability = zc_get_ability( 'my-plugin/export-data' );
 *     if ( $ability ) {
 ...
 * @since 6.9.0
 *
 * @see ZC_Abilities_Registry::get_registered()
 * @see zc_has_ability()
 *
 * @param string $name The name of the ability, including namespace prefix
 *                     (e.g., 'my-plugin/my-ability').
 * @return ZC_Ability|null The registered ability instance, or `null` if not registered.
 */
function zc_get_ability( string $name ): ?ZC_Ability {
	$registry = ZC_Abilities_Registry::get_instance();
	if ( null === $registry ) {
		return null;
	}
 ...
 * @since 6.9.0
 *
 * @see ZC_Abilities_Registry::get_all_registered()
 *
 * @return ZC_Ability[] An array of registered ZC_Ability instances. Returns an empty
 *                     array if no abilities are registered or if the registry is unavailable.
 */
function zc_get_abilities(): array {
	$registry = ZC_Abilities_Registry::get_instance();
	if ( null === $registry ) {
		return array();
	}
 ...
 * that reference them.
 *
 * Ability categories must be registered on the `zc_abilities_api_categories_init` action hook.
 *
 * Example:
 *
 ...
 * @since 6.9.0
 *
 * @see ZC_Ability_Categories_Registry::register()
 * @see zc_register_ability()
 ...
 */
function zc_register_ability_category( string $slug, array $args ): ?ZC_Ability_Category {
	if ( ! doing_action( 'zc_abilities_api_categories_init' ) ) {
		_doing_it_wrong(
			__FUNCTION__,
			sprintf(
				/* translators: 1: zc_abilities_api_categories_init, 2: ability category slug. */
				__( 'Ability categories must be registered on the %1$s action. The ability category %2$s was not registered.' ),
				'<code>zc_abilities_api_categories_init</code>',
	...
	$registry = ZC_Ability_Categories_Registry::get_instance();
	if ( null === $registry ) {
		return null;
	}
 ...
 * Example:
 *
 *     if ( zc_has_ability_category( 'deprecated-category' ) ) {
 *         zc_unregister_ability_category( 'deprecated-category' );
 *     }
 *
 * @since 6.9.0
 *
 * @see ZC_Ability_Categories_Registry::unregister()
 * @see zc_register_ability_category()
 *
 ...
 */
function zc_unregister_ability_category( string $slug ): ?ZC_Ability_Category {
	$registry = ZC_Ability_Categories_Registry::get_instance();
	if ( null === $registry ) {
		return null;
	}
 ...
 *     if ( zc_has_ability_category( 'premium-features' ) ) {
 ...
 * @since 6.9.0
 *
 * @see ZC_Ability_Categories_Registry::is_registered()
 ...
 */
function zc_has_ability_category( string $slug ): bool {
	$registry = ZC_Ability_Categories_Registry::get_instance();
	if ( null === $registry ) {
		return false;
	}
 ...
 *     $ability_category = zc_get_ability_category( 'content-management' );
 *     if ( $ability_category ) {
 ...
 * @since 6.9.0
 *
 * @see ZC_Ability_Categories_Registry::get_registered()
 * @see zc_has_ability_category()
 * @see zc_get_ability_categories()
 *
 * @param string $slug The slug of the ability category.
 * @return ZC_Ability_Category|null The ability category instance, or `null` if not registered.
 */
function zc_get_ability_category( string $slug ): ?ZC_Ability_Category {
	$registry = ZC_Ability_Categories_Registry::get_instance();
	if ( null === $registry ) {
		return null;
	}
 ...
 *     $ability_categories = zc_get_ability_categories();
 ...
 * @since 6.9.0
 *
 * @see ZC_Ability_Categories_Registry::get_all_registered()
 * @see zc_get_ability_category()
 *
 * @return ZC_Ability_Category[] An array of registered ability category instances. Returns an empty array
 *                               if no ability categories are registered or if the registry is unavailable.
 */
function zc_get_ability_categories(): array {
	$registry = ZC_Ability_Categories_Registry::get_instance();
	if ( null === $registry ) {
		return array();
	}
