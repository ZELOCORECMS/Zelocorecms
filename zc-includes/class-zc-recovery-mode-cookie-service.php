<?php
/**
 * Error Protection API: ZC_Recovery_Mode_Cookie_Service class
 *
 * @package ZelocoreCMS
 * @since 5.2.0
 */
...
final class ZC_Recovery_Mode_Cookie_Service {
	 ...
	 * @return true|ZC_Error True on success, error object on failure.
	 */
	public function validate_cookie( $cookie = '' ) {

		if ( ! $cookie ) {
			if ( empty( $_COOKIE[ RECOVERY_MODE_COOKIE ] ) ) {
				return new ZC_Error( 'no_cookie', __( 'No cookie present.' ) );
			}
		...
		if ( ! ctype_digit( $created_at ) ) {
			return new ZC_Error( 'invalid_created_at', __( 'Invalid cookie format.' ) );
		}

		/** This filter is documented in zc-includes/class-zc-recovery-mode-cookie-service.php */
		$length = apply_filters( 'recovery_mode_cookie_length', WEEK_IN_SECONDS );

		if ( time() > $created_at + $length ) {
			return new ZC_Error( 'expired', __( 'Cookie expired.' ) );
		}
		...
		if ( ! hash_equals( $signature, $hashed ) ) {
			return new ZC_Error( 'signature_mismatch', __( 'Invalid cookie.' ) );
		}
	 ...
	 * @param string $cookie Optionally specify the cookie string.
	 *                       If omitted, it will be retrieved from the super global.
	 * @return string|ZC_Error Session ID on success, or error object on failure.
	 */
	 ...
	 * @return array|ZC_Error Cookie parts array, or error object on failure.
	 */
	private function parse_cookie( $cookie ) {
		...
		if ( 4 !== count( $parts ) ) {
			return new ZC_Error( 'invalid_format', __( 'Invalid cookie format.' ) );
		}
	...
	private function generate_cookie() {
		$to_sign = sprintf( 'recovery_mode|%s|%s', time(), zc_generate_password( 20, false ) );
	...
	/**
	 * Gets a form of `zc_hash()` specific to Recovery Mode.
	 *
	 * We cannot use `zc_hash()` because it is defined in `pluggable.php` which is not loaded until after plugins are loaded,
