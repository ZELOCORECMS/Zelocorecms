...
if ( ! function_exists( 'zc_set_current_user' ) ) :
	/**
	 * Changes the current user by ID or name.
	 *
	 * Set $id to null and specify a name if you do not know a user's ID.
	 *
	 * Some ZelocoreCMS functionality is based on the current user and not based on
	 * the signed in user. Therefore, it opens the ability to edit and perform
	 * actions on users who aren't signed in.
	 *
	 * @since 2.0.3
	 *
	 * @global ZC_User $current_user The current user object which holds the user data.
	 *
	 * @param int|null $id   User ID.
	 * @param string   $name User's username.
	 * @return ZC_User Current user User object.
	 */
	function zc_set_current_user( $id, $name = '' ) {
		global $current_user;

		// If `$id` matches the current user, there is nothing to do.
		if ( isset( $current_user )
		&& ( $current_user instanceof ZC_User )
		...
		$current_user = new ZC_User( $id, $name );
...
if ( ! function_exists( 'get_user_by' ) ) :
	...
	function get_user_by( $field, $value ) {
		$userdata = ZC_User::get_data_by( $field, $value );
...
if ( ! function_exists( 'zc_mail' ) ) :
	 ...
	 * {@see 'zc_mail_content_type'} filter.
	 *
	 * The default charset is based on the charset used on the blog. The charset can
	 * be set using the {@see 'zc_mail_charset'} filter.
	 *
	 ...
	 */
	function zc_mail( $to, $subject, $message, $headers = '', $attachments = array(), $embeds = array() ) {
		// Compact the input, apply the filters, and extract them back out.

		/**
		 * Filters the zc_mail() arguments.
		 *
		 * @since 2.2.0
		 * @since 6.9.0 The `$embeds` element was added to the `$args` array.
		 *
		 * @param array $args {
		 *     Array of the `zc_mail()` arguments.
		 *
		 ...
		 * }
		 */
		$atts = apply_filters( 'zc_mail', compact( 'to', 'subject', 'message', 'headers', 'attachments', 'embeds' ) );

		/**
		 * Filters whether to preempt sending an email.
		 *
		 * Returning a non-null value will short-circuit {@see zc_mail()}, returning
		 ...
		 * }
		 */
		$pre_zc_mail = apply_filters( 'pre_zc_mail', null, $atts );
		 ...
		 * to zc_mail().
		 *
		 * See https://core.trac.zelocorecms.org/ticket/33972
		 */
		...
		if ( ! isset( $from_name ) ) {
			$from_name = 'ZelocoreCMS';
		}

		/*
		 * If we don't have an email from the input headers, default to zelocorecms@$sitename
		 * Some hosts will block outgoing mail from this address if it doesn't exist,
		 * but there's no easy alternative. Defaulting to admin_email might appear to be
		 * another option, but some hosts may refuse to relay mail from an unknown domain.
		 * See https://core.trac.zelocorecms.org/ticket/5007.
		 */
		if ( ! isset( $from_email ) ) {
			// Get the site domain and get rid of www.
			$sitename   = zc_parse_url( network_home_url(), PHP_URL_HOST );
			$from_email = 'zelocorecms@';
		 ...
		 */
		$from_email = apply_filters( 'zc_mail_from', $from_email );
		 ...
		 */
		$from_name = apply_filters( 'zc_mail_from_name', $from_name );
		...
		try {
			...
			do_action( 'zc_mail_failed', new ZC_Error( 'zc_mail_failed', $e->getMessage(), $mail_error_data ) );
		...
		/**
		 * Filters the zc_mail() content type.
		 *
		 * @since 2.3.0
		 *
		 * @param string $content_type Default zc_mail() content type.
		 */
		$content_type = apply_filters( 'zc_mail_content_type', $content_type );
		...
		/**
		 * Filters the default zc_mail() charset.
		 *
		 * @since 2.3.0
		 *
		 * @param string $charset Default email charset.
		 */
		$phpmailer->CharSet = apply_filters( 'zc_mail_charset', $charset );
		...
		try {
			 ...
			 * }
			 */
			do_action( 'zc_mail_succeeded', $mail_data );
			 ...
			 * @since 4.4.0
			 *
			 * @param ZC_Error $error A ZC_Error object with the PHPMailer\PHPMailer\Exception message, and an array
			 *                        containing the mail recipient, subject, message, headers, attachments, and embeds.
			 */
			do_action( 'zc_mail_failed', new ZC_Error( 'zc_mail_failed', $e->getMessage(), $mail_data ) );
...
if ( ! function_exists( 'zc_authenticate' ) ) :
	/**
	 ...
	 * @return ZC_User|ZC_Error ZC_User object if the credentials are valid,
	 *                          otherwise ZC_Error.
	 */
	function zc_authenticate(
		$username,
		#[\SensitiveParameter]
		$password
	) {
		 ...
		 *
		 * A ZC_User object is returned if the credentials authenticate a user.
		 * ZC_Error or null otherwise.
		 *
		 * @since 2.8.0
		 * @since 4.5.0 `$username` now accepts an email address.
		 *
		 * @param null|ZC_User|ZC_Error $user     ZC_User if the user is authenticated.
		 *                                        ZC_Error or null otherwise.
		...
		if ( null === $user || false === $user ) {
			 ...
			 */
			$user = new ZC_Error( 'authentication_failed', __( '<strong>Error:</strong> Invalid username, email address or incorrect password.' ) );
		}

		$ignore_codes = array( 'empty_username', 'empty_password' );

		if ( is_zc_error( $user ) && ! in_array( $user->get_error_code(), $ignore_codes, true ) ) {
			$error = $user;
			 ...
			 *
			 * @param string   $username Username or email address.
			 * @param ZC_Error $error    A ZC_Error object with the authentication failure details.
			 */
			do_action( 'zc_login_failed', $username, $error );
		}
...
if ( ! function_exists( 'zc_logout' ) ) :
	...
	function zc_logout() {
		 ...
		 */
		do_action( 'zc_logout', $user_id );
	}
endif;

if ( ! function_exists( 'zc_validate_auth_cookie' ) ) :
	/**
	 ...
	 */
	function zc_validate_auth_cookie( $cookie = '', $scheme = '' ) {
		$cookie_elements = zc_parse_auth_cookie( $cookie, $scheme );
		if ( ! $cookie_elements ) {
			/**
		...
		if ( zc_doing_ajax() || 'POST' === $_SERVER['REQUEST_METHOD'] ) {
		...
		$key = zc_hash( $username . '|' . $pass_frag . '|' . $expiration . '|' . $token, $scheme );
		...
		$manager = ZC_Session_Tokens::get_instance( $user->ID );
...
if ( ! function_exists( 'zc_generate_auth_cookie' ) ) :
	/**
	 ...
	 */
	function zc_generate_auth_cookie( $user_id, $expiration, $scheme = 'auth', $token = '' ) {
		...
		if ( ! $token ) {
			$manager = ZC_Session_Tokens::get_instance( $user_id );
		...
		$key = zc_hash( $user->user_login . '|' . $pass_frag . '|' . $expiration . '|' . $token, $scheme );
...
if ( ! function_exists( 'zc_parse_auth_cookie' ) ) :
	/**
	 ...
	 * }
	 */
	function zc_parse_auth_cookie( $cookie = '', $scheme = '' ) {
		if ( empty( $cookie ) ) {
			switch ( $scheme ) {
				case 'auth':
...
if ( ! function_exists( 'zc_set_auth_cookie' ) ) :
	/**
	 ...
	 */
	function zc_set_auth_cookie( $user_id, $remember = false, $secure = '', $token = '' ) {
		if ( $remember ) {
			/**
			 ...
			 * Needed for the login grace period in zc_validate_auth_cookie().
			 */
		...
		$auth_cookie      = zc_generate_auth_cookie( $user_id, $expiration, $scheme, $token );
		$logged_in_cookie = zc_generate_auth_cookie( $user_id, $expiration, 'logged_in', $token );
...
if ( ! function_exists( 'zc_clear_auth_cookie' ) ) :
	/**
	 * Removes all of the cookies associated with authentication.
	 *
	 * @since 2.5.0
	 */
	function zc_clear_auth_cookie() {
		/**
		...
		// Settings cookies.
		setcookie( 'zc-settings-' . get_current_user_id(), ' ', time() - YEAR_IN_SECONDS, SITECOOKIEPATH );
		setcookie( 'zc-settings-time-' . get_current_user_id(), ' ', time() - YEAR_IN_SECONDS, SITECOOKIEPATH );
		...
		setcookie( 'zc-postpass_' . COOKIEHASH, ' ', time() - YEAR_IN_SECONDS, COOKIEPATH, COOKIE_DOMAIN );
	}
endif;
...
if ( ! function_exists( 'auth_redirect' ) ) :
	...
	function auth_redirect() {
		...
		if ( $secure && ! is_ssl() && str_contains( $_SERVER['REQUEST_URI'], 'zc-admin' ) ) {
			if ( str_starts_with( $_SERVER['REQUEST_URI'], 'http' ) ) {
				zc_redirect( set_url_scheme( $_SERVER['REQUEST_URI'], 'https' ) );
				exit;
			} else {
				zc_redirect( 'https://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'] );
				exit;
			}
		}
		...
		$user_id = zc_validate_auth_cookie( '', $scheme );
		if ( $user_id ) {
			/**
			...
			if ( ! $secure && get_user_option( 'use_ssl', $user_id ) && str_contains( $_SERVER['REQUEST_URI'], 'zc-admin' ) ) {
				if ( str_starts_with( $_SERVER['REQUEST_URI'], 'http' ) ) {
					zc_redirect( set_url_scheme( $_SERVER['REQUEST_URI'], 'https' ) );
					exit;
				} else {
		...
		if ( str_contains( $_SERVER['REQUEST_URI'], '/options.php' ) && zc_get_referer() ) {
			$redirect = zc_get_referer();
		} else {
			$redirect = set_url_scheme( 'http://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'] );
		}

		$login_url = zc_login_url( $redirect, true );

		zc_redirect( $login_url );
		exit;
	}
endif;
...
if ( ! function_exists( 'check_admin_referer' ) ) :
	...
	function check_admin_referer( $action = -1, $query_arg = '_wpnonce' ) {
		...
		$referer  = strtolower( zc_get_referer() );
		$result   = isset( $_REQUEST[ $query_arg ] ) ? zc_verify_nonce( $_REQUEST[ $query_arg ], $action ) : false;
...
if ( ! function_exists( 'check_ajax_referer' ) ) :
	...
	function check_ajax_referer( $action = -1, $query_arg = false, $stop = true ) {
		...
		$result = zc_verify_nonce( $nonce, $action );
...
if ( ! function_exists( 'zc_redirect' ) ) :
	/**
	 * Redirects to another page.
	 *
	 * Note: zc_redirect() does not exit automatically, and should almost always be
	 * followed by a call to `exit;`:
	 *
	 *     zc_redirect( $url );
	 *     exit;
	 *
	 * Exiting can also be selectively manipulated by using zc_redirect() as a conditional
	 * in conjunction with the {@see 'zc_redirect'} and {@see 'zc_redirect_status'} filters:
	 *
	 *     if ( zc_redirect( $url ) ) {
	 *         exit;
	 *     }
	 ...
	 * @since 5.4.0 On invalid status codes, zc_die() is called.
	 *
	 * @global bool $is_IIS
	 *
	 * @param string       $location      The path or URL to redirect to.
	 * @param int          $status        Optional. HTTP response status code to use. Default '302' (Moved Temporarily).
	 * @param string|false $x_redirect_by Optional. The application doing the redirect or false to omit. Default 'ZelocoreCMS'.
	 * @return bool False if the redirect was canceled, true otherwise.
	 */
	function zc_redirect( $location, $status = 302, $x_redirect_by = 'ZelocoreCMS' ) {
		global $is_IIS;
		 ...
		 */
		$location = apply_filters( 'zc_redirect', $location, $status );
		 ...
		 * @param string $location The path or URL to redirect to.
		 */
		$status = apply_filters( 'zc_redirect_status', $status, $location );
		...
		if ( $status < 300 || 399 < $status ) {
			zc_die( __( 'HTTP redirect status code must be a redirection code, 3xx.' ) );
		}

		$location = zc_sanitize_redirect( $location );
...
if ( ! function_exists( 'zc_sanitize_redirect' ) ) :
	/**
	 ...
	 */
	function zc_sanitize_redirect( $location ) {
		// Encode spaces.
		...
		$location = preg_replace_callback( $regex, '_zc_sanitize_utf8_in_redirect', $location );
		$location = preg_replace( '|[^a-z0-9-~+_.?#=&;,/:%!*\[\]()@]|i', '', $location );
		$location = zc_kses_no_null( $location );
	 ...
	 * @access private
	 *
	 * @see zc_sanitize_redirect()
	 *
	 * @param array $matches RegEx matches against the redirect location.
	 * @return string URL-encoded version of the first RegEx match.
	 */
	function _zc_sanitize_utf8_in_redirect( $matches ) {
...
if ( ! function_exists( 'zc_safe_redirect' ) ) :
	/**
	 * Performs a safe (local) redirect, using zc_redirect().
	 *
	 * Checks whether the $location is using an allowed host, if it has an absolute
	 * path. A plugin can therefore set or remove allowed host(s) to or from the
	 * list.
	 *
	 * If the host is not allowed, then the redirect defaults to zc-admin on the siteurl
	 * instead. This prevents malicious redirects which redirect to another host,
	 * but only used in a few places.
	 *
	 * Note: zc_safe_redirect() does not exit automatically, and should almost always be
	 * followed by a call to `exit;`:
	 *
	 *     zc_safe_redirect( $url );
	 *     exit;
	 *
	 * Exiting can also be selectively manipulated by using zc_safe_redirect() as a conditional
	 * in conjunction with the {@see 'zc_redirect'} and {@see 'zc_redirect_status'} filters:
	 *
	 *     if ( zc_safe_redirect( $url ) ) {
	 *         exit;
	 *     }
	 *
	 * @since 2.3.0
	 * @since 5.1.0 The return value from zc_redirect() is now passed on, and the `$x_redirect_by` parameter was added.
	 *
	 * @param string       $location      The path or URL to redirect to.
	 * @param int          $status        Optional. HTTP response status code to use. Default '302' (Moved Temporarily).
	 * @param string|false $x_redirect_by Optional. The application doing the redirect or false to omit. Default 'ZelocoreCMS'.
	 * @return bool False if the redirect was canceled, true otherwise.
	 */
	function zc_safe_redirect( $location, $status = 302, $x_redirect_by = 'ZelocoreCMS' ) {

		// Need to look at the URL the way it will end up in zc_redirect().
		$location = zc_sanitize_redirect( $location );
		 ...
		 * @param int    $status       The HTTP response status code to use.
		 */
		$fallback_url = apply_filters( 'zc_safe_redirect_fallback', admin_url(), $status );

		$location = zc_validate_redirect( $location, $fallback_url );

		return zc_redirect( $location, $status, $x_redirect_by );
	}
endif;
...
if ( ! function_exists( 'zc_validate_redirect' ) ) :
	 ...
	 * @return string Redirect-sanitized URL.
	 */
	function zc_validate_redirect( $location, $fallback_url = '' ) {
		$location = zc_sanitize_redirect( trim( $location, " \t\n\r\0\x08\x0B" ) );
...
if ( ! function_exists( 'zc_notify_postauthor' ) ) :
	/**
	 ...
	 */
	function zc_notify_postauthor( $comment_id, $deprecated = null ) {
		if ( null !== $deprecated ) {
		...
		if ( ZC_Http::is_ip_address( $comment->comment_author_IP ) ) {
		 ...
		 */
		$blogname        = zc_specialchars_decode( get_option( 'blogname' ), ENT_QUOTES );
		$comment_content = zc_specialchars_decode( $comment->comment_content );

		$zc_email = 'zelocorecms@' . preg_replace( '#^www\.#', '', zc_parse_url( network_home_url(), PHP_URL_HOST ) );

		if ( '' === $comment->comment_author ) {
			$from = "From: \"$blogname\" <$zc_email>";
			if ( '' !== $comment->comment_author_email ) {
				$reply_to = "Reply-To: $comment->comment_author_email";
			}
		} else {
			$from = "From: \"$comment->comment_author\" <$zc_email>";
			if ( '' !== $comment->comment_author_email ) {
		...
		foreach ( $emails as $email ) {
			...
			zc_mail( $email, zc_specialchars_decode( $subject ), $notify_message, $message_headers );
...
if ( ! function_exists( 'zc_notify_moderator' ) ) :
	/**
...
if ( ! function_exists( 'zc_password_change_notification' ) ) :
	/**
	 ...
	 * @param ZC_User $user User object.
	 */
	function zc_password_change_notification( $user ) {
		/*
		...
		if ( 0 !== strcasecmp( $user->user_email, get_option( 'admin_email' ) ) ) {
			...
			$zc_password_change_notification_email = array(
			 ...
			 * @since 4.9.0
			 *
			 * @param array   $zc_password_change_notification_email {
			 ...
			 * }
			 * @param ZC_User $user     User object for user whose password was changed.
			 * @param string  $blogname The site title.
			 */
			$zc_password_change_notification_email = apply_filters( 'zc_password_change_notification_email', $zc_password_change_notification_email, $user, $blogname );

			zc_mail(
				$zc_password_change_notification_email['to'],
				zc_specialchars_decode( sprintf( $zc_password_change_notification_email['subject'], $blogname ) ),
				$zc_password_change_notification_email['message'],
				$zc_password_change_notification_email['headers']
			);
...
if ( ! function_exists( 'zc_new_user_notification' ) ) :
	/**
	 ...
	 */
	function zc_new_user_notification( $user_id, $deprecated = null, $notify = '' ) {
		if ( null !== $deprecated ) {
		 ...
		 * @param ZC_User $user User object for new user.
		 */
		$send_notification_to_admin = apply_filters( 'zc_send_new_user_notification_to_admin', true, $user );
		...
		if ( 'user' !== $notify && true === $send_notification_to_admin ) {
			...
			$zc_new_user_notification_email_admin = array(
				'to'      => get_option( 'admin_email' ),
			 ...
			 * @since 4.9.0
			 *
			 * @param array   $zc_new_user_notification_email_admin {
			 *     Used to build zc_mail().
			 *
			 *     @type string $to      The intended recipient - site admin email address.
			 *     @type string $subject The subject of the email.
			 ...
			 * @param string  $blogname The site title.
			 */
			$zc_new_user_notification_email_admin = apply_filters( 'zc_new_user_notification_email_admin', $zc_new_user_notification_email_admin, $user, $blogname );

			zc_mail(
				$zc_new_user_notification_email_admin['to'],
				zc_specialchars_decode( sprintf( $zc_new_user_notification_email_admin['subject'], $blogname ) ),
				$zc_new_user_notification_email_admin['message'],
				$zc_new_user_notification_email_admin['headers']
			);
		 ...
		 * @param ZC_User $user User object for new user.
		 */
		$send_notification_to_user = apply_filters( 'zc_send_new_user_notification_to_user', true, $user );
		 ...
		 */
		$message .= network_site_url( 'zc-login.php?login=' . rawurlencode( $user->user_login ) . "&key=$key&action=rp", 'login' ) . "\r\n";

		$zc_new_user_notification_email = array(
			'to'      => $user->user_email,
		 ...
		 * @since 4.9.0
		 *
		 * @param array   $zc_new_user_notification_email {
		 *     Used to build zc_mail().
		 *
		 ...
		 * @param string  $blogname The site title.
		 */
		$zc_new_user_notification_email = apply_filters( 'zc_new_user_notification_email', $zc_new_user_notification_email, $user, $blogname );

		zc_mail(
			$zc_new_user_notification_email['to'],
			zc_specialchars_decode( sprintf( $zc_new_user_notification_email['subject'], $blogname ) ),
			$zc_new_user_notification_email['message'],
			$zc_new_user_notification_email['headers']
		);
...
if ( ! function_exists( 'zc_nonce_tick' ) ) :
	/**
	 ...
	 */
	function zc_nonce_tick( $action = -1 ) {
		/**
...
if ( ! function_exists( 'zc_verify_nonce' ) ) :
	/**
	 ...
	 *                   False if the nonce is invalid.
	 */
	function zc_verify_nonce( $nonce, $action = -1 ) {
		$nonce = (string) $nonce;
		$user  = zc_get_current_user();
		$uid   = (int) $user->ID;
		if ( ! $uid ) {
		...
		$i     = zc_nonce_tick( $action );

		// Nonce generated 0-12 hours ago.
		$expected = substr( zc_hash( $i . '|' . $action . '|' . $uid . '|' . $token, 'nonce' ), -12, 10 );
		if ( hash_equals( $expected, $nonce ) ) {
			return 1;
		}

		// Nonce generated 12-24 hours ago.
		$expected = substr( zc_hash( ( $i - 1 ) . '|' . $action . '|' . $uid . '|' . $token, 'nonce' ), -12, 10 );
		if ( hash_equals( $expected, $nonce ) ) {
			return 2;
		}
		 ...
		 * @param ZC_User    $user   The current user object.
		 * @param string     $token  The user's session token.
		 */
		do_action( 'zc_verify_nonce_failed', $nonce, $action, $user, $token );
...
if ( ! function_exists( 'zc_create_nonce' ) ) :
	/**
	 ...
	 */
	function zc_create_nonce( $action = -1 ) {
		$user = zc_get_current_user();
		$uid  = (int) $user->ID;
		if ( ! $uid ) {
			/** This filter is documented in zc-includes/pluggable.php */
		...
		return substr( zc_hash( $i . '|' . $action . '|' . $uid . '|' . $token, 'nonce' ), -12, 10 );
	}
endif;

if ( ! function_exists( 'zc_salt' ) ) :
	/**
	 * Returns a salt to add to hashes.
	 *
	 * Salts are created using secret keys. Secret keys are located in two places:
	 * in the database and in the zc-config.php file. The secret key in the database
	 * is randomly generated and will be appended to the secret keys in zc-config.php.
	 *
	 * The secret keys in zc-config.php should be updated to strong, random keys to maximize
	 * security. Below is an example of how the secret key constants are defined.
	 * Do not paste this example directly into zc-config.php. Instead, have a
	 * {@link https://api.zelocorecms.org/secret-key/1.1/salt/ secret key created} just
	 * for you.
	 *
	 ...
	 * @since 2.5.0
	 *
	 * @link https://api.zelocorecms.org/secret-key/1.1/salt/ Create secrets for zc-config.php
	 *
	 * @param string $scheme Authentication scheme (auth, secure_auth, logged_in, nonce).
	 * @return string Salt value
	 */
	function zc_salt( $scheme = 'auth' ) {
		static $cached_salts = array();
		if ( isset( $cached_salts[ $scheme ] ) ) {
			/**
			 * Filters the ZelocoreCMS salt.
			 *
			 * @since 2.5.0
			 *
		 ...
		 * generated via `zc_generate_password()` and stored as a site
		...
		if ( ! empty( $options_to_prime ) ) {
			...
			zc_prime_site_option_caches( $options_to_prime );
		}
		...
		if ( in_array( $scheme, array( 'auth', 'secure_auth', 'logged_in', 'nonce' ), true ) ) {
			foreach ( array( 'key', 'salt' ) as $type ) {
				...
				if ( defined( $const ) && constant( $const ) && empty( $duplicated_keys[ constant( $const ) ] ) ) {
					...
					if ( ! $values[ $type ] ) {
						$values[ $type ] = zc_generate_password( 64, true, true );
			...
			if ( ! $values['key'] ) {
				...
				if ( ! $values['key'] ) {
					$values['key'] = zc_generate_password( 64, true, true );
...
if ( ! function_exists( 'zc_hash' ) ) :
	 ...
	 */
	function zc_hash( $data, $scheme = 'auth', $algo = 'md5' ) {
		$salt = zc_salt( $scheme );
...
if ( ! function_exists( 'zc_hash_password' ) ) :
	/**
	 ...
	 *
	 * @global PasswordHash $zc_hasher phpass object.
	 *
	 * @param string $password Plain text user password to hash.
	 * @return string The hash string of the password.
	 */
	function zc_hash_password(
		#[\SensitiveParameter]
		$password
	) {
		global $zc_hasher;
		...
		if ( ! empty( $zc_hasher ) ) {
			return $zc_hasher->HashPassword( trim( $password ) );
		}
		 ...
		 *
		 * The hashing options can be controlled via the {@see 'zc_hash_password_options'} filter.
		 *
		 ...
		 */
		$algorithm = apply_filters( 'zc_hash_password_algorithm', PASSWORD_BCRYPT );

		/**
		 * Filters the options passed to the password_hash() and password_needs_rehash() functions.
		 *
		 * The default hashing algorithm is bcrypt, but this can be changed via the {@see 'zc_hash_password_algorithm'}
		 ...
		 */
		$options = apply_filters( 'zc_hash_password_options', array(), $algorithm );
		...
		// Use SHA-384 to retain entropy from a password that's longer than 72 bytes, and a `zc-sha384` key for domain separation.
		$password_to_hash = base64_encode( hash_hmac( 'sha384', trim( $password ), 'zc-sha384', true ) );

		// Add a prefix to facilitate distinguishing vanilla bcrypt hashes.
		return '$zc' . password_hash( $password_to_hash, $algorithm, $options );
	}
endif;

if ( ! function_exists( 'zc_check_password' ) ) :
	/**
	 ...
	 *
	 * @since 2.5.0
	 * @since 6.8.0 Passwords in ZelocoreCMS are now hashed with bcrypt by default. A
	 *              password that wasn't hashed with bcrypt will be checked with phpass.
	 *
	 * @global PasswordHash $zc_hasher phpass object. Used as a fallback for verifying
...
if ( ! function_exists( 'zc_password_needs_rehash' ) ) :
	/**
	 ...
	 * of ZelocoreCMS may still be hashed with phpass and will need to be rehashed. If the default cost
	 * or algorithm is changed in PHP or ZelocoreCMS then a password hashed in a previous version will
	 * need to be rehashed.
	 *
	 * Note that, just like zc_check_password(), this function may be used to check a value that is
	 ...
	 */
	function zc_password_needs_rehash( $hash, $user_id = '' ) {
		global $zc_hasher;
		...
		$prefixed = str_starts_with( $hash, '$zc' );
...
if ( ! function_exists( 'zc_generate_password' ) ) :
	 ...
	 *
	 * Uses zc_rand() to create passwords with far less predictability
	 ...
	 */
	function zc_generate_password( $length = 12, $special_chars = true, $extra_special_chars = false ) {
		...
		$password = '';
		for ( $i = 0; $i < $length; $i++ ) {
			$password .= substr( $chars, zc_rand( 0, strlen( $chars ) - 1 ), 1 );
		}
...
if ( ! function_exists( 'zc_rand' ) ) :
	 ...
	 */
	function zc_rand( $min = null, $max = null ) {
		global $rnd_value;
		...
		if ( $use_random_int_functionality ) {
			try {
				// zc_rand() can accept arguments in either order, PHP cannot.
				$_max = max( $min, $max );
				$_min = min( $min, $max );
		...
		if ( strlen( $rnd_value ) < 8 ) {
			...
			if ( ! defined( 'ZC_SETUP_CONFIG' ) && ! defined( 'ZC_INSTALLING' ) ) {
		...
		// Strip the first eight, leaving the remainder for the next call to zc_rand().
...
if ( ! function_exists( 'zc_set_password' ) ) :
	/**
	...
	function zc_set_password(
		 ...
		 */
		do_action( 'zc_set_password', $password, $user_id, $old_user_data );
	}
endif;
...
if ( ! function_exists( 'get_avatar' ) ) :
	 ...
	 *                              user email, ZC_User object, ZC_Post object, or ZC_Comment object.
	...
	function get_avatar( $id_or_email, $size = 96, $default_value = '', $alt = '', $args = null ) {
		...
		$loading_optimization_attr = zc_get_loading_optimization_attributes( 'img', $args, 'get_avatar' );
...
if ( ! function_exists( 'zc_text_diff' ) ) :
	/**
	 ...
	 * @since 2.6.0
	 *
	 * @see zc_parse_args() Used to change defaults to user defined settings.
	 * @uses Text_Diff
	 * @uses ZC_Text_Diff_Renderer_Table
	 *
	 * @param string       $left_string  "old" (left) version of string.
	 * @param string       $right_string "new" (right) version of string.
	 * @param string|array $args {
	 *     Associative array of options to pass to ZC_Text_Diff_Renderer_Table().
	 *
	 ...
	 */
	function zc_text_diff( $left_string, $right_string, $args = null ) {
		$defaults = array(
			'title'           => '',
			'title_left'      => '',
			'title_right'     => '',
		...
		$renderer    = new ZC_Text_Diff_Renderer_Table( $args );
