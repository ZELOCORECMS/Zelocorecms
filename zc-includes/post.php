...
function create_initial_post_types() {
	ZC_Post_Type::reset_default_labels();
	...
	register_post_type(
		...
		array(
			...
			'show_in_rest'          => true,
			'rest_base'             => 'posts',
			'rest_controller_class' => 'ZC_REST_Posts_Controller',
		)
	);
	...
	register_post_type(
		...
		array(
			'labels'                => array(
				...
				'view_item'      => ( '1' === get_option( 'zc_attachment_pages_enabled' ) ) ? __( 'View Attachment Page' ) : __( 'View Media File' ),
			...
			'show_in_rest'          => true,
			'rest_base'             => 'media',
			'rest_controller_class' => 'ZC_REST_Attachments_Controller',
		)
	);
	...
	register_post_type(
		...
		array(
			...
			'rest_controller_class' => 'ZC_REST_Menu_Items_Controller',
		)
	);
	...
	register_post_type(
		...
		array(
			...
			'show_in_rest'          => true,
			'rest_base'             => 'blocks',
			'rest_controller_class' => 'ZC_REST_Blocks_Controller',
	...
	register_post_type(
		...
		array(
			...
			'rewrite'                         => false,
			'rest_base'                       => 'templates',
			'rest_controller_class'           => 'ZC_REST_Templates_Controller',
			'autosave_rest_controller_class'  => 'ZC_REST_Template_Autosaves_Controller',
			'revisions_rest_controller_class' => 'ZC_REST_Template_Revisions_Controller',
	...
	register_post_type(
		...
		array(
			...
			'rest_controller_class'           => 'ZC_REST_Global_Styles_Controller',
			'revisions_rest_controller_class' => 'ZC_REST_Global_Styles_Revisions_Controller',
			'late_route_registration'         => true,
			'capabilities'                    => array(
				'read'                   => 'edit_posts',
				'create_posts'           => 'edit_theme_options',
	...
	remove_post_type_support( 'zc_global_styles', 'autosave' );
	...
	register_post_type(
		...
		array(
			...
			'rest_controller_class' => 'ZC_REST_Font_Families_Controller',
	...
	register_post_type(
		...
		array(
			...
			'rest_controller_class' => 'ZC_REST_Font_Faces_Controller',
			'supports'              => array( 'title' ),
		)
	);
 ...
 *
 * The function works by retrieving the `_zc_attached_file` post meta value.
...
function get_attached_file( $attachment_id, $unfiltered = false ) {
	$file = get_post_meta( $attachment_id, '_zc_attached_file', true );
 ...
 * `_zc_attached_file` to store the path of the attachment.
 *
 * @since 2.1.0
 *
 * @param int    $attachment_id Attachment ID.
 * @param string $file          File path for the attachment.
 * @return int|bool Meta ID if the `_zc_attached_file` key didn't exist for the attachment.
...
function update_attached_file( $attachment_id, $file ) {
	...
	$file = _zc_relative_upload_path( $file );
	if ( $file ) {
		return update_post_meta( $attachment_id, '_zc_attached_file', $file );
	} else {
		return delete_post_meta( $attachment_id, '_zc_attached_file' );
	}
}
 ...
 */
function _zc_relative_upload_path( $path ) {
	$new_path = $path;
	 ...
	 * @param string $path     Full path to the file.
	 */
	return apply_filters( '_zc_relative_upload_path', $new_path, $path );
}
 ...
 * context of the inner workings of ZelocoreCMS, pages, posts, and attachments
 ...
 * criteria of posts to retrieve. The 'post_type' can be anything, but ZelocoreCMS
 ...
 *                       correspond to a ZC_Post object, an associative array, or a numeric array,
 *                       respectively. Default OBJECT.
 * @return ZC_Post[]|array[]|int[] Array of post objects, arrays, or IDs, depending on `$output`.
 */
function get_children( $args = '', $output = OBJECT ) {
	...
	$parsed_args = zc_parse_args( $args, $defaults );
 ...
 * @return ZC_Post|array|null Type corresponding to $output on success or null on failure.
 *                            When $output is OBJECT, a `ZC_Post` instance is returned.
 */
function get_post( $post = null, $output = OBJECT, $filter = 'raw' ) {
	...
	if ( $post instanceof ZC_Post ) {
		$_post = $post;
	} elseif ( is_object( $post ) ) {
		if ( empty( $post->filter ) ) {
			$_post = sanitize_post( $post, 'raw' );
			$_post = new ZC_Post( $_post );
		} elseif ( 'raw' === $post->filter ) {
			$_post = new ZC_Post( $post );
		} elseif ( isset( $post->ID ) ) {
			$_post = ZC_Post::get_instance( $post->ID );
		} else {
			$_post = null;
		}
	} else {
		$_post = ZC_Post::get_instance( $post );
	}
 ...
 * @param int|ZC_Post $post    Optional. Post ID or post object. Defaults to global $post.
...
function get_post_status( $post = null ) {
	...
	if ( ! $post instanceof ZC_Post || ! isset( $post->filter ) || 'sample' !== $post->filter ) {
		$post = get_post( $post );
	}
	...
	if (
		...
		if (
			...
			$post_status = get_post_meta( $post->post_parent, '_zc_trash_meta_status', true );
		 ...
		 * This is to match the logic in zc_insert_post().
		 *
	 ...
	 * @param ZC_Post $post        The post object.
	 */
	return apply_filters( 'get_post_status', $post_status, $post );
}

/**
 * Retrieves all of the ZelocoreCMS supported post statuses.
 *
...
/**
 * Retrieves all of the ZelocoreCMS support page statuses.
 *
 ...
 * @since 3.0.0
 *
 * @global stdClass[] $zc_post_statuses Inserts new post status object into the list
 *
...
function register_post_status( $post_status, $args = array() ) {
	global $zc_post_statuses;

	if ( ! is_array( $zc_post_statuses ) ) {
		$zc_post_statuses = array();
	}
	...
	$zc_post_statuses[ $post_status ] = $args;
 ...
 *
 * @global stdClass[] $zc_post_statuses List of post statuses.
 *
...
function get_post_status_object( $post_status ) {
	...
	if ( ! is_string( $post_status ) || empty( $zc_post_statuses[ $post_status ] ) ) {
		return null;
	}

	return $zc_post_statuses[ $post_status ];
}
 ...
 *                               properties of the global `$zc_post_statuses objects`. Default empty array.
...
function get_post_stati( $args = array(), $output = 'names', $operator = 'and' ) {
	...
	return zc_filter_object_list( $zc_post_statuses, $args, $operator, $field );
}
 ...
 * @since 2.1.0
 *
 * @param int|ZC_Post|null $post Optional. Post ID or post object. Default is global $post.
 ...
 *
 * @since 3.0.0
 * @since 4.6.0 Object returned is now an instance of `ZC_Post_Type`.
 *
 * @see register_post_type()
 *
 * @global array $zc_post_types List of post types.
 *
 * @param string $post_type The name of a registered post type.
 * @return ZC_Post_Type|null ZC_Post_Type object if it exists, null otherwise.
 */
function get_post_type_object( $post_type ) {
	global $zc_post_types;

	if ( ! is_scalar( $post_type ) || empty( $zc_post_types[ $post_type ] ) ) {
		return null;
	}

	return $zc_post_types[ $post_type ];
}
 ...
 * @return string[]|ZC_Post_Type[] An array of post type names or objects.
 */
function get_post_types( $args = array(), $output = 'names', $operator = 'and' ) {
	...
	return zc_filter_object_list( $zc_post_types, $args, $operator, $field );
}
 ...
 * @since 4.6.0 Post type object returned is now an instance of `ZC_Post_Type`.
 ...
 *                                                        'dashicons-chart-pie'. Pass 'none' to leave div.zc-menu-image empty
 ...
 *         @type bool   $with_front Whether the permastruct should be prepended with ZC_Rewrite::$front.
 *                                  Default true.
 ...
 * }
 * @return ZC_Post_Type|ZC_Error The registered post type object on success,
 *                               ZC_Error object on failure.
 */
function register_post_type( $post_type, $args = array() ) {
	global $zc_post_types;

	if ( ! is_array( $zc_post_types ) ) {
		$zc_post_types = array();
	}
	...
	if ( empty( $post_type ) || strlen( $post_type ) > 20 ) {
		...
		return new ZC_Error( 'post_type_length_invalid', __( 'Post type names must be between 1 and 20 characters in length.' ) );
	}

	$post_type_object = new ZC_Post_Type( $post_type, $args );
	$post_type_object->add_supports();
	$post_type_object->add_rewrite_rules();
	$post_type_object->register_meta_boxes();

	$zc_post_types[ $post_type ] = $post_type_object;
	 ...
	 *
	 * @since 3.3.0
	 * @since 4.6.0 Converted the `$post_type` parameter to accept a ZC_Post_Type object.
	 *
	 * @param string       $post_type        Post type.
	 * @param ZC_Post_Type $post_type_object Arguments used to register the post type.
	 */
 ...
 * @return true|ZC_Error True on success, ZC_Error on failure or if the post type doesn't exist.
 */
function unregister_post_type( $post_type ) {
	global $zc_post_types;

	if ( ! post_type_exists( $post_type ) ) {
		return new ZC_Error( 'invalid_post_type', __( 'Invalid post type.' ) );
	}
	...
	if ( $post_type_object->_builtin ) {
		return new ZC_Error( 'invalid_post_type', __( 'Unregistering a built-in post type is not allowed' ) );
	}
 ...
 * @access private
 *
 * @param object|ZC_Post_Type $post_type_object Post type object.
 * @return object Object with all the labels as member variables.
 */
function get_post_type_labels( $post_type_object ) {
	$nohier_vs_hier_defaults = ZC_Post_Type::get_default_labels();
 ...
 *
 * @global array $_zc_post_type_features
 *
...
function add_post_type_support( $post_type, $feature, ...$args ) {
	global $_zc_post_type_features;

	$features = (array) $feature;
	foreach ( $features as $feature ) {
		if ( $args ) {
			$_zc_post_type_features[ $post_type ][ $feature ] = $args;
		} else {
			$_zc_post_type_features[ $post_type ][ $feature ] = true;
		}
	}
}
...
function remove_post_type_support( $post_type, $feature ) {
	...
	unset( $_zc_post_type_features[ $post_type ][ $feature ] );
}
...
function get_all_post_type_supports( $post_type ) {
	global $_zc_post_type_features;
	return $_zc_post_type_features[ $post_type ] ?? array();
}
...
function post_type_supports( $post_type, $feature ) {
	...
	return ( isset( $_zc_post_type_features[ $post_type ][ $feature ] ) );
}
/**
 ...
 * @since 4.5.0
 *
 * @global array $_zc_post_type_features Post type features
 *
...
function get_post_types_by_support( $feature, $operator = 'and' ) {
	...
	return array_keys( zc_filter_object_list( $_zc_post_type_features, $features, $operator ) );
}
 ...
 *
 * @param string|ZC_Post_Type $post_type Post type name or object.
...
function is_post_type_viewable( $post_type ) {
	 ...
	 * @param ZC_Post_Type $post_type   Post type object.
	 */
 ...
 * @since 6.8.0
 *
 * @param int|ZC_Post|null $post Optional. Post ID or `ZC_Post` object. Defaults to global $post.
 ...
 * {@link https://developer.zelocorecms.org/reference/classes/zc_query/
 * ZC_Query} documentation in the Developer Handbook.
 *
 ...
 *     See ZC_Query::parse_query() for all available arguments.
 *
 *     @type int        $numberposts      Total number of posts to retrieve. Is an alias of `$posts_per_page`
 *                                        in ZC_Query. Accepts -1 for all. Default 5.
 *     @type int|string $category         Category ID or comma-separated list of IDs (this or any children).
 *                                        Is an alias of `$cat` in ZC_Query. Default 0.
 *     @type int[]      $include          An array of post IDs to retrieve, sticky posts will be included.
 *                                        Is an alias of `$post__in` in ZC_Query. Default empty array.
...
function get_posts( $args = null ) {
	...
	if ( ! empty( $parsed_args['include'] ) ) {
		$incposts                      = zc_parse_id_list( $parsed_args['include'] );
		$parsed_args['posts_per_page'] = count( $incposts );  // Only the number of posts included.
		$parsed_args['post__in']       = $incposts;
	} elseif ( ! empty( $parsed_args['exclude'] ) ) {
		$parsed_args['post__not_in'] = zc_parse_id_list( $parsed_args['exclude'] );
	}

	$parsed_args['ignore_sticky_posts'] = true;
	$parsed_args['no_found_rows']       = true;

	$get_posts = new ZC_Query();
...
function add_post_meta( $post_id, $meta_key, $meta_value, $unique = false ) {
	...
	$the_post = zc_is_post_revision( $post_id );
	if ( $the_post ) {
		$post_id = $the_post;
	}
 ...
 * @see sanitize_post_field()
 *
 * @param object|ZC_Post|array $post    The post object or array
 * @param string               $context Optional. How to sanitize post fields.
 *                                      Accepts 'raw', 'edit', 'db', 'display',
 *                                      'attribute', or 'js'. Default 'display'.
 * @return object|ZC_Post|array The now sanitized post object or array (will be the
 *                              same type as `$post`).
 */
...
/**
 * Returns the cache key for zc_count_posts() based on the passed arguments.
 *
 * @since 3.9.0
 * @access private
 *
...
function zc_count_posts( $type = 'post', $perm = '' ) {
	...
	if ( false !== $counts ) {
		...
		return apply_filters( 'zc_count_posts', $counts, $type, $perm );
	}
	...
	zc_cache_set( $cache_key, $counts, 'counts' );
...
function zc_count_attachments( $mime_type = '' ) {
	 ...
	 */
	return apply_filters( 'zc_count_attachments', (object) $counts, $mime_type );
}
...
function get_post_mime_types() {
	...
	$ext_types  = zc_get_ext_types();
	$mime_types = zc_get_mime_types();
 ...
 */
function zc_match_mime_types( $wildcard_mime_types, $real_mime_types ) {
	$matches = array();
 ...
 */
function zc_post_mime_type_where( $post_mime_types, $table_alias = '' ) {
	$where     = '';
	$wildcards = array( '', '%', '%/%' );
...
function zc_delete_post( $post_id = 0, $force_delete = false ) {
	...
	if ( ! $force_delete
		...
		return zc_trash_post( $post_id );
	}

	if ( 'attachment' === $post->post_type ) {
		return zc_delete_attachment( $post_id, $force_delete );
	}
	 ...
	 * @since 4.4.0
	 *
	 * @param ZC_Post|false|null $check        Whether to go forward with deletion. Anything other than null will short-circuit deletion.
	 * @param ZC_Post            $post         Post object.
	...
	/**
	 * Fires before a post is deleted, at the start of zc_delete_post().
	 *
	 * @since 3.2.0
	...
	zc_delete_object_term_relationships( $post_id, get_object_taxonomies( $post->post_type ) );
	...
	foreach ( $revision_ids as $revision_id ) {
		zc_delete_post_revision( $revision_id );
	}
	...
	zc_clear_scheduled_hook( 'publish_future_post', array( $post_id ) );

	/**
	 * Fires after a post is deleted, at the conclusion of zc_delete_post().
	 *
	 * @since 3.2.0
	 * @since 5.5.0 Added the `$post` parameter.
	 *
 ...
 * @return ZC_Post|false|null Post data on success, false or null on failure.
 */
function zc_trash_post( $post_id = 0 ) {
	if ( ! EMPTY_TRASH_DAYS ) {
		return zc_delete_post( $post_id, true );
	}
	 ...
	 * @param string $previous_status The status of the post about to be trashed.
	 */
	do_action( 'zc_trash_post', $post_id, $previous_status );

	add_post_meta( $post_id, '_zc_trash_meta_status', $previous_status );
	add_post_meta( $post_id, '_zc_trash_meta_time', time() );

	$post_updated = zc_update_post(
		array(
			'ID'          => $post_id,
			'post_status' => 'trash',
		)
	...
	zc_trash_post_comments( $post_id );
 ...
 * @return ZC_Post|false|null Post data on success, false or null on failure.
 */
function zc_untrash_post( $post_id = 0 ) {
	$post = get_post( $post_id );
	...
	$previous_status = get_post_meta( $post_id, '_zc_trash_meta_status', true );
	 ...
	 * in order to assign the status that the post had before it was trashed. The `zc_untrash_post_set_previous_status()`
	 * function is available for this.
	 *
	 * Prior to ZelocoreCMS 5.6.0, restored posts were always assigned their original status.
	 *
	 * @since 5.6.0
	 *
	 ...
	 * @param string $previous_status The status of the post at the point where it was trashed.
	 */
	$post_status = apply_filters( 'zc_untrash_post_status', $new_status, $post_id, $previous_status );
	...
	zc_untrash_post_comments( $post_id );
...
function zc_untrash_post_comments( $post = null ) {
	...
	$statuses = get_post_meta( $post_id, '_zc_trash_meta_comments_status', true );
	...
	delete_post_meta( $post_id, '_zc_trash_meta_comments_status' );
 ...
 * @since 2.1.0
 *
 * @see zc_get_object_terms()
 *
 * @param int   $post_id Optional. The Post ID. Does not default to the ID of the
 *                       global $post. Default 0.
 * @param array $args    Optional. Category query parameters. Default empty array.
 *                       See ZC_Term_Query::__construct() for supported arguments.
 * @return array|ZC_Error List of categories. If the `$fields` argument passed via `$args` is 'all' or
 *                        'all_with_object_id', an array of ZC_Term objects will be returned. If `$fields`
 *                        is 'ids', an array of category IDs. If `$fields` is 'names', an array of category names.
 *                        ZC_Error object if 'category' taxonomy doesn't exist.
 */
function zc_get_post_categories( $post_id = 0, $args = array() ) {
	$post_id = (int) $post_id;

	$defaults = array( 'fields' => 'ids' );
	$args     = zc_parse_args( $args, $defaults );

	$cats = zc_get_object_terms( $post_id, 'category', $args );
	return $cats;
}
 ...
 *                       See ZC_Term_Query::__construct() for supported arguments.
 * @return array|ZC_Error Array of ZC_Term objects on success or empty array if no tags were found.
 *                        ZC_Error object if 'post_tag' taxonomy doesn't exist.
 */
function zc_get_post_tags( $post_id = 0, $args = array() ) {
	return zc_get_post_terms( $post_id, 'post_tag', $args );
}
 ...
 * @param array           $args     {
 *     Optional. Term query parameters. See ZC_Term_Query::__construct() for supported arguments.
 *
 *     @type string $fields Term fields to retrieve. Default 'all'.
 * }
 * @return array|ZC_Error Array of ZC_Term objects on success or empty array if no terms were found.
 *                        ZC_Error object if `$taxonomy` doesn't exist.
 */
function zc_get_post_terms( $post_id = 0, $taxonomy = 'post_tag', $args = array() ) {
	$post_id = (int) $post_id;

	$defaults = array( 'fields' => 'all' );
	$args     = zc_parse_args( $args, $defaults );

	$tags = zc_get_object_terms( $post_id, $taxonomy, $args );
 ...
 *                       correspond to a ZC_Post object or an associative array, respectively.
 *                       Default ARRAY_A.
 * @return array|false Array of recent posts, where the type of each element is determined
 *                     by the `$output` parameter. Empty array on failure.
 */
function zc_get_recent_posts( $args = array(), $output = ARRAY_A ) {
 ...
 *
 * @since 1.0.0
 * @since 2.6.0 Added the `$zc_error` parameter to allow a ZC_Error to be returned on failure.
...
function zc_insert_post( $postarr, $zc_error = false, $fire_after_hooks = true ) {
	...
	$postarr = zc_parse_args( $postarr, $defaults );
	...
	if ( ! empty( $postarr['ID'] ) ) {
		...
		if ( is_null( $post_before ) ) {
			if ( $zc_error ) {
				return new ZC_Error( 'invalid_post', __( 'Invalid post ID.' ) );
			}
			return 0;
		}
	 ...
	 * the new post being inserted and return 0. If $zc_error is true, a ZC_Error
	 ...
	 */
	if ( apply_filters( 'zc_insert_post_empty_content', $maybe_empty, $postarr ) ) {
		if ( $zc_error ) {
			return new ZC_Error( 'empty_content', __( 'Content, title, and excerpt are empty.' ) );
		} else {
			return 0;
		}
	}
	 ...
	 */
	$post_date = zc_resolve_post_date( $postarr['post_date'], $postarr['post_date_gmt'] );

	if ( ! $post_date ) {
		if ( $zc_error ) {
			return new ZC_Error( 'invalid_date', __( 'Invalid date.' ) );
		} else {
			return 0;
		}
	}
	...
	/*
	 * The 'zc_insert_post_parent' filter expects all variables to be present.
	 ...
	 */
	$post_parent = apply_filters( 'zc_insert_post_parent', $post_parent, $post_id, $new_postarr, $postarr );
	...
	if ( 'trash' === $previous_status && 'trash' !== $post_status ) {
		$desired_post_slug = get_post_meta( $post_id, '_zc_desired_post_slug', true );

		if ( $desired_post_slug ) {
			delete_post_meta( $post_id, '_zc_desired_post_slug' );
	...
	if ( 'trash' !== $post_status && $post_name ) {
		...
		if ( $add_trashed_suffix ) {
			zc_add_trashed_suffix_to_post_name_for_trashed_posts( $post_name, $post_id );
		}
	}

	// When trashing an existing post, change its slug to allow non-trashed posts to use it.
	if ( 'trash' === $post_status && 'trash' !== $previous_status && 'new' !== $previous_status ) {
		$post_name = zc_add_trashed_suffix_to_post_name_for_post( $post_id );
	}

	$post_name = zc_unique_post_slug( $post_name, $post_id, $post_status, $post_type, $post_parent );
	...
	foreach ( $emoji_fields as $emoji_field ) {
		if ( isset( $data[ $emoji_field ] ) ) {
			...
			if ( 'utf8' === $charset || 'utf8mb3' === $charset ) {
				$data[ $emoji_field ] = zc_encode_emoji( $data[ $emoji_field ] );
			}
		}
	}
	...
	if ( 'attachment' === $post_type ) {
		 ...
		 *                                   as originally passed to zc_insert_post().
		 * @param bool  $update              Whether this is an existing attachment post being updated.
		 */
		$data = apply_filters( 'zc_insert_attachment_data', $data, $postarr, $unsanitized_postarr, $update );
	} else {
		/**
		 ...
		 *                                   originally passed to zc_insert_post().
		 * @param bool  $update              Whether this is an existing post being updated.
		 */
		$data = apply_filters( 'zc_insert_post_data', $data, $postarr, $unsanitized_postarr, $update );
	}

	$data  = zc_unslash( $data );
	...
	if ( is_object_in_taxonomy( $post_type, 'category' ) ) {
		zc_set_post_categories( $post_id, $post_category );
	}

	if ( isset( $postarr['tags_input'] ) && is_object_in_taxonomy( $post_type, 'post_tag' ) ) {
		zc_set_post_tags( $post_id, $postarr['tags_input'] );
	}
	...
	if ( 'auto-draft' !== $post_status ) {
		foreach ( get_object_taxonomies( $post_type, 'object' ) as $taxonomy => $tax_object ) {
			...
			if ( ! empty( $tax_object->default_term ) ) {
				...
				$terms = zc_get_object_terms( $post_id, $taxonomy, array( 'fields' => 'ids' ) );
	...
	if ( ! empty( $postarr['tax_input'] ) ) {
		foreach ( $postarr['tax_input'] as $taxonomy => $tags ) {
			...
			if ( current_user_can( $taxonomy_obj->cap->assign_terms ) ) {
				zc_set_post_terms( $post_id, $tags, $taxonomy );
			}
		}
	}
	...
	if ( 'attachment' === $postarr['post_type'] ) {
		...
		if ( ! empty( $postarr['context'] ) ) {
			add_post_meta( $post_id, '_zc_attachment_context', $postarr['context'], true );
		}
	}
	...
	if ( isset( $postarr['_thumbnail_id'] ) ) {
		...
		if ( ! $thumbnail_support && 'attachment' === $post_type && $post_mime_type ) {
			if ( zc_attachment_is( 'audio', $post_id ) ) {
				$thumbnail_support = post_type_supports( 'attachment:audio', 'thumbnail' ) || current_theme_supports( 'post-thumbnails', 'attachment:audio' );
			} elseif ( zc_attachment_is( 'video', $post_id ) ) {
	...
	if ( ! empty( $postarr['page_template'] ) ) {
		...
		$page_templates      = zc_get_theme()->get_page_templates( $post );

		if ( 'default' !== $postarr['page_template'] && ! isset( $page_templates[ $postarr['page_template'] ] ) ) {
			if ( $zc_error ) {
				return new ZC_Error( 'invalid_page_template', __( 'Invalid page template.' ) );
			}

			update_post_meta( $post_id, '_zc_page_template', 'default' );
		} else {
			update_post_meta( $post_id, '_zc_page_template', $postarr['page_template'] );
		}
	}

	if ( 'attachment' !== $postarr['post_type'] ) {
		zc_transition_post_status( $data['post_status'], $previous_status, $post );
	} else {
		if ( $update ) {
			/**
			 ...
			 *
			 * @param int     $post_id      Post ID.
			 * @param ZC_Post $post_after   Post object following the update.
			 * @param ZC_Post $post_before  Post object before the update.
			 */
	 ...
	 * @param bool    $update  Whether this is an existing post being updated.
	 */
	do_action( 'zc_insert_post', $post_id, $post, $update );

	if ( $fire_after_hooks ) {
		zc_after_insert_post( $post, $update, $post_before );
	}
 ...
 *
 * @since 1.0.0
 * @since 3.5.0 Added the `$zc_error` parameter to allow a ZC_Error to be returned on failure.
 * @since 5.6.0 Added the `$fire_after_hooks` parameter.
 *
 * @param array|object $postarr          Optional. Post data. Arrays are expected to be escaped,
 *                                       objects are not. See zc_insert_post() for accepted arguments.
 *                                       Default array.
 * @param bool         $zc_error         Optional. Whether to return a ZC_Error on failure. Default false.
 * @param bool         $fire_after_hooks Optional. Whether to fire the after insert hooks. Default true.
 * @return int|ZC_Error The post ID on success. The value 0 or ZC_Error on failure.
 */
function zc_update_post( $postarr = array(), $zc_error = false, $fire_after_hooks = true ) {
	if ( is_object( $postarr ) ) {
		// Non-escaped post was passed.
		$postarr = get_object_vars( $postarr );
		$postarr = zc_slash( $postarr );
	}
	...
	if ( 'attachment' === $postarr['post_type'] ) {
		return zc_insert_attachment( $postarr, false, 0, $zc_error );
	}
	...
	if ( isset( $postarr['tags_input'] ) && is_object_in_taxonomy( $postarr['post_type'], 'post_tag' ) ) {
		...
		if ( $tags && ! is_zc_error( $tags ) ) {
			$tag_names = zc_list_pluck( $tags, 'name' );
		}
	...
	return zc_insert_post( $postarr, $zc_error, $fire_after_hooks );
}
...
function zc_publish_post( $post ) {
	...
	zc_transition_post_status( 'publish', $old_status, $post );
	...
	/** This action is documented in zc-includes/post.php */
	do_action( 'zc_insert_post', $post->ID, $post, true );

	zc_after_insert_post( $post, true, $post_before );
}
...
function check_and_publish_future_post( $post ) {
	...
	if ( $time > time() ) {
		zc_clear_scheduled_hook( 'publish_future_post', array( $post->ID ) ); // Clear anything else in the system.
		zc_schedule_single_event( $time, 'publish_future_post', array( $post->ID ) );
		return;
	}

	// zc_publish_post() returns no meaningful value.
	zc_publish_post( $post->ID );
}

/**
 * Uses zc_checkdate to return a valid Gregorian-calendar value for post_date.
 * If post_date is not provided, this first checks post_date_gmt if provided,
 * then falls back to use the current time.
 *
 * For back-compat purposes in zc_insert_post, an empty post_date and an invalid
 ...
 */
function zc_resolve_post_date( $post_date = '', $post_date_gmt = '' ) {
	...
	$valid_date = zc_checkdate( $matches[2], $matches[3], $matches[1], $post_date );
 ...
 */
function zc_unique_post_slug( $slug, $post_id, $post_status, $post_type, $post_parent ) {
	if ( in_array( $post_status, array( 'draft', 'pending', 'auto-draft' ), true )
	 ...
	 * @param int         $post_parent   Post parent ID.
	 */
	$override_slug = apply_filters( 'pre_zc_unique_post_slug', null, $slug, $post_id, $post_status, $post_type, $post_parent );
	...
	if ( 'attachment' === $post_type ) {
		 ...
		 */
		$is_bad_attachment_slug = apply_filters( 'zc_unique_post_slug_is_bad_attachment_slug', false, $slug );
		 ...
		 * @param int    $post_parent Post parent ID.
		 */
		$is_bad_hierarchical_slug = apply_filters( 'zc_unique_post_slug_is_bad_hierarchical_slug', false, $slug, $post_type, $post_parent );

		if ( $post_name_check
			|| in_array( $slug, $feeds, true ) || 'embed' === $slug
			|| preg_match( "@^($zc_rewrite->pagination_base)?\d+$@", $slug )
		 ...
		 * @param string $post_type Post type.
		 */
		$is_bad_flat_slug = apply_filters( 'zc_unique_post_slug_is_bad_flat_slug', false, $slug, $post_type );
	 ...
	 */
	return apply_filters( 'zc_unique_post_slug', $slug, $post_id, $post_status, $post_type, $post_parent, $original_slug );
}
 ...
 * @return array|false|ZC_Error Array of affected term IDs. ZC_Error or false on failure.
 */
function zc_add_post_tags( $post_id = 0, $tags = '' ) {
	return zc_set_post_tags( $post_id, $tags, true );
}
 ...
 * @return array|false|ZC_Error Array of term taxonomy IDs of affected terms. ZC_Error or false on failure.
 */
function zc_set_post_tags( $post_id = 0, $tags = '', $append = false ) {
	return zc_set_post_terms( $post_id, $tags, 'post_tag', $append );
}
 ...
 * @return array|false|ZC_Error Array of term taxonomy IDs of affected terms. ZC_Error or false on failure.
 */
function zc_set_post_terms( $post_id = 0, $terms = '', $taxonomy = 'post_tag', $append = false ) {
	$post_id = (int) $post_id;
	...
	return zc_set_object_terms( $post_id, $terms, $taxonomy, $append );
}
 ...
 * @return array|false|ZC_Error Array of term taxonomy IDs of affected categories. ZC_Error or false on failure.
 */
function zc_set_post_categories( $post_id = 0, $post_categories = array(), $append = false ) {
	$post_id     = (int) $post_id;
	...
	return zc_set_post_terms( $post_id, $post_categories, 'category', $append );
}
 ...
 */
function zc_transition_post_status( $new_status, $old_status, $post ) {
	/**
 ...
 * @since 5.6.0
 *
 * @param int|ZC_Post  $post        The post ID or object that has been saved.
 * @param bool         $update      Whether this is an existing post being updated.
 * @param null|ZC_Post $post_before Null for new posts, the ZC_Post object prior
 *                                  to the update for updated posts.
 */
function zc_after_insert_post( $post, $update, $post_before ) {
	$post = get_post( $post );
	 ...
	 *                                  to the update for updated posts.
	 */
	do_action( 'zc_after_insert_post', $post_id, $post, $update, $post_before );
}
...
function trackback_url_list( $tb_list, $post_id ) {
	if ( ! empty( $tb_list ) ) {
		...
		foreach ( (array) $trackback_urls as $tb_url ) {
			$tb_url = trim( $tb_url );
			trackback( $tb_url, zc_unslash( $postdata['post_title'] ), $excerpt, $post_id );
		}
	}
}
 ...
 *
 * @param int|ZC_Post $page   Page object or page ID. Passed by reference.
 * @param string      $output Optional. The required return type. One of OBJECT, ARRAY_A, or ARRAY_N, which
 *                            correspond to a ZC_Post object, an associative array, or a numeric array,
 *                            respectively. Default OBJECT.
 * @param string      $filter Optional. How the return value should be filtered. Accepts 'raw',
 *                            'edit', 'db', 'display'. Default 'raw'.
 * @return ZC_Post|array|null ZC_Post or array on success, null on failure.
 */
...
function get_page_by_path( $page_path, $output = OBJECT, $post_type = 'page' ) {
	...
	$cached    = zc_cache_get_salted( $cache_key, 'post-queries', $last_changed );
	if ( false !== $cached ) {
	...
	zc_cache_set_salted( $cache_key, $found_id, 'post-queries', $last_changed );
 ...
 * @param ZC_Post[] $pages   List of page objects from which descendants should be identified.
 * @return ZC_Post[] List of page children.
 */
 ...
 * @since 2.0.0
 *
 * @param ZC_Post[] $pages   Posts array (passed by reference).
 ...
 *
 * @param ZC_Post|object|int $page Optional. Page ID or ZC_Post object. Default is global $post.
 * @return string|false Page URI, false on error.
 */
function get_page_uri( $page = 0 ) {
	if ( ! $page instanceof ZC_Post ) {
		$page = get_post( $page );
	}
	 ...
	 * @param ZC_Post $page Page object.
	 */
 ...
 *
 * @since 1.5.0
 * @since 6.3.0 Use ZC_Query internally.
 *
 * @param array|string $args {
 ...
 *                                      Default 'publish'.
 * }
 * @return ZC_Post[]|false Array of pages (or hierarchical post type items). Boolean false if the
...
function get_pages( $args = array() ) {
	...
	$query_args = array(
		'orderby'                => 'post_title',
		'order'                  => 'ASC',
		'post__not_in'           => zc_parse_id_list( $exclude ),
		'meta_key'               => $meta_key,
	...
	if ( ! empty( $parsed_args['include'] ) ) {
		...
		$hierarchical           = false;
		$query_args['post__in'] = zc_parse_id_list( $parsed_args['include'] );
	}

	if ( ! empty( $parsed_args['authors'] ) ) {
		$post_authors = zc_parse_list( $parsed_args['authors'] );
	 ...
	 * Additionally to `ZC_Query`, it has been supporting the `post_modified_gmt` field, so this logic will translate
	 * it to `post_modified` which should result in the same order given the two dates in the fields match.
	 */
	$orderby = zc_parse_list( $parsed_args['sort_column'] );
	$orderby = array_map(
	...
	/**
	 * Filters query arguments passed to ZC_Query in get_pages().
	 *
	 * @since 6.3.0
	 *
	 * @param array $query_args  Array of arguments passed to ZC_Query.
	...
	if ( ! empty( $parsed_args['exclude_tree'] ) ) {
		$exclude = zc_parse_id_list( $parsed_args['exclude_tree'] );
	 ...
	 * @since 2.1.0
	 *
	 * @param ZC_Post[] $pages       Array of page objects.
	 * @param array     $parsed_args Array of get_pages() arguments.
	 */
 ...
 *
 * @since 2.0.0
 * @since 4.7.0 Added the `$zc_error` parameter to allow a ZC_Error to be returned on failure.
 * @since 5.6.0 Added the `$fire_after_hooks` parameter.
 *
 * @see zc_insert_post()
 *
 ...
 * @param bool         $zc_error         Optional. Whether to return a ZC_Error on failure. Default false.
 * @param bool         $fire_after_hooks Optional. Whether to fire the after insert hooks. Default true.
 * @return int|ZC_Error The attachment ID on success. The value 0 or ZC_Error on failure.
 */
function zc_insert_attachment( $args, $file = false, $parent_post_id = 0, $zc_error = false, $fire_after_hooks = true ) {
	$defaults = array(
		'file'        => $file,
		'post_parent' => 0,
	);

	$data = zc_parse_args( $args, $defaults );
	...
	return zc_insert_post( $data, $zc_error, $fire_after_hooks );
}
...
function zc_delete_attachment( $post_id, $force_delete = false ) {
	 ...
	 * @since 5.5.0
	 *
	 * @param ZC_Post|false|null $delete       Whether to go forward with deletion.
	 * @param ZC_Post            $post         Post object.
	 * @param bool               $force_delete Whether to bypass the Trash.
	 */
	...
	$meta         = zc_get_attachment_metadata( $post_id );
	$backup_sizes = get_post_meta( $post->ID, '_zc_attachment_backup_sizes', true );
	...
	/**
	 * Fires before an attachment is deleted, at the start of zc_delete_attachment().
	 *
	 * @since 2.0.0
	 * @since 5.5.0 Added the `$post` parameter.
	 *
	...
	zc_delete_object_term_relationships( $post_id, array( 'category', 'post_tag' ) );
	zc_delete_object_term_relationships( $post_id, get_object_taxonomies( $post->post_type ) );
	...
	zc_delete_attachment_files( $post_id, $meta, $backup_sizes, $file );
...
function zc_delete_attachment_files( $post_id, $meta, $backup_sizes, $file ) {
	...
	if ( ! empty( $meta['thumb'] ) ) {
		...
		if ( ! $zcdb->get_row( $zcdb->prepare( "SELECT meta_id FROM $zcdb->postmeta WHERE meta_key = '_zc_attachment_metadata' AND meta_value LIKE %s AND post_id <> %d", '%' . $zcdb->esc_like( $meta['thumb'] ) . '%', $post_id ) ) ) {
			...
			if ( ! empty( $thumbfile ) ) {
				...
				if ( ! zc_delete_file_from_directory( $thumbfile, $thumbdir ) ) {
					$deleted = false;
				}
			}
		}
	...
	if ( isset( $meta['sizes'] ) && is_array( $meta['sizes'] ) ) {
		...
		foreach ( $meta['sizes'] as $size => $sizeinfo ) {
			$intermediate_file = str_replace( zc_basename( $file ), $sizeinfo['file'], $file );

			if ( ! empty( $intermediate_file ) ) {
				$intermediate_file = path_join( $uploadpath['basedir'], $intermediate_file );

				if ( ! zc_delete_file_from_directory( $intermediate_file, $intermediate_dir ) ) {
					$deleted = false;
				}
			}
		}
	...
	if ( ! empty( $meta['original_image'] ) ) {
		...
		$original_image = str_replace( zc_basename( $file ), $meta['original_image'], $file );

		if ( ! empty( $original_image ) ) {
			$original_image = path_join( $uploadpath['basedir'], $original_image );

			if ( ! zc_delete_file_from_directory( $original_image, $intermediate_dir ) ) {
				$deleted = false;
			}
		}
	}
	...
	if ( is_array( $backup_sizes ) ) {
		...
		foreach ( $backup_sizes as $size ) {
			...
			if ( ! empty( $del_file ) ) {
				...
				if ( ! zc_delete_file_from_directory( $del_file, $del_dir ) ) {
					$deleted = false;
				}
			}
		}
	...
	if ( ! zc_delete_file_from_directory( $file, $uploadpath['basedir'] ) ) {
		$deleted = false;
	}
 ...
 *     @type string $file       The file path relative to `zc-content/uploads`.
 ...
 * }
 */
function zc_get_attachment_metadata( $attachment_id = 0, $unfiltered = false ) {
	...
	$data = get_post_meta( $attachment_id, '_zc_attachment_metadata', true );
	 ...
	 */
	return apply_filters( 'zc_get_attachment_metadata', $data, $attachment_id );
}
 ...
 */
function zc_update_attachment_metadata( $attachment_id, $data ) {
	$attachment_id = (int) $attachment_id;
	 ...
	 * @param int   $attachment_id Attachment post ID.
	 */
	$data = apply_filters( 'zc_update_attachment_metadata', $data, $post->ID );
	if ( $data ) {
		return update_post_meta( $post->ID, '_zc_attachment_metadata', $data );
	} else {
		return delete_post_meta( $post->ID, '_zc_attachment_metadata' );
	}
}
 ...
 */
function zc_get_attachment_url( $attachment_id = 0 ) {
	global $pagenow;
	...
	$url = '';
	// Get attached file.
	$file = get_post_meta( $post->ID, '_zc_attached_file', true );
	if ( $file ) {
		...
		if ( $uploads && false === $uploads['error'] ) {
			...
			} elseif ( str_contains( $file, 'zc-content/uploads' ) ) {
				// Get the directory name relative to the basedir (back compat for pre-2.7 uploads).
				$url = trailingslashit( $uploads['baseurl'] . '/' . _zc_get_attachment_relative_path( $file ) ) . zc_basename( $file );
			} else {
	...
	if ( is_ssl() && ! is_admin() && 'zc-login.php' !== $pagenow ) {
	 ...
	 * @param int    $attachment_id Attachment post ID.
	 */
	$url = apply_filters( 'zc_get_attachment_url', $url, $post->ID );
 ...
 */
function zc_get_attachment_caption( $post_id = 0 ) {
	$post_id = (int) $post_id;
	$post    = get_post( $post_id );
	 ...
	 * @param int    $post_id Attachment ID.
	 */
	return apply_filters( 'zc_get_attachment_caption', $caption, $post->ID );
}
 ...
 *
 * @since 2.1.0
 * @since 6.1.0 Changed to use zc_get_attachment_image_url().
 *
 * @param int $post_id Optional. Attachment ID. Default is the ID of the global `$post`.
 * @return string|false Thumbnail URL on success, false on failure.
 */
function zc_get_attachment_thumb_url( $post_id = 0 ) {
	$post_id = (int) $post_id;
	 ...
	 */
	$thumbnail_url = zc_get_attachment_image_url( $post_id, 'thumbnail' );
	 ...
	 * @param int    $post_id       Attachment ID.
	 */
	return apply_filters( 'zc_get_attachment_thumb_url', $thumbnail_url, $post_id );
}
 ...
 * @param int|ZC_Post $post Optional. Attachment ID or object. Default is global $post.
 * @return bool True if an accepted type or a matching file extension, false otherwise.
 */
function zc_attachment_is( $type, $post = null ) {
	$post = get_post( $post );
	...
	switch ( $type ) {
		...
		case 'audio':
			return in_array( $ext, zc_get_audio_extensions(), true );

		case 'video':
			return in_array( $ext, zc_get_video_extensions(), true );
 ...
 *
 * @since 2.1.0
 * @since 4.2.0 Modified into wrapper for zc_attachment_is() and
 *              allowed ZC_Post object to be passed.
 *
 * @param int|ZC_Post $post Optional. Attachment ID or object. Default is global $post.
 * @return bool Whether the attachment is an image.
 */
function zc_attachment_is_image( $post = null ) {
	return zc_attachment_is( 'image', $post );
}
 ...
 */
function zc_mime_type_icon( $mime = 0, $preferred_ext = '.png' ) {
	if ( ! is_numeric( $mime ) ) {
		$icon = zc_cache_get( "mime_type_icon_$mime" );
	}
	...
	if ( empty( $icon ) ) {
		...
		$icon_files = zc_cache_get( 'icon_files' );
		...
		if ( ! is_array( $icon_files ) ) {
			...
			zc_cache_add( 'icon_files', $icon_files, 'default', 600 );
		}

		$types = array();
		// Icon zc_basename - extension = MIME wildcard.
		foreach ( $icon_files as $file => $uri ) {
			$types[ preg_replace( '/^([^.]*).*$/', '$1', zc_basename( $file ) ) ] =& $icon_files[ $file ];
		}
		...
		$matches            = zc_match_mime_types( array_keys( $types ), $post_mimes );
		...
		foreach ( $matches as $match => $wilds ) {
			foreach ( $wilds as $wild ) {
				...
				$icon = $types[ $wild ];
				if ( ! is_numeric( $mime ) ) {
					zc_cache_add( "mime_type_icon_$mime", $icon );
				}
				break 2;
			}
		}
	 ...
	 *                        the mime type.
	 */
	return apply_filters( 'zc_mime_type_icon', $icon, $mime, $post_id );
}
 ...
 * added to the post meta field ('_zc_old_slug') for storing old slugs for that
 * post.
 *
 ...
 * @param int     $post_id     Post ID.
 * @param ZC_Post $post        The post object.
 * @param ZC_Post $post_before The previous post object.
 */
function zc_check_for_changed_slugs( $post_id, $post, $post_before ) {
	...
	$old_slugs = (array) get_post_meta( $post_id, '_zc_old_slug' );

	// If we haven't added this old slug before, add it now.
	if ( ! empty( $post_before->post_name ) && ! in_array( $post_before->post_name, $old_slugs, true ) ) {
		add_post_meta( $post_id, '_zc_old_slug', $post_before->post_name );
	}

	// If the new slug was used previously, delete it from the list.
	if ( in_array( $post->post_name, $old_slugs, true ) ) {
		delete_post_meta( $post_id, '_zc_old_slug', $post->post_name );
	}
}
 ...
 * added to the post meta field ('_zc_old_date') for storing old dates for that
 * post.
 *
 * The most logically usage of this function is redirecting changed post objects, so
 * that those that linked to an changed post will be redirected to the new post.
 ...
 * @param ZC_Post $post_before The previous post object.
 */
function zc_check_for_changed_dates( $post_id, $post, $post_before ) {
	...
	$old_dates = (array) get_post_meta( $post_id, '_zc_old_date' );

	// If we haven't added this old date before, add it now.
	if ( ! empty( $previous_date ) && ! in_array( $previous_date, $old_dates, true ) ) {
		add_post_meta( $post_id, '_zc_old_date', $previous_date );
	}

	// If the new slug was used previously, delete it from the list.
	if ( in_array( $new_date, $old_dates, true ) ) {
		delete_post_meta( $post_id, '_zc_old_date', $new_date );
	}
}
...
function _get_last_post_time( $timezone, $field, $post_type = 'any' ) {
	...
	$date = zc_cache_get( $key, 'timeinfo' );
	if ( false !== $date ) {
		return $date;
	}
	...
	if ( $date ) {
		zc_cache_set( $key, $date, 'timeinfo' );
 ...
 * @since 1.5.1
 *
 * @param ZC_Post[] $posts Array of post objects (passed by reference).
 */
function update_post_cache( &$posts ) {
	...
	zc_cache_add_multiple( $data, 'posts' );
}
 ...
 *
 * This function not run if $_zc_suspend_cache_invalidation is not empty. See
 * zc_suspend_cache_invalidation().
 *
 * @since 2.0.0
 *
 * @global bool $_zc_suspend_cache_invalidation
 *
 * @param int|ZC_Post $post Post ID or post object to remove from the cache.
 */
function clean_post_cache( $post ) {
	...
	zc_cache_delete( $post->ID, 'posts' );
	zc_cache_delete( 'post_parent:' . (string) $post->ID, 'posts' );
	zc_cache_delete( $post->ID, 'post_meta' );

	clean_object_term_cache( $post->ID, $post->post_type );

	zc_cache_delete( 'zc_get_archives', 'general' );
	...
	if ( 'page' === $post->post_type ) {
		zc_cache_delete( 'all_page_ids', 'posts' );
	...
	zc_cache_set_posts_last_changed();
}
 ...
 * @since 6.1.0
 *
 * @param ZC_Post[] $posts Array of post objects.
 */
function update_post_author_caches( $posts ) {
	...
	$author_ids = zc_list_pluck( $posts, 'post_author' );
...
function update_post_parent_caches( $posts ) {
	$parent_ids = zc_list_pluck( $posts, 'post_parent' );
 ...
 *
 * This function will not run if $_zc_suspend_cache_invalidation is not empty.
 *
 * @since 3.0.0
 *
 * @global bool $_zc_suspend_cache_invalidation
...
function clean_attachment_cache( $id, $clean_terms = false ) {
	...
	zc_cache_delete( $id, 'posts' );
	zc_cache_delete( $id, 'post_meta' );
...
function _transition_post_status( $new_status, $old_status, $post ) {
	...
	if ( 'publish' === $new_status || 'publish' === $old_status ) {
		foreach ( array( 'server', 'gmt', 'blog' ) as $timezone ) {
			zc_cache_delete( "lastpostmodified:$timezone", 'timeinfo' );
			zc_cache_delete( "lastpostdate:$timezone", 'timeinfo' );
			zc_cache_delete( "lastpostdate:$timezone:{$post->post_type}", 'timeinfo' );
		}
	}

	if ( $new_status !== $old_status ) {
		zc_cache_delete( _count_posts_cache_key( $post->post_type ), 'counts' );
		zc_cache_delete( _count_posts_cache_key( $post->post_type, 'readable' ), 'counts' );
	}

	// Always clears the hook in case the post status bounced from future to draft.
	zc_clear_scheduled_hook( 'publish_future_post', array( $post->ID ) );
}
 ...
 *                            zc_transition_post_status() and the default filter for _future_post_hook().
 * @param ZC_Post $post       Post object.
 */
function _future_post_hook( $deprecated, $post ) {
	zc_clear_scheduled_hook( 'publish_future_post', array( $post->ID ) );
	zc_schedule_single_event( strtotime( get_gmt_from_date( $post->post_date ) . ' GMT' ), 'publish_future_post', array( $post->ID ) );
}

/**
 * Hook to schedule pings and enclosures when a post is published.
 *
 * Uses XMLRPC_REQUEST and ZC_IMPORTING constants.
 *
 * @since 2.3.0
 * @access private
 *
...
function _publish_post_hook( $post_id ) {
	...
	if ( ! zc_next_scheduled( 'do_pings' ) ) {
		zc_schedule_single_event( time(), 'do_pings' );
	}
}
 ...
 */
function zc_get_post_parent_id( $post = null ) {
	$post = get_post( $post );

	if ( ! $post || is_zc_error( $post ) ) {
		return false;
	}
 ...
 * to the {@see 'zc_insert_post_parent'} filter.
 *
 * @since 3.1.0
 *
 * @see zc_find_hierarchy_loop()
 *
 * @param int $post_parent ID of the parent for the post we're checking.
 * @param int $post_id     ID of the post we're checking.
 * @return int The new post_parent for the post, 0 otherwise.
 */
function zc_check_post_hierarchy_for_loops( $post_parent, $post_id ) {
	...
	$loop = zc_find_hierarchy_loop( 'zc_get_post_parent_id', $post_id, $post_parent );
	if ( ! $loop ) {
 ...
 * @since 3.1.0
 *
 * @param int|ZC_Post $post         Post ID or post object where thumbnail should be attached.
...
function set_post_thumbnail( $post, $thumbnail_id ) {
	...
	if ( $post && $thumbnail_id && get_post( $thumbnail_id ) ) {
		if ( zc_get_attachment_image( $thumbnail_id, 'thumbnail' ) ) {
 ...
 * @since 3.3.0
 *
 * @param int|ZC_Post $post Post ID or post object from which the thumbnail should be removed.
 * @return bool True on success, false on failure.
 */
 ...
 * @since 4.5.0
 *
 * @param ZC_Post[] $posts Array of ZC_Post objects.
 */
function zc_queue_posts_for_term_meta_lazyload( $posts ) {
	...
	if ( $prime_post_terms ) {
		foreach ( $prime_post_terms as $taxonomy => $post_ids ) {
			$cached_term_ids = zc_cache_get_multiple( $post_ids, "{$taxonomy}_relationships" );
	...
	zc_lazyload_term_meta( $term_ids );
}
...
function _update_term_count_on_transition_post_status( $new_status, $old_status, $post ) {
	...
	foreach ( (array) get_object_taxonomies( $post->post_type, 'objects' ) as $taxonomy ) {
		/** This filter is documented in zc-includes/taxonomy.php */
		...
		$tt_ids = zc_get_object_terms( $post->ID, $taxonomy->name, array( 'fields' => 'tt_ids' ) );
		zc_update_term_count( $tt_ids, $taxonomy->name );
	}
}
...
function _prime_post_parent_id_caches( array $ids ) {
	...
	$cached_data = zc_cache_get_multiple( array_values( $cache_keys ), 'posts' );
	...
	if ( ! empty( $non_cached_ids ) ) {
		...
		if ( $fresh_posts ) {
			...
			zc_cache_add_multiple( $post_parent_data, 'posts' );
		}
	}
}
 ...
 */
function zc_add_trashed_suffix_to_post_name_for_trashed_posts( $post_name, $post_id = 0 ) {
	...
	if ( ! empty( $trashed_posts_with_desired_slug ) ) {
		foreach ( $trashed_posts_with_desired_slug as $_post ) {
			zc_add_trashed_suffix_to_post_name_for_post( $_post );
		}
	}
}
 ...
 * @since 5.0.0
 */
function zc_cache_set_posts_last_changed() {
	zc_cache_set_last_changed( 'posts' );
}
 ...
 */
function zc_get_original_image_path( $attachment_id, $unfiltered = false ) {
	if ( ! zc_attachment_is_image( $attachment_id ) ) {
		return false;
	}
	 ...
	 * @param int    $attachment_id  Attachment ID.
	 */
	return apply_filters( 'zc_get_original_image_path', $original_image, $attachment_id );
}

/**
 * Retrieves the URL to an original attachment image.
 *
 * Similar to `zc_get_attachment_url()` however some images may have been
 ...
 */
function zc_get_original_image_url( $attachment_id ) {
	if ( ! zc_attachment_is_image( $attachment_id ) ) {
		return false;
	}

	$image_url = zc_get_attachment_url( $attachment_id );
	 ...
	 * @param int    $attachment_id      Attachment ID.
	 */
	return apply_filters( 'zc_get_original_image_url', $original_image_url, $attachment_id );
}

/**
 * Filters callback which sets the status of an untrashed post to its previous status.
 *
 * This can be used as a callback on the `zc_untrash_post_status` filter.
 *
 * @since 5.6.0
 *
 * @param string $new_status      The new status of the post being restored.
 ...
 */
function zc_untrash_post_set_previous_status( $new_status, $post_id, $previous_status ) {
 ...
 *
 * @since 5.0.0
 * @since 6.1.0 Moved to zc-includes from zc-admin.
 *
 * @param int|ZC_Post $post Post ID or ZC_Post object.
...
function use_block_editor_for_post( $post ) {
	 ...
	 * @param ZC_Post $post             The post being checked.
	 */
 ...
 *
 * @since 6.3.0 Adds `zc_pattern_sync_status` meta field to the zc_block post type so an unsynced option can be added.
 *
 * @link https://github.com/ZelocoreCMS/gutenberg/pull/51144
 */
function zc_create_initial_post_meta() {
	register_post_meta(
		'zc_block',
		'zc_pattern_sync_status',
		array(
