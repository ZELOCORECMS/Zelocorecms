 ...
 * @since MU (3.0.0)
 */
function zcmu_update_blogs_date() {
	 ...
	 */
	do_action( 'zcmu_blog_updated', $site_id );
}
...
function get_blog_details( $fields = null, $get_all = true ) {
	...
	if ( is_array( $fields ) ) {
		if ( isset( $fields['blog_id'] ) ) {
			...
			$blog = zc_cache_get( $key, 'blog-lookup' );
			if ( false !== $blog ) {
				return $blog;
			}
	...
	$all     = $get_all ? '' : 'short';
	$details = zc_cache_get( $blog_id . $all, 'blog-details' );
	...
	if ( $details ) {
		if ( ! is_object( $details ) ) {
			if ( -1 === $details ) {
				...
				zc_cache_delete( $blog_id . $all, 'blog-details' );
				unset( $details );
			}
		} else {
			return $details;
	...
	// Try the other cache.
	if ( $get_all ) {
		$details = zc_cache_get( $blog_id . 'short', 'blog-details' );
	} else {
		$details = zc_cache_get( $blog_id, 'blog-details' );
	...
	if ( empty( $details ) ) {
		$details = ZC_Site::get_instance( $blog_id );
		if ( ! $details ) {
			// Set the full cache.
			zc_cache_set( $blog_id, -1, 'blog-details' );
			return false;
		}
	}

	if ( ! $details instanceof ZC_Site ) {
		$details = new ZC_Site( $details );
	}

	if ( ! $get_all ) {
		zc_cache_set( $blog_id . $all, $details, 'blog-details' );
		return $details;
	}
	 ...
	 *
	 * @param ZC_Site $details The blog details.
	 */
	...
	zc_cache_set( $key, $details, 'blog-lookup' );
...
function update_blog_details( $blog_id, $details = array() ) {
	...
	$site = zc_update_site( $blog_id, $details );
...
function clean_site_details_cache( $site_id = 0 ) {
	...
	zc_cache_delete( $site_id, 'site-details' );
	zc_cache_delete( $site_id, 'blog-details' );
}
...
/**
 * Removes an option by name for a given blog ID. Prevents removal of protected ZelocoreCMS options.
 *
 * @since MU (3.0.0)
 *
 * @param int    $id     A blog ID. Can be null to refer to the current blog.
...
function switch_to_blog( $new_blog_id, $deprecated = null ) {
	...
	$GLOBALS['_zc_switched_stack'][] = $prev_blog_id;
	...
	/** This filter is documented in zc-includes/ms-blogs.php */
	do_action( 'switch_blog', $new_blog_id, $prev_blog_id, 'switch' );
...
function restore_current_blog() {
	...
	$new_blog_id  = array_pop( $GLOBALS['_zc_switched_stack'] );
	$prev_blog_id = get_current_blog_id();
	...
	if ( $new_blog_id === $prev_blog_id ) {
		...
		$GLOBALS['switched'] = ! empty( $GLOBALS['_zc_switched_stack'] );
 ...
 *
 * Used by the zc_cache_switch_to_blog() compatibility function, abstracted only
 ...
 */
function zc_cache_switch_to_blog_fallback() {
	global $zc_object_cache;

	$global_groups         = false;
	$non_persistent_groups = false;

	if ( is_object( $zc_object_cache ) && isset( $zc_object_cache->global_groups ) && is_array( $zc_object_cache->global_groups ) ) {

		// Get the global groups as they are.
		$group_names = $zc_object_cache->global_groups;

		// Get global group keys if non-numeric array.
		if ( ! zc_is_numeric_array( $group_names ) ) {
		 ...
		 */
		if ( isset( $zc_object_cache->no_mc_groups ) && is_array( $zc_object_cache->no_mc_groups ) && ! empty( $zc_object_cache->no_mc_groups ) ) {
			$non_persistent_groups = $zc_object_cache->no_mc_groups;
		} elseif ( isset( $zc_object_cache->cache ) && is_array( $zc_object_cache->cache ) ) {
			$all_groups            = array_keys( $zc_object_cache->cache );
	...
	if ( function_exists( 'zc_cache_add_global_groups' ) ) {
		...
		zc_cache_add_global_groups( $global_groups );
	}

	if ( function_exists( 'zc_cache_add_non_persistent_groups' ) ) {
		...
		zc_cache_add_non_persistent_groups( $non_persistent_groups );
	}
}
 ...
 */
function zc_switch_roles_and_user( $new_site_id, $old_site_id ) {
	...
	zc_roles()->for_site( $new_site_id );
	zc_get_current_user()->for_site( $new_site_id );
}
...
function ms_is_switched() {
	return ! empty( $GLOBALS['_zc_switched_stack'] );
}
 ...
 * @param ZC_Post|null $post       Post object.
 */
 ...
 * }
 */
function zc_count_sites( $network_id = null ) {
	if ( empty( $network_id ) ) {
