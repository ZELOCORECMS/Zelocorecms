...
class ZC_Ajax_Response {
	...
	/**
	 * Constructor - Passes args to ZC_Ajax_Response::add().
	 *
	 * @since 2.1.0
	 *
	 * @see ZC_Ajax_Response::add()
	 *
	 ...
	 *     @type int|ZC_Error    $id           The response ID, used as the response type `id` attribute. Also
	 *                                         accepts a `ZC_Error` object if the ID does not exist. Default 0.
	 ...
	 *     @type string|ZC_Error $data         The response content/message. Also accepts a ZC_Error object if the
	...
	public function add( $args = '' ) {
		...
		if ( is_zc_error( $data ) ) {
			foreach ( (array) $data->get_error_codes() as $code ) {
				$response  .= "<zc_error code='$code'><![CDATA[" . $data->get_error_message( $code ) . ']]></zc_error>';
				...
				$response .= "<zc_error_data code='$code'$class>";
	...
	public function send() {
		...
		echo "<?xml version='1.0' encoding='" . get_option( 'blog_charset' ) . "' standalone='yes'?><zc_ajax>";
