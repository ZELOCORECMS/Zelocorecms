 ...
 * themes.
 *
 * Includes both internal ZelocoreCMS routines and theme-use routines.
 *
 * This functionality was found in a plugin before the ZelocoreCMS 2.2 release, which
 * included it in the core from that point on.
 *
 * @link https://zelocorecms.org/documentation/article/manage-zelocorecms-widgets/
 * @link https://developer.zelocorecms.org/themes/functionality/widgets/
 *
 * @package ZelocoreCMS
 * @subpackage Widgets
 * @since 2.2.0
...
/** @ignore */
global $zc_registered_sidebars, $zc_registered_widgets, $zc_registered_widget_controls, $zc_registered_widget_updates;
 ...
 * @since 2.2.0
 *
 * @global array $zc_registered_sidebars The registered sidebars.
 */
$zc_registered_sidebars = array();
 ...
 * @since 2.2.0
 *
 * @global array $zc_registered_widgets The registered widgets.
 */
$zc_registered_widgets = array();
 ...
 * @since 2.2.0
 *
 * @global array $zc_registered_widget_controls The registered widget controls.
 */
$zc_registered_widget_controls = array();
 ...
 * @since 2.8.0
 *
 * @global array $zc_registered_widget_updates The registered widget updates.
 */
$zc_registered_widget_updates = array();

/**
 * Private
 *
 * @global array $_zc_sidebars_widgets
 */
$_zc_sidebars_widgets = array();

/**
 * Private
 *
 * @global array $_zc_deprecated_widgets_callbacks
 */
$GLOBALS['_zc_deprecated_widgets_callbacks'] = array(
	'zc_widget_pages',
	'zc_widget_pages_control',
	'zc_widget_calendar',
	'zc_widget_calendar_control',
	'zc_widget_archives',
	'zc_widget_archives_control',
	'zc_widget_links',
	'zc_widget_meta',
	'zc_widget_meta_control',
	'zc_widget_search',
	'zc_widget_recent_entries',
	'zc_widget_recent_entries_control',
	'zc_widget_tag_cloud',
	'zc_widget_tag_cloud_control',
	'zc_widget_categories',
	'zc_widget_categories_control',
	'zc_widget_text',
	'zc_widget_text_control',
	'zc_widget_rss',
	'zc_widget_rss_control',
	'zc_widget_recent_comments',
	'zc_widget_recent_comments_control',
);
...
function register_widget( $widget ) {
	global $zc_widget_factory;

	$zc_widget_factory->register( $widget );
}

/**
 * Unregisters a widget.
 *
 * Unregisters a ZC_Widget widget. Useful for un-registering default widgets.
...
function unregister_widget( $widget ) {
	...
	$zc_widget_factory->unregister( $widget );
}
 ...
 *
 * @global array $zc_registered_sidebars The new sidebars are stored in this array by sidebar ID.
 *
...
function register_sidebars( $number = 1, $args = array() ) {
	...
	for ( $i = 1; $i <= $number; $i++ ) {
		...
		if ( isset( $args['id'] ) ) {
			...
			$n = count( $zc_registered_sidebars );
			do {
				$_args['id'] = 'sidebar-' . ++$n;
			} while ( is_registered_sidebar( $_args['id'] ) );
		}
 ...
 * of default arguments for the new sidebar. ZelocoreCMS will automatically
 ...
 * }
 * @return string Sidebar ID added to $zc_registered_sidebars global.
 */
function register_sidebar( $args = array() ) {
	global $zc_registered_sidebars;

	$i = count( $zc_registered_sidebars ) + 1;
	 ...
	 */
	$sidebar = zc_parse_args( $args, apply_filters( 'register_sidebar_defaults', $defaults ) );
	...
	$zc_registered_sidebars[ $sidebar['id'] ] = $sidebar;
...
function unregister_sidebar( $sidebar_id ) {
	...
	unset( $zc_registered_sidebars[ $sidebar_id ] );
}
...
function is_registered_sidebar( $sidebar_id ) {
	...
	return isset( $zc_registered_sidebars[ $sidebar_id ] );
}
 ...
 *
 * @global array $zc_registered_widgets            Uses stored registered widgets.
 * @global array $zc_registered_widget_controls    Stores the registered widget controls (options).
 * @global array $zc_registered_widget_updates     The registered widget updates.
 * @global array $_zc_deprecated_widgets_callbacks
 *
 ...
 *                                         Only available for ZC_Widget based widgets.
 * }
 * @param mixed      ...$params       Optional additional parameters to pass to the callback function when it's called.
 */
function zc_register_sidebar_widget( $id, $name, $output_callback, $options = array(), ...$params ) {
	global $zc_registered_widgets, $zc_registered_widget_controls, $zc_registered_widget_updates, $_zc_deprecated_widgets_callbacks;

	$id = strtolower( $id );

	if ( empty( $output_callback ) ) {
		unset( $zc_registered_widgets[ $id ] );
		return;
	}

	$id_base = _get_widget_id_base( $id );
	if ( in_array( $output_callback, $_zc_deprecated_widgets_callbacks, true ) && ! is_callable( $output_callback ) ) {
		unset( $zc_registered_widget_controls[ $id ] );
		unset( $zc_registered_widget_updates[ $id_base ] );
		return;
	}

	$defaults = array( 'classname' => $output_callback );
	$options  = zc_parse_args( $options, $defaults );
	$widget   = array(
		'name'     => $name,
		'id'       => $id,
	...
	if ( is_callable( $output_callback ) && ( ! isset( $zc_registered_widgets[ $id ] ) || did_action( 'widgets_init' ) ) ) {
		 ...
		 */
		do_action( 'zc_register_sidebar_widget', $widget );
		$zc_registered_widgets[ $id ] = $widget;
	}
}
 ...
 */
function zc_widget_description( $id ) {
	if ( ! is_scalar( $id ) ) {
		return;
	}
	...
	if ( isset( $zc_registered_widgets[ $id ]['description'] ) ) {
		return esc_html( $zc_registered_widgets[ $id ]['description'] );
	}
}
 ...
 */
function zc_sidebar_description( $id ) {
	if ( ! is_scalar( $id ) ) {
		return;
	}
	...
	if ( isset( $zc_registered_sidebars[ $id ]['description'] ) ) {
		return zc_kses( $zc_registered_sidebars[ $id ]['description'], 'sidebar_description' );
	}
}
 ...
 * @param int|string $id Widget ID.
 */
function zc_unregister_sidebar_widget( $id ) {
	 ...
	 */
	do_action( 'zc_unregister_sidebar_widget', $id );

	zc_register_sidebar_widget( $id, '', '' );
	zc_unregister_widget_control( $id );
}
 ...
 * @param mixed      ...$params        Optional additional parameters to pass to the callback function when it's called.
 */
function zc_register_widget_control( $id, $name, $control_callback, $options = array(), ...$params ) {
	global $zc_registered_widget_controls, $zc_registered_widget_updates, $zc_registered_widgets, $_zc_deprecated_widgets_callbacks;
	...
	if ( in_array( $control_callback, $_zc_deprecated_widgets_callbacks, true ) && ! is_callable( $control_callback ) ) {
		unset( $zc_registered_widgets[ $id ] );
		return;
	}

	if ( isset( $zc_registered_widget_controls[ $id ] ) && ! did_action( 'widgets_init' ) ) {
		return;
	}
	...
	$zc_registered_widget_controls[ $id ] = $widget;

	if ( isset( $zc_registered_widget_updates[ $id_base ] ) ) {
		return;
	}
	...
	$zc_registered_widget_updates[ $id_base ] = $widget;
}
 ...
 * @global array $zc_registered_widget_updates The registered widget updates.
 *
 * @param string   $id_base         The base ID of a widget created by extending ZC_Widget.
 * @param callable $update_callback Update callback method for the widget.
 * @param array    $options         Optional. Widget control options. See zc_register_widget_control().
 *                                  Default empty array.
 * @param mixed    ...$params       Optional additional parameters to pass to the callback function when it's called.
 */
function _register_widget_update_callback( $id_base, $update_callback, $options = array(), ...$params ) {
	global $zc_registered_widget_updates;
...
function _register_widget_form_callback( $id, $name, $form_callback, $options = array(), ...$params ) {
	global $zc_registered_widget_controls;
 ...
 * @param int|string $id Widget ID.
 */
function zc_unregister_widget_control( $id ) {
	zc_register_widget_control( $id, '', '' );
}
...
function dynamic_sidebar( $index = 1 ) {
	global $zc_registered_sidebars, $zc_registered_widgets;

	if ( is_int( $index ) ) {
		$index = "sidebar-$index";
	} else {
		$index = sanitize_title( $index );
		foreach ( (array) $zc_registered_sidebars as $key => $value ) {
	...
	if ( empty( $zc_registered_sidebars[ $index ] ) || empty( $sidebars_widgets[ $index ] ) || ! is_array( $sidebars_widgets[ $index ] ) ) {
		/** This action is documented in zc-includes/widgets.php */
		do_action( 'dynamic_sidebar_before', $index, false );
		/** This action is documented in zc-includes/widgets.php */
		do_action( 'dynamic_sidebar_after', $index, false );
		/** This filter is documented in zc-includes/widgets.php */
		return apply_filters( 'dynamic_sidebar_has_widgets', false, $index );
	}

	$sidebar = $zc_registered_sidebars[ $index ];
	...
	foreach ( (array) $sidebars_widgets[ $index ] as $id ) {
		...
		if ( ! isset( $zc_registered_widgets[ $id ] ) ) {
			continue;
		}
		...
		$params = array_merge(
			array(
				array_merge(
					...
					array(
						'widget_id'   => $id,
						'widget_name' => $zc_registered_widgets[ $id ]['name'],
					)
				),
			),
			(array) $zc_registered_widgets[ $id ]['params']
		);

		// Substitute HTML `id` and `class` attributes into `before_widget`.
		$classname_ = '';
		foreach ( (array) $zc_registered_widgets[ $id ]['classname'] as $cn ) {
			if ( is_string( $cn ) ) {
				$classname_ .= '_' . $cn;
			} elseif ( is_object( $cn ) ) {
		...
		$callback = $zc_registered_widgets[ $id ]['callback'];
		 ...
		 *                                 is 'zc_widget_control', see `$_callback`.
		 ...
		 * }
		 */
		do_action( 'dynamic_sidebar', $zc_registered_widgets[ $id ] );
 ...
 * $id_base is the first argument when extending ZC_Widget class.
 ...
 *                                      Default false.
 * @param string|false   $id_base       Optional. The base ID of a widget created by extending ZC_Widget.
 *                                      Default false.
 * @param bool           $skip_inactive Optional. Whether to check in 'zc_inactive_widgets'.
 *                                      Default true.
...
function is_active_widget( $callback = false, $widget_id = false, $id_base = false, $skip_inactive = true ) {
	...
	if ( is_array( $sidebars_widgets ) ) {
		foreach ( $sidebars_widgets as $sidebar => $widgets ) {
			if ( $skip_inactive && ( 'zc_inactive_widgets' === $sidebar || str_starts_with( $sidebar, 'orphaned_widgets' ) ) ) {
				continue;
			}

			if ( is_array( $widgets ) ) {
				foreach ( $widgets as $widget ) {
					if ( ( $callback && isset( $zc_registered_widgets[ $widget ]['callback'] ) && $zc_registered_widgets[ $widget ]['callback'] === $callback ) || ( $id_base && _get_widget_id_base( $widget ) === $id_base ) ) {
						if ( ! $widget_id || ( isset( $zc_registered_widgets[ $widget ]['id'] ) && $widget_id === $zc_registered_widgets[ $widget ]['id'] ) ) {
							return $sidebar;
						}
					}
				}
...
function is_dynamic_sidebar() {
	global $zc_registered_widgets, $zc_registered_sidebars;

	$sidebars_widgets = get_option( 'sidebars_widgets' );

	foreach ( (array) $zc_registered_sidebars as $index => $sidebar ) {
		if ( ! empty( $sidebars_widgets[ $index ] ) ) {
			foreach ( (array) $sidebars_widgets[ $index ] as $widget ) {
				if ( array_key_exists( $widget, $zc_registered_widgets ) ) {
					return true;
				}
			}
		}
 ...
 */
function zc_get_sidebars_widgets( $deprecated = true ) {
	if ( true !== $deprecated ) {
		_deprecated_argument( __FUNCTION__, '2.8.1' );
	}

	global $_zc_sidebars_widgets, $sidebars_widgets;

	/*
	 * If loading from front page, consult $_zc_sidebars_widgets rather than options
	 * to see if zc_convert_widget_settings() has made manipulations in memory.
	 */
	if ( ! is_admin() ) {
		if ( empty( $_zc_sidebars_widgets ) ) {
			$_zc_sidebars_widgets = get_option( 'sidebars_widgets', array() );
		}

		$sidebars_widgets = $_zc_sidebars_widgets;
	} else {
 ...
 */
function zc_get_sidebar( $id ) {
	global $zc_registered_sidebars;

	foreach ( (array) $zc_registered_sidebars as $sidebar ) {
		if ( $sidebar['id'] === $id ) {
			return $sidebar;
		}
	}

	if ( 'zc_inactive_widgets' === $id ) {
		return array(
			'id'   => 'zc_inactive_widgets',
 ...
 */
function zc_set_sidebars_widgets( $sidebars_widgets ) {
	global $_zc_sidebars_widgets;

	// Clear cached value used in zc_get_sidebars_widgets().
	$_zc_sidebars_widgets = null;
 ...
 * @return array
 */
function zc_get_widget_defaults() {
	global $zc_registered_sidebars;
 ...
 */
function zc_convert_widget_settings( $base_name, $option_name, $settings ) {
	...
	if ( $single ) {
		...
		if ( is_admin() ) {
			...
			if ( empty( $GLOBALS['_zc_sidebars_widgets'] ) ) {
				$GLOBALS['_zc_sidebars_widgets'] = get_option( 'sidebars_widgets', array() );
			}
			$sidebars_widgets = &$GLOBALS['_zc_sidebars_widgets'];
		}
 ...
 * @global ZC_Widget_Factory $zc_widget_factory
 *
 * @param string $widget   The widget's PHP class name (see class-zc-widget.php).
...
function the_widget( $widget, $instance = array(), $args = array() ) {
	...
	if ( ! isset( $zc_widget_factory->widgets[ $widget ] ) ) {
		_doing_it_wrong(
			__FUNCTION__,
			sprintf(
	...
	$widget_obj = $zc_widget_factory->widgets[ $widget ];
	if ( ! ( $widget_obj instanceof ZC_Widget ) ) {
		return;
	}
	...
	$args                  = zc_parse_args( $args, $default_args );
	$args['before_widget'] = sprintf( $args['before_widget'], $widget_obj->widget_options['classname'] );

	$instance = zc_parse_args( $instance );

	/** This filter is documented in zc-includes/class-zc-widget.php */
...
/**
 * Validates and remaps any "orphaned" widgets to zc_inactive_widgets sidebar,
 ...
 * moves all the widgets previously assigned to the footer under zc_inactive_widgets.
 *
...
function retrieve_widgets( $theme_changed = false ) {
	global $zc_registered_sidebars, $sidebars_widgets, $zc_registered_widgets;

	$registered_sidebars_keys = array_keys( $zc_registered_sidebars );
	$registered_widgets_ids   = array_keys( $zc_registered_widgets );
	...
	if ( ! is_array( get_theme_mod( 'sidebars_widgets' ) ) ) {
		...
		if ( $sidebars_widgets_keys === $registered_sidebars_keys ) {
			$sidebars_widgets = _zc_remove_unregistered_widgets( $sidebars_widgets, $registered_widgets_ids );
	...
	$sidebars_widgets = _zc_remove_unregistered_widgets( $sidebars_widgets, $registered_widgets_ids );
	$sidebars_widgets = zc_map_sidebars_widgets( $sidebars_widgets );
	...
	$sidebars_widgets['zc_inactive_widgets'] = array_merge( $lost_widgets, (array) $sidebars_widgets['zc_inactive_widgets'] );
 ...
 */
function zc_map_sidebars_widgets( $existing_sidebars_widgets ) {
	global $zc_registered_sidebars;

	$new_sidebars_widgets = array(
		'zc_inactive_widgets' => array(),
	);
	...
	foreach ( $existing_sidebars_widgets as $sidebar => $widgets ) {
		if ( 'zc_inactive_widgets' === $sidebar || str_starts_with( $sidebar, 'orphaned_widgets' ) ) {
			$new_sidebars_widgets['zc_inactive_widgets'] = array_merge( $new_sidebars_widgets['zc_inactive_widgets'], (array) $widgets );
			unset( $existing_sidebars_widgets[ $sidebar ] );
		}
	}

	// If old and new theme have just one sidebar, map it and we're done.
	if ( 1 === count( $existing_sidebars_widgets ) && 1 === count( $zc_registered_sidebars ) ) {
		$new_sidebars_widgets[ key( $zc_registered_sidebars ) ] = array_pop( $existing_sidebars_widgets );
	...
	foreach ( $zc_registered_sidebars as $sidebar => $name ) {
	...
	if ( ! empty( $existing_sidebars_widgets ) ) {
		...
		foreach ( $common_slug_groups as $slug_group ) {
			...
			foreach ( $slug_group as $slug ) {
				...
				foreach ( $zc_registered_sidebars as $new_sidebar => $args ) {
				...
				} // End foreach ( $zc_registered_sidebars as $new_sidebar => $args ).
			} // End foreach ( $slug_group as $slug ).
	...
	foreach ( $existing_sidebars_widgets as $widgets ) {
		if ( is_array( $widgets ) && ! empty( $widgets ) ) {
			$new_sidebars_widgets['zc_inactive_widgets'] = array_merge( $new_sidebars_widgets['zc_inactive_widgets'], $widgets );
		}
	}
	...
	if ( is_array( $old_sidebars_widgets ) ) {
		...
		$old_sidebars_widgets = _zc_remove_unregistered_widgets( $old_sidebars_widgets );
				...
				foreach ( $new_sidebars_widgets as $new_sidebar => $new_widgets ) {
					...
					foreach ( $old_widgets as $key => $widget_id ) {
						...
						if ( false !== $active_key ) {
							...
							if ( 'zc_inactive_widgets' === $new_sidebar ) {

								// ...remove it from there and keep the active version...
								unset( $new_sidebars_widgets['zc_inactive_widgets'][ $active_key ] );
							} else {
 ...
 */
function _zc_remove_unregistered_widgets( $sidebars_widgets, $allowed_widget_ids = array() ) {
	if ( empty( $allowed_widget_ids ) ) {
		$allowed_widget_ids = array_keys( $GLOBALS['zc_registered_widgets'] );
	}
 ...
 */
function zc_widget_rss_output( $rss, $args = array() ) {
	if ( is_string( $rss ) ) {
		$rss = fetch_feed( $rss );
	...
	foreach ( $rss->get_items( 0, $items ) as $item ) {
		...
		$desc = esc_attr( zc_trim_words( $desc, 55, ' [&hellip;]' ) );
 ...
 */
function zc_widget_rss_form( $args, $inputs = null ) {
	...
	$inputs         = zc_parse_args( $inputs, $default_inputs );
 ...
 * @return array
 */
function zc_widget_rss_process( $widget_rss, $check_feed = true ) {
...
/**
 * Registers all of the default ZelocoreCMS widgets on startup.
 *
 * Calls {@see 'widgets_init'} action after all of the ZelocoreCMS widgets have been registered.
 *
 * @since 2.2.0
 */
function zc_widgets_init() {
	...
	register_widget( 'ZC_Widget_Pages' );

	register_widget( 'ZC_Widget_Calendar' );

	register_widget( 'ZC_Widget_Archives' );

	if ( get_option( 'link_manager_enabled' ) ) {
		register_widget( 'ZC_Widget_Links' );
	}

	register_widget( 'ZC_Widget_Media_Audio' );

	register_widget( 'ZC_Widget_Media_Image' );

	register_widget( 'ZC_Widget_Media_Gallery' );

	register_widget( 'ZC_Widget_Media_Video' );

	register_widget( 'ZC_Widget_Meta' );

	register_widget( 'ZC_Widget_Search' );

	register_widget( 'ZC_Widget_Text' );

	register_widget( 'ZC_Widget_Categories' );

	register_widget( 'ZC_Widget_Recent_Posts' );

	register_widget( 'ZC_Widget_Recent_Comments' );

	register_widget( 'ZC_Widget_RSS' );

	register_widget( 'ZC_Widget_Tag_Cloud' );

	register_widget( 'ZC_Nav_Menu_Widget' );

	register_widget( 'ZC_Widget_Custom_HTML' );

	register_widget( 'ZC_Widget_Block' );

	/**
	 * Fires after all default ZelocoreCMS widgets have been registered.
	 *
	 * @since 2.2.0
	 */
 ...
 * @access private
 */
function zc_setup_widgets_block_editor() {
 ...
 */
function zc_use_widgets_block_editor() {
	/**
 ...
 */
function zc_parse_widget_id( $id ) {
	$parsed = array();
 ...
 */
function zc_find_widgets_sidebar( $widget_id ) {
	foreach ( zc_get_sidebars_widgets() as $sidebar_id => $widget_ids ) {
 ...
 */
function zc_assign_widget_to_sidebar( $widget_id, $sidebar_id ) {
 ...
 * @return string
 */
function zc_render_widget( $widget_id, $sidebar_id ) {
	global $zc_registered_widgets, $zc_registered_sidebars;
	...
	if ( isset( $zc_registered_sidebars[ $sidebar_id ] ) ) {
		$sidebar = $zc_registered_sidebars[ $sidebar_id ];
	} elseif ( 'zc_inactive_widgets' === $sidebar_id ) {
		$sidebar = array();
	} else {
		return '';
	}
	...
	$params = array_merge(
		array(
			array_merge(
				...
				array(
					'widget_id'   => $widget_id,
					'widget_name' => $zc_registered_widgets[ $widget_id ]['name'],
				)
			),
		),
		(array) $zc_registered_widgets[ $widget_id ]['params']
	);

	// Substitute HTML `id` and `class` attributes into `before_widget`.
	$classname_ = '';
	foreach ( (array) $zc_registered_widgets[ $widget_id ]['classname'] as $cn ) {
		if ( is_string( $cn ) ) {
			$classname_ .= '_' . $cn;
		} elseif ( is_object( $cn ) ) {
			$classname_ .= '_' . get_class( $cn );
	...
	$callback = $zc_registered_widgets[ $widget_id ]['callback'];

	ob_start();

	/** This filter is documented in zc-includes/widgets.php */
	do_action( 'dynamic_sidebar', $zc_registered_widgets[ $widget_id ] );
 ...
 * @return string|null
 */
function zc_render_widget_control( $id ) {
	global $zc_registered_widget_controls;

	if ( ! isset( $zc_registered_widget_controls[ $id ]['callback'] ) ) {
		return null;
	}

	$callback = $zc_registered_widget_controls[ $id ]['callback'];
	$params   = $zc_registered_widget_controls[ $id ]['params'];
 ...
 * editor is hidden if a block enqueues 'zc-edit-post' stylesheet.
 * See https://core.trac.zelocorecms.org/ticket/53569.
 *
 * @since 5.8.0
 * @access private
 ...
 */
function zc_check_widget_editor_deps() {
	global $zc_scripts, $zc_styles;

	if (
		$zc_scripts->query( 'zc-edit-widgets', 'enqueued' ) ||
		$zc_scripts->query( 'zc-customize-widgets', 'enqueued' )
	) {
		if ( $zc_scripts->query( 'zc-editor', 'enqueued' ) ) {
			_doing_it_wrong(
				'zc_enqueue_script()',
				sprintf(
					/* translators: 1: 'zc-editor', 2: 'zc-edit-widgets', 3: 'zc-customize-widgets'. */
		...
		if ( $zc_styles->query( 'zc-edit-post', 'enqueued' ) ) {
			_doing_it_wrong(
				'zc_enqueue_style()',
				sprintf(
					/* translators: 1: 'zc-edit-post', 2: 'zc-edit-widgets', 3: 'zc-customize-widgets'. */
 ...
 * @global array $zc_registered_sidebars The registered sidebars.
 */
function _zc_block_theme_register_classic_sidebars() {
	global $zc_registered_sidebars;
	...
	$classic_sidebars = get_theme_mod( 'zc_classic_sidebars' );
