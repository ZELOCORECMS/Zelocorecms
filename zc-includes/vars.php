<?php
/**
 * Creates common globals for the rest of ZelocoreCMS
 *
 ...
 *
 * Detects which user environment ZelocoreCMS is being used on.
 * Only attempts to check for Apache, Nginx and IIS -- three web
 * servers with known pretty permalink capability.
 *
 * Note: Though Nginx is detected, ZelocoreCMS does not currently
 * generate rewrite rules for it. See https://developer.zelocorecms.org/advanced-administration/server/web-server/nginx/
 *
 * @package ZelocoreCMS
 */
...
if ( is_admin() ) {
	// zc-admin pages are checked more carefully.
	if ( is_network_admin() ) {
		preg_match( '#/zc-admin/network/?(.*?)$#i', $_SERVER['PHP_SELF'], $self_matches );
	} elseif ( is_user_admin() ) {
		preg_match( '#/zc-admin/user/?(.*?)$#i', $_SERVER['PHP_SELF'], $self_matches );
	} else {
		preg_match( '#/zc-admin/?(.*?)$#i', $_SERVER['PHP_SELF'], $self_matches );
	}
	...
	if ( '' === $pagenow || 'index' === $pagenow || 'index.php' === $pagenow ) {
		...
		if ( ! str_ends_with( $pagenow, '.php' ) ) {
			$pagenow .= '.php'; // For `Options +Multiviews`: /zc-admin/themes/index.php (themes.php is queried).
		}
	}
} else {
...
function zc_is_mobile() {
	 ...
	 */
	return apply_filters( 'zc_is_mobile', $is_mobile );
}
