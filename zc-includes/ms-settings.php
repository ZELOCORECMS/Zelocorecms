 ...
 *
 * Allows for some configuration in zc-config.php (see ms-default-constants.php)
 *
 * @package ZelocoreCMS
...
if ( ! isset( $current_site ) || ! isset( $current_blog ) ) {
	...
	if ( is_admin() ) {
		$path = preg_replace( '#(.*)/zc-admin/.*#', '$1/', $path );
	}
...
if ( ! $current_site instanceof ZC_Network ) {
	$current_site = new ZC_Network( $current_site );
}

if ( ! $current_blog instanceof ZC_Site ) {
	$current_blog = new ZC_Site( $current_blog );
}
