 ...
 */
function get_locale() {
	global $locale, $zc_local_package;

	if ( isset( $locale ) ) {
		/** This filter is documented in zc-includes/l10n.php */
		return apply_filters( 'locale', $locale );
	}

	if ( isset( $zc_local_package ) ) {
		$locale = $zc_local_package;
	}

	// WPLANG was defined in zc-config.
	if ( defined( 'WPLANG' ) ) {
		$locale = WPLANG;
	}
	...
	/**
	 * Filters the locale ID of the ZelocoreCMS installation.
	 *
	 * @since 1.5.0
	 *
 ...
 * @since 4.7.0
 *
 * @param int|ZC_User $user User's ID or a ZC_User object. Defaults to current user.
 * @return string The locale of the user.
 */
function get_user_locale( $user = 0 ) {
	$user_object = false;

	if ( 0 === $user && function_exists( 'zc_get_current_user' ) ) {
		$user_object = zc_get_current_user();
	} elseif ( $user instanceof ZC_User ) {
		$user_object = $user;
...
function determine_locale() {
	...
	if (
		isset( $GLOBALS['pagenow'] ) && 'zc-login.php' === $GLOBALS['pagenow'] &&
		( ! empty( $_GET['zc_lang'] ) || ! empty( $_COOKIE['zc_lang'] ) )
	) {
		if ( ! empty( $_GET['zc_lang'] ) ) {
			$determined_locale = sanitize_locale_name( $_GET['zc_lang'] );
		} else {
			$determined_locale = sanitize_locale_name( $_COOKIE['zc_lang'] );
		}
	} elseif (
		is_admin() ||
		( isset( $_GET['_locale'] ) && 'user' === $_GET['_locale'] && zc_is_json_request() )
	) {
		$determined_locale = get_user_locale();
	} elseif (
		( ! empty( $_REQUEST['language'] ) || isset( $GLOBALS['zc_local_package'] ) )
		&& zc_installing()
	) {
		if ( ! empty( $_REQUEST['language'] ) ) {
			$determined_locale = sanitize_locale_name( $_REQUEST['language'] );
		} else {
			$determined_locale = $GLOBALS['zc_local_package'];
		}
	}
...
function load_textdomain( $domain, $mofile, $locale = null ) {
	/** @var ZC_Textdomain_Registry $zc_textdomain_registry */
	global $l10n, $l10n_unloaded, $zc_textdomain_registry;
	...
	$i18n_controller = ZC_Translation_Controller::get_instance();
	...
	foreach ( $translation_files as $file ) {
		...
		if ( $success ) {
			...
			$l10n[ $domain ] = new ZC_Translations( $i18n_controller, $domain );

			$zc_textdomain_registry->set( $domain, $locale, dirname( $file ) );
...
function unload_textdomain( $domain, $reloadable = false ) {
	...
	if ( ! $reloadable ) {
		ZC_Translation_Controller::get_instance()->unload_textdomain( $domain );
	}
 ...
 *
 * Loads the .mo file in ZC_LANG_DIR constant path from ZelocoreCMS root.
...
function load_default_textdomain( $locale = null ) {
	...
	$return = load_textdomain( 'default', ZC_LANG_DIR . "/$locale.mo", $locale );

	if ( ( is_multisite() || ( defined( 'ZC_INSTALLING_NETWORK' ) && ZC_INSTALLING_NETWORK ) ) && ! file_exists( ZC_LANG_DIR . "/admin-$locale.mo" ) ) {
		load_textdomain( 'default', ZC_LANG_DIR . "/ms-$locale.mo", $locale );
		return $return;
	}

	if ( is_admin() || zc_installing() || ( defined( 'ZC_REPAIRING' ) && ZC_REPAIRING ) || doing_action( 'zc_maybe_auto_update' ) ) {
		load_textdomain( 'default', ZC_LANG_DIR . "/admin-$locale.mo", $locale );
	}

	if ( is_network_admin() || ( defined( 'ZC_INSTALLING_NETWORK' ) && ZC_INSTALLING_NETWORK ) ) {
		load_textdomain( 'default', ZC_LANG_DIR . "/admin-network-$locale.mo", $locale );
	}
 ...
 *
 * @global ZC_Textdomain_Registry $zc_textdomain_registry ZelocoreCMS Textdomain Registry.
 * @global array<string, ZC_Translations|NOOP_Translations> $l10n An array of all currently loaded text domains.
 *
 * @param string       $domain          Unique identifier for retrieving translated strings
 * @param string|false $deprecated      Optional. Deprecated. Use the $plugin_rel_path parameter instead.
 *                                      Default false.
 * @param string|false $plugin_rel_path Optional. Relative path to ZC_PLUGIN_DIR where the .mo file resides.
 *                                      Default false.
 * @return bool True when textdomain is successfully loaded, false otherwise.
 */
function load_plugin_textdomain( $domain, $deprecated = false, $plugin_rel_path = false ) {
	/** @var ZC_Textdomain_Registry $zc_textdomain_registry */
	/** @var array<string, ZC_Translations|NOOP_Translations> $l10n */
	global $zc_textdomain_registry, $l10n;
	...
	if ( false !== $plugin_rel_path ) {
		$path = ZC_PLUGIN_DIR . '/' . trim( $plugin_rel_path, '/' );
	...
	$zc_textdomain_registry->set_custom_path( $domain, $path );
 ...
 *
 * @param string $domain             Text domain. Unique identifier for retrieving translated strings.
 * @param string $mu_plugin_rel_path Optional. Relative to `ZCMU_PLUGIN_DIR` directory in which the .mo
...
function load_muplugin_textdomain( $domain, $mu_plugin_rel_path = '' ) {
	...
	$path = ZCMU_PLUGIN_DIR . '/' . ltrim( $mu_plugin_rel_path, '/' );
...
function load_script_textdomain( $handle, $domain = 'default', $path = '' ) {
	...
	if ( ! isset( $zc_scripts->registered[ $handle ] ) ) {
		return false;
	}

	$src = $zc_scripts->registered[ $handle ]->src;

	if ( ! preg_match( '|^(https?:)?//|', $src ) && ! ( $zc_scripts->content_url && str_starts_with( $src, $zc_scripts->content_url ) ) ) {
		$src = $zc_scripts->base_url . $src;
	}
 ...
 * via {@see zc_register_script_module()}.
 *
 * @since 7.0.0
 *
...
function load_script_module_textdomain( string $id, string $domain = 'default', string $path = '' ) {
	$module = zc_script_modules()->get_registered( $id );
	if ( null === $module ) {
		return false;
	}
	$src = $module['src'];
...
function _load_script_textdomain_from_src( string $handle, string $src, string $domain, string $path, bool $is_module ) {
	/** @var ZC_Textdomain_Registry $zc_textdomain_registry */
	global $zc_textdomain_registry;

	$locale = determine_locale();

	if ( ! $path ) {
		$path = $zc_textdomain_registry->get( $domain, $locale );
	}
	...
	$relative       = false;
	$languages_path = ZC_LANG_DIR;

	$src_url = zc_parse_url( $src );
	if ( ! $src_url ) {
		return load_script_translations( false, $handle, $domain );
	}
	$src_url['path'] ??= '';

	$content_url = zc_parse_url( content_url() );
	if ( ! $content_url ) {
		return load_script_translations( false, $handle, $domain );
	}

	$plugins_url = zc_parse_url( plugins_url() );
	$site_url    = zc_parse_url( site_url() );
	...
	if (
		...
		/*
		 * Ensure correct languages path when using a custom `ZC_PLUGIN_DIR` / `ZC_PLUGIN_URL` configuration,
		 * a custom theme root, and/or using Multisite with subdirectories.
		 * See https://core.trac.zelocorecms.org/ticket/60891 and https://core.trac.zelocorecms.org/ticket/62016.
		 */

		$theme_dir = array_slice( explode( '/', $theme_root ), -1 );
		$dirname   = $theme_dir[0] === $relative[0] ? 'themes' : 'plugins';

		$languages_path = ZC_LANG_DIR . '/' . $dirname;
		...
		$languages_path = ZC_LANG_DIR . '/plugins';
 ...
 * the translation file from `zc-content/languages`, removing the need
...
function _load_textdomain_just_in_time( $domain ) {
	/** @var ZC_Textdomain_Registry $zc_textdomain_registry */
	global $l10n_unloaded, $zc_textdomain_registry;
	...
	if ( ! $zc_textdomain_registry->has( $domain ) ) {
		return false;
	}
	...
	// Themes with their language directory outside of ZC_LANG_DIR have a different file name.
 ...
 *
 * The default directory is ZC_LANG_DIR.
 *
 * @since 3.0.0
...
function get_available_languages( $dir = null ) {
	...
	$path       = $dir ?? ZC_LANG_DIR;
	$lang_files = $zc_textdomain_registry->get_language_files_from_path( $path );
 ...
 *
 * Looks in the zc-content/languages directory for translations of
 * plugins or themes.
 *
 * @since 3.7.0
 *
 ...
 */
function zc_get_installed_translations( $type ) {
	global $zc_textdomain_registry;
	...
	$dir = 'core' === $type ? ZC_LANG_DIR : ZC_LANG_DIR . "/$type";

	if ( ! is_dir( $dir ) ) {
		return array();
	}

	$files = $zc_textdomain_registry->get_language_files_from_path( $dir );
	if ( ! $files ) {
		return array();
	}
	...
	foreach ( $files as $file ) {
		...
		if ( str_ends_with( $file, '.mo' ) ) {
			...
			$language_data[ $textdomain ][ $language ] = zc_get_pomo_file_data( $pofile );
		} else {
			...
			$language_data[ $textdomain ][ $language ] = zc_get_l10n_php_file_data( $file );
		}
	}
 ...
 */
function zc_get_pomo_file_data( $po_file ) {
 ...
 */
function zc_get_l10n_php_file_data( $php_file ) {
 ...
 */
function zc_dropdown_languages( $args = array() ) {

	$parsed_args = zc_parse_args(
		$args,
		array(
			'id'                          => 'locale',
			'name'                        => 'locale',
...
function is_rtl() {
	global $zc_locale;
	if ( ! ( $zc_locale instanceof ZC_Locale ) ) {
		return false;
	}
	return $zc_locale->is_rtl();
}
...
function switch_to_locale( $locale ) {
	/* @var ZC_Locale_Switcher $zc_locale_switcher */
	global $zc_locale_switcher;

	if ( ! $zc_locale_switcher ) {
		return false;
	}

	return $zc_locale_switcher->switch_to_locale( $locale );
}
...
function switch_to_user_locale( $user_id ) {
	...
	return $zc_locale_switcher->switch_to_user_locale( $user_id );
}
...
function restore_previous_locale() {
	...
	return $zc_locale_switcher->restore_previous_locale();
}
...
function restore_current_locale() {
	...
	return $zc_locale_switcher->restore_current_locale();
}
...
function is_locale_switched() {
	...
	return $zc_locale_switcher->is_switched();
}
 ...
 */
function zc_get_list_item_separator() {
	global $zc_locale;

	if ( ! ( $zc_locale instanceof ZC_Locale ) ) {
		// Default value of ZC_Locale::get_list_item_separator().
		/* translators: Used between list items, there is a space after the comma. */
		return __( ', ' );
	}

	return $zc_locale->get_list_item_separator();
}
 ...
 */
function zc_get_word_count_type() {
	global $zc_locale;

	if ( ! ( $zc_locale instanceof ZC_Locale ) ) {
		// Default value of ZC_Locale::get_word_count_type().
		return 'words';
	}

	return $zc_locale->get_word_count_type();
}
...
function has_translation( string $singular, string $textdomain = 'default', ?string $locale = null ): bool {
	return ZC_Translation_Controller::get_instance()->has_translation( $singular, $textdomain, $locale );
}
