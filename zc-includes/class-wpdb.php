...
class wpdb {
	 ...
	 *
	 * Default is to show errors if both ZC_DEBUG and ZC_DEBUG_DISPLAY evaluate to true.
	 *
	 * @since 0.71
	 *
	 * @var bool
	...
	/**
	 * ZelocoreCMS table prefix.
	 *
	 * You can set this to have multiple ZelocoreCMS installations in a single database.
	 *
	 * @since 2.5.0
	 *
	 * @var string
	...
	/**
	 * List of ZelocoreCMS per-site tables.
	 *
	 * @since 2.5.0
	 *
	 * @see wpdb::tables()
	...
	/**
	 * List of deprecated ZelocoreCMS tables.
	 *
	...
	/**
	 * List of ZelocoreCMS global tables.
	 *
	 * @since 3.0.0
	 *
	 * @see wpdb::tables()
	...
	/**
	 * List of deprecated ZelocoreCMS Multisite global tables.
	 *
	 * @since 6.1.0
	 *
	 * @see wpdb::tables()
	...
	/**
	 * ZelocoreCMS Comment Metadata table.
	 *
	 * @since 2.9.0
	 *
	 * @var string
	...
	/**
	 * ZelocoreCMS Post Metadata table.
	 *
	 * @since 1.5.0
	 *
	 * @var string
	...
	/**
	 * ZelocoreCMS Term Relationships table.
	 *
	 * @since 2.3.0
	 *
	 * @var string
	...
	/**
	 * ZelocoreCMS Term Taxonomy table.
	 *
	 * @since 2.3.0
	 *
	 * @var string
	...
	/**
	 * ZelocoreCMS User Metadata table.
	 *
	 * @since 2.3.0
	 *
	 * @var string
	 ...
	 * @since 2.0.8
	 *
	 * @link https://core.trac.zelocorecms.org/ticket/3354
	 *
	...
	public function __construct(
		...
		// zc-config.php creation will manually connect when ready.
	...
	/**
	 * Changes the current SQL mode, and ensures its ZelocoreCMS compatibility.
	 *
	...
	/**
	 * Sets the table prefix for the ZelocoreCMS tables.
	 *
	 * @since 2.5.0
	 *
	...
	public function set_prefix( $prefix, $set_table_names = true ) {
		...
		if ( preg_match( '|[^a-z0-9_]|i', $prefix ) ) {
			return new ZC_Error( 'invalid_db_prefix', 'Invalid database prefix' );
		}
	...
	/**
	 * Returns an array of ZelocoreCMS tables.
	 *
	 * Also allows for the `CUSTOM_USER_TABLE` and `CUSTOM_USER_META_TABLE` to override the ZelocoreCMS users
		...
		if ( ! $success ) {
			...
			if ( ! did_action( 'template_redirect' ) ) {
				...
				$message .= '<p>' . sprintf(
					...
					__( 'If you do not know how to set up a database you should <strong>contact your host</strong>. If all else fails you may find help at the <a href="%s">ZelocoreCMS support forums</a>.' ),
	...
	public function prepare( $query, ...$args ) {
		 ...
		 * directly outside of ZelocoreCMS core, e.g. by HyperDB, in which case
	...
	public function print_error( $str = '' ) {
		...
		if ( $caller ) {
			...
			$error_str = sprintf( 'ZelocoreCMS database error %1$s for query %2$s made by %3$s', $str, $this->last_query, $caller );
		} else {
			$error_str = sprintf( 'ZelocoreCMS database error %1$s for query %2$s', $str, $this->last_query );
		}
	...
	public function db_connect( $allow_bail = true ) {
		...
		/*
		 * Switch error reporting off because ZelocoreCMS handles its own.
		...
		if ( ! $this->dbh && $allow_bail ) {
			...
			$message .= '<p>' . sprintf(
				/* translators: 1: zc-config.php, 2: Database host. */
	...
	public function check_connection( $allow_bail = true ) {
		...
		for ( $tries = 1; $tries <= $this->reconnect_retries; $tries++ ) {
			 ...
			 */
			if ( $this->reconnect_retries === $tries && ZC_DEBUG ) {
				error_reporting( $error_reporting );
			}
		...
		/*
		 * If template_redirect has already happened, it's too late for zc_die()/dead_db().
	...
	public function query( $query ) {
		...
		if ( $this->check_current_query && ! $this->check_ascii( $query ) ) {
			...
			if ( $stripped_query !== $query ) {
				...
				$this->last_error = __( 'ZelocoreCMS database error: Could not perform query because it contains invalid data.' );
		...
		if ( $data !== $converted_data ) {
			...
			if ( 1 === count( $problem_fields ) ) {
				$this->last_error = sprintf(
					...
					__( 'ZelocoreCMS database error: Processing the value for the following field failed: %s. The supplied value may be too long or contains invalid data.' ),
					reset( $problem_fields )
				);
			} else {
				$this->last_error = sprintf(
					/* translators: %s: Database fields where the error occurred. */
					__( 'ZelocoreCMS database error: Processing the values for the following fields failed: %s. The supplied values may be too long or contain invalid data.' ),
	 ...
	 *
	 * @param string $table Table name.
	 * @return string|ZC_Error Table character set, ZC_Error object if it couldn't be found.
	 */
	protected function get_table_charset( $table ) {
		 ...
		 * @since 4.2.0
		 *
		 * @param string|ZC_Error|null $charset The character set to use, ZC_Error object
	 ...
	 * @return string|false|ZC_Error Column character set as a string. False if the column has
	 *                               no character set. ZC_Error object if there was an error.
	 */
	public function get_col_charset( $table, $column ) {
		 ...
		 * @since 4.2.0
		 *
		 * @param string|null|false|ZC_Error $charset The character set to use. Default null.
		 * @param string                     $table   The name of the table being checked.
	 ...
	 * @param string $table  Table name.
	 * @param string $column Column name.
	 * @return array|false|ZC_Error {
	 *     Array of column length information, false if the column has no length (for
	 *     example, numeric column), ZC_Error object if there was an error.
	 *
	 *     @type string $type   One of 'byte' or 'char'.
	 *     @type int    $length The column length.
	 * }
	 ...
	 * @return array|ZC_Error The $data parameter, with invalid characters removed from each value.
	 ...
	 * @return string|ZC_Error The converted query, or a ZC_Error object if the conversion fails.
	 */
	 ...
	 * @return string|ZC_Error The converted string, or a ZC_Error object if the conversion fails.
	 */
	...
	protected function get_table_from_query( $query ) {
		...
		// SHOW TABLE STATUS and SHOW TABLES WHERE Name = 'zc_posts'
	...
	public function bail( $message, $error_code = '500' ) {
		if ( $this->show_errors ) {
			...
			if ( class_exists( 'ZC_Error', false ) ) {
				$this->error = new ZC_Error( $error_code, $message );
			} else {
				$this->error = $message;
			}
	...
	public function check_database_version() {
		...
		if ( version_compare( $this->db_version(), $required_mysql_version, '<' ) ) {
			/* translators: 1: ZelocoreCMS version number, 2: Minimum required MySQL version number. */
			return new ZC_Error( 'database_version', sprintf( __( '<strong>Error:</strong> ZelocoreCMS %1$s requires MySQL %2$s or higher' ), $zc_version, $required_mysql_version ) );
		}
	}
	...
	public function get_caller() {
		return zc_debug_backtrace_summary( __CLASS__ );
	}
