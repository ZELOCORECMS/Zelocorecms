 ...
 *
 * Standardizes the HTTP requests for ZelocoreCMS. Handles cookies, gzip encoding and decoding, chunk
...
/**
 * Returns the initialized ZC_Http Object
 *
 * @since 2.7.0
 * @access private
 *
 * @return ZC_Http HTTP Transport object.
 */
function _zc_http_get_object() {
	static $http = null;
 ...
 * URL. The URL, and every URL it redirects to, are validated with zc_http_validate_url()
 ...
 * @since 3.6.0
 *
 * @see zc_remote_request() For more information on the response array format.
 * @see ZC_Http::request() For default arguments information.
 * @see zc_http_validate_url() For more information about how the URL is validated.
 *
 * @link https://owasp.org/www-community/attacks/Server_Side_Request_Forgery
 *
 * @param string $url  URL to retrieve.
 * @param array  $args Optional. Request arguments. Default empty array.
 *                     See ZC_Http::request() for information on accepted arguments.
 * @return array|ZC_Error The response or ZC_Error on failure.
 *                        See ZC_Http::request() for information on return value.
 */
function zc_safe_remote_request( $url, $args = array() ) {
	$args['reject_unsafe_urls'] = true;
	$http                       = _zc_http_get_object();
 ...
 *                        See ZC_Http::request() for information on return value.
 */
function zc_safe_remote_get( $url, $args = array() ) {
	$args['reject_unsafe_urls'] = true;
	$http                       = _zc_http_get_object();
 ...
 *                        See ZC_Http::request() for information on return value.
 */
function zc_safe_remote_post( $url, $args = array() ) {
	$args['reject_unsafe_urls'] = true;
	$http                       = _zc_http_get_object();
 ...
 *                        See ZC_Http::request() for information on return value.
 */
function zc_safe_remote_head( $url, $args = array() ) {
	$args['reject_unsafe_urls'] = true;
	$http                       = _zc_http_get_object();
 ...
 *
 *  - Default 'GET'  for zc_remote_get()
 *  - Default 'POST' for zc_remote_post()
 *  - Default 'HEAD' for zc_remote_head()
 *
 * Important: If the URL is user-controlled, use `zc_safe_remote_request()` instead.
 *
 * @since 2.7.0
 *
 * @see ZC_Http::request() For information on default arguments.
 *
 * @param string $url  URL to retrieve.
 * @param array  $args Optional. Request arguments. Default empty array.
 *                     See ZC_Http::request() for information on accepted arguments.
 * @return array|ZC_Error The response array or a ZC_Error on failure.
 *                        See ZC_Http::request() for information on return value.
 */
function zc_remote_request( $url, $args = array() ) {
	$http = _zc_http_get_object();
	return $http->request( $url, $args );
}
 ...
 *
 * Important: If the URL is user-controlled, use `zc_safe_remote_get()` instead.
 *
 * @since 2.7.0
 *
 * @see zc_remote_request() For more information on the response array format.
 ...
 *                        See ZC_Http::request() for information on return value.
 */
function zc_remote_get( $url, $args = array() ) {
	$http = _zc_http_get_object();
	return $http->get( $url, $args );
}
 ...
 *
 * Important: If the URL is user-controlled, use `zc_safe_remote_post()` instead.
 *
 * @since 2.7.0
 *
 * @see zc_remote_request() For more information on the response array format.
 ...
 *                        See ZC_Http::request() for information on return value.
 */
function zc_remote_post( $url, $args = array() ) {
	$http = _zc_http_get_object();
	return $http->post( $url, $args );
}
 ...
 *
 * Important: If the URL is user-controlled, use `zc_safe_remote_head()` instead.
 *
 * @since 2.7.0
 *
 * @see zc_remote_request() For more information on the response array format.
 ...
 *                        See ZC_Http::request() for information on return value.
 */
function zc_remote_head( $url, $args = array() ) {
	$http = _zc_http_get_object();
	return $http->head( $url, $args );
}
 ...
 *
 * @param array|ZC_Error $response HTTP response.
 ...
 */
function zc_remote_retrieve_headers( $response ) {
	if ( is_zc_error( $response ) || ! isset( $response['headers'] ) ) {
		return array();
	}
 ...
 */
function zc_remote_retrieve_header( $response, $header ) {
	if ( is_zc_error( $response ) || ! isset( $response['headers'] ) ) {
		return '';
	}
 ...
 */
function zc_remote_retrieve_response_code( $response ) {
	if ( is_zc_error( $response ) || ! isset( $response['response'] ) || ! is_array( $response['response'] ) ) {
		return '';
	}
 ...
 */
function zc_remote_retrieve_response_message( $response ) {
	if ( is_zc_error( $response ) || ! isset( $response['response'] ) || ! is_array( $response['response'] ) ) {
		return '';
	}
 ...
 */
function zc_remote_retrieve_body( $response ) {
	if ( is_zc_error( $response ) || ! isset( $response['body'] ) ) {
		return '';
	}
 ...
 *
 * @param array|ZC_Error $response HTTP response.
 * @return ZC_Http_Cookie[] An array of `ZC_Http_Cookie` objects from the response.
 *                          Empty array if there are none, or the response is a ZC_Error.
 */
function zc_remote_retrieve_cookies( $response ) {
	if ( is_zc_error( $response ) || empty( $response['cookies'] ) ) {
		return array();
	}
 ...
 * @return ZC_Http_Cookie|string The `ZC_Http_Cookie` object, or empty string
 *                               if the cookie is not present in the response.
 */
function zc_remote_retrieve_cookie( $response, $name ) {
	$cookies = zc_remote_retrieve_cookies( $response );
 ...
 *                if the cookie is not present in the response.
 */
function zc_remote_retrieve_cookie_value( $response, $name ) {
	$cookie = zc_remote_retrieve_cookie( $response, $name );

	if ( ! ( $cookie instanceof ZC_Http_Cookie ) ) {
		return '';
	}
 ...
 * @since 3.2.0
 *
 * @param array  $capabilities Array of capabilities to test or a zc_remote_request() $args array.
 * @param string $url          Optional. If given, will check if the URL requires SSL and adds
 *                             that requirement to the capabilities array.
 *
 * @return bool
 */
function zc_http_supports( $capabilities = array(), $url = null ) {
	$capabilities = zc_parse_args( $capabilities );

	$count = count( $capabilities );

	// If we have a numeric $capabilities array, spoof a zc_remote_request() associative $args array.
 ...
 */
function zc_http_validate_url( $url ) {
	if ( ! is_string( $url ) || '' === $url || is_numeric( $url ) ) {
		return false;
	}

	$original_url = $url;
	$url          = zc_kses_bad_protocol( $url, array( 'http', 'https' ) );
...
function allowed_http_request_hosts( $is_external, $host ) {
	if ( ! $is_external && zc_validate_redirect( 'http://' . $host ) ) {
		$is_external = true;
	}
	return $is_external;
}
 ...
 * )
 */
function zc_parse_url( $url, $component = -1 ) {
	$to_unset = array();
	$url      = (string) $url;
...
function _get_component_from_parsed_url_array( $url_parts, $component = -1 ) {
	...
	$key = _zc_translate_php_url_constant_to_key( $component );
 ...
 */
function _zc_translate_php_url_constant_to_key( $constant ) {
	$translation = array(
