...
use ZelocoreCMS\AiClient\Providers\Http\DTO\ApiKeyRequestAuthentication;
 ...
 * @since 7.0.0
 *
 * @see ZC_Connector_Registry::is_registered()
 *
 * @param string $id The connector identifier.
 * @return bool True if the connector is registered, false otherwise.
 */
function zc_is_connector_registered( string $id ): bool {
	$registry = ZC_Connector_Registry::get_instance();
	if ( null === $registry ) {
		return false;
	}
 ...
 * @since 7.0.0
 *
 * @see ZC_Connector_Registry::get_registered()
 *
 * @param string $id The connector identifier.
 * @return array|null {
 ...
 * }
 */
function zc_get_connector( string $id ): ?array {
	$registry = ZC_Connector_Registry::get_instance();
	if ( null === $registry ) {
		return null;
	}
 ...
 * @since 7.0.0
 *
 * @see ZC_Connector_Registry::get_all_registered()
 *
 * @return array {
 ...
 * }>
 */
function zc_get_connectors(): array {
	$registry = ZC_Connector_Registry::get_instance();
	if ( null === $registry ) {
		return array();
	}
 ...
 */
function _zc_connectors_resolve_ai_provider_logo_url( string $path ): ?string {
	if ( ! $path ) {
		return null;
	}
	...
	if ( str_starts_with( $path, $mu_plugin_dir . '/' ) ) {
		$logo_url = plugins_url( substr( $path, strlen( $mu_plugin_dir ) ), ZCMU_PLUGIN_DIR . '/.' );
 ...
 * and then fires the `zc_connectors_init` action for plugins to register their own connectors.
 *
 * @since 7.0.0
 * @access private
 */
function _zc_connectors_init(): void {
	$registry = new ZC_Connector_Registry();
	ZC_Connector_Registry::set_instance( $registry );

	// Only register default AI providers if AI support is enabled.
	if ( zc_supports_ai() ) {
		_zc_connectors_register_default_ai_providers( $registry );
	}
	...
	$registry->register(
		...
		array(
			...
			'authentication' => array(
				...
				'setting_name'    => 'zelocorecms_api_key',
	 ...
	 *
	 *     add_action( 'zc_connectors_init', function ( ZC_Connector_Registry $registry ) {
	 ...
	 * @since 7.0.0
	 *
	 * @param ZC_Connector_Registry $registry Connector registry instance.
	 */
	do_action( 'zc_connectors_init', $registry );
}
 ...
 * @access private
 *
 * @param ZC_Connector_Registry $registry The connector registry instance.
 */
function _zc_connectors_register_default_ai_providers( ZC_Connector_Registry $registry ): void {
	// Built-in connectors.
	$defaults = array(
		'anthropic' => array(
			'name'           => 'Anthropic',
	...
	foreach ( array_filter( $ai_registry->getRegisteredProviderIds() ) as $connector_id ) {
		...
		$logo_url    = $provider_metadata->getLogoPath()
			? _zc_connectors_resolve_ai_provider_logo_url( $provider_metadata->getLogoPath() )
			: null;
 ...
 */
function _zc_connectors_mask_api_key( string $key ): string {
	if ( strlen( $key ) <= 4 ) {
		return $key;
	}
 ...
 */
function _zc_connectors_get_api_key_source( string $setting_name, string $env_var_name = '', string $constant_name = '' ): string {
...
function _zc_connectors_is_ai_api_key_valid( string $key, string $provider_id ): ?bool {
	try {
		...
		zc_trigger_error( __FUNCTION__, $e->getMessage() );
		return null;
	}
}
 ...
 * @param ZC_REST_Server   $server   The server instance.
...
function _zc_connectors_rest_settings_dispatch( ZC_REST_Response $response, ZC_REST_Server $server, ZC_REST_Request $request ): ZC_REST_Response {
	...
	foreach ( zc_get_connectors() as $connector_id => $connector_data ) {
		...
		if ( $is_update && is_string( $value ) && '' !== $value && 'ai_provider' === $connector_data['type'] ) {
			if ( true !== _zc_connectors_is_ai_api_key_valid( $value, $connector_id ) ) {
		...
		if ( is_string( $value ) && '' !== $value ) {
			$data[ $setting_name ] = _zc_connectors_mask_api_key( $value );
		}
	}
...
add_filter( 'rest_post_dispatch', '_zc_connectors_rest_settings_dispatch', 10, 3 );
 ...
 * @access private
 */
function _zc_register_default_connector_settings(): void {
	$registered_settings = get_registered_settings();

	foreach ( zc_get_connectors() as $connector_data ) {
		$auth = $connector_data['authentication'];
		if ( 'api_key' !== $auth['method'] || empty( $auth['setting_name'] ) ) {
			continue;
		}
 ...
 * @access private
 */
function _zc_connectors_pass_default_keys_to_ai_client(): void {
	try {
		$ai_registry = AiClient::defaultRegistry();
		foreach ( zc_get_connectors() as $connector_id => $connector_data ) {
			...
			$key_source = _zc_connectors_get_api_key_source( $auth['setting_name'], $auth['env_var_name'] ?? '', $auth['constant_name'] ?? '' );
...
add_action( 'init', '_zc_connectors_pass_default_keys_to_ai_client', 20 );

/**
 * Exposes connector settings to the connectors-zc-admin script module.
 *
 * @since 7.0.0
 * @access private
 *
 ...
 */
function _zc_connectors_get_connector_script_module_data( array $data ): array {
	$registry = AiClient::defaultRegistry();
	...
	$data['isFileModDisabled'] = ! zc_is_file_mod_allowed( 'install_plugins' );
	return $data;
}
add_filter( 'script_module_data_options-connectors-zc-admin', '_zc_connectors_get_connector_script_module_data' );
