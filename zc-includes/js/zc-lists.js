/**
 * @output zc-includes/js/zc-lists.js
 */
...
wpList = {
	 ...
	 *
	 * Example 1: data-zc-lists="delete:the-comment-list:comment-{comment_ID}:66cc66:unspam=1"
	 * Example 2: data-zc-lists="dim:the-comment-list:comment-{comment_ID}:unapproved:e7e7d3:e7e7d3:new=approved"
	 *
	...
	parseData: function( element, type ) {
		...
		try {
			wpListsData = $( element ).data( 'zc-lists' ) || '';
	...
	process: function( elementId ) {
		...
		$element.on( 'submit', 'form[data-zc-lists^="add:' + list.id + ':"]', function() {
			return list.wpList.add( this );
		} );

		$element.on( 'click', '[data-zc-lists^="add:' + list.id + ':"], input[data-zc-lists^="add:' + list.id + ':"]', function() {
			return list.wpList.add( this );
		} );

		$element.on( 'click', '[data-zc-lists^="delete:' + list.id + ':"]', function() {
			return list.wpList.del( this );
		} );

		$element.on( 'click', '[data-zc-lists^="dim:' + list.id + ':"]', function() {
