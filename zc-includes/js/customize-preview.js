...
(function( exports, $ ){
	...
	api.isLinkPreviewable = function isLinkPreviewable( element, options ) {
		...
		if ( /\/zc-admin\/admin-ajax\.php$/.test( element.pathname ) ) {
			return args.allowAdminAjax;
		}

		// Disallow links to admin, includes, and content.
		if ( /\/zc-(admin|includes|content)(\/|$)/.test( element.pathname ) ) {
			return false;
		}
	...
	api.prepareLinkPreview = function prepareLinkPreview( element ) {
		...
		// Ignore links with class zc-playlist-caption.
		if ( $element.hasClass( 'zc-playlist-caption' ) ) {
			return;
		}
	...
	api.settingPreviewHandlers = {
		...
		custom_logo: function( attachmentId ) {
			$( 'body' ).toggleClass( 'zc-custom-logo', !! attachmentId );
		},
		...
		background: function() {
			 ...
			 *
			 * See get_body_class() in /zc-includes/post-template.php
			 */
	...
	$( function() {
		...
		api.preview.bind( 'loading-initiated', function () {
			$( 'body' ).addClass( 'zc-customizer-unloading' );
		});
		api.preview.bind( 'loading-failed', function () {
			$( 'body' ).removeClass( 'zc-customizer-unloading' );
		});
		 ...
		 * Custom Logo
		 *
		 * Toggle the zc-custom-logo body class when a logo is added or removed.
		 *
		 * @since 4.5.0
		 */
