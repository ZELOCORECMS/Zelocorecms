...
( function( $, wpLinkL10n, zc ) {
	...
	window.wpLink = {
		...
		init: function() {
			inputs.wrap = $('#zc-link-wrap');
			inputs.dialog = $( '#zc-link' );
			inputs.backdrop = $( '#zc-link-backdrop' );
			inputs.submit = $( '#zc-link-submit' );
			inputs.close = $( '#zc-link-close' );

			// Input.
			inputs.text = $( '#zc-link-text' );
			inputs.url = $( '#zc-link-url' );
			inputs.nonce = $( '#_ajax_linking_nonce' );
			inputs.openInNewTab = $( '#zc-link-target' );
			inputs.search = $( '#zc-link-search' );
			...
			inputs.close.add( inputs.backdrop ).add( '#zc-link-cancel button' ).on( 'click', function( event ) {
				event.preventDefault();
				wpLink.close();
			});
		...
		mceUpdate: function() {
			...
			editor.undoManager.transact( function() {
				if ( ! $link.length ) {
					editor.execCommand( 'mceInsertLink', false, { href: '_zc_link_placeholder', 'data-zc-temp-link': 1 } );
					$link = editor.$( 'a[data-zc-temp-link="1"]' ).removeAttr( 'data-zc-temp-link' );
		...
		keydown: function( event ) {
			...
			// Tab key.
				...
				// zc-link-submit must always be the last focusable element in the dialog.
				// Following focusable elements will be skipped on keyboard navigation.
				if ( id === 'zc-link-submit' && ! event.shiftKey ) {
					inputs.close.trigger( 'focus' );
					event.preventDefault();
				} else if ( id === 'zc-link-close' && event.shiftKey ) {
	...
	River = function( element, search ) {
		...
		$( '#zc-link .query-results, #zc-link #link-selector' ).on( 'scroll', function() {
			self.maybeLoad();
		});
