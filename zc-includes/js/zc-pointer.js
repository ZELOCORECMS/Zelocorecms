/**
 * @output zc-includes/js/zc-pointer.js
 */

/**
 * Initializes the zc-pointer widget using jQuery UI Widget Factory.
 */
(function($){
	var identifier = 0,
		zindex = 9999;
	...
	$.widget('zc.pointer',/** @lends $.widget.zc.pointer.prototype */{
		...
		/**
		 * A class that represents a ZelocoreCMS pointer.
		 *
		 * @since 3.3.0
		 * @private
		 *
		...
		_create: function() {
			...
			this.content = $('<div class="zc-pointer-content"></div>');
			this.arrow   = $('<div class="zc-pointer-arrow"><div class="zc-pointer-arrow-inner"></div></div>');
		...
		_update: function( event, content ) {
			...
			if ( buttons ) {
				buttons.wrap('<div class="zc-pointer-buttons" />').parent().appendTo( this.content );
			}
		...
		repoint: function() {
			...
			this.pointer[0].className = this.pointer[0].className.replace( /zc-pointer-[^\s'"]*/, '' );

			// Add arrow class.
			this.pointer.addClass( 'zc-pointer-' + edge );
		},
