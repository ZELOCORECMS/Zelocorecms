/**
 * @output zc-includes/js/zc-api.js
 */
	...
	zc.api.utils.addMixinsAndHelpers = function( model, modelClassName, loadingObjects ) {
		...
		var hasDate = false,
			...
			TimeStampedMixin = {
				 ...
				 *
				 * @param {Date}   date   A JavaScript date object. ZelocoreCMS expects dates in UTC.
				 ...
				 *
				 * ZelocoreCMS returns 'date' and 'date_modified' in the timezone of the server as well as
			...
			sync: function( method, model, options ) {
				...
				if ( _.isFunction( model.nonce ) && ! _.isEmpty( model.nonce() ) ) {
					...
					options.beforeSend = function( xhr ) {
						xhr.setRequestHeader( 'X-ZC-Nonce', model.nonce() );
					...
					options.complete = function( xhr ) {
						var returnedNonce = xhr.getResponseHeader( 'X-ZC-Nonce' );
				...
				if ( 'read' === method ) {
					...
					options.success = function( data, textStatus, request ) {
						if ( ! _.isUndefined( request ) ) {
							self.state.totalPages   = parseInt( request.getResponseHeader( 'x-zc-totalpages' ), 10 );
							self.state.totalObjects = parseInt( request.getResponseHeader( 'x-zc-total' ), 10 );
						}
...
( function() {
	...
	if ( _.isEmpty( wpApiSettings ) ) {
		wpApiSettings.root = window.location.origin + '/zc-json/';
	}
	...
	Endpoint = Backbone.Model.extend(/** @lends Endpoint.prototype */{
		...
		initialize: function() {
			...
			if ( model.get( 'schema' ) ) {
				...
				sessionStorage.getItem( 'zc-api-schema-model' + model.get( 'apiRoot' ) + model.get( 'versionString' ) )
			) {

				// Used a cached copy of the schema model if available.
				model.schemaModel.set( model.schemaModel.parse( JSON.parse( sessionStorage.getItem( 'zc-api-schema-model' + model.get( 'apiRoot' ) + model.get( 'versionString' ) ) ) ) );
			} else {
				model.schemaModel.fetch( {
					...
					success: function( newSchemaModel ) {
						...
						if ( ! _.isUndefined( sessionStorage ) && ( _.isUndefined( wpApiSettings.cacheSchema ) || wpApiSettings.cacheSchema ) ) {
							try {
								sessionStorage.setItem( 'zc-api-schema-model' + model.get( 'apiRoot' ) + model.get( 'versionString' ), JSON.stringify( newSchemaModel ) );
							} catch ( error ) {
