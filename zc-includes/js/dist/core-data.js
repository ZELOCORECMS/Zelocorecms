...
(wp ||= {}).coreData = (() => {
  ...
  var SYNC_API_PATH = "/zc-sync/v1/updates";
  ...
  function createUndoManager() {
    ...
    return {
       ...
       * Since Yjs automatically tracks changes, this method translates the ZelocoreCMS
  ...
  var rootEntitiesConfig = [
    {
      ...
      baseURLParams: {
        ...
        // @see lib/compat/zelocorecms-7.0/preload.php
        _fields: [
          "description",
          "gmt_offset",
          "home",
  ...
  async function loadPostTypeEntities() {
    ...
    return Object.entries(postTypes ?? {}).map(([name, postType]) => {
      ...
      const entity2 = {
        ...
        revisionKey: isTemplate && !window?.__experimentalTemplateActivate ? "zc_id" : DEFAULT_ENTITY_KEY
      };
      entity2.syncConfig = {
        /**
         ...
         *
         * @param {import('@zelocorecms/sync').CRDTDoc}               crdtDoc
         * @param {Partial< import('@zelocorecms/sync').ObjectData >} changes
         * @return {void}
         */
         ...
         *
         * @param {import('@zelocorecms/sync').CRDTDoc}  ydoc
         * @param {import('@zelocorecms/sync').ObjectID} objectId
         * @return {import('@zelocorecms/sync').Awareness} Awareness instance
         */
         ...
         *
         * @param {import('@zelocorecms/sync').CRDTDoc}    crdtDoc
         * @param {import('@zelocorecms/sync').ObjectData} editedRecord
         * @return {Partial< import('@zelocorecms/sync').ObjectData >} Changes to record
         */
         ...
         * local editor state.
         *
         * @param {import('@zelocorecms/sync').ObjectData} record
         * @return {Partial< import('@zelocorecms/sync').ObjectData >} Changes to record
         */
  ...
  var getHomePage = (0, import_data9.createRegistrySelector)(
    (select5) => (0, import_data9.createSelector)(
      () => {
        ...
        return { postType: "zc_template", postId: frontPageTemplateId };
      },
      (state) => [
  ...
  var getTemplateId = (0, import_data9.createRegistrySelector)(
    (select5) => (state, postType, postId) => {
      ...
      if (currentTemplateSlug) {
        const currentTemplate = select5(STORE_NAME).getEntityRecords("postType", "zc_template", {
          per_page: -1
  ...
  var deleteEntityRecord = (kind, name, recordId, query, { __unstableFetch = import_api_fetch4.default, throwOnError = false } = {}) => async ({ dispatch: dispatch3, resolveSelect: resolveSelect2 }) => {
    ...
    try {
      ...
      if (kind === "postType" && name === "zc_template" && (recordId && typeof recordId === "string" && !/^\d+$/.test(recordId) || !window?.__experimentalTemplateActivate)) {
  ...
  var fetchUrlData = async (url, options = {}) => {
    const endpoint = "/zc-block-editor/v1/url-details";
    const args2 = {
  ...
  var getEntityRecord2 = (kind, name, key = "", query) => async ({ select: select5, dispatch: dispatch3, registry, resolveSelect: resolveSelect2 }) => {
    ...
    try {
      ...
      let { baseURL } = entityConfig;
      if (kind === "postType" && name === "zc_template" && (key && typeof key === "string" && !/^\d+$/.test(key) || !window?.__experimentalTemplateActivate)) {
        baseURL = baseURL.slice(0, baseURL.lastIndexOf("/")) + "/templates";
      }
  ...
  getEntityRecord2.shouldInvalidate = (action, kind, name) => {
    ...
    // have persistedEdits.
    action.persistedEdits && action.persistedEdits.status !== "auto-draft" || action.type === "REMOVE_ITEMS") && action.kind === "postType" && action.name === "zc_template";
  };
  ...
  var getEntityRecords2 = (kind, name, query = {}) => async ({ dispatch: dispatch3, registry, resolveSelect: resolveSelect2 }) => {
    ...
    try {
      ...
      if (kind === "postType" && name === "zc_template" && combinedTemplates) {
        baseURL = baseURL.slice(0, baseURL.lastIndexOf("/")) + "/templates";
      }
      ...
      if (entityConfig.supportsPagination && query.per_page !== -1) {
        ...
        meta = {
          totalItems: parseInt(
            response.headers.get("X-ZC-Total")
          ),
          totalPages: parseInt(
            response.headers.get("X-ZC-TotalPages")
          )
        };
  ...
  var getNavigationFallbackId2 = () => async ({ dispatch: dispatch3, select: select5, registry }) => {
    const fallback = await (0, import_api_fetch9.default)({
      path: (0, import_url6.addQueryArgs)("/zc-block-editor/v1/navigation-fallback", {
        _embed: true
      })
    });
  ...
  var getEditorSettings2 = () => async ({ dispatch: dispatch3 }) => {
    const settings = await (0, import_api_fetch9.default)({
      path: "/zc-block-editor/v1/settings"
    });
  ...
  var getEditorAssets2 = () => async ({ dispatch: dispatch3 }) => {
    const assets = await (0, import_api_fetch9.default)({
      path: "/zc-block-editor/v1/assets"
    });
