...
(zc ||= {}).listReusableBlocks = (() => {
  ...
  async function exportReusableBlock(id) {
    ...
    const syncStatus = post.zc_pattern_sync_status;
  ...
  async function importReusableBlock(file) {
    ...
    const reusableBlock = await (0, import_api_fetch2.default)({
      ...
      data: {
        ...
        status: "publish",
        meta: parsedContent.syncStatus === "unsynced" ? { zc_pattern_sync_status: parsedContent.syncStatus } : void 0
      },
      method: "POST"
    });
  ...
  document.body.addEventListener("click", (event) => {
    if (!event.target.classList.contains("zc-list-reusable-blocks__export")) {
      return;
    }
    event.preventDefault();
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    const showNotice = () => {
      ...
      const headerEnd = document.querySelector(".zc-header-end");
      if (!headerEnd) {
        return;
      }
