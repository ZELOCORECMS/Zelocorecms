...
(wp ||= {}).reusableBlocks = (() => {
  ...
  var __experimentalConvertBlocksToReusable = (clientIds, title, syncType) => async ({ registry, dispatch }) => {
    ...
    const updatedRecord = await registry.dispatch("core").saveEntityRecord("postType", "zc_block", reusableBlock);
  ...
  var __experimentalDeleteReusableBlock = (id) => async ({ registry }) => {
    const reusableBlock = registry.select("core").getEditedEntityRecord("postType", "zc_block", id);
    if (!reusableBlock) {
      return;
    }
    ...
    await registry.dispatch("core").deleteEntityRecord("postType", "zc_block", id);
  };
