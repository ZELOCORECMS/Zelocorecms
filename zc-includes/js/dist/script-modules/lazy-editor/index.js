...
var PRESET_METADATA = [
  ...
  },
  {
    ...
    valueKey: "colors",
    cssVarInfix: "duotone",
    valueFunc: ({ slug }) => `url( '#zc-duotone-${slug}' )`,
    classes: []
  },
  {
    path: ["shadow", "presets"],
...
function getBlockSelector(blockType, target = "root", options = {}) {
  ...
  if (hasSelectors && selectors.root) {
    ...
    rootSelector = ".zc-block-" + name.replace("core/", "").replace("/", "-");
  }
  if (path === "root") {
    return rootSelector;
  }
...
function getSpacingPresetCssVar(value) {
  ...
  return `var(--zc--preset--spacing--${slug[1]})`;
}
        ...
        if (presetByOrigin[origin]) {
          presetByOrigin[origin].forEach((value) => {
            if (valueKey && !valueFunc) {
              declarations.push(
                `--zc--preset--${cssVarInfix}--${kebabCase(
                  value.slug
                )}: ${value[valueKey]}`
              );
            ...
            ({ slug }) => {
              classes.forEach(
                ({
                  ...
                  const value = `var(--zc--preset--${cssVarInfix}--${kebabCase(
                    slug
                  )})`;
...
var updateParagraphTextIndentSelector = (featureDeclarations, settings, blockName) => {
  ...
  const oldSelector = ".zc-block-paragraph + .zc-block-paragraph";
  const newSelector = ".zc-block-paragraph";
...
function getStylesDeclarations(blockStyles = {}, selector = "", useRootPaddingAlign, tree = {}, disableRootPadding = false) {
  ...
  const output = Object.entries(
    ...
    (declarations, [key, { value, properties, useEngine, rootOnly }]) => {
      ...
      if (key === "--zc--style--root--padding" && (typeof styleValue === "string" || !useRootPaddingAlign)) {
        return declarations;
      }
...
function getLayoutStyles({
  ...
  if (gapValue && layoutDefinitions) {
    ...
    if (selector === ROOT_BLOCK_SELECTOR && hasBlockGapSupport) {
      ruleset += `${ROOT_CSS_PROPERTIES_SELECTOR} { --zc--style--block-gap: ${gapValue}; }`;
    }
  }
...
var generateCustomProperties = (tree, blockSelectors) => {
  ...
  settings.forEach(({ presets, custom, selector }) => {
    ...
    const customProps = flattenTree(custom, "--zc--custom--", "--");
...
var transformToStyles = (tree, blockSelectors, hasBlockGapSupport, hasFallbackGapSupport, disableLayoutStyles = false, disableRootPadding = false, styleOptions = {}) => {
  ...
  if (options.presets && (contentSize || wideSize)) {
    ...
    ruleset = contentSize ? ruleset + ` --zc--style--global--content-size: ${contentSize};` : ruleset;
    ruleset = wideSize ? ruleset + ` --zc--style--global--wide-size: ${wideSize};` : ruleset;
    ruleset += "}";
  }
  if (hasBodyStyles) {
    ruleset += ":where(body) {margin: 0;";
    if (options.rootPadding && useRootPaddingAlign) {
      ruleset += `padding-right: 0; padding-left: 0; padding-top: var(--zc--style--root--padding-top); padding-bottom: var(--zc--style--root--padding-bottom) }
				.has-global-padding { padding-right: var(--zc--style--root--padding-right); padding-left: var(--zc--style--root--padding-left); }
				.has-global-padding > .alignfull { margin-right: calc(var(--zc--style--root--padding-right) * -1); margin-left: calc(var(--zc--style--root--padding-left) * -1); }
				.has-global-padding :where(:not(.alignfull.is-layout-flow) > .has-global-padding:not(.zc-block-block, .alignfull)) { padding-right: 0; padding-left: 0; }
				.has-global-padding :where(:not(.alignfull.is-layout-flow) > .has-global-padding:not(.zc-block-block, .alignfull)) > .alignfull { margin-left: 0; margin-right: 0;
				`;
    }
    ruleset += "}";
  }
  ...
  if (options.layoutStyles) {
    ruleset = ruleset + ".zc-site-blocks > .alignleft { float: left; margin-right: 2em; }";
    ruleset = ruleset + ".zc-site-blocks > .alignright { float: right; margin-left: 2em; }";
    ruleset = ruleset + ".zc-site-blocks > .aligncenter { justify-content: center; margin-left: auto; margin-right: auto; }";
  }
  if (options.blockGap && hasBlockGapSupport) {
    const gapValue = getGapCSSValue(tree?.styles?.spacing?.blockGap) || "0.5em";
    ruleset = ruleset + `:root :where(.zc-site-blocks) > * { margin-block-start: ${gapValue}; margin-block-end: 0; }`;
    ruleset = ruleset + ":root :where(.zc-site-blocks) > :first-child { margin-block-start: 0; }";
    ruleset = ruleset + ":root :where(.zc-site-blocks) > :last-child { margin-block-end: 0; }";
  }
  if (options.presets) {
...
function injectImportMap(scriptModules) {
  ...
  const existingMapElement = document.querySelector(
    "script#zc-importmap[type=importmap]"
  );
...
function Editor({
  ...
  const templateId = (0, import_data6.useSelect)(
    (select2) => {
      ...
      if (resolvedPostType === "zc_template") {
...
if (typeof document !== "undefined" && !document.head.querySelector("style[data-zc-hash='5619aa31a1']")) {
  const style = document.createElement("style");
  style.setAttribute("data-zc-hash", "5619aa31a1");
