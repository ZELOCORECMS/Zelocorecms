...
import { store, privateApis, getConfig } from "@zelocorecms/interactivity";
...
var preloadScriptModules = (doc) => {
  ...
  const moduleUrls = [
    ...doc.querySelectorAll(
      "script[type=module][src][data-zc-router-options]"
    )
  ].filter((script) => {
    try {
      const parsed = JSON.parse(
        script.getAttribute("data-zc-router-options")
      );
...
var {
  ...
  "I acknowledge that using private APIs means my theme or plugin will inevitably break in the next version of ZelocoreCMS."
);
var regionAttr = `data-zc-router-region`;
var interactiveAttr = `data-zc-interactive`;
...
var { state, actions } = store("core/router", {
  state: {
    get navigation() {
      if (true) {
        warn(
          `The usage of state.navigation.{hasStarted|hasFinished} from core/router is deprecated and will stop working in ZelocoreCMS 7.1.`
        );
      }
...
function a11ySpeak(messageKey) {
  if (!hasLoadedNavigationTextsData) {
    ...
    const content = document.getElementById(
      "zc-script-module-data-@zelocorecms/interactivity-router"
    )?.textContent;
    if (content) {
      try {
