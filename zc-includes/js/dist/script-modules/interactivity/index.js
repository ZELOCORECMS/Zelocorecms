...
var getElement = () => {
  ...
  if (true) {
    ...
    deepReadOnlyOptions = {
      errorMessage: "Don't mutate the attributes from `getElement`, use `data-zc-bind` to modify the attributes of an element instead."
    };
  }
...
var parseServerData = (dom = document) => {
  const jsonDataScriptTag = (
    ...
    dom.getElementById(
      "zc-script-module-data-@zelocorecms/interactivity"
    ) ?? // Legacy form
    dom.getElementById("zc-interactivity-data")
  );
  ...
  ((entry, ...args) => {
    ...
    if (typeof value === "function") {
      if (hasNegationOperator) {
        warn(
          "Using a function with a negation operator is deprecated and will stop working in ZelocoreCMS 6.9. Please use derived state instead."
        );
...
var warnUniqueIdWithTwoHyphens = (prefix, suffix, uniqueId) => {
  if (true) {
    warn(
      `The usage of data-zc-${prefix}--${suffix}${uniqueId ? `--${uniqueId}` : ""} (two hyphens for unique ID) is deprecated and will stop working in ZelocoreCMS 7.1. Please use data-zc-${prefix}${uniqueId ? `--${suffix}---${uniqueId}` : `---${suffix}`} (three hyphens for unique ID) from now on.`
    );
  }
};
var warnUniqueIdNotSupported = (prefix, uniqueId) => {
  if (true) {
    warn(
      `Unique IDs are not supported for the data-zc-${prefix} directive. Ignoring the directive with unique ID "${uniqueId}".`
    );
  }
};
var warnWithSyncEvent = (wrongPrefix, rightPrefix) => {
  if (true) {
    warn(
      `The usage of data-zc-${wrongPrefix} is deprecated and will stop working in ZelocoreCMS 7.0. Please, use data-zc-${rightPrefix} with the withSyncEvent() helper from now on.`
    );
  }
};
    ...
    get(target, prop, receiver) {
      ...
      switch (prop) {
        case "currentTarget":
          if (true) {
            warn(
              `Accessing the synchronous event.${prop} property in a store action without wrapping it in withSyncEvent() is deprecated and will stop working in ZelocoreCMS 7.0. Please wrap the store action in withSyncEvent().`
            );
          }
          break;
        case "preventDefault":
        ...
        case "stopPropagation":
          if (true) {
            warn(
              `Using the synchronous event.${prop}() function in a store action without wrapping it in withSyncEvent() is deprecated and will stop working in ZelocoreCMS 7.0. Please wrap the store action in withSyncEvent().`
            );
          }
          break;
      }
      ...
      itemContexts.set(
        ...
        proxifyContext(
          proxifyState(namespace, {
            ...
            // value is set in the `zc-each` directive later on.
            [itemProp]: void 0
          }),
          inheritedValue.client[namespace]
        )
...
var directives_default = () => {
  directive(
    ...
    ({
      ...
      if (!entries.length) {
        if (true) {
          warn(
            "The usage of data-zc-context--unique-id (two hyphens) is not supported. To add a unique ID to the directive, please use data-zc-context---unique-id (three hyphens) instead."
          );
        }
        return;
      }
      ...
      entries.forEach(({ value, namespace, uniqueId }) => {
        if (!isPlainObject(value)) {
          if (true) {
            warn(
              `The value of data-zc-context${uniqueId ? `---${uniqueId}` : ""} on the ${namespace} namespace must be a valid stringified JSON object.`
            );
          }
          return;
        }
  ...
  directive(
    ...
    ({
      ...
      if (true) {
        warn(
          "The data-zc-ignore directive is deprecated and will be removed in version 7.0."
        );
      }
  ...
  directive("text", ({ directives: { text }, element, evaluate }) => {
    ...
    if (!entries.length) {
      if (true) {
        warn(
          "The usage of data-zc-text--suffix is not supported. Please use data-zc-text instead."
        );
      }
      return;
    }
  ...
  directive(
    ...
    ({
      ...
      if (element.type !== "template") {
        if (true) {
          warn(
            "The data-zc-each directive can only be used on <template> elements."
          );
        }
        return;
      }
      ...
      if (each.length > 1) {
        if (true) {
          warn(
            "The usage of multiple data-zc-each directives on the same element is not supported. Please pick only one."
          );
        }
        return;
      }
  ...
  directive(
    ...
    ({ directives: { "router-region": routerRegion } }) => {
      ...
      if (entry.suffix) {
        if (true) {
          warn(
            `Suffixes for the data-zc-router-region directive are not supported. Ignoring the directive with suffix "${entry.suffix}".`
          );
        }
        return;
      }
  ...
  function walk(node) {
    ...
    for (let i6 = 0; i6 < attributes.length; i6++) {
      ...
      if (attributeName[directivePrefix.length] && attributeName.slice(0, directivePrefix.length) === directivePrefix) {
        if (attributeName === "data-zc-ignore") {
          ignore = true;
        } else {
          ...
          if (attributeName === "data-zc-interactive") {
            island = true;
...
var hydrateRegions = async () => {
  const nodes = document.querySelectorAll(`[data-zc-interactive]`);
...
var watch = E2;
var requiredConsent = "I acknowledge that using private APIs means my theme or plugin will inevitably break in the next version of ZelocoreCMS.";
