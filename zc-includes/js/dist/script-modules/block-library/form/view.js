...
try {
  formSettings = JSON.parse(
    document.getElementById(
      "zc-script-module-data-@zelocorecms/block-library/form/view"
    )?.textContent
  );
} catch {
}
document.querySelectorAll("form.zc-block-form").forEach(function(form) {
  if (!formSettings || !form.action || !form.action.startsWith("mailto:")) {
    return;
  }
  const redirectNotification = (status) => {
    const urlParams = new URLSearchParams(window.location.search);
    urlParams.append("zc-form-result", status);
  ...
  form.addEventListener("submit", async function(event) {
    ...
    formData._zc_http_referer = window.location.href;
    formData.formAction = form.action;
    try {
