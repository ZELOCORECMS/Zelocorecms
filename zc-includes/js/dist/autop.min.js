...
`)),e}function j(e){let n="blockquote|ul|ol|li|dl|dt|dd|table|thead|tbody|tfoot|tr|th|td|h[1-6]|fieldset|figure",r=n+"|div|p",c=n+"|pre",i=[],s=!1,a=!1;return e?((e.indexOf("<script")!==-1||e.indexOf("<style")!==-1)&&(e=e.replace(/<(script|style)[^>]*>[\s\S]*?<\/\1>/g,p=>(i.push(p),"<zc-preserve>"))),e.indexOf("<pre")!==-1&&(s=!0,e=e.replace(/<pre[^>]*>[\s\S]+?<\/pre>/g,p=>(p=p.replace(/<br ?\/?>(\r\n|\n)?/g,"<zc-line-break>"),p=p.replace(/<\/?p( [^>]*)?>(\r\n|\n)?/g,"<zc-line-break>"),p.replace(/\r?\n/g,"<zc-line-break>")))),e.indexOf("[caption")!==-1&&(a=!0,e=e.replace(/\[caption[\s\S]+?\[\/caption\]/g,p=>p.replace(/<br([^>]*)>/g,"<zc-temp-br$1>").replace(/[\r\n\t]+/,""))),e=e.replace(new RegExp("\\s*</("+r+")>\\s*","g"),`</$1>
...
$1`),e=e.replace(/^\s+/,""),e=e.replace(/[\s\u00a0]+$/,""),s&&(e=e.replace(/<zc-line-break>/g,`
`)),a&&(e=e.replace(/<zc-temp-br([^>]*)>/g,"<br$1>")),i.length&&(e=e.replace(/<zc-preserve>/g,()=>i.shift())),e):""}return v(S);})();
