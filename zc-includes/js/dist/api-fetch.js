...
(wp ||= {}).apiFetch = (() => {
  ...
  function createNonceMiddleware(nonce) {
    const middleware = (options, next) => {
      ...
      for (const headerName in headers) {
        if (headerName.toLowerCase() === "x-zc-nonce" && headers[headerName] === middleware.nonce) {
          return next(options);
        }
      }
      return next({
  ...
  var DEFAULT_HEADERS = {
    ...
    //
    // See: https://core.trac.zelocorecms.org/ticket/44534
