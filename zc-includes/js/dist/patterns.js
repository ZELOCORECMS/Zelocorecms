...
(wp ||= {}).patterns = (() => {
  ...
  var createPattern = (title, syncType, content, categories) => async ({ registry }) => {
    ...
    const reusableBlock = {
      ...
      status: "publish",
      meta,
      zc_pattern_category: categories
    };
    const updatedRecord = await registry.dispatch(import_core_data.store).saveEntityRecord("postType", "zc_block", reusableBlock);
  ...
  var CATEGORY_SLUG = "zc_pattern_category";
  ...
  function getTermLabels(pattern, categories) {
    ...
    return categories.user?.filter(
      (category) => pattern.zc_pattern_category?.includes(category.id)
    ).map((category) => category.label);
  }
  function useDuplicatePatternProps({ pattern, onSuccess }) {
    ...
    return {
      ...
      defaultSyncType: pattern.type !== PATTERN_TYPES.user ? PATTERN_SYNC_TYPES.unsynced : pattern.zc_pattern_sync_status || PATTERN_SYNC_TYPES.full,
  ...
  function PatternConvertButton({
    ...
    const canConvert = (0, import_data8.useSelect)(
      (select) => {
        ...
        const _canConvert = (
          ...
          // Blocks refers to the zc_block post type, this checks the ability to create a post of that type.
          !!canUser("create", {
            kind: "postType",
            name: "zc_block"
          })
    ...
    const handleSuccess = ({ pattern }) => {
      if (pattern.zc_pattern_sync_status === PATTERN_SYNC_TYPES.unsynced) {
      ...
      createSuccessNotice(
        pattern.zc_pattern_sync_status === PATTERN_SYNC_TYPES.unsynced ? (0, import_i18n6.sprintf)(
