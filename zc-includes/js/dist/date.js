...
(wp ||= {}).date = (() => {
  ...
  function isInTheFuture(dateValue) {
    const now = import_moment.default.tz(ZC_ZONE);
    const momentObject = import_moment.default.tz(dateValue, ZC_ZONE);
    return momentObject.isAfter(now);
  }
  function getDate(dateString) {
    if (!dateString) {
      return import_moment.default.tz(ZC_ZONE).toDate();
    }
    return import_moment.default.tz(dateString, ZC_ZONE).toDate();
  }
  function humanTimeDiff(from, to) {
    const fromMoment = import_moment.default.tz(from, ZC_ZONE);
    const toMoment = to ? import_moment.default.tz(to, ZC_ZONE) : import_moment.default.tz(ZC_ZONE);
