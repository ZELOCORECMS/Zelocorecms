...
(zc ||= {}).coreCommands = (() => {
  ...
  // package-external:@zelocorecms/router
  var require_router = __commonJS({
    "package-external:@zelocorecms/router"(exports, module) {
      module.exports = window.zc.router;
    }
  });
  ...
  var icons = {
    post: post_default,
    page: page_default,
    zc_template: layout_default,
    zc_template_part: symbol_filled_default
  };
  ...
  function mapRoute(path) {
    ...
    for (const [oldPath, newPath] of Object.entries(ROUTE_MAPPING)) {
      if (path === oldPath || path.startsWith(oldPath + "?")) {
        if (path.includes("postType=zc_template_part")) {
  ...
  var getNavigationCommandLoaderPerTemplate = (templateType) => function useNavigationCommandLoader({ search }) {
    ...
    const commands = (0, import_element2.useMemo)(() => {
      if (!canCreateTemplate || !isBlockBasedTheme && !templateType === "zc_template_part") {
        return [];
      }
      ...
      if (orderedRecords?.length > 0 && templateType === "zc_template_part") {
        result.push({
          ...
          callback: ({ close }) => {
            if (isSiteEditor) {
              history.navigate(
                mapRoute(
                  "/pattern?postType=zc_template_part&categoryId=all-parts"
                )
              );
            } else {
              document.location = (0, import_url.addQueryArgs)(
        ...
        result.push({
          ...
          callback: ({ close }) => {
            if (canCreateTemplate) {
              ...
              document.location.href = "edit.php?post_type=zc_block";
            }
          }
        });
      }
  ...
  function useSiteEditorNavigationCommands(isNetworkAdmin) {
    ...
    (0, import_commands2.useCommandLoader)({
      ...
      hook: getNavigationCommandLoaderPerTemplate("zc_template"),
      disabled: isNetworkAdmin
    });
    (0, import_commands2.useCommandLoader)({
      name: "core/edit-site/navigate-template-parts",
      hook: getNavigationCommandLoaderPerTemplate("zc_template_part"),
      disabled: isNetworkAdmin
    });
    (0, import_commands2.useCommandLoader)({
