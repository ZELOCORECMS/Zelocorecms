...
(wp ||= {}).dataControls = (() => {
  ...
  function select(storeNameOrDescriptor, selectorName, ...args) {
    (0, import_deprecated.default)("`select` control in `@zelocorecms/data-controls`", {
      since: "5.7",
      alternative: "built-in `resolveSelect` control in `@zelocorecms/data`"
    });
  ...
  function syncSelect(storeNameOrDescriptor, selectorName, ...args) {
    (0, import_deprecated.default)("`syncSelect` control in `@zelocorecms/data-controls`", {
      since: "5.7",
      alternative: "built-in `select` control in `@zelocorecms/data`"
    });
    return import_data.controls.select(storeNameOrDescriptor, selectorName, ...args);
  }
  function dispatch(storeNameOrDescriptor, actionName, ...args) {
    (0, import_deprecated.default)("`dispatch` control in `@zelocorecms/data-controls`", {
      since: "5.7",
      alternative: "built-in `dispatch` control in `@zelocorecms/data`"
    });
