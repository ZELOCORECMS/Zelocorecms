 ...
 * @see zc_ajax_nopriv_heartbeat(), zc_ajax_heartbeat()
 *
 ...
 *
 * @since 3.6.0
 * @output zc-includes/js/heartbeat.js
 */
	...
	var Heartbeat = function() {
		...
		function initialize() {
			...
			if ( hidden ) {
				...
				$document.on( visibilitychange + '.zc-heartbeat', function() {
			...
			$(window).on( 'pagehide.zc-heartbeat', function() {
		...
		function connect() {
			...
			settings.xhr = $.ajax({
				...
				// Update the Rest API nonce if set and zc-api loaded.
		...
		function userIsActive() {
			...
			$document.off( '.zc-heartbeat-active' );

			$('iframe').each( function( i, frame ) {
				if ( isLocalFrame( frame ) ) {
					$( frame.contentWindow ).off( '.zc-heartbeat-active' );
				}
			});
		...
		function checkUserActivity() {
			...
			if ( ! settings.userActivityEvents ) {
				$document.on( 'mouseover.zc-heartbeat-active keyup.zc-heartbeat-active touchend.zc-heartbeat-active', function() {
					userIsActive();
				});

				$('iframe').each( function( i, frame ) {
					if ( isLocalFrame( frame ) ) {
						$( frame.contentWindow ).on( 'mouseover.zc-heartbeat-active keyup.zc-heartbeat-active touchend.zc-heartbeat-active', function() {
							userIsActive();
						});
					}
				});
