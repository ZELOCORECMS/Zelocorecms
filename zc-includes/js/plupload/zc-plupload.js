...
( function( exports, $ ) {
	...
	Uploader = function( options ) {
		...
		for ( key in elements ) {
			...
			if ( ! this[ key ].prop('id') ) {
				this[ key ].prop( 'id', '__zc-uploader-id-' + Uploader.uuid++ );
			}
		...
		tryAgain = function( message, data, file ) {
			...
			id = data.responseHeaders.match( /x-zc-upload-attachment-id:\s*(\d+)/i );
			...
			if ( times && times > 4 ) {
				...
				$.ajax({
					...
					data: {
						...
						attachment_id: id,
						_zc_upload_failed_cleanup: true,
					}
				});
		...
		this.uploader.bind( 'init', function( uploader ) {
			...
			if ( ! dragdrop ) {
				return dropzone.unbind('.zc-uploader');
			}

			// 'dragenter' doesn't fire correctly, simulate it with a limited 'dragover'.
			dropzone.on( 'dragover.zc-uploader', function() {
				if ( timer ) {
					clearTimeout( timer );
				}
			...
			dropzone.on('dragleave.zc-uploader, drop.zc-uploader', function() {
				/*
				 ...
				 *
				 * @see https://core.trac.zelocorecms.org/ticket/21705
				 */
	...
	$.extend( Uploader.prototype, /** @lends zc.Uploader.prototype */{
		...
		refresh:  function() {
			...
			if ( this.browser ) {
				 ...
				 */
				if ( ! attached ) {
					id = 'zc-uploader-browser-' + this.uploader.id;

					container = $( '#' + id );
					if ( ! container.length ) {
						container = $('<div class="zc-uploader-browser" />').css({
							position: 'fixed',
							top: '-1000px',
							left: '-1000px',
							height: 0,
							width: 0
						}).attr( 'id', 'zc-uploader-browser-' + this.uploader.id ).appendTo('body');
					}
