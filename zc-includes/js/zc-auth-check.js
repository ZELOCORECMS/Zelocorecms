 ...
 *
 * @output zc-includes/js/zc-auth-check.js
 */
...
( function( $ ) {
	 ...
	 */
	function show() {
		var parent = $( '#zc-auth-check' ),
			form = $( '#zc-auth-check-form' ),
			noframe = wrap.find( '.zc-auth-fallback-expired' ),
			frame, loaded = false;
	...
	$( function() {
		 ...
		 * @since 3.6.0
		 */
		wrap = $( '#zc-auth-check-wrap' );
		wrap.find( '.zc-auth-check-close' ).on( 'click', function() {
			hide();
			setShowTimeout();
		});
	}).on( 'heartbeat-tick.zc-auth-check', function( e, data ) {
		if ( ! ( 'zc-auth-check' in data ) ) {
			return;
		}

		var showOrHide = function () {
			if ( ! data['zc-auth-check'] && wrap.hasClass( 'hidden' ) && ! tempHidden ) {
				show();
			} else if ( data['zc-auth-check'] && ! wrap.hasClass( 'hidden' ) ) {
				hide();
			}
		};
