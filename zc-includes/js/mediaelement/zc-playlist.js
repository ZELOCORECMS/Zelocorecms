...
(function ($, _, Backbone) {
	...
	var WPPlaylistView = Backbone.View.extend(/** @lends WPPlaylistView.prototype */{
		...
		initialize : function (options) {
			this.index = 0;
			this.settings = {};
			this.data = options.metadata || $.parseJSON( this.$('script.zc-playlist-script').html() );
		...
		renderCurrent : function () {
			var dimensions, defaultImage = 'zc-includes/images/media/video.svg';
		...
		renderTracks : function () {
			var self = this, i = 1, tracklist = $( '<div class="zc-playlist-tracks"></div>' );
			...
			this.$( '.zc-playlist-item' ).eq(0).addClass( this.playingClass );
		},

		events : {
			'click .zc-playlist-item' : 'clickTrack',
			'click .zc-playlist-next' : 'next',
			'click .zc-playlist-prev' : 'prev'
		},

		clickTrack : function (e) {
			e.preventDefault();

			this.index = this.$( '.zc-playlist-item' ).index( e.currentTarget );
			this.setCurrent();
		},
	 ...
	 */
	function initialize() {
		$( '.zc-playlist:not(:has(.mejs-container))' ).each( function() {
