...
// Forked for ZelocoreCMS so it can be turned on/off after loading.
...
tinymce.PluginManager.add( 'zcautoresize', function( editor ) {
	...
	function resize( e ) {
		...
		if (resizeHeight !== oldSize) {
			...
			editor.fire( 'zc-autoresize', { height: resizeHeight, deltaHeight: e.type === 'nodechange' ? deltaSize : null } );
		}
	}
	...
	function on() {
		if ( ! editor.dom.hasClass( editor.getBody(), 'zc-autoresize' ) ) {
			isActive = true;
			editor.dom.addClass( editor.getBody(), 'zc-autoresize' );
	...
	function off() {
		...
		if ( ! settings.zc_autoresize_on ) {
			isActive = false;
			doc = editor.getDoc();
			editor.dom.removeClass( editor.getBody(), 'zc-autoresize' );
