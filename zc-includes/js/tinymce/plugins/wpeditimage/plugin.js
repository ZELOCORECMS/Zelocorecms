...
tinymce.PluginManager.add( 'wpeditimage', function( editor ) {
	...
	editor.addButton( 'zc_img_remove', {
		tooltip: 'Remove',
		...
		editor.addButton( 'zc_img_' + name, {
			...
			onPostRender: function() {
				...
				editor.on( 'NodeChange', function( event ) {
					...
					node = editor.dom.getParent( event.element, '.zc-caption' ) || event.element;
	...
	function parseShortcode( content ) {
		return content.replace( /(?:<p>)?\[(?:zc_)?caption([^\]]+)\]([\s\S]+?)\[\/(?:zc_)?caption\](?:<\/p>)?/g, function( a, b, c ) {
			...
			return '<div class="mceTemp"><dl id="' + id + '" class="zc-caption ' + align + classes + '" style="width: ' + width + 'px">' +
				'<dt class="zc-caption-dt">'+ img +'</dt><dd class="zc-caption-dd">'+ caption +'</dd></dl></div>';
		});
	}

	function getShortcode( content ) {
		return content.replace( /(?:<div [^>]+mceTemp[^>]+>)?\s*(<dl [^>]+zc-caption[^>]+>[\s\S]+?<\/dl>)\s*(?:<\/div>)?/g, function( all, dl ) {
			var out = '';
			...
			out = dl.replace( /\s*<dl ([^>]+)>\s*<dt [^>]+>([\s\S]+?)<\/dt>\s*<dd [^>]+>([\s\S]*?)<\/dd>\s*<\/dl>\s*/gi, function( a, b, c, caption ) {
				...
				classes = classes.replace( /zc-caption ?|align[a-z]+ ?/gi, '' );
	...
	function extractImageData( imageNode ) {
		...
		tinymce.each( classes, function( name ) {
			...
			if ( /^zc-image/.test( name ) ) {
				metadata.attachment_id = parseInt( name.replace( 'zc-image-', '' ), 10 );
		...
		// Extract caption.
		captionBlock = dom.getParents( imageNode, '.zc-caption' );
		...
		if ( captionBlock.length ) {
			...
			caption = dom.select( 'dd.zc-caption-dd', captionBlock );
			if ( caption.length ) {
				caption = caption[0];
	...
	function updateImage( $imageNode, imageData ) {
		...
		if ( imageData.attachment_id ) {
			classes.push( 'zc-image-' + imageData.attachment_id );
		...
		if ( imageData.caption ) {
			...
			if ( captionNode ) {
				dl = dom.select( 'dl.zc-caption', captionNode );
				...
				dd = dom.select( '.zc-caption-dd', captionNode );
				...
				html =  '<dl ' + id + 'class="' + className +'" style="width: '+ width +'px">' +
					'<dt class="zc-caption-dt"></dt><dd class="zc-caption-dd">'+ imageData.caption +'</dd></dl>';
				...
				editor.$( wrap ).find( 'dt.zc-caption-dt' ).append( node );
	...
	function editImage( img ) {
		...
		frame.on( 'close', function() {
			 ...
			 */
			imageNode = editor.$( 'img[data-zc-editing]' )
			imageNode.removeAttr( 'data-zc-editing' );
		});
	...
	editor.on( 'init', function() {
		...
		// Prevent IE11 from making dl.zc-caption resizable.
		if ( tinymce.Env.ie && tinymce.Env.ie > 10 ) {
			// The 'mscontrolselect' event is supported only in IE11+.
			dom.bind( editor.getBody(), 'mscontrolselect', function( event ) {
				if ( event.target.nodeName === 'IMG' && dom.getParent( event.target, '.zc-caption' ) ) {
					// Hide the thick border with resize handles around dl.zc-caption.
					editor.getBody().focus(); // :(
				} else if ( event.target.nodeName === 'DL' && dom.hasClass( event.target, 'zc-caption' ) ) {
	...
	editor.on( 'ObjectResized', function( event ) {
		...
		if ( node.nodeName === 'IMG' ) {
			editor.undoManager.transact( function() {
				...
				if ( parent = dom.getParent( node, '.zc-caption' ) ) {
	...
	editor.on( 'pastePostProcess', function( event ) {
		...
		if ( editor.dom.getParent( editor.selection.getNode(), 'dd.zc-caption-dd' ) ) {
	...
	editor.on( 'BeforeExecCommand', function( event ) {
		...
		if ( cmd === 'mceInsertContent' || cmd === 'Indent' || cmd === 'Outdent' ) {
			...
			DL = editor.dom.getParent( node, '.zc-caption' );
	...
	editor.on( 'beforeGetContent', function( event ) {
		if ( event.format !== 'raw' ) {
			editor.$( 'img[id="__zc-temp-img-id"]' ).removeAttr( 'id' );
		}
	});
