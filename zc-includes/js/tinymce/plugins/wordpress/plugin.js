...
tinymce.PluginManager.add( 'zelocorecms', function( editor ) {
	...
	function toggleToolbars( state ) {
		...
		editor.fire( 'zc-toolbar-toggle' );
	}

	// Add the kitchen sink button :)
	editor.addButton( 'zc_adv', {
	...
	editor.on( 'PostRender', function() {
		if ( editor.getParam( 'zelocorecms_adv_hidden', true ) && getUserSetting( 'hidetb', '0' ) === '0' ) {
			toggleToolbars( 'hide' );
		} else {
	...
	editor.addCommand( 'ZC_Adv', function() {
		toggleToolbars();
	});
		...
		if ( event.content ) {
			if ( event.content.indexOf( '<!--more' ) !== -1 ) {
				...
				event.content = event.content.replace( /<!--more(.*?)-->/g, function( match, moretext ) {
					return '<img src="' + tinymce.Env.transparentSrc + '" data-zc-more="more" data-zc-more-text="' + moretext + '" ' +
						'class="zc-more-tag mce-zc-more" alt="' + title + '" data-mce-resize="false" data-mce-placeholder="1" />';
				});
			}
			...
			if ( event.content.indexOf( '<!--nextpage-->' ) !== -1 ) {
				...
				event.content = event.content.replace( /<!--nextpage-->/g,
					'<img src="' + tinymce.Env.transparentSrc + '" data-zc-more="nextpage" class="zc-more-tag mce-zc-nextpage" ' +
			...
			if ( event.content.indexOf( '<script' ) !== -1 || event.content.indexOf( '<style' ) !== -1 ) {
				event.content = event.content.replace( /<(script|style)[^>]*>[\s\S]*?<\/\1>/g, function( match, tag ) {
					return '<img ' +
						...
						'data-zc-preserve="' + encodeURIComponent( match ) + '" ' +
	...
	editor.on( 'PostProcess', function( event ) {
		if ( event.get ) {
			event.content = event.content.replace(/<img[^>]+>/g, function( image ) {
				...
				if ( image.indexOf( 'data-zc-more="more"' ) !== -1 ) {
					if ( match = image.match( /data-zc-more-text="([^"]+)"/ ) ) {
						moretext = match[1];
					}

					string = '<!--more' + moretext + '-->';
				} else if ( image.indexOf( 'data-zc-more="nextpage"' ) !== -1 ) {
					string = '<!--nextpage-->';
				} else if ( image.indexOf( 'data-zc-preserve' ) !== -1 ) {
					if ( match = image.match( / data-zc-preserve="([^"]+)"/ ) ) {
	...
	editor.on( 'ResolveName', function( event ) {
		...
		if ( event.target.nodeName === 'IMG' && ( attr = editor.dom.getAttrib( event.target, 'data-zc-more' ) ) ) {
			event.name = attr;
		}
	});

	// Register commands.
	editor.addCommand( 'ZC_More', function( tag ) {
		var parent, html, title,
			classname = 'zc-more-tag',
			dom = editor.dom,
			node = editor.selection.getNode(),
			rootNode = editor.getBody();

		tag = tag || 'more';
		classname += ' mce-zc-' + tag;
		title = tag === 'more' ? 'Read more...' : 'Next page';
		title = __( title );
		html = '<img src="' + tinymce.Env.transparentSrc + '" alt="' + title + '" class="' + classname + '" ' +
			'data-zc-more="' + tag + '" data-mce-resize="false" data-mce-placeholder="1" />';
	...
	editor.addCommand( 'ZC_Code', function() {
		editor.formatter.toggle('code');
	});

	editor.addCommand( 'ZC_Page', function() {
		editor.execCommand( 'ZC_More', 'nextpage' );
	});

	editor.addCommand( 'ZC_Help', function() {
		var access = tinymce.Env.mac ? __( 'Ctrl + Alt + letter:' ) : __( 'Shift + Alt + letter:' ),
			...
			i1 = 0,
			i2 = 0,
			labels = editor.settings.zc_shortcut_labels,
		...
		if ( dialog.$el ) {
			...
			$wrap = dialog.$el.find( '.mce-zc-help' );
	...
	// Register buttons.
	editor.addButton( 'zc_more', {
		tooltip: 'Insert Read More tag',
		onclick: function() {
			editor.execCommand( 'ZC_More', 'more' );
		}
	});

	editor.addButton( 'zc_page', {
		tooltip: 'Page break',
		onclick: function() {
			editor.execCommand( 'ZC_More', 'nextpage' );
		}
	...
	editor.addButton( 'zc_help', {
		tooltip: 'Keyboard Shortcuts',
		cmd: 'ZC_Help'
	});

	editor.addButton( 'zc_code', {
		tooltip: 'Code',
		cmd: 'ZC_Code',
		stateSelector: 'code'
	});
	...
	editor.on( 'init', function() {
		...
		dom.setAttrib( doc.documentElement, 'lang', editor.getParam( 'zc_lang_attr' ) );
		...
		if ( editor.getParam( 'zc_paste_filters', true ) ) {
	...
	editor.on( 'preInit', function() {
		...
		if ( ! editor.settings.classic_block_editor ) {
			editor.addShortcut( 'access+z', '', 'ZC_Adv' );
		}
	...
	function prepareTooltips() {
		...
		if ( editor.settings.zc_shortcut_labels ) {
			each( editor.settings.zc_shortcut_labels, function( value, tooltip ) {
