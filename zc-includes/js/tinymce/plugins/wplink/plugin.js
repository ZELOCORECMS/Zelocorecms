( function( tinymce ) {
	tinymce.ui.Factory.add( 'WPLinkPreview', tinymce.ui.Control.extend( {
		...
		renderHtml: function() {
			return (
				'<div id="' + this._id + '" class="zc-link-preview">' +
	...
	tinymce.ui.Factory.add( 'WPLinkInput', tinymce.ui.Control.extend( {
		renderHtml: function() {
			return (
				'<div id="' + this._id + '" class="zc-link-input">' +
	...
	tinymce.PluginManager.add( 'wplink', function( editor ) {
		...
		function removePlaceholderStrings( content, dataAttr ) {
			return content.replace( /(<a [^>]+>)([\s\S]*?)<\/a>/g, function( all, tag, text ) {
				if ( tag.indexOf( ' href="_zc_link_placeholder"' ) > -1 ) {
					return text;
				}
		...
		editor.addCommand( 'ZC_Link', function() {
			...
			if ( ! linkNode ) {
				removePlaceholders();
				editor.execCommand( 'mceInsertLink', false, { href: '_zc_link_placeholder' } );

				linkNode = editor.$( 'a[href="_zc_link_placeholder"]' )[0];
				editor.nodeChanged();
			}
		...
		editor.addCommand( 'zc_link_cancel', function() {
			inputInstance.reset();
		...
		editor.addCommand( 'zc_unlink', function() {
		...
		editor.addShortcut( 'meta+k', '', 'ZC_Link' );
		...
		editor.addButton( 'zc_link_preview', {
			type: 'WPLinkPreview',
		...
		editor.addButton( 'zc_link_input', {
			type: 'WPLinkInput',
			onPostRender: function() {
				...
				if ( $ && $.ui && $.ui.autocomplete ) {
					...
					.autocomplete( {
						...
						focus: function( event, ui ) {
							$input.attr( 'aria-activedescendant', 'mce-zc-autocomplete-' + ui.item.ID );
							/*
						...
						return $( '<li role="option" id="mce-zc-autocomplete-' + item.ID + '">' )
						.append( '<span>' + title + '</span>&nbsp;<span class="zc-editor-float-right">' + item.info + '</span>' )
						.appendTo( ul );
					};
				...
				tinymce.$( input ).on( 'keydown', function( event ) {
					if ( event.keyCode === 13 ) {
						editor.execCommand( 'zc_link_apply' );
						event.preventDefault();
					}
				} );
			}
		...
		editor.addButton( 'zc_link_edit', {
			tooltip: 'Edit|button', // '|button' is not displayed, only used for context.
			icon: 'dashicon dashicons-edit',
			cmd: 'ZC_Link'
		} );

		editor.addButton( 'zc_link_remove', {
			tooltip: 'Remove link',
			icon: 'dashicon dashicons-editor-unlink',
			cmd: 'zc_unlink'
		} );

		editor.addButton( 'zc_link_advanced', {
		...
		editor.addButton( 'zc_link_apply', {
			tooltip: 'Apply',
