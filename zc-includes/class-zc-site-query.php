 ...
 * @since 4.6.0
 *
 * @see ZC_Site_Query::__construct() for accepted arguments.
 */
...
class ZC_Site_Query {
	 ...
	 *                                                   Default false.
	 *     @type array           $date_query             Date query clauses to limit sites by. See ZC_Date_Query.
	 *                                                   Default null.
	 ...
	 * @since 4.6.0
	 *
	 * @see ZC_Site_Query::__construct()
	 *
	 * @param string|array $query Array or string of ZC_Site_Query arguments. See ZC_Site_Query::__construct().
	 */
	...
	/**
	 * Sets up the ZelocoreCMS query for retrieving sites.
	 *
	 * @since 4.6.0
	 *
	...
	public function get_sites() {
		 ...
		 * @since 4.6.0
		 *
		 * @param ZC_Site_Query $query Current instance of ZC_Site_Query (passed by reference).
		 */
		 ...
		 *
		 * Return a non-null value to bypass ZelocoreCMS' default site queries.
		 *
		 ...
		 * - Otherwise the filter should return an array of ZC_Site objects.
		 *
		 * Note that if the filter returns an array of site data, it will be assigned
		 * to the `sites` property of the current ZC_Site_Query instance.
		 *
		 * Filtering functions that require pagination information are encouraged to set
		 * the `found_sites` and `max_num_pages` properties of the ZC_Site_Query object,
		 * passed to the filter by reference. If ZC_Site_Query does not perform a database
		...
		$last_changed = zc_cache_get_last_changed( 'sites' );

		$cache_key   = "get_sites:$key";
		$cache_value = zc_cache_get_salted( $cache_key, 'site-queries', $last_changed );
		...
		if ( false === $cache_value ) {
			...
			zc_cache_set_salted( $cache_key, $cache_value, 'site-queries', $last_changed );
		} else {
		...
		if ( $this->query_vars['update_site_meta_cache'] ) {
			zc_lazyload_site_meta( $site_ids );
		}
		 ...
		 * @since 4.6.0
		 *
		 * @param ZC_Site[]     $_sites An array of ZC_Site objects.
		 * @param ZC_Site_Query $query  Current instance of ZC_Site_Query (passed by reference).
		 */
		$_sites = apply_filters_ref_array( 'the_sites', array( $_sites, &$this ) );

		// Convert to ZC_Site instances.
	...
	protected function get_site_ids() {
		...
		if ( ! empty( $this->query_vars['network__in'] ) ) {
			$this->sql_clauses['where']['network__in'] = 'site_id IN ( ' . implode( ',', zc_parse_id_list( $this->query_vars['network__in'] ) ) . ' )';
		}

		// Parse site network IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['network__not_in'] ) ) {
			$this->sql_clauses['where']['network__not_in'] = 'site_id NOT IN ( ' . implode( ',', zc_parse_id_list( $this->query_vars['network__not_in'] ) ) . ' )';
		}
		...
		if ( ! empty( $this->query_vars['lang__in'] ) ) {
			$this->sql_clauses['where']['lang__in'] = 'lang_id IN ( ' . implode( ',', zc_parse_id_list( $this->query_vars['lang__in'] ) ) . ' )';
		}

		// Parse site language IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['lang__not_in'] ) ) {
			$this->sql_clauses['where']['lang__not_in'] = 'lang_id NOT IN ( ' . implode( ',', zc_parse_id_list( $this->query_vars['lang__not_in'] ) ) . ' )';
		}
		...
		if ( strlen( $this->query_vars['search'] ) ) {
			...
			/**
			 * Filters the columns to search in a ZC_Site_Query search.
			 *
			 ...
			 * @param ZC_Site_Query $query          The current ZC_Site_Query instance.
			 */
		...
		if ( ! empty( $date_query ) && is_array( $date_query ) ) {
			$this->date_query = new ZC_Date_Query( $date_query, 'registered' );
	...
	private function set_found_sites() {
		...
		if ( $this->query_vars['number'] && ! $this->query_vars['no_found_rows'] ) {
			 ...
			 * @param ZC_Site_Query $site_query        The `ZC_Site_Query` instance.
			 */
