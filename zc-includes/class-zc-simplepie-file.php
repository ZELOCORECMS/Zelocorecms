<?php
/**
 * Feed API: ZC_SimplePie_File class
 *
 * @package ZelocoreCMS
 * @subpackage Feed
 * @since 4.7.0
...
class ZC_SimplePie_File extends SimplePie\File {
	...
	public function __construct( $url, $timeout = 10, $redirects = 5, $headers = null, $useragent = null, $force_fsockopen = false ) {
		...
		if ( preg_match( '/^http(s)?:\/\//i', $url ) ) {
			...
			if ( is_zc_error( $res ) ) {
				...
				$this->headers = zc_remote_retrieve_headers( $res );
				 ...
				 * but `zc_remote_retrieve_headers()` returns them as an array, so they need
				 * to be converted.
				 *
				...
				$this->body        = zc_remote_retrieve_body( $res );
				$this->status_code = zc_remote_retrieve_response_code( $res );
			}
		} else {
			$this->error   = '';
			$this->success = false;
