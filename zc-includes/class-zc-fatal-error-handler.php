<?php
/**
 * Error Protection API: ZC_Fatal_Error_Handler class
 *
 * @package ZelocoreCMS
 * @since 5.2.0
 */
 ...
 * implementation for the fatal error handler that ZelocoreCMS registers. The custom class should extend this class and
...
class ZC_Fatal_Error_Handler {
	...
	public function handle() {
		if ( defined( 'ZC_SANDBOX_SCRAPING' ) && ZC_SANDBOX_SCRAPING ) {
			return;
		}
		...
		try {
			...
			if ( ! isset( $GLOBALS['zc_locale'] ) && function_exists( 'load_default_textdomain' ) ) {
				load_default_textdomain();
			}

			$handled = false;

			if ( ! is_multisite() && zc_recovery_mode()->is_initialized() ) {
				$handled = zc_recovery_mode()->handle_error( $error );
			}
	...
	/**
	 * Determines whether we are dealing with an error that ZelocoreCMS should handle
	 * in order to protect the admin backend against WSODs.
	 *
	 * @since 5.2.0
	 *
	 * @param array $error Error information retrieved from `error_get_last()`.
	 * @return bool Whether ZelocoreCMS should handle this error.
	 */
	protected function should_handle_error( $error ) {
		 ...
		 *
		 * This filter is only fired if the error is not already configured to be handled by ZelocoreCMS core. As such,
		 ...
		 * @param array $error               Error information retrieved from `error_get_last()`.
		 */
		return (bool) apply_filters( 'zc_should_handle_php_error', false, $error );
	}
	 ...
	 * very early in the ZelocoreCMS bootstrap process, so any core functions used that are not part of
	 * `zc-includes/load.php` should be checked for before being called.
	 *
	 * If no such drop-in is available, this will call {@see ZC_Fatal_Error_Handler::display_default_error_template()}.
	 *
	 * @since 5.2.0
	 * @since 5.3.0 The `$handled` parameter was added.
	 *
	 * @param array         $error   Error information retrieved from `error_get_last()`.
	 * @param true|ZC_Error $handled Whether Recovery Mode handled the fatal error.
	 */
	protected function display_error_template( $error, $handled ) {
		if ( defined( 'ZC_CONTENT_DIR' ) ) {
			// Load custom PHP error template, if present.
			$php_error_pluggable = ZC_CONTENT_DIR . '/php-error.php';
	 ...
	 *
	 * It calls {@see zc_die()} with a message indicating that the site is experiencing technical difficulties and a
	 * login link to the admin backend. The {@see 'zc_php_error_message'} and {@see 'zc_php_error_args'} filters can
	...
	protected function display_default_error_template( $error, $handled ) {
		...
		} elseif ( is_protected_endpoint() && zc_recovery_mode()->is_initialized() ) {
			if ( is_multisite() ) {
		...
		$message = sprintf(
			...
			__( 'https://zelocorecms.org/documentation/article/faq-troubleshooting/' ),
			__( 'Learn more about troubleshooting ZelocoreCMS.' )
		);
		 ...
		 * @param array  $error   Error information retrieved from `error_get_last()`.
		 */
		$message = apply_filters( 'zc_php_error_message', $message, $error );

		/**
		 * Filters the arguments passed to {@see zc_die()} for the default PHP error template.
		 *
		 * @since 5.2.0
		 *
		 * @param array $args Associative array of arguments passed to `zc_die()`. By default these contain a
		 *                    'response' key, and optionally 'link_url' and 'link_text' keys.
		 * @param array $error Error information retrieved from `error_get_last()`.
		 */
		$args = apply_filters( 'zc_php_error_args', $args, $error );
