<?php
/**
 * Dependencies API: ZC_Styles class
 *
 * @since 2.6.0
 *
 * @package ZelocoreCMS
 ...
 * @see ZC_Dependencies
 */
class ZC_Styles extends ZC_Dependencies {
	/**
	 * Base URL for styles.
	 *
	...
	public function __construct() {
		/**
		 * Fires when the ZC_Styles instance is initialized.
		 *
		 * @since 2.6.0
		 *
		 * @param ZC_Styles $zc_styles ZC_Styles instance (passed by reference).
		 */
		do_action_ref_array( 'zc_default_styles', array( &$this ) );
	}
	...
	public function do_item( $handle, $group = false ) {
		...
		if ( 'rtl' === $this->text_direction && isset( $obj->extra['rtl'] ) && $obj->extra['rtl'] ) {
			...
			/** This filter is documented in zc-includes/class-zc-styles.php */
	...
	public function print_inline_style( $handle, $display = true ) {
		...
		if ( ! $this->do_concat ) {
			...
			// Obtain the original `src` for a stylesheet possibly inlined by zc_maybe_inline_styles().
	...
	/**
	 * Overrides the add_data method from ZC_Dependencies, to allow unsetting dependencies for conditional styles.
	 *
	 * @since 6.9.0
	 *
