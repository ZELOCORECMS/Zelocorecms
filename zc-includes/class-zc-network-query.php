<?php
/**
 * Network API: ZC_Network_Query class
 *
 * @package ZelocoreCMS
 ...
 * @since 4.6.0
 *
 * @see ZC_Network_Query::__construct() for accepted arguments.
 */
#[AllowDynamicProperties]
class ZC_Network_Query {
	 ...
	 * @since 4.6.0
	 *
	 * @param string|array $query ZC_Network_Query arguments. See ZC_Network_Query::__construct() for accepted arguments.
	 */
	...
	/**
	 * Sets up the ZelocoreCMS query for retrieving networks.
	 *
	 * @since 4.6.0
	 *
	 * @param string|array $query Array or URL query string of parameters.
	 * @return array|int List of ZC_Network objects, a list of network IDs when 'fields' is set to 'ids',
	...
	public function get_networks() {
		 ...
		 * @since 4.6.0
		 *
		 * @param ZC_Network_Query $query Current instance of ZC_Network_Query (passed by reference).
		 */
		 ...
		 *
		 * Return a non-null value to bypass ZelocoreCMS' default network queries.
		 *
		 ...
		 * - Otherwise the filter should return an array of ZC_Network objects.
		 *
		 * Note that if the filter returns an array of network data, it will be assigned
		 * to the `networks` property of the current ZC_Network_Query instance.
		 *
		 * Filtering functions that require pagination information are encouraged to set
		 * the `found_networks` and `max_num_pages` properties of the ZC_Network_Query object,
		 * passed to the filter by reference. If ZC_Network_Query does not perform a database
		...
		$last_changed = zc_cache_get_last_changed( 'networks' );

		$cache_key   = "get_network_ids:$key";
		$cache_value = zc_cache_get_salted( $cache_key, 'network-queries', $last_changed );
		...
		if ( false === $cache_value ) {
			...
			zc_cache_set_salted( $cache_key, $cache_value, 'network-queries', $last_changed );
		} else {
		 ...
		 * @since 4.6.0
		 *
		 * @param ZC_Network[]     $_networks An array of ZC_Network objects.
		 * @param ZC_Network_Query $query     Current instance of ZC_Network_Query (passed by reference).
		 */
		$_networks = apply_filters_ref_array( 'the_networks', array( $_networks, &$this ) );

		// Convert to ZC_Network instances.
	...
	private function set_found_networks() {
		...
		if ( $this->query_vars['number'] && ! $this->query_vars['no_found_rows'] ) {
			 ...
			 * @param ZC_Network_Query $network_query        The `ZC_Network_Query` instance.
			 */
