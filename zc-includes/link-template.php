<?php
/**
 * ZelocoreCMS Link Template Functions
 *
 * @package ZelocoreCMS
 * @subpackage Template
 */
 ...
 *
 * @param int|ZC_Post $post Optional. Post ID or post object. Default is the global `$post`.
 */
function the_permalink( $post = 0 ) {
	 ...
	 * @param int|ZC_Post $post      Post ID, ZC_Post object, or 0. Default 0.
	 */
...
function user_trailingslashit( $url, $type_of_url = '' ) {
	global $zc_rewrite;
	if ( $zc_rewrite->use_trailing_slashes ) {
 ...
 */
function zc_force_plain_post_permalink( $post = null, $sample = null ) {
	if (
		null === $sample &&
		is_object( $post ) &&
		isset( $post->filter ) &&
...
function get_permalink( $post = 0, $leavename = false ) {
	 ...
	 * @param ZC_Post $post      The post in question.
	...
	if (
		$permalink &&
		! zc_force_plain_post_permalink( $post )
	) {
		...
		if ( str_contains( $permalink, '%category%' ) ) {
			...
			if ( $cats ) {
				 ...
				 * @since 3.5.0
				 *
				 * @param ZC_Term  $cat  The category to use in the permalink.
				 * @param array    $cats Array of all categories (ZC_Term objects) associated with the post.
				 * @param ZC_Post  $post The post in question.
				 */
			...
			if ( empty( $category ) ) {
				...
				if ( $default_category && ! is_zc_error( $default_category ) ) {
...
function get_post_permalink( $post = 0, $leavename = false, $sample = false ) {
	...
	$post_link = $zc_rewrite->get_extra_permastruct( $post->post_type );

	$slug = $post->post_name;

	$force_plain_link = zc_force_plain_post_permalink( $post );
 ...
 * @since 1.5.0
 *
 * @param int|ZC_Post $post      Optional. Post ID or object. Default uses the global `$post`.
...
function _get_page_link( $post = 0, $leavename = false, $sample = false ) {
	...
	$link = $zc_rewrite->get_page_permastruct();
 ...
 *
 * This can be used in the ZelocoreCMS Loop or outside of it.
 *
 * @since 2.0.0
 *
 * @global ZC_Rewrite $zc_rewrite ZelocoreCMS rewrite component.
...
function get_attachment_link( $post = null, $leavename = false ) {
	...
	if ( $force_plain_link || ! $parent_valid ) {
		$link = false;
	} elseif ( $zc_rewrite->using_permalinks() && $parent ) {
	...
	} elseif ( $zc_rewrite->using_permalinks() && ! $leavename ) {
...
function get_year_link( $year ) {
	...
	$yearlink = $zc_rewrite->get_year_permastruct();
	if ( ! empty( $yearlink ) ) {
...
function get_month_link( $year, $month ) {
	...
	$monthlink = $zc_rewrite->get_month_permastruct();
	if ( ! empty( $monthlink ) ) {
...
function get_day_link( $year, $month, $day ) {
	...
	$daylink = $zc_rewrite->get_day_permastruct();
	if ( ! empty( $daylink ) ) {
...
function get_feed_link( $feed = '' ) {
	...
	$permalink = $zc_rewrite->get_feed_permastruct();

	if ( $permalink ) {
		if ( str_contains( $feed, 'comments_' ) ) {
			$feed      = str_replace( 'comments_', '', $feed );
			$permalink = $zc_rewrite->get_comment_feed_permastruct();
		}
 ...
 * @since 2.5.0
 *
 * @param int|ZC_Term|object $cat  The ID or category object whose feed link will be retrieved.
 * @param string             $feed Optional. Feed type. Possible values include 'rss2', 'atom'.
 *                                 Default is the value of get_default_feed().
 ...
 * @since 3.0.0
 *
 * @param int|ZC_Term|object $term     The ID or term object whose feed link will be retrieved.
...
function get_term_feed_link( $term, $taxonomy = '', $feed = '' ) {
	...
	if ( empty( $term ) || is_zc_error( $term ) ) {
		return false;
	}
 ...
 * @since 2.3.0
 *
 * @param int|ZC_Term|object $tag  The ID or term object whose feed link will be retrieved.
 * @param string             $feed Optional. Feed type. Possible values include 'rss2', 'atom'.
 *                                 Default is the value of get_default_feed().
 ...
 * @since 2.7.0
 *
 * @param int|ZC_Term|object $tag      The ID or term object whose edit link will be retrieved.
 ...
 * @param ZC_Term $tag    Optional. Term object. If null, the queried object will be inspected.
 *                        Default null.
 */
 ...
 *
 * @param int|ZC_Term|object $term        The ID or term object whose edit link will be retrieved.
 ...
 * @param string           $before  Optional. Display before edit link. Default empty.
 * @param string           $after   Optional. Display after edit link. Default empty.
 * @param int|ZC_Term|null $term    Optional. Term ID or object. If null, the queried object will be inspected. Default null.
...
function get_search_link( $query = '' ) {
	...
	$permastruct = $zc_rewrite->get_search_permastruct();
...
function get_post_type_archive_link( $post_type ) {
	...
	if ( get_option( 'permalink_structure' ) && is_array( $post_type_obj->rewrite ) ) {
		...
		if ( $post_type_obj->rewrite['with_front'] ) {
			$struct = $zc_rewrite->front . $struct;
		} else {
			$struct = $zc_rewrite->root . $struct;
		}
 ...
 * @since 4.4.0
 *
 * @param int|ZC_Post $post         Optional. Post ID or `ZC_Post` object. Defaults to global `$post`.
 ...
 *
 * Can be used within the ZelocoreCMS loop or outside of it. Can be used with
 * pages, posts, attachments, revisions, global styles, templates, and template parts.
 *
 * @since 2.3.0
 * @since 6.3.0 Adds custom link for zc_navigation post types.
 *              Adds custom links for zc_template_part and zc_template post types.
 *
 * @param int|ZC_Post $post    Optional. Post ID or post object. Default is the global `$post`.
...
function get_edit_post_link( $post = 0, $context = 'display' ) {
	...
	if ( 'zc_template' === $post->post_type || 'zc_template_part' === $post->post_type ) {
		$slug = urlencode( get_stylesheet() . '//' . $post->post_name );
		$link = admin_url( sprintf( $post_type_object->_edit_link, $post->post_type, $slug ) );
	} elseif ( 'zc_navigation' === $post->post_type ) {
 ...
 *
 * Can be used within the ZelocoreCMS loop or outside of it, with any post type.
 *
 * @since 2.9.0
 *
 * @param int|ZC_Post $post         Optional. Post ID or post object. Default is the global `$post`.
...
function get_delete_post_link( $post = 0, $deprecated = '', $force_delete = false ) {
	 ...
	 */
	return apply_filters( 'get_delete_post_link', zc_nonce_url( $delete_link, "$action-post_{$post->ID}" ), $post->ID, $force_delete );
}
 ...
 *
 * @param int|ZC_Comment $comment_id Optional. Comment ID or ZC_Comment object.
 ...
 * @return ZC_Post|null|string Post object if successful. Null if global `$post` is not set.
...
function get_adjacent_post( $in_same_term = false, $excluded_terms = '', $previous = true, $taxonomy = 'category' ) {
	...
	if ( $in_same_term || ! empty( $excluded_terms ) ) {
		if ( $in_same_term ) {
			...
			$term_array = zc_get_object_terms( $post->ID, $taxonomy, array( 'fields' => 'ids' ) );
			if ( is_zc_error( $term_array ) ) {
				return '';
			}
	 ...
	 * @param ZC_Post      $post           ZC_Post object.
	 */
	...
	$result = zc_cache_get_salted( $cache_key, 'post-queries', $last_changed );
	if ( false !== $result ) {
		if ( $result ) {
	...
	zc_cache_set_salted( $cache_key, $result, 'post-queries', $last_changed );
 ...
 *
 * This is meant to be attached to actions like 'zc_head'. Do not call this directly in plugins
 * or theme templates.
 *
 * @since 3.0.0
 ...
 */
function adjacent_posts_rel_link_zc_head() {
...
function get_adjacent_post_link( $format, $link, $in_same_term = false, $excluded_terms = '', $previous = true, $taxonomy = 'category' ) {
	 ...
	 * @param string         $format   Link anchor format.
	 * @param string         $link     Link permalink format.
	 * @param ZC_Post|string $post     The adjacent post. Empty string if no corresponding post exists.
...
function get_pagenum_link( $pagenum = 1, $escape = true ) {
	...
	if ( ! $zc_rewrite->using_permalinks() || is_admin() ) {
		...
		$request = preg_replace( "|$zc_rewrite->pagination_base/\d+/?$|", '', $request );
		$request = preg_replace( '|^' . preg_quote( $zc_rewrite->index, '|' ) . '|i', '', $request );
		$request = ltrim( $request, '/' );

		if ( $zc_rewrite->using_index_permalinks() && ( $pagenum > 1 || '' !== $request ) ) {
			$parts[] = $zc_rewrite->index;
		}

		$parts[] = untrailingslashit( $request );

		if ( $pagenum > 1 ) {
			$parts[] = $zc_rewrite->pagination_base;
			$parts[] = $pagenum;
		}
...
function get_next_posts_link( $label = null, $max_page = 0 ) {
	global $paged, $zc_query;

	if ( ! $max_page ) {
		$max_page = $zc_query->max_num_pages;
	}
...
function get_posts_nav_link( $args = array() ) {
	...
	if ( ! is_singular() ) {
		...
		$max_num_pages = $zc_query->max_num_pages;
	...
	if ( 'newest' === get_option( 'default_comments_page' ) ) {
		if ( $pagenum !== $max_page ) {
			if ( $zc_rewrite->using_permalinks() ) {
				$result = user_trailingslashit( trailingslashit( $result ) . $zc_rewrite->comments_pagination_base . '-' . $pagenum, 'commentpaged' );
			} else {
...
function get_next_comments_link( $label = '', $max_page = 0, $page = null ) {
	...
	if ( empty( $max_page ) ) {
		$max_page = $zc_query->max_num_comment_pages;
	}
...
function paginate_comments_links( $args = array() ) {
	...
	if ( $zc_rewrite->using_permalinks() ) {
		$defaults['base'] = user_trailingslashit( trailingslashit( get_permalink() ) . $zc_rewrite->comments_pagination_base . '-%#%', 'commentpaged' );
	}
...
/**
 * Retrieves the URL for the current site where ZelocoreCMS application files
 * (e.g. zc-blog-header.php or the zc-admin/ folder) are accessible.
 *
...
/**
 * Retrieves the URL for a given site where ZelocoreCMS application files
 * (e.g. zc-blog-header.php or the zc-admin/ folder) are accessible.
 *
 * Returns the 'site_url' option with the appropriate protocol, 'https' if
...
function get_admin_url( $blog_id = null, $path = '', $scheme = 'admin' ) {
	$url = get_site_url( $blog_id, 'zc-admin/', $scheme );
...
function includes_url( $path = '', $scheme = null ) {
	 ...
	 * @param string      $path   Path relative to the URL to the zc-includes directory. Blank string
...
function content_url( $path = '' ) {
	$url = set_url_scheme( ZC_CONTENT_URL );
...
function plugins_url( $path = '', $plugin = '' ) {
	...
	$plugin        = zc_normalize_path( $plugin );
	$mu_plugin_dir = zc_normalize_path( ZCMU_PLUGIN_DIR );
...
function network_admin_url( $path = '', $scheme = 'admin' ) {
	...
	$url = network_site_url( 'zc-admin/network/', $scheme );
...
function user_admin_url( $path = '', $scheme = 'admin' ) {
	$url = network_site_url( 'zc-admin/user/', $scheme );
 ...
 * @since 4.6.0
 *
 * @param int|ZC_Post $post Optional. Post ID or object. Default is global `$post`.
 * @return string|false The canonical URL. False if the post does not exist
 *                      or has not been published yet.
 */
function zc_get_canonical_url( $post = null ) {
	$post = get_post( $post );
 ...
 *
 * @since 2.9.0
 * @since 4.6.0 Adjusted to use `zc_get_canonical_url()`.
 */
 ...
 *                are not enabled.
 */
function zc_get_shortlink( $id = 0, $context = 'post', $allow_slugs = true ) {
	/**
 ...
 *
 * Attached to the {@see 'zc_head'} action.
 *
 * @since 3.0.0
 */
function zc_shortlink_zc_head() {
	$shortlink = zc_get_shortlink( 0, 'query' );
...
function the_shortlink( $text = '', $title = '', $before = '', $after = '' ) {
	...
	$shortlink = zc_get_shortlink( $post->ID );
...
function get_avatar_data( $id_or_email, $args = null ) {
	...
	if ( is_numeric( $id_or_email ) ) {
	...
	} elseif ( $id_or_email instanceof ZC_User ) {
		// User object.
		$user = $id_or_email;
	} elseif ( $id_or_email instanceof ZC_Post ) {
		// Post object.
		$user = get_user_by( 'id', (int) $id_or_email->post_author );
	} elseif ( $id_or_email instanceof ZC_Comment ) {
		...
		if ( ( ! $user || is_zc_error( $user ) ) && ! empty( $id_or_email->comment_author_email ) ) {
...
function get_privacy_policy_url() {
	$url            = '';
	$policy_page_id = (int) get_option( 'zc_page_for_privacy_policy' );
 ...
 * the site's home_url() (as parsed by zc_parse_url()).
 *
 ...
 * link.
 *
 * @see zc_is_internal_link
 *
 * @since 6.2.0
 *
 * @return string[] An array of URL hosts.
 */
function zc_internal_hosts() {
	...
	if ( empty( $internal_hosts ) ) {
		...
		$internal_hosts = apply_filters(
			'zc_internal_hosts',
			array(
				zc_parse_url( home_url(), PHP_URL_HOST ),
			)
		);
 ...
 */
function zc_is_internal_link( $link ) {
	$link = strtolower( $link );
	if ( in_array( zc_parse_url( $link, PHP_URL_SCHEME ), zc_allowed_protocols(), true ) ) {
		return in_array( zc_parse_url( $link, PHP_URL_HOST ), zc_internal_hosts(), true );
	}
	return false;
}
