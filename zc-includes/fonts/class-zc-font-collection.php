 ...
 * @since 6.5.0
 *
 * @see zc_register_font_collection()
 */
final class ZC_Font_Collection {
	/**
	...
	/**
	 * ZC_Font_Collection constructor.
	 *
	 * @since 6.5.0
	 *
	 * @param string $slug Font collection slug. May only contain alphanumeric characters, dashes,
	 *                     and underscores. See sanitize_title().
	 * @param array  $args Font collection data. See zc_register_font_collection() for information on accepted arguments.
	 */
	 ...
	 * @since 6.5.0
	 *
	 * @return array|ZC_Error An array containing the font collection data, or a ZC_Error on failure.
	 */
	public function get_data() {
		if ( is_zc_error( $this->data ) ) {
			return $this->data;
		}
		...
		return zc_parse_args( $this->data, $defaults );
	}
	 ...
	 * @return array|ZC_Error An array containing the font collection data on success,
	 *                        else an instance of ZC_Error on failure.
	 */
	private function load_from_json( $file_or_url ) {
		$url  = zc_http_validate_url( $file_or_url );
		$file = file_exists( $file_or_url ) ? zc_normalize_path( realpath( $file_or_url ) ) : false;

		if ( ! $url && ! $file ) {
			// translators: %s: File path or URL to font collection JSON file.
			$message = __( 'Font collection JSON file is invalid or does not exist.' );
			_doing_it_wrong( __METHOD__, $message, '6.5.0' );
			return new ZC_Error( 'font_collection_json_missing', $message );
		}
	...
	private function load_from_file( $file ) {
		$data = zc_json_file_decode( $file, array( 'associative' => true ) );
		if ( empty( $data ) ) {
			return new ZC_Error( 'font_collection_decode_error', __( 'Error decoding the font collection JSON file contents.' ) );
		}
	...
	private function load_from_url( $url ) {
		...
		$transient_key = substr( 'zc_font_collection_url_' . $url, 0, 167 );
		$data          = get_site_transient( $transient_key );

		if ( false === $data ) {
			$response = zc_safe_remote_get( $url );
			...
			$data = json_decode( zc_remote_retrieve_body( $response ), true );
			if ( empty( $data ) ) {
				return new ZC_Error( 'font_collection_decode_error', __( 'Error decoding the font collection data from the HTTP response JSON.' ) );
			}
	 ...
	 * @return array|ZC_Error Sanitized data if valid, otherwise a ZC_Error instance.
	 */
	private function sanitize_and_validate_data( $data, $required_properties = array() ) {
		$schema = self::get_sanitization_schema();
		$data   = ZC_Font_Utils::sanitize_from_schema( $data, $schema );
		...
		foreach ( $required_properties as $property ) {
			if ( empty( $data[ $property ] ) ) {
				...
				_doing_it_wrong( __METHOD__, $message, '6.5.0' );
				return new ZC_Error( 'font_collection_missing_property', $message );
			}
		}
				...
				array(
					'font_family_settings' => array(
						...
						'slug'       => static function ( $value ) {
							return _zc_to_kebab_case( sanitize_title( $value ) );
						},
						'fontFamily' => 'ZC_Font_Utils::sanitize_font_family',
