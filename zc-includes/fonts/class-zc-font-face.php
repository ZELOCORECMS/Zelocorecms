...
class ZC_Font_Face {
	 ...
	 *                         See {@see zc_print_font_faces()} for the supported fields.
	 *                         Default empty array.
	 */
	public function generate_and_print( array $fonts ) {
		 ...
		 * as CSS in the browser. Using zc_strip_all_tags() is sufficient escaping
		 * to avoid malicious attempts to close </style> and open a <script>.
		 */
		$css = zc_strip_all_tags( $css );
		...
		$processor = new ZC_HTML_Tag_Processor( '<style class="zc-fonts-local"></style>' );
		$processor->next_tag();
	...
	private function validate_font_face_declarations( array $font_face ) {
		$font_face = zc_parse_args( $font_face, $this->font_face_property_defaults );

		// Check the font-family.
		if ( empty( $font_face['font-family'] ) || ! is_string( $font_face['font-family'] ) ) {
			// @todo replace with `zc_trigger_error()`.
			_doing_it_wrong(
				__METHOD__,
