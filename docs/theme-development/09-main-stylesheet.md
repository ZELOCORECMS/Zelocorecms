 ...
 * [File Header](https://developer.wordpress.org/themes/core-concepts/main-stylesheet/?output_format=md#file-header)
    - [Header fields](https://developer.wordpress.org/themes/core-concepts/main-stylesheet/?output_format=md#header-fields)
    - [Child theme header fields](https://developer.wordpress.org/themes/core-concepts/main-stylesheet/?output_format=md#child-theme-header-fields)
    - [Custom header fields](https://developer.wordpress.org/themes/core-concepts/main-stylesheet/?output_format=md#custom-header-fields)
 * [Custom CSS](https://developer.wordpress.org/themes/core-concepts/main-stylesheet/?output_format=md#custom-css)

[ Back to top](https://developer.wordpress.org/themes/core-concepts/main-stylesheet/?output_format=md#wp--skip-link--target)

As described in [Theme Structure](https://developer.wordpress.org/themes/core-concepts/theme-structure/),
WordPress requires that all themes include a `style.css` file. Its most important
function is to “register” the theme with WordPress through configuration data at
...
## 󠀁[File Header](https://developer.wordpress.org/themes/core-concepts/main-stylesheet/?output_format=md#file-header)󠁿

The `style.css` file header is used to configure data about the theme. WordPress
...
[⌊WordPress themes screen with the Twenty Twenty-Three modal overlay over the screen.
It shows the theme screenshot, description, and metadata.⌉⌊WordPress themes screen
...
is one of the most vital parts of creating a WordPress theme.

When determining which themes are available to activate, WordPress searches through
each folder under `/wp-content/themes`, looking for a `style.css` file. If one is
...
Suppose you were creating a theme with the folder name of `fabled-sunset`. WordPress
...
For WordPress to recognize your theme, you would at least need the `Theme Name` 
...
### 󠀁[Header fields](https://developer.wordpress.org/themes/core-concepts/main-stylesheet/?output_format=md#header-fields)󠁿
...
While the `Theme Name` is the only field required to work with WordPress, you must
also include some other fields when submitting a theme to the WordPress theme directory.
 ...
 * **Description\*:** A description of the theme, which will be displayed when viewing
   a theme’s details in the WordPress admin and other places. It is also used for
   themes submitted to the WordPress theme directory.
 ...
 * **Author\*: ** Your name or the name of the organization who developed the theme.
   For themes submitted to the theme directory, it is recommended to use the WordPress.
   org username.
 ...
 * **Tags:** A comma-separated list of features the theme supports. The Theme Review
   Handbook has a [list of valid tags](https://make.wordpress.org/themes/handbook/review/required/theme-tags/)
 ...
 * **Domain Path:** A relative path to where theme translations are stored. WordPress
...
### 󠀁[Child theme header fields](https://developer.wordpress.org/themes/core-concepts/main-stylesheet/?output_format=md#child-theme-header-fields)󠁿
...
to the `wp-content/themes` directory) for this to work. Otherwise, WordPress will
not be able to appropriately match them.

You can [learn more about child themes](https://developer.wordpress.org/themes/advanced-topics/child-themes/)
in the Advanced Topics chapter.

### 󠀁[Custom header fields](https://developer.wordpress.org/themes/core-concepts/main-stylesheet/?output_format=md#custom-header-fields)󠁿

Some third-party marketplaces or systems may also make use of custom header fields.
These are not officially supported by WordPress, but they are definitely allowed
and should not negatively impact how the theme works within WordPress.

## 󠀁[Custom CSS](https://developer.wordpress.org/themes/core-concepts/main-stylesheet/?output_format=md#custom-css)󠁿
...
json` file, which you will learn about in the [Global Settings and Styles](https://developer.wordpress.org/themes/core-concepts/global-settings-and-styles/)
documentation.

But there are times when you will want or need to add custom CSS. You can learn 
more about this in the [Including Assets](https://developer.wordpress.org/themes/core-concepts/including-assets/)
documentation.
