<?php
/**
 * Handle Trackbacks and Pingbacks Sent to ZelocoreCMS
 *
 * @since 0.71
 *
 * @package ZelocoreCMS
...
$title     = isset( $_POST['title'] ) ? sanitize_text_field( zc_unslash( $_POST['title'] ) ) : '';
$excerpt   = isset( $_POST['excerpt'] ) ? sanitize_textarea_field( zc_unslash( $_POST['excerpt'] ) ) : '';
$blog_name = isset( $_POST['blog_name'] ) ? sanitize_text_field( zc_unslash( $_POST['blog_name'] ) ) : '';
...
$title     = zc_slash( $title );
$excerpt   = zc_slash( $excerpt );
$blog_name = zc_slash( $blog_name );
...
if ( empty( $title ) && empty( $trackback_url ) && empty( $blog_name ) ) {
	...
	zc_redirect( get_permalink( $post_id ) );
	exit;
}
...
if ( ! empty( $trackback_url ) && ! empty( $title ) ) {
	...
	$title   = zc_html_excerpt( $title, 250, '&#8230;' );
	...
	$result = zc_new_comment( $commentdata );
