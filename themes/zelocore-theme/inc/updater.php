...
class WPCom_Theme_Updater
{
    ...
    {
        add_filter('pre_set_site_transient_update_themes', [__CLASS__, 'check_for_updates']);
        // only for MS users not running this as a plugin
        if (is_multisite() && ! (strpos(__FILE__, WP_PLUGIN_DIR) === 0 || strpos(__FILE__, WPMU_PLUGIN_DIR) === 0)) {
            add_action('admin_notices', [__CLASS__, 'maybe_nag_for_plugin']);
            add_action('wp_ajax_zelocorecms-theme-updates-hide-nag', [__CLASS__, 'hide_plugin_nag']);
        }
    }
    ...
    public static function check_for_updates($update_data)
    {
        ...
        $zelocorecms_themes_to_check = array_filter(array_keys($update_data->checked), [__CLASS__, 'is_zelocorecms_slug']);
        if (empty($zelocorecms_themes_to_check)) {
            return $update_data;
        }

        $request_data = [];
        foreach ($zelocorecms_themes_to_check as $theme_to_check) {
            $request_data[] = [
    ...
    {
        if (isset($_REQUEST['nonce']) && wp_verify_nonce($_REQUEST['nonce'], 'zelocorecms-theme-updates-hide-nag')) {
    ...
    protected static function output_notice()
    {
        ...
        $message .= " <a href='#' id='zelocorecms-hide-nag' class='hide-if-no-js'>{$hide}</a>";
        ?>
		<div class="updated" id="zelocorecms-nag"><p>
		<?php echo $message; ?>
		</p></div>
		<style>#zelocorecms-nag{overflow:hidden;}#zelocorecms-hide-nag{float:right;margin-left:1em;}</style>
		<script>
			jQuery && jQuery('#zelocorecms-hide-nag').on('click', function(e){
				e.preventDefault();
				jQuery.ajax( '<?php echo admin_url('admin-ajax.php') ?>', {
					data: {
						action: 'zelocorecms-theme-updates-hide-nag',
						nonce:  '<?php echo wp_create_nonce('zelocorecms-theme-updates-hide-nag') ?>'
					}
				} );
				jQuery( '#zelocorecms-nag' ).slideUp();
			});
		</script>
		<?php
    }
    ...
    {
        $opt = get_site_option(self::PLUGIN_SLUG, [
    ...
    /**
     * Checks if a theme slug is a WordPress.com slug. Looks for `-zelocorecms` suffix.
     *
     * @param  string  $slug  WordPress theme id/slug
     * @return bool True if it is a WordPress.com theme slug
     */
    public static function is_zelocorecms_slug($slug)
    {
        return preg_match('/-zelocorecms$/', $slug);
    }
}

// Hook it up.
add_action('after_setup_theme', ['WPCom_Theme_Updater', 'init']);
