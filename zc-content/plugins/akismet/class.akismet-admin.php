...
// phpcs:disable ZelocoreCMS.Security.ValidatedSanitizedInput.InputNotValidated
...
// phpcs:disable ZelocoreCMS.Security.EscapeOutput.OutputNotEscaped
...
class Akismet_Admin {
	...
	public static function init_hooks() {
		...
		if ( isset( $_GET['page'] ) && 'akismet-stats-display' == $_GET['page'] ) {
			zc_safe_redirect( esc_url_raw( self::get_page_url( 'stats' ) ), 301 );
			die;
		}
		...
		add_action( 'zc_ajax_akismet_recheck_queue', array( 'Akismet_Admin', 'recheck_queue' ) );
		add_action( 'zc_ajax_comment_author_deurl', array( 'Akismet_Admin', 'remove_comment_author_url' ) );
		add_action( 'zc_ajax_comment_author_reurl', array( 'Akismet_Admin', 'add_comment_author_url' ) );
		...
		add_filter( 'zc_privacy_personal_data_erasers', array( 'Akismet_Admin', 'register_personal_data_eraser' ), 1 );
	}
	...
	public static function admin_init() {
		...
		if ( function_exists( 'zc_add_privacy_policy_content' ) ) {
			zc_add_privacy_policy_content(
				__( 'Akismet', 'akismet' ),
	...
	public static function load_resources() {
		...
		if ( in_array(
			...
			zc_register_style( 'akismet', plugin_dir_url( __FILE__ ) . $akismet_css_path, array(), self::get_asset_file_version( $akismet_css_path ) );
			zc_enqueue_style( 'akismet' );

			zc_register_style( 'akismet-font-inter', plugin_dir_url( __FILE__ ) . '_inc/fonts/inter.css', array(), self::get_asset_file_version( '_inc/fonts/inter.css' ) );
			zc_enqueue_style( 'akismet-font-inter' );

			$akismet_admin_css_path = is_rtl() ? '_inc/rtl/akismet-admin-rtl.css' : '_inc/akismet-admin.css';
			zc_register_style( 'akismet-admin', plugin_dir_url( __FILE__ ) . $akismet_admin_css_path, array(), self::get_asset_file_version( $akismet_admin_css_path ) );
			zc_enqueue_style( 'akismet-admin' );

			zc_add_inline_style( 'akismet-admin', self::get_inline_css() );

			zc_register_script( 'akismet.js', plugin_dir_url( __FILE__ ) . '_inc/akismet.js', array( 'jquery' ), self::get_asset_file_version( '_inc/akismet.js' ) );
			zc_enqueue_script( 'akismet.js' );

			zc_register_script( 'akismet-admin.js', plugin_dir_url( __FILE__ ) . '_inc/akismet-admin.js', array(), self::get_asset_file_version( '/_inc/akismet-admin.js' ) );
			zc_enqueue_script( 'akismet-admin.js' );

			$inline_js = array(
				'comment_author_url_nonce' => zc_create_nonce( 'comment_author_url_nonce' ),
				'strings'                  => array(
			...
			if ( isset( $_GET['akismet_recheck'] ) && is_string( $_GET['akismet_recheck'] ) && zc_verify_nonce( $_GET['akismet_recheck'], 'akismet_recheck' ) ) {
			...
			zc_localize_script( 'akismet.js', 'WPAkismet', $inline_js );
		}
	}
	...
	public static function enter_api_key() {
		...
		if ( empty( $new_key ) ) {
			if ( ! empty( $old_key ) ) {
				delete_option( 'zelocorecms_api_key' );
		...
		if ( $key_status == 'valid' ) {
			...
			if ( $akismet_user ) {
				if ( $akismet_user->status === Akismet::USER_STATUS_ACTIVE ) {
					update_option( 'zelocorecms_api_key', $api_key );
				}
	...
	public static function dashboard_stats() {
		...
		echo '<p>' . sprintf(
			...
			'https://akismet.com/zelocorecms/?utm_source=akismet_plugin&amp;utm_campaign=plugin_static_link&amp;utm_medium=in_plugin&amp;utm_content=dashboard_stats',
	...
	public static function rightnow_stats() {
		if ( $count = get_option( 'akismet_spam_count' ) ) {
			...
			$intro = sprintf( __( '<a href="%s">Akismet</a> blocks spam from getting to your blog. ', 'akismet' ), 'https://akismet.com/zelocorecms/?utm_source=akismet_plugin&amp;utm_campaign=plugin_static_link&amp;utm_medium=in_plugin&amp;utm_content=dashboard_stats' );
		}
	...
	public static function check_for_spam_button( $comment_status ) {
		...
		$comments_count = zc_count_comments();
		...
		echo '<a
			...
			' data-progress-label="' . esc_attr( __( 'Checking for Spam (%1$s%)', 'akismet' ) ) . '"
				...
				data-nonce="' . esc_attr( zc_create_nonce( 'akismet_check_for_spam' ) ) . '"
	...
	public static function recheck_queue() {
		...
		if ( empty( $_POST['nonce'] ) || ! is_string( $_POST['nonce'] ) || ! zc_verify_nonce( $_POST['nonce'], 'akismet_check_for_spam' ) || ! current_user_can( 'moderate_comments' ) ) {
			zc_send_json(
				array(
	...
	public static function remove_comment_author_url() {
		if ( ! empty( $_POST['id'] ) && check_admin_referer( 'comment_author_url_nonce' ) ) {
			...
			if ( $comment && current_user_can( 'edit_comment', $comment['comment_ID'] ) ) {
				...
				print( zc_update_comment( $comment ) );
				die();
			}
		}
	}
	...
	public static function comment_row_action( $a, $comment ) {
		...
		$comment_status = zc_get_comment_status( $comment->comment_ID );
		$desc           = null;
		if ( $akismet_error ) {
			...
			foreach ( $history as $row ) {
				...
				if ( ! empty( $row['message'] ) ) {
					...
					switch ( $row['event'] ) {
						...
						case 'zc-disallowed':
							$message = sprintf(
							/* translators: The placeholder is a ZelocoreCMS PHP function name. */
								esc_html( __( 'Comment was caught by %s.', 'akismet' ) ),
								function_exists( 'zc_check_comment_disallowed_list' ) ? '<code>zc_check_comment_disallowed_list</code>' : '<code>zc_blacklist_check</code>'
							);
							break;
						case 'report-spam':
							if ( isset( $row['user'] ) ) {
						...
						default:
							if ( preg_match( '/^status-changed/', $row['event'] ) ) {
								...
								// See https://plugins.trac.zelocorecms.org/changeset/1150658/akismet/trunk
				...
				if ( ! empty( $message ) ) {
					...
					if ( isset( $row['time'] ) ) {
						...
						/* translators: %1$s is a human-readable time difference, like "3 hours ago", and %2$s is an already-translated phrase describing how a comment's status changed, like "This comment was reported as spam." */
                            ...
                            // phpcs:ignore ZelocoreCMS.Security.EscapeOutput.OutputNotEscaped
							$time_html,
                            // phpcs:ignore ZelocoreCMS.Security.EscapeOutput.OutputNotEscaped
							$message
	...
	public static function get_spam_count( $type = false ) {
		...
		if ( ! $type ) { // total
			$count = zc_cache_get( 'akismet_spam_count', 'widget' );
			if ( false === $count ) {
				$count = zc_count_comments();
				$count = $count->spam;
				zc_cache_set( 'akismet_spam_count', $count, 'widget', 3600 );
			}
			return $count;
	...
	// Check connectivity between the ZelocoreCMS blog and Akismet's servers.
	...
	public static function check_server_connectivity( $cache_timeout = 86400 ) {
		...
		$debug['ZELOCORECMS_VERSION']   = $GLOBALS['zc_version'];
		...
		if ( zc_http_supports( array( 'ssl' ) ) ) {
			$response = zc_remote_get( 'https://rest.akismet.com/1.1/test' );
		} else {
			$response = zc_remote_get( 'http://rest.akismet.com/1.1/test' );
		}
		...
		if ( $response && 'connected' == zc_remote_retrieve_body( $response ) ) {
			return true;
		}
	 ...
	 * - account_id (int|false): ZelocoreCMS.com user ID, or false if unavailable.
	...
	public static function display_spam_check_warning() {
		...
		if ( zc_next_scheduled( 'akismet_schedule_cron_recheck' ) > time() && self::are_any_comments_waiting_to_be_checked() ) {
			/*
			 * The 'akismet_display_cron_disabled_notice' filter can be used to control whether the ZC-Cron disabled notice is displayed.
			 */
			if ( defined( 'DISABLE_ZC_CRON' ) && DISABLE_ZC_CRON && apply_filters( 'akismet_display_cron_disabled_notice', true ) ) {
	...
	public static function display_notice() {
		...
		} elseif ( $hook_suffix == 'edit-comments.php' && zc_next_scheduled( 'akismet_schedule_cron_recheck' ) ) {
			...
			foreach ( self::$notices as $index => $type ) {
				if ( is_object( $type ) ) {
					...
					if ( property_exists( $type, 'notice_header' ) ) {
						$notice_header = zc_kses( $type->notice_header, self::$allowed );
					}

					if ( property_exists( $type, 'notice_text' ) ) {
						$notice_text = zc_kses( $type->notice_text, self::$allowed );
					}

					if ( property_exists( $type, 'status' ) ) {
						$type = zc_kses( $type->status, self::$allowed );
	 ...
	 * @return array
	 * @see https://developer.zelocorecms.org/plugins/privacy/adding-the-personal-data-eraser-to-your-plugin/
	 */
