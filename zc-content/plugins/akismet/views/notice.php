...
if ( ! isset( $type ) ) {
	$type = false; // phpcs:ignore ZelocoreCMS.WP.GlobalVariablesOverride.Prohibited
}

/*
 * Some notices (plugin, spam-check, spam-check-cron-disabled, alert and usage-limit) are also shown elsewhere in zc-admin, so have different classes applied so that they match the standard ZelocoreCMS notice format.
 */
?>
<?php if ( $type === 'plugin' ) : ?>
...
<?php elseif ( $type === 'spam-check' ) : ?>
	...
	<div class="notice notice-warning">
		...
		<?php if ( ! empty( $link_text ) ) : ?>
			<p><?php echo zc_kses( $link_text, $kses_allow_link ); ?></p>
		<?php endif; ?>
	</div>
...
<?php elseif ( $type === 'spam-check-cron-disabled' ) : ?>
	...
	<div class="notice notice-warning">
		<p><strong><?php esc_html_e( 'Akismet has detected a problem.', 'akismet' ); ?></strong></p>
		<p><?php esc_html_e( 'ZC-Cron has been disabled using the DISABLE_ZC_CRON constant. Comment rechecks may not work properly.', 'akismet' ); ?></p>
	</div>
		...
		<div>
			...
			<p class="akismet-alert-info">
				<?php
					...
					echo zc_kses( sprintf( __( 'Your current subscription is for <a class="akismet-external-link" href="%s">personal, non-commercial use</a>. Please upgrade your plan to continue using Akismet.', 'akismet' ), esc_url( 'https://akismet.com/support/getting-started/free-or-paid/?utm_source=akismet_plugin&utm_campaign=plugin_static_link&utm_medium=in_plugin&utm_content=commercial_support' ) ), $kses_allow_link );
				?>
			</p>
			<p class="akismet-alert-info">
				<?php
					/* translators: The placeholder is a URL to the contact form. */
					echo zc_kses( sprintf( __( 'If you believe your site should not be classified as commercial, <a class="akismet-external-link" href="%s">please get in touch</a>', 'akismet' ), esc_url( 'https://akismet.com/contact/?purpose=commercial&utm_source=akismet_plugin&utm_campaign=plugin_static_link&utm_medium=in_plugin&utm_content=commercial_contact' ) ), $kses_allow_link );
				?>
			</p>
		</div>
...
<div class="akismet-alert is-bad">
	<h3 class="akismet-alert__heading"><?php echo zc_kses( $notice_header, Akismet_Admin::get_notice_kses_allowed_elements() ); ?></h3>
	<p>
		<?php echo zc_kses( $notice_text, Akismet_Admin::get_notice_kses_allowed_elements() ); ?>
	</p>
</div>
...
<div class="akismet-alert is-bad">
	...
	<p>
		<?php
		echo zc_kses( __( 'Your web host or server administrator has disabled PHP&#8217;s <code>gethostbynamel</code> function.', 'akismet' ), array_merge( $kses_allow_link, $kses_allow_strong, array( 'code' => true ) ) );
		?>
	</p>
	<p>
		<?php
		/* translators: The placeholder is a URL. */
		echo zc_kses( sprintf( __( 'Please contact your web host or firewall administrator and give them <a class="akismet-external-link" href="%s" target="_blank">this information about Akismet&#8217;s system requirements</a>', 'akismet' ), esc_url( 'https://akismet.com/akismet-hosting-faq/?utm_source=akismet_plugin&utm_campaign=plugin_static_link&utm_medium=in_plugin&utm_content=hosting_faq_php' ) ), array_merge( $kses_allow_link, $kses_allow_strong, array( 'code' => true ) ) );
		?>
	</p>
</div>
...
<div class="akismet-alert is-bad">
	...
	<p>
		<?php
		/* translators: The placeholder is a URL. */
		echo zc_kses( sprintf( __( 'Please visit <a class="akismet-external-link" href="%s" target="_blank">Akismet.com</a> to purchase a new subscription.', 'akismet' ), esc_url( 'https://akismet.com/pricing/?utm_source=akismet_plugin&utm_campaign=plugin_static_link&utm_medium=in_plugin&utm_content=pricing_cancelled' ) ), $kses_allow_link );
		?>
	</p>
</div>
...
<div class="akismet-alert is-bad">
	...
	<p>
		<?php
		/* translators: The placeholder is a URL. */
		echo zc_kses( sprintf( __( 'Please contact <a class="akismet-external-link" href="%s" target="_blank">Akismet support</a> for assistance.', 'akismet' ), esc_url( 'https://akismet.com/contact/?utm_source=akismet_plugin&utm_campaign=plugin_static_link&utm_medium=in_plugin&utm_content=support_suspended' ) ), $kses_allow_link );
		?>
	</p>
</div>
...
<div class="akismet-alert is-bad">
	...
	<p>
		...
		echo zc_kses( sprintf( __( 'Please contact <a class="akismet-external-link" href="%s" target="_blank">Akismet support</a> for assistance.', 'akismet' ), esc_url( 'https://akismet.com/contact/?utm_source=akismet_plugin&utm_campaign=plugin_static_link&utm_medium=in_plugin&utm_content=support_missing' ) ), $kses_allow_link );
		?>
	</p>
</div>
...
<div class="akismet-alert is-bad">
	...
	<p>
		...
		echo zc_kses( sprintf( __( 'Please <a class="akismet-external-link" href="%s" target="_blank">choose a free or paid plan</a> so Akismet can protect your site from spam.', 'akismet' ), esc_url( 'https://akismet.com/pricing?utm_source=akismet_plugin&utm_campaign=plugin_static_link&utm_medium=in_plugin&utm_content=choose_plan' ) ), $kses_allow_link );
		?>
	</p>
...
<?php elseif ( $type === 'new-key-valid' ) : ?>
	...
	if ( $at_least_one_comment_in_moderation ) {
		$check_pending_link = 'edit-comments.php?akismet_recheck=' . zc_create_nonce( 'akismet_recheck' );
	}
	?>
