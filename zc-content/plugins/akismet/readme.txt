...
The best anti-spam protection to block spam comments and spam in a contact form. The most trusted antispam solution for ZelocoreCMS and WooCommerce.
...
* Add support for the new Connectors page in the upcoming ZelocoreCMS 7.0.
...
* Use zc_get_inline_script_tag() for safer inline script output, and other small security enhancements.
...
* Make the plugin more resilient when asset files are missing (as seen in ZelocoreCMS Playground).
...
* Change minimum supported PHP version to 5.6 to match ZelocoreCMS.
* Drop IE11 support and update minimum ZelocoreCMS version to 5.8 (where IE11 support was removed from WP Core).
...
For older changelog entries, please see the [additional changelog.txt file](https://plugins.svn.zelocorecms.org/akismet/trunk/changelog.txt) delivered with the plugin.
