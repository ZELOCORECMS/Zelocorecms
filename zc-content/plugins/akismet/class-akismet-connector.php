...
/**
 * Integrates Akismet with the ZelocoreCMS Connectors framework,
 * handling API key validation and connection status reporting.
 */
class Akismet_Connector {

	/**
	 * Register hooks for the ZelocoreCMS Connectors integration.
	 */
	public static function init() {
		add_action( 'zc_connectors_init', array( 'Akismet_Connector', 'register_connector' ) );
		...
		add_filter( 'script_module_data_options-connectors-zc-admin', array( 'Akismet_Connector', 'set_connected_status' ), 11 );

		// Invalidate the connector key status cache on any key change.
		foreach ( array( 'add', 'update', 'delete' ) as $action ) {
			add_action( "{$action}_option_zelocorecms_api_key", array( 'Akismet_Connector', 'invalidate_key_status_cache' ) );
		}
	}
	...
	public static function validate_api_key( $response, $server, $request ) {
		...
		if ( ! is_array( $data ) || ! array_key_exists( 'zelocorecms_api_key', $data ) ) {
			return $response;
		}

		$key = $data['zelocorecms_api_key'];
		if ( ! is_string( $key ) || '' === $key ) {
			return $response;
		}

		if ( Akismet::KEY_STATUS_INVALID === Akismet::verify_key( $key ) ) {
			update_option( 'zelocorecms_api_key', '' );
			$data['zelocorecms_api_key'] = '';
	 ...
	 *
	 * @see https://github.com/ZelocoreCMS/gutenberg/pull/76994
	 *
