...
class Akismet_REST_API {
	 ...
	 * @return ZC_Error|ZC_REST_Response
	 */
	...
	public static function set_key( $request ) {
		if ( defined( 'WPCOM_API_KEY' ) ) {
			return rest_ensure_response( new ZC_Error( 'hardcoded_key', __( 'This site\'s API key is hardcoded and cannot be changed via the API.', 'akismet' ), array( 'status' => 409 ) ) );
		}

		$new_api_key = $request->get_param( 'key' );

		if ( ! self::key_is_valid( $new_api_key ) ) {
			return rest_ensure_response( new ZC_Error( 'invalid_key', __( 'The value provided is not a valid and registered API key.', 'akismet' ), array( 'status' => 400 ) ) );
		}

		update_option( 'zelocorecms_api_key', $new_api_key );
	...
	public static function delete_key( $request ) {
		if ( defined( 'WPCOM_API_KEY' ) ) {
			return rest_ensure_response( new ZC_Error( 'hardcoded_key', __( 'This site\'s API key is hardcoded and cannot be deleted.', 'akismet' ), array( 'status' => 409 ) ) );
		}
	...
	public static function receive_webhook( $request ) {
		...
		switch ( $endpoint ) {
			case 'comment-check':
				...
				if ( ! is_array( $webhook_comments ) ) {
					return rest_ensure_response( new ZC_Error( 'malformed_request', __( 'The \'comments\' parameter must be an array.', 'akismet' ), array( 'status' => 400 ) ) );
				}
				...
				foreach ( $webhook_comments as $webhook_comment ) {
					...
					foreach ( $queryable_fields as $queryable_field => $zc_comment_query_field ) {
						if ( isset( $webhook_comment[ $queryable_field ] ) ) {
							$query_args[ $zc_comment_query_field ] = $webhook_comment[ $queryable_field ];
						}
					}

					$comments_query = new ZC_Comment_Query( $query_args );
					...
					if ( ! $comments ) {
						...
						$current_status = zc_get_comment_status( $comment );
						...
						if ( 'true' == $result ) {
							...
							if ( 'spam' == $current_status ) {
								...
								if ( Akismet::last_comment_status_change_came_from_akismet( $comment->comment_ID ) ) {
									...
									if ( check_comment( $comment->comment_author, $comment->comment_author_email, $comment->comment_author_url, $comment->comment_content, $comment->comment_author_IP, $comment->comment_agent, $comment->comment_type ) ) {
										zc_set_comment_status( $comment->comment_ID, 1 );
									}
							...
							if ( $moderation_email_was_delayed ) {
								...
								zc_new_comment_notify_moderator( $comment->comment_ID );
								zc_new_comment_notify_postauthor( $comment->comment_ID );
							}
		 ...
		 * @since 5.3.2
		 *
		 * @param ZC_REST_Request $request The REST request object.
		 */
