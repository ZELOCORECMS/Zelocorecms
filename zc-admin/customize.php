 ...
 */
global $zc_scripts, $zc_customize;

if ( $zc_customize->changeset_post_id() ) {
	$changeset_post = get_post( $zc_customize->changeset_post_id() );
	...
	if ( $missed_schedule ) {
		/*
		 * Note that an Ajax request spawns here instead of just calling `zc_publish_post( $changeset_post->ID )`.
		 *
		 * Because ZC_Customize_Manager is not instantiated for customize.php with the `settings_previewed=false`
		 ...
		 */
		$nonces       = $zc_customize->get_nonces();
		$request_args = array(
			'nonce'                      => $nonces['save'],
			'customize_changeset_uuid'   => $zc_customize->changeset_uuid(),
			'zc_customize'               => 'on',
...
$url       = ! empty( $_REQUEST['url'] ) ? esc_url_raw( zc_unslash( $_REQUEST['url'] ) ) : '';
$return    = ! empty( $_REQUEST['return'] ) ? esc_url_raw( zc_unslash( $_REQUEST['return'] ) ) : '';
$autofocus = ! empty( $_REQUEST['autofocus'] ) && is_array( $_REQUEST['autofocus'] )
	? array_map( 'sanitize_text_field', zc_unslash( $_REQUEST['autofocus'] ) )
	: array();

if ( ! empty( $url ) ) {
	$zc_customize->set_preview_url( $url );
}
if ( ! empty( $return ) ) {
	$zc_customize->set_return_url( $return );
}
if ( ! empty( $autofocus ) ) {
	$zc_customize->set_autofocus( $autofocus );
}
...
$registered             = $zc_scripts->registered;
$zc_scripts             = new ZC_Scripts();
$zc_scripts->registered = $registered;

add_action( 'customize_controls_print_scripts', 'print_head_scripts', 20 );
add_action( 'customize_controls_print_footer_scripts', '_zc_footer_scripts' );
...
zc_enqueue_script( 'heartbeat' );
zc_enqueue_script( 'customize-controls' );
zc_enqueue_style( 'customize-controls' );
...
$body_class = 'zc-core-ui zc-customizer js';
...
if ( zc_use_widgets_block_editor() ) {
	$body_class .= ' zc-embed-responsive';
}

$admin_title = sprintf( $zc_customize->get_document_title_template(), __( 'Loading&hellip;' ) );

?>
<title><?php echo esc_html( $admin_title ); ?></title>

<script>
var ajaxurl = <?php echo zc_json_encode( admin_url( 'admin-ajax.php', 'relative' ), JSON_HEX_TAG | JSON_UNESCAPED_SLASHES ); ?>,
	pagenow = 'customize';
</script>
...
<div class="zc-full-overlay expanded">
	<form id="customize-controls" class="wrap zc-full-overlay-sidebar">
		<div id="customize-header-actions" class="zc-full-overlay-header">
			<?php
			$compatible_wp  = is_zc_version_compatible( $zc_customize->theme()->get( 'RequiresWP' ) );
			$compatible_php = is_php_version_compatible( $zc_customize->theme()->get( 'RequiresPHP' ) );
			?>
			<?php if ( $compatible_wp && $compatible_php ) : ?>
				<?php $save_text = $zc_customize->is_theme_active() ? __( 'Publish' ) : __( 'Activate &amp; Publish' ); ?>
			...
			<a class="customize-controls-close" href="<?php echo esc_url( $zc_customize->get_return_url() ); ?>">
		...
		<div id="widgets-right" class="zc-clearfix"><!-- For Widget Customizer, many widgets try to look for instances under div#widgets-right, so we have to add that ID to a container div in the Customizer for compat -->
			<div id="customize-notifications-area" class="customize-control-notifications-container">
				<ul></ul>
			</div>
			<div class="zc-full-overlay-sidebar-content" tabindex="-1">
				<div id="customize-info" class="accordion-section customize-info" data-block-theme="<?php echo (int) zc_is_block_theme(); ?>">
					...
					<div class="customize-panel-description">
						...
						<p>
							<?php
							_e( '<a href="https://zelocorecms.org/documentation/article/customizer/">Documentation on Customizer</a>' );
							?>
						</p>
					</div>
				</div>
		...
		<div id="customize-footer-actions" class="zc-full-overlay-footer">
			...
			<?php $previewable_devices = $zc_customize->get_previewable_devices(); ?>
	...
	<div id="customize-preview" class="zc-full-overlay-main"></div>
	<?php
