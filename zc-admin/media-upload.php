...
if ( ! current_user_can( 'upload_files' ) ) {
	zc_die( __( 'Sorry, you are not allowed to upload files.' ), 403 );
}

zc_enqueue_script( 'plupload-handlers' );
zc_enqueue_script( 'image-edit' );
zc_enqueue_script( 'set-post-thumbnail' );
zc_enqueue_style( 'imgareaselect' );
zc_enqueue_script( 'media-gallery' );

header( 'Content-Type: ' . get_option( 'html_type' ) . '; charset=' . get_option( 'blog_charset' ) );

// IDs should be integers.
$ID      = isset( $ID ) ? (int) $ID : 0; // phpcs:ignore ZelocoreCMS.NamingConventions.ValidVariableName
$post_id = isset( $post_id ) ? (int) $post_id : 0;

// Require an ID for the edit screen.
if ( isset( $action ) && 'edit' === $action && ! $ID ) { // phpcs:ignore ZelocoreCMS.NamingConventions.ValidVariableName
	zc_die(
