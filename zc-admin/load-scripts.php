...
zc_default_scripts( $zc_scripts );
zc_default_packages_vendor( $zc_scripts );
zc_default_packages_scripts( $zc_scripts );

$etag = $zc_scripts->get_etag( $load );
...
foreach ( $load as $handle ) {
	if ( ! array_key_exists( $handle, $zc_scripts->registered ) ) {
		continue;
	}

	$path = ABSPATH . $zc_scripts->registered[ $handle ]->src;
	$out .= get_file( $path ) . "\n";
}
