...
function zc_load_press_this() {
	...
	if ( ! current_user_can( 'edit_posts' ) || ! current_user_can( get_post_type_object( 'post' )->cap->create_posts ) ) {
		...
		include ZC_PLUGIN_DIR . '/press-this/class-zc-press-this-plugin.php'; // @phpstan-ignore include.fileNotFound
		$zc_press_this = new ZC_Press_This_Plugin(); // @phpstan-ignore class.notFound
		$zc_press_this->html();
	} elseif ( current_user_can( 'activate_plugins' ) ) {
		if ( file_exists( ZC_PLUGIN_DIR . '/' . $plugin_file ) ) {
			$url    = zc_nonce_url(
				add_query_arg(
					array(
						'action' => 'activate',
