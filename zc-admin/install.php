...
<body class="zc-core-ui admin-color-modern">
	...
	<p>ZelocoreCMS requires that your web server is running PHP. Your server does not have PHP installed, or PHP is turned off.</p>
</body>
</html>
	<?php
}
...
<head>
	...
	<title><?php _e( 'ZelocoreCMS &rsaquo; Installation' ); ?></title>
	<?php zc_admin_css( 'install', true ); ?>
</head>
<body class="zc-core-ui admin-color-modern<?php echo $body_classes; ?>">
...
function display_setup_form( $error = null ) {
	...
	$weblog_title = isset( $_POST['weblog_title'] ) ? trim( zc_unslash( $_POST['weblog_title'] ) ) : '';
	$user_name    = isset( $_POST['user_name'] ) ? trim( zc_unslash( $_POST['user_name'] ) ) : '';
	$admin_email  = isset( $_POST['admin_email'] ) ? trim( zc_unslash( $_POST['admin_email'] ) ) : '';
...
<form id="setup" method="post" action="install.php?step=2" novalidate="novalidate">
	<table class="form-table" role="presentation">
		...
		<tr class="form-field form-required user-pass1-wrap">
			...
			<td>
				<div class="zc-pwd">
					<?php $initial_password = isset( $_POST['admin_password'] ) ? stripslashes( $_POST['admin_password'] ) : zc_generate_password( 18 ); ?>
					...
					<button type="button" class="button zc-hide-pw hide-if-no-js" data-start-masked="<?php echo (int) isset( $_POST['admin_password'] ); ?>" data-toggle="0" aria-label="<?php esc_attr_e( 'Hide password' ); ?>">
			...
			<td>
				<fieldset>
					...
					if ( has_action( 'blog_privacy_selector' ) ) {
						...
						<?php
						/** This action is documented in zc-admin/options-reading.php */
	...
	<p class="step"><?php submit_button( __( 'Install ZelocoreCMS' ), 'large', 'Submit', false, array( 'id' => 'submit' ) ); ?></p>
...
if ( is_blog_installed() ) {
	...
	die(
		...
		'<p>' . __( 'You appear to have already installed ZelocoreCMS. To reinstall please clear your old database tables first.' ) . '</p>' .
		'<p class="step"><a href="' . esc_url( zc_login_url() ) . '">' . __( 'Log In' ) . '</a></p>' .
		'</body></html>'
	);
}
...
if ( ! $mysql_compat && ! $php_compat ) {
	$compat = sprintf(
		...
		__( 'You cannot install because <a href="%1$s">ZelocoreCMS %2$s</a> requires PHP version %3$s or higher and MySQL version %4$s or higher. You are running PHP version %5$s and MySQL version %6$s.' ),
		$version_url,
		$zc_version,
	...
	$compat = sprintf(
		...
		__( 'You cannot install because <a href="%1$s">ZelocoreCMS %2$s</a> requires PHP version %3$s or higher. You are running version %4$s.' ),
		$version_url,
		$zc_version,
		$required_php_version,
		$php_version
	...
	$compat = sprintf(
		...
		__( 'You cannot install because <a href="%1$s">ZelocoreCMS %2$s</a> requires MySQL version %3$s or higher. You are running version %4$s.' ),
		$version_url,
		$zc_version,
		$required_mysql_version,
		$mysql_version
...
if ( isset( $required_php_extensions ) && is_array( $required_php_extensions ) ) {
	...
	foreach ( $required_php_extensions as $extension ) {
		...
		$missing_extensions[] = sprintf(
			...
			__( 'You cannot install because <a href="%1$s">ZelocoreCMS %2$s</a> requires the %3$s PHP extension.' ),
			$version_url,
			$zc_version,
			$extension
		);
...
if ( defined( 'DO_NOT_UPGRADE_GLOBAL_TABLES' ) ) {
	...
	die(
		...
		'<p>' . sprintf(
			...
			__( 'The constant %s cannot be defined when installing ZelocoreCMS.' ),
...
switch ( $step ) {
	...
	case 1: // Step 1, direct link or from language chooser.
		...
		?>
<h1><?php _ex( 'Welcome', 'Howdy' ); ?></h1>
<p><?php _e( 'Welcome to the famous five-minute ZelocoreCMS installation process! Just fill in the information below and you&#8217;ll be on your way to using the most extendable and powerful personal publishing platform in the world.' ); ?></p>
...
<p><?php _e( 'Please provide the following information. Do not worry, you can always change these settings later.' ); ?></p>
	...
	case 2:
		...
		$weblog_title         = isset( $_POST['weblog_title'] ) ? trim( zc_unslash( $_POST['weblog_title'] ) ) : '';
		$user_name            = isset( $_POST['user_name'] ) ? trim( zc_unslash( $_POST['user_name'] ) ) : '';
		$admin_password       = isset( $_POST['admin_password'] ) ? zc_unslash( $_POST['admin_password'] ) : '';
		$admin_password_check = isset( $_POST['admin_password2'] ) ? zc_unslash( $_POST['admin_password2'] ) : '';
		$admin_email          = isset( $_POST['admin_email'] ) ? trim( zc_unslash( $_POST['admin_email'] ) ) : '';
...
<p><?php _e( 'ZelocoreCMS has been installed. Thank you, and enjoy!' ); ?></p>
...
<p class="step"><a href="<?php echo esc_url( zc_login_url() ); ?>"><?php _e( 'Log In' ); ?></a></p>
...
zc_print_scripts( $scripts_to_print );
?>
<script>
jQuery( function( $ ) {
