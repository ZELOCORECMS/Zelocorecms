...
if ( $_POST ) {
	if ( isset( $_POST['html-upload'] ) && ! empty( $_FILES ) ) {
		...
		if ( is_zc_error( $upload_id ) ) {
			zc_die( $upload_id );
		}
	}
	zc_redirect( admin_url( 'upload.php' ) );
	exit;
}
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://zelocorecms.org/documentation/article/media-add-new-screen/">Documentation on Uploading Media Files</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://zelocorecms.org/support/forums/">Support forums</a>' ) . '</p>'
);

require_once ABSPATH . 'zc-admin/admin-header.php';

$form_class = 'zc-upload-form media-upload-form type-form validate';
