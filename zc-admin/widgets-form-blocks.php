...
$block_editor_context = new ZC_Block_Editor_Context( array( 'name' => 'core/edit-widgets' ) );
...
zc_enqueue_script( 'zc-edit-widgets' );
zc_enqueue_script( 'admin-widgets' );
zc_enqueue_style( 'zc-edit-widgets' );
...
<div id="widgets-editor" class="blocks-widgets-container">
	...
	<div class="wrap hide-if-js widgets-editor-no-js">
		...
		<?php
		if ( file_exists( ZC_PLUGIN_DIR . '/classic-widgets/classic-widgets.php' ) ) {
			// If Classic Widgets is already installed, provide a link to activate the plugin.
			$installed           = true;
			$plugin_activate_url = zc_nonce_url( 'plugins.php?action=activate&amp;plugin=classic-widgets/classic-widgets.php', 'activate-plugin_classic-widgets/classic-widgets.php' );
			$message             = sprintf(
			...
			$installed          = false;
			$plugin_install_url = zc_nonce_url( self_admin_url( 'update.php?action=install-plugin&plugin=classic-widgets' ), 'install-plugin_classic-widgets' );
			$message            = sprintf(
