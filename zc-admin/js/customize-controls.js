/**
 * @output zc-admin/js/customize-controls.js
 */
...
(function( exports, $ ){
	...
	api.Setting = api.Value.extend(/** @lends wp.customize.Setting.prototype */{
		 ...
		 *
		 * A setting is ZelocoreCMS data (theme mod, option, menu, etc.) that the user can
	...
	Container = api.Class.extend(/** @lends wp.customize~Container.prototype */{
		...
		_animateChangeExpanded: function( completeCallback ) {
			...
			var construct = this,
				...
				overlay = content.closest( '.zc-full-overlay' ),
			...
			_.defer( function() {
				var container = content.closest( '.zc-full-overlay-sidebar-content' ),
	...
	api.Section = Container.extend(/** @lends wp.customize.Section.prototype */{
		...
		onChangeExpanded: function ( expanded, args ) {
			var section = this,
				container = section.headContainer.closest( '.zc-full-overlay-sidebar-content' ),
				content = section.contentContainer,
				overlay = section.headContainer.closest( '.zc-full-overlay' ),
	...
	api.ThemesSection = api.Section.extend(/** @lends wp.customize.ThemesSection.prototype */{
		...
		attachEvents: function () {
			...
			if ( 'local' === section.params.filter_type ) {
				...
				section.container.on( 'input', '.zc-filter-search-themes', function( event ) {
				...
				section.contentContainer.on( 'input', '.zc-filter-search', function() {
				...
				api.section.each( function ( otherSection ) {
					...
					if ( otherSection !== section ) {
						...
						if ( 'themes' === otherSection.params.type ) {
							searchTerm = otherSection.contentContainer.find( '.zc-filter-search' ).val();
							section.contentContainer.find( '.zc-filter-search' ).val( searchTerm );
		...
		loadThemes: function() {
			...
			request.done(function( data ) {
				...
				section.headContainer.closest( '.zc-full-overlay' ).removeClass( 'loading' );
				section.loading = false;
			});
		...
		checkTerm: function( section ) {
			var newTerm;
			if ( 'remote' === section.params.filter_type ) {
				newTerm = section.contentContainer.find( '.zc-filter-search' ).val();
	...
	api.Panel = Container.extend(/** @lends wp.customize.Panel.prototype */{
		...
		onChangeExpanded: function ( expanded, args ) {
			...
			var panel = this,
				...
				overlay = accordionSection.closest( '.zc-full-overlay' ),
				container = accordionSection.closest( '.zc-full-overlay-sidebar-content' ),
	...
	api.ThemesPanel = api.Panel.extend(/** @lends wp.customize.ThemsPanel.prototype */{
		...
		attachEvents: function() {
			...
			panel.contentContainer.on( 'click', '.customize-themes-section-title, .customize-themes-mobile-back', function() {
				$( '.zc-full-overlay' ).toggleClass( 'showing-themes' );
			});
		...
		onChangeExpanded: function( expanded, args ) {
			...
			overlay = panel.headContainer.closest( '.zc-full-overlay' );
	...
	api.ColorControl = api.Control.extend(/** @lends wp.customize.ColorControl.prototype */{
		ready: function() {
			...
			control.container.on( 'keydown', function( event ) {
				...
				pickerContainer = control.container.find( '.zc-picker-container' );
				if ( pickerContainer.hasClass( 'zc-picker-active' ) ) {
					picker.wpColorPicker( 'close' );
					control.container.find( '.zc-color-result' ).focus();
					event.stopPropagation(); // Prevent section from being collapsed.
				}
			} );
		}
	...
	api.ThemeControl = api.Control.extend(/** @lends wp.customize.ThemeControl.prototype */{
		...
		filter: function( terms ) {
			...
			// Back-compat for behavior in ZelocoreCMS 4.2.0 to 4.8.X.
			if ( ! _.isArray( terms ) ) {
				terms = [ terms ];
			}
	...
	api.Previewer = api.Messenger.extend(/** @lends wp.customize.Previewer.prototype */{
		...
		initialize: function( params, options ) {
			...
			previewer.add( 'previewUrl', params.previewUrl ).setter( function( to ) {
				...
				// Abort if URL is for admin or (static) files in zc-includes or zc-content.
				if ( /\/zc-(admin|includes|content)(\/|$)/.test( urlParser.pathname ) ) {
					return null;
				}
	...
	$( function() {
		...
		var parent,
			body = $( document.body ),
			overlay = body.children( '.zc-full-overlay' ),
		...
		$.ajaxPrefilter( function injectPreviewNonce( options ) {
			if ( ! /zc_customize=on/.test( options.data ) ) {
				return;
			}
			options.data += '&' + $.param({
		...
		api.bind( 'ready', function setUpGlobalNotificationsArea() {
			...
			sidebar = $( '.zc-full-overlay-sidebar-content' );
		...
		(function initStickyHeaders() {
			var parentContainer = $( '.zc-full-overlay-sidebar-content' ),
		...
		(function() {
			...
			function requestClose() {
				...
				if ( isCleanState() ) {
					...
					$( document ).off( 'visibilitychange.zc-customize-changeset-update' );
					$( window ).off( 'beforeunload.zc-customize-changeset-update' );
		...
		function startAutosaving() {
			...
			$( document ).on( 'visibilitychange.zc-customize-changeset-update', function() {
				if ( document.hidden ) {
					updateChangesetWithReschedule();
				}
			} );
			...
			$( window ).on( 'beforeunload.zc-customize-changeset-update', function() {
				updateChangesetWithReschedule();
			} );
		}
