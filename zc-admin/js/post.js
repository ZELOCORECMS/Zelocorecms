		...
		if ( data['zc-refresh-post-lock'] ) {
			...
			if ( received.lock_error ) {
				...
				if ( wrap.length && ! wrap.is(':visible') ) {
					...
					wrap.find('.zc-tab-first').trigger( 'focus' );
				}
...
(function($) {
	...
	$( function() {
		schedule();
	}).on( 'heartbeat-send.zc-refresh-nonces', function( e, data ) {
		var post_id,
			$authCheck = $('#zc-auth-check-wrap');

		if ( check || ( $authCheck.length && ! $authCheck.hasClass( 'hidden' ) ) ) {
			if ( ( post_id = $('#post_ID').val() ) && $('#_wpnonce').val() ) {
				data['zc-refresh-post-nonces'] = {
					post_id: post_id
				};
			}
		}
	}).on( 'heartbeat-tick.zc-refresh-nonces', function( e, data ) {
		var nonces = data['zc-refresh-post-nonces'];
...
jQuery( function($) {
	...
	$('#post-lock-dialog .notification-dialog').on( 'keydown', function(e) {
		...
		if ( target.hasClass('zc-tab-first') && e.shiftKey ) {
			$(this).find('.zc-tab-last').trigger( 'focus' );
			e.preventDefault();
		// [Tab] on last tab cycles back to first tab.
		} else if ( target.hasClass('zc-tab-last') && ! e.shiftKey ) {
			$(this).find('.zc-tab-first').trigger( 'focus' );
			e.preventDefault();
		}
	...
	$('#post-preview').on( 'click.post-preview', function( event ) {
		var $this = $(this),
			$form = $('form#post'),
			$previewField = $('input#zc-preview'),
			target = $this.attr('target') || 'zc-preview',
	...
	$('.categorydiv').each( function(){
		...
		// @todo Move to jQuery 1.3+, support for multiple hierarchical taxonomies, see zc-lists.js.
		...
		$('#' + taxonomy + '-add-toggle').on( 'click', function( e ) {
			e.preventDefault();
			$('#' + taxonomy + '-adder').toggleClass( 'zc-hidden-children' );
	...
	( function() {
		...
		function dragging( event ) {
			if ( $postdivrich.hasClass( 'zc-editor-expand' ) ) {
				return;
			}
		...
		function endDrag() {
			...
			if ( mce ) {
				editor.focus();
				toolbarHeight = parseInt( $( '#zc-content-editor-container .mce-toolbar-grp' ).height(), 10 );
		...
		$handle.on( 'mousedown.zc-editor-resize', function( event ) {
			...
			$document.on( 'mousemove.zc-editor-resize', dragging )
				.on( 'mouseup.zc-editor-resize mouseleave.zc-editor-resize', endDrag );

			event.preventDefault();
		}).on( 'mouseup.zc-editor-resize', endDrag );
	})();
	...
	$textarea.on( 'keydown.zc-autosave', function( event ) {
		// Key [S] has code 83.
		if ( event.which === 83 ) {
			if (
				event.shiftKey ||
	...
	if ( $( '#original_post_status' ).val() === 'auto-draft' && window.history.replaceState ) {
		...
		$( '#publish' ).on( 'click', function() {
			...
			location += 'zc-post-new-reload=true';
...
( function( $, counter ) {
	$( function() {
		var $content = $( '#content' ),
			$count = $( '#zc-word-count' ).find( '.word-count' ),
			prevCount = 0,
			contentEditor;
