...
( function( $, wp ) {
	...
	function SwitchEditors() {
		...
		function init() {
			if ( ! tinymce && window.tinymce ) {
				...
				$$( document ).on( 'click', function( event ) {
					...
					if ( target.hasClass( 'zc-switch-editor' ) ) {
						id = target.attr( 'data-zc-editor-id' );
		...
		function scrollVisualModeToStartElement( editor, element ) {
			var elementTop = editor.$( element ).offset().top,
				...
				edTools = $( '#zc-content-editor-tools' ),
				edToolsHeight = 0,
				edToolsOffsetTop = 0,
			 ...
			 * experience.
			 *
			 * In order to find the top of the editor, we calculate the offset of `#zc-content-editor-tools` and
			 * subtracting the height. This gives the scroll position where the top of the editor tools aligns with
			 * the top of the viewport (under the Master Bar)
			 */
			var adjustedScroll;
			if ( editor.settings.zc_autoresize_on ) {
		 ...
		 * `zc-includes/js/tinymce/plugins/zelocorecms/plugin.js`.
		 *
		...
		function removep( html ) {
			...
			if ( html.indexOf( '[caption' ) !== -1 ) {
				...
				html = html.replace( /\[caption[\s\S]+?\[\/caption\]/g, function( a ) {
					return a.replace( /<br([^>]*)>/g, '<zc-temp-br$1>' ).replace( /[\r\n\t]+/, '' );
				});
			}
			...
			if ( preserve_linebreaks ) {
				html = html.replace( /<zc-line-break>/g, '\n' );
			}

			if ( preserve_br ) {
				html = html.replace( /<zc-temp-br([^>]*)>/g, '<br$1>' );
			}

			// Restore preserved tags.
			if ( preserve.length ) {
				html = html.replace( /<zc-preserve>/g, function() {
		...
		function autop( text ) {
			...
			if ( preserve_linebreaks ) {
				text = text.replace( /<zc-line-break>/g, '\n' );
			}

			// Restore the <br> tags in captions.
			if ( preserve_br ) {
				text = text.replace( /<zc-temp-br([^>]*)>/g, '<br$1>' );
			}
	...
	/**
	 * Initialize TinyMCE and/or Quicktags. For use with zc_enqueue_editor() (PHP).
	 *
	 * Intended for use with an existing textarea that will become the Code editor tab.
	 * The editor width will be the width of the textarea container, height will be adjustable.
	 *
	 * Settings for both TinyMCE and Quicktags can be passed on initialization, and are "filtered"
	 * with custom jQuery events on the document element, zc-before-tinymce-init and zc-before-quicktags-init.
	 *
	 * @since 4.8.0
	 *
	...
	wp.editor.initialize = function( id, settings ) {
		...
		if ( settings.tinymce && settings.quicktags ) {
			...
			var $wrap = $( '<div>' ).attr( {
					'class': 'zc-core-ui zc-editor-wrap tmce-active',
					id: 'zc-' + id + '-wrap'
				} );

			var $editorContainer = $( '<div class="zc-editor-container">' );

			var $button = $( '<button>' ).attr( {
					type: 'button',
					'data-zc-editor-id': id
				} );

			var $editorTools = $( '<div class="zc-editor-tools">' );
			...
			if ( settings.mediaButtons ) {
				...
				$addMediaButton.append( '<span class="zc-media-buttons-icon" aria-hidden="true"></span>' );
				$addMediaButton.append( document.createTextNode( ' ' + buttonText ) );
				$addMediaButton.data( 'editor', id );

				$editorTools.append(
					$( '<div class="zc-media-buttons">' )
			...
			$wrap.append(
				$editorTools
					.append( $( '<div class="zc-editor-tabs">' )
						.append( $button.clone().attr({
							id: id + '-tmce',
							'class': 'zc-switch-editor switch-tmce'
						}).text( window.tinymce.translate( 'Visual' ) ) )
						.append( $button.attr({
							id: id + '-html',
							'class': 'zc-switch-editor switch-html'
		...
		if ( window.tinymce && settings.tinymce ) {
			...
			$( document ).trigger( 'zc-before-tinymce-init', init );
		...
		if ( window.quicktags && settings.quicktags ) {
			...
			$( document ).trigger( 'zc-before-quicktags-init', init );
