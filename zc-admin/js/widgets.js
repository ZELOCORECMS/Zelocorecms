...
window.wpWidgets = {
	...
	init : function() {
		...
		$( '#widgets-right .sidebar-name' )
			...
			.on( 'click', function() {
				...
				$document.triggerHandler( 'zc-pin-menu' );
			})
			/*
	...
	saveOrder : function( sidebarId ) {
		...
		$.post( ajaxurl, data, function() {
			$( '#inactive-widgets-control-remove' ).prop( 'disabled' , ! $( '#zc_inactive_widgets .widget' ).length );
		...
		$.post( ajaxurl, data, function(r) {
			...
			if ( del ) {
				...
				if ( animate ) {
					...
					if ( sidebarId === 'zc_inactive_widgets' ) {
						$( '#inactive-widgets-control-remove' ).prop( 'disabled' , ! $( '#zc_inactive_widgets .widget' ).length );
					}
				}
			} else {
	...
	removeInactiveWidgets : function() {
		...
		$.post( ajaxurl, data, function() {
			$( '#zc_inactive_widgets .widget' ).each(function() {
				var $widget = $( this );
