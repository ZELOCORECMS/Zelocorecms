 ...
 * @since 2.1.0
 * @output zc-admin/js/edit-comments.js
 */
...
window.setCommentsList = function() {
	...
	delBefore = function( settings, list ) {
		var note, id, el, n, h, a, author,
			action = false,
			wpListsData = $( settings.target ).attr( 'data-zc-lists' );
		...
		if ( action ) {
			...
			a.attr('data-zc-lists', 'delete:the-comment-list:comment-' + id + '::un' + action + '=1');
	...
	refillTheExtraList = function(ev) {
		...
		if (ev) {
			theExtraList.empty();
			args.number = Math.min(8, per_page); // See ZC_Comments_List_Table::prepare_items() in class-zc-comments-list-table.php.
		} else {
			args.number = 1;
	...
	window.theList = $('#the-comment-list').wpList( { alt: '', delBefore: delBefore, dimAfter: dimAfter, delAfter: delAfter, addColor: 'none' } )
		.on('wpListDelEnd', function(e, s){
			var wpListsData = $(s.target).attr('data-zc-lists'), id = s.element.replace(/[^0-9]+/g, '');
...
$( function(){
	...
	if ( typeof $.table_hotkeys != 'undefined' ) {
		 ...
		 */
		toggle_all = function() {
			$('#cb-select-all-1').data( 'zc-toggle', 1 ).trigger( 'click' ).removeData( 'zc-toggle' );
		};
