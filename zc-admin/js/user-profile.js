/**
 * @output zc-admin/js/user-profile.js
 */
...
(function($) {
	...
	function bindToggleButton() {
		...
		$toggleButton = $pass1Row.find('.zc-hide-pw');
	...
	function bindPasswordForm() {
		...
		$submitButton = $( '#submit, #zc-submit' ).on( 'click', function () {
			updateLock = false;
		});
		...
		$passwordWrapper = $pass1Row.find( '.zc-pwd' );
		$generateButton  = $pass1Row.find( 'button.zc-generate-pw' );
		...
		$cancelButton = $pass1Row.find( 'button.zc-cancel-pw' );
	...
	$( function() {
		var $colorpicker, $stylesheet, user_id, current_user_id,
			...
			greeting     = $( '#zc-admin-bar-my-account' ).find( '.display-name' );
	...
	$( function() {
		if ( $( '.reset-pass-submit' ).length ) {
			$( '.reset-pass-submit button.zc-generate-pw' ).trigger( 'click' );
		}
	});
