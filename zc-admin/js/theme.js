...
themes.view.Appearance = zc.Backbone.View.extend({
	...
	search: function() {
		...
		this.searchContainer
			.find( '.search-box' )
			.append( $.parseHTML( '<label for="zc-filter-search-input">' + l10n.search + '</label>' ) )
			.append( view.el );
...
themes.view.Details = zc.Backbone.View.extend({
	...
	containFocus: function( $el ) {
		...
		$el.on( 'keydown.zc-themes', function( event ) {
	...
	autoupdateState: function() {
		...
		callback = function( event, data ) {
			...
			if ( _this.model.get( 'id' ) === data.asset ) {
				...
				$( document ).off( 'zc-auto-update-setting-changed', callback );
			}
		};

		// Triggered in updates.js
		$( document ).on( 'zc-auto-update-setting-changed', callback );
	},
...
themes.view.Preview = themes.view.Details.extend({
	...
	className: 'zc-full-overlay expanded',
	...
	togglePreviewDeviceButtons: function( newDevice ) {
		var $devices = $( '.zc-full-overlay-footer .devices' );
...
themes.Router = Backbone.Router.extend({
	...
	search: function( query ) {
		$( '.zc-filter-search' ).val( query.replace( /\+/g, ' ' ) );
	},
...
themes.Run = {
	...
	routes: function() {
		...
		themes.router.on( 'route:search', function() {
			$( '.zc-filter-search' ).trigger( 'keyup' );
		});
...
themes.view.InstallerSearch =  themes.view.Search.extend({
	...
	doSearch: function( value ) {
		...
		// Get the themes by sending Ajax POST request to api.zelocorecms.org/themes
...
themes.view.Installer = themes.view.Appearance.extend({
	...
	// Save the user's ZelocoreCMS.org username and get his favorite themes.
...
themes.RunInstaller = {
	...
	routes: function() {
		...
		themes.router.on( 'route:search', function() {
			$( '.zc-filter-search' ).trigger( 'focus' ).trigger( 'keyup' );
		});
