 ...
 *
 * @output zc-admin/js/revisions.js
 */
...
(function($) {
	...
	revisions.init = function() {
		...
		state = new revisions.model.FrameState({
			initialDiffState: {
				// zc_localize_script doesn't stringifies ints, so cast them.
				to: parseInt( revisions.settings.to, 10 ),
				from: parseInt( revisions.settings.from, 10 ),
				// zc_localize_script does not allow for top-level booleans so do a comparator here.
