/**
 * @output zc-admin/js/customize-widgets.js
 */
...
(function( wp, $ ){
	...
	api.Widgets.WidgetControl = api.Control.extend(/** @lends wp.customize.Widgets.WidgetControl.prototype */{
		...
		_setupWideWidget: function() {
			...
			$customizeSidebar = $( '.zc-full-overlay-sidebar-content:first' );
		...
		_setupRemoveUI: function() {
			...
			replaceDeleteWithRemove = function() {
				$removeBtn.text( l10n.removeBtnLabel ); // zc_widget_control() outputs the button as "Delete".
	...
	api.Widgets.WidgetsPanel = api.Panel.extend(/** @lends wp.customize.Widgets.WigetsPanel.prototype */{
		...
		ready: function () {
			...
			panel.deferred.embedded.done(function() {
				...
				// @todo This should use the Notifications API introduced to panels. See <https://core.trac.zelocorecms.org/ticket/38794>.
				...
				_( removedWidgetIds ).each( function( removedWidgetId ) {
					...
					setTimeout( function() {
						...
						api.each( function( otherSetting ) {
							if ( otherSetting.id === self.setting.id || 0 !== otherSetting.id.indexOf( 'sidebars_widgets[' ) || otherSetting.id === 'sidebars_widgets[zc_inactive_widgets]' ) {
								return;
							}
						...
						if ( api.Widgets.savedWidgetIds[removedWidgetId] ) {
							inactiveWidgets = api.value( 'sidebars_widgets[zc_inactive_widgets]' )().slice();
							inactiveWidgets.push( removedWidgetId );
							api.value( 'sidebars_widgets[zc_inactive_widgets]' )( _( inactiveWidgets ).unique() );
						}
