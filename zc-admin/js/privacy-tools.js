/**
 * Interactions used by the User Privacy tools in ZelocoreCMS.
 *
 * @output zc-admin/js/privacy-tools.js
 */
			...
			$.ajax(
				{
					url: window.ajaxurl,
					data: {
						action: 'zc-privacy-export-personal-data',
	...
	$( '.remove-personal-data-handle' ).on( 'click', function( event ) {
		...
		function doNextErasure( eraserIndex, pageIndex ) {
			$.ajax({
				url: window.ajaxurl,
				data: {
					action: 'zc-privacy-erase-personal-data',
					eraser: eraserIndex,
					id: requestID,
					page: pageIndex,
					security: nonce
