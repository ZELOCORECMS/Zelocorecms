/**
 * Functions for ajaxified updates, deletions and installs inside the ZelocoreCMS admin.
 *
 * @version 4.2.0
 * @output zc-admin/js/updates.js
 */
...
(function( $, wp, settings ) {
	...
	wp.updates.addAdminNotice = function( data ) {
		...
		$document.trigger( 'zc-updates-notice-added' );
	};

	/**
	 * Handles Ajax requests to ZelocoreCMS.
	 *
	 * @since 4.6.0
	 *
	...
	wp.updates.updatePluginSuccess = function( response ) {
		...
		$document.trigger( 'zc-plugin-update-success', response );
	};
	...
	wp.updates.updatePluginError = function( response ) {
		...
		$document.trigger( 'zc-plugin-update-error', response );
	};
	...
	wp.updates.installPluginError = function( response ) {
		...
		$document.trigger( 'zc-plugin-install-error', response );
	};
	...
	wp.updates.deletePluginError = function( response ) {
		...
		$document.trigger( 'zc-plugin-delete-error', response );
	};
	...
	wp.updates.installThemeSuccess = function( response ) {
		...
		$document.trigger( 'zc-theme-install-success', response );
	...
	$( function() {
		var $pluginFilter        = $( '#plugin-filter, #plugin-information-footer' ),
			...
			$pluginSearch        = $( '.plugins-php .zc-filter-search' ),
			$pluginInstallSearch = $( '.plugin-install-php .zc-filter-search' );
		...
		$bulkActionForm.on( 'click', '[type="submit"]:not([name="clear-recent-list"])', function( event ) {
			...
			$document.trigger( 'zc-' + type + '-bulk-' + bulkAction, itemsSelected );
			...
			$document.on( 'zc-plugin-update-success zc-plugin-update-error zc-theme-update-success zc-theme-update-error', function( event, response ) {
				var $itemRow = $( '[data-slug="' + response.slug + '"]' ),
					$bulkActionNotice, itemName;

				if ( 'zc-' + response.update + '-update-success' === event.type ) {
					success++;
				} else {
		...
		$pluginInstallSearch.on( 'keyup input', _.debounce( function( event, eventtype ) {
			...
			if ( ! $searchTab.length ) {
				...
				$( '.zc-filter .filter-links .current' )
					.removeClass( 'current' )
		...
		$document.on( 'submit', '.search-plugins', function( event ) {
			...
			$( 'input.zc-filter-search' ).trigger( 'input' );
		} );
		...
		$document.on( 'click keyup', '.column-auto-updates .toggle-auto-update, .theme-overlay .toggle-auto-update', function( event ) {
			var data, asset, type, $parent,
				$toggler = $( this ),
				action = $toggler.attr( 'data-zc-action' ),
			...
			$.post( window.ajaxurl, data )
				.done( function( response ) {
					...
					if ( 'enable' === action ) {
						...
						$toggler.attr( 'data-zc-action', 'disable' );
						...
						$toggler.attr( 'data-zc-action', 'enable' );
