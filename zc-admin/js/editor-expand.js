/**
 * @output zc-admin/js/editor-expand.js
 */
...
( function( window, $, undefined ) {
	...
	$( function() {
		var $wrap = $( '#postdivrich' ),
			$contentWrap = $( '#zc-content-wrap' ),
			$tools = $( '#zc-content-editor-tools' ),
			$visualTop = $(),
			$visualEditor = $(),
			$textTop = $( '#ed_toolbar' ),
			$textEditor = $( '#content' ),
		...
		$document.on( 'tinymce-editor-init.editor-expand', function( event, editor ) {
			...
			mceBind = function() {
				...
				editor.on( 'zc-toolbar-toggle', toggleAdvanced );

				// Adjust when the editor resizes.
				editor.on( 'setcontent zc-autoresize zc-toolbar-toggle', adjust );
			...
			mceUnbind = function() {
				...
				editor.off( 'zc-toolbar-toggle', toggleAdvanced );
				editor.off( 'setcontent zc-autoresize zc-toolbar-toggle', adjust );
			...
			if ( $wrap.hasClass( 'zc-editor-expand' ) ) {
		...
		function adjust( event ) {
			...
			if ( ! canPin ) {
				...
				if ( ( ! fixedBottom || ( resize && advanced ) ) &&
						// Add borderWidth for the border around the .zc-editor-container.
		...
		function on() {
			...
			$wrap.addClass( 'zc-editor-expand' );
			 ...
			 */
			$document.on( 'zc-collapse-menu.editor-expand postboxes-columnchange.editor-expand editor-classchange.editor-expand', adjust )
				.on( 'postbox-toggled.editor-expand postbox-moved.editor-expand', function() {
					...
					adjust();
				}).on( 'zc-window-resized.editor-expand', function() {
					if ( mceEditor && ! mceEditor.isHidden() ) {
			...
			if ( mceEditor ) {
				mceEditor.settings.zc_autoresize_on = true;
		...
		function off() {
			...
			$wrap.removeClass( 'zc-editor-expand' );
			...
			if ( mceEditor ) {
				mceEditor.settings.zc_autoresize_on = false;
		...
		$document.on( 'tinymce-editor-init.focus', function( event, editor ) {
			...
			if ( editor.id === 'content' ) {
				...
				mceBind = function() {
					...
					editor.on( 'blur', blur );
					editor.on( 'zc-autoresize', recalcEditorRect );
				};
				...
				mceUnbind = function() {
					...
					editor.off( 'zc-autoresize', recalcEditorRect );
				};
