 ...
 *
 * @output zc-admin/js/tags-suggest.js
 */
( function( $ ) {
	var tempID = 0;
	 ...
	 *
	 * The taxonomy can be passed in a `data-zc-taxonomy` attribute on the element or
	...
	$.fn.wpTagsSuggest = function( options ) {
		...
		var taxonomy = options.taxonomy || $element.attr( 'data-zc-taxonomy' ) || 'post_tag';
		...
		options = $.extend( {
			...
			focus: function( event, ui ) {
				$element.attr( 'aria-activedescendant', 'zc-tags-autocomplete-' + ui.item.id );
		...
		$element.autocomplete( 'instance' )._renderItem = function( ul, item ) {
			return $( '<li role="option" id="zc-tags-autocomplete-' + item.id + '">' )
				.text( item.name )
				.appendTo( ul );
		};
