 ...
 */
global $post_type, $post_type_object, $post, $title, $zc_meta_boxes;

$block_editor_context = new ZC_Block_Editor_Context( array( 'post' => $post ) );
...
zc_enqueue_script( 'zc-edit-post' );
...
$meta_box_url = add_query_arg(
	array(
		...
		'meta-box-loader-nonce' => zc_create_nonce( 'meta-box-loader' ),
	),
	$meta_box_url
);
zc_add_inline_script(
	'zc-editor',
	sprintf( 'var _wpMetaBoxUrl = %s;', zc_json_encode( $meta_box_url, JSON_HEX_TAG | JSON_UNESCAPED_SLASHES ) ),
	'before'
);
 ...
 */
$available_templates = zc_get_theme()->get_page_templates( get_post( $post->ID ) );
...
$editor_settings = array(
	...
	'postLockUtils'        => array(
		'nonce'       => zc_create_nonce( 'lock-post_' . $post->ID ),
		'unlockNonce' => zc_create_nonce( 'update-post_' . $post->ID ),
...
$editor_settings['__experimentalAdditionalBlockPatterns']          = ZC_Block_Patterns_Registry::get_instance()->get_all_registered( true );
$editor_settings['__experimentalAdditionalBlockPatternCategories'] = ZC_Block_Pattern_Categories_Registry::get_instance()->get_all_registered( true );
...
if ( zc_is_block_theme() && $editor_settings['supportsTemplateMode'] ) {
...
zc_tinymce_inline_scripts();
zc_enqueue_editor();

/**
 * Styles
 */
zc_enqueue_style( 'zc-edit-post' );
 ...
 * `zc_enqueue_style` to add your functionality to the block editor.
 *
 * @since 5.0.0
 */
...
$core_meta_boxes = $zc_meta_boxes[ $current_screen->id ]['normal']['core'];
...
$script = sprintf(
	...
	zc_json_encode( $initial_edits, JSON_HEX_TAG | JSON_UNESCAPED_SLASHES )
);
zc_add_inline_script( 'zc-edit-post', $script );

if ( (int) get_option( 'page_for_posts' ) === $post->ID ) {
	add_action( 'admin_enqueue_scripts', '_zc_block_editor_posts_page_notice' );
}
...
<div class="block-editor">
	...
	<div class="wrap hide-if-js block-editor-no-js">
		...
		<?php
		if ( file_exists( ZC_PLUGIN_DIR . '/classic-editor/classic-editor.php' ) ) {
			// If Classic Editor is already installed, provide a link to activate the plugin.
			$installed           = true;
			$plugin_activate_url = zc_nonce_url( 'plugins.php?action=activate&amp;plugin=classic-editor/classic-editor.php', 'activate-plugin_classic-editor/classic-editor.php' );
			$message             = sprintf(
			...
			$installed          = false;
			$plugin_install_url = zc_nonce_url( self_admin_url( 'update.php?action=install-plugin&plugin=classic-editor' ), 'install-plugin_classic-editor' );
			$message            = sprintf(
