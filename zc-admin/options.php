 ...
 *
 * @since 3.2.0
 * @since 7.0.0 Applied when `zc-admin/options.php` is accessed directly.
 *
...
if ( ! empty( $_GET['adminhash'] ) ) {
	...
	zc_redirect( admin_url( $redirect ) );
	exit;
} elseif ( ! empty( $_GET['dismiss'] ) && 'new_admin_email' === $_GET['dismiss'] ) {
	check_admin_referer( 'dismiss-' . get_current_blog_id() . '-new_admin_email' );
	delete_option( 'adminhash' );
	delete_option( 'new_admin_email' );
	zc_redirect( admin_url( 'options-general.php?updated=true' ) );
	exit;
}
...
if ( ! is_multisite() ) {
	 ...
	 * or upload_path is not the default ('zc-content/uploads' or empty),
	 * they can be edited, otherwise they're locked.
	 */
	if ( get_option( 'upload_url_path' )
		|| get_option( 'upload_path' ) && 'zc-content/uploads' !== get_option( 'upload_path' )
	) {
...
if ( 'update' === $action ) { // We are saving settings sent from a settings page.
	...
	if ( 'options' === $option_page ) {
		if ( is_multisite() && ! current_user_can( 'manage_network_options' ) ) {
			zc_die( __( 'Sorry, you are not allowed to modify unregistered settings for this site.' ) );
		}
		$options = isset( $_POST['page_options'] ) ? explode( ',', zc_unslash( $_POST['page_options'] ) ) : null;
	} else {
	...
	if ( 'general' === $option_page ) {
		...
		if ( ! empty( $_POST['date_format'] ) && isset( $_POST['date_format_custom'] )
			&& '\c\u\s\t\o\m' === zc_unslash( $_POST['date_format'] )
		) {
			$_POST['date_format'] = $_POST['date_format_custom'];
		}

		if ( ! empty( $_POST['time_format'] ) && isset( $_POST['time_format_custom'] )
			&& '\c\u\s\t\o\m' === zc_unslash( $_POST['time_format'] )
		) {
		...
		if ( ! empty( $_POST['WPLANG'] ) && current_user_can( 'install_languages' ) ) {
			...
			if ( zc_can_install_language_pack() ) {
			...
			if ( $unregistered ) {
				_deprecated_argument(
					...
					sprintf(
						...
						__( 'https://developer.zelocorecms.org/plugins/settings/settings-api/' )
					)
				);
			}
	...
	$goback = add_query_arg( 'settings-updated', 'true', zc_get_referer() );
	zc_redirect( $goback );
	exit;
}
...
<div class="wrap">
	...
	<form name="form" action="options.php" method="post" id="all-options">
		<?php zc_nonce_field( 'options-options' ); ?>
...
foreach ( (array) $options as $option ) :
	...
	if ( 'home' === $option->option_name && defined( 'ZC_HOME' ) ) {
		$disabled = true;
	}

	if ( 'siteurl' === $option->option_name && defined( 'ZC_SITEURL' ) ) {
		$disabled = true;
	}
	...
	if ( is_serialized( $option->option_value ) ) {
		...
		$value    = _zc_connectors_mask_api_key( $option->option_value );
		$disabled = true;
	} else {
