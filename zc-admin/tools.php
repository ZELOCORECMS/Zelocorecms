...
if ( isset( $_GET['page'] ) && ! empty( $_POST ) ) {
	...
	if ( 'export_personal_data' === $_GET['page'] ) {
		require_once ABSPATH . 'zc-admin/export-personal-data.php';
		return;
	} elseif ( 'remove_personal_data' === $_GET['page'] ) {
		require_once ABSPATH . 'zc-admin/erase-personal-data.php';
		return;
	}
}
...
if ( isset( $_GET['zc-privacy-policy-guide'] ) ) {
	...
	if ( 'export_personal_data' === $_GET['page'] ) {
		...
		require_once dirname( __DIR__ ) . '/zc-load.php';
		zc_redirect( admin_url( 'erase-personal-data.php' ), 301 );
		exit;
	}
}
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://zelocorecms.org/documentation/article/tools-screen/">Documentation on Tools</a>' ) . '</p>' .
