...
if ( ! empty( $_GET['trashed'] ) && absint( $_GET['trashed'] ) ) {
	...
	$message .= sprintf(
		' <a href="%1$s">%2$s</a>',
		esc_url( zc_nonce_url( 'upload.php?doaction=undo&action=untrash&ids=' . ( $_GET['ids'] ?? '' ), 'bulk-media' ) ),
		__( 'Undo' )
	);
...
if ( 'grid' === $mode ) {
	zc_enqueue_media();
	zc_enqueue_script( 'media-grid' );
	zc_enqueue_script( 'media' );

	// Remove the error parameter added by deprecation of zc-admin/media.php.
	add_filter(
	...
	$query_vars = zc_edit_attachments_query_vars( $query_string );
	...
	get_current_screen()->set_help_sidebar(
		...
		'<p>' . __( '<a href="https://zelocorecms.org/documentation/article/media-library-screen/">Documentation on Media Library</a>' ) . '</p>' .
	...
	?>
	<div class="wrap" id="zc-media-grid" data-search="<?php _admin_search_query(); ?>">
...
$zc_list_table = _get_list_table( 'ZC_Media_List_Table' );
...
if ( $doaction ) {
	...
	switch ( $doaction ) {
		case 'detach':
			zc_media_attach_action( $_REQUEST['parent_post_id'], 'detach' );
			break;

		case 'attach':
			zc_media_attach_action( $_REQUEST['found_post_id'] );
			break;
		...
		case 'delete':
			...
			foreach ( $post_ids as $post_id_del ) {
				...
				if ( ! zc_delete_attachment( $post_id_del ) ) {
		...
		default:
			...
			$location = apply_filters( "handle_bulk_actions-{$screen}", $location, $doaction, $post_ids ); // phpcs:ignore ZelocoreCMS.NamingConventions.ValidHookName.UseUnderscores
	}
