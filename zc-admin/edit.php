...
if ( ! in_array( $typenow, get_post_types( array( 'show_ui' => true ) ), true ) ) {
	zc_die( __( 'Sorry, you are not allowed to edit posts in this post type.' ) );
}

if ( 'attachment' === $typenow ) {
	if ( zc_redirect( admin_url( 'upload.php' ) ) ) {
		exit;
	}
}
...
$zc_list_table = _get_list_table( 'ZC_Posts_List_Table' );
$pagenum       = $zc_list_table->get_pagenum();

// Back-compat for viewing comments of an entry.
foreach ( array( 'p', 'attachment_id', 'page_id' ) as $_redirect ) {
	if ( ! empty( $_REQUEST[ $_redirect ] ) ) {
		zc_redirect( admin_url( 'edit-comments.php?p=' . absint( $_REQUEST[ $_redirect ] ) ) );
		exit;
	}
}
unset( $_redirect );
...
if ( $doaction ) {
	...
	$sendback = remove_query_arg( array( 'trashed', 'untrashed', 'deleted', 'locked', 'ids' ), zc_get_referer() );
	if ( ! $sendback ) {
	...
	switch ( $doaction ) {
		case 'trash':
			...
			foreach ( (array) $post_ids as $post_id ) {
				if ( ! current_user_can( 'delete_post', $post_id ) ) {
					zc_die( __( 'Sorry, you are not allowed to move this item to the Trash.' ) );
				}
				...
				if ( ! zc_trash_post( $post_id ) ) {
					zc_die( __( 'Error in moving the item to Trash.' ) );
				}
		...
		case 'untrash':
			...
			if ( isset( $_GET['doaction'] ) && ( 'undo' === $_GET['doaction'] ) ) {
				add_filter( 'zc_untrash_post_status', 'zc_untrash_post_set_previous_status', 10, 3 );
			}

			foreach ( (array) $post_ids as $post_id ) {
				if ( ! current_user_can( 'delete_post', $post_id ) ) {
					zc_die( __( 'Sorry, you are not allowed to restore this item from the Trash.' ) );
				}

				if ( ! zc_untrash_post( $post_id ) ) {
					zc_die( __( 'Error in restoring the item from Trash.' ) );
				}
			...
			remove_filter( 'zc_untrash_post_status', 'zc_untrash_post_set_previous_status', 10 );
		...
		case 'delete':
			...
			foreach ( (array) $post_ids as $post_id ) {
				...
				if ( ! current_user_can( 'delete_post', $post_id ) ) {
					zc_die( __( 'Sorry, you are not allowed to delete this item.' ) );
				}

				if ( 'attachment' === $post_del->post_type ) {
					if ( ! zc_delete_attachment( $post_id ) ) {
						zc_die( __( 'Error in deleting the attachment.' ) );
					}
				} else {
					if ( ! zc_delete_post( $post_id ) ) {
						zc_die( __( 'Error in deleting the item.' ) );
					}
				}
				++$deleted;
			}
		...
		default:
			 ...
			 */
			$sendback = apply_filters( "handle_bulk_actions-{$screen}", $sendback, $doaction, $post_ids ); // phpcs:ignore ZelocoreCMS.NamingConventions.ValidHookName.UseUnderscores
			break;
	}
	...
	zc_redirect( $sendback );
	exit;
} elseif ( ! empty( $_REQUEST['_zc_http_referer'] ) ) {
...
zc_enqueue_script( 'inline-edit-post' );
zc_enqueue_script( 'heartbeat' );

if ( 'zc_block' === $post_type ) {
	zc_enqueue_script( 'zc-list-reusable-blocks' );
	zc_enqueue_style( 'zc-list-reusable-blocks' );
}
...
if ( 'post' === $post_type ) {
	...
	get_current_screen()->set_help_sidebar(
		...
		'<p>' . __( '<a href="https://zelocorecms.org/documentation/article/posts-screen/">Documentation on Managing Posts</a>' ) . '</p>' .
	...
	get_current_screen()->set_help_sidebar(
		'<p><strong>' . __( 'For more information:' ) . '</strong></p>' .
		'<p>' . __( '<a href="https://zelocorecms.org/documentation/article/pages-screen/">Documentation on Managing Pages</a>' ) . '</p>' .
		'<p>' . __( '<a href="https://zelocorecms.org/support/forums/">Support forums</a>' ) . '</p>'
	);
...
foreach ( $bulk_counts as $message => $count ) {
	...
	if ( 'trashed' === $message && isset( $_REQUEST['ids'] ) ) {
		...
		$messages[] = sprintf(
			'<a href="%1$s">%2$s</a>',
			esc_url( zc_nonce_url( "edit.php?post_type=$post_type&doaction=undo&action=untrash&ids=$ids", 'bulk-posts' ) ),
			__( 'Undo' )
		);
	}
...
<?php $zc_list_table->search_box( $post_type_object->labels->search_items, 'post' ); ?>
