<?php
/**
 * ZelocoreCMS Plugin Install Administration API
 *
 * @package ZelocoreCMS
...
/**
 * Retrieves plugin installer pages from the ZelocoreCMS.org Plugins API.
 *
 ...
 *
 * The second filter, {@see 'plugins_api'}, allows a plugin to override the ZelocoreCMS.org
 ...
 *         @type bool $requires          Whether to return the required ZelocoreCMS version. Default true.
 ...
 *         @type bool $added             Whether to return the date when the plugin was added to the zelocorecms.org
 *                                       repository. Default true.
 *         @type bool $tags              Whether to return the assigned tags. Default true.
 *         @type bool $compatibility     Whether to return the ZelocoreCMS compatibility list. Default true.
 ...
 *         {@link https://developer.zelocorecms.org/reference/functions/plugins_api/ function reference article}
...
function plugins_api( $action, $args = array() ) {
	...
	/**
	 * Filters the ZelocoreCMS.org Plugin Installation API arguments.
	 *
	...
	/**
	 * Filters the response for the current ZelocoreCMS.org Plugin Installation API request.
	 *
	...
	if ( false === $res ) {
		...
		$url = 'http://api.zelocorecms.org/plugins/info/1.2/';
		$url = add_query_arg(
			array(
				'action'  => $action,
				'request' => $args,
		...
		if ( is_zc_error( $request ) ) {
			...
			if ( isset( $res->error ) ) {
				$res = new ZC_Error( 'plugins_api_failed', $res->error );
			}
		}
	} elseif ( ! is_zc_error( $res ) ) {
		$res->external = true;
	}
	 ...
	 * @since 2.7.0
	 *
	 * @param object|ZC_Error $res    Response object or ZC_Error.
	 * @param string          $action The type of information being requested from the Plugin Installation API.
	 * @param object          $args   Plugin API arguments.
	 */
...
/**
 * Retrieves popular ZelocoreCMS plugin tags.
 *
 * @since 2.7.0
 *
 * @param array $args
...
function install_dashboard() {
	...
	if ( is_zc_error( $api_tags ) ) {
		echo $api_tags->get_error_message();
	} else {
		// Set up the tags in a way which can be interpreted by zc_generate_tag_cloud().
		$tags = array();
...
function install_search_form( $deprecated = true ) {
	$type = isset( $_REQUEST['type'] ) ? zc_unslash( $_REQUEST['type'] ) : 'term';
	$term = isset( $_REQUEST['s'] ) ? urldecode( zc_unslash( $_REQUEST['s'] ) ) : '';
	?>
	<form class="search-form search-plugins" method="get">
		<input type="hidden" name="tab" value="search" />
		<label for="search-plugins"><?php _e( 'Search Plugins' ); ?></label>
		<input type="search" name="s" id="search-plugins" value="<?php echo esc_attr( $term ); ?>" class="zc-filter-search" />
...
<div class="upload-plugin">
	...
	<form method="post" enctype="multipart/form-data" class="zc-upload-form" action="<?php echo esc_url( self_admin_url( 'update.php?action=upload-plugin' ) ); ?>">
		<?php zc_nonce_field( 'plugin-upload' ); ?>
...
function install_plugins_favorites_form() {
	...
	?>
	<p><?php _e( 'If you have marked plugins as favorites on ZelocoreCMS.org, you can browse them here.' ); ?></p>
	<form method="get">
		<input type="hidden" name="tab" value="favorites" />
		<p>
			<label for="user"><?php _e( 'Your ZelocoreCMS.org username:' ); ?></label>
			<input type="search" id="user" name="user" value="<?php echo esc_attr( $user ); ?>" />
			<input type="submit" class="button" value="<?php esc_attr_e( 'Get Favorites' ); ?>" />
			<input type="hidden" id="wporg-username-nonce" name="_wpnonce" value="<?php echo esc_attr( zc_create_nonce( $action ) ); ?>" />
		</p>
	</form>
	<?php
}
	...
	switch ( current_filter() ) {
		case 'install_plugins_beta':
			printf(
				...
				'<p>' . __( 'You are using a development version of ZelocoreCMS. These feature plugins are also under development. <a href="%s">Learn more</a>.' ) . '</p>',
				'https://make.zelocorecms.org/core/handbook/about/release-cycle/features-as-plugins/'
			);
			break;
...
function install_plugin_install_status( $api, $loop = false ) {
		...
		foreach ( (array) $update_plugins->response as $file => $plugin ) {
			if ( $plugin->slug === $api->slug ) {
				...
				if ( current_user_can( 'update_plugins' ) ) {
					$url = zc_nonce_url( self_admin_url( 'update.php?action=upgrade-plugin&plugin=' . $update_file ), 'upgrade-plugin_' . $update_file );
				}
				break;
			}
		}
	...
	if ( 'install' === $status ) {
		if ( is_dir( ZC_PLUGIN_DIR . '/' . $api->slug ) ) {
			$installed_plugin = get_plugins( '/' . $api->slug );
			if ( empty( $installed_plugin ) ) {
				if ( current_user_can( 'install_plugins' ) ) {
					$url = zc_nonce_url( self_admin_url( 'update.php?action=install-plugin&plugin=' . $api->slug ), 'install-plugin_' . $api->slug );
				}
			} else {
	...
	if ( isset( $_GET['from'] ) ) {
		$url .= '&amp;from=' . urlencode( zc_unslash( $_GET['from'] ) );
	}
...
function install_plugin_information() {
	...
	$api = plugins_api(
		'plugin_information',
		array(
			'slug' => zc_unslash( $_REQUEST['plugin'] ),
		)
	);
	...
	foreach ( (array) $api->sections as $section_name => $content ) {
		$api->sections[ $section_name ] = zc_kses( $content, $plugins_allowedtags );
	}

	foreach ( array( 'version', 'author', 'requires', 'tested', 'homepage', 'downloaded', 'slug' ) as $key ) {
		if ( isset( $api->$key ) ) {
			$api->$key = zc_kses( $api->$key, $plugins_allowedtags );
		}
	}
	...
	$section = isset( $_REQUEST['section'] ) ? zc_unslash( $_REQUEST['section'] ) : 'description';
...
<div id="<?php echo $_tab; ?>-content" class='<?php echo $_with_banner; ?>'>
	<div class="fyi">
		<ul>
			...
			<?php } if ( ! empty( $api->requires ) ) { ?>
				<li>
					<strong><?php _e( 'Requires ZelocoreCMS Version:' ); ?></strong>
					<?php
			...
			<?php } if ( ! empty( $api->slug ) && empty( $api->external ) ) { ?>
				<li><a target="_blank" href="<?php echo esc_url( __( 'https://zelocorecms.org/plugins/' ) . $api->slug ); ?>/"><?php _e( 'ZelocoreCMS.org Plugin Page &#187;' ); ?></a></li>
		...
		if ( ! empty( $api->ratings ) && array_sum( (array) $api->ratings ) > 0 ) {
			...
			<p class="fyi-description"><?php _e( 'Read all reviews on ZelocoreCMS.org or write your own!' ); ?></p>
			<?php
				...
				<div class="counter-container">
						<span class="counter-label">
							...
							printf(
								...
								"https://zelocorecms.org/support/plugin/{$api->slug}/reviews/?filter={$key}",
								$aria_label,
	...
	if ( ! $compatible_php ) {
		...
		if ( current_user_can( 'update_php' ) ) {
			...
			) . zc_update_php_annotation( '</p><p><em>', '</em>', false );
		} else {
			$compatible_php_notice_message .= '</p>';
		}
	...
	if ( ! $tested_wp ) {
		zc_admin_notice(
			__( '<strong>Warning:</strong> This plugin <strong>has not been tested</strong> with your current version of ZelocoreCMS.' ),
			array(
				'type'               => 'warning',
				'additional_classes' => array( 'notice-alt' ),
			)
		...
		$compatible_zc_notice_message = __( '<strong>Error:</strong> This plugin <strong>requires a newer version of ZelocoreCMS</strong>.' );
		if ( current_user_can( 'update_core' ) ) {
			$compatible_zc_notice_message .= sprintf(
				/* translators: %s: URL to ZelocoreCMS Updates screen. */
				' ' . __( '<a href="%s" target="_parent">Click here to update ZelocoreCMS</a>.' ),
				esc_url( self_admin_url( 'update-core.php' ) )
			);
		}

		zc_admin_notice(
			$compatible_zc_notice_message,
			array(
				'type'               => 'error',
				'additional_classes' => array( 'notice-alt' ),
			)
	...
	foreach ( (array) $api->sections as $section_name => $content ) {
		$content = links_add_base_url( $content, 'https://zelocorecms.org/plugins/' . $api->slug . '/' );
	...
	if ( ! empty( $api->download_link ) && ( current_user_can( 'install_plugins' ) || current_user_can( 'update_plugins' ) ) ) {
		$button = zc_get_plugin_action_button( $api->name, $api, $compatible_php, $compatible_wp );
				...
				if ( is_plugin_active( $status['file'] ) ) {
					...
					if ( $compatible_php && $compatible_wp && $all_plugin_dependencies_active ) {
						...
						$activate_url = add_query_arg(
							array(
								'_wpnonce' => zc_create_nonce( 'activate-plugin_' . $status['file'] ),
								'action'   => 'activate',
								'plugin'   => $status['file'],
							),
