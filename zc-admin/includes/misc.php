<?php
/**
 * Misc ZelocoreCMS Administration API.
 *
 * @package ZelocoreCMS
...
dynamically generated, and should only be modified via ZelocoreCMS filters.
...
function save_mod_rewrite_rules() {
	 ...
	 */
	if ( ! file_exists( $htaccess_file ) && is_writable( $home_path ) && $zc_rewrite->using_mod_rewrite_permalinks()
		|| is_writable( $htaccess_file )
	) {
		if ( got_mod_rewrite() ) {
			$rules = explode( "\n", $zc_rewrite->mod_rewrite_rules() );

			return insert_with_markers( $htaccess_file, 'ZelocoreCMS', $rules );
		}
	}
...
function iis7_save_url_rewrite_rules() {
	...
	if ( iis7_supports_permalinks()
		&& ( ! file_exists( $web_config_file ) && win_is_writable( $home_path ) && $zc_rewrite->using_mod_rewrite_permalinks()
			|| win_is_writable( $web_config_file ) )
	) {
		$rule = $zc_rewrite->iis7_url_rewrite_rules( false );
 ...
 */
function zc_make_theme_file_tree( $allowed_files ) {
	$tree_list = array();
 ...
 */
function zc_print_theme_file_tree( $tree, $level = 2, $size = 1, $index = 1 ) {
	...
	if ( is_array( $tree ) ) {
		...
		foreach ( $tree as $label => $theme_file ) :
			...
			if ( ! is_array( $theme_file ) ) {
				zc_print_theme_file_tree( $theme_file, $level, $index, $size );
				continue;
			}
			?>
			<li role="treeitem" aria-expanded="true" tabindex="-1"
				...
				<ul role="group" class="tree-folder"><?php zc_print_theme_file_tree( $theme_file, $level + 1, $index, $size ); ?></ul>
			</li>
			<?php
		endforeach;
	} else {
		...
		<li role="none" class="<?php echo esc_attr( $relative_file === $filename ? 'current-file' : '' ); ?>">
			<a role="treeitem" tabindex="<?php echo esc_attr( $relative_file === $filename ? '0' : '-1' ); ?>"
				...
				if ( $file_description !== $filename && zc_basename( $filename ) !== $file_description ) {
 ...
 */
function zc_make_plugin_file_tree( $plugin_editable_files ) {
	$tree_list = array();
 ...
 * @param int          $index The aria-posinset for the current iteration.
 */
function zc_print_plugin_file_tree( $tree, $label = '', $level = 2, $size = 1, $index = 1 ) {
	global $file, $plugin;
	...
	if ( is_array( $tree ) ) {
		...
		foreach ( $tree as $label => $plugin_file ) :
			...
			if ( ! is_array( $plugin_file ) ) {
				zc_print_plugin_file_tree( $plugin_file, $label, $level, $index, $size );
				continue;
			}
			?>
			<li role="treeitem" aria-expanded="true" tabindex="-1"
				...
				<ul role="group" class="tree-folder"><?php zc_print_plugin_file_tree( $plugin_file, '', $level + 1, $index, $size ); ?></ul>
			</li>
			<?php
		endforeach;
	} else {
 ...
 * @since 2.1.0
 *
 * @param string|ZC_Error $message The message to display, or a ZC_Error object.
 */
function show_message( $message ) {
	if ( is_zc_error( $message ) ) {
 ...
 */
function zc_doc_link_parse( $content ) {
...
function set_screen_options() {
	if ( ! isset( $_POST['zc_screen_options'] ) || ! is_array( $_POST['zc_screen_options'] ) ) {
		return;
	}
	...
	$option = $_POST['zc_screen_options']['option'];
	$value  = $_POST['zc_screen_options']['value'];
	...
	switch ( $map_option ) {
		...
		default:
			...
			if ( str_ends_with( $option, '_page' ) || 'layout_columns' === $option ) {
				 ...
				 */
				$screen_option = apply_filters( 'set-screen-option', $screen_option, $option, $value ); // phpcs:ignore ZelocoreCMS.NamingConventions.ValidHookName.UseUnderscores
			}
	...
	$url = remove_query_arg( array( 'pagenum', 'apage', 'paged' ), zc_get_referer() );
...
/**
 * Checks if rewrite rule for ZelocoreCMS already exists in the IIS 7+ configuration file.
 *
 * @since 2.8.0
 *
...
function iis7_rewrite_rule_exists( $filename ) {
	...
	$rules = $xpath->query( '/configuration/system.webServer/rewrite/rules/rule[starts-with(@name,\'zelocorecms\')] | /configuration/system.webServer/rewrite/rules/rule[starts-with(@name,\'ZelocoreCMS\')]' );
...
/**
 * Deletes ZelocoreCMS rewrite rule from web.config file if it exists there.
 *
 * @since 2.8.0
 *
...
/**
 * Adds ZelocoreCMS rewrite rule to the IIS 7+ configuration file.
 *
 * @since 2.8.0
 *
 * @param string $filename     The file path to the configuration file.
...
function iis7_add_rewrite_rule( $filename, $rewrite_rule ) {
	...
	$zelocorecms_rules = $xpath->query( '/configuration/system.webServer/rewrite/rules/rule[starts-with(@name,\'zelocorecms\')] | /configuration/system.webServer/rewrite/rules/rule[starts-with(@name,\'ZelocoreCMS\')]' );

	if ( $zelocorecms_rules->length > 0 ) {
		return true;
	}
 ...
 */
function saveDomDocument( $doc, $filename ) { // phpcs:ignore ZelocoreCMS.NamingConventions.ValidFunctionName.FunctionNameInvalid
	$config = $doc->saveXML();
...
function admin_color_scheme_picker( $user_id ) {
	...
	ksort( $_zc_admin_css_colors );

	if ( isset( $_zc_admin_css_colors['modern'] ) ) {
		// Set Modern (new default), Classic ('fresh'), and Light first.
		$_zc_admin_css_colors = array_filter(
			array_merge(
				array(
					'modern' => '',
					'fresh'  => '',
	...
	if ( empty( $current_color ) || ! isset( $_zc_admin_css_colors[ $current_color ] ) ) {
	...
	<fieldset id="color-picker" class="scheme-list">
		...
		<?php
		zc_nonce_field( 'save-color-scheme', 'color-nonce', false );
		foreach ( $_zc_admin_css_colors as $color => $color_info ) :
			...
			<div class="color-option <?php echo ( $color === $current_color ) ? 'selected' : ''; ?>">
				...
				<input type="hidden" class="icon_colors" value="<?php echo esc_attr( zc_json_encode( array( 'icons' => $color_info->icon_colors ) ) ); ?>" />
 ...
 * @global array $_zc_admin_css_colors
 */
function zc_color_scheme_settings() {
	global $_zc_admin_css_colors;

	$color_scheme = get_user_option( 'admin_color' );

	// It's possible to have a color scheme set that is no longer registered.
	if ( empty( $_zc_admin_css_colors[ $color_scheme ] ) ) {
		$color_scheme = 'modern';
	}

	if ( ! empty( $_zc_admin_css_colors[ $color_scheme ]->icon_colors ) ) {
		$icon_colors = $_zc_admin_css_colors[ $color_scheme ]->icon_colors;
	} elseif ( ! empty( $_zc_admin_css_colors['modern']->icon_colors ) ) {
		$icon_colors = $_zc_admin_css_colors['modern']->icon_colors;
	} else {
	...
	echo '<script>var _wpColorScheme = ' . zc_json_encode( array( 'icons' => $icon_colors ), JSON_HEX_TAG | JSON_UNESCAPED_SLASHES ) . ";</script>\n";
}
 ...
 * @since 5.5.0
 */
function zc_admin_viewport_meta() {
	/**
 ...
 */
function zc_check_locked_posts( $response, $data, $screen_id ) {
	$checked = array();

	if ( array_key_exists( 'zc-check-locked-posts', $data ) && is_array( $data['zc-check-locked-posts'] ) ) {
		foreach ( $data['zc-check-locked-posts'] as $key ) {
			...
			$user_id = zc_check_post_lock( $post_id );
	...
	if ( ! empty( $checked ) ) {
		$response['zc-check-locked-posts'] = $checked;
	}
 ...
 * @return array The Heartbeat response.
 */
function zc_refresh_post_lock( $response, $data, $screen_id ) {
	if ( array_key_exists( 'zc-refresh-post-lock', $data ) ) {
		$received = $data['zc-refresh-post-lock'];
		$send     = array();
		...
		if ( $user ) {
			...
			$new_lock = zc_set_post_lock( $post_id );
		...
		$response['zc-refresh-post-lock'] = $send;
	}
 ...
 * @return array The Heartbeat response.
 */
function zc_refresh_post_nonces( $response, $data, $screen_id ) {
	if ( array_key_exists( 'zc-refresh-post-nonces', $data ) ) {
		$received = $data['zc-refresh-post-nonces'];

		$response['zc-refresh-post-nonces'] = array( 'check' => 1 );
		...
		$response['zc-refresh-post-nonces'] = array(
			'replace' => array(
				'getpermalinknonce'    => zc_create_nonce( 'getpermalink' ),
				'samplepermalinknonce' => zc_create_nonce( 'samplepermalink' ),
				'closedpostboxesnonce' => zc_create_nonce( 'closedpostboxes' ),
				'_ajax_linking_nonce'  => zc_create_nonce( 'internal-linking' ),
				'_wpnonce'             => zc_create_nonce( 'update-post_' . $post_id ),
			),
		);
	}
 ...
 * @return array The Heartbeat response.
 */
function zc_refresh_metabox_loader_nonces( $response, $data ) {
	if ( empty( $data['zc-refresh-metabox-loader-nonces'] ) ) {
		return $response;
	}

	$received = $data['zc-refresh-metabox-loader-nonces'];
	...
	$response['zc-refresh-metabox-loader-nonces'] = array(
		'replace' => array(
			'metabox_loader_nonce' => zc_create_nonce( 'meta-box-loader' ),
			'_wpnonce'             => zc_create_nonce( 'update-post_' . $post_id ),
		),
	);
 ...
 * @return array The Heartbeat response.
 */
function zc_refresh_heartbeat_nonces( $response ) {
	// Refresh the Rest API nonce.
	$response['rest_nonce'] = zc_create_nonce( 'zc_rest' );

	// Refresh the Heartbeat nonce.
	$response['heartbeat_nonce'] = zc_create_nonce( 'heartbeat-nonce' );
 ...
 */
function zc_heartbeat_set_suspension( $settings ) {
	global $pagenow;
...
function heartbeat_autosave( $response, $data ) {
	if ( ! empty( $data['zc_autosave'] ) ) {
		$saved = zc_autosave( $data['zc_autosave'] );

		if ( is_zc_error( $saved ) ) {
			$response['zc_autosave'] = array(
				'success' => false,
 ...
 * @since 4.2.0
 */
function zc_admin_canonical_url() {
	$removable_query_args = zc_removable_query_args();
	 ...
	 */
	$filtered_url = apply_filters( 'zc_admin_canonical_url', $filtered_url );
	?>
	<link id="zc-admin-canonical" rel="canonical" href="<?php echo esc_url( $filtered_url ); ?>" />
	<script>
		if ( window.history.replaceState ) {
			window.history.replaceState( null, null, document.getElementById( 'zc-admin-canonical' ).href + window.location.hash );
		}
	</script>
	<?php
}
 ...
 * @since 4.6.0
 */
function zc_page_reload_on_back_button_js() {
	?>
	<script>
...
###SITEURL###'
	...
	$content      = str_replace( '###SITENAME###', zc_specialchars_decode( get_option( 'blogname' ), ENT_QUOTES ), $content );
	$content      = str_replace( '###SITEURL###', home_url(), $content );

	if ( '' !== get_option( 'blogname' ) ) {
		$site_title = zc_specialchars_decode( get_option( 'blogname' ), ENT_QUOTES );
	} else {
 ...
 * @param ZC_Post $page  Page data object.
 * @return string Page title.
 */
function _zc_privacy_settings_filter_draft_page_titles( $title, $page ) {
 ...
 *
 * @since 5.1.0
 * @since 5.1.1 Added the {@see 'zc_is_php_version_acceptable'} filter.
 *
 * @return array|false {
 *     Array of PHP version data. False on failure.
 *
 *     @type string $recommended_version The PHP version recommended by ZelocoreCMS.
...
function zc_check_php_version() {
	...
	if ( false === $response ) {
		$url = 'http://api.zelocorecms.org/core/serve-happy/1.0/';
	...
	if ( isset( $response['is_acceptable'] ) && $response['is_acceptable'] ) {
		/**
		 * Filters whether the active PHP version is considered acceptable by ZelocoreCMS.
		 *
		 * Returning false will trigger a PHP version warning to show up in the admin dashboard to administrators.
		 *
		 * This filter is only run if the zelocorecms.org Serve Happy API considers the PHP version acceptable, ensuring
		 ...
		 */
		$response['is_acceptable'] = (bool) apply_filters( 'zc_is_php_version_acceptable', true, $version );
	}
