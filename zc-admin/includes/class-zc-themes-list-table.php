<?php
/**
 * List Table API: ZC_Themes_List_Table class
 *
 * @package ZelocoreCMS
 ...
 * @see ZC_List_Table
 */
class ZC_Themes_List_Table extends ZC_List_Table {
	...
	public function prepare_items() {
		...
		if ( ! empty( $_REQUEST['s'] ) ) {
			$this->search_terms = array_unique( array_filter( array_map( 'trim', explode( ',', strtolower( zc_unslash( $_REQUEST['s'] ) ) ) ) ) );
		}
		...
		if ( is_multisite() ) {
			...
			if ( current_user_can( 'install_themes' ) ) {
				printf(
					...
					__( 'You only have one theme installed right now. Live a little! You can choose from over 1,000 free themes in the ZelocoreCMS Theme Directory at any time: just click on the <a href="%s">Install Themes</a> tab above.' ),
	...
	public function display_rows() {
		...
		foreach ( $themes as $theme ) :
			...
			$activate_link = zc_nonce_url( 'themes.php?action=activate&amp;template=' . urlencode( $template ) . '&amp;stylesheet=' . urlencode( $stylesheet ), 'switch-theme_' . $stylesheet );
			...
			if ( ! is_multisite() && current_user_can( 'delete_themes' ) ) {
				$actions['delete'] = sprintf(
					...
					zc_nonce_url( 'themes.php?action=delete&amp;stylesheet=' . urlencode( $stylesheet ), 'delete-theme_' . $stylesheet ),
					/* translators: %s: Theme name. */
			...
			/** This filter is documented in zc-admin/includes/class-zc-ms-themes-list-table.php */
			...
			<a href="<?php echo zc_customize_url( $stylesheet ); ?>" class="screenshot load-customize hide-if-no-customize">
				<?php if ( $screenshot ) : ?>
					<img src="<?php echo esc_url( $screenshot . '?ver=' . $theme->version ); ?>" alt="" />
				<?php endif; ?>
			</a>
	...
	public function _js_vars( $extra_args = array() ) {
		$search_string = isset( $_REQUEST['s'] ) ? esc_attr( zc_unslash( $_REQUEST['s'] ) ) : '';
