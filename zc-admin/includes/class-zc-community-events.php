...
/**
 * Class ZC_Community_Events.
 *
 * A client for api.zelocorecms.org/events.
 *
 * @since 4.8.0
 */
...
class ZC_Community_Events {
	/**
	 * ID for a ZelocoreCMS user account.
	 *
	 * @since 4.8.0
	 *
	 * @var int
	...
	/**
	 * Constructor for ZC_Community_Events.
	 *
	 * @since 4.8.0
	 *
	 ...
	 * than having the browser make the request directly to api.zelocorecms.org,
	 ...
	 * @return array|ZC_Error A ZC_Error on failure; an array with location and events on
	 *                        success.
	 */
	public function get_events( $location_search = '', $timezone = '' ) {
		...
		$request_args['user-agent'] = 'ZelocoreCMS/' . $zc_version . '; ' . home_url( '/' );

		if ( zc_http_supports( array( 'ssl' ) ) ) {
			$api_url = set_url_scheme( $api_url, 'https' );
		}

		$response       = zc_remote_get( $api_url, $request_args );
		$response_code  = zc_remote_retrieve_response_code( $response );
		$response_body  = json_decode( zc_remote_retrieve_body( $response ), true );
		$response_error = null;

		if ( is_zc_error( $response ) ) {
			$response_error = $response;
		} elseif ( 200 !== $response_code ) {
			$response_error = new ZC_Error(
				'api-error',
	...
	protected function get_request_args( $search = '', $timezone = '' ) {
		...
		// Wrap the args in an array compatible with the second parameter of `zc_remote_get()`.
		return array(
			'body' => $args,
		);
	}
	...
	public static function get_unsafe_client_ip() {
		...
		$anon_ip = zc_privacy_anonymize_ip( $client_ip, true );
			...
			foreach ( $response_body['events'] as $key => $event ) {
				...
				if ( isset( $event['end_date'] ) ) {
					...
					if ( 'meetup' !== $event['type'] && $formatted_end_date !== $formatted_date ) {
						...
						$formatted_date = zc_maybe_decline_date( $formatted_date, 'F j, Y' );
					}
				}
	...
	protected function trim_events( array $events ) {
		...
		$trimmed_event_types = zc_list_pluck( $trimmed_events, 'type' );
