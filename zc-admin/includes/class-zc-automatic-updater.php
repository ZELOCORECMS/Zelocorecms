<?php
/**
 * Upgrade API: ZC_Automatic_Updater class
 *
 * @package ZelocoreCMS
 * @subpackage Upgrader
 * @since 4.6.0
...
class ZC_Automatic_Updater {
	...
	public function is_disabled() {
		...
		if ( ! zc_is_file_mod_allowed( 'automatic_updater' ) ) {
			return true;
		}
		...
		// More fine grained control can be done through the ZC_AUTO_UPDATE_CORE constant and filters.
	 ...
	 * the PHP `open_basedir` restrictions, so that ZelocoreCMS does not try to access
	 ...
	 * zc-content directory, for example). The underlying assumption is that if you are
	...
	public function should_update( $type, $item, $context ) {
		// Used to see if ZC_Filesystem is set up to allow unattended updates.
		...
		if ( 'core' === $type ) {
			...
			if ( ! $update && zc_is_auto_update_enabled_for_type( $type ) ) {
		 ...
		 *
		 * Since ZelocoreCMS 3.7, minor and development versions of core, and translations have
		 * been auto-updated by default. New installs on ZelocoreCMS 5.6 or higher will also
	...
	protected function send_core_update_notification_email( $item ) {
		 ...
		 * from ZelocoreCMS.org sets a particular flag. This allows some discretion
		 * in if and when to notify.
		 *
		 * This filter is only evaluated once per release. If the same email address
		 * was already notified of the same new version, ZelocoreCMS won't repeatedly
	...
	public function update( $type, $item ) {
		...
		switch ( $type ) {
			...
			case 'plugin':
				...
				$context  = ZC_PLUGIN_DIR; // We don't support custom Plugin directories, or updates for ZCMU_PLUGIN_DIR.
				break;
			case 'theme':
			...
			case 'translation':
				...
				$context  = ZC_CONTENT_DIR; // ZC_LANG_DIR;
				break;
		}
		...
		switch ( $type ) {
			case 'core':
				...
				$skin->feedback( __( 'Updating to ZelocoreCMS %s' ), $item->version );
				/* translators: %s: ZelocoreCMS version. */
				$item_name = sprintf( __( 'ZelocoreCMS %s' ), $item->version );
				break;
			case 'theme':
				$upgrader_item = $item->theme;
				$theme         = zc_get_theme( $upgrader_item );
			...
			case 'translation':
				...
				/* translators: %s: Project name (plugin, theme, or ZelocoreCMS). */
				$item_name = sprintf( __( 'Translations for %s' ), $language_item_name );
				/* translators: 1: Project name (plugin, theme, or ZelocoreCMS), 2: Language. */
		...
		$is_debug = ZC_DEBUG && ZC_DEBUG_LOG;
		if ( 'plugin' === $type ) {
		...
		/*
		 * After ZC_Upgrader::upgrade() completes, maintenance mode is disabled.
		 *
		...
		if ( false === $upgrade_result ) {
			$upgrade_result = new ZC_Error( 'fs_unavailable', __( 'Could not access filesystem.' ) );
		}

		if ( 'core' === $type ) {
			if ( is_zc_error( $upgrade_result )
			...
			if ( is_zc_error( $upgrade_result ) ) {
				...
				$skin->feedback( __( 'ZelocoreCMS updated successfully.' ) );
			}
		}
		...
		if ( 'plugin' === $type ) {
			...
			if ( $was_active && ! is_zc_error( $upgrade_result ) ) {
				...
				if ( $this->has_fatal_error() ) {
					...
					if ( is_zc_error( $backup_restored ) ) {
						$upgrade_result->add(
	...
	public function run() {
		...
		if ( ! ZC_Upgrader::create_lock( 'auto_updater' ) ) {
			return;
		}
		...
		remove_action( 'upgrader_process_complete', 'zc_version_check' );
		remove_action( 'upgrader_process_complete', 'zc_update_plugins' );
		remove_action( 'upgrader_process_complete', 'zc_update_themes' );

		// Next, plugins.
		zc_update_plugins(); // Check for plugin updates.
		...
		zc_update_themes();  // Check for theme updates.
		...
		zc_version_check(); // Check for core updates.
		...
		zc_update_themes( $theme_stats ); // Check for theme updates.
		...
		zc_update_plugins( $plugin_stats ); // Check for plugin updates.

		// Finally, process any new translations.
		$language_updates = zc_get_translation_updates();
		if ( $language_updates ) {
		...
		if ( ! empty( $this->update_results ) ) {
			$development_version = str_contains( zc_get_zc_version(), '-' );
		...
		ZC_Upgrader::release_lock( 'auto_updater' );
	}
	...
	protected function after_core_update( $update_result ) {
		...
		/*
		 * Any of these ZC_Error codes are critical failures, as in they occurred after we started to copy core files.
		 * We should not try to perform a background update again until there is a successful one-click update performed by the user.
		 */
		$critical = false;
		if ( 'disk_full' === $error_code || str_contains( $error_code, '__copy_dir' ) ) {
			$critical = true;
		} elseif ( 'rollback_was_required' === $error_code && is_zc_error( $result->get_error_data()->rollback ) ) {
		...
		/*
		 * Any other ZC_Error code (like download_failed or files_not_writable) occurs before
		 ...
		 * the issue could actually be on ZelocoreCMS.org's side.) If that one fails, then email.
		 */
		$send               = true;
		$transient_failures = array( 'incompatible_archive', 'download_failed', 'insane_distro', 'locked' );
		if ( in_array( $error_code, $transient_failures, true ) && ! get_site_option( 'auto_core_update_failed' ) ) {
			zc_schedule_single_event( time() + HOUR_IN_SECONDS, 'zc_maybe_auto_update' );
			$send = false;
		}
	 ...
	 * @param mixed  $result      Optional. The result for the core update. Can be ZC_Error.
	 */
	protected function send_email( $type, $core_update, $result = null ) {
		 ...
		 * @param object $core_update The update offer that was attempted.
		 * @param mixed  $result      The result for the core update. Can be ZC_Error.
		 */
		...
		switch ( $type ) {
			case 'success': // We updated.
				/* translators: Site updated notification email subject. 1: Site title, 2: ZelocoreCMS version. */
				$subject = __( '[%1$s] Your site has updated to ZelocoreCMS %2$s' );
				break;

			case 'fail':   // We tried to update but couldn't.
			case 'manual': // We can't update (and made no attempt).
				/* translators: Update available notification email subject. 1: Site title, 2: ZelocoreCMS version. */
				$subject = __( '[%1$s] ZelocoreCMS %2$s is available. Please update!' );
				break;
		...
		switch ( $type ) {
			case 'success':
				$body .= sprintf(
					/* translators: 1: Home URL, 2: ZelocoreCMS version. */
					__( 'Howdy! Your site at %1$s has been updated automatically to ZelocoreCMS %2$s.' ),
					home_url(),
					$core_update->current
				);
				$body .= "\n\n";
				...
				/* translators: %s: ZelocoreCMS version. */
				$body .= sprintf( __( 'For more on version %s, see the About ZelocoreCMS screen:' ), $about_version );
				$body .= "\n" . admin_url( 'about.php' );

				if ( $newer_version_available ) {
					/* translators: %s: ZelocoreCMS latest version. */
					$body .= "\n\n" . sprintf( __( 'ZelocoreCMS %s is also now available.' ), $next_user_core_update->current ) . ' ';
			...
			case 'manual':
				$body .= sprintf(
					/* translators: 1: Home URL, 2: ZelocoreCMS version. */
					__( 'Please update your site at %1$s to ZelocoreCMS %2$s.' ),
					home_url(),
			...
			case 'critical':
				if ( $newer_version_available ) {
					$body .= sprintf(
						/* translators: 1: Home URL, 2: ZelocoreCMS version. */
						__( 'Your site at %1$s experienced a critical failure while trying to update ZelocoreCMS to version %2$s.' ),
						home_url(),
						$core_update->current
					);
				} else {
					$body .= sprintf(
						/* translators: 1: Home URL, 2: ZelocoreCMS latest version. */
						__( 'Your site at %1$s experienced a critical failure while trying to update to the latest version of ZelocoreCMS, %2$s.' ),
						home_url(),
						$core_update->current
					);
				}
		...
		if ( $critical_support ) {
			...
			$body .= "\n\n" . sprintf(
				...
				__( 'The ZelocoreCMS team is willing to help you. Forward this email to %s and the team will work with you to make sure your site is working.' ),
				$core_update->support_email
			);
		} else {
			// Add a note about the support forums.
			$body .= "\n\n" . __( 'If you experience any issues or need support, the volunteers in the ZelocoreCMS.org support forums may be able to help.' );
		...
		if ( $critical_support ) {
			$body .= ' ' . __( "Reach out to ZelocoreCMS Core developers to ensure you'll never have this problem again." );
		}
		...
		$body .= "\n\n" . __( 'The ZelocoreCMS Team' ) . "\n";

		if ( 'critical' === $type && is_zc_error( $result ) ) {
			$body .= "\n***\n\n";
			/* translators: %s: ZelocoreCMS version. */
			 ...
			 * Loop through all errors (the main ZC_Error, the update result, the rollback result) for code, data, etc.
			 */
		 ...
		 *
		 * @param array $email {
		 *     Array of email arguments that will be passed to zc_mail().
		 *
		 *     @type string $to      The email recipient. An array of emails
		 *                            can be returned, as handled by zc_mail().
		...
		zc_mail( $email['to'], zc_specialchars_decode( $email['subject'] ), $email['body'], $email['headers'] );
	...
	protected function send_plugin_theme_email( $type, $successful_updates, $failed_updates ) {
		...
		// Add a note about the support forums.
		$body[] = __( 'If you experience any issues or need support, the volunteers in the ZelocoreCMS.org support forums may be able to help.' );
		$body[] = __( 'https://zelocorecms.org/support/forums/' );
		$body[] = "\n" . __( 'The ZelocoreCMS Team' );
		...
		$result = zc_mail( $email['to'], zc_specialchars_decode( $email['subject'] ), $email['body'], $email['headers'] );
	...
	protected function send_debug_email() {
		...
		$body[] = sprintf( __( 'ZelocoreCMS site: %s' ), network_home_url( '/' ) );

		// Core.
		if ( isset( $this->update_results['core'] ) ) {
			$result = $this->update_results['core'][0];

			if ( $result->result && ! is_zc_error( $result->result ) ) {
				/* translators: %s: ZelocoreCMS version. */
				$body[] = sprintf( __( 'SUCCESS: ZelocoreCMS was successfully updated to %s' ), $result->name );
			} else {
				/* translators: %s: ZelocoreCMS version. */
				$body[] = sprintf( __( 'FAILED: ZelocoreCMS failed to update to %s' ), $result->name );
				++$failures;
			}
		...
		foreach ( array( 'plugin', 'theme', 'translation' ) as $type ) {
			...
			$success_items = zc_list_filter( $this->update_results[ $type ], array( 'result' => true ) );
			...
			if ( $success_items ) {
				...
				$body[] = $messages[ $type ];
				foreach ( zc_list_pluck( $success_items, 'name' ) as $name ) {
			...
			if ( $success_items !== $this->update_results[ $type ] ) {
				...
				foreach ( $this->update_results[ $type ] as $item ) {
					if ( ! $item->result || is_zc_error( $item->result ) ) {
						/* translators: %s: Name of plugin / theme / translation. */
		...
		if ( '' !== get_bloginfo( 'name' ) ) {
			$site_title = zc_specialchars_decode( get_bloginfo( 'name' ), ENT_QUOTES );
		} else {
			$site_title = parse_url( home_url(), PHP_URL_HOST );
		}
...
This debugging email is sent when you are using a development version of ZelocoreCMS.

If you think these failures might be due to a bug in ZelocoreCMS, could you report it?
 * Open a thread in the support forums: https://zelocorecms.org/support/forum/alphabeta
 * Or, if you're comfortable writing a bug report: https://core.trac.zelocorecms.org/

Thanks! -- The ZelocoreCMS Team"
				)
			);
			$body[] = '';
...
=========='
	...
	protected function has_fatal_error() {
		...
		$response     = zc_remote_get( $url, compact( 'cookies', 'headers', 'timeout', 'sslverify' ) );
		...
		if ( $is_debug ) {
			error_log( var_export( substr( $response['body'], strpos( $response['body'], '###### zc_scraping_result_start:' ) ), true ) );
		}

		$body                   = zc_remote_retrieve_body( $response );
