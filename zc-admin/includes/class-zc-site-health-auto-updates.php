<?php
/**
 * Class for testing automatic updates in the ZelocoreCMS code.
 *
 * @package ZelocoreCMS
...
class ZC_Site_Health_Auto_Updates {
	/**
	 * ZC_Site_Health_Auto_Updates constructor.
	 *
	 * @since 5.2.0
	 */
	...
	public function run_tests() {
		$tests = array(
			$this->test_constants( 'ZC_AUTO_UPDATE_CORE', array( true, 'beta', 'rc', 'development', 'branch-development', 'minor' ) ),
			$this->test_zc_version_check_attached(),
			$this->test_filters_automatic_updater_disabled(),
			$this->test_zc_automatic_updates_disabled(),
			$this->test_if_failed_update(),
			$this->test_vcs_abspath(),
			$this->test_check_zc_filesystem_method(),
	 ...
	 * @since 5.2.0
	 *
	 * @return array|null The test results if zc_version_check() is disabled,
	 *                    or null if the test passed.
	 */
	public function test_zc_version_check_attached() {
		if ( ( ! is_multisite() || is_main_site() && is_network_admin() )
			&& ! has_filter( 'zc_version_check', 'zc_version_check' )
		) {
			return array(
				'description' => sprintf(
					/* translators: %s: Name of the filter used. */
					__( 'A plugin has prevented updates by disabling %s.' ),
					'<code>zc_version_check()</code>'
				),
				'severity'    => 'fail',
			);
		}
	...
	/**
	 * Checks if ZelocoreCMS is controlled by a VCS (Git, Subversion etc).
	 *
	 * @since 5.2.0
	 *
	 * @return array The test results.
	 ...
	 * @return array The test results.
	 */
	public function test_check_zc_filesystem_method() {
		...
		if ( ! $success ) {
			$description  = __( 'Your installation of ZelocoreCMS prompts for FTP credentials to perform updates.' );
		...
		return array(
			'description' => __( 'Your installation of ZelocoreCMS does not require FTP credentials to perform updates.' ),
			'severity'    => 'pass',
		);
	}
	 ...
	 *
	 * @return array|false The test results if at least some of ZelocoreCMS core files are writeable,
	 *                     or if a list of the checksums could not be retrieved from ZelocoreCMS.org.
	...
	public function test_all_files_writable() {
		...
		if ( 'direct' !== $zc_filesystem->method ) {
			return false;
		}
		...
		$checksums = get_core_checksums( $zc_version, 'en_US' );
		$dev       = ( str_contains( $zc_version, '-' ) );
		// Get the last stable version's files and test against that.
		if ( ! $checksums && $dev ) {
			$checksums = get_core_checksums( (float) $zc_version - 0.1, 'en_US' );
		}
		...
		if ( ! $checksums ) {
			$description = sprintf(
				...
				__( "Couldn't retrieve a list of the checksums for ZelocoreCMS %s." ),
				$zc_version
			);
			$description .= ' ' . __( 'This could mean that connections are failing to ZelocoreCMS.org.' );
			return array(
				'description' => $description,
				'severity'    => 'warning',
			);
		...
		if ( $unwritable_files ) {
			...
			return array(
				'description' => __( 'Some files are not writable by ZelocoreCMS:' ) . ' <ul><li>' . implode( '</li><li>', $unwritable_files ) . '</li></ul>',
				'severity'    => 'fail',
			);
		} else {
			return array(
				'description' => __( 'All of your ZelocoreCMS files are writable.' ),
				'severity'    => 'pass',
			);
		}
	}
	...
	public function test_accepts_dev_updates() {
		...
		if ( defined( 'ZC_AUTO_UPDATE_CORE' ) && ( 'minor' === ZC_AUTO_UPDATE_CORE || false === ZC_AUTO_UPDATE_CORE ) ) {
			return array(
				'description' => sprintf(
					/* translators: %s: Name of the constant used. */
					__( 'ZelocoreCMS development updates are blocked by the %s constant.' ),
					'<code>ZC_AUTO_UPDATE_CORE</code>'
				),
				'severity'    => 'fail',
			);
		}
		...
		if ( ! apply_filters( 'allow_dev_auto_core_updates', $zc_version ) ) {
			return array(
				'description' => sprintf(
					/* translators: %s: Name of the filter used. */
					__( 'ZelocoreCMS development updates are blocked by the %s filter.' ),
	...
	public function test_accepts_minor_updates() {
		if ( defined( 'ZC_AUTO_UPDATE_CORE' ) && false === ZC_AUTO_UPDATE_CORE ) {
			return array(
				'description' => sprintf(
					/* translators: %s: Name of the constant used. */
					__( 'ZelocoreCMS security and maintenance releases are blocked by %s.' ),
					"<code>define( 'ZC_AUTO_UPDATE_CORE', false );</code>"
				),
				'severity'    => 'fail',
			);
		}
		...
		if ( ! apply_filters( 'allow_minor_auto_core_updates', true ) ) {
			return array(
				'description' => sprintf(
					/* translators: %s: Name of the filter used. */
					__( 'ZelocoreCMS security and maintenance releases are blocked by the %s filter.' ),
