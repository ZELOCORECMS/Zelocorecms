<?php
/**
 * ZelocoreCMS Dashboard Widget Administration Screen API
 *
 * @package ZelocoreCMS
 ...
 * @global array $zc_registered_widget_controls
 * @global callable[] $zc_dashboard_control_callbacks
 */
function zc_dashboard_setup() {
	global $zc_registered_widgets, $zc_registered_widget_controls, $zc_dashboard_control_callbacks;

	$screen = get_current_screen();

	/* Register Widgets and Controls */
	$zc_dashboard_control_callbacks = array();

	// Browser version
	$check_browser = zc_check_browser_version();

	if ( $check_browser && $check_browser['upgrade'] ) {
		add_filter( 'postbox_classes_dashboard_dashboard_browser_nag', 'dashboard_browser_nag_class' );

		if ( $check_browser['insecure'] ) {
			zc_add_dashboard_widget( 'dashboard_browser_nag', __( 'You are using an insecure browser!' ), 'zc_dashboard_browser_nag' );
		} else {
			zc_add_dashboard_widget( 'dashboard_browser_nag', __( 'Your browser is out of date!' ), 'zc_dashboard_browser_nag' );
		}
	}

	// PHP Version.
	$check_php = zc_check_php_version();
	...
	if ( $check_php && current_user_can( 'update_php' ) ) {
		...
		if ( isset( $check_php['is_acceptable'] ) && ! $check_php['is_acceptable'] ) {
			...
			if ( $check_php['is_lower_than_future_minimum'] ) {
				zc_add_dashboard_widget( 'dashboard_php_nag', __( 'PHP Update Required' ), 'zc_dashboard_php_nag' );
			} else {
				zc_add_dashboard_widget( 'dashboard_php_nag', __( 'PHP Update Recommended' ), 'zc_dashboard_php_nag' );
			}
		}
	}
	...
	if ( current_user_can( 'view_site_health_checks' ) && ! is_network_admin() ) {
		...
		zc_enqueue_style( 'site-health' );
		zc_enqueue_script( 'site-health' );

		zc_add_dashboard_widget( 'dashboard_site_health', __( 'Site Health Status' ), 'zc_dashboard_site_health' );
	}

	// Right Now.
	if ( is_blog_admin() && current_user_can( 'edit_posts' ) ) {
		zc_add_dashboard_widget( 'dashboard_right_now', __( 'At a Glance' ), 'zc_dashboard_right_now' );
	}

	if ( is_network_admin() ) {
		zc_add_dashboard_widget( 'network_dashboard_right_now', __( 'Right Now' ), 'zc_network_dashboard_right_now' );
	}

	// Activity Widget.
	if ( is_blog_admin() ) {
		zc_add_dashboard_widget( 'dashboard_activity', __( 'Activity' ), 'zc_dashboard_site_activity' );
	}
	...
	if ( is_network_admin() ) {
		 ...
		 * @since 3.1.0
		 */
		do_action( 'zc_network_dashboard_setup' );
		 ...
		 */
		$dashboard_widgets = apply_filters( 'zc_network_dashboard_widgets', array() );
		 ...
		 * @since 3.1.0
		 */
		do_action( 'zc_user_dashboard_setup' );
		 ...
		 * @param string[] $dashboard_widgets An array of dashboard widget IDs.
		 */
		$dashboard_widgets = apply_filters( 'zc_user_dashboard_widgets', array() );
	} else {
		 ...
		 * @param string[] $dashboard_widgets An array of dashboard widget IDs.
		 */
		$dashboard_widgets = apply_filters( 'zc_dashboard_widgets', array() );
	}

	foreach ( $dashboard_widgets as $widget_id ) {
		$name = empty( $zc_registered_widgets[ $widget_id ]['all_link'] ) ? $zc_registered_widgets[ $widget_id ]['name'] : $zc_registered_widgets[ $widget_id ]['name'] . " <a href='{$zc_registered_widgets[$widget_id]['all_link']}' class='edit-box open-box'>" . __( 'View all' ) . '</a>';
		zc_add_dashboard_widget( $widget_id, $name, $zc_registered_widgets[ $widget_id ]['callback'], $zc_registered_widget_controls[ $widget_id ]['callback'] );
	}

	if ( 'POST' === $_SERVER['REQUEST_METHOD'] && isset( $_POST['widget_id'] ) ) {
		check_admin_referer( 'edit-dashboard-widget_' . $_POST['widget_id'], 'dashboard-widget-nonce' );
		ob_start(); // Hack - but the same hack zc-admin/widgets.php uses.
		zc_dashboard_trigger_widget_control( $_POST['widget_id'] );
		ob_end_clean();
		zc_redirect( remove_query_arg( 'edit' ) );
		exit;
	}

	/** This action is documented in zc-admin/includes/meta-boxes.php */
 ...
 */
function zc_add_dashboard_widget( $widget_id, $widget_name, $callback, $control_callback = null, $callback_args = null, $context = 'normal', $priority = 'core' ) {
	global $zc_dashboard_control_callbacks;
	...
	if ( $control_callback && is_callable( $control_callback ) && current_user_can( 'edit_dashboard' ) ) {
		$zc_dashboard_control_callbacks[ $widget_id ] = $control_callback;

		if ( isset( $_GET['edit'] ) && $widget_id === $_GET['edit'] ) {
			list($url)    = explode( '#', add_query_arg( 'edit', false ), 2 );
			$widget_name .= ' <span class="postbox-title-action"><a href="' . esc_url( $url ) . '">' . __( 'Cancel' ) . '</a></span>';
			$callback     = '_zc_dashboard_control_callback';
		} else {
 ...
 */
function _zc_dashboard_control_callback( $dashboard, $meta_box ) {
	echo '<form method="post" class="dashboard-widget-control-form zc-clearfix">';
	zc_dashboard_trigger_widget_control( $meta_box['id'] );
	zc_nonce_field( 'edit-dashboard-widget_' . $meta_box['id'], 'dashboard-widget-nonce' );
...
<div id="dashboard-widgets" class="metabox-holder<?php echo $columns_css; ?>">
	...
	<?php
	zc_nonce_field( 'closedpostboxes', 'closedpostboxesnonce', false );
	zc_nonce_field( 'meta-box-order', 'meta-box-order-nonce', false );
}
 ...
 * @since 2.7.0
 */
function zc_dashboard_right_now() {
	?>
	<div class="main">
	<ul>
	<?php
	...
	foreach ( array( 'post', 'page' ) as $post_type ) {
		$num_posts = zc_count_posts( $post_type );
 ...
 * @since 3.1.0
 */
function zc_network_dashboard_right_now() {
	$actions = array();
 ...
 */
function zc_dashboard_quick_press( $error_msg = false ) {
	global $post_ID;
	...
	<form name="post" action="<?php echo esc_url( admin_url( 'post.php' ) ); ?>" method="post" id="quick-press" class="initial-form hide-if-no-js">
		...
		<div class="input-text-wrap" id="title-wrap">
			<label for="title">
				<?php
				/** This filter is documented in zc-admin/edit-form-advanced.php */
 ...
 * @since 2.7.0
 *
 * @param ZC_Post[]|false $drafts Optional. Array of posts to display. Default false.
 */
function zc_dashboard_recent_drafts( $drafts = false ) {
	if ( ! $drafts ) {
		$query_args = array(
			'post_type'      => 'post',
			'post_status'    => 'draft',
	...
	foreach ( $drafts as $draft ) {
		...
		$the_content = zc_trim_words( $draft->post_content, $draft_length );
 ...
 *
 * @param ZC_Comment $comment   The current comment.
 * @param bool       $show_date Optional. Whether to display the date.
 */
function _zc_dashboard_recent_comments_row( &$comment, $show_date = true ) {
	...
	if ( current_user_can( 'edit_comment', $comment->comment_ID ) ) {
		...
		$approve_nonce = esc_html( '_wpnonce=' . zc_create_nonce( 'approve-comment_' . $comment->comment_ID ) );
		$del_nonce     = esc_html( '_wpnonce=' . zc_create_nonce( 'delete-comment_' . $comment->comment_ID ) );
		...
		$actions['approve'] = sprintf(
			'<a href="%s" data-zc-lists="%s" class="vim-a aria-button-if-js" aria-label="%s">%s</a>',
			esc_url( $approve_url ),
		...
		$actions['unapprove'] = sprintf(
			'<a href="%s" data-zc-lists="%s" class="vim-u aria-button-if-js" aria-label="%s">%s</a>',
		...
		$actions['spam'] = sprintf(
			'<a href="%s" data-zc-lists="%s" class="vim-s vim-destructive aria-button-if-js" aria-label="%s">%s</a>',
			esc_url( $spam_url ),
		...
		if ( ! EMPTY_TRASH_DAYS ) {
			$actions['delete'] = sprintf(
				'<a href="%s" data-zc-lists="%s" class="delete vim-d vim-destructive aria-button-if-js" aria-label="%s">%s</a>',
				esc_url( $delete_url ),
		...
		/** This filter is documented in zc-admin/includes/class-zc-comments-list-table.php */
	...
	?>
		...
		<li id="comment-<?php echo $comment->comment_ID; ?>" <?php comment_class( array( 'comment-item', zc_get_comment_status( $comment ) ), $comment ); ?>>
 ...
 * @since 3.8.0
 */
function zc_dashboard_site_activity() {

	echo '<div id="activity-widget">';

	$future_posts = zc_dashboard_recent_posts(
		array(
			'max'    => 5,
			'status' => 'future',
			'order'  => 'ASC',
	...
	$recent_posts = zc_dashboard_recent_posts(
		array(
			'max'    => 5,
			'status' => 'publish',
			'order'  => 'DESC',
	...
	$recent_comments = zc_dashboard_recent_comments();
 ...
 */
function zc_dashboard_recent_posts( $args ) {
	$query_args = array(
		'post_type'      => 'post',
	 ...
	 * @since 4.2.0
	 *
	 * @param array $query_args The arguments passed to ZC_Query to produce the list of posts.
	 */
	$query_args = apply_filters( 'dashboard_recent_posts_query_args', $query_args );

	$posts = new ZC_Query( $query_args );
 ...
 */
function zc_dashboard_recent_comments( $total_items = 5 ) {
	...
	if ( $comments ) {
		...
		echo '<ul id="the-comment-list" data-zc-lists="list:comment">';
		...
		if ( current_user_can( 'edit_posts' ) ) {
			...
			'</h3>';
			_get_list_table( 'ZC_Comments_List_Table' )->views();
		}

		zc_comment_reply( -1, false, 'dashboard', false );
		zc_comment_trashnotice();
 ...
 */
function zc_dashboard_rss_output( $widget_id ) {
	$widgets = get_option( 'dashboard_widget_options' );
	echo '<div class="rss-widget">';
	zc_widget_rss_output( $widgets[ $widget_id ] );
	echo '</div>';
}
 ...
 * which is later replaced by Ajax call (see top of /zc-admin/index.php)
 *
 * @since 2.5.0
 ...
 */
function zc_dashboard_cached_rss_widget( $widget_id, $callback, $check_urls = array(), ...$args ) {
	$doing_ajax = zc_doing_ajax();
	$loading    = '<p class="widget-loading hide-if-no-js">' . __( 'Loading&hellip;' ) . '</p>';
	$loading   .= zc_get_admin_notice(
 ...
 */
function zc_dashboard_trigger_widget_control( $widget_control_id = false ) {
	global $zc_dashboard_control_callbacks;

	if ( is_scalar( $widget_control_id ) && $widget_control_id
		&& isset( $zc_dashboard_control_callbacks[ $widget_control_id ] )
		&& is_callable( $zc_dashboard_control_callbacks[ $widget_control_id ] )
	) {
		call_user_func(
			$zc_dashboard_control_callbacks[ $widget_control_id ],
			'',
			array(
				'id'       => $widget_control_id,
				'callback' => $zc_dashboard_control_callbacks[ $widget_control_id ],
			)
		);
	}
}
...
/**
 * Sets up the RSS dashboard widget control and $args to be used as input to zc_widget_rss_form().
 *
 ...
 */
function zc_dashboard_rss_control( $widget_id, $form_inputs = array() ) {
	...
	$number = 1; // Hack to use zc_widget_rss_form().

	$widget_options[ $widget_id ]['number'] = $number;

	if ( 'POST' === $_SERVER['REQUEST_METHOD'] && isset( $_POST['widget-rss'][ $number ] ) ) {
		$_POST['widget-rss'][ $number ]         = zc_unslash( $_POST['widget-rss'][ $number ] );
		$widget_options[ $widget_id ]           = zc_widget_rss_process( $_POST['widget-rss'][ $number ] );
		$widget_options[ $widget_id ]['number'] = $number;
	...
	zc_widget_rss_form( $widget_options[ $widget_id ], $form_inputs );
}
 ...
 * @since 4.8.0
 */
function zc_dashboard_events_news() {
	zc_print_community_events_markup();

	?>

	<div class="zelocorecms-news hide-if-no-js">
		<?php zc_dashboard_primary(); ?>
	</div>
	...
	<p class="community-events-footer">
		<?php
			printf(
				...
				'https://make.zelocorecms.org/community/meetups-landing-page',
				__( 'Meetups' ),
				/* translators: Hidden accessibility text. */
		...
		<?php
			printf(
				'<a href="%1$s" target="_blank">%2$s <span class="screen-reader-text"> %3$s</span><span aria-hidden="true" class="dashicons dashicons-external"></span></a>',
				/* translators: If a Rosetta site exists (e.g. https://es.zelocorecms.org/news/), then use that. Otherwise, leave untranslated. */
				esc_url( _x( 'https://zelocorecms.org/news/', 'Events and News dashboard widget' ) ),
				__( 'News' ),
				/* translators: Hidden accessibility text. */
				__( '(opens in a new tab)' )
			);
 ...
 * @since 4.8.0
 */
function zc_print_community_events_markup() {
 ...
 * @since 4.8.0
 */
function zc_print_community_events_templates() {
	?>
	...
	<script id="tmpl-community-events-event-list" type="text/template">
		<# _.each( data.events, function( event ) { #>
			<li class="event event-{{ event.type }} zc-clearfix">
		...
		<# if ( data.events.length <= 2 ) { #>
			<li class="event-none">
				...
				printf(
					...
					__( 'https://make.zelocorecms.org/community/organize-event-landing-page/' )
				);
				?>
			</li>
		<# } #>
		...
		<li class="event-none">
			<# if ( data.location.description ) { #>
				...
				printf(
					...
					__( 'There are no events scheduled near %1$s at the moment. Would you like to <a href="%2$s">organize a ZelocoreCMS event</a>?' ),
					'{{ data.location.description }}',
					__( 'https://make.zelocorecms.org/community/handbook/meetup-organizer/welcome/' )
				);
				?>
			...
			<# } else { #>
				...
				printf(
					...
					__( 'There are no events scheduled near you at the moment. Would you like to <a href="%s">organize a ZelocoreCMS event</a>?' ),
					__( 'https://make.zelocorecms.org/community/handbook/meetup-organizer/welcome/' )
				);
				?>
			<# } #>
...
/**
 * 'ZelocoreCMS Events and News' dashboard widget.
 *
 * @since 2.7.0
 * @since 4.8.0 Removed popular plugins feed.
 */
function zc_dashboard_primary() {
	$feeds = array(
		'news'   => array(

			/**
			 * Filters the primary link URL for the 'ZelocoreCMS Events and News' dashboard widget.
			 *
			 * @since 2.5.0
			 *
			 * @param string $link The widget's primary link URL.
			 */
			'link'         => apply_filters( 'dashboard_primary_link', __( 'https://zelocorecms.org/news/' ) ),

			/**
			 * Filters the primary feed URL for the 'ZelocoreCMS Events and News' dashboard widget.
			 *
			 * @since 2.3.0
			 *
			 * @param string $url The widget's primary feed URL.
			 */
			'url'          => apply_filters( 'dashboard_primary_feed', __( 'https://zelocorecms.org/news/feed/' ) ),

			/**
			 * Filters the primary link title for the 'ZelocoreCMS Events and News' dashboard widget.
			 *
			 * @since 2.3.0
			 *
			 * @param string $title Title attribute for the widget's primary link.
			 */
			'title'        => apply_filters( 'dashboard_primary_title', __( 'ZelocoreCMS Blog' ) ),
			'items'        => 2,
			'show_summary' => 0,
			'show_author'  => 0,
			'show_date'    => 0,
		...
		'planet' => array(
			...
			/**
			 * Filters the secondary link URL for the 'ZelocoreCMS Events and News' dashboard widget.
			 *
			 * @since 2.3.0
			 *
			...
			/**
			 * Filters the secondary feed URL for the 'ZelocoreCMS Events and News' dashboard widget.
			 *
			 * @since 2.3.0
			 *
			...
			'url'          => apply_filters(
				...
				__( 'https://planet.zelocorecms.org/feed/' )
			),

			/**
			 * Filters the secondary link title for the 'ZelocoreCMS Events and News' dashboard widget.
			 *
			 * @since 2.3.0
			 *
			 * @param string $title Title attribute for the widget's secondary link.
			 */
			'title'        => apply_filters( 'dashboard_secondary_title', __( 'Other ZelocoreCMS News' ) ),

			/**
			 * Filters the number of secondary link items for the 'ZelocoreCMS Events and News' dashboard widget.
			 *
			 * @since 4.4.0
			 *
	...
	zc_dashboard_cached_rss_widget( 'dashboard_primary', 'zc_dashboard_primary_output', $feeds );
}

/**
 * Displays the ZelocoreCMS events and news feeds.
 *
 * @since 3.8.0
 * @since 4.8.0 Removed popular plugins feed.
 *
 ...
 */
function zc_dashboard_primary_output( $widget_id, $feeds ) {
	foreach ( $feeds as $type => $args ) {
		$args['type'] = $type;
		echo '<div class="rss-widget">';
			zc_widget_rss_output( $args['url'], $args );
		echo '</div>';
	}
}
 ...
 * @global bool $is_IE
 */
function zc_dashboard_browser_nag() {
	global $is_IE;

	$notice   = '';
	$response = zc_check_browser_version();

	if ( $response ) {
		if ( $is_IE ) {
			$msg = __( 'Internet Explorer does not give you the best ZelocoreCMS experience. Switch to Microsoft Edge, or another more modern browser to get the most from your site.' );
		} elseif ( $response['insecure'] ) {
			$msg = sprintf(
				/* translators: %s: Browser name and link. */
				__( "It looks like you're using an insecure version of %s. Using an outdated browser makes your computer unsafe. For the best ZelocoreCMS experience, please update your browser." ),
				sprintf( '<a href="%s">%s</a>', esc_url( $response['update_url'] ), esc_html( $response['name'] ) )
			);
		} else {
			$msg = sprintf(
				/* translators: %s: Browser name and link. */
				__( "It looks like you're using an old version of %s. For the best ZelocoreCMS experience, please update your browser." ),
				sprintf( '<a href="%s">%s</a>', esc_url( $response['update_url'] ), esc_html( $response['name'] ) )
			);
		}
	 ...
	 *                              false on failure. See zc_check_browser_version().
	 */
	echo apply_filters( 'browse-happy-notice', $notice, $response ); // phpcs:ignore ZelocoreCMS.NamingConventions.ValidHookName.UseUnderscores
}
 ...
 */
function zc_check_browser_version() {
	...
	if ( false === $response ) {
		$url     = 'http://api.zelocorecms.org/core/browse-happy/1.1/';
		$options = array(
			'body'       => array( 'useragent' => $_SERVER['HTTP_USER_AGENT'] ),
			'user-agent' => 'ZelocoreCMS/' . zc_get_zc_version() . '; ' . home_url( '/' ),
		);

		if ( zc_http_supports( array( 'ssl' ) ) ) {
		 ...
		 */
		$response = json_decode( zc_remote_retrieve_body( $response ), true );
...
function zc_dashboard_php_nag() {
	...
	if ( isset( $response['is_secure'] ) && ! $response['is_secure'] ) {
		...
		if ( $response['is_lower_than_future_minimum'] ) {
			$message = sprintf(
				...
				__( 'Your site is running on an outdated version of PHP (%s), which does not receive security updates and soon will not be supported by ZelocoreCMS. Ensure that PHP is updated on your server as soon as possible. Otherwise you will not be able to upgrade ZelocoreCMS.' ),
				PHP_VERSION
			);
		} else {
			$message = sprintf(
		...
		$message = sprintf(
			/* translators: %s: The server PHP version. */
			__( 'Your site is running on an outdated version of PHP (%s), which soon will not be supported by ZelocoreCMS. Ensure that PHP is updated on your server as soon as possible. Otherwise you will not be able to upgrade ZelocoreCMS.' ),
			PHP_VERSION
		);
	} else {
		$message = sprintf(
	...
	<p>
		<?php _e( 'PHP is one of the programming languages used to build ZelocoreCMS. Newer versions of PHP receive regular security updates and may increase your site&#8217;s performance.' ); ?>
		<?php
	...
	zc_update_php_annotation();
	zc_direct_php_update_button();
}
 ...
 * @since 5.4.0
 */
function zc_dashboard_site_health() {
 ...
 * @since 2.5.0
 */
function zc_dashboard_empty() {}

/**
 * Displays a welcome panel to introduce users to ZelocoreCMS.
 *
 * @since 3.3.0
 * @since 5.9.0 Send users to the Site Editor if the active theme is block-based.
 */
function zc_welcome_panel() {
	list( $display_version ) = explode( '-', zc_get_zc_version() );
	...
	<div class="welcome-panel-header-wrap">
		<div class="welcome-panel-header">
			...
			<h2><?php _e( 'Welcome to ZelocoreCMS!' ); ?></h2>
			<p>
				<a href="<?php echo esc_url( admin_url( 'about.php' ) ); ?>">
				<?php
					/* translators: %s: Current ZelocoreCMS version. */
	...
	<div class="welcome-panel-column-container">
		...
		<div class="welcome-panel-column">
			...
			<?php else : ?>
				...
				<?php if ( $can_customize ) : ?>
					<a class="load-customize hide-if-no-customize" href="<?php echo zc_customize_url(); ?>"><?php _e( 'Open the Customizer' ); ?></a>
				<?php endif; ?>
			<?php endif; ?>
			</div>
		</div>
		<div class="welcome-panel-column">
			...
			<?php else : ?>
				...
				<p><?php _e( 'There is a new kind of ZelocoreCMS theme, called a block theme, that lets you build the site you&#8217;ve always wanted &#8212; with blocks and styles.' ); ?></p>
				<a href="<?php echo esc_url( __( 'https://zelocorecms.org/documentation/article/block-themes/' ) ); ?>"><?php _e( 'Learn about block themes' ); ?></a>
			<?php endif; ?>
			</div>
		</div>
	</div>
...
/**
 * Outputs the Quick Stats dashboard widget.
 *
 * @since ZelocoreCMS 1.0.0
 */
...
/**
 * Outputs the Content Performance dashboard widget.
 *
 * @since ZelocoreCMS 1.0.0
 */
