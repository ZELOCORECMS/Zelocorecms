 ...
 */
function zc_create_category( $category_name, $category_parent = 0 ) {
	$id = category_exists( $category_name, $category_parent );
	if ( $id ) {
		return (int) $id;
	}

	return zc_insert_category(
		array(
 ...
 */
function zc_create_categories( $categories, $post_id = 0 ) {
	$cat_ids = array();
	foreach ( $categories as $category ) {
		...
		if ( $id ) {
			...
			$id = zc_create_category( $category );
			if ( $id ) {
				$cat_ids[] = $id;
			}
		}
 ...
 *
 * @since 2.0.0
 * @since 2.5.0 $zc_error parameter was added.
 ...
 * }
 * @param bool  $zc_error Optional. Default false.
 * @return int|ZC_Error The ID number of the new or updated Category on success. Zero or a ZC_Error on failure,
 *                      depending on param `$zc_error`.
 */
function zc_insert_category( $catarr, $zc_error = false ) {
	$cat_defaults = array(
		'cat_ID'               => 0,
		'taxonomy'             => 'category',
		'cat_name'             => '',
	...
	$catarr       = zc_parse_args( $catarr, $cat_defaults );

	if ( '' === trim( $catarr['cat_name'] ) ) {
		if ( ! $zc_error ) {
			return 0;
		} else {
			return new ZC_Error( 'cat_name', __( 'You did not enter a category name.' ) );
		}
	}
	...
	if ( $update ) {
		$catarr['cat_ID'] = zc_update_term( $catarr['cat_ID'], $catarr['taxonomy'], $args );
	} else {
		$catarr['cat_ID'] = zc_insert_term( $catarr['cat_name'], $catarr['taxonomy'], $args );
	}

	if ( is_zc_error( $catarr['cat_ID'] ) ) {
		if ( $zc_error ) {
...
/**
 * Aliases zc_insert_category() with minimal args.
 *
 ...
 */
function zc_update_category( $catarr ) {
	...
	$category = zc_slash( $category );

	// Merge old and new fields with new fields overwriting old ones.
	$catarr = array_merge( $category, $catarr );

	return zc_insert_category( $catarr );
}
 ...
 */
function zc_create_tag( $tag_name ) {
	return zc_create_term( $tag_name, 'post_tag' );
}
...
function get_terms_to_edit( $post_id, $taxonomy = 'post_tag' ) {
	...
	if ( false === $terms ) {
		$terms = zc_get_object_terms( $post_id, $taxonomy );
		zc_cache_add( $post_id, zc_list_pluck( $terms, 'term_id' ), $taxonomy . '_relationships' );
	}
 ...
 * @return array|ZC_Error
 */
function zc_create_term( $tag_name, $taxonomy = 'post_tag' ) {
	$id = term_exists( $tag_name, $taxonomy );
	if ( $id ) {
		return $id;
	}

	return zc_insert_term( $tag_name, $taxonomy );
}
