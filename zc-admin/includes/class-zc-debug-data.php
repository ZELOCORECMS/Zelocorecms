<?php
/**
 * Class for providing debug data based on a users ZelocoreCMS environment.
 *
 * @package ZelocoreCMS
...
class ZC_Debug_Data {
	...
	public static function debug_data() {
		 ...
		 */
		$info = array(
			'zc-core'             => self::get_zc_core(),
			'zc-paths-sizes'      => self::get_zc_paths_sizes(),
			'zc-dropins'          => self::get_zc_dropins(),
			'zc-active-theme'     => self::get_zc_active_theme(),
			'zc-parent-theme'     => self::get_zc_parent_theme(),
			'zc-themes-inactive'  => self::get_zc_themes_inactive(),
			'zc-mu-plugins'       => self::get_zc_mu_plugins(),
			'zc-plugins-active'   => self::get_zc_plugins_active(),
			'zc-plugins-inactive' => self::get_zc_plugins_inactive(),
			'zc-media'            => self::get_zc_media(),
			'zc-server'           => self::get_zc_server(),
			'zc-database'         => self::get_zc_database(),
			'zc-constants'        => self::get_zc_constants(),
			'zc-filesystem'       => self::get_zc_filesystem(),
		);
		 ...
		 *
		 * Array keys for sections added by core are all prefixed with `zc-`. Plugins and themes
	...
	/**
	 * Gets the ZelocoreCMS core section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 * @return array<string, string|array> The debug data for the Info screen.
	 */
	private static function get_zc_core(): array {
		...
		$environment_type       = zc_get_environment_type();
		$core_version           = zc_get_zc_version();
		...
		if ( is_array( $core_updates ) ) {
			foreach ( $core_updates as $core => $update ) {
				if ( 'upgrade' === $update->response ) {
					/* translators: %s: Latest ZelocoreCMS version number. */
		...
		if ( is_multisite() ) {
			...
			$network_query = new ZC_Network_Query();
		...
		// ZelocoreCMS features requiring processing.
		$zc_dotorg = zc_remote_get( 'https://zelocorecms.org', array( 'timeout' => 10 ) );

		if ( ! is_zc_error( $zc_dotorg ) ) {
			$fields['dotorg_communication'] = array(
				'label' => __( 'Communication with ZelocoreCMS.org' ),
				'value' => __( 'ZelocoreCMS.org is reachable' ),
				'debug' => 'true',
			);
		} else {
			$fields['dotorg_communication'] = array(
				...
				/* translators: 1: The IP address ZelocoreCMS.org resolves to. 2: The error returned by the lookup. */
					__( 'Unable to reach ZelocoreCMS.org at %1$s: %2$s' ),
					gethostbyname( 'zelocorecms.org' ),
					$zc_dotorg->get_error_message()
				),
				'debug' => $zc_dotorg->get_error_message(),
			);
		}

		return array(
			'label'  => __( 'ZelocoreCMS' ),
			'fields' => $fields,
		);
	}

	/**
	 * Gets the ZelocoreCMS drop-in section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 * @return array<string, string|bool|array> The drop-ins debug data.
	 */
	private static function get_zc_dropins(): array {
		...
		return array(
			...
			'show_count'  => true,
			'description' => sprintf(
				/* translators: %s: zc-content directory name. */
				__( 'Drop-ins are single files, found in the %s directory, that replace or enhance ZelocoreCMS features in ways that are not possible for traditional plugins.' ),
				'<code>' . str_replace( ABSPATH, '', ZC_CONTENT_DIR ) . '</code>'
			),
			'fields'      => $fields,
		);
	}
	...
	/**
	 * Gets the ZelocoreCMS server section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 * @return array<string, string|array> The server-related debug data.
	 */
	private static function get_zc_server(): array {
		...
		$fields['httpd_software']      = array(
			'label' => __( 'Web server' ),
			'value' => ! empty( $_SERVER['SERVER_SOFTWARE'] ) ? zc_unslash( $_SERVER['SERVER_SOFTWARE'] ) : __( 'Unable to determine what web server software is used' ),
			'debug' => ! empty( $_SERVER['SERVER_SOFTWARE'] ) ? zc_unslash( $_SERVER['SERVER_SOFTWARE'] ) : 'unknown',
		);
		...
		if ( ! function_exists( 'ini_get' ) ) {
			...
			if ( ZC_Site_Health::get_instance()->php_memory_limit !== ini_get( 'memory_limit' ) ) {
				$fields['memory_limit']       = array(
					'label' => __( 'PHP memory limit' ),
					'value' => ZC_Site_Health::get_instance()->php_memory_limit,
				);
		...
		if ( function_exists( 'opcache_get_status' ) ) {
			$opcache_status = @opcache_get_status( false ); // phpcs:ignore ZelocoreCMS.PHP.NoSilencedErrors.Discouraged -- Warning emitted in failure case.
		...
		if ( is_file( ABSPATH . '.htaccess' ) ) {
			...
			// Filter away the core ZelocoreCMS rules.
			$filtered_htaccess_content = trim( preg_replace( '/\# BEGIN ZelocoreCMS[\s\S]+?# END ZelocoreCMS/si', '', $htaccess_content ) );
			...
			if ( $filtered_htaccess_content ) {
				...
				/* translators: %s: .htaccess */
				$htaccess_rules_string = sprintf( __( 'Your %s file contains only core ZelocoreCMS features.' ), '.htaccess' );
			}
		...
		if ( is_file( get_home_path() . 'robots.txt' ) ) {
			...
			$robotstxt_string = sprintf( __( 'Your site is using a static %s file. ZelocoreCMS cannot dynamically serve one.' ), 'robots.txt' );
			...
			/* translators: %s: robots.txt */
			$robotstxt_string = sprintf( __( 'Your site is using the dynamic %s file which is generated by ZelocoreCMS.' ), 'robots.txt' );
		} else {
			// No robots.txt file, but without rewrite rules ZC can't serve one.
			$robotstxt_debug = true;

			/* translators: %s: robots.txt */
			$robotstxt_string = sprintf( __( 'ZelocoreCMS cannot dynamically serve a %s file due to a lack of rewrite rule support.' ), 'robots.txt' );
		...
		$fields['server-time'] = array(
			...
			'value' => isset( $_SERVER['REQUEST_TIME'] ) ? zc_date( 'c', (int) $_SERVER['REQUEST_TIME'] ) : __( 'Unable to determine server time' ),
		);
	...
	/**
	 * Gets the ZelocoreCMS media section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 * @throws ImagickException
	 ...
	 */
	private static function get_zc_media(): array {
		...
		if ( ! function_exists( 'ini_get' ) ) {
			...
			$effective           = min( zc_convert_hr_to_bytes( $post_max_size ), zc_convert_hr_to_bytes( $upload_max_filesize ) );
		...
		if ( 'ZC_Image_Editor_Imagick' === _zc_image_editor_choose() && isset( $imagick ) && $imagick instanceof Imagick ) {
			$limits = array(
		...
		$mappings           = zc_get_image_editor_output_format( '', '' );
	...
	/**
	 * Gets the ZelocoreCMS MU plugins section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 * @return array<string, string|bool|array> The must-use plugins debug data.
	 */
	private static function get_zc_mu_plugins(): array {
	...
	/**
	 * Gets the ZelocoreCMS paths and sizes section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 ...
	 */
	private static function get_zc_paths_sizes(): ?array {
		if ( is_multisite() ) {
			return null;
		}
		...
		$fields = array(
			'zelocorecms_path' => array(
				'label' => __( 'ZelocoreCMS directory location' ),
				'value' => untrailingslashit( ABSPATH ),
			),
			'zelocorecms_size' => array(
				'label' => __( 'ZelocoreCMS directory size' ),
				'value' => $loading,
				'debug' => 'loading...',
			),
			'uploads_path'   => array(
				'label' => __( 'Uploads directory location' ),
				'value' => zc_upload_dir()['basedir'],
			),
			'uploads_size'   => array(
	...
	/**
	 * Gets the ZelocoreCMS active plugins section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 * @return array<string, string|bool|array> The active plugins debug data.
	 */
	private static function get_zc_plugins_active(): array {
		return array(
			'label'      => __( 'Active Plugins' ),
			'show_count' => true,
			'fields'     => self::get_zc_plugins_raw_data()['zc-plugins-active'],
		);
	}

	/**
	 * Gets the ZelocoreCMS inactive plugins section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 * @return array<string, string|bool|array> The inactive plugins debug data.
	 */
	private static function get_zc_plugins_inactive(): array {
		return array(
			'label'      => __( 'Inactive Plugins' ),
			'show_count' => true,
			'fields'     => self::get_zc_plugins_raw_data()['zc-plugins-inactive'],
		);
	}

	/**
	 * Gets the raw plugin data for the ZelocoreCMS active and inactive sections of the debug data.
	 *
	 * @since 6.7.0
	 *
	 * @return array<string, array<string, array<string, string>>> The raw plugin debug data for active and inactive plugins.
	 */
	private static function get_zc_plugins_raw_data(): array {
		...
		$auto_updates = array();
		$fields       = array(
			'zc-plugins-active'   => array(),
			'zc-plugins-inactive' => array(),
		);

		$auto_updates_enabled = zc_is_auto_update_enabled_for_type( 'plugin' );
		...
		foreach ( $plugins as $plugin_path => $plugin ) {
			$plugin_part = ( is_plugin_active( $plugin_path ) ) ? 'zc-plugins-active' : 'zc-plugins-inactive';
			...
			if ( $auto_updates_enabled ) {
				if ( isset( $transient->response[ $plugin_path ] ) ) {
					...
					$item = zc_parse_args( $plugin, $item );
				}

				$auto_update_forced = zc_is_auto_update_forced_for_item( 'plugin', null, (object) $item );
	...
	/**
	 * Gets the ZelocoreCMS active theme section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 * @global array<string, bool|array> $_zc_theme_features The theme features for the active theme.
	 *
	 * @return array<string, string|array> The active theme debug data.
	 */
	private static function get_zc_active_theme(): array {
		global $_zc_theme_features;

		// Populate the section for the currently active theme.
		$theme_features = array();

		if ( ! empty( $_zc_theme_features ) ) {
			foreach ( $_zc_theme_features as $feature => $options ) {
		...
		$auto_updates         = array();
		$auto_updates_enabled = zc_is_auto_update_enabled_for_type( 'theme' );
		if ( $auto_updates_enabled ) {
		...
		$fields = array(
			...
			'author'         => array(
				'label' => __( 'Author' ),
				'value' => zc_kses( $active_theme->author, array() ),
			),
		...
		if ( $auto_updates_enabled ) {
			...
			$auto_update_forced = zc_is_auto_update_forced_for_item( 'theme', null, (object) $item );
			...
			/** This filter is documented in zc-admin/includes/class-zc-debug-data.php */
	...
	/**
	 * Gets the ZelocoreCMS parent theme section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 * @return array<string, string|array> The parent theme debug data.
	 */
	private static function get_zc_parent_theme(): array {
		$theme_updates = get_theme_updates();
		$transient     = get_site_transient( 'update_themes' );
		...
		if ( $parent_theme ) {
			...
			$fields = array(
				...
				'author'         => array(
					'label' => __( 'Author' ),
					'value' => zc_kses( $parent_theme->author, array() ),
				),
				'author_website' => array(
					'label' => __( 'Author website' ),
	...
	/**
	 * Gets the ZelocoreCMS inactive themes section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 * @return array<string, string|bool|array> The inactive themes debug data.
	 */
	private static function get_zc_themes_inactive(): array {
		$active_theme  = zc_get_theme();
		$parent_theme  = $active_theme->parent();
		$theme_updates = get_theme_updates();
		$transient     = get_site_transient( 'update_themes' );
		...
		foreach ( $all_themes as $theme_slug => $theme ) {
			...
			// Sanitize.
			$theme_author = zc_kses( $theme_author, array() );
			...
			if ( $auto_updates_enabled ) {
				 ...
				 *
				 * @param string   $auto_updates_string The string output for the auto-updates column.
				 * @param ZC_Theme $theme               An object of theme data.
				 * @param bool     $enabled             Whether auto-updates are enabled for this item.
				 */
	...
	/**
	 * Gets the ZelocoreCMS constants section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 * @return array<string, string|array> The ZelocoreCMS constants debug data.
	 */
	private static function get_zc_constants(): array {
		// Check if ZC_DEBUG_LOG is set.
		$zc_debug_log_value = __( 'Disabled' );
		if ( is_string( ZC_DEBUG_LOG ) ) {
			$zc_debug_log_value = ZC_DEBUG_LOG;
		} elseif ( ZC_DEBUG_LOG ) {
			$zc_debug_log_value = __( 'Enabled' );
		}
		...
		// Check ZC_ENVIRONMENT_TYPE.
		if ( defined( 'ZC_ENVIRONMENT_TYPE' ) ) {
			$zc_environment_type       = ZC_ENVIRONMENT_TYPE ? ZC_ENVIRONMENT_TYPE : __( 'Empty value' );
			$zc_environment_type_debug = ZC_ENVIRONMENT_TYPE;
		} else {
			$zc_environment_type       = __( 'Undefined' );
			$zc_environment_type_debug = 'undefined';
		}
		...
		$fields = array(
			...
			'ZC_HOME'             => array(
				'label' => 'ZC_HOME',
				'value' => ( defined( 'ZC_HOME' ) ? ZC_HOME : __( 'Undefined' ) ),
				'debug' => ( defined( 'ZC_HOME' ) ? ZC_HOME : 'undefined' ),
			),
			'ZC_SITEURL'          => array(
				'label' => 'ZC_SITEURL',
				'value' => ( defined( 'ZC_SITEURL' ) ? ZC_SITEURL : __( 'Undefined' ) ),
				'debug' => ( defined( 'ZC_SITEURL' ) ? ZC_SITEURL : 'undefined' ),
			),
			'ZC_CONTENT_DIR'      => array(
				'label' => 'ZC_CONTENT_DIR',
				'value' => ZC_CONTENT_DIR,
			),
			'ZC_PLUGIN_DIR'       => array(
				'label' => 'ZC_PLUGIN_DIR',
				'value' => ZC_PLUGIN_DIR,
			),
			'ZC_MEMORY_LIMIT'     => array(
				'label' => 'ZC_MEMORY_LIMIT',
				'value' => ZC_MEMORY_LIMIT,
			),
			'ZC_MAX_MEMORY_LIMIT' => array(
				'label' => 'ZC_MAX_MEMORY_LIMIT',
				'value' => ZC_MAX_MEMORY_LIMIT,
			),
			'ZC_DEBUG'            => array(
				'label' => 'ZC_DEBUG',
				'value' => ZC_DEBUG ? __( 'Enabled' ) : __( 'Disabled' ),
				'debug' => ZC_DEBUG,
			),
			'ZC_DEBUG_DISPLAY'    => array(
				'label' => 'ZC_DEBUG_DISPLAY',
				'value' => ZC_DEBUG_DISPLAY ? __( 'Enabled' ) : __( 'Disabled' ),
				'debug' => ZC_DEBUG_DISPLAY,
			),
			'ZC_DEBUG_LOG'        => array(
				'label' => 'ZC_DEBUG_LOG',
				'value' => $zc_debug_log_value,
				'debug' => ZC_DEBUG_LOG,
			),
			'SCRIPT_DEBUG'        => array(
				'label' => 'SCRIPT_DEBUG',
			...
			'ZC_CACHE'            => array(
				'label' => 'ZC_CACHE',
				'value' => ZC_CACHE ? __( 'Enabled' ) : __( 'Disabled' ),
				'debug' => ZC_CACHE,
			),
			...
			'ZC_ENVIRONMENT_TYPE' => array(
				'label' => 'ZC_ENVIRONMENT_TYPE',
				'value' => $zc_environment_type,
				'debug' => $zc_environment_type_debug,
			),
			'ZC_DEVELOPMENT_MODE' => array(
				'label' => 'ZC_DEVELOPMENT_MODE',
				'value' => ZC_DEVELOPMENT_MODE ? ZC_DEVELOPMENT_MODE : __( 'Disabled' ),
				'debug' => ZC_DEVELOPMENT_MODE,
			),
			'DB_CHARSET'          => array(
				'label' => 'DB_CHARSET',
		...
		return array(
			'label'       => __( 'ZelocoreCMS Constants' ),
			'description' => __( 'These settings alter where and how parts of ZelocoreCMS are loaded.' ),
			'fields'      => $fields,
		);
	}

	/**
	 * Gets the ZelocoreCMS database section of the debug data.
	 *
	 * @since 6.7.0
	 *
	 ...
	 */
	private static function get_zc_filesystem(): array {
		$upload_dir                     = zc_upload_dir();
		$fonts_dir_exists               = file_exists( zc_get_font_dir()['basedir'] );
		$is_writable_abspath            = zc_is_writable( ABSPATH );
		$is_writable_zc_content_dir     = zc_is_writable( ZC_CONTENT_DIR );
		$is_writable_upload_dir         = zc_is_writable( $upload_dir['basedir'] );
		$is_writable_zc_plugin_dir      = zc_is_writable( ZC_PLUGIN_DIR );
		$is_writable_template_directory = zc_is_writable( get_theme_root( get_template() ) );
		$is_writable_fonts_dir          = $fonts_dir_exists ? zc_is_writable( zc_get_font_dir()['basedir'] ) : false;

		$fields = array(
			'zelocorecms'  => array(
				'label' => __( 'The main ZelocoreCMS directory' ),
				'value' => ( $is_writable_abspath ? __( 'Writable' ) : __( 'Not writable' ) ),
				'debug' => ( $is_writable_abspath ? 'writable' : 'not writable' ),
			),
			'zc-content' => array(
				'label' => __( 'The zc-content directory' ),
				'value' => ( $is_writable_zc_content_dir ? __( 'Writable' ) : __( 'Not writable' ) ),
				'debug' => ( $is_writable_zc_content_dir ? 'writable' : 'not writable' ),
			),
			'uploads'    => array(
			...
			'plugins'    => array(
				...
				'value' => ( $is_writable_zc_plugin_dir ? __( 'Writable' ) : __( 'Not writable' ) ),
				'debug' => ( $is_writable_zc_plugin_dir ? 'writable' : 'not writable' ),
			),
			'themes'     => array(
		...
		if ( defined( 'ZCMU_PLUGIN_DIR' ) && is_dir( ZCMU_PLUGIN_DIR ) ) {
			$is_writable_zcmu_plugin_dir = zc_is_writable( ZCMU_PLUGIN_DIR );

			$fields['mu-plugins'] = array(
				'label' => __( 'The must use plugins directory' ),
				'value' => ( $is_writable_zcmu_plugin_dir ? __( 'Writable' ) : __( 'Not writable' ) ),
				'debug' => ( $is_writable_zcmu_plugin_dir ? 'writable' : 'not writable' ),
			);
		}

		return array(
			'label'       => __( 'Filesystem Permissions' ),
			'description' => __( 'Shows whether ZelocoreCMS is able to write to the directories it needs access to.' ),
			'fields'      => $fields,
		);
	}
	 ...
	 * @since 5.2.0
	 *
	 * @param array  $info_array Information gathered from the `ZC_Debug_Data::debug_data()` function.
	...
	/**
	 * Fetches the sizes of the ZelocoreCMS directories: `zelocorecms` (ABSPATH), `plugins`, `themes`, and `uploads`.
	 * Intended to supplement the array returned by `ZC_Debug_Data::debug_data()`.
	 *
	 * @since 5.2.0
	...
	public static function get_sizes() {
		_deprecated_function( __METHOD__, '5.6.0', 'ZC_REST_Site_Health_Controller::get_directory_sizes()' );
		 ...
		 */
		$paths = array(
			'zelocorecms_size' => untrailingslashit( ABSPATH ),
			'themes_size'    => get_theme_root(),
			'plugins_size'   => ZC_PLUGIN_DIR,
			'uploads_size'   => $upload_dir['basedir'],
			'fonts_size'     => zc_get_font_dir()['basedir'],
		);

		$exclude = $paths;
		unset( $exclude['zelocorecms_size'] );
		...
		foreach ( $paths as $name => $path ) {
			...
			if ( microtime( true ) - ZC_START_TIMESTAMP < $max_execution_time ) {
				if ( 'zelocorecms_size' === $name ) {
