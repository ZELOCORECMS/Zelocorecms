 ...
 * @see ZC_Upgrader
 */
class Theme_Upgrader extends ZC_Upgrader {
	 ...
	 * }
	 *
	 * @return bool|ZC_Error True if the installation was successful, false or a ZC_Error object otherwise.
	 */
	public function install( $package, $args = array() ) {
		...
		if ( $parsed_args['clear_update_cache'] ) {
			// Clear cache so zc_update_themes() knows about the new theme.
			add_action( 'upgrader_process_complete', 'zc_clean_themes_cache', 9, 0 );
		}
		...
		remove_action( 'upgrader_process_complete', 'zc_clean_themes_cache', 9 );
		...
		zc_clean_themes_cache( $parsed_args['clear_update_cache'] );

		if ( $parsed_args['overwrite_package'] ) {
			/** This action is documented in zc-admin/includes/class-plugin-upgrader.php */
	...
	public function bulk_upgrade( $themes, $args = array() ) {
		...
		$connected = $this->fs_connect( array( ZC_CONTENT_DIR ) );
		if ( ! $connected ) {
			$this->skin->footer();
			return false;
		}
		...
		foreach ( $themes as $theme ) {
			...
			if ( isset( $upgrade_data['requires'] ) && ! is_zc_version_compatible( $upgrade_data['requires'] ) ) {
				$result = new ZC_Error(
					...
					sprintf(
						/* translators: 1: Current ZelocoreCMS version, 2: ZelocoreCMS version required by the new theme version. */
						__( 'Your ZelocoreCMS version is %1$s, however the new theme version requires %2$s.' ),
						$zc_version,
		...
		foreach ( $results as $theme => $result ) {
			...
			if ( ! $result || is_zc_error( $result ) || ! isset( $past_failure_emails[ $theme ] ) ) {
				continue;
			}
	...
	public function check_package( $source ) {
		...
		if ( ! is_zc_version_compatible( $requires_wp ) ) {
			$error = sprintf(
				/* translators: 1: Current ZelocoreCMS version, 2: Version required by the uploaded theme. */
				__( 'Your ZelocoreCMS version is %1$s, however the uploaded theme requires %2$s.' ),
				$zc_version,
				$requires_wp
			);
	 ...
	 * @since 2.8.0
	 *
	 * @global ZC_Filesystem_Base $zc_filesystem Filesystem subclass.
	 *
	 * @param bool   $removed
	...
	public function delete_old_theme( $removed, $local_destination, $remote_destination, $theme ) {
		...
		$theme      = $theme['theme'];
		$themes_dir = trailingslashit( $zc_filesystem->zc_themes_dir( $theme ) );
		if ( $zc_filesystem->exists( $themes_dir . $theme ) ) {
			if ( ! $zc_filesystem->delete( $themes_dir . $theme, true ) ) {
				return false;
			}
		}
	...
	/**
	 * Gets the ZC_Theme object for a theme.
	 *
	 * @since 2.8.0
	 ...
	 * @return ZC_Theme|false The theme's info object, or false `$theme` is not supplied
