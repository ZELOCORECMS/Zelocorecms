<?php
/**
 * List Table API: ZC_MS_Themes_List_Table class
 *
 * @package ZelocoreCMS
 ...
 * @see ZC_List_Table
 */
class ZC_MS_Themes_List_Table extends ZC_List_Table {
	...
	public function __construct( $args = array() ) {
		...
		$this->show_autoupdates = zc_is_auto_update_enabled_for_type( 'theme' ) &&
	...
	public function prepare_items() {
		...
		$themes = array(
			/**
			 * Filters the full array of ZC_Theme objects to list in the Multisite
			 * themes list table.
			 *
			 * @since 3.1.0
			 *
			 * @param ZC_Theme[] $all Array of ZC_Theme objects to display in the list table.
			 */
			'all'      => apply_filters( 'all_themes', zc_get_themes() ),
			'search'   => array(),
			'enabled'  => array(),
			'disabled' => array(),
			'upgrade'  => array(),
			'broken'   => $this->is_site_themes ? array() : zc_get_themes( array( 'errors' => true ) ),
		);
		...
		foreach ( (array) $themes['all'] as $key => $theme ) {
			...
			$auto_update_forced = zc_is_auto_update_forced_for_item( 'theme', null, $filter_payload );
		...
		ZC_Theme::sort_by_name( $this->items );
	 ...
	 * @global string $order   The order of the themes list (ASC or DESC).
	 *
	 * @param ZC_Theme $theme_a The first theme to compare.
	 * @param ZC_Theme $theme_b The second theme to compare.
	 ...
	 *
	 * @param ZC_Theme $item The current ZC_Theme object.
	 */
	 ...
	 * @global string $s      The search string.
	 *
	 * @param ZC_Theme $theme The current ZC_Theme object.
	 */
	public function column_name( $theme ) {
		...
		if ( ! $allowed ) {
			if ( ! $theme->errors() ) {
				...
				$actions['enable'] = sprintf(
					...
					esc_url( zc_nonce_url( $url, 'enable-theme_' . $stylesheet ) ),
					esc_attr( $aria_label ),
			...
			$actions['disable'] = sprintf(
				...
				esc_url( zc_nonce_url( $url, 'disable-theme_' . $stylesheet ) ),
				esc_attr( $aria_label ),
		...
		if ( ! $allowed && ! $this->is_site_themes
			...
			$actions['delete'] = sprintf(
				...
				esc_url( zc_nonce_url( $url, 'bulk-themes' ) ),
				esc_attr( $aria_label ),
				__( 'Delete' )
			);
		}
	...
	public function column_default( $item, $column_name ) {
		 ...
		 * @param string   $stylesheet  Directory name of the theme.
		 * @param ZC_Theme $theme       Current ZC_Theme object.
		 */
	...
	public function single_row( $theme ) {
		...
		if ( $this->is_site_themes ) {
			remove_action( "after_theme_row_$stylesheet", 'zc_theme_update_row' );
		}
