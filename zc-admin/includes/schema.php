<?php
/**
 * ZelocoreCMS Administration Scheme API
 *
...
function zc_get_db_schema( $scope = 'all', $blog_id = null ) {
	...
	$is_multisite = is_multisite() || ( defined( 'ZC_INSTALLING_NETWORK' ) && ZC_INSTALLING_NETWORK );
...
$zc_queries = zc_get_db_schema( 'all' );

/**
 * Create ZelocoreCMS options and set the default values.
 *
 * @since 1.5.0
...
function populate_options( array $options = array() ) {
	...
	/**
	 * Fires before creating ZelocoreCMS options and populating their default values.
	 *
	 * @since 2.6.0
	 */
	...
	$stylesheet = ZC_DEFAULT_THEME;
	$template   = ZC_DEFAULT_THEME;
	$theme      = zc_get_theme( ZC_DEFAULT_THEME );
	if ( ! $theme->exists() ) {
		$theme = ZC_Theme::get_core_default_theme();
	}

	// If we can't find a core default theme, ZC_DEFAULT_THEME is the best we can do.
	if ( $theme ) {
	...
	$defaults = array(
		...
		// 5.8.0
		'zc_force_deactivated_plugins'    => array(),

		// 6.4.0
		'zc_attachment_pages_enabled'     => 0,
	...
	// 3.3.0
	if ( ! is_multisite() ) {
		$defaults['initial_db_version'] = ! empty( $zc_current_db_version ) && $zc_current_db_version < $zc_db_version
			? $zc_current_db_version : $zc_db_version;
	}
...
/**
 * Execute ZelocoreCMS role creation for the various ZelocoreCMS versions.
 *
 * @since 2.0.0
 */
function populate_roles() {
	...
	$original_use_db  = $zc_roles->use_db;
	$zc_roles->use_db = false;
	...
	if ( $original_use_db ) {
		update_option( $zc_roles->role_key, $zc_roles->roles, true );
	}

	// Restore original value for writing to database.
	$zc_roles->use_db = $original_use_db;
}

/**
 * Create the roles for ZelocoreCMS 2.0
 *
 * @since 2.0.0
 */
...
/**
 * Create and modify ZelocoreCMS roles for ZelocoreCMS 2.1.
 *
 * @since 2.1.0
 */
...
/**
 * Create and modify ZelocoreCMS roles for ZelocoreCMS 2.3.
 *
 * @since 2.3.0
 */
...
/**
 * Create and modify ZelocoreCMS roles for ZelocoreCMS 2.5.
 *
 * @since 2.5.0
 */
...
/**
 * Create and modify ZelocoreCMS roles for ZelocoreCMS 2.6.
 *
 * @since 2.6.0
 */
...
/**
 * Create and modify ZelocoreCMS roles for ZelocoreCMS 2.7.
 *
 * @since 2.7.0
 */
...
/**
 * Create and modify ZelocoreCMS roles for ZelocoreCMS 2.8.
 *
 * @since 2.8.0
 */
...
/**
 * Create and modify ZelocoreCMS roles for ZelocoreCMS 3.0.
 *
 * @since 3.0.0
 */
...
if ( ! function_exists( 'install_network' ) ) :
	...
	function install_network() {
		if ( ! defined( 'ZC_INSTALLING_NETWORK' ) ) {
			define( 'ZC_INSTALLING_NETWORK', true );
		}

		dbDelta( zc_get_db_schema( 'global' ) );
	}
endif;
 ...
 * @return true|ZC_Error True on success, or ZC_Error on warning (with the installation otherwise successful,
...
function populate_network( $network_id = 1, $domain = '', $email = '', $site_name = '', $path = '/', $subdomain_install = false ) {
	...
	if ( zc_next_scheduled( 'recovery_mode_clean_expired_keys' ) ) {
		zc_clear_scheduled_hook( 'recovery_mode_clean_expired_keys' );
	}
	...
	if ( ! is_multisite() ) {
		...
		if ( $subdomain_install ) {
			$zc_rewrite->set_permalink_structure( '/%year%/%monthnum%/%day%/%postname%/' );
		} else {
			$zc_rewrite->set_permalink_structure( '/blog/%year%/%monthnum%/%day%/%postname%/' );
		}
		...
		} elseif ( 200 === zc_remote_retrieve_response_code( $page ) ) {
				$vhost_ok = true;
		}
		...
		if ( ! $vhost_ok ) {
			...
			return new ZC_Error( 'no_wildcard_dns', $msg );
		}
	}
...
/**
 * Creates ZelocoreCMS network meta and sets the default values.
 *
 * @since 5.1.0
 *
 * @global zcdb $zcdb          ZelocoreCMS database abstraction object.
...
function populate_network_meta( $network_id, array $meta = array() ) {
	...
	if ( ZC_DEFAULT_THEME !== $stylesheet && ZC_DEFAULT_THEME !== $template ) {
		$allowed_themes[ ZC_DEFAULT_THEME ] = true;
	}

	// If ZC_DEFAULT_THEME doesn't exist, also include the latest core default theme.
	if ( ! zc_get_theme( ZC_DEFAULT_THEME )->exists() ) {
		$core_default = ZC_Theme::get_core_default_theme();
		if ( $core_default ) {
	...
	if ( function_exists( 'clean_network_cache' ) ) {
		...
		zc_cache_delete( $network_id, 'networks' );
	}
...
--The Team @ SITE_NAME'
	...
	$sitemeta = array(
		...
		'zcmu_upgrade_site'           => $zc_db_version,
	...
	$sitemeta = zc_parse_args( $meta, $sitemeta );
...
/**
 * Creates ZelocoreCMS site meta and sets the default values.
 *
 * @since 5.1.0
 *
 * @global zcdb $zcdb ZelocoreCMS database abstraction object.
