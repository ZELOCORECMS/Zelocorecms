 ...
 *     Requires at least: Optional. Specify the minimum required ZelocoreCMS version.
 ...
 *     @type string $RequiresWP      Minimum required version of ZelocoreCMS.
...
function _get_plugin_data_markup_translate( $plugin_file, $plugin_data, $markup = true, $translate = true ) {
	...
	// Sanitize the plugin filename to a ZC_PLUGIN_DIR relative path.
	 ...
	 */
	$plugin_data['Name']   = zc_kses( $plugin_data['Name'], $allowed_tags_in_links );
	$plugin_data['Author'] = zc_kses( $plugin_data['Author'], $allowed_tags );

	$plugin_data['Description'] = zc_kses( $plugin_data['Description'], $allowed_tags );
	$plugin_data['Version']     = zc_kses( $plugin_data['Version'], $allowed_tags );
...
function get_plugin_files( $plugin ) {
	$plugin_file = ZC_PLUGIN_DIR . '/' . $plugin;
	$dir         = dirname( $plugin_file );

	$plugin_files = array( plugin_basename( $plugin_file ) );

	if ( is_dir( $dir ) && ZC_PLUGIN_DIR !== $dir ) {
 ...
 *
 * ZelocoreCMS only supports plugin files in the base plugins directory
 * (zc-content/plugins) and in one directory above the plugins directory
 * (zc-content/plugins/my-plugin). The file it looks for has the plugin data
...
function get_plugins( $plugin_folder = '' ) {
	...
	$cache_plugins = zc_cache_get( 'plugins', 'plugins' );
	if ( ! $cache_plugins ) {
		$cache_plugins = array();
	}
	...
	$zc_plugins  = array();
	$plugin_root = ZC_PLUGIN_DIR;
	if ( ! empty( $plugin_folder ) ) {
		$plugin_root .= $plugin_folder;
	}

	// Files in zc-content/plugins directory.
	...
	foreach ( $plugin_files as $plugin_file ) {
		...
		$zc_plugins[ plugin_basename( $plugin_file ) ] = $plugin_data;
	}

	uasort( $zc_plugins, '_sort_uname_callback' );

	$cache_plugins[ $plugin_folder ] = $zc_plugins;
	zc_cache_set( 'plugins', $cache_plugins, 'plugins' );
 ...
 *
 * ZelocoreCMS only includes mu-plugin files in the base mu-plugins directory (zc-content/mu-plugins).
 *
 * @since 3.0.0
...
function get_mu_plugins() {
	...
	// Files in zc-content/mu-plugins directory.
	$plugins_dir = @opendir( ZCMU_PLUGIN_DIR );
	if ( $plugins_dir ) {
		while ( ( $file = readdir( $plugins_dir ) ) !== false ) {
	...
	foreach ( $plugin_files as $plugin_file ) {
		if ( ! is_readable( ZCMU_PLUGIN_DIR . "/$plugin_file" ) ) {
			continue;
		}

		// Do not apply markup/translate as it will be cached.
		$plugin_data = get_plugin_data( ZCMU_PLUGIN_DIR . "/$plugin_file", false, false );

		if ( empty( $plugin_data['Name'] ) ) {
			$plugin_data['Name'] = $plugin_file;
		}

		$zc_plugins[ $plugin_file ] = $plugin_data;
	}

	if ( isset( $zc_plugins['index.php'] ) && filesize( ZCMU_PLUGIN_DIR . '/index.php' ) <= 30 ) {
		// Silence is golden.
		unset( $zc_plugins['index.php'] );
	}
...
/**
 * Checks the zc-content directory and retrieve all drop-ins with any plugin data.
 *
 * @since 3.0.0
...
function get_dropins() {
	...
	// Files in zc-content directory.
	$plugins_dir = @opendir( ZC_CONTENT_DIR );
	if ( $plugins_dir ) {
		while ( ( $file = readdir( $plugins_dir ) ) !== false ) {
	...
	foreach ( $plugin_files as $plugin_file ) {
		if ( ! is_readable( ZC_CONTENT_DIR . "/$plugin_file" ) ) {
			continue;
		}

		// Do not apply markup/translate as it will be cached.
		$plugin_data = get_plugin_data( ZC_CONTENT_DIR . "/$plugin_file", false, false );
...
/**
 * Returns drop-in plugins that ZelocoreCMS uses.
 *
...
function _get_dropins() {
	$dropins = array(
		'advanced-cache.php'      => array( __( 'Advanced caching plugin.' ), 'ZC_CACHE' ),  // ZC_CACHE
 ...
 *
 * @since 2.5.0
 * @since 5.2.0 Test for ZelocoreCMS version and PHP version compatibility.
 *
 * @param string $plugin       Path to the plugin file relative to the plugins directory.
 ...
 * @return null|ZC_Error Null on success, ZC_Error on invalid file.
 */
function activate_plugin( $plugin, $redirect = '', $network_wide = false, $silent = false ) {
	...
	if ( $network_wide && ! isset( $current[ $plugin ] )
		...
		if ( ! empty( $redirect ) ) {
			...
			zc_redirect( add_query_arg( '_error_nonce', zc_create_nonce( 'plugin-activation-error_' . $plugin ), $redirect ) );
		}
		...
		if ( ob_get_length() > 0 ) {
			$output = ob_get_clean();
			return new ZC_Error( 'unexpected_output', __( 'The plugin generated unexpected output.' ), $output );
		}
...
function deactivate_plugins( $plugins, $silent = false, $network_wide = null ) {
	...
	foreach ( (array) $plugins as $plugin ) {
		...
		if ( $do_blog && zc_is_recovery_mode() ) {
			list( $extension ) = explode( '/', $plugin );
			zc_paused_plugins()->delete( $extension );
		}
 ...
 *
 * When ZC_Error is returned, it does not mean that one of the plugins had
 ...
 * @return true|ZC_Error True when finished or ZC_Error if there were errors during a plugin activation.
 */
function activate_plugins( $plugins, $redirect = '', $network_wide = false, $silent = false ) {
	...
	if ( ! empty( $errors ) ) {
		return new ZC_Error( 'plugins_invalid', __( 'One of the plugins is invalid.' ), $errors );
	}
 ...
 * @return bool|null|ZC_Error True on success, false if `$plugins` is empty, `ZC_Error` on failure.
...
function delete_plugins( $plugins, $deprecated = '' ) {
	...
	$url = zc_nonce_url( 'plugins.php?action=delete-selected&verify-delete=1&' . implode( '&', $checked ), 'bulk-plugins' );
	...
	if ( is_zc_error( $zc_filesystem->errors ) && $zc_filesystem->errors->has_errors() ) {
		return new ZC_Error( 'fs_error', __( 'Filesystem error.' ), $zc_filesystem->errors );
	}

	// Get the base plugin folder.
	$plugins_dir = $zc_filesystem->zc_plugins_dir();
	if ( empty( $plugins_dir ) ) {
		return new ZC_Error( 'fs_no_plugins_dir', __( 'Unable to locate ZelocoreCMS plugin directory.' ) );
	}

	$plugins_dir = trailingslashit( $plugins_dir );

	$plugin_translations = zc_get_installed_translations( 'plugins' );
	...
	foreach ( $plugins as $plugin_file ) {
		...
		if ( strpos( $plugin_file, '/' ) && $this_plugin_dir !== $plugins_dir ) {
			$deleted = $zc_filesystem->delete( $this_plugin_dir, true );
		} else {
			$deleted = $zc_filesystem->delete( $plugins_dir . $plugin_file );
		}
		...
		if ( '.' !== $plugin_slug && ! empty( $plugin_translations[ $plugin_slug ] ) ) {
			...
			foreach ( $translations as $translation => $data ) {
				$zc_filesystem->delete( ZC_LANG_DIR . '/plugins/' . $plugin_slug . '-' . $translation . '.po' );
				$zc_filesystem->delete( ZC_LANG_DIR . '/plugins/' . $plugin_slug . '-' . $translation . '.mo' );
				$zc_filesystem->delete( ZC_LANG_DIR . '/plugins/' . $plugin_slug . '-' . $translation . '.l10n.php' );

				$json_translation_files = glob( ZC_LANG_DIR . '/plugins/' . $plugin_slug . '-' . $translation . '-*.json' );
				if ( $json_translation_files ) {
					array_map( array( $zc_filesystem, 'delete' ), $json_translation_files );
				}
			}
		}
	}
	...
	if ( ! empty( $errors ) ) {
		...
		return new ZC_Error( 'could_not_remove_plugin', sprintf( $message, implode( ', ', $errors ) ) );
	}
 ...
 *
 * @since 2.5.0
 * @return ZC_Error[] Array of plugin errors keyed by plugin file name.
 */
 ...
 *
 * @param string $plugin Path to the plugin file relative to the plugins directory.
 * @return int|ZC_Error 0 on success, ZC_Error on failure.
 */
function validate_plugin( $plugin ) {
	if ( validate_file( $plugin ) ) {
		return new ZC_Error( 'plugin_invalid', __( 'Invalid plugin path.' ) );
	}
	if ( ! file_exists( ZC_PLUGIN_DIR . '/' . $plugin ) ) {
		return new ZC_Error( 'plugin_not_found', __( 'Plugin file does not exist.' ) );
	}

	$installed_plugins = get_plugins();
	if ( ! isset( $installed_plugins[ $plugin ] ) ) {
		return new ZC_Error( 'no_plugin_header', __( 'The plugin does not have a valid header.' ) );
	}
	return 0;
}

/**
 * Validates the plugin requirements for ZelocoreCMS version and PHP version.
 *
...
function validate_plugin_requirements( $plugin ) {
	$plugin_headers = get_plugin_data( ZC_PLUGIN_DIR . '/' . $plugin );
	...
	if ( ! $compatible_wp && ! $compatible_php ) {
		return new ZC_Error(
			'plugin_zc_php_incompatible',
			'<p>' . sprintf(
				/* translators: 1: Current ZelocoreCMS version, 2: Current PHP version, 3: Plugin name, 4: Required ZelocoreCMS version, 5: Required PHP version. */
				_x( '<strong>Error:</strong> Current versions of ZelocoreCMS (%1$s) and PHP (%2$s) do not meet minimum requirements for %3$s. The plugin requires ZelocoreCMS %4$s and PHP %5$s.', 'plugin' ),
		...
		return new ZC_Error(
			'plugin_zc_incompatible',
			'<p>' . sprintf(
				/* translators: 1: Current ZelocoreCMS version, 2: Plugin name, 3: Required ZelocoreCMS version. */
				_x( '<strong>Error:</strong> Current ZelocoreCMS version (%1$s) does not meet minimum requirements for %2$s. The plugin requires ZelocoreCMS %3$s.', 'plugin' ),
				get_bloginfo( 'version' ),
				$plugin_headers['Name'],
				$requirements['requires']
			) . '</p>'
	...
	if ( ZC_Plugin_Dependencies::has_unmet_dependencies( $plugin ) ) {
		$dependency_names       = ZC_Plugin_Dependencies::get_dependency_names( $plugin );
		$unmet_dependencies     = array();
		$unmet_dependency_names = array();

		foreach ( $dependency_names as $dependency => $dependency_name ) {
			$dependency_file = ZC_Plugin_Dependencies::get_dependency_filepath( $dependency );
		...
		$error_message = sprintf(
			...
			$plugin_headers['Name'],
			count( $unmet_dependency_names ),
			implode( zc_get_list_item_separator(), $unmet_dependency_names )
		);
	 ...
	 * @since 6.9.0
	 *
	 * @param bool|ZC_Error $met_requirements True if the plugin meets requirements, ZC_Error if not.
	 * @param string $plugin Path to the plugin file relative to the plugins directory.
	 */
...
function is_uninstallable_plugin( $plugin ) {
	...
	if ( isset( $uninstallable_plugins[ $file ] ) || file_exists( ZC_PLUGIN_DIR . '/' . dirname( $file ) . '/uninstall.php' ) ) {
		return true;
	}
...
function uninstall_plugin( $plugin ) {
	...
	if ( file_exists( ZC_PLUGIN_DIR . '/' . dirname( $file ) . '/uninstall.php' ) ) {
		...
		define( 'ZC_UNINSTALL_PLUGIN', $file );

		zc_register_plugin_realpath( ZC_PLUGIN_DIR . '/' . $file );
		include_once ZC_PLUGIN_DIR . '/' . dirname( $file ) . '/uninstall.php';
	...
	if ( isset( $uninstallable_plugins[ $file ] ) ) {
		...
		zc_register_plugin_realpath( ZC_PLUGIN_DIR . '/' . $file );
		include_once ZC_PLUGIN_DIR . '/' . $file;
 ...
 *                              * Pass 'none' to leave div.zc-menu-image empty so an icon can be added via CSS.
 ...
 * @global array $menu
 * @global array $_zc_real_parent_file
 * @global bool  $_zc_submenu_nopriv
 * @global array $_registered_pages
 * @global array $_parent_pages
 *
 * @param string    $parent_slug The slug name for the parent menu (or the file name of a standard
 *                               ZelocoreCMS admin page).
...
function add_submenu_page( $parent_slug, $page_title, $menu_title, $capability, $menu_slug, $callback = '', $position = null ) {
	global $submenu, $menu, $_zc_real_parent_file, $_zc_submenu_nopriv,
		$_registered_pages, $_parent_pages;

	$menu_slug   = plugin_basename( $menu_slug );
	$parent_slug = plugin_basename( $parent_slug );

	if ( isset( $_zc_real_parent_file[ $parent_slug ] ) ) {
		$parent_slug = $_zc_real_parent_file[ $parent_slug ];
	}

	if ( ! current_user_can( $capability ) ) {
		$_zc_submenu_nopriv[ $parent_slug ][ $menu_slug ] = true;
		return false;
	}
	 ...
	 * See zc-admin/admin.php for redirect from edit.php to tools.php.
	 */
 ...
 * @global array  $_zc_real_parent_file
 * @global array  $_zc_menu_nopriv
 * @global array  $_zc_submenu_nopriv
 *
 * @param string $parent_page Optional. The slug name for the parent menu (or the file name
 *                            of a standard ZelocoreCMS admin page). Default empty string.
 * @return string The parent file of the current admin page.
 */
function get_admin_page_parent( $parent_page = '' ) {
	global $parent_file, $menu, $submenu, $pagenow, $typenow,
		$plugin_page, $_zc_real_parent_file, $_zc_menu_nopriv, $_zc_submenu_nopriv;

	if ( ! empty( $parent_page ) && 'admin.php' !== $parent_page ) {
		return $_zc_real_parent_file[ $parent_page ] ?? $parent_page;
	}
	...
	if ( 'admin.php' === $pagenow && isset( $plugin_page ) ) {
		foreach ( (array) $menu as $parent_menu ) {
			if ( $parent_menu[2] === $plugin_page ) {
				...
				return $_zc_real_parent_file[ $parent_file ] ?? $parent_file;
			}
		}
		if ( isset( $_zc_menu_nopriv[ $plugin_page ] ) ) {
			$parent_file = $plugin_page;
			return $_zc_real_parent_file[ $parent_file ] ?? $parent_file;
		}
	}

	if ( isset( $plugin_page ) && isset( $_zc_submenu_nopriv[ $pagenow ][ $plugin_page ] ) ) {
		$parent_file = $pagenow;
		return $_zc_real_parent_file[ $parent_file ] ?? $parent_file;
	}
	...
	foreach ( array_keys( (array) $submenu ) as $parent_page ) {
		foreach ( $submenu[ $parent_page ] as $submenu_array ) {
			if ( isset( $_zc_real_parent_file[ $parent_page ] ) ) {
				$parent_page = $_zc_real_parent_file[ $parent_page ];
			}
...
function user_can_access_admin_page() {
	global $pagenow, $menu, $submenu, $_zc_menu_nopriv, $_zc_submenu_nopriv,
		$plugin_page, $_registered_pages;

	$parent = get_admin_page_parent();

	if ( ! isset( $plugin_page ) && isset( $_zc_submenu_nopriv[ $parent ][ $pagenow ] ) ) {
		return false;
	}

	if ( isset( $plugin_page ) ) {
		if ( isset( $_zc_submenu_nopriv[ $parent ][ $plugin_page ] ) ) {
			return false;
		}
	...
	if ( empty( $parent ) ) {
		if ( isset( $_zc_menu_nopriv[ $pagenow ] ) ) {
			return false;
		}
		if ( isset( $_zc_submenu_nopriv[ $pagenow ][ $pagenow ] ) ) {
			return false;
		}
		if ( isset( $plugin_page ) && isset( $_zc_submenu_nopriv[ $pagenow ][ $plugin_page ] ) ) {
			return false;
		}
		if ( isset( $plugin_page ) && isset( $_zc_menu_nopriv[ $plugin_page ] ) ) {
			return false;
		}

		foreach ( array_keys( $_zc_submenu_nopriv ) as $key ) {
			if ( isset( $_zc_submenu_nopriv[ $key ][ $pagenow ] ) ) {
				return false;
			}
			if ( isset( $plugin_page ) && isset( $_zc_submenu_nopriv[ $key ][ $plugin_page ] ) ) {
				return false;
			}
		}
	...
	if ( isset( $plugin_page ) && $plugin_page === $parent && isset( $_zc_menu_nopriv[ $plugin_page ] ) ) {
		return false;
	}
...
function settings_fields( $option_group ) {
	...
	zc_nonce_field( "$option_group-options" );
}
 ...
 */
function zc_clean_plugins_cache( $clear_update_cache = true ) {
	if ( $clear_update_cache ) {
		delete_site_transient( 'update_plugins' );
	}
	zc_cache_delete( 'plugins', 'plugins' );
}
 ...
 *
 * @since 3.0.0
 * @since 4.4.0 Function was moved into the `zc-admin/includes/plugin.php` file.
 *
 * @param string $plugin Path to the plugin file relative to the plugins directory.
 */
function plugin_sandbox_scrape( $plugin ) {
	...
	zc_register_plugin_realpath( ZC_PLUGIN_DIR . '/' . $plugin );
	include_once ZC_PLUGIN_DIR . '/' . $plugin;
}
 ...
 * https://developer.zelocorecms.org/plugins/privacy/suggesting-text-for-the-site-privacy-policy/.
 *
 ...
 */
function zc_add_privacy_policy_content( $plugin_name, $policy_text ) {
	if ( ! is_admin() ) {
		_doing_it_wrong(
			__FUNCTION__,
			sprintf(
				/* translators: %s: admin_init */
				__( 'The suggested privacy policy content should be added only in zc-admin by using the %s (or later) action.' ),
	...
	ZC_Privacy_Policy_Content::add( $plugin_name, $policy_text );
}
 ...
 */
function zc_get_plugin_error( $plugin ) {
	if ( ! isset( $GLOBALS['_paused_plugins'] ) ) {
		return false;
	}
 ...
 * @return true|ZC_Error True on success, false if `$plugin` was not paused,
 *                       `ZC_Error` on failure.
 */
function resume_plugin( $plugin, $redirect = '' ) {
	...
	if ( ! empty( $redirect ) ) {
		zc_redirect(
			add_query_arg(
				'_error_nonce',
				zc_create_nonce( 'plugin-resume-error_' . $plugin ),
				$redirect
			)
		);
	...
	$result = zc_paused_plugins()->delete( $extension );
 ...
 * upgrade due to incompatibility with the current version of ZelocoreCMS.
 *
 * @since 5.8.0
 * @access private
 *
...
function deactivated_plugins_notice() {
	...
	$blog_deactivated_plugins = get_option( 'zc_force_deactivated_plugins' );
	$site_deactivated_plugins = array();

	if ( false === $blog_deactivated_plugins ) {
		// Option not in database, add an empty array to avoid extra DB queries on subsequent loads.
		update_option( 'zc_force_deactivated_plugins', array(), false );
	}

	if ( is_multisite() ) {
		$site_deactivated_plugins = get_site_option( 'zc_force_deactivated_plugins' );
		if ( false === $site_deactivated_plugins ) {
			// Option not in database, add an empty array to avoid extra DB queries on subsequent loads.
			update_site_option( 'zc_force_deactivated_plugins', array() );
		}
	}
	...
	foreach ( $deactivated_plugins as $plugin ) {
		...
		$message = sprintf(
			...
			sprintf(
				...
				__( '%s plugin deactivated during ZelocoreCMS upgrade.' ),
				$plugin['plugin_name']
			),
			$explanation,
		...
		zc_admin_notice( $message, array( 'type' => 'warning' ) );
	}
