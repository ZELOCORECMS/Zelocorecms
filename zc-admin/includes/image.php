 ...
 * @return string|ZC_Error New filepath on success, ZC_Error on failure.
 */
function zc_crop_image( $src, $src_x, $src_y, $src_w, $src_h, $dst_w, $dst_h, $src_abs = false, $dst_file = false ) {
	$src_file = $src;
	...
	$editor = zc_get_image_editor( $src );
	...
	if ( ! $dst_file ) {
		$dst_file = str_replace( zc_basename( $src_file ), 'cropped-' . zc_basename( $src_file ), $src_file );
	}
	 ...
	 */
	zc_mkdir_p( dirname( $dst_file ) );

	$dst_file = dirname( $dst_file ) . '/' . zc_unique_filename( dirname( $dst_file ), zc_basename( $dst_file ) );
 ...
 */
function zc_get_missing_image_subsizes( $attachment_id ) {
	...
	if ( ! empty( $image_meta['original_image'] ) ) {
		...
		$imagesize  = zc_getimagesize( $image_file );
	}
	 ...
	 * @param int     $attachment_id The image attachment post ID.
	 */
	return apply_filters( 'zc_get_missing_image_subsizes', $missing_sizes, $image_meta, $attachment_id );
}
 ...
 *
 * @param int $attachment_id The image attachment post ID.
 * @return array|ZC_Error The updated image meta data array or ZC_Error object
 *                        if both the image meta and the attached file are missing.
 */
function zc_update_image_subsizes( $attachment_id ) {
	$image_meta = zc_get_attachment_metadata( $attachment_id );
	$image_file = zc_get_original_image_path( $attachment_id );
	...
	if ( empty( $image_meta ) || ! is_array( $image_meta ) ) {
		 ...
		 */
		if ( ! empty( $image_file ) ) {
			$image_meta = zc_create_image_subsizes( $image_file, $attachment_id );
		} else {
			return new ZC_Error( 'invalid_attachment', __( 'The attached file cannot be found.' ) );
		}
	} else {
		$missing_sizes = zc_get_missing_image_subsizes( $attachment_id );
		...
		$image_meta = _zc_make_subsizes( $missing_sizes, $image_file, $image_meta, $attachment_id );
	}

	/** This filter is documented in zc-admin/includes/image.php */
	$image_meta = apply_filters( 'zc_generate_attachment_metadata', $image_meta, $attachment_id, 'update' );

	// Save the updated metadata.
	zc_update_attachment_metadata( $attachment_id, $image_meta );
 ...
 * @access private
 *
 * @param array  $saved_data    The data returned from ZC_Image_Editor after successfully saving an image.
 * @param string $original_file Path to the original file.
 * @param array  $image_meta    The image meta data.
 * @param int    $attachment_id The attachment post ID.
 * @return array The updated image meta data.
 */
function _zc_image_meta_replace_original( $saved_data, $original_file, $image_meta, $attachment_id ) {
	...
	$image_meta['file'] = _zc_relative_upload_path( $new_file );

	// Add image file size.
	$image_meta['filesize'] = zc_filesize( $new_file );

	// Store the original image file name in image_meta.
	$image_meta['original_image'] = zc_basename( $original_file );
 ...
 */
function zc_create_image_subsizes( $file, $attachment_id ) {
	$imagesize = zc_getimagesize( $file );
	...
	$image_meta = array(
		...
		'filesize' => zc_filesize( $file ),
		'sizes'    => array(),
	);

	// Fetch additional metadata from EXIF/IPTC.
	$exif_meta = zc_read_image_metadata( $file );
	 ...
	 * the `_zc_attached_file` post meta value.
	 *
	...
	if ( $threshold && ( $image_meta['width'] > $threshold || $image_meta['height'] > $threshold ) ) {
		...
		$output_format = zc_get_image_editor_output_format( $file, $imagesize['mime'] );
	...
	if ( $scale_down || $convert ) {
		...
		if ( ! is_zc_error( $resized ) && is_array( $exif_meta ) ) {
			$resized = $editor->maybe_exif_rotate();
			$rotated = $resized; // bool true or ZC_Error
		}
		...
		if ( ! is_zc_error( $resized ) ) {
			...
			if ( ! is_zc_error( $saved ) ) {
				$image_meta = _zc_image_meta_replace_original( $saved, $file, $image_meta, $attachment_id );
	...
	$new_sizes = zc_get_registered_image_subsizes();
	...
	return _zc_make_subsizes( $new_sizes, $file, $image_meta, $attachment_id );
}
 ...
 */
function _zc_make_subsizes( $new_sizes, $file, $image_meta, $attachment_id ) {
	if ( empty( $image_meta ) || ! is_array( $image_meta ) ) {
 ...
 */
function zc_copy_parent_attachment_properties( $cropped, $parent_attachment_id, $context = '' ) {
	$parent          = get_post( $parent_attachment_id );
	$parent_url      = zc_get_attachment_url( $parent->ID );
	$parent_basename = zc_basename( $parent_url );
	$url             = str_replace( zc_basename( $parent_url ), zc_basename( $cropped ), $parent_url );

	$size       = zc_getimagesize( $cropped );
	...
	$attachment = array(
		'post_title'     => $use_original_title ? $parent->post_title : zc_basename( $cropped ),
	...
	if ( '' !== trim( $parent->_zc_attachment_image_alt ) ) {
		$attachment['meta_input'] = array(
			'_zc_attachment_image_alt' => zc_slash( $parent->_zc_attachment_image_alt ),
		);
	}
 ...
 */
function zc_generate_attachment_metadata( $attachment_id, $file ) {
	...
	if ( 'image/heic' === $mime_type || ( preg_match( '!^image/!', $mime_type ) && file_is_displayable_image( $file ) ) ) {
		...
		$metadata = zc_create_image_subsizes( $file, $attachment_id );
	} elseif ( zc_attachment_is( 'video', $attachment ) ) {
		$metadata = zc_read_video_metadata( $file );
		$support  = current_theme_supports( 'post-thumbnails', 'attachment:video' ) || post_type_supports( 'attachment:video', 'thumbnail' );
	} elseif ( zc_attachment_is( 'audio', $attachment ) ) {
		$metadata = zc_read_audio_metadata( $file );
		$support  = current_theme_supports( 'post-thumbnails', 'attachment:audio' ) || post_type_supports( 'attachment:audio', 'thumbnail' );
	}

	/*
	 * zc_read_video_metadata() and zc_read_audio_metadata() return `false`
	...
	if ( $support && ! empty( $metadata['image']['data'] ) ) {
		...
		if ( ! empty( $exists ) ) {
			...
			$basename = str_replace( '.', '-', zc_basename( $file ) ) . '-image' . $ext;
			$uploaded = zc_upload_bits( $basename, '', $metadata['image']['data'] );
			if ( false === $uploaded['error'] ) {
				...
				$sub_attachment_id = zc_insert_attachment( $image_attachment, $uploaded['file'] );
				add_post_meta( $sub_attachment_id, '_cover_hash', $hash );
				$attach_data = zc_generate_attachment_metadata( $sub_attachment_id, $uploaded['file'] );
				zc_update_attachment_metadata( $sub_attachment_id, $attach_data );
		...
		if ( ! empty( $merged_sizes ) ) {
			...
			if ( ! is_zc_error( $editor ) ) { // No support for this type of file.
				/*
				...
				$preview_file = $dirname . zc_unique_filename( $dirname, zc_basename( $file, $ext ) . '-pdf.jpg' );

				$uploaded = $editor->save( $preview_file, 'image/jpeg' );
				unset( $editor );

				// Resize based on the full size image, rather than the source.
				if ( ! is_zc_error( $uploaded ) ) {
					...
					$metadata = _zc_make_subsizes( $merged_sizes, $image_file, $metadata, $attachment_id );
				}
			}
		}
	}
	...
	if ( ! isset( $metadata['filesize'] ) && file_exists( $file ) ) {
		$metadata['filesize'] = zc_filesize( $file );
	}
	 ...
	 */
	return apply_filters( 'zc_generate_attachment_metadata', $metadata, $attachment_id, 'create' );
}
 ...
 */
function zc_read_image_metadata( $file ) {
	if ( ! file_exists( $file ) ) {
		return false;
	}

	$image_size = zc_getimagesize( $file );
	...
	if ( is_callable( 'iptcparse' ) ) {
		...
		if ( ! empty( $info['APP13'] ) ) {
			...
			if ( defined( 'ZC_DEBUG' ) && ZC_DEBUG
				...
				// Silencing notice and warning is intentional. See https://core.trac.zelocorecms.org/ticket/42480
				$iptc = @iptcparse( $info['APP13'] );
			}
	...
	$meta['alt'] = zc_get_image_alttext( $file );
	 ...
	 */
	$exif_image_types = apply_filters( 'zc_read_image_metadata_types', array( IMAGETYPE_JPEG, IMAGETYPE_TIFF_II, IMAGETYPE_TIFF_MM ) );
	...
	if ( is_callable( 'exif_read_data' ) && in_array( $image_type, $exif_image_types, true ) ) {
		...
		if ( ! empty( $exif['FNumber'] ) && is_scalar( $exif['FNumber'] ) ) {
			$meta['aperture'] = round( zc_exif_frac2dec( $exif['FNumber'] ), 2 );
		}
		if ( ! empty( $exif['Model'] ) ) {
			$meta['camera'] = trim( $exif['Model'] );
		}
		if ( empty( $meta['created_timestamp'] ) && ! empty( $exif['DateTimeDigitized'] ) ) {
			$meta['created_timestamp'] = zc_exif_date2ts( $exif['DateTimeDigitized'] );
		}
		if ( ! empty( $exif['FocalLength'] ) ) {
			$meta['focal_length'] = (string) $exif['FocalLength'];
			if ( is_scalar( $exif['FocalLength'] ) ) {
				$meta['focal_length'] = (string) zc_exif_frac2dec( $exif['FocalLength'] );
			}
		}
		...
		if ( ! empty( $exif['ExposureTime'] ) ) {
			...
			if ( is_scalar( $exif['ExposureTime'] ) ) {
				$meta['shutter_speed'] = (string) zc_exif_frac2dec( $exif['ExposureTime'] );
			}
		}
	...
	foreach ( array( 'title', 'caption', 'credit', 'copyright', 'camera', 'iso' ) as $key ) {
		if ( $meta[ $key ] && ! zc_is_valid_utf8( $meta[ $key ] ) ) {
			$meta[ $key ] = utf8_encode( $meta[ $key ] );
		}
	}

	foreach ( $meta['keywords'] as $key => $keyword ) {
		if ( ! zc_is_valid_utf8( $keyword ) ) {
			$meta['keywords'][ $key ] = utf8_encode( $keyword );
		}
	}

	$meta = zc_kses_post_deep( $meta );
	 ...
	 */
	return apply_filters( 'zc_read_image_metadata', $meta, $file, $image_type, $iptc, $exif );
}
 ...
 */
function zc_get_image_alttext( $file ) {
	$alt_text     = '';
...
function _load_image_to_edit_path( $attachment_id, $size = 'full' ) {
	...
	if ( $filepath && file_exists( $filepath ) ) {
		 ...
		 *                                    an array of width and height values in pixels (in that order).
		 */
		$filepath = apply_filters( 'load_image_to_edit_attachmenturl', zc_get_attachment_url( $attachment_id ), $attachment_id, $size );
	}
...
function _copy_image_file( $attachment_id ) {
	...
	if ( $src_file ) {
		$dst_file = str_replace( zc_basename( $dst_file ), 'copy-' . zc_basename( $dst_file ), $dst_file );
		$dst_file = dirname( $dst_file ) . '/' . zc_unique_filename( dirname( $dst_file ), zc_basename( $dst_file ) );
