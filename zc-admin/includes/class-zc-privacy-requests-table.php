<?php
/**
 * List Table API: ZC_Privacy_Requests_Table class
 *
 * @package ZelocoreCMS
...
abstract class ZC_Privacy_Requests_Table extends ZC_List_Table {

	/**
	 * Action name for the requests this table will work with. Classes
	 * which inherit from ZC_Privacy_Requests_Table should define this.
	 *
	...
	public function process_bulk_action() {
		...
		$request_ids = isset( $_REQUEST['request_id'] ) ? zc_parse_id_list( zc_unslash( $_REQUEST['request_id'] ) ) : array();
		...
		switch ( $action ) {
			case 'resend':
				foreach ( $request_ids as $request_id ) {
					$resend = _zc_privacy_resend_request( $request_id );

					if ( $resend && ! is_zc_error( $resend ) ) {
						++$count;
					} else {
						++$failures;
					}
			...
			case 'complete':
				foreach ( $request_ids as $request_id ) {
					$result = _zc_privacy_completed_request( $request_id );
			...
			case 'delete':
				foreach ( $request_ids as $request_id ) {
					if ( zc_delete_post( $request_id, true ) ) {
						++$count;
					} else {
						++$failures;
					}
	...
	public function prepare_items() {
		...
		$requests_query = new ZC_Query( $args );
		$requests       = $requests_query->posts;

		foreach ( $requests as $request ) {
			$this->items[] = zc_get_user_request( $request->ID );
		}
	 ...
	 * @since 4.9.6
	 *
	 * @param ZC_User_Request $item Item being shown.
	...
	public function column_default( $item, $column_name ) {
		 ...
		 * @param ZC_User_Request $item        The item being shown.
		 */
	 ...
	 * @since 4.9.6
	 *
	 * @param ZC_User_Request $item The current item.
	 */
