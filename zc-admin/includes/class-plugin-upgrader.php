 ...
 * @see ZC_Upgrader
 */
class Plugin_Upgrader extends ZC_Upgrader {
	 ...
	 *                                    Default true.
	 * }
	 * @return bool|ZC_Error True if the installation was successful, false or a ZC_Error otherwise.
	 */
	public function install( $package, $args = array() ) {
		...
		if ( $parsed_args['clear_update_cache'] ) {
			// Clear cache so zc_update_plugins() knows about the new plugin.
			add_action( 'upgrader_process_complete', 'zc_clean_plugins_cache', 9, 0 );
		}

		$this->run(
			array(
				'package'           => $package,
				'destination'       => ZC_PLUGIN_DIR,
		...
		remove_action( 'upgrader_process_complete', 'zc_clean_plugins_cache', 9 );
		remove_filter( 'upgrader_source_selection', array( $this, 'check_package' ) );

		if ( ! $this->result || is_zc_error( $this->result ) ) {
			return $this->result;
		}

		// Force refresh of plugin update information.
		zc_clean_plugins_cache( $parsed_args['clear_update_cache'] );
	 ...
	 *                                    Default true.
	 * }
	 * @return bool|ZC_Error True if the upgrade was successful, false or a ZC_Error object otherwise.
	 */
	...
	public function bulk_upgrade( $plugins, $args = array() ) {
		...
		$connected = $this->fs_connect( array( ZC_CONTENT_DIR, ZC_PLUGIN_DIR ) );
		if ( ! $connected ) {
			$this->skin->footer();
			return false;
		}
		...
		foreach ( $plugins as $plugin ) {
			++$this->update_current;
			$this->skin->plugin_info = get_plugin_data( ZC_PLUGIN_DIR . '/' . $plugin, false, true );
			...
			if ( isset( $upgrade_data->requires ) && ! is_zc_version_compatible( $upgrade_data->requires ) ) {
				$result = new ZC_Error(
					'incompatible_zc_required_version',
					sprintf(
						/* translators: 1: Current ZelocoreCMS version, 2: ZelocoreCMS version required by the new plugin version. */
						__( 'Your ZelocoreCMS version is %1$s, however the new plugin version requires %2$s.' ),
						$zc_version,
		...
		foreach ( $results as $plugin => $result ) {
			...
			if ( ! $result || is_zc_error( $result ) || ! isset( $past_failure_emails[ $plugin ] ) ) {
				continue;
			}
	 ...
	 * @return string|ZC_Error The source as passed, or a ZC_Error object on failure.
	 */
	public function check_package( $source ) {
		...
		$working_directory = str_replace( $zc_filesystem->zc_content_dir(), trailingslashit( ZC_CONTENT_DIR ), $source );
		...
		if ( empty( $this->new_plugin_data ) ) {
			return new ZC_Error( 'incompatible_archive_no_plugins', $this->strings['incompatible_archive'], __( 'No valid plugins were found.' ) );
		}
		...
		if ( ! is_php_version_compatible( $requires_php ) ) {
			...
			return new ZC_Error( 'incompatible_php_required_version', $this->strings['incompatible_archive'], $error );
		}

		if ( ! is_zc_version_compatible( $requires_wp ) ) {
			$error = sprintf(
				/* translators: 1: Current ZelocoreCMS version, 2: Version required by the uploaded plugin. */
				__( 'Your ZelocoreCMS version is %1$s, however the uploaded plugin requires %2$s.' ),
				$zc_version,
				$requires_wp
			);

			return new ZC_Error( 'incompatible_zc_required_version', $this->strings['incompatible_archive'], $error );
		}
	 ...
	 *
	 * @param bool|ZC_Error $response The installation response before the installation has started.
	 * @param array         $plugin   Plugin package arguments.
	 * @return bool|ZC_Error The original `$response` parameter or ZC_Error.
	 */
	public function deactivate_plugin_before_upgrade( $response, $plugin ) {

		if ( is_zc_error( $response ) ) { // Bypass.
			return $response;
		}
	 ...
	 * @since 5.4.0
	 *
	 * @param bool|ZC_Error $response The installation response after the installation has finished.
	 * @param array         $plugin   Plugin package arguments.
	 * @return bool|ZC_Error The original `$response` parameter or ZC_Error.
	 */
	 ...
	 * @global ZC_Filesystem_Base $zc_filesystem ZelocoreCMS filesystem subclass.
	 *
	 * @param bool|ZC_Error $removed            Whether the destination was cleared.
	 *                                          True on success, ZC_Error on failure.
	 * @param string        $local_destination  The local package destination.
	 * @param string        $remote_destination The remote package destination.
	 * @param array         $plugin             Extra arguments passed to hooked filters.
	 * @return bool|ZC_Error
	 */
	public function delete_old_plugin( $removed, $local_destination, $remote_destination, $plugin ) {
		...
		if ( ! $zc_filesystem->exists( $this_plugin_dir ) ) { // If it's already vanished.
			return $removed;
		}
		...
		if ( strpos( $plugin, '/' ) && $this_plugin_dir !== $plugins_dir ) {
			...
			$deleted = $zc_filesystem->delete( $plugins_dir . $plugin );
		}
