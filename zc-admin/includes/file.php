...
$zc_file_descriptions = array(
	...
	// Deprecated files.
	'zc-layout.css'         => __( 'Stylesheet' ),
	'zc-comments.php'       => __( 'Comments Template' ),
	'zc-comments-popup.php' => __( 'Popup Comments Template' ),
	'comments-popup.php'    => __( 'Popup Comments' ),
);

/**
 * Gets the description for standard ZelocoreCMS theme files.
 *
 * @since 1.5.0
 *
 * @global array $zc_file_descriptions Theme file descriptions.
 * @global array $allowed_files        List of allowed files.
 *
 * @param string $file Filesystem path or filename.
 * @return string Description of file from $zc_file_descriptions or basename of $file if description doesn't exist.
 *                Appends 'Page Template' to basename of $file if the file is a page template.
 */
function get_file_description( $file ) {
	global $zc_file_descriptions, $allowed_files;

	$dirname   = pathinfo( $file, PATHINFO_DIRNAME );
	$file_path = $allowed_files[ $file ];

	if ( isset( $zc_file_descriptions[ basename( $file ) ] ) && '.' === $dirname ) {
		return $zc_file_descriptions[ basename( $file ) ];
...
/**
 * Gets the absolute filesystem path to the root of the ZelocoreCMS installation.
 *
 * @since 1.5.0
 *
 * @return string Full filesystem path to the root of the ZelocoreCMS installation.
 */
function get_home_path() {
	...
	if ( ! empty( $home ) && 0 !== strcasecmp( $home, $siteurl ) ) {
		$zc_path_rel_to_home = str_ireplace( $home, '', $siteurl ); /* $siteurl - $home */
		$pos                 = strripos( str_replace( '\\', '/', $_SERVER['SCRIPT_FILENAME'] ), trailingslashit( $zc_path_rel_to_home ) );
 ...
 */
function zc_get_plugin_file_editable_extensions( $plugin ) {
 ...
 * @since 4.9.0
 *
 * @param ZC_Theme $theme Theme object.
 * @return string[] Array of editable file extensions.
 */
function zc_get_theme_file_editable_extensions( $theme ) {
	 ...
	 * @param ZC_Theme $theme         The active theme object.
	 */
	$file_types = apply_filters( 'zc_theme_editor_filetypes', $default_types, $theme );
 ...
 * @since 4.9.0
 */
function zc_print_file_editor_templates() {
	?>
	<script type="text/html" id="tmpl-zc-file-editor-notice">
			...
			<# } else if ( 'file_not_writable' === data.code ) { #>
				<p>
					...
					printf(
						...
						__( 'https://developer.zelocorecms.org/advanced-administration/server/file-permissions/' )
					);
					?>
				</p>
			<# } else { #>
 ...
 * }
 * @return true|ZC_Error True on success or `ZC_Error` on failure.
 */
function zc_edit_theme_plugin_file( $args ) {
	if ( empty( $args['file'] ) ) {
		return new ZC_Error( 'missing_file' );
	}

	if ( 0 !== validate_file( $args['file'] ) ) {
		return new ZC_Error( 'bad_file' );
	}

	if ( ! isset( $args['newcontent'] ) ) {
		return new ZC_Error( 'missing_content' );
	}

	if ( ! isset( $args['nonce'] ) ) {
		return new ZC_Error( 'missing_nonce' );
	}
	...
	if ( ! empty( $args['plugin'] ) ) {
		...
		if ( ! current_user_can( 'edit_plugins' ) ) {
			return new ZC_Error( 'unauthorized', __( 'Sorry, you are not allowed to edit plugins for this site.' ) );
		}

		if ( ! zc_verify_nonce( $args['nonce'], 'edit-plugin_' . $file ) ) {
			return new ZC_Error( 'nonce_failure' );
		}

		if ( ! array_key_exists( $plugin, get_plugins() ) ) {
			return new ZC_Error( 'invalid_plugin' );
		}

		if ( 0 !== validate_file( $file, get_plugin_files( $plugin ) ) ) {
			return new ZC_Error( 'bad_plugin_file_path', __( 'Sorry, that file cannot be edited.' ) );
		}

		$editable_extensions = zc_get_plugin_file_editable_extensions( $plugin );

		$real_file = ZC_PLUGIN_DIR . '/' . $file;
		...
		if ( 0 !== validate_file( $stylesheet ) ) {
			return new ZC_Error( 'bad_theme_path' );
		}

		if ( ! current_user_can( 'edit_themes' ) ) {
			return new ZC_Error( 'unauthorized', __( 'Sorry, you are not allowed to edit templates for this site.' ) );
		}

		$theme = zc_get_theme( $stylesheet );
		if ( ! $theme->exists() ) {
			return new ZC_Error( 'non_existent_theme', __( 'The requested theme does not exist.' ) );
		}

		if ( ! zc_verify_nonce( $args['nonce'], 'edit-theme_' . $stylesheet . '_' . $file ) ) {
			return new ZC_Error( 'nonce_failure' );
		}
		...
		$editable_extensions = zc_get_theme_file_editable_extensions( $theme );
		...
		if ( 0 !== validate_file( $file, array_keys( $allowed_files ) ) ) {
			return new ZC_Error( 'disallowed_theme_file', __( 'Sorry, that file cannot be edited.' ) );
		}
		...
		return new ZC_Error( 'missing_theme_or_plugin' );
	}

	// Ensure file is real.
	if ( ! is_file( $real_file ) ) {
		return new ZC_Error( 'file_does_not_exist', __( 'File does not exist! Please double check the name and try again.' ) );
	}
	...
	if ( preg_match( '/\.([^.]+)$/', $real_file, $matches ) ) {
		...
		if ( ! in_array( $extension, $editable_extensions, true ) ) {
			return new ZC_Error( 'illegal_file_type', __( 'Files of this type are not editable.' ) );
		}
	}
	...
	if ( ! is_writable( $real_file ) ) {
		return new ZC_Error( 'file_not_writable' );
	}
	...
	if ( false === $written ) {
		return new ZC_Error( 'unable_to_write', __( 'Unable to write to file.' ) );
	}

	zc_opcache_invalidate( $real_file, true );
	...
	if ( $is_active && 'php' === $extension ) {
		...
		$scrape_params = array(
			'zc_scrape_key'   => $scrape_key,
			'zc_scrape_nonce' => $scrape_nonce,
		);
		$headers       = array(
		...
		$needle_start = "###### zc_scraping_result_start:$scrape_key ######";
		$needle_end   = "###### zc_scraping_result_end:$scrape_key ######";
		...
		$body                   = zc_remote_retrieve_body( $r );
		...
		if ( true !== $result ) {
			...
			return new ZC_Error( 'php_error', $message, $result );
		}
	}

	if ( $theme instanceof ZC_Theme ) {
		$theme->cache_delete();
	}
 ...
 */
function zc_tempnam( $filename = '', $dir = '' ) {
	if ( empty( $dir ) ) {
		$dir = get_temp_dir();
	}
	...
	if ( ! $temp_filename ) {
		return zc_tempnam( dirname( $filename ), $dir );
	}

	// Suffix some random data to avoid filename conflicts.
	$temp_filename .= '-' . zc_generate_password( 6, false );
	$temp_filename .= '.tmp';
	$temp_filename  = zc_unique_filename( $dir, $temp_filename );
	...
	if ( $characters_over_limit > 0 ) {
		...
		return zc_tempnam( $filename, $dir );
	}
...
function validate_file_to_edit( $file, $allowed_files = array() ) {
	...
	switch ( $code ) {
		case 1:
			zc_die( __( 'Sorry, that file cannot be edited.' ) );

			// case 2 :
			// zc_die( __('Sorry, cannot call files with their real path.' ));
...
/**
 * Handles PHP uploads in ZelocoreCMS.
 *
 ...
 * }
 */
function _zc_handle_upload( &$file, $overrides, $time, $action ) {
	// The default error handler.
	if ( ! function_exists( 'zc_handle_upload_error' ) ) {
		function zc_handle_upload_error( &$file, $message ) {
			return array( 'error' => $message );
		}
	}

	/**
	 * Filters the data for a file before it is uploaded to ZelocoreCMS.
	 *
	 * The dynamic portion of the hook name, `$action`, refers to the post action.
	 *
	 * Possible hook names include:
	 *
	 *  - `zc_handle_sideload_prefilter`
	 *  - `zc_handle_upload_prefilter`
	 *
	 * @since 2.9.0 as 'zc_handle_upload_prefilter'.
	...
	/**
	 * Filters the override parameters for a file before it is uploaded to ZelocoreCMS.
	 *
	 * The dynamic portion of the hook name, `$action`, refers to the post action.
	 *
	 * Possible hook names include:
	 *
	 *  - `zc_handle_sideload_overrides`
	 *  - `zc_handle_upload_overrides`
	 *
	 * @since 5.7.0
	 *
	 * @param array|false $overrides An array of override parameters for this file. Boolean false if none are
	 *                               provided. See {@see _zc_handle_upload()}.
	 * @param array       $file      {
	 *     Reference to a single element from `$_FILES`.
	 *
	 *     @type string $name     The original name of the file on the client machine.
	...
	$upload_error_handler = 'zc_handle_upload_error';
	if ( isset( $overrides['upload_error_handler'] ) ) {
		$upload_error_handler = $overrides['upload_error_handler'];
	}

	// You may have had one or more 'zc_handle_upload_prefilter' functions error out the file. Handle that gracefully.
	...
	$test_uploaded_file = 'zc_handle_upload' === $action ? is_uploaded_file( $file['tmp_name'] ) : @is_readable( $file['tmp_name'] );
	if ( ! $test_uploaded_file ) {
		return call_user_func_array( $upload_error_handler, array( &$file, __( 'Specified file failed upload test.' ) ) );
	}

	$test_file_size = 'zc_handle_upload' === $action ? $file['size'] : filesize( $file['tmp_name'] );
	...
	if ( $test_type ) {
		$zc_filetype     = zc_check_filetype_and_ext( $file['tmp_name'], $file['name'], $mimes );
		$ext             = empty( $zc_filetype['ext'] ) ? '' : $zc_filetype['ext'];
		$type            = empty( $zc_filetype['type'] ) ? '' : $zc_filetype['type'];
		$proper_filename = empty( $zc_filetype['proper_filename'] ) ? '' : $zc_filetype['proper_filename'];

		// Check to see if zc_check_filetype_and_ext() determined the filename was incorrect.
		if ( $proper_filename ) {
	 ...
	 * overriding this one.
	 */
	$uploads = zc_upload_dir( $time );
	if ( ! ( $uploads && false === $uploads['error'] ) ) {
		return call_user_func_array( $upload_error_handler, array( &$file, $uploads['error'] ) );
	}

	$filename = zc_unique_filename( $uploads['path'], $file['name'], $unique_filename_callback );
	...
	if ( null === $move_new_file ) {
		if ( 'zc_handle_upload' === $action ) {
			$move_new_file = @move_uploaded_file( $file['tmp_name'], $new_file );
		} else {
			// Use copy and unlink because rename breaks streams.
			// phpcs:ignore ZelocoreCMS.PHP.NoSilencedErrors.Discouraged
	...
	return apply_filters(
		...
		'zc_handle_sideload' === $action ? 'sideload' : 'upload'
	);
}

/**
 * Wrapper for _zc_handle_upload().
 *
 * Passes the {@see 'zc_handle_upload'} action.
 *
 * @since 2.0.0
 *
 * @see _zc_handle_upload()
 *
 * @param array       $file      Reference to a single element of `$_FILES`.
 *                               Call the function once for each uploaded file.
 *                               See _zc_handle_upload() for accepted values.
 * @param array|false $overrides Optional. An associative array of names => values
 *                               to override default variables. Default false.
 *                               See _zc_handle_upload() for accepted values.
 * @param string|null $time      Optional. Time formatted in 'yyyy/mm'. Default null.
 * @return array See _zc_handle_upload() for return value.
 */
function zc_handle_upload( &$file, $overrides = false, $time = null ) {
	/*
	 *  $_POST['action'] must be set and its value must equal $overrides['action']
	 *  or this:
	 */
	$action = $overrides['action'] ?? 'zc_handle_upload';
	return _zc_handle_upload( $file, $overrides, $time, $action );
}

/**
 * Wrapper for _zc_handle_upload().
 *
 * Passes the {@see 'zc_handle_sideload'} action.
 *
 * @since 2.6.0
 *
 * @see _zc_handle_upload()
 ...
 * @return array See _zc_handle_upload() for return value.
 */
function zc_handle_sideload( &$file, $overrides = false, $time = null ) {
	/*
	 *  $_POST['action'] must be set and its value must equal $overrides['action']
	 *  or this:
	 */
	$action = $overrides['action'] ?? 'zc_handle_sideload';
	return _zc_handle_upload( $file, $overrides, $time, $action );
}

/**
 * Downloads a URL to a local temporary file using the ZelocoreCMS HTTP API.
 *
 * Please note that the calling function must delete or move the file.
 *
 * @since 2.5.0
 ...
 *                                       Default false.
 * @return string|ZC_Error Filename on success, ZC_Error on failure.
 */
function download_url( $url, $timeout = 300, $signature_verification = false ) {
	// WARNING: The file is not automatically deleted, the script must delete or move the file.
	if ( ! $url ) {
		return new ZC_Error( 'http_no_url', __( 'No URL Provided.' ) );
	}
	...
	$tmpfname = zc_tempnam( $url_filename );
	if ( ! $tmpfname ) {
		return new ZC_Error( 'http_no_file', __( 'Could not create temporary file.' ) );
	}
	...
	$response_code = zc_remote_retrieve_response_code( $response );
	...
	if ( 200 !== $response_code ) {
		...
		return new ZC_Error( 'http_404', trim( zc_remote_retrieve_response_message( $response ) ), $data );
	}

	$content_disposition = zc_remote_retrieve_header( $response, 'Content-Disposition' );
	...
	$mime_type = zc_remote_retrieve_header( $response, 'content-type' );
	...
	$content_md5 = zc_remote_retrieve_header( $response, 'Content-MD5' );

	if ( $content_md5 ) {
		$md5_check = verify_file_md5( $tmpfname, $content_md5 );

		if ( is_zc_error( $md5_check ) ) {
			unlink( $tmpfname );
			return $md5_check;
		}
	}
	...
	if ( $signature_verification ) {
		 ...
		 */
		$signed_hostnames = apply_filters( 'zc_signature_hosts', array( 'zelocorecms.org', 'downloads.zelocorecms.org', 's.w.org' ) );
	...
	if ( $signature_verification ) {
		$signature = zc_remote_retrieve_header( $response, 'X-Content-Signature' );

		if ( ! $signature ) {
			/*
			 * Retrieve signatures from a file if the header wasn't included.
			 * ZelocoreCMS.org stores signatures at $package_url.sig.
			 */
			 ...
			 */
			$signature_url = apply_filters( 'zc_signature_url', $signature_url, $url );

			if ( $signature_url ) {
				$signature_request = zc_safe_remote_get(
					$signature_url,
					array(
				...
				if ( ! is_zc_error( $signature_request ) && 200 === zc_remote_retrieve_response_code( $signature_request ) ) {
					$signature = explode( "\n", zc_remote_retrieve_body( $signature_request ) );
				}
			}
		}
	...
	if ( is_zc_error( $signature_verification ) ) {
		if (
			/**
			 ...
			 */
			apply_filters( 'zc_signature_softfail', true, $url )
		) {
 ...
 * @return bool|ZC_Error True on success, false when the MD5 format is unknown/unexpected,
 *                       ZC_Error on failure.
 */
 ...
 * @return bool|ZC_Error True on success, false if verification not attempted,
 *                       or ZC_Error describing an error condition.
 */
function verify_file_signature( $filename, $signatures, $filename_for_errors = false ) {
	if ( ! $filename_for_errors ) {
		$filename_for_errors = zc_basename( $filename );
	}
	...
	if ( ! extension_loaded( 'sodium' ) && ! ParagonIE_Sodium_Compat::polyfill_is_fast() ) {
		...
		// Allow for an old version of Sodium_Compat being loaded before the bundled ZelocoreCMS one.
		if ( method_exists( 'ParagonIE_Sodium_Compat', 'runtime_speed_test' ) ) {
			/*
			 * Run `ParagonIE_Sodium_Compat::runtime_speed_test()` in optimized integer mode,
			 * as that's what ZelocoreCMS utilizes during signing verifications.
			 */
			// phpcs:disable ZelocoreCMS.NamingConventions.ValidVariableName
...
/**
 * Retrieves the list of signing keys trusted by ZelocoreCMS.
 *
 * @since 5.2.0
 *
 * @return string[] Array of base64-encoded signing keys.
 */
function zc_trusted_keys() {
	$trusted_keys = array();

	if ( time() < 1617235200 ) {
		// ZelocoreCMS.org Key #1 - This key is only valid before April 1st, 2021.
	 ...
	 */
	return apply_filters( 'zc_trusted_keys', $trusted_keys );
}
 ...
 */
function zc_zip_file_is_valid( $file ) {
	...
	require_once ABSPATH . 'zc-admin/includes/class-pclzip.php';
...
/**
 * Unzips a specified ZIP file to a location on the filesystem via the ZelocoreCMS
 * Filesystem Abstraction.
 *
 * Assumes that ZC_Filesystem() has already been called and set up. Does not extract
...
function unzip_file( $file, $to ) {
	...
	if ( ! $zc_filesystem || ! is_object( $zc_filesystem ) ) {
		return new ZC_Error( 'fs_unavailable', __( 'Could not access filesystem.' ) );
	}

	// Unzip can use a lot of memory, but not this much hopefully.
	zc_raise_memory_limit( 'admin' );

	$needed_dirs = array();
	$to          = trailingslashit( $to );

	// Determine any parent directories needed (of the upgrade directory).
	if ( ! $zc_filesystem->is_dir( $to ) ) { // Only do parents if no children exist.
 ...
 *
 * Assumes that ZC_Filesystem() has already been called and set up.
 *
 * @since 3.0.0
 * @access private
 *
...
function _unzip_file_ziparchive( $file, $to, $needed_dirs = array() ) {
	...
	if ( true !== $zopen ) {
		return new ZC_Error( 'incompatible_archive', __( 'Incompatible Archive.' ), array( 'ziparchive_error' => $zopen ) );
	}
	...
	for ( $i = 0; $i < $z->numFiles; $i++ ) {
		...
		if ( ! $info ) {
			$z->close();
			return new ZC_Error( 'stat_failed_ziparchive', __( 'Could not retrieve file from archive.' ) );
		}
	...
	foreach ( $needed_dirs as $_dir ) {
		...
		if ( ! $zc_filesystem->mkdir( $_dir, FS_CHMOD_DIR ) && ! $zc_filesystem->is_dir( $_dir ) ) {
			$z->close();
			return new ZC_Error( 'mkdir_failed_ziparchive', __( 'Could not create directory.' ), $_dir );
		}
	}
	 ...
	 * @since 6.4.0
	 *
	 * @param null|true|ZC_Error $result         The result of the override. True on success, otherwise WP Error. Default null.
	 * @param string             $file           Full path and filename of ZIP archive.
	 * @param string             $to             Full path on the filesystem to extract archive to.
	...
	for ( $i = 0; $i < $z->numFiles; $i++ ) {
		...
		if ( false === $contents ) {
			$z->close();
			return new ZC_Error( 'extract_failed_ziparchive', __( 'Could not extract file from archive.' ), $info['name'] );
		}

		if ( ! $zc_filesystem->put_contents( $to . $info['name'], $contents, FS_CHMOD_FILE ) ) {
			$z->close();
			return new ZC_Error( 'copy_failed_ziparchive', __( 'Could not copy file.' ), $info['name'] );
		}
	}
	 ...
	 * @since 6.4.0
	 *
	 * @param true|ZC_Error $result         The result of unzipping the archive. True on success, otherwise ZC_Error. Default true.
	 * @param string        $file           Full path and filename of ZIP archive.
...
function _unzip_file_pclzip( $file, $to, $needed_dirs = array() ) {
	...
	if ( ! is_array( $archive_files ) ) {
		return new ZC_Error( 'incompatible_archive', __( 'Incompatible Archive.' ), $archive->errorInfo( true ) );
	}

	if ( 0 === count( $archive_files ) ) {
		return new ZC_Error( 'empty_archive_pclzip', __( 'Empty archive.' ) );
	}
	...
	foreach ( $needed_dirs as $_dir ) {
		// Only check to see if the dir exists upon creation failure. Less I/O this way.
		if ( ! $zc_filesystem->mkdir( $_dir, FS_CHMOD_DIR ) && ! $zc_filesystem->is_dir( $_dir ) ) {
			return new ZC_Error( 'mkdir_failed_pclzip', __( 'Could not create directory.' ), $_dir );
		}
	}
	...
	foreach ( $archive_files as $archive_file ) {
		...
		if ( ! $zc_filesystem->put_contents( $to . $archive_file['filename'], $archive_file['content'], FS_CHMOD_FILE ) ) {
			return new ZC_Error( 'copy_failed_pclzip', __( 'Could not copy file.' ), $archive_file['filename'] );
		}
	}
...
/**
 * Copies a directory from one location to another via the ZelocoreCMS Filesystem
 * Abstraction.
 *
 * Assumes that ZC_Filesystem() has already been called and setup.
 *
 * @since 2.5.0
 *
 * @global ZC_Filesystem_Base $zc_filesystem ZelocoreCMS filesystem subclass.
...
function copy_dir( $from, $to, $skip_list = array() ) {
	...
	$dirlist = $zc_filesystem->dirlist( $from );

	if ( false === $dirlist ) {
		return new ZC_Error( 'dirlist_failed_copy_dir', __( 'Directory listing failed.' ), basename( $from ) );
	}

	$from = trailingslashit( $from );
	$to   = trailingslashit( $to );

	if ( ! $zc_filesystem->exists( $to ) && ! $zc_filesystem->mkdir( $to ) ) {
		return new ZC_Error(
	...
	foreach ( (array) $dirlist as $filename => $fileinfo ) {
		...
		if ( 'f' === $fileinfo['type'] ) {
			if ( ! $zc_filesystem->copy( $from . $filename, $to . $filename, true, FS_CHMOD_FILE ) ) {
				// If copy failed, chmod file to 0644 and try again.
				$zc_filesystem->chmod( $to . $filename, FS_CHMOD_FILE );

				if ( ! $zc_filesystem->copy( $from . $filename, $to . $filename, true, FS_CHMOD_FILE ) ) {
					return new ZC_Error( 'copy_failed_copy_dir', __( 'Could not copy file.' ), $to . $filename );
				}
			}

			zc_opcache_invalidate( $to . $filename );
		} elseif ( 'd' === $fileinfo['type'] ) {
			if ( ! $zc_filesystem->is_dir( $to . $filename ) ) {
				if ( ! $zc_filesystem->mkdir( $to . $filename, FS_CHMOD_DIR ) ) {
					return new ZC_Error( 'mkdir_failed_copy_dir', __( 'Could not create directory.' ), $to . $filename );
				}
			}
...
function move_dir( $from, $to, $overwrite = false ) {
	...
	if ( trailingslashit( strtolower( $from ) ) === trailingslashit( strtolower( $to ) ) ) {
		return new ZC_Error( 'source_destination_same_move_dir', __( 'The source and destination are the same.' ) );
	}

	if ( $zc_filesystem->exists( $to ) ) {
		if ( ! $overwrite ) {
			return new ZC_Error( 'destination_already_exists_move_dir', __( 'The destination folder already exists.' ), $to );
		} elseif ( ! $zc_filesystem->delete( $to, true ) ) {
			// Can't overwrite if the destination couldn't be deleted.
			return new ZC_Error( 'destination_not_deleted_move_dir', __( 'The destination directory already exists and could not be removed.' ) );
		}
	}

	if ( $zc_filesystem->move( $from, $to ) ) {
		/*
		 ...
		 */
		usleep( 200000 );
		zc_opcache_invalidate_directory( $to );

		return true;
	}

	// Fall back to a recursive copy.
	if ( ! $zc_filesystem->is_dir( $to ) ) {
		if ( ! $zc_filesystem->mkdir( $to, FS_CHMOD_DIR ) ) {
			return new ZC_Error( 'mkdir_failed_move_dir', __( 'Could not create directory.' ), $to );
		}
	}
...
/**
 * Initializes and connects the ZelocoreCMS Filesystem Abstraction classes.
 *
 * This function will include the chosen transport and attempt connecting.
 *
 * Plugins may add extra transports, And force ZelocoreCMS to use them by returning
 ...
 *                                                   directly to the `ZC_Filesystem_*()` classes.
 *                                                   Default false.
 ...
 */
function ZC_Filesystem( $args = false, $context = false, $allow_relaxed_file_ownership = false ) { // phpcs:ignore ZelocoreCMS.NamingConventions.ValidFunctionName.FunctionNameInvalid
	global $zc_filesystem;

	require_once ABSPATH . 'zc-admin/includes/class-zc-filesystem-base.php';
	...
	if ( ! class_exists( "ZC_Filesystem_$method" ) ) {
		 ...
		 */
		$abstraction_file = apply_filters( 'filesystem_method_file', ABSPATH . 'zc-admin/includes/class-zc-filesystem-' . $method . '.php', $method );
	...
	$method = "ZC_Filesystem_$method";

	$zc_filesystem = new $method( $args );
	...
	if ( is_zc_error( $zc_filesystem->errors ) && $zc_filesystem->errors->has_errors() ) {
		return false;
	}

	if ( ! $zc_filesystem->connect() ) {
 ...
 *
 * The return value can be overridden by defining the `FS_METHOD` constant in `zc-config.php`,
 * or filtering via {@see 'filesystem_method'}.
 *
 * @link https://developer.zelocorecms.org/advanced-administration/zelocorecms/zc-config/#zelocorecms-upgrade-constants
 *
 * Plugins may define a custom transport handler, See ZC_Filesystem().
 *
 * @since 2.5.0
 *
 * @global callable $_zc_filesystem_direct_method
 *
...
function get_filesystem_method( $args = array(), $context = '', $allow_relaxed_file_ownership = false ) {
	...
	if ( ! $context ) {
		$context = ZC_CONTENT_DIR;
	}

	// If the directory doesn't exist (zc-content/languages) then use the parent directory as we'll create it.
	if ( ZC_LANG_DIR === $context && ! is_dir( $context ) ) {
	...
	if ( ! $method ) {
		...
		if ( $temp_handle ) {
			...
			// Attempt to determine the file owner of the ZelocoreCMS files, and that of newly created files.
			$zc_file_owner   = false;
			$temp_file_owner = false;
			if ( function_exists( 'fileowner' ) ) {
				$zc_file_owner   = @fileowner( __FILE__ );
				$temp_file_owner = @fileowner( $temp_file_name );
			}

			if ( false !== $zc_file_owner && $zc_file_owner === $temp_file_owner ) {
				/*
				 * ZelocoreCMS is creating files as the same owner as the ZelocoreCMS files,
				 * this means it's safe to modify & create new files via PHP.
				 */
				$method                                  = 'direct';
				$GLOBALS['_zc_filesystem_direct_method'] = 'file_owner';
				 ...
				 */
				$method                                  = 'direct';
				$GLOBALS['_zc_filesystem_direct_method'] = 'relaxed_ownership';
			}
 ...
 *
 * Hostnames may be in the form of hostname:portnumber (eg: zelocorecms.org:2467)
 ...
 * @param bool|ZC_Error $error                        Optional. Whether the current request has failed
...
function request_filesystem_credentials( $form_post, $type = '', $error = false, $context = '', $extra_fields = null, $allow_relaxed_file_ownership = false ) {
	 ...
	 * @param bool|ZC_Error $error                        Whether the current request has failed to connect,
	 *                                                    or an error object.
	...
	$submitted_form = zc_unslash( $_POST );

	// Verify nonce, or unset submitted form field values on failure.
	if ( ! isset( $_POST['_fs_nonce'] ) || ! zc_verify_nonce( $_POST['_fs_nonce'], 'filesystem-credentials' ) ) {
		unset(
...
<p id="request-filesystem-credentials-desc">
	...
	_e( 'To perform the requested action, ZelocoreCMS needs to access your web server.' );
	echo ' ';
...
<label for="hostname">
	...
	<input name="hostname" type="text" id="hostname" aria-describedby="request-filesystem-credentials-desc" class="code" placeholder="<?php esc_attr_e( 'example: www.zelocorecms.org' ); ?>" value="<?php echo $hostname_value; ?>"<?php disabled( defined( 'FTP_HOST' ) ); ?> />
</label>
...
<p id="auth-keys-desc"><?php _e( 'Enter the location on the server where the public and private keys are located. If a passphrase is needed, enter that in the password field above.' ); ?></p>
	 ...
	 * from ZC_Site_Health_Auto_Updates::test_check_zc_filesystem_method().
	 */
	...
	<p class="request-filesystem-credentials-action-buttons">
		<?php zc_nonce_field( 'filesystem-credentials', '_fs_nonce', false, true ); ?>
 ...
 * @since 4.2.0
 */
function zc_print_request_filesystem_credentials_modal() {
 ...
 */
function zc_opcache_invalidate( $filepath, $force = false ) {
	static $can_invalidate = null;

	/*
	 * Check to see if ZelocoreCMS is able to run `opcache_invalidate()` or not, and cache the value.
	 *
	 ...
	 * - https://core.trac.zelocorecms.org/ticket/36455
	 */
	 ...
	 * @since 5.5.0
	 *
	 * @param bool   $will_invalidate Whether ZelocoreCMS will invalidate `$filepath`. Default true.
	 * @param string $filepath        The path to the PHP file to invalidate.
	 */
	if ( apply_filters( 'zc_opcache_invalidate_file', true, $filepath ) ) {
 ...
 */
function zc_opcache_invalidate_directory( $dir ) {
	global $zc_filesystem;
	...
	if ( ! is_string( $dir ) || '' === trim( $dir ) ) {
		if ( ZC_DEBUG ) {
			$error_message = sprintf(
				...
				'<code>zc_opcache_invalidate_directory()</code>'
			);
			zc_trigger_error( '', $error_message );
		}
		return;
	}

	$dirlist = $zc_filesystem->dirlist( $dir, false, true );
	 ...
	 *
	 * ZC_Filesystem_*::dirlist() returns an array of file and directory information.
	 *
	 * This does not include a path to the file or directory.
	 * To invalidate files within sub-directories, recursion is needed
	 * to prepend an absolute path containing the sub-directory's name.
	 *
	 * @param array  $dirlist Array of file/directory information from ZC_Filesystem_Base::dirlist(),
	...
	$invalidate_directory = static function ( $dirlist, $path ) use ( &$invalidate_directory ) {
		...
		foreach ( $dirlist as $name => $details ) {
			if ( 'f' === $details['type'] ) {
				zc_opcache_invalidate( $path . $name, true );
