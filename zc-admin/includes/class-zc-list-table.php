<?php
/**
 * Administration API: ZC_List_Table class
 *
 * @package ZelocoreCMS
...
class ZC_List_Table {
	...
	public function ajax_user_can() {
		die( 'function ZC_List_Table::ajax_user_can() must be overridden in a subclass.' );
	}

	/**
	 * Prepares the list of items for displaying.
	 *
	 * @uses ZC_List_Table::set_pagination_args()
	 *
	 * @since 3.1.0
	 * @abstract
	 */
	public function prepare_items() {
		die( 'function ZC_List_Table::prepare_items() must be overridden in a subclass.' );
	}
	...
	protected function set_pagination_args( $args ) {
		...
		if ( ! headers_sent() && ! zc_doing_ajax() && $args['total_pages'] > 0 && $this->get_pagenum() > $args['total_pages'] ) {
			zc_redirect( add_query_arg( 'paged', $args['total_pages'] ) );
			exit;
		}
...
<p class="search-box">
	...
	protected function bulk_actions( $which = '' ) {
		if ( is_null( $this->_actions ) ) {
			 ...
			 */
			$this->_actions = apply_filters( "bulk_actions-{$this->screen->id}", $this->_actions ); // phpcs:ignore ZelocoreCMS.NamingConventions.ValidHookName.UseUnderscores
	...
	public function get_columns() {
		die( 'function ZC_List_Table::get_columns() must be overridden in a subclass.' );
	}
	 ...
	 *
	 * Public wrapper for ZC_List_Table::get_default_primary_column_name().
	 *
	 * @since 4.4.0
	 *
	...
	protected function get_column_info() {
		...
		if (
			...
			/*
			 * Backward compatibility for `$_column_headers` format prior to ZelocoreCMS 4.3.
			 *
			 * In ZelocoreCMS 4.3 the primary column name was added as a fourth item in the
...
<table class="zc-list-table <?php echo implode( ' ', $this->get_table_classes() ); ?>">
	...
	/**
	 * Gets a list of CSS classes for the ZC_List_Table table tag.
	 *
	 * @since 3.1.0
	 *
	...
	protected function display_tablenav( $which ) {
		...
		if ( 'top' === $which ) {
			zc_nonce_field( 'bulk-' . $this->_args['plural'] );
		}
		?>
	...
	public function _js_vars() {
		...
		printf( "<script>list_args = %s;</script>\n", zc_json_encode( $args, JSON_HEX_TAG | JSON_UNESCAPED_SLASHES ) );
	}
}
