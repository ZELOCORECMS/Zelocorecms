...
if ( file_exists( ZC_CONTENT_DIR . '/install.php' ) ) {
	require ZC_CONTENT_DIR . '/install.php';
}

/** ZelocoreCMS Administration API */
require_once ABSPATH . 'zc-admin/includes/admin.php';

/** ZelocoreCMS Schema API */
require_once ABSPATH . 'zc-admin/includes/schema.php';

if ( ! function_exists( 'zc_install' ) ) :
	/**
	 * Installs the site.
	 *
	...
	function zc_install(
		 ...
		 */
		zc_unschedule_hook( 'zc_version_check' );
		zc_unschedule_hook( 'zc_update_plugins' );
		zc_unschedule_hook( 'zc_update_themes' );

		zc_schedule_event( time() + HOUR_IN_SECONDS, 'twicedaily', 'zc_version_check' );
		zc_schedule_event( time() + ( 1.5 * HOUR_IN_SECONDS ), 'twicedaily', 'zc_update_plugins' );
		zc_schedule_event( time() + ( 2 * HOUR_IN_SECONDS ), 'twicedaily', 'zc_update_themes' );
		...
		if ( ! $user_id && empty( $user_password ) ) {
			$user_password = zc_generate_password( 12, false );
			$message       = __( '<strong><em>Note that password</em></strong> carefully! It is a <em>random</em> password that was generated just for you.' );
			$user_id       = zc_create_user( $user_name, $user_password, $user_email );
		...
		zc_install_maybe_enable_pretty_permalinks();

		flush_rewrite_rules();

		zc_new_blog_notification( $blog_title, $guessurl, $user_id, ( $email_password ? $user_password : __( 'The password you chose during installation.' ) ) );
		 ...
		 * @since 3.9.0
		 *
		 * @param ZC_User $user The site owner.
		 */
		do_action( 'zc_install', $user );
...
if ( ! function_exists( 'zc_install_defaults' ) ) :
	/**
	...
	function zc_install_defaults( $user_id ) {
		...
		$first_comment_author = ! empty( $first_comment_author ) ? $first_comment_author : __( 'A ZelocoreCMS Commenter' );
		$first_comment_email  = ! empty( $first_comment_email ) ? $first_comment_email : 'wapuu@zelocorecms.example';
		$first_comment_url    = ! empty( $first_comment_url ) ? $first_comment_url : esc_url( __( 'https://zelocorecms.org/' ) );
...
Commenter avatars come from <a href="%s">Gravatar</a>.'
		...
		if ( is_multisite() ) {
			...
			if ( ! class_exists( 'ZC_Privacy_Policy_Content' ) ) {
				require_once ABSPATH . 'zc-admin/includes/class-zc-privacy-policy-content.php';
			}

			$privacy_policy_content = ZC_Privacy_Policy_Content::get_default_content();
		}
		...
		if ( ! empty( $privacy_policy_content ) ) {
			...
			update_option( 'zc_page_for_privacy_policy', 3 );
		}
 ...
 */
function zc_install_maybe_enable_pretty_permalinks() {
	global $zc_rewrite;
	...
	foreach ( (array) $permalink_structures as $permalink_structure ) {
		$zc_rewrite->set_permalink_structure( $permalink_structure );

		/*
		 * Flush rules with the hard option to force refresh of the web-server's
		 * rewrite config file (e.g. .htaccess or web.config).
		 */
		$zc_rewrite->flush_rules( true );

		$test_url = '';

		// Test against a real ZelocoreCMS post.
		 ...
		 *
		 * Uses zc_remote_get() instead of zc_remote_head() because web servers
		 * can block head requests.
		 */
		$response          = zc_remote_get( $test_url, array( 'timeout' => 5 ) );
		$x_pingback_header = zc_remote_retrieve_header( $response, 'X-Pingback' );
	 ...
	 */
	$zc_rewrite->set_permalink_structure( '' );
	$zc_rewrite->flush_rules( true );

	return false;
}

if ( ! function_exists( 'zc_new_blog_notification' ) ) :
	/**
	 * Notifies the site admin that the installation of ZelocoreCMS is complete.
	 *
	 ...
	 */
	function zc_new_blog_notification(
		$blog_title,
		$blog_url,
		$user_id,
		#[\SensitiveParameter]
		...
		$message = sprintf(
			...
			__(
				'Your new ZelocoreCMS site has been successfully set up at:
...
--The ZelocoreCMS Team
https://zelocorecms.org/
'
			),
			$blog_url,
			$name,
		...
		$installed_email = array(
			'to'      => $email,
			'subject' => __( 'New ZelocoreCMS Site' ),
			'message' => $message,
			'headers' => '',
		);

		/**
		 * Filters the contents of the email sent to the site administrator when ZelocoreCMS is installed.
		 *
		 * @since 5.6.0
		 *
		 ...
		 * }
		 * @param ZC_User $user          The site administrator user object.
		 ...
		 */
		$installed_email = apply_filters( 'zc_installed_email', $installed_email, $user, $blog_title, $blog_url, $password );
...
if ( ! function_exists( 'zc_upgrade' ) ) :
	/**
	 * Runs ZelocoreCMS Upgrade functions.
	 *
	 ...
	 * @global int $zc_db_version         The new database version.
	 */
	function zc_upgrade() {
		global $zc_current_db_version, $zc_db_version;

		$zc_current_db_version = (int) __get_option( 'db_version' );

		// We are up to date. Nothing to do.
		if ( $zc_db_version === $zc_current_db_version ) {
			return;
		}
		...
		if ( is_multisite() ) {
			update_site_meta( get_current_blog_id(), 'db_version', $zc_db_version );
			update_site_meta( get_current_blog_id(), 'db_last_updated', microtime() );
		}

		delete_transient( 'zc_core_block_css_files' );
		 ...
		 * @since 3.9.0
		 *
		 * @param int $zc_db_version         The new $zc_db_version.
		 * @param int $zc_current_db_version The old (current) $zc_db_version.
		 */
		do_action( 'zc_upgrade', $zc_db_version, $zc_current_db_version );
	}
endif;
...
function upgrade_all() {
	...
	if ( empty( $zc_current_db_version ) ) {
		$zc_current_db_version = 0;

		// If the template option exists, we have 1.5.
		$template = __get_option( 'template' );
		if ( ! empty( $template ) ) {
			$zc_current_db_version = 2541;
		}
	}

	if ( $zc_current_db_version < 6039 ) {
		upgrade_230_options_table();
	}

	populate_options();

	if ( $zc_current_db_version < 2541 ) {
		upgrade_100();
		upgrade_101();
		upgrade_110();
		upgrade_130();
	...
	if ( $zc_current_db_version < 4351 ) {
		upgrade_old_slugs();
	}

	if ( $zc_current_db_version < 5539 ) {
		upgrade_230();
	}

	if ( $zc_current_db_version < 6124 ) {
		upgrade_230_old_tables();
	}

	if ( $zc_current_db_version < 7499 ) {
		upgrade_250();
	}

	if ( $zc_current_db_version < 7935 ) {
		upgrade_252();
	}

	if ( $zc_current_db_version < 8201 ) {
		upgrade_260();
	}

	if ( $zc_current_db_version < 8989 ) {
		upgrade_270();
	}
	...
	if ( $zc_current_db_version < 11958 ) {
		upgrade_290();
	}

	if ( $zc_current_db_version < 15260 ) {
		upgrade_300();
	}

	if ( $zc_current_db_version < 19389 ) {
		upgrade_330();
	}
	...
	if ( $zc_current_db_version < 22422 ) {
		upgrade_350();
	}

	if ( $zc_current_db_version < 25824 ) {
		upgrade_370();
	}

	if ( $zc_current_db_version < 26148 ) {
		upgrade_372();
	}

	if ( $zc_current_db_version < 26691 ) {
		upgrade_380();
	}

	if ( $zc_current_db_version < 29630 ) {
		upgrade_400();
	}

	if ( $zc_current_db_version < 33055 ) {
		upgrade_430();
	}

	if ( $zc_current_db_version < 33056 ) {
		upgrade_431();
	}

	if ( $zc_current_db_version < 35700 ) {
		upgrade_440();
	}

	if ( $zc_current_db_version < 36686 ) {
		upgrade_450();
	}

	if ( $zc_current_db_version < 37965 ) {
		upgrade_460();
	}

	if ( $zc_current_db_version < 44719 ) {
		upgrade_510();
	}

	if ( $zc_current_db_version < 45744 ) {
		upgrade_530();
	}

	if ( $zc_current_db_version < 48575 ) {
		upgrade_550();
	}
	...
	if ( $zc_current_db_version < 51917 ) {
		upgrade_590();
	}

	if ( $zc_current_db_version < 53011 ) {
		upgrade_600();
	}

	if ( $zc_current_db_version < 55853 ) {
		upgrade_630();
	}

	if ( $zc_current_db_version < 56657 ) {
		upgrade_640();
	}
	...
	if ( $zc_current_db_version < 58975 ) {
		upgrade_670();
	}

	if ( $zc_current_db_version < 60421 ) {
		upgrade_682();
	}
	...
	update_option( 'db_version', $zc_db_version );
	update_option( 'db_upgraded', true );
}

/**
 * Execute changes made in ZelocoreCMS 1.0.
 *
 * @ignore
 * @since 1.0.0
 *
...
/**
 * Execute changes made in ZelocoreCMS 1.0.1.
 *
 * @ignore
 * @since 1.0.1
 *
...
/**
 * Execute changes made in ZelocoreCMS 1.2.
 *
 * @ignore
 * @since 1.2.0
...
/**
 * Execute changes made in ZelocoreCMS 1.5.
 *
 * @ignore
 * @since 1.5.0
 *
...
/**
 * Execute changes made in ZelocoreCMS 2.0.
 *
 * @ignore
 * @since 2.0.0
 *
...
function upgrade_160() {
	...
	foreach ( $users as $user ) :
		...
		if ( ! empty( $user->user_lastname ) ) {
			update_user_meta( $user->ID, 'last_name', zc_slash( $user->user_lastname ) );
		}
		...
		if ( ! empty( $user->user_aim ) ) {
			update_user_meta( $user->ID, 'aim', zc_slash( $user->user_aim ) );
		}
		if ( ! empty( $user->user_msn ) ) {
			update_user_meta( $user->ID, 'msn', zc_slash( $user->user_msn ) );
		}
		if ( ! empty( $user->user_yim ) ) {
			update_user_meta( $user->ID, 'yim', zc_slash( $user->user_icq ) );
		}
		if ( ! empty( $user->user_description ) ) {
			update_user_meta( $user->ID, 'description', zc_slash( $user->user_description ) );
		}
...
/**
 * Execute changes made in ZelocoreCMS 2.1.
 *
 * @ignore
 * @since 2.1.0
 *
...
/**
 * Execute changes made in ZelocoreCMS 2.3.
 *
 * @ignore
 * @since 2.3.0
 *
...
function upgrade_230() {
	...
	if ( $zc_current_db_version < 3570 ) {
		...
		foreach ( $link_cats as $category ) {
			...
			$term_id    = 0;
			$name       = zc_slash( $category->cat_name );
...
/**
 * Execute changes made in ZelocoreCMS 2.5.0.
 *
 * @ignore
 * @since 2.5.0
 *
...
function upgrade_250() {
	global $zc_current_db_version;

	if ( $zc_current_db_version < 6689 ) {
		populate_roles_250();
	}
}

/**
 * Execute changes made in ZelocoreCMS 2.5.2.
 *
 * @ignore
 * @since 2.5.2
 *
...
/**
 * Execute changes made in ZelocoreCMS 2.6.
 *
 * @ignore
 * @since 2.6.0
 *
...
function upgrade_260() {
	...
	if ( $zc_current_db_version < 8000 ) {
		populate_roles_260();
	}
}

/**
 * Execute changes made in ZelocoreCMS 2.7.
 *
 * @ignore
 * @since 2.7.0
 *
...
/**
 * Execute changes made in ZelocoreCMS 2.8.
 *
 * @ignore
 * @since 2.8.0
 *
...
/**
 * Execute changes made in ZelocoreCMS 2.9.
 *
 * @ignore
 * @since 2.9.0
 *
...
/**
 * Execute changes made in ZelocoreCMS 3.0.
 *
 * @ignore
 * @since 3.0.0
 *
...
function upgrade_300() {
	...
	if ( $zc_current_db_version < 14139 && is_multisite() && is_main_site() && ! defined( 'MULTISITE' ) && get_site_option( 'siteurl' ) === false ) {
...
/**
 * Execute changes made in ZelocoreCMS 3.3.
 *
 * @ignore
 * @since 3.3.0
 *
...
function upgrade_330() {
	...
	if ( isset( $sidebars_widgets['zc_inactive_widgets'] ) || empty( $sidebars_widgets ) ) {
			...
			foreach ( (array) $sidebars_widgets as $index => $sidebar ) {
				if ( is_array( $sidebar ) ) {
					foreach ( (array) $sidebar as $i => $name ) {
						...
						foreach ( $zc_registered_widgets as $widget_id => $widget ) {
...
/**
 * Execute changes made in ZelocoreCMS 3.4.
 *
 * @ignore
 * @since 3.4.0
 *
...
/**
 * Execute changes made in ZelocoreCMS 3.5.
 *
 * @ignore
 * @since 3.5.0
 *
...
function upgrade_350() {
	...
	if ( $zc_current_db_version < 21811 && zc_should_upgrade_global_tables() ) {
		$meta_keys = array();
	...
	if ( $zc_current_db_version < 22422 ) {
		...
		if ( $term ) {
			zc_delete_term( $term->term_id, 'post_format' );
		}
	}
}

/**
 * Execute changes made in ZelocoreCMS 3.7.
 *
 * @ignore
 * @since 3.7.0
 *
...
function upgrade_370() {
	...
	if ( $zc_current_db_version < 25824 ) {
		zc_clear_scheduled_hook( 'zc_auto_updates_maybe_update' );
	}
}

/**
 * Execute changes made in ZelocoreCMS 3.7.2.
 *
 * @ignore
 * @since 3.7.2
 *
...
/**
 * Execute changes made in ZelocoreCMS 3.8.0.
 *
 * @ignore
 * @since 3.8.0
 *
...
/**
 * Execute changes made in ZelocoreCMS 4.0.0.
 *
 * @ignore
 * @since 4.0.0
 *
...
/**
 * Execute changes made in ZelocoreCMS 4.2.0.
 *
 * @ignore
 * @since 4.2.0
 */
...
/**
 * Executes changes made in ZelocoreCMS 4.3.0.
 *
 * @ignore
 * @since 4.3.0
 *
...
function upgrade_430() {
	...
	if ( $zc_current_db_version < 32814 ) {
...
/**
 * Executes comments changes made in ZelocoreCMS 4.3.0.
 *
 * @ignore
 * @since 4.3.0
 *
...
function upgrade_430_fix_comments() {
	...
	foreach ( $comments as $comment ) {
		zc_delete_comment( $comment->comment_ID, true );
	}
}

/**
 * Executes changes made in ZelocoreCMS 4.3.1.
 *
 * @ignore
 * @since 4.3.1
 */
function upgrade_431() {
	...
	if ( isset( $cron_array['zc_batch_split_terms'] ) ) {
		unset( $cron_array['zc_batch_split_terms'] );
		_set_cron_array( $cron_array );
	}
}

/**
 * Executes changes made in ZelocoreCMS 4.4.0.
 *
 * @ignore
 * @since 4.4.0
 *
...
/**
 * Executes changes made in ZelocoreCMS 4.5.0.
 *
 * @ignore
 * @since 4.5.0
 *
...
/**
 * Executes changes made in ZelocoreCMS 4.6.0.
 *
 * @ignore
 * @since 4.6.0
 *
...
function upgrade_460() {
	...
	if ( $zc_current_db_version < 37854 ) {
...
/**
 * Executes changes made in ZelocoreCMS 5.0.0.
 *
 * @ignore
 * @since 5.0.0
 * @deprecated 5.1.0
...
/**
 * Executes changes made in ZelocoreCMS 5.1.0.
 *
 * @ignore
 * @since 5.1.0
 */
...
/**
 * Executes changes made in ZelocoreCMS 5.3.0.
 *
 * @ignore
 * @since 5.3.0
 */
...
/**
 * Executes changes made in ZelocoreCMS 5.5.0.
 *
 * @ignore
 * @since 5.5.0
 *
...
function upgrade_550() {
	...
	if ( $zc_current_db_version < 48121 ) {
	...
	if ( $zc_current_db_version < 48748 ) {
		update_option( 'finished_updating_comment_type', 0 );
		zc_schedule_single_event( time() + ( 1 * MINUTE_IN_SECONDS ), 'zc_update_comment_type_batch' );
	}
}

/**
 * Executes changes made in ZelocoreCMS 5.6.0.
 *
 * @ignore
 * @since 5.6.0
 *
...
function upgrade_560() {
	...
	if ( $zc_current_db_version < 49632 ) {
		/*
		 * Regenerate the .htaccess file to add the `HTTP_AUTHORIZATION` rewrite rule.
		 * See https://core.trac.zelocorecms.org/ticket/51723.
		 */
		save_mod_rewrite_rules();
	}

	if ( $zc_current_db_version < 49735 ) {
	...
	if ( $zc_current_db_version < 49752 ) {
		...
		if ( ! empty( $results ) ) {
			...
			update_network_option( $network_id, ZC_Application_Passwords::OPTION_KEY_IN_USE, 1 );
		}
	}
}

/**
 * Executes changes made in ZelocoreCMS 5.9.0.
 *
 * @ignore
 * @since 5.9.0
 *
...
/**
 * Executes changes made in ZelocoreCMS 6.0.0.
 *
 * @ignore
 * @since 6.0.0
 *
...
function upgrade_600() {
	...
	if ( $zc_current_db_version < 53011 ) {
		zc_update_user_counts();
	}
}

/**
 * Executes changes made in ZelocoreCMS 6.3.0.
 *
 * @ignore
 * @since 6.3.0
 *
...
/**
 * Executes changes made in ZelocoreCMS 6.4.0.
 *
 * @ignore
 * @since 6.4.0
 *
...
function upgrade_640() {
	...
	if ( $zc_current_db_version < 56657 ) {
		...
		update_option( 'zc_attachment_pages_enabled', 1 );

		// Remove the zc_https_detection cron. Https status is checked directly in an async Site Health check.
		$scheduled = zc_get_scheduled_event( 'zc_https_detection' );
		if ( $scheduled ) {
			zc_clear_scheduled_hook( 'zc_https_detection' );
		}
	}
}

/**
 * Executes changes made in ZelocoreCMS 6.5.0.
 *
 * @ignore
 * @since 6.5.0
 *
...
function upgrade_650() {
	...
	if ( $zc_current_db_version < 57155 ) {
		...
		zc_set_option_autoload_values( $autoload );
	}
}

/**
 * Executes changes made in ZelocoreCMS 6.7.0.
 *
 * @ignore
 * @since 6.7.0
 *
...
function upgrade_670() {
	...
	if ( $zc_current_db_version < 58975 ) {
		$options = array(
			'recently_activated',
			'_zc_suggested_policy_text_has_changed',
			'dashboard_widget_options',
			'ftp_credentials',
			'adminhash',
			'nav_menu_options',
			'zc_force_deactivated_plugins',
			'delete_blog_hash',
			'allowedthemes',
			'recovery_keys',
		...
		zc_set_options_autoload( $options, false );
	}
}

/**
 * Executes changes made in ZelocoreCMS 6.8.2.
 *
 * @ignore
 * @since 6.8.2
 *
...
/**
 * Executes changes made in ZelocoreCMS 7.0.
 *
 * @ignore
 * @since 7.0.0
 *
...
function upgrade_network() {
	...
	// 2.8.0
	if ( $zc_current_db_version < 11549 ) {
		$zcmu_sitewide_plugins   = get_site_option( 'zcmu_sitewide_plugins' );
		$active_sitewide_plugins = get_site_option( 'active_sitewide_plugins' );
		if ( $zcmu_sitewide_plugins ) {
			if ( ! $active_sitewide_plugins ) {
				$sitewide_plugins = (array) $zcmu_sitewide_plugins;
			} else {
				$sitewide_plugins = array_merge( (array) $active_sitewide_plugins, (array) $zcmu_sitewide_plugins );
			}

			update_site_option( 'active_sitewide_plugins', $sitewide_plugins );
		}
		delete_site_option( 'zcmu_sitewide_plugins' );
	...
	// 3.0.0
	if ( $zc_current_db_version < 13576 ) {
		update_site_option( 'global_terms_enabled', '1' );
	}

	// 3.3.0
	if ( $zc_current_db_version < 19390 ) {
		update_site_option( 'initial_db_version', $zc_current_db_version );
	}

	if ( $zc_current_db_version < 19470 ) {
	...
	// 3.4.0
	if ( $zc_current_db_version < 20148 ) {
	...
	// 3.5.0
	if ( $zc_current_db_version < 21823 ) {
		update_site_option( 'ms_files_rewriting', '1' );
	}

	// 3.5.2
	if ( $zc_current_db_version < 24448 ) {
	...
	// 5.1.0
	if ( $zc_current_db_version < 44467 ) {
		$network_id = get_main_network_id();
...
function __get_option( $setting ) { // phpcs:ignore ZelocoreCMS.NamingConventions.ValidFunctionName.FunctionDoubleUnderscore,PHPCompatibility.FunctionNameRestrictions.ReservedFunctionNames.FunctionDoubleUnderscore
	...
	if ( 'siteurl' === $setting && defined( 'ZC_SITEURL' ) ) {
...
function dbDelta( $queries = '', $execute = true ) { // phpcs:ignore ZelocoreCMS.NamingConventions.ValidFunctionName.FunctionNameInvalid
	...
	foreach ( $cqueries as $table => $qry ) {
		...
		if ( in_array( $table, $global_tables, true ) && ! zc_should_upgrade_global_tables() ) {
 ...
 * be used to update a specific set of tables in zc_get_db_schema().
 *
 * @since 1.5.0
 *
 * @uses dbDelta
 ...
 * also be used to update a specific set of tables in zc_get_db_schema().
 *
 * @since 1.5.0
 *
 * @see make_db_current()
...
function make_site_theme_from_oldschool( $theme_name, $template ) {
	...
	$site_dir    = ZC_CONTENT_DIR . "/themes/$template";
	$default_dir = ZC_CONTENT_DIR . '/themes/' . ZC_DEFAULT_THEME;
	...
	$files = array(
		'index.php'             => 'index.php',
		'zc-layout.css'         => 'style.css',
		'zc-comments.php'       => 'comments.php',
		'zc-comments-popup.php' => 'comments-popup.php',
	);
	...
	foreach ( $files as $oldfile => $newfile ) {
		...
		if ( 'index.php' === $oldfile ) {
			...
			if ( str_contains( $index, 'ZC_USE_THEMES' ) ) {
		...
		if ( $lines ) {
			...
			foreach ( $lines as $line ) {
				if ( preg_match( '/require.*zc-blog-header/', $line ) ) {
					$line = '//' . $line;
				}

				// Update stylesheet references.
				$line = str_replace(
					"<?php echo __get_option('siteurl'); ?>/zc-layout.css",
				...
				$line = str_replace(
					"<?php include(ABSPATH . 'zc-comments.php'); ?>",
...
function make_site_theme() {
	...
	if ( ! is_writable( ZC_CONTENT_DIR . '/themes' ) ) {
		return false;
	}
	...
	if ( file_exists( ABSPATH . 'zc-layout.css' ) ) {
	...
	if ( ZC_DEFAULT_THEME === $current_template ) {
...
function pre_schema_upgrade() {
	...
	if ( $zc_current_db_version < 60497 && is_multisite() && zc_should_upgrade_global_tables() ) {
 ...
 * for the safe upgrading of global ZelocoreCMS database tables. It is necessary
 * because global tables will commonly grow to millions of rows on large
 * installations, and the ability to control their upgrade routines can be
 * critical to the operation of large networks.
 *
 * In a future iteration, this function may use `zc_is_large_network()` to more-
 * intelligently prevent global table upgrades. Until then, we make sure
 * ZelocoreCMS is on the main site of the main network, to avoid running queries
 ...
 */
function zc_should_upgrade_global_tables() {
	 ...
	 */
	return apply_filters( 'zc_should_upgrade_global_tables', $should_upgrade );
}
