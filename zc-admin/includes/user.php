 ...
 * @since 2.0.0
 *
 * @return int|ZC_Error ZC_Error or User ID.
 */
function add_user() {
	return edit_user();
}
 ...
 * @return int|ZC_Error User ID of the updated user or ZC_Error on failure.
 */
function edit_user( $user_id = 0 ) {
	...
	if ( $user_id ) {
		...
		$user->user_login = zc_slash( $userdata->user_login );
	} else {
		$update = false;
	}

	if ( ! $update && isset( $_POST['user_login'] ) ) {
		$user->user_login = sanitize_user( zc_unslash( $_POST['user_login'] ), true );
	}
	...
	if ( isset( $_POST['role'] ) && current_user_can( 'promote_users' ) && ( ! $user_id || current_user_can( 'promote_user', $user_id ) ) ) {
		...
		if ( ! empty( $new_role ) && empty( $editable_roles[ $new_role ] ) ) {
			zc_die( __( 'Sorry, you are not allowed to give users that role.' ), 403 );
		}

		$potential_role = $zc_roles->role_objects[ $new_role ] ?? false;
	...
	if ( isset( $_POST['email'] ) ) {
		$user->user_email = sanitize_text_field( zc_unslash( $_POST['email'] ) );
	}
	if ( isset( $_POST['url'] ) ) {
		if ( empty( $_POST['url'] ) || 'http://' === $_POST['url'] ) {
			...
			$protocols      = implode( '|', array_map( 'preg_quote', zc_allowed_protocols() ) );
	...
	foreach ( zc_get_user_contact_methods( $user ) as $method => $name ) {
	...
	if ( isset( $_POST['locale'] ) ) {
		...
		if ( 'site-default' === $locale ) {
			...
			if ( current_user_can( 'install_languages' ) && zc_can_install_language_pack() ) {
				if ( ! zc_download_language_pack( $locale ) ) {
					$locale = '';
				}
			} else {
				$locale = '';
	...
	if ( str_contains( zc_unslash( $pass1 ), '\\' ) ) {
	 ...
	 * @since 2.8.0
	 *
	 * @param ZC_Error $errors ZC_Error object (passed by reference).
	...
	if ( $update ) {
		$user_id = zc_update_user( $user );
	} else {
		$user_id = zc_insert_user( $user );
		 ...
		 * @since 4.4.0
		 *
		 * @param int|ZC_Error $user_id ID of the newly created user or ZC_Error on failure.
		 * @param string       $notify  Type of notification that should happen. See
		 *                              zc_send_new_user_notifications() for more information.
		 */
 ...
 * list of roles in the $zc_roles object so that plugins can remove
...
function zc_delete_user( $id, $reassign = null ) {
	 ...
	 * @param ZC_User  $user     ZC_User object of the user to delete.
	 */
	 ...
	 * the database depending on whether `zc_delete_user()` or `zcmu_delete_user()`
	 * was called.
	 *
	 * @since 2.9.0
	 * @since 5.5.0 Added the `$user` parameter.
	 ...
	 * @param int|null $reassign ID of the user to reassign posts and links to.
	 *                           Default null, for no reassignment.
	 * @param ZC_User  $user     ZC_User object of the deleted user.
	 */
 ...
 *
 * @param int     $user_ID  User ID.
 * @param ZC_User $old_data The user object before the update.
 */
 ...
 * @since 2.7.0
 *
 * @param ZC_User $user User data object.
 */
 ...
 * }
 * @param ZC_User $user The user authorizing the application.
 * @return true|ZC_Error True if the request is valid, a ZC_Error object contains errors if not.
 */
function zc_is_authorize_application_password_request_valid( $request, $user ) {
	$error = new ZC_Error();

	if ( isset( $request['success_url'] ) ) {
		$validated_success_url = zc_is_authorize_application_redirect_url_valid( $request['success_url'] );
		if ( is_zc_error( $validated_success_url ) ) {
			$error->add(
	...
	if ( isset( $request['reject_url'] ) ) {
		$validated_reject_url = zc_is_authorize_application_redirect_url_valid( $request['reject_url'] );
		if ( is_zc_error( $validated_reject_url ) ) {
			$error->add(
	...
	if ( ! empty( $request['app_id'] ) && ! zc_is_uuid( $request['app_id'] ) ) {
		$error->add(
			'invalid_app_id',
	 ...
	 * @param ZC_User  $user    The user authorizing the application.
	 */
	do_action( 'zc_authorize_application_password_request_errors', $error, $request, $user );
 ...
 * @return true|ZC_Error True if the redirect URL is valid, a ZC_Error object otherwise.
 */
function zc_is_authorize_application_redirect_url_valid( $url ) {
	 ...
	 * @param string   $url The redirect URL to be validated.
	 */
	$invalid_protocols = apply_filters( 'zc_authorize_application_redirect_url_invalid_protocols', $bad_protocols, $url );
	$invalid_protocols = array_map( 'strtolower', $invalid_protocols );

	$scheme   = zc_parse_url( $url, PHP_URL_SCHEME );
	$host     = zc_parse_url( $url, PHP_URL_HOST );
	$is_local = 'local' === zc_get_environment_type();
