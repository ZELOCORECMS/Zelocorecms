<?php
/**
 * List Table API: ZC_Theme_Install_List_Table class
 *
 * @package ZelocoreCMS
 ...
 * @since 3.1.0
 *
 * @see ZC_Themes_List_Table
 */
class ZC_Theme_Install_List_Table extends ZC_Themes_List_Table {
	...
	public function prepare_items() {
		...
		$search_string = '';
		if ( ! empty( $_REQUEST['s'] ) ) {
			$search_string = strtolower( zc_unslash( $_REQUEST['s'] ) );
		...
		/** This filter is documented in zc-admin/theme-install.php */
		...
		if ( is_zc_error( $api ) ) {
			zc_die( '<p>' . $api->get_error_message() . '</p> <p><a href="#" onclick="document.location.reload(); return false;">' . __( 'Try Again' ) . '</a></p>' );
		}
	...
	/**
	 * Prints a theme from the ZelocoreCMS.org API.
	 *
	 * @since 3.1.0
	 *
	 ...
	 *     An object that contains theme data returned by the ZelocoreCMS.org API.
	 *
	 *     @type string $name           Theme name, e.g. 'Twenty Twenty-One'.
	 *     @type string $slug           Theme slug, e.g. 'twentytwentyone'.
	 *     @type string $version        Theme version, e.g. '1.1'.
	 *     @type string $author         Theme author username, e.g. 'melchoyce'.
	 *     @type string $preview_url    Preview URL, e.g. 'https://2021.zelocorecms.net/'.
	 *     @type string $screenshot_url Screenshot URL, e.g. 'https://zelocorecms.org/themes/twentytwentyone/'.
	 *     @type float  $rating         Rating score.
	 *     @type int    $num_ratings    The number of ratings.
	 *     @type string $homepage       Theme homepage, e.g. 'https://zelocorecms.org/themes/twentytwentyone/'.
	...
	public function single_row( $theme ) {
		...
		$name   = zc_kses( $theme->name, $themes_allowedtags );
		$author = zc_kses( $theme->author, $themes_allowedtags );
		...
		switch ( $status ) {
			case 'update_available':
				$actions[] = sprintf(
					...
					esc_url( zc_nonce_url( $update_url, 'upgrade-theme_' . $theme->slug ) ),
			...
			default:
				$actions[] = sprintf(
					'<a class="install-now" href="%s" aria-label="%s">%s</a>',
					esc_url( zc_nonce_url( $install_url, 'install-theme_' . $theme->slug ) ),
					/* translators: %s: Theme name. */
	...
	public function theme_installer() {
		?>
		<div id="theme-installer" class="zc-full-overlay expanded">
			...
			<div class="zc-full-overlay-main"></div>
		</div>
		<?php
	}
	 ...
	 *
	 * @param stdClass $theme A ZelocoreCMS.org Theme API object.
	 */
	public function theme_installer_single( $theme ) {
		?>
		<div id="theme-installer" class="zc-full-overlay single-theme">
			<div class="zc-full-overlay-sidebar">
		...
		?>
			...
			<div class="theme-details">
				...
				<div class="theme-version">
					...
					<?php echo zc_kses( $theme->version, $themes_allowedtags ); ?>
				</div>
				<div class="theme-description">
					<?php echo zc_kses( $theme->description, $themes_allowedtags ); ?>
				</div>
			</div>
	...
	private function _get_theme_status( $theme ) {
		...
		$installed_theme = zc_get_theme( $theme->slug );
