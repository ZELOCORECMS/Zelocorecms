...
final class ZC_Screen {
	 ...
	 *
	 * @param string|ZC_Screen $hook_name Optional. The hook name (also known as the hook suffix) used to determine the screen.
	 *                                    Defaults to the current $hook_suffix global.
	 * @return ZC_Screen Screen object.
	 */
	public static function get( $hook_name = '' ) {
		if ( $hook_name instanceof ZC_Screen ) {
			return $hook_name;
		}
		...
		if ( ! $post_type && $hook_name ) {
			...
			if ( defined( 'ZC_NETWORK_ADMIN' ) && ZC_NETWORK_ADMIN ) {
				$in_admin = 'network';
			} elseif ( defined( 'ZC_USER_ADMIN' ) && ZC_USER_ADMIN ) {
				$in_admin = 'user';
			} else {
				$in_admin = 'site';
			}
				...
				case 'post':
					...
					if ( $post_id ) {
						...
						if ( $post ) {
							...
							/** This filter is documented in zc-admin/post.php */
	...
	public function set_current_screen() {
		 ...
		 * @since 3.0.0
		 *
		 * @param ZC_Screen $current_screen Current ZC_Screen object.
		 */
	 ...
	 * @since 3.3.0
	 *
	 * @param ZC_Screen $screen A screen object.
	...
	public function set_screen_reader_content( $content = array() ) {
		...
		$content  = zc_parse_args( $content, $defaults );
	...
	public function render_screen_meta() {
		 ...
		 * @param ZC_Screen $screen          Current ZC_Screen instance.
		 */
	...
	public function show_screen_options() {
		...
		$show_screen = ! empty( $zc_meta_boxes[ $this->id ] ) || $columns || $this->get_option( 'per_page' );
		 ...
		 * @param ZC_Screen $screen          ZC_Screen object.
		 */
	...
	public function render_screen_options( $options = array() ) {
		...
		if ( 'widgets' !== $this->base ) {
			...
			$form_end   = "\n" . zc_nonce_field( 'screen-options-nonce', 'screenoptionnonce', false, false ) . "\n</form>\n";
		}
	...
	public function render_meta_boxes_preferences() {
		...
		if ( ! isset( $zc_meta_boxes[ $this->id ] ) ) {
			return;
		}
		?>
		...
		if ( 'dashboard' === $this->id && has_action( 'welcome_panel' ) && current_user_can( 'edit_theme_options' ) ) {
			if ( isset( $_GET['welcome'] ) ) {
				...
				if ( 2 === $welcome_checked && zc_get_current_user()->user_email !== get_option( 'admin_email' ) ) {
					$welcome_checked = false;
				}
			}
			echo '<label for="zc_welcome_panel-hide">';
			echo '<input type="checkbox" id="zc_welcome_panel-hide"' . checked( (bool) $welcome_checked, true, false ) . ' />';
	...
	public function render_per_page_options() {
		...
		if ( 'edit_comments_per_page' === $option ) {
			...
			/** This filter is documented in zc-admin/includes/class-zc-terms-list-table.php */
		...
		<legend><?php _e( 'Pagination' ); ?></legend>
			<?php if ( $per_page_label ) : ?>
				...
				<input type="number" step="1" min="1" max="999" class="screen-per-page small-text" name="zc_screen_options[value]"
					id="<?php echo esc_attr( $option ); ?>"
					value="<?php echo esc_attr( $per_page ); ?>" />
			<?php endif; ?>
				<input type="hidden" name="zc_screen_options[option]" value="<?php echo esc_attr( $option ); ?>" />
		</fieldset>
		<?php
	}
