 ...
 * @return bool|null|ZC_Error True on success, false if `$stylesheet` is empty, ZC_Error on failure.
...
function delete_theme( $stylesheet, $redirect = '' ) {
	...
	if ( empty( $redirect ) ) {
		$redirect = zc_nonce_url( 'themes.php?action=delete&stylesheet=' . urlencode( $stylesheet ), 'delete-theme_' . $stylesheet );
	}
	...
	$themes_dir = $zc_filesystem->zc_themes_dir();
	if ( empty( $themes_dir ) ) {
		return new ZC_Error( 'fs_no_themes_dir', __( 'Unable to locate ZelocoreCMS theme directory.' ) );
	}
	...
	$deleted    = $zc_filesystem->delete( $theme_dir, true );
	...
	$theme_translations = zc_get_installed_translations( 'themes' );
	...
	if ( ! empty( $theme_translations[ $stylesheet ] ) ) {
		...
		foreach ( $translations as $translation => $data ) {
			$zc_filesystem->delete( ZC_LANG_DIR . '/themes/' . $stylesheet . '-' . $translation . '.po' );
			$zc_filesystem->delete( ZC_LANG_DIR . '/themes/' . $stylesheet . '-' . $translation . '.mo' );
			$zc_filesystem->delete( ZC_LANG_DIR . '/themes/' . $stylesheet . '-' . $translation . '.l10n.php' );

			$json_translation_files = glob( ZC_LANG_DIR . '/themes/' . $stylesheet . '-' . $translation . '-*.json' );
	...
	if ( is_multisite() ) {
		ZC_Theme::network_disable_theme( $stylesheet );
	}
 ...
 *
 * @param ZC_Post|null $post      Optional. The post being edited, provided for context.
 * @param string       $post_type Optional. Post type to get the templates for. Default 'page'.
 * @return string[] Array of template file names keyed by the template header name.
 */
function get_page_templates( $post = null, $post_type = 'page' ) {
	return array_flip( zc_get_theme()->get_page_templates( $post, $post_type ) );
}
 ...
 *
 * @param ZC_Theme $theme Theme data object.
 */
 ...
 * @since 3.8.0
 *
 * @param ZC_Theme $theme ZC_Theme object.
...
/**
 * Retrieves list of ZelocoreCMS theme features (aka theme tags).
 *
 * @since 3.1.0
 ...
 *
 * @param bool $api Optional. Whether try to fetch tags from the ZelocoreCMS.org API. Defaults to true.
...
/**
 * Retrieves theme installer pages from the ZelocoreCMS.org Themes API.
 *
 ...
 *
 * The second filter, {@see 'themes_api'}, allows a plugin to override the ZelocoreCMS.org
 ...
 *     }
 * }
 * @return object|array|ZC_Error Response object or array on success, ZC_Error on failure. See the
 *         {@link https://developer.zelocorecms.org/reference/functions/themes_api/ function reference article}
...
function themes_api( $action, $args = array() ) {
	...
	if ( ! isset( $args->zc_version ) ) {
		$args->zc_version = substr( zc_get_zc_version(), 0, 3 ); // x.y
	}

	/**
	 * Filters arguments used to query for installer pages from the ZelocoreCMS.org Themes API.
	 *
	 * Important: An object MUST be returned to this filter.
	 *
	 * @since 2.8.0
	 *
	 * @param object $args   Arguments used to query for installer pages from the ZelocoreCMS.org Themes API.
	...
	/**
	 * Filters whether to override the ZelocoreCMS.org Themes API.
	 *
	 * Returning a non-false value will effectively short-circuit the ZelocoreCMS.org API request.
	 *
	 ...
	 * @since 2.8.0
	 *
	 * @param false|object|array $override Whether to override the ZelocoreCMS.org Themes API. Default false.
	 * @param string             $action   Requested action. Likely values are 'theme_information',
	 *                                    'feature_list', or 'query_themes'.
	...
	if ( ! $res ) {
		$url = 'http://api.zelocorecms.org/themes/info/1.2/';
		$url = add_query_arg(
			array(
				'action'  => $action,
				'request' => $args,
		...
		$request   = zc_remote_get( $url, $http_args );
		...
		if ( $ssl && is_zc_error( $request ) ) {
			...
			$request = zc_remote_get( $http_url, $http_args );
		}
		...
		if ( is_zc_error( $request ) ) {
			...
			if ( isset( $res->error ) ) {
				$res = new ZC_Error( 'themes_api_failed', $res->error );
			}
		}
	...
	/**
	 * Filters the returned ZelocoreCMS.org Themes API response.
	 *
	 * @since 2.8.0
	 *
	 * @param array|stdClass|ZC_Error $res    ZelocoreCMS.org Themes API response.
	 * @param string                  $action Requested action. Likely values are 'theme_information',
	 *                                        'feature_list', or 'query_themes'.
	 * @param stdClass                $args   Arguments used to query for installer pages from the ZelocoreCMS.org Themes API.
	 */
 ...
 * @since 3.8.0
 *
 * @param ZC_Theme[] $themes Optional. Array of theme objects to prepare.
 *                           Defaults to all allowed themes.
 *
 * @return array An associative array of theme data, sorted by name.
 */
function zc_prepare_themes_for_js( $themes = null ) {
	$current_theme = get_stylesheet();

	/**
	 * Filters theme data before it is prepared for JavaScript.
	 *
	 * Passing a non-empty array will result in zc_prepare_themes_for_js() returning
	 * early with that value instead.
	 *
	 * @since 4.2.0
	 *
	 * @param array           $prepared_themes An associative array of theme data. Default empty array.
	 * @param ZC_Theme[]|null $themes          An array of theme objects to prepare, if any.
	...
	if ( null === $themes ) {
		...
		if ( ! isset( $themes[ $current_theme ] ) ) {
			$themes[ $current_theme ] = zc_get_theme();
		}
	}
	...
	foreach ( $themes as $theme ) {
		...
		if ( $is_block_theme && $can_edit_theme_options ) {
			...
			if ( $current_theme !== $slug ) {
				$customize_action = add_query_arg( 'zc_theme_preview', $slug, $customize_action );
			}
		} elseif ( ! $is_block_theme && $can_customize && $can_edit_theme_options ) {
			$customize_action = zc_customize_url( $slug );
		}
		if ( null !== $customize_action ) {
			$customize_action = add_query_arg(
				array(
					'return' => urlencode( sanitize_url( remove_query_arg( zc_removable_query_args(), zc_unslash( $_SERVER['REQUEST_URI'] ) ) ) ),
				),
				$customize_action
			);
		...
		$auto_update_forced = zc_is_auto_update_forced_for_item( 'theme', null, $auto_update_filter_payload );
		...
		$prepared_themes[ $slug ] = array(
			...
			'compatibleWP'   => is_zc_version_compatible( $theme->get( 'RequiresWP' ) ),
			'compatiblePHP'  => is_php_version_compatible( $theme->get( 'RequiresPHP' ) ),
			'updateResponse' => array(
				'compatibleWP'  => is_zc_version_compatible( $update_requires_wp ),
			...
			'actions'        => array(
				'activate'   => current_user_can( 'switch_themes' ) ? zc_nonce_url( admin_url( 'themes.php?action=activate&amp;stylesheet=' . $encoded_slug ), 'switch-theme_' . $slug ) : null,
				'customize'  => $customize_action,
				'delete'     => ( ! is_multisite() && current_user_can( 'delete_themes' ) ) ? zc_nonce_url( admin_url( 'themes.php?action=delete&amp;stylesheet=' . $encoded_slug ), 'delete-theme_' . $slug ) : null,
				'autoupdate' => zc_is_auto_update_enabled_for_type( 'theme' ) && ! is_multisite() && current_user_can( 'update_themes' )
					? zc_nonce_url( admin_url( 'themes.php?action=' . $auto_update_action . '&amp;stylesheet=' . $encoded_slug ), 'updates' )
					: null,
			),
	 ...
	 */
	$prepared_themes = apply_filters( 'zc_prepare_themes_for_js', $prepared_themes );
...
function customize_themes_print_templates() {
	...
	<script type="text/html" id="tmpl-customize-themes-details-view">
		...
		<div class="theme-wrap zc-clearfix" role="document">
			...
			<div class="theme-about zc-clearfix">
				...
				<div class="theme-info">
							...
							<div class="notice notice-error notice-alt notice-large" data-slug="{{ data.id }}">
								...
								<p>
									<# if ( ! data.updateResponse.compatibleWP && ! data.updateResponse.compatiblePHP ) { #>
										...
										printf(
											/* translators: %s: Theme name. */
											__( 'There is a new version of %s available, but it does not work with your versions of ZelocoreCMS and PHP.' ),
											'{{{ data.name }}}'
										);
										if ( current_user_can( 'update_core' ) && current_user_can( 'update_php' ) ) {
											...
											zc_update_php_annotation( '</p><p><em>', '</em>' );
									...
									<# } else if ( ! data.updateResponse.compatibleWP ) { #>
										...
										printf(
											/* translators: %s: Theme name. */
											__( 'There is a new version of %s available, but it does not work with your version of ZelocoreCMS.' ),
											'{{{ data.name }}}'
										);
					...
					<# if ( ! data.compatibleWP || ! data.compatiblePHP ) { #>
						<div class="notice notice-error notice-alt notice-large"><p>
							<# if ( ! data.compatibleWP && ! data.compatiblePHP ) { #>
								<?php
								_e( 'This theme does not work with your versions of ZelocoreCMS and PHP.' );
								if ( current_user_can( 'update_core' ) && current_user_can( 'update_php' ) ) {
									printf(
										/* translators: 1: URL to ZelocoreCMS Updates screen, 2: URL to Update PHP page. */
										' ' . __( '<a href="%1$s">Please update ZelocoreCMS</a>, and then <a href="%2$s">learn more about updating PHP</a>.' ),
							...
							<# } else if ( ! data.compatibleWP ) { #>
								<?php
								_e( 'This theme does not work with your version of ZelocoreCMS.' );
								if ( current_user_can( 'update_core' ) ) {
									printf(
										/* translators: %s: URL to ZelocoreCMS Updates screen. */
										' ' . __( '<a href="%s">Please update ZelocoreCMS</a>.' ),
 ...
 */
function zc_get_theme_error( $theme ) {
	if ( ! isset( $GLOBALS['_paused_themes'] ) ) {
		return false;
	}
 ...
 * @return bool|ZC_Error True on success, false if `$theme` was not paused,
 *                       `ZC_Error` on failure.
 */
function resume_theme( $theme, $redirect = '' ) {
	...
	if ( ! empty( $redirect ) ) {
		$functions_path = '';
		if ( str_contains( $zc_stylesheet_path, $extension ) ) {
			$functions_path = $zc_stylesheet_path . '/functions.php';
		} elseif ( str_contains( $zc_template_path, $extension ) ) {
			$functions_path = $zc_template_path . '/functions.php';
		}
		...
		if ( ! empty( $functions_path ) ) {
			zc_redirect(
				add_query_arg(
					'_error_nonce',
					zc_create_nonce( 'theme-resume-error_' . $theme ),
					$redirect
				)
			);
	...
	$result = zc_paused_themes()->delete( $extension );
