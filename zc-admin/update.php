...
if ( isset( $_GET['action'] ) ) {
	...
	if ( 'update-selected' === $action ) {
		if ( ! current_user_can( 'update_plugins' ) ) {
			zc_die( __( 'Sorry, you are not allowed to update plugins for this site.' ) );
		}
		...
		if ( ! isset( $_GET['failure'] ) && ! isset( $_GET['success'] ) ) {
			zc_redirect( admin_url( 'update.php?action=activate-plugin&failure=true&plugin=' . urlencode( $plugin ) . '&_wpnonce=' . $_GET['_wpnonce'] ) );
			activate_plugin( $plugin, '', ! empty( $_GET['networkwide'] ), true );
			zc_redirect( admin_url( 'update.php?action=activate-plugin&success=true&plugin=' . urlencode( $plugin ) . '&_wpnonce=' . $_GET['_wpnonce'] ) );
			die();
		}
		...
		require_once ABSPATH . 'zc-admin/includes/plugin-install.php'; // For plugins_api().
		...
		if ( isset( $_FILES['pluginzip']['name'] ) && ! str_ends_with( strtolower( $_FILES['pluginzip']['name'] ), '.zip' ) ) {
			zc_die( __( 'Only .zip archives may be uploaded.' ) );
		}
		...
		// Make sure the attachment still exists, or File_Upload_Upgrader will call zc_die()
		...
		zc_redirect( self_admin_url( 'plugin-install.php' ) );
		exit;
	} elseif ( 'upgrade-theme' === $action ) {

		if ( ! current_user_can( 'update_themes' ) ) {
			zc_die( __( 'Sorry, you are not allowed to update themes for this site.' ) );
		}
		...
		require_once ABSPATH . 'zc-admin/includes/class-zc-upgrader.php'; // For themes_api().
		...
		zc_redirect( self_admin_url( 'theme-install.php' ) );
		exit;
	} else {
		/**
		 ...
		 * provided in the request for zc-admin/update.php. Can be used to
		 * provide custom update functionality for themes and plugins.
		 *
		 * @since 2.8.0
		 */
		do_action( "update-custom_{$action}" ); // phpcs:ignore ZelocoreCMS.NamingConventions.ValidHookName.UseUnderscores
	}
}
