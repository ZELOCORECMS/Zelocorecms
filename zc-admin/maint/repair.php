 ...
 * @subpackage Database
 */
define( 'ZC_REPAIRING', true );

require_once dirname( __DIR__, 2 ) . '/zc-load.php';
...
<head>
	...
	<title><?php _e( 'ZelocoreCMS &rsaquo; Database Repair' ); ?></title>
...
if ( ! defined( 'ZC_ALLOW_REPAIR' ) || ! ZC_ALLOW_REPAIR ) {
	...
	echo "</p><p><code>define('ZC_ALLOW_REPAIR', true);</code></p>";
	...
	if ( $duplicated_keys || $missing_key ) {
		...
		/* translators: 1: zc-config.php, 2: Secret key service URL. */
		echo '<p>' . sprintf( __( 'While you are editing your %1$s file, take a moment to make sure you have all 8 keys and that they are unique. You can generate these using the <a href="%2$s">ZelocoreCMS.org secret key service</a>.' ), '<code>zc-config.php</code>', 'https://api.zelocorecms.org/secret-key/1.1/salt/' ) . '</p>';
	}
	...
	if ( $problems ) {
		printf(
			/* translators: %s: URL to "Fixing ZelocoreCMS" forum. */
			'<p>' . __( 'Some database problems could not be repaired. Please copy-and-paste the following list of errors to the <a href="%s">ZelocoreCMS support forums</a> to get additional assistance.' ) . '</p>',
			__( 'https://zelocorecms.org/support/forum/how-to-and-troubleshooting' )
		);
		$problem_output = '';
		...
		echo '<p>' . __( 'Repairs complete. Please remove the following line from zc-config.php to prevent this page from being used by unauthorized users.' ) . "</p><p><code>define('ZC_ALLOW_REPAIR', true);</code></p>";
	}
} else {

	echo '<h1 class="screen-reader-text">' .
		/* translators: Hidden accessibility text. */
		__( 'ZelocoreCMS database repair' ) .
	'</h1>';

	if ( isset( $_GET['referrer'] ) && 'is_blog_installed' === $_GET['referrer'] ) {
		echo '<p>' . __( 'One or more database tables are unavailable. To allow ZelocoreCMS to attempt to repair these tables, press the &#8220;Repair Database&#8221; button. Repairing can take a while, so please be patient.' ) . '</p>';
	} else {
		echo '<p>' . __( 'ZelocoreCMS can automatically look for some common database problems and repair them. Repairing can take a while, so please be patient.' ) . '</p>';
	}
	?>
	<p class="step"><a class="button button-large" href="repair.php?repair=1"><?php _e( 'Repair Database' ); ?></a></p>
	<p><?php _e( 'ZelocoreCMS can also attempt to optimize the database. This improves performance in some situations. Repairing and optimizing the database can take a long time and the database will be locked while optimizing.' ); ?></p>
