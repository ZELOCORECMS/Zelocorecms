...
if ( 'on' === $widgets_access ) {
	add_filter( 'admin_body_class', 'zc_widgets_access_body_class' );
} else {
	zc_enqueue_script( 'admin-widgets' );
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://zelocorecms.org/documentation/article/appearance-widgets-screen-classic-editor/">Documentation on Widgets</a>' ) . '</p>' .
...
if ( empty( $sidebars_widgets ) ) {
	$sidebars_widgets = zc_get_widget_defaults();
}

foreach ( $sidebars_widgets as $sidebar_id => $widgets ) {
	if ( 'zc_inactive_widgets' === $sidebar_id ) {
		continue;
	}
...
if ( isset( $_POST['savewidget'] ) || isset( $_POST['removewidget'] ) ) {
	...
	if ( isset( $_POST['removewidget'] ) && $_POST['removewidget'] ) {
		...
		if ( ! in_array( $widget_id, $sidebar, true ) ) {
			zc_redirect( admin_url( 'widgets.php?error=0' ) );
			exit;
		}
	...
	zc_redirect( admin_url( 'widgets.php?message=0' ) );
	exit;
}
...
if ( isset( $_GET['editwidget'] ) && $_GET['editwidget'] ) {
	...
	if ( isset( $_GET['addnew'] ) ) {
		...
		$keys    = array_keys( $zc_registered_sidebars );
		$sidebar = reset( $keys );

		if ( isset( $_GET['base'] ) && isset( $_GET['num'] ) ) { // Multi-widget.
			// Copy minimal info from an existing instance of this widget to a new instance.
			foreach ( $zc_registered_widget_controls as $control ) {
				if ( $_GET['base'] === $control['id_base'] ) {
					...
					$zc_registered_widget_controls[ $control['id'] ] = $control;
					break;
				}
			}
		}
	...
	if ( isset( $zc_registered_widget_controls[ $widget_id ] ) && ! isset( $control ) ) {
		$control          = $zc_registered_widget_controls[ $widget_id ];
		$control_callback = $control['callback'];
	} elseif ( ! isset( $zc_registered_widget_controls[ $widget_id ] ) && isset( $zc_registered_widgets[ $widget_id ] ) ) {
		$name = esc_html( strip_tags( $zc_registered_widgets[ $widget_id ]['name'] ) );
	}
	...
	if ( ! isset( $sidebar ) ) {
		$sidebar = $_GET['sidebar'] ?? 'zc_inactive_widgets';
	}
	...
	<?php
	foreach ( $zc_registered_sidebars as $sidebar_name => $sidebar_data ) {
		...
		if ( 'zc_inactive_widgets' === $sidebar_name || str_starts_with( $sidebar_name, 'orphaned_widgets' ) ) {
			echo '&nbsp;';
		} else {
	...
	<div class="widget-control-actions">
		...
		<div class="right-actions">
			...
			<?php zc_nonce_field( "save-delete-widget-$widget_id" ); ?>
		</div>
	</div>
...
$nonce = zc_create_nonce( 'widgets-access' );
?>
...
foreach ( $zc_registered_sidebars as $sidebar => $registered_sidebar ) {
	if ( str_contains( $registered_sidebar['class'], 'inactive-sidebar' ) || str_starts_with( $sidebar, 'orphaned_widgets' ) ) {
		...
		$is_inactive_widgets = 'zc_inactive_widgets' === $registered_sidebar['id'];
		?>
		<div class="<?php echo esc_attr( $wrap_class ); ?>">
			<div class="widget-holder inactive">
				<?php zc_list_widget_controls( $registered_sidebar['id'], $registered_sidebar['name'] ); ?>
				...
				<div class="remove-inactive-widgets">
					<form method="post">
						<p>
							...
							if ( empty( $sidebars_widgets['zc_inactive_widgets'] ) ) {
								$attributes['disabled'] = '';
							}
						...
						<?php zc_nonce_field( 'remove-inactive-widgets', '_wpnonce_remove_inactive_widgets' ); ?>
					</form>
				</div>
				<?php } ?>
			</div>
...
<div id="widgets-right" class="zc-clearfix<?php echo $single_sidebar_class; ?>">
...
foreach ( $theme_sidebars as $sidebar => $registered_sidebar ) {
	...
	<div class="<?php echo esc_attr( $wrap_class ); ?>">
		...
		zc_list_widget_controls( $sidebar, $registered_sidebar['name'] );
		?>
	</div>
	<?php
...
<form method="post">
<?php zc_nonce_field( 'save-sidebar-widgets', '_wpnonce_widgets', false ); ?>
</form>
<br class="clear" />
</div>
