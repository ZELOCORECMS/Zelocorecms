...
if ( is_multisite() ) {
	add_filter( 'zcmu_signup_user_notification_email', 'admin_created_user_email' );
}
...
if ( isset( $_REQUEST['action'] ) && 'adduser' === $_REQUEST['action'] ) {
	...
	$user_details = null;
	$user_email   = zc_unslash( $_REQUEST['email'] );
	...
	if ( str_contains( $user_email, '@' ) ) {
		...
		if ( current_user_can( 'manage_network_users' ) ) {
			...
			zc_redirect( add_query_arg( array( 'update' => 'enter_email' ), 'user-new.php' ) );
			die();
		}
	}

	if ( ! $user_details ) {
		zc_redirect( add_query_arg( array( 'update' => 'does_not_exist' ), 'user-new.php' ) );
		die();
	}
	...
	if ( array_key_exists( $blog_id, get_blogs_of_user( $user_id ) ) ) {
		...
		if ( isset( $_POST['noconfirmation'] ) && current_user_can( 'manage_network_users' ) ) {
			...
			zc_ensure_editable_role( $_REQUEST['role'] );
			...
			$newuser_key = zc_generate_password( 20, false );
			add_option(
...
%4$s'
			...
			$new_user_email['message'] = sprintf(
				...
				get_option( 'blogname' ),
				home_url(),
				zc_specialchars_decode( translate_user_role( $role['name'] ) ),
	...
	if ( ! is_multisite() ) {
		...
		$new_user_email = zc_unslash( $_REQUEST['email'] );
		$user_details   = zcmu_validate_user_signup( $_REQUEST['user_login'], $new_user_email );

		if ( is_zc_error( $user_details['errors'] ) && $user_details['errors']->has_errors() ) {
			$add_user_errors = $user_details['errors'];
		} else {
			/** This filter is documented in zc-includes/user.php */
			$new_user_login = apply_filters( 'pre_user_login', sanitize_user( zc_unslash( $_REQUEST['user_login'] ), true ) );

			if ( isset( $_POST['noconfirmation'] ) && current_user_can( 'manage_network_users' ) ) {
				add_filter( 'zcmu_signup_user_notification', '__return_false' );  // Disable confirmation email.
				add_filter( 'zcmu_welcome_user_notification', '__return_false' ); // Disable welcome email.
			}
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://zelocorecms.org/documentation/article/users-add-new-screen/">Documentation on Adding New Users</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://zelocorecms.org/support/forums/">Support forums</a>' ) . '</p>'
);

zc_enqueue_script( 'zc-ajax-response' );
...
if ( is_multisite() && current_user_can( 'promote_users' ) && ! zc_is_large_network( 'users' )
	...
	zc_enqueue_script( 'user-suggest' );
}
...
if ( isset( $_GET['update'] ) ) {
	...
	if ( is_multisite() ) {
		...
		if ( ( isset( $_GET['user_id'] ) ) ) {
			...
			if ( $user_id_new ) {
				$edit_link = esc_url( add_query_arg( 'zc_http_referer', urlencode( zc_unslash( $_SERVER['REQUEST_URI'] ) ), get_edit_user_link( $user_id_new ) ) );
			}
		}
		...
		switch ( $_GET['update'] ) {
			...
			case 'addexisting':
				...
				break;
			case 'could_not_add':
				$add_user_errors = new ZC_Error( 'could_not_add', __( 'That user could not be added to this site.' ) );
				break;
			case 'created_could_not_add':
				$add_user_errors = new ZC_Error( 'created_could_not_add', __( 'User has been created, but could not be added to this site.' ) );
				break;
			case 'does_not_exist':
				$add_user_errors = new ZC_Error( 'does_not_exist', __( 'The requested user does not exist.' ) );
				break;
			case 'enter_email':
				$add_user_errors = new ZC_Error( 'enter_email', __( 'Please enter a valid email address.' ) );
				break;
		}
	} else {
		if ( 'add' === $_GET['update'] ) {
...
<?php
if ( isset( $add_user_errors ) && is_zc_error( $add_user_errors ) ) :
	$error_message = '';
...
<input name="action" type="hidden" value="adduser" />
	<?php zc_nonce_field( 'add-user', '_wpnonce_add-user' ); ?>

<table class="form-table" role="presentation">
	<tr class="form-field form-required">
		<th scope="row"><label for="adduser-email"><?php echo esc_html( $label ); ?></label></th>
		<td><input name="email" type="<?php echo esc_attr( $type ); ?>" id="adduser-email" class="zc-suggest-user" value="" /></td>
	</tr>
	<tr class="form-field">
		<th scope="row"><label for="adduser-role"><?php _e( 'Role' ); ?></label></th>
		<td><select name="role" id="adduser-role">
			<?php zc_dropdown_roles( get_option( 'default_role' ) ); ?>
			</select>
		</td>
	</tr>
...
<form method="post" name="createuser" id="createuser" class="validate" novalidate="novalidate"
	<?php
	/** This action is documented in zc-admin/user-new.php */
	do_action( 'user_new_form_tag' );
	?>
>
<input name="action" type="hidden" value="createuser" />
	<?php zc_nonce_field( 'create-user', '_wpnonce_create-user' ); ?>
	<?php
	// Load up the passed data, else set to a default.
	$creating = isset( $_POST['createuser'] );

	$new_user_login             = $creating && isset( $_POST['user_login'] ) ? zc_unslash( $_POST['user_login'] ) : '';
	$new_user_firstname         = $creating && isset( $_POST['first_name'] ) ? zc_unslash( $_POST['first_name'] ) : '';
	$new_user_lastname          = $creating && isset( $_POST['last_name'] ) ? zc_unslash( $_POST['last_name'] ) : '';
	$new_user_email             = $creating && isset( $_POST['email'] ) ? zc_unslash( $_POST['email'] ) : '';
	$new_user_uri               = $creating && isset( $_POST['url'] ) ? zc_unslash( $_POST['url'] ) : '';
	$new_user_role              = $creating && isset( $_POST['role'] ) ? zc_unslash( $_POST['role'] ) : '';
	$new_user_send_notification = $creating && ! isset( $_POST['send_user_notification'] ) ? false : true;
	$new_user_ignore_pass       = $creating && isset( $_POST['noconfirmation'] ) ? zc_unslash( $_POST['noconfirmation'] ) : '';
...
<table class="form-table" role="presentation">
	...
	<tr class="form-field form-required user-pass1-wrap">
		...
		<td>
			...
			<button type="button" class="button zc-generate-pw hide-if-no-js"><?php _e( 'Generate password' ); ?></button>
			<div class="zc-pwd">
				<?php $initial_password = zc_generate_password( 24 ); ?>
				...
				<button type="button" class="button zc-hide-pw hide-if-no-js" data-toggle="0" aria-label="<?php esc_attr_e( 'Hide password' ); ?>">
	...
	<tr class="form-field">
		...
		<td><select name="role" id="role">
			...
			zc_dropdown_roles( $new_user_role );
			?>
			</select>
		</td>
	</tr>
