...
$doaction = $zc_list_table->current_action();
...
if ( $doaction ) {
	...
	foreach ( $comment_ids as $comment_id ) { // Check the permissions on each.
		...
		switch ( $doaction ) {
			case 'approve':
				zc_set_comment_status( $comment_id, 'approve' );
				++$approved;
				break;
			case 'unapprove':
				zc_set_comment_status( $comment_id, 'hold' );
				++$unapproved;
				break;
			case 'spam':
				...
				break;
			case 'unspam':
				zc_unspam_comment( $comment_id );
				++$unspammed;
				break;
			case 'trash':
				zc_trash_comment( $comment_id );
				++$trashed;
				break;
			case 'untrash':
				zc_untrash_comment( $comment_id );
				++$untrashed;
				break;
			case 'delete':
				zc_delete_comment( $comment_id );
				++$deleted;
				break;
		}
	}
	...
	if ( ! in_array( $doaction, array( 'approve', 'unapprove', 'spam', 'unspam', 'trash', 'delete' ), true ) ) {
		...
		$redirect_to = apply_filters( "handle_bulk_actions-{$screen}", $redirect_to, $doaction, $comment_ids ); // phpcs:ignore ZelocoreCMS.NamingConventions.ValidHookName.UseUnderscores
	}
	...
	exit;
} elseif ( ! empty( $_GET['_zc_http_referer'] ) ) {
...
if ( $post_id ) {
	$comments_count      = zc_count_comments( $post_id );
	$draft_or_post_title = zc_html_excerpt( _draft_or_post_title( $post_id ), 50, '&hellip;' );
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://zelocorecms.org/documentation/article/comments-screen/">Documentation on Comments</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://zelocorecms.org/documentation/article/understand-comment-spam/">Documentation on Comment Spam</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://zelocorecms.org/documentation/article/keyboard-shortcuts-classic-editor/#keyboard-shortcuts-for-comments">Documentation on Keyboard Shortcuts</a>' ) . '</p>' .
...
if ( $post_id ) {
	printf(
		...
		sprintf(
			...
			zc_html_excerpt( _draft_or_post_title( $post_id ), 50, '&hellip;' )
		)
	);
} else {
	_e( 'Comments' );
	...
	if ( $approved > 0 || $deleted > 0 || $trashed > 0 || $untrashed > 0 || $spammed > 0 || $unspammed > 0 || $same > 0 ) {
		...
		if ( $spammed > 0 ) {
			...
			$messages[] = sprintf(
				...
				' <a href="%1$s">%2$s</a><br />',
				esc_url( zc_nonce_url( "edit-comments.php?doaction=undo&action=unspam&ids=$ids", 'bulk-comments' ) ),
				__( 'Undo' )
			);
		}
		...
		if ( $trashed > 0 ) {
			...
			$messages[] = sprintf(
				...
				' <a href="%1$s">%2$s</a><br />',
				esc_url( zc_nonce_url( "edit-comments.php?doaction=undo&action=untrash&ids=$ids", 'bulk-comments' ) ),
				__( 'Undo' )
			);
		}
...
<?php $zc_list_table->search_box( __( 'Search Comments' ), 'comment' ); ?>
...
<input type="hidden" name="_total" value="<?php echo esc_attr( $zc_list_table->get_pagination_arg( 'total_items' ) ); ?>" />
<input type="hidden" name="_per_page" value="<?php echo esc_attr( $zc_list_table->get_pagination_arg( 'per_page' ) ); ?>" />
<input type="hidden" name="_page" value="<?php echo esc_attr( $zc_list_table->get_pagination_arg( 'page' ) ); ?>" />
...
<?php
zc_comment_reply( '-1', true, 'detail' );
