...
if ( ! current_user_can( 'create_sites' ) ) {
	zc_die( __( 'Sorry, you are not allowed to add sites to this network.' ) );
}
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://developer.zelocorecms.org/advanced-administration/multisite/admin/#network-admin-sites-screen">Documentation on Site Management</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://zelocorecms.org/support/forum/multisite/">Support forums</a>' ) . '</p>'
);
...
if ( isset( $_REQUEST['action'] ) && 'add-site' === $_REQUEST['action'] ) {
	...
	if ( ! is_array( $_POST['blog'] ) ) {
		zc_die( __( 'Cannot create an empty site.' ) );
	}
		...
		if ( in_array( $domain, $subdirectory_reserved_names, true ) ) {
			zc_die(
				sprintf(
					...
					__( 'The following words are reserved for use by ZelocoreCMS functions and cannot be used as site names: %s' ),
	...
	if ( isset( $_POST['WPLANG'] ) ) {
		...
		} elseif ( current_user_can( 'install_languages' ) && zc_can_install_language_pack() ) {
			$language = zc_download_language_pack( zc_unslash( $_POST['WPLANG'] ) );
			if ( $language ) {
	...
	if ( empty( $title ) ) {
		zc_die( __( 'Missing site title.' ) );
	}

	if ( empty( $domain ) ) {
		zc_die( __( 'Missing or invalid site address.' ) );
	}

	if ( isset( $blog['email'] ) && '' === trim( $blog['email'] ) ) {
		zc_die( __( 'Missing email address.' ) );
	}

	$email = sanitize_email( $blog['email'] );
	if ( ! is_email( $email ) ) {
		zc_die( __( 'Invalid email address.' ) );
	}
	...
	if ( ! $user_id ) { // Create a new user with a random password.
		...
		if ( $user_id ) {
			zc_die( __( 'The domain or path entered conflicts with an existing username.' ) );
		}
		$password = zc_generate_password( 12, false );
		$user_id  = zcmu_create_user( $domain, $password, $email );
		if ( false === $user_id ) {
			zc_die( __( 'There was an error creating the user.' ) );
		}
	...
	if ( ! is_zc_error( $id ) ) {
		...
		zcmu_new_site_admin_notification( $id, $user_id );
		zcmu_welcome_notification( $id, $user_id, $password, $title, array( 'public' => 1 ) );
		zc_redirect(
			add_query_arg(
				array(
					'update' => 'added',
...
if ( ! empty( $messages ) ) {
	...
	foreach ( $messages as $msg ) {
		zc_admin_notice( $msg, $notice_args );
	}
}
?>
<p><?php echo zc_required_field_message(); ?></p>
...
<?php zc_nonce_field( 'add-blog', '_wpnonce_add-blog' ); ?>
	<table class="form-table" role="presentation">
		<tr class="form-field form-required">
			<th scope="row">
				<label for="site-address">
					<?php
					_e( 'Site Address (URL)' );
					echo ' ' . zc_required_field_indicator();
					?>
				</label>
			</th>
			<td>
		...
		<tr class="form-field form-required">
			...
			<td><input name="blog[email]" type="email" class="regular-text zc-suggest-user" id="admin-email" data-autocomplete-type="search" data-autocomplete-field="user_email" aria-describedby="site-admin-email" required /></td>
		</tr>
		<tr class="form-field">
