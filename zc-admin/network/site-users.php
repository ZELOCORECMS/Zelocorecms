...
if ( ! current_user_can( 'manage_sites' ) ) {
	zc_die( __( 'Sorry, you are not allowed to edit this site.' ), 403 );
}
...
$referer                = remove_query_arg( 'update', zc_get_referer() );
...
if ( ! $id ) {
	zc_die( __( 'Invalid site ID.' ) );
}

$details = get_site( $id );
if ( ! $details ) {
	zc_die( __( 'The requested site does not exist.' ) );
}

if ( ! can_edit_network( $details->site_id ) ) {
	zc_die( __( 'Sorry, you are not allowed to access this page.' ), 403 );
}
	...
	switch ( $action ) {
		case 'newuser':
			...
			if ( ! is_array( $_POST['user'] ) || empty( $user['username'] ) || empty( $user['email'] ) ) {
				...
				$user_id  = zcmu_create_user( esc_html( strtolower( $user['username'] ) ), $password, esc_html( $user['email'] ) );
		...
		default:
			...
			$referer = apply_filters( 'handle_network_bulk_actions-' . get_current_screen()->id, $referer, $action, $userids, $id ); // phpcs:ignore ZelocoreCMS.NamingConventions.ValidHookName.UseUnderscores

			$update = $action;
			break;
	}

	zc_safe_redirect( add_query_arg( 'update', $update, $referer ) );
	exit;
}
 ...
 */
if ( ! zc_is_large_network( 'users' ) && apply_filters( 'show_network_site_users_add_existing_form', true ) ) {
...
/** This filter is documented in zc-admin/network/site-users.php */
...
<form action="site-users.php?action=adduser" id="adduser" method="post">
	...
	<table class="form-table" role="presentation">
		<tr>
			...
			<td><input type="text" class="regular-text zc-suggest-user" name="newuser" id="newuser" /></td>
		</tr>
		<tr>
			...
			<?php
			switch_to_blog( $id );
			zc_dropdown_roles( get_option( 'default_role' ) );
			restore_current_blog();
			?>
			</select></td>
		</tr>
 ...
 *
 * Note: While ZelocoreCMS is moving towards simplifying labels by removing "New" from "Add New X" labels,
...
<form action="<?php echo esc_url( network_admin_url( 'site-users.php?action=newuser' ) ); ?>" id="newuser" method="post">
	...
	<?php zc_nonce_field( 'add-user', '_wpnonce_add-new-user' ); ?>
