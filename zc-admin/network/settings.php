...
get_current_screen()->add_help_tab(
	array(
		...
		'content' =>
			...
			'<p>' . __( 'You can set the language, and ZelocoreCMS will automatically download and install the translation files (available if your filesystem is writable).' ) . '</p>' .
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://developer.zelocorecms.org/advanced-administration/multisite/admin/settings/">Documentation on Network Settings</a>' ) . '</p>' .
...
if ( $_POST ) {
	...
	if ( ! empty( $_POST['WPLANG'] ) && current_user_can( 'install_languages' ) && zc_can_install_language_pack() ) {
		$language = zc_download_language_pack( $_POST['WPLANG'] );
		if ( $language ) {
	...
	foreach ( $options as $option_name ) {
		...
		$value = zc_unslash( $_POST[ $option_name ] );
	 ...
	 * @since MU (3.0.0)
	 */
	do_action( 'update_zcmu_options' );

	zc_redirect( add_query_arg( 'updated', 'true', network_admin_url( 'settings.php' ) ) );
	exit;
}
...
<div class="wrap">
	...
	<form method="post" action="settings.php" novalidate="novalidate">
		<?php zc_nonce_field( 'siteoptions' ); ?>
			...
			<tr>
				...
				<td>
					...
					if ( $new_admin_email && get_site_option( 'admin_email' ) !== $new_admin_email ) :
						...
						$notice_message .= sprintf(
							' <a href="%1$s">%2$s</a>',
							esc_url( zc_nonce_url( network_admin_url( 'settings.php?dismiss=new_network_admin_email' ), 'dismiss_new_network_admin_email' ) ),
							__( 'Cancel' )
						);
			...
			<tr>
				...
				<td>
					...
					if ( is_subdomain_install() ) {
						...
						printf(
							/* translators: 1: NOBLOGREDIRECT, 2: zc-config.php */
