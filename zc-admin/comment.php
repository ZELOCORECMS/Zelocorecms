...
switch ( $action ) {
	...
	case 'editcomment':
		...
		require ABSPATH . 'zc-admin/edit-form-comment.php';
	...
	case 'spam':
		...
		if ( ! $comment ) {
			zc_redirect( admin_url( 'edit-comments.php?error=1' ) );
			die();
		}

		if ( ! current_user_can( 'edit_comment', $comment->comment_ID ) ) {
			zc_redirect( admin_url( 'edit-comments.php?error=2' ) );
			die();
		}

		// No need to re-approve/re-trash/re-spam a comment.
		if ( str_replace( '1', 'approve', $comment->comment_approved ) === $action ) {
			zc_redirect( admin_url( 'edit-comments.php?same=' . $comment_id ) );
			die();
		}
...
<form action="comment.php" method="get" class="comment-ays-submit">
	...
	case 'unapprovecomment':
		...
		if ( zc_get_referer() && ! $noredir && ! str_contains( zc_get_referer(), 'comment.php' ) ) {
			$redir = zc_get_referer();
		} elseif ( zc_get_original_referer() && ! $noredir ) {
			$redir = zc_get_original_referer();
		...
		switch ( $action ) {
			case 'deletecomment':
				zc_delete_comment( $comment );
				$redir = add_query_arg( array( 'deleted' => '1' ), $redir );
				break;
			case 'trashcomment':
				zc_trash_comment( $comment );
				$redir = add_query_arg(
					array(
						'trashed' => '1',
						'ids'     => $comment_id,
			...
			case 'unspamcomment':
				...
				break;
			case 'approvecomment':
				zc_set_comment_status( $comment, 'approve' );
				$redir = add_query_arg( array( 'approved' => 1 ), $redir );
				break;
			case 'unapprovecomment':
				zc_set_comment_status( $comment, 'hold' );
