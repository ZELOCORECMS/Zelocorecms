 ...
 *
 * Requires zc-admin/includes/revision.php.
 *
 * @package ZelocoreCMS
...
switch ( $action ) {
	case 'restore':
		...
		if ( ! zc_revisions_enabled( $post ) && ! zc_is_post_autosave( $revision ) ) {
		 ...
		 * Because zc_insert_post() and zc_transition_post_status() are called
		 * during the process, plugins can unexpectedly modify $post.
		 */
		$backup_global_post = clone $post;

		zc_restore_post_revision( $revision->ID );
...
zc_enqueue_script( 'revisions' );
zc_localize_script( 'revisions', '_wpRevisionsSettings', zc_prepare_revisions_for_js( $post, $revision_id, $from ) );
...
$revisions_sidebar .= '<p>' . __( '<a href="https://zelocorecms.org/documentation/article/revisions/">Revisions Management</a>' ) . '</p>';
$revisions_sidebar .= '<p>' . __( '<a href="https://zelocorecms.org/support/forums/">Support forums</a>' ) . '</p>';
