...
$menu[4] = array( '', 'read', 'separator1', '', 'zc-menu-separator' );
...
if ( current_user_can( 'edit_posts' ) ) {
	$awaiting_moderation      = zc_count_comments();
...
$_zc_last_object_menu = 25; // The index of the last top-level menu in the object menu group.
...
foreach ( array_merge( $builtin, $post_types ) as $post_type ) {
	...
	$post_type_menu_position = is_int( $post_type_obj->menu_position )
		...
		: ++$_zc_last_object_menu; // If we're to use $_zc_last_object_menu, increment it first.
	$post_type_for_id        = sanitize_html_class( $post_type );

	$menu_icon = 'dashicons-admin-post';
	if ( is_string( $post_type_obj->menu_icon ) ) {
		// Special handling for an empty div.zc-menu-image, data:image/svg+xml, and Dashicons.
...
$menu[59] = array( '', 'read', 'separator2', '', 'zc-menu-separator' );
...
if ( zc_is_block_theme() ) {
	...
	$supports_stylebook = ( current_theme_supports( 'editor-styles' ) || zc_theme_has_theme_json() );
...
$customize_url = add_query_arg( 'return', urlencode( remove_query_arg( zc_removable_query_args(), zc_unslash( $_SERVER['REQUEST_URI'] ) ) ), 'customize.php' );
...
function _add_themes_utility_last() {
	add_submenu_page(
		zc_is_block_theme() ? 'tools.php' : 'themes.php',
...
if ( current_user_can( 'list_users' ) ) {
	$_zc_real_parent_file['profile.php'] = 'users.php'; // Back-compat for plugins adding submenus to profile.php.
...
if ( ! is_multisite() && defined( 'ZC_ALLOW_MULTISITE' ) && ZC_ALLOW_MULTISITE ) {
...
$_zc_last_utility_menu = 80; // The index of the last top-level menu in the utility menu group.

$menu[99] = array( '', 'read', 'separator-last', '', 'zc-menu-separator' );

// Back-compat for old top-levels.
$_zc_real_parent_file['post.php']       = 'edit.php';
$_zc_real_parent_file['post-new.php']   = 'edit.php';
$_zc_real_parent_file['edit-pages.php'] = 'edit.php?post_type=page';
$_zc_real_parent_file['page-new.php']   = 'edit.php?post_type=page';
$_zc_real_parent_file['wpmu-admin.php'] = 'tools.php';
$_zc_real_parent_file['ms-admin.php']   = 'tools.php';
