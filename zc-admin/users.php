...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://zelocorecms.org/documentation/article/users-screen/">Documentation on Managing Users</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://zelocorecms.org/documentation/article/roles-and-capabilities/">Descriptions of Roles and Capabilities</a>' ) . '</p>' .
...
if ( empty( $_REQUEST ) ) {
	$referer = '<input type="hidden" name="zc_http_referer" value="' . esc_attr( zc_unslash( $_SERVER['REQUEST_URI'] ) ) . '" />';
} elseif ( isset( $_REQUEST['zc_http_referer'] ) ) {
	$redirect = remove_query_arg( array( 'zc_http_referer', 'updated', 'delete_count' ), zc_unslash( $_REQUEST['zc_http_referer'] ) );
	$referer  = '<input type="hidden" name="zc_http_referer" value="' . esc_attr( $redirect ) . '" />';
} else {
	$redirect = 'users.php';
	$referer  = '';
}
...
switch ( $zc_list_table->current_action() ) {
	...
	case 'promote':
		...
		if ( ! current_user_can( 'promote_users' ) ) {
			zc_die( __( 'Sorry, you are not allowed to edit this user.' ), 403 );
		}
		...
		foreach ( $user_ids as $id ) {
			...
			if ( $id === $current_user->ID ) {
				if ( '' === $role ) {
					zc_die( __( 'Sorry, you cannot remove your own role.' ), 403 );
				}

				if ( $zc_roles->role_objects[ $role ]->has_cap( 'promote_users' ) || ( is_multisite() && current_user_can( 'manage_network_users' ) ) ) {
					continue;
				}
		...
		zc_redirect( add_query_arg( 'update', $update, $redirect ) );
		exit;

	case 'dodelete':
		if ( is_multisite() ) {
			zc_die( __( 'User deletion is not allowed from this screen.' ), 400 );
		}
		...
		if ( empty( $_REQUEST['delete_option'] ) ) {
			...
			$url = str_replace( '&amp;', '&', zc_nonce_url( $url, 'bulk-users' ) );
			zc_redirect( $url );
			exit;
		}

		if ( ! current_user_can( 'delete_users' ) ) {
			zc_die( __( 'Sorry, you are not allowed to delete users.' ), 403 );
		}
		...
		foreach ( $user_ids as $id ) {
			if ( ! current_user_can( 'delete_user', $id ) ) {
				zc_die( __( 'Sorry, you are not allowed to delete that user.' ), 403 );
			}
			...
			switch ( $_REQUEST['delete_option'] ) {
				case 'delete':
					...
					break;
				case 'reassign':
					zc_delete_user( $id, $_REQUEST['reassign_user'] );
					break;
			}
	...
	case 'resetpassword':
		...
		if ( ! current_user_can( 'edit_users' ) ) {
			$errors = new ZC_Error( 'edit_users', __( 'Sorry, you are not allowed to edit users.' ) );
		}
	...
	case 'delete':
		...
		if ( ! current_user_can( 'delete_users' ) ) {
			$errors = new ZC_Error( 'edit_users', __( 'Sorry, you are not allowed to delete users.' ) );
		}
		...
		<?php zc_nonce_field( 'delete-users' ); ?>
		<?php echo $referer; ?>
		...
		if ( $go_delete ) :
			 ...
			 *
			 * @param ZC_User $current_user ZC_User object for the current user.
			 * @param int[]   $user_ids     Array of IDs for users being deleted.
			 */
	...
	case 'doremove':
		...
		if ( ! is_multisite() ) {
			zc_die( __( 'You cannot remove users.' ), 400 );
		}
		...
		if ( ! current_user_can( 'remove_users' ) ) {
			zc_die( __( 'Sorry, you are not allowed to remove users.' ), 403 );
		}
	...
	case 'remove':
		...
		if ( ! current_user_can( 'remove_users' ) ) {
			$error = new ZC_Error( 'edit_users', __( 'Sorry, you are not allowed to remove users.' ) );
		}
		...
		?>
		<form method="post" name="updateusers" id="updateusers">
		<?php zc_nonce_field( 'remove-users' ); ?>
		<?php echo $referer; ?>
	...
	default:
		if ( ! empty( $_GET['_zc_http_referer'] ) ) {
			zc_redirect( remove_query_arg( array( '_zc_http_referer', '_wpnonce' ), zc_unslash( $_SERVER['REQUEST_URI'] ) ) );
			exit;
		}

		if ( $zc_list_table->current_action() && ! empty( $_REQUEST['users'] ) ) {
			$screen   = get_current_screen()->id;
			$sendback = zc_get_referer();
			$user_ids = array_map( 'intval', (array) $_REQUEST['users'] );

			/** This action is documented in zc-admin/edit.php */
			$sendback = apply_filters( "handle_bulk_actions-{$screen}", $sendback, $zc_list_table->current_action(), $user_ids ); // phpcs:ignore ZelocoreCMS.NamingConventions.ValidHookName.UseUnderscores
					...
					if ( $user_id && current_user_can( 'edit_user', $user_id ) ) {
						$message .= sprintf(
							...
							esc_url(
								add_query_arg(
									'zc_http_referer',
									urlencode( zc_unslash( $_SERVER['REQUEST_URI'] ) ),
		...
		<?php $zc_list_table->search_box( __( 'Search Users' ), 'user' ); ?>
