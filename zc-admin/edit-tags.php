...
if ( ! in_array( $tax->name, get_taxonomies( array( 'show_ui' => true ) ), true ) ) {
	zc_die( __( 'Sorry, you are not allowed to edit terms in this taxonomy.' ) );
}
...
/**
 * $post_type is set when the ZC_Terms_List_Table instance is created.
 *
 * @global string $post_type Global post type.
 */
global $post_type;

$zc_list_table = _get_list_table( 'ZC_Terms_List_Table' );
...
if ( ! $referer ) { // For POST requests.
	$referer = zc_unslash( $_SERVER['REQUEST_URI'] );
}
$referer = remove_query_arg( array( '_zc_http_referer', '_wpnonce', 'error', 'message', 'paged' ), $referer );
switch ( $zc_list_table->current_action() ) {
	...
	case 'add-tag':
		...
		$ret = zc_insert_term( $_POST['tag-name'], $taxonomy, $_POST );
	...
	case 'delete':
		...
		zc_delete_term( $tag_ID, $taxonomy );
	...
	case 'edit':
		...
		if ( ! $term instanceof ZC_Term ) {
			zc_die( __( 'You attempted to edit an item that does not exist. Perhaps it was deleted?' ) );
		}

		zc_redirect( sanitize_url( get_edit_term_link( $term_id, $taxonomy, $post_type ) ) );
		exit;
	...
	case 'editedtag':
		...
		$ret = zc_update_term( $tag_ID, $taxonomy, $_POST );
		...
		break;
	default:
		if ( ! $zc_list_table->current_action() || ! isset( $_REQUEST['delete_tags'] ) ) {
			break;
		}
		check_admin_referer( 'bulk-tags' );
		...
		$location = apply_filters( "handle_bulk_actions-{$screen}", $location, $zc_list_table->current_action(), $tags ); // phpcs:ignore ZelocoreCMS.NamingConventions.ValidHookName.UseUnderscores
		break;
}

if ( ! $location && ! empty( $_REQUEST['_zc_http_referer'] ) ) {
	$location = remove_query_arg( array( '_zc_http_referer', '_wpnonce' ), zc_unslash( $_SERVER['REQUEST_URI'] ) );
}
...
if ( $location ) {
	 ...
	 * @param ZC_Taxonomy $tax      The taxonomy object.
	 */
	zc_redirect( apply_filters( 'redirect_term_location', $location, $tax ) );
	exit;
}
...
zc_enqueue_script( 'admin-tags' );
if ( current_user_can( $tax->cap->edit_terms ) ) {
	zc_enqueue_script( 'inline-edit-tax' );
}
...
if ( 'category' === $taxonomy || 'link_category' === $taxonomy || 'post_tag' === $taxonomy ) {
	...
	if ( 'category' === $taxonomy ) {
		$help .= '<p>' . __( '<a href="https://zelocorecms.org/documentation/article/posts-categories-screen/">Documentation on Categories</a>' ) . '</p>';
	} elseif ( 'link_category' === $taxonomy ) {
		$help .= '<p>' . __( '<a href="https://codex.zelocorecms.org/Links_Link_Categories_Screen">Documentation on Link Categories</a>' ) . '</p>';
	} else {
		$help .= '<p>' . __( '<a href="https://zelocorecms.org/documentation/article/posts-tags-screen/">Documentation on Tags</a>' ) . '</p>';
	}

	$help .= '<p>' . __( '<a href="https://zelocorecms.org/support/forums/">Support forums</a>' ) . '</p>';
...
require_once ABSPATH . 'zc-admin/includes/edit-tag-messages.php';
...
<h1 class="zc-heading-inline"><?php echo esc_html( $title ); ?></h1>
...
if ( isset( $_REQUEST['s'] ) && strlen( $_REQUEST['s'] ) ) {
	...
	printf(
		...
		'<strong>' . esc_html( zc_unslash( $_REQUEST['s'] ) ) . '</strong>'
	);
	echo '</span>';
}
?>
...
<form class="search-form zc-clearfix" method="get">
<input type="hidden" name="taxonomy" value="<?php echo esc_attr( $taxonomy ); ?>" />
<input type="hidden" name="post_type" value="<?php echo esc_attr( $post_type ); ?>" />

<?php $zc_list_table->search_box( $tax->labels->search_items, 'tag' ); ?>
...
if ( $can_edit_terms ) {
	?>
<div id="col-container" class="zc-clearfix">
...
<input type="hidden" name="taxonomy" value="<?php echo esc_attr( $taxonomy ); ?>" />
<input type="hidden" name="post_type" value="<?php echo esc_attr( $post_type ); ?>" />
	<?php zc_nonce_field( 'add-tag', '_wpnonce_add-tag' ); ?>
...
<div class="form-field term-parent-wrap">
	<label for="parent"><?php echo esc_html( $tax->labels->parent_item ); ?></label>
		...
		zc_dropdown_categories( $dropdown_args );
		?>
 ...
 */
do_action( "after-{$taxonomy}-table", $taxonomy );  // phpcs:ignore ZelocoreCMS.NamingConventions.ValidHookName.UseUnderscores
